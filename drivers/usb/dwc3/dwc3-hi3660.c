...
static int is_abb_clk_selected(struct hisi_dwc3_device *hisi_dwc3)
{
	volatile uint32_t scdeepsleeped;
	void __iomem *sctrl_base = hisi_dwc3->sctrl_reg_base;

	scdeepsleeped = readl(sctrl_base + SCTRL_SCDEEPSLEEPED);
	if ((scdeepsleeped & (1 << 30)) == 0)
		return 1;
...
static int usb3_clk_init(struct hisi_dwc3_device *hisi_dwc3)
{
	...
	void __iomem *pmctrl_base = hisi_dwc3->pmctrl_reg_base;
	...
	if (is_abb_clk_selected(hisi_dwc3)) {
		...
		writel(SC_SEL_ABB_BACKUP | (SC_SEL_ABB_BACKUP << CLKDIV_MASK_START),
				pericfg_base + PERI_CRG_CLKDIV21);

		/* close backup clk */
		writel(GT_CLK_ABB_BACKUP, pericfg_base + PERI_CRG_CLK_DIS5);

		temp = readl(pmctrl_base + PMC_PPLL3CTRL1);
		temp |= PPLL3_INT_MOD;
		temp &= ~GT_CLK_PPLL3;
		writel(temp, pmctrl_base + PMC_PPLL3CTRL1);

		temp = readl(pmctrl_base + PMC_PPLL3CTRL0);
		temp &= ~(0xFFF << PPLL3_FBDIV_START);
		temp |= (0x40 << PPLL3_FBDIV_START);
		temp &= ~PPLL3_BP;
		temp |= PPLL3_EN;
		writel(temp, pmctrl_base + PMC_PPLL3CTRL0);
		...
		/* select usbphy clk from pll */
		temp = readl(pctrl_base + PCTRL_PERI_CTRL24);
		temp |= SC_CLK_USB3PHY_3MUX1_SEL;
		writel(temp, pctrl_base + PCTRL_PERI_CTRL24);

		/* open backup clk */
		writel(GT_CLK_ABB_BACKUP, pericfg_base + PERI_CRG_CLK_EN5);
	}
