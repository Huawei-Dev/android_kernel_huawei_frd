...
static int fsa9685_scp_adaptor_support_second_output(void)
{
    int val;
    int ret;
    ret = accp_adapter_reg_read(&val, SCP_SPID);
    if(ret < 0)
    {
        hwlog_err("%s : failed \n ",__func__);
        return 0;
    }
    hwlog_info("%s : SCP_SPID = 0x%x \n ",__func__, val);
    if(SCP_NEED_OPEN_OUTPUT2 == val)
        return 1;
    return 0;
}
static int fsa9685_scp_open_second_output(void)
{
    int val;
    int ret;
    ret = accp_adapter_reg_read(&val, SCP_SCTRL);
    hwlog_info("%s : SCP_SCTRL = 0x%x \n ",__func__, val);
    val &= ~(SCP_OUTPUT2_ENABLE_MASK);
    val |= SCP_OUTPUT2_ENABLE;
    ret |= accp_adapter_reg_write(val, SCP_SCTRL);
    if(ret < 0)
    {
        hwlog_err("%s : failed \n ",__func__);
        return -1;
...
static int fsa9685_scp_init(void)
{
    /*open 5v boost*/
    int ret;
    int val;
    ret = fsa9685_scp_output_mode_enable(1);
    ret |= fsa9685_scp_config_vset_boundary(5700);
    ret |= fsa9685_scp_config_iset_boundary(8000);
    ret |= fsa9685_scp_set_adaptor_voltage(4400);
    ret |= accp_adapter_reg_read(&val, SCP_CTRL_BYTE0);
    hwlog_info("%s : CTRL_BYTE0 = 0x%x \n ",__func__, val);
    ret |= accp_adapter_reg_read(&val, SCP_CTRL_BYTE1);
    hwlog_info("%s : CTRL_BYTE1 = 0x%x \n ",__func__, val);
    ret |= accp_adapter_reg_read(&val, SCP_STATUS_BYTR0);
...
struct smart_charge_ops scp_fsa9685_ops = {
    ...
    .scp_adaptor_support_second_output = fsa9685_scp_adaptor_support_second_output,
    .scp_open_second_output = fsa9685_scp_open_second_output,
