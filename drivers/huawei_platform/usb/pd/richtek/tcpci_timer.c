...
static inline uint64_t rt_get_value(volatile uint64_t*p)
{
	unsigned long flags;
	uint64_t data;
...
static inline void rt_set_value(volatile uint64_t *p, uint64_t data)
{
	unsigned long flags;
...
static inline void rt_clear_bit(int nr, volatile uint64_t *addr)
{
...
static inline void rt_set_bit(int nr, volatile uint64_t *addr)
{
        uint64_t mask = ((uint64_t)1) << nr;
        unsigned long flags;
...
static const uint32_t tcpc_timer_timeout[PD_TIMER_NR] = {
	...
	TIMEOUT_VAL_US(3500),       // PD_PE_VDM_POSTPONE (out of spec)
...
do \
{				\
	rt_set_bit(index, (volatile uint64_t *)&tcpc_dev->timer_tick);	\
...
{
	int i;
	uint64_t mask = rt_get_value((volatile uint64_t *)&tcpc->timer_enable_mask);

	for (i = start; i <= end; i++) {
		if (mask & (((uint64_t)1) << i)) {
			hrtimer_try_to_cancel(&tcpc->tcpc_timer[i]);
			rt_clear_bit(i, (volatile uint64_t *)&tcpc->timer_enable_mask);
		}
	}
}
...
void tcpc_enable_timer(struct tcpc_device *tcpc, uint32_t timer_id)
{
	...
	rt_set_bit(timer_id, (volatile uint64_t *)&tcpc->timer_enable_mask);
...
void tcpc_disable_timer(struct tcpc_device *tcpc_dev, uint32_t timer_id)
{
	...
	if (mask&(((uint64_t)1)<<timer_id)) {
		...
		rt_clear_bit(timer_id, (volatile uint64_t*)&tcpc_dev->timer_enable_mask);
	}
}
...
void tcpc_timer_reset(struct tcpc_device *tcpc_dev)
{
	...
	rt_set_value((volatile uint64_t *)&tcpc_dev->timer_enable_mask, 0);
}
...
static int tcpc_timer_thread(void *param)
{
    ...
    volatile uint64_t *timer_tick = &tcpc_dev->timer_tick;
