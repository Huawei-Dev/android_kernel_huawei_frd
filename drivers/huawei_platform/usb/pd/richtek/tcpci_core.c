				   ...
				   const char *buf, size_t count)
{
	...
	case TCPC_DESC_PD_TEST:
		...
		case 8:
			ret = gpio_get_value(212);	
			pr_info("tcpci_alert_pin=%d\n", ret);
			
			ret = tcpci_get_alert_status(tcpc, &alert_status);
			if (ret)
				return ret;

			if (alert_status != 0)
				pr_info("tcpci_alert_status:0x%04x\r\n", alert_status);

    			tcpci_alert_status_clear(tcpc, alert_status);

			tcpci_get_power_status(tcpc, &power_status);
			pr_info("tcpci_power_status:0x%04x\r\n", power_status);
			break;
#endif 
		default:
			break;
