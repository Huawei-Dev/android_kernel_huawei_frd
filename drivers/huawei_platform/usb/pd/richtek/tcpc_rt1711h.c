...
#include <huawei_platform/misc/rt-regmap.h>
...
RT_REG_DECL(RT1711H_REG_SOFTRESET, 1, RT_VOLATILE, {});
...
{
	int ret = rt1711_i2c_write8(tcpc, RT1711H_REG_SOFTRESET, 1);
...
static int rt1711_init_alert_mask(struct tcpc_device *tcpc)
{
	...
	mask |= TCPC_V10_REG_ALERT_TX_SUCCESS | TCPC_V10_REG_ALERT_TX_DISCARDED
			...
			| TCPC_V10_REG_ALERT_RX_STATUS | TCPC_V10_REG_RX_OVERFLOW; // Need to handle RX overflow
#endif
...
static int rt1711_init_power_status_mask(struct tcpc_device *tcpc)
{
	...
	rt1711_reg_write(chip->client,
		RT1711H_REG_RT_MASK, RT1711H_REG_M_VBUS_80);
#endif

	mask = TCPC_V10_REG_POWER_STATUS_VBUS_PRES;
...
static int rt1711_init_fault_mask(struct tcpc_device *tcpc)
{
	...
	/* OC OV - force enable */
	rt1711_reg_write(chip->client,
		TCPC_V10_REG_FAULT_STATUS_MASK, 0x82);
...
static int rt1711_init_alert(struct tcpc_device *tcpc)
{
	...
	ret = rt1711_write_word(chip->client, TCPC_V10_REG_ALERT_MASK, 0);
	ret = rt1711_write_word(chip->client, TCPC_V10_REG_ALERT, 0xffff);
...
static int rt1711_tcpc_init(struct tcpc_device *tcpc, bool sw_reset)
{
	...
	rt1711_i2c_write8(tcpc, TCPC_V10_REG_ROLE_CTRL,
			TCPC_V10_REG_ROLE_CTRL_RES_SET(0, 0, CC_RD, CC_RD));// DRP = 0, RpVal = 0 (Default), Rd, Rd
	...
	rt1711_i2c_write8(tcpc, RT1711H_REG_PHY_CTRL1, 0xB1); // GoodCRC Discard issue (Retry)
#endif

	// By Patrick, check with designer (change default setting)
	rt1711_i2c_write8(tcpc, RT1711H_REG_PHY_CTRL3, 0x70); // For No-GoodCRC Case (0x70)
...
{
	return rt1711_i2c_write8(tcpc, TCPC_V10_REG_RX_DETECT, enable);
}
	...
	enum tcpm_transmit_type type, uint16_t header, const uint32_t *data)
{
	...
	if (type < TCPC_TX_HARD_RESET) {
		...
		rv = rt1711_block_write(chip->client,
			TCPC_V10_REG_TX_BYTE_CNT, packet_cnt+1, (uint8_t *)temp);
		if (rv < 0)
			return rv;
	}
...
static int rt1711_check_i2c(struct i2c_client *i2c)
{
	...
	if (ret < 0)
		return ret;
	data = 1;
	rt1711_write_device(i2c, RT1711H_REG_SOFTRESET, 1, &data);
	msleep(5);
	return 0;
}
#endif /* #if 0 */
...
static inline int rt1711h_check_revision(struct i2c_client *client)
{
	...
	ret = rt1711_write_device(client, RT1711H_REG_SOFTRESET, 1, &data);
	if (ret < 0)
		return ret;
...
static void rt1711h_shutdown(struct i2c_client *client)
{
	...
	i2c_smbus_write_byte_data(client, RT1711H_REG_SOFTRESET, 0x01);
}
