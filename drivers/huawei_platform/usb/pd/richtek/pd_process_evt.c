...
static const char *pd_ctrl_msg_name[] = {
	"ctrl0",
	"good_crc",
	"goto_min",
	"accept",
...
static const char *pd_data_msg_name[] = {
	"data0",
	"src_cap",
	"request",
	"bist",
...
static const char *pd_hw_msg_name[] = {
	"Detached",
	"Attached",
	"hard_reset",
	"vbus_high",
...
static const char *pd_pe_msg_name[] = {
	"reset_prl_done",
	"pr_at_dft",
	"hard_reset_done",
};
...
static const char *pd_dpm_msg_name[] = {
	"ack",
	"nak",
...
const char *pd_dpm_pd_request_name[] = {
	"pr_swap",
	"dr_swap",
	"vs_swap",
	"gotomin",
...
bool pd_make_pe_state_transit_virt(pd_port_t *pd_port,
	uint8_t curr_state, const pe_state_reaction_t* state_reaction)
{
	bool ret = pd_make_pe_state_transit(pd_port, curr_state, state_reaction);
	...
	pd_port_t *pd_port, pd_event_t *pd_event)
{
	 ...
	 * by the receiver and no error recovery action shall be taken if it is incorrect.
	 */
	 ...
	 * set to the same Data Role as its current Data Role, except for the GoodCRC Message,
	 * Type-C error recovery actions as defined in [USBType-C 1.0] shall be performed.
	 */
