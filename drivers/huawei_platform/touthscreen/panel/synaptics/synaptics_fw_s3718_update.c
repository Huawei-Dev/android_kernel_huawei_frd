...
#ifdef V3
	memcpy(fwu->rmi4_data->rmi4_mod_info.project_id_string, "MAHA211000", 10);
	synaptics_parse_chip_specific_dts(fwu->rmi4_data->synaptics_chip_data);
#endif
...
int synaptics_fw_s3718_update(void)
{
	int retval;
#ifdef V3
	unsigned char temp_id[10] = "MAHA211000";
#endif
...
#ifdef V3
	if (fwu->write_project_id) {
		fwu_read_f34_queries();
		retval = set_lockdown_data(temp_id, 10);
	}
#endif
	fwu->image = NULL;
...
#ifdef V3
	unsigned char temp_id_TD4322[SYNAPTICS_RMI4_PROJECT_ID_SIZE] = "MAHA211000";
	unsigned char temp_id_TD4722[SYNAPTICS_RMI4_PROJECT_ID_SIZE] = "MAHA261001";
#endif
...
#ifdef V3
	if (!strcmp(g_ts_data.product_name, "manhattan")) {
		fwu_read_f34_queries();
		...
		if (retval < 0) {
			...
			if (!strncmp(fwu->rmi4_data->rmi4_mod_info.product_id_string, "TD4722", 6)) {
				memcpy(fwu->rmi4_data->rmi4_mod_info.project_id_string, temp_id_TD4722, SYNAPTICS_RMI4_PROJECT_ID_SIZE);
			} else {
				memcpy(fwu->rmi4_data->rmi4_mod_info.project_id_string, temp_id_TD4322, SYNAPTICS_RMI4_PROJECT_ID_SIZE);
			}
			retval = 0;
