...
static int compare_product_name(const char *product_name)
{
	return strncmp(product_name, HW_PRODUCT_NAME, strlen(HW_PRODUCT_NAME));
}
						      ...
						      int *cp_button_calibration)
{
	...
	uint32_t scaling_factor_self;
	uint32_t scaling_factor_self_rx = 0;
	uint32_t scaling_factor_self_tx = 0;
	...
	int compare_result = compare_product_name(cd->cpdata->product_name);
	...
	*cp_sensor_tx_calibration = kzalloc(tx_num * sizeof(int), GFP_KERNEL);
	...
	if(compare_result){
		...
		ret = get_configuration_parameter(SCALING_FACTOR_SELF_RX, &parameter_type,
						  &parameter_value);
		if (ret || parameter_type != INTEGER) {
			TS_LOG_ERR("%s:Unable to get scaling_factor_self_rx!\n", __func__);
			goto free_buffers;
		}

		scaling_factor_self_rx = parameter_value.integer;
		TS_LOG_INFO("%s: scaling_factor_self_rx: %d\n", __func__,
		...
		TS_LOG_INFO("%s: scaling_factor_self_tx: %d\n", __func__,
	...
	*cp_sensor_rx_average = 0;
	for (i = 0; i < rx_num; i++) {
		(*cp_sensor_rx_data)[i] = calculate_cp_gen6l((*sensor_rx_raw_data)[i],
				...
				(compare_result == 0) ? scaling_factor_self_rx : scaling_factor_self,
				idac_lsb, idac_self_rx,
				rx_atten_self_rx, gidac_mult, clk, v_swing, 
				hw_bl_gidac_lsb_config, hw_bl_gidac_self_rx, hw_bl_pwc_self_rx[i]);
	...
	*cp_sensor_tx_average = 0;
	...
	for (i = 0; i < tx_num; i++) {
		(*cp_sensor_tx_data)[i] = calculate_cp_gen6l((*sensor_tx_raw_data)[i],
				tx_period_self, balancing_target_self,
				(compare_result == 0) ? scaling_factor_self_tx : scaling_factor_self,
				idac_lsb, idac_self_tx,
				rx_atten_self_tx, gidac_mult, clk, v_swing, 
				hw_bl_gidac_lsb_config, hw_bl_gidac_self_tx, hw_bl_pwc_self_tx[i]);
