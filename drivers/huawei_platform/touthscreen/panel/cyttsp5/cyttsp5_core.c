				...
				char *buf, size_t size, u8 data_type)
{
	...
	/*
	 *In TRM, origin_x equal 2 means x is rx
	 *origin_x equal 0 means x is tx
	 */
	TS_LOG_INFO("%s:origin_x = %d\n", __func__,
					cd->sysinfo.sensing_conf_data.origin_x);
	if (cd->sysinfo.sensing_conf_data.origin_x) {
		tx_num = cd->sysinfo.sensing_conf_data.electrodes_y;
		rx_num = cd->sysinfo.sensing_conf_data.electrodes_x;
	} else {
		rx_num = cd->sysinfo.sensing_conf_data.electrodes_y;
		tx_num = cd->sysinfo.sensing_conf_data.electrodes_x;
	}
	...
	rc = cyttsp5_receive_panel_scan_data(dev, input_buf, CY_MAX_INPUT, mutual_data_id);
	if (rc) {
	...
	rc = cyttsp5_receive_panel_scan_data(dev, input_buf, CY_MAX_INPUT, self_cap_data_id);
	if (rc) {
	...
	for (i = 0; i < tx_num * rx_num ; i++) {
		if (!(i%tx_num))
			len += snprintf(buf + len, size - len, "\n %5d ",
								self_cap_raw_data[tx_num + i/tx_num]);
