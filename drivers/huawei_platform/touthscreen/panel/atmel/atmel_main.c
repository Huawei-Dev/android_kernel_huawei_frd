...
static void mxt_proc_t100_message(struct mxt_data *data, u8 *message)
{
	...
	TS_LOG_DEBUG
	    ("[%u] status:%02X x:%u y:%u [amp]:%02X [vec]:%02X [area]:%02X [peak]:%02X [width]:%02X [height]:%02X\n",
	     id, status, x, y, info.amp, info.vec, info.area, info.peak,
...
static int mxt_parse_feature_file(const struct firmware *cfg)
{
	...
	for (featurecode = 0; featurecode < feature_list_count; featurecode++) {
		...
		snprintf(feature_begin, sizeof(feature_begin), "[%s BEGIN]",
			 feature->name);
		snprintf(feature_end, sizeof(feature_end), "[%s END]",
			 feature->name);
		TS_LOG_DEBUG
		    ("feature_name: %s, feature_begin: %s,  feature_end: %s\n",
		     feature->name, feature_begin, feature_end);

		/*find begin/end postions*/
		begin_pos =
		    strstr((char *)(cfg->data),
			   feature_begin) - (char *)(cfg->data);
		end_pos =
		    strstr((char *)cfg->data,
			   feature_end) - (char *)(cfg->data);
		...
		regnum =
		    mxt_search_string(cfg->data + read_pos, end_pos - read_pos,
				      ';');
		TS_LOG_DEBUG
		    ("%s, begin_pos: %d, end_pos: %d, read_pos: %d, regnum: %d, old regnum :%d \n",
		     __func__, begin_pos, end_pos, read_pos, regnum,
		     feature->reg_num);
		...
		for (i = 0; i < regnum; i++) {
			if (sscanf(cfg->data + read_pos, "OBJECT=T%d,INSTANCE=%d,OFFSIZE=%d,VALUE=%d;\n",
