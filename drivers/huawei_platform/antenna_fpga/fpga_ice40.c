...
int check_input_exc(const char *inputdata, const char *outputdata)
{
	...
	for (i = 0; i < RETRY_TIME; i++) {
		plat_data = &drv_data->plat_data;
		(void)kstrtoint(inputdata, BASE_BINARY, &ret);
		hwlog_info("%s ret = %d\n", __func__, ret);
		memset(indata, 0, SPI_DATA_LEN);
		indata[0] = FPGA_WRITE;
				...
				size_t count)
{
	...
	ice40_table =
	    __get_free_pages(GFP_KERNEL | GFP_DMA, get_order(file_size + 1));
			    ...
			    const char *buf, size_t count)
{
	...
	(void)kstrtoint(input_address, BASE_BINARY, &ret);
	hwlog_info("%s ret = %d\n", __func__, ret);
	memset(indata, 0, SPI_DATA_LEN);
	indata[0] = FPGA_WRITE;
	indata[1] = FPGA_REGISTER_TESTMODE;
...
int fpga_ice40_probe(struct platform_device *pdev)
{
	...
	hwlog_info("%s date:0606", __func__);
