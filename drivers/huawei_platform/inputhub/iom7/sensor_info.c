...
BH1745_ALS_PARA_TABLE als_para_diff_tp_color_table[] = {
	{NEXT, V3, LG_TPLCD, BLACK,
	 {249, 480, 403, 1826, 1472, 249, 451, 1650, 100, -2512, 7591, -2862,
			...
			unsigned long action, void *data)
{
	...
	if (ret != 0) {
		hwlog_err("notify mag environment change failed, ret = %d!\n",
			  ret);
		__dmd_log_report(DSM_SHB_ERR_CFG_DATA, __func__,
	...
	uint8_t register_add, uint8_t rw, int len, uint8_t *buf)
{
	...
	if (pkg_mcu.errno != 0) {
		hwlog_err("mcu_i2c_rw fail, addr=%d, reg=%d,rw=%d\n",
				i2c_add, register_add, rw);
		__dmd_log_report(DSM_SHB_ERR_IOM7_I2C_DBG, __func__,
			       "mcu_i2c_rw fail\n");
		kfree(pkt_i2c_write);
		return -1;
	} else {
	...
	if (1 == val) {
		...
		if (pkg_mcu.errno != 0) {
			...
			hwlog_info("cap_prox calibrate  success, data len=%d\n",
				   pkg_mcu.data_length);
			cap_prox_calibrate_save(pkg_mcu.data,
						pkg_mcu.data_length);
		}
	}
	return count;
				...
				struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (val == 1) {
		...
		schedule_work(&handpress_calibrate_work);
		msleep(50);
