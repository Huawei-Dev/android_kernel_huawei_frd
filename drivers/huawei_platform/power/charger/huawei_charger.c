...
*  return value:  NULL
**********************************************************/
static void charge_rename_charger_type(enum hisi_charger_type type,
...
static inline void tcpc_report_host_attach(void)
{
	pr_info("tcpc_report_host_attach\r\n");
	fsa9685_dcd_timeout_enable(true);
	fsa9685_manual_sw(FSA9685_USB1_ID_TO_IDBYPASS);
	hisi_usb_otg_event(ID_FALL_EVENT);
}

static inline void tcpc_report_host_detach(void)
{
	pr_info("tcpc_report_host_detach\r\n");
...
*  return value:  NOTIFY_OK-success or others
**********************************************************/
static int tcpc_notifier_call(struct notifier_block *tcpc_nb, unsigned long event, void *data)
{
	struct tcp_notify *tcp_noti = (struct tcp_notify *)data;
	...
	case TCP_NOTIFY_SOURCE_VBUS:
		...
		pr_info("Huawei charger SOURCE VBUS %d mV, %d mA\n", tcp_noti->vbus_state.mv, tcp_noti->vbus_state.ma);

		mutex_lock(&di->tcpc_otg_lock);
		if (tcp_noti->vbus_state.mv > 0) {
			if (di->tcpc_otg == 0) {

				pr_info("PD_OTG=1\n");

				tcpc_report_host_attach();
				di->tcpc_otg = 1;
				...
				if(di->ops->set_otg_current)
				{
					//ret = di->ops->set_otg_current(1500);
					ret = di->ops->set_otg_current(tcp_noti->vbus_state.ma);/*otg current set 500mA form dtsi*/
					if(ret)
					   hwlog_err("[%s]set otg current fail!\n",__func__);
				}
				#endif
		...
		tcpm_notify_vbus_stable(di->tcpc);
		break;

	case TCP_NOTIFY_SINK_VBUS:
		/* Implement sink vubs behavior here */
		pr_info("Huawei charger SINK VBUS %d mV, %d mA\n", tcp_noti->vbus_state.mv, tcp_noti->vbus_state.ma);

		mutex_lock(&di->tcpc_otg_lock);
		if (tcp_noti->vbus_state.mv > 500) {
			di->charger_type = CHARGER_TYPE_STANDARD;
			di->charger_source = POWER_SUPPLY_TYPE_MAINS;
			charge_send_uevent(di);
			schedule_work(&di->usb_work);
		} else if (tcp_noti->vbus_state.mv > 0) {
			di->charger_type = CHARGER_TYPE_USB;
			di->charger_source = POWER_SUPPLY_TYPE_USB;
			charge_send_uevent(di);
			schedule_work(&di->usb_work);
...
static int charge_probe(struct platform_device *pdev)
{
	...
	di->core_data =
	    (struct charge_core_data *)kzalloc(sizeof(struct charge_core_data), GFP_KERNEL);
	...
	di->core_data = charge_core_get_params();
	if(!di->core_data) {
		hwlog_err("charge_core_get_params fail!\n");
		ret = -EINVAL;
		goto charge_fail_1;
	}
	...
	hwlog_info("Register TCPC notifier\n");
	di->tcpc = tcpc_dev_get_by_name("type_c_port0");
	if (!di->tcpc) {
		hwlog_err("get type_c_port0 fail\n");
	} else {
		hwlog_info("Found TCPC type_c_port0\n");
		di->tcpc_nb.notifier_call = tcpc_notifier_call;
		ret = register_tcp_dev_notifier(di->tcpc, &di->tcpc_nb);
		if (ret < 0)
		{
			hwlog_err("charge_fault_register_notifier failed\n");
		}
		hwlog_info("charge_fault_register_notifier OK\n");
	}
#endif
	...
	charge_done_sleep_dts =
	    of_property_read_bool(pdev->dev.of_node, "charge_done_sleep_enabled");
