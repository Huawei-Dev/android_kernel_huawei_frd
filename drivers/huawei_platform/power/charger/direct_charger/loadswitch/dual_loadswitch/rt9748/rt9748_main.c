...
static int rt9748_charge_enable (int  enable)
{
	...
	if (ret)
	{
		hwlog_err("%s: rt9748 charge enable fail, val = %d\n", __func__, enable);
		return -1;
	}
	return 0;
}
...
static int rt9748_config_vbus_ovp_threshold_mv(int ovp_threshold)
{
	...
	hwlog_info("ovp_threshold = %d, value = 0x%x\n", ovp_threshold, value);
...
static int rt9748_config_vout_reg_threshold_mv(int vout_reg_threshold)
{
	...
	hwlog_info("vout_reg_threshold = %d, value = 0x%x\n", vout_reg_threshold, value);
...
static int rt9748_get_bat_current_ma(int * ibat)
{
	...
	*ibat = (reg_high & RT9748_IBAT_ADC_MASK) * RT9748_VBAT_HIGH_LSB;
...
static int rt9748_get_ls_vdrop(int * vdrop)
{
	u8 reg_high = 0;
	u8 reg_low = 0;
	int polarity = 0;
	...
	ret = rt9748_read_byte(RT9748_VDROP_ADC2, &reg_high);
	hwlog_info("%s: VDROP_ADC2 = 0x%x\n", __func__, reg_high);
	ret |= rt9748_read_byte(RT9748_VDROP_ADC1,&reg_low);
	hwlog_info("%s: VDROP_ADC1 = 0x%x\n", __func__, reg_low);
	if (ret)
	{
		hwlog_err("%s: vdrop reg read fail\n", __func__);
		return -1;
	}
	polarity = (reg_high & RT9748_VDROP_POLARITY_MASK) >> (LENTH_OF_BYTE - 1);
	*vdrop = (reg_high & RT9748_VDROP_ADC_MASK) * RT9748_VDROP_HIGH_LSB;
	*vdrop += reg_low * RT9748_VDROP_LOW_LSB;
	if (1 == polarity)
		*vdrop *= -1;
	return ret;
...
static int rt9748_get_ls_resistance(int *r)
{
	int vdrop;
	int ibus;
	int ret;

	ret = rt9748_get_ls_vdrop(&vdrop);
	if (ret)
	{
		hwlog_err("%s: get vdrop fail\n", __func__);
		return -1;
	}
	ret = rt9748_get_ls_ibus(&ibus);
	if (ret)
	{
	...
	*r = vdrop * 1000 / ibus;
	hwlog_info("%s: vdrop = %d, ibus = %d, r = %d\n", __func__, vdrop, ibus, *r);
	return 0;
}
...
static int rt9748_charge_init(void )
{
	...
	ret |= rt9748_config_vbus_ovp_threshold_mv(5500);
	...
	ret |= rt9748_config_vbat_reg_threshold_mv(4600);
	ret |= rt9748_config_ibat_ocp_threshold_ma(5000);
	ret |= rt9748_config_ibus_ocp_threshold_ma(5000);
	if(ret)
	{
	...
	rt9748_read_byte(RT9748_EVENT_1, &reg_irq);
	hwlog_info("reg_event1 = 0x%x\n", reg_irq);
	rt9748_read_byte(RT9748_EVENT_2, &reg_irq);
	hwlog_info("reg_event2 = 0x%x\n", reg_irq);
	return 0;
}
...
static int rt9748_get_ls_id(void)
{
	u8 reg = 0;
	int ret = 0;
...
static struct loadswitch_ops  rt9748_sysinfo_ops ={
	...
	.get_ls_id = rt9748_get_ls_id,
	.get_ls_resistance = rt9748_get_ls_resistance,
...
static void rt9748_irq_work(struct work_struct *work)
{
	...
	rt9748_read_byte(RT9748_EVENT_1, &reg_irq);
	hwlog_info("reg_irq_event1 = 0x%x\n", reg_irq);

	rt9748_read_byte(RT9748_EVENT_2, &reg_irq);
	hwlog_info("reg_irq_event2 = 0x%x\n", reg_irq);

	rt9748_read_byte(RT9748_EVENT_STATUS, &reg_irq);
	hwlog_info("reg_irq_event_status = 0x%x\n", reg_irq);
	//*clear irq*/
	enable_irq(di->irq_int);
}
	...
	if (power_class) {
		...
		ret =
		    sysfs_create_link(&charge_dev->kobj, &di->dev->kobj,
				      "rt9748");
		if (ret)
			hwlog_err("create link to rt9748 fail.\n");
	}
*/
