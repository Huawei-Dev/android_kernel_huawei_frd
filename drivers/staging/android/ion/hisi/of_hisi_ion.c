...
static int get_type_by_name(const char *name, enum ion_heap_type *type)
{
	...
	for (i = 0; i < ARRAY_SIZE(ion_type_table); i++) {
		if (strncmp(name, ion_type_table[i].name,
			    strlen(ion_type_table[i].name)))
			continue;
...
static int hisi_set_platform_data(struct platform_device *pdev)
{
	...
	for_each_child_of_node(dt_node, np) {
		struct device_node* phandle_node = of_parse_phandle(np, "heap-name", 0);
		if (phandle_node) {
			int len;
			struct property *prop = of_find_property(phandle_node, "heap-name", &len);
			if (!prop) {
