...
static struct page *seccm_dma_alloc_from_contiguous(struct device *dev,
				int count, unsigned int align, int trynum)
{
	int i;
	struct page *page = NULL;

	for (i = 0; i < trynum; i++) {
		page = dma_alloc_from_contiguous(dev, count, align);
		if (page)
			break;
		mdelay(10);
	}
						      ...
						      unsigned long align)
{
	...
	if (seccm_heap->heap_status == CREAT) {
		...
		if(seccm_heap->heap.id == ION_DRM_GRALLOC_HEAP_ID)
			atfd_hisi_service_access_register_smc(ACCESS_REGISTER_FN_MAIN_ID,
					seccm_heap->base, seccm_heap->size,
					ACCESS_REGISTER_FN_SUB_ID_DDR_DRM_GRALLOC_SET);
		else if(seccm_heap->heap.id == ION_DRM_VCODEC_HEAP_ID)
			atfd_hisi_service_access_register_smc(ACCESS_REGISTER_FN_MAIN_ID,
					seccm_heap->base, seccm_heap->size,
					ACCESS_REGISTER_FN_SUB_ID_DDR_DRM_VCODEC_SET);

		seccm_heap->pool = gen_pool_create(12, -1);
		if (seccm_heap->pool == NULL)
			return ION_CARVEOUT_ALLOCATE_FAIL;

		(void)gen_pool_add(seccm_heap->pool, seccm_heap->base,
				seccm_heap->size, -1);
	...
	if (!offset) {
		pr_err("seccm_alloc size 0x%lx avail size 0x%lx\n", size,
				gen_pool_avail(seccm_heap->pool));
		return ION_CARVEOUT_ALLOCATE_FAIL;
	}
	return offset;
}
		       ...
		       unsigned long size)
{
	...
	if (gen_pool_avail(seccm_heap->pool) == seccm_heap->size) {
		if (seccm_heap->heap_status == ALLOC) {
			atfd_hisi_service_access_register_smc(ACCESS_REGISTER_FN_MAIN_ID,
					seccm_heap->base, seccm_heap->size,
					ACCESS_REGISTER_FN_SUB_ID_DDR_DRM_CLR);
			create_mapping_late(seccm_heap->base,
				(unsigned long)phys_to_virt(seccm_heap->base),
...
struct ion_heap *ion_seccm_heap_create(struct ion_platform_heap *heap_data)
{
	...
	seccm_heap->dev = hisi_get_cma_area_device((char *)heap_data->name);
