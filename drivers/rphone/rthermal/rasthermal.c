...
{	
	...
	for (i = 0; i < len; i++) {
		if (NULL == flt.ft[i].thermal_type) continue;
...
static int fail(const char *type, int temp)
{
	...
	if (j == -1)
		return -1;
	ras_retn_iferr(ras_malloc((void**)&flt.ft[j].thermal_type, strlen(type)+1));
...
static int rasprobe_handler(hisi_peripheral_ntc_2_temp) (struct rasprobe_instance *ri,
                                          struct pt_regs *regs) {
        int temp;
        ...
        if (VALID_TEMPREATURE != temp){
                printk("ThermalType:%s,Old:%d,New:%d\n", pzone->type, *temperature, temp);
                *temperature = temp;
        }
       return 0;
}
...
static int rasprobe_handler(hisi_battery_temperature) (struct rasprobe_instance *ri,
                                          struct pt_regs *regs) {
        int temp = should_fail("Battery");
        
	if (VALID_TEMPREATURE != temp){
        	printk("ThermalType:Battery,Old:%d,New:%d\n", (int)regs_return_value(regs), temp);
...
#endif
static int rasprobe_handler(power_supply_read_temp) (struct rasprobe_instance *ri,
                                          struct pt_regs *regs) {
        int temp;
	struct RasRegs *rd = (struct RasRegs *)ri->data;
	struct thermal_zone_device *pzone = rd->args[0];
...
static int proc_ops_show(rThermal)(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < len; i++) {
		...
		if (NULL == ft->thermal_type)
			continue;
		seq_printf(m, "%2d\t%20s\t%d\n", i, ft->thermal_type, ft->temperature);
	}
	/*read_unlock(&flt.rwk);*/
	return 0;
}
