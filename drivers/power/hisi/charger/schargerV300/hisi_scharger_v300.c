...
static int hi6523_config_opt_param(int vbus_vol)
{
	if (CHG_VERSION_V300 == hi6523_version) {
		...
		hi6523_write_byte(0x65, 0x81);
		hi6523_write_byte(0x66, 0x41);
		hi6523_write_byte(0x69, 0xd4);
		hi6523_write_byte(0x6a, 0x1f);
		hi6523_write_byte(0x6b, 0xfc);
		hi6523_write_byte(0x71, 0x57);
		hi6523_write_byte(0x73, 0x58);
		hi6523_write_byte(0x74, 0x01);
		hi6523_write_byte(0x75, 0x95);
		hi6523_write_byte(0x76, 0x01);
		hi6523_write_byte(0x79, 0xd0);
		hi6523_write_byte(0x84, 0x25);
		hi6523_write_byte(0x87, 0x65);
		hi6523_write_byte(0x8e, 0x40);
	} else {
		//need to add after v110 ec
	}
	return 0;
}
...
static int hi6523_set_dpm_voltage(int vindpm)
{
	...
	return hi6523_write_mask(CHG_DPM_SEL_REG, CHG_DPM_SEL_MSK,
...
static int hi6523_set_terminal_current(int term_current)
{
	...
	if (term_current <= CHG_TERM_ICHG_50MA)
		Iterm = 0;
	else if (term_current > CHG_TERM_ICHG_50MA
		 && term_current <= CHG_TERM_ICHG_100MA)
		Iterm = 1;
	else if (term_current > CHG_TERM_ICHG_100MA
		 && term_current <= CHG_TERM_ICHG_150MA)
		Iterm = 2;
	else if (term_current > CHG_TERM_ICHG_150MA
		 && term_current <= CHG_TERM_ICHG_200MA)
		Iterm = 3;
...
static int hi6523_get_ibus_ma(void)
{
	...
	SCHARGER_INF
	    ("[%s],ibus: %d, ilimit: %d, ibus_data: %x, ibus_ref_data: %x\n",
...
static int hi6523_fcp_chip_init(void)
{
	...
	ret |= hi6523_set_otg_enable(OTG_DISABLE);
	ret |= hi6523_set_charge_enable(CHG_DISABLE);
	/*set fcp dpm to 82.5%,because we need not to adjust dpm in fcp */
	ret |=
	    hi6523_write_mask(CHG_DPM_SEL_REG, CHG_DPM_SEL_MSK,
			      CHG_DPM_SEL_SHIFT, CHG_DPM_SEL_82_5);
	return ret;
}
...
static int hi6523_stop_charge_config(void)
{
	int ret = 0;
	ret |= hi6523_set_vbus_vset(VBUS_VSET_5V);
	ret = hi6523_set_dpm_voltage(CHARGE_VOLTAGE_4520_MV);
	return ret;
}

/**********************************************************
*  Function:       hi6523_fcp_stop_charge_config
*  Discription:    fcp config chip after stop charging
*  Parameters:     NULL
*  return value:  0-sucess or others-fail
**********************************************************/
...
static void hi6523_plugout_check_work(struct delayed_work *work)
{
	...
	struct hi6523_device_info *di =
	    container_of(work, struct hi6523_device_info, plugout_check_work);
	...
	if ((regval & HI6523_CHG_BUCK_OK) && (vbatt_mv > plugout_check_vbatt_mv)
	    && (current_ma >= plugout_check_ibatt_ma)
	    && (regval & CHG_IN_DPM_STATE)) {
		hi6523_set_charger_hiz(TRUE);
		msleep(100);
		hi6523_set_charger_hiz(FALSE);
		SCHARGER_INF
		    ("%s : set buck on and off again to avoid chargerIC plugout det fail.\n",
		     __func__);
	}
	schedule_delayed_work(&di->plugout_check_work,
			...
			const struct i2c_device_id *id)
{
	...
	if (BAT_BOARD_ASIC == is_board_type) {
		ret =
		    request_irq(di->irq_int, hi6523_interrupt,
