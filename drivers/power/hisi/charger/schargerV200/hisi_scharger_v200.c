...
static void voltage_base_input_current_select(unsigned int *piin_limit,int iin_option_num,struct hi6522_device_info *di)
{
    static int index = 0;
    unsigned int iin_lim = 0;
    unsigned int state = 0;
    int ret = 0;
    di->ico_iin = *piin_limit;
...
int hi6522_input_current_optimize(enum usb_charger_type charger_type, unsigned int *piin_limit)
{
    struct hi6522_device_info *di = g_hi6522_dev;
    int bat_voltage = hisi_battery_voltage();
    int bat_exist = is_hisi_battery_exist();
    int avg_voltage = 0;
    if((di == NULL) || (piin_limit == NULL))
    {
        return -1;
    }
    if((!bat_exist) || (di->ico_support == FALSE))
    {
        return -1;
    }
    ...
    if(avg_voltage < DPM_CHECK_VOLTAGE)
    {
        if(di->ico_en)
        {
            voltage_base_input_current_select(piin_limit,1,di);
    ...
    else
    {
        ...
        {
            ico_hig_thr_count = 0;
            voltage_base_input_current_select(piin_limit,0,di);
            di->ico_en = TRUE;
        }
    }
    *piin_limit = di->ico_iin;
    return 0;
}
...
static void hi6522_dpm_check_work(struct work_struct *work)
{
	...
	if ((current_ma > dpm_check_current_threshold_ma)
		...
		if (dpm_switch_with_charge_stop) {
			if (down_interruptible(&di->charger_data_busy_lock)) {
				SCHARGER_ERR
				    ("hi6522_dpm_check_work get sema fail! \n");
			}
...
struct charge_device_ops hi6522_ops = {
    ...
    .turn_on_force_ico = hi6522_input_current_optimize,
};
...
static void parse_dts(struct device_node *np, struct hi6522_device_info *di)
{
    ...
    ret = of_property_read_u32( np,"input_current_support",&(di->ico_support));
    if(ret)
    {
        SCHARGER_ERR("get ico_support failed\n");
        return ;
    }
    SCHARGER_INF("get support ico is %d!\n",di->ico_support);
    if(di->ico_support)
    {
