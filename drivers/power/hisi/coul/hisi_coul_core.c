...
int coul_is_fcc_invalid(void)
{
    struct smartstar_coul_device *di = g_smartstar_coul_dev;
    if (di)
	return di->fcc_invalid;
    else
        return 0;
}
/**********************************************************
...
void refresh_fcc(struct smartstar_coul_device *di)
{
        ...
        )
	{
		...
		fcc_origin = interpolate_fcc(di, di->batt_temp);
 		max_delta_fcc_uah = fcc_origin*DELTA_FCC_PERCENT*10;
		...
		fcc_min_uah = fcc_origin*MIN_FCC_PERCENT*10;
		if (new_fcc_uah <= fcc_min_uah)
		{
			new_fcc_uah = fcc_min_uah;
			di->fcc_invalid = 1;
		}
...
static int  hisi_coul_probe(struct platform_device *pdev)
{
    ...
    di->charging_stop_time = di->coul_dev_ops->get_coul_time();
    if (di->adjusted_fcc_temp_lut != NULL) {
        fcc_now_uah = 1000 * interpolate_fcc_adjusted(di, di->batt_temp);
        fcc_min_uah = interpolate_fcc(di, di->batt_temp)*MIN_FCC_PERCENT*10;
        if (fcc_now_uah < fcc_min_uah) {
            fcc_need_reset = 1;
            di->fcc_invalid = 1;
            hwlog_info("need reset fcc, fcc_now, fcc_min %d.%d\n", fcc_now_uah, fcc_min_uah);
        }
    }

    /* battery moved, clear battery data  */
    if(fcc_need_reset || di->coul_dev_ops->is_battery_moved()){
        di->batt_chargecycles = 0;
        di->batt_changed_flag = 1;
        di->batt_limit_fcc = 0;
...
coul_no_battery:
    ...
    coul_ops->is_fcc_invalid              = coul_is_fcc_invalid;
