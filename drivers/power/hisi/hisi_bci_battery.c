...
static int charge_current_max = 7000;
...
int check_charge_curr_overhigh(char *buf)
{
	...
	for (i = 0; i < 3; i++) {
		...
		if (current_now[i] > charge_current_max)
			msleep(100);
		else
			break;
...
int check_fcc_invalid(char *buf)
{
	if (is_fcc_invalid()) {
		snprintf(buf, 1024, " fcc is invalid\n");
		return 1;
	}
	return 0;
}
static struct batt_dsm batt_dsm_array[] = {
	...
	{ERROR_FCC_INVALID, true, check_fcc_invalid},
};
...
static int hisi_bci_battery_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_u32(np, "charge_current_max", &charge_current_max)) {
		charge_current_max = 7000;
		hwlog_err("error:get charge_current_max value failed!\n");
	}
