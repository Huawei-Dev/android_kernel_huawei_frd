...
OAL_STATIC oal_uint32 dmac_config_set_ant(mac_vap_stru *pst_mac_vap, oal_uint8 uc_len, oal_uint8 *puc_param);
...
OAL_STATIC oal_uint32 dmac_config_set_ant(mac_vap_stru *pst_mac_vap, oal_uint8 uc_len, oal_uint8 *puc_param)
{
    ...
    hal_chip_get_device(pst_mac_vap->uc_chip_id, pst_mac_vap->uc_device_id, &pst_hal_device);

    hal_reg_info16(pst_hal_device, GPIO_MASK_ADDR, &us_reg_value);
    OAM_WARNING_LOG2(0, OAM_SF_PWR, "dmac_config_set_ant  ant:%d, GPIO_MASK 0x%x", uc_param, us_reg_value);

    hal_reg_write16(pst_hal_device, GPIO_MASK_ADDR, 0);

    oal_config_gpio_pin(BCPU_GPIO_BASE, WLAN_ANT_GPIO,GPIO_INOUT_OUT,GPIO_TYPE_IO);

    oal_set_gpio_level(BCPU_GPIO_BASE, WLAN_ANT_GPIO, uc_param);

    for (ul_cnt = 0; ul_cnt < 1000; ul_cnt++)
    {
        oal_get_gpio_level(BCPU_GPIO_BASE, WLAN_ANT_GPIO, &uc_result);

        if (uc_result == uc_param)
        {
            break;
        }
    }
    ...
    if (uc_result != uc_param)
    {
        OAM_WARNING_LOG2(0, OAM_SF_PWR, "dmac_config_set_ant fail want:%d actual %d", uc_param, uc_result);
    }
