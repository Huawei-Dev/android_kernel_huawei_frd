...
oal_int32 wal_android_priv_cmd(oal_net_device_stru *pst_net_dev, oal_ifreq_stru *pst_ifr, oal_int32 ul_cmd)
{
...
#endif
#ifdef _PRE_WLAN_FEATURE_VOWIFI
    else if (oal_memcmp(puc_command, CMD_VOWIFI_SET_PARAM, OAL_STRLEN(CMD_VOWIFI_SET_PARAM)) == 0)
    {
        l_ret = wal_ioctl_set_vowifi_param(pst_net_dev, (oal_int8*)puc_command);
        if (OAL_UNLIKELY(OAL_SUCC != l_ret))
        {
    ...
    else if (oal_memcmp(puc_command, CMD_VOWIFI_GET_PARAM, OAL_STRLEN(CMD_VOWIFI_GET_PARAM)) == 0)
    {
        l_value = 0;
        l_ret = wal_ioctl_get_vowifi_param(pst_net_dev, puc_command, &l_value);
        if (OAL_UNLIKELY(OAL_SUCC != l_ret))
        {
        ...
        oal_memset(puc_command, 0, (oal_uint32)(st_priv_cmd.l_total_len + 1));
        puc_command[st_priv_cmd.l_total_len] = '\0';
        pl_value  = (oal_int32 *)puc_command;
        *pl_value = l_value;
    ...
    else if (oal_memcmp(puc_command, CMD_VOWIFI_IS_SUPPORT, OAL_STRLEN(CMD_VOWIFI_IS_SUPPORT)) == 0)
    {
        /* 将buf清零 */
        oal_memset(puc_command, 0, (oal_uint32)(st_priv_cmd.l_total_len + 1));
        puc_command[st_priv_cmd.l_total_len] = '\0';
        oal_memcopy(puc_command, CMD_VOWIFI_IS_SUPPORT_REPLY, OAL_STRLEN(CMD_VOWIFI_IS_SUPPORT_REPLY));
        if(oal_copy_to_user(pst_ifr->ifr_data+8, puc_command, (oal_uint32)(st_priv_cmd.l_total_len)))
        {
