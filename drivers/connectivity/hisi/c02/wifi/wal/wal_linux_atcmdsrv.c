...
oal_int32 wal_atcmdsrv_wifi_priv_cmd(oal_net_device_stru *pst_net_dev, oal_ifreq_stru *pst_ifr, oal_int32 ul_cmd)
{
    ...
    switch(st_priv_cmd.ul_cmd)
    {
...
#ifdef _PRE_WLAN_FEATURE_SMARTANT
        case WAL_ATCMDSRV_IOCTL_CMD_GET_ANT_INFO:
            l_ret = wal_atcmsrv_ioctl_get_ant_info(pst_net_dev,&st_priv_cmd.pri_data.double_ant.uc_ant_type,
                &st_priv_cmd.pri_data.double_ant.ul_last_ant_change_time_ms,
                &st_priv_cmd.pri_data.double_ant.ul_ant_change_number,
                &st_priv_cmd.pri_data.double_ant.ul_main_ant_time_s,
                &st_priv_cmd.pri_data.double_ant.ul_aux_ant_time_s,
                &st_priv_cmd.pri_data.double_ant.ul_total_time_s);
            if(oal_copy_to_user(pst_ifr->ifr_data,&st_priv_cmd,sizeof(wal_atcmdsrv_wifi_priv_cmd_stru)))
            {
                OAM_ERROR_LOG0(0, OAM_SF_ANY,"wal_atcmdsrv_wifi_priv_cmd:Failed to copy ioctl_data to user !");
                l_ret = -OAL_EINVAL;
        ...
        case WAL_ATCMDSRV_IOCTL_CMD_DOUBLE_ANT_SW:
            l_ret = wal_atcmsrv_ioctl_double_ant_switch(pst_net_dev,
                st_priv_cmd.pri_data.double_ant_sw.ul_double_ant_sw,
                &st_priv_cmd.pri_data.double_ant_sw.ul_ret);
            if(oal_copy_to_user(pst_ifr->ifr_data,&st_priv_cmd,sizeof(wal_atcmdsrv_wifi_priv_cmd_stru)))
            {
                OAM_ERROR_LOG0(0, OAM_SF_ANY,"wal_atcmdsrv_wifi_priv_cmd:Failed to copy ioctl_data to user !");
                l_ret = -OAL_EINVAL;
