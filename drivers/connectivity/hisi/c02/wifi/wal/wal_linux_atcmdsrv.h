...
enum WAL_ATCMDSRV_IOCTL_CMD
{
    ...
    WAL_ATCMDSRV_IOCTL_CMD_GET_ANT_INFO,//22  CHR获取天线信息，包括当前用的哪根天线、多久之前切换过天线、切换了多少次，以及每个天线所用的时长
    WAL_ATCMDSRV_IOCTL_CMD_DOUBLE_ANT_SW,//23  开关双天线算法
    HWIFI_IOCTL_CMD_TEST_BUTT
...
typedef struct wal_atcmdsrv_wifi_priv_cmd {
    ...
    struct
    {
        oal_uint32      ul_double_ant_sw;           //开关双天线算法，0为关闭，1为打开
        oal_uint32      ul_ret;                     //开关是否生效
    }double_ant_sw;
#endif
