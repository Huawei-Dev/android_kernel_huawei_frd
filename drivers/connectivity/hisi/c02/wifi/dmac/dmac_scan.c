        ...
        if ((OAL_PTR_NULL != puc_ssid) && (0 != uc_ssid_len) && (st_mib_ssid.uc_ssid_len == uc_ssid_len))
        {
            if (!OAL_MEMCMP(st_mib_ssid.ac_ssid, puc_ssid, uc_ssid_len))
            {
                ...
                if ((pst_dmac_vap->st_vap_base_info.st_channel.uc_chan_number != uc_frame_channel)
                    && (0 != uc_frame_channel))
                {
                    en_band = mac_get_band_by_channel_num(uc_frame_channel);

                    /* 更新VAP下的主20MHz信道号、带宽模式、信道索引 */
                    ul_ret = mac_get_channel_idx_from_num(en_band, uc_frame_channel, &uc_idx);
                    if (OAL_SUCC != ul_ret)
                    {
                    ...
                    /* STA当前信道与获取管理帧中的信道不一致,则更新STA的当前频段,信道,信道索引 */
                    pst_dmac_vap->st_vap_base_info.st_channel.en_band        = en_band;
                    pst_dmac_vap->st_vap_base_info.st_channel.uc_chan_number = uc_frame_channel;
                    pst_dmac_vap->st_vap_base_info.st_channel.uc_idx         = uc_idx;

                    if (!mac_is_dbac_running(pst_mac_device))
                    {
                        /* 非DBAC时上报信道切换完毕消息 */
                        pst_dmac_vap->st_linkloss_info.us_link_loss = 0;
                        OAM_WARNING_LOG2(pst_dmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SCAN, "{dmac_scan_check_assoc_ap_channel::AP has switch channel home chan[%d], new chan[%d].}\r\n",
                                                                  ...
                                                                  dmac_vap_stru    *pst_dmac_vap)
{
    ...
    pst_scan_params->us_scan_work_time_swith_home_channel    = MAC_SCAN_WORK_TIME_ON_HOME_CHANNEL_DEFAULT;
...
OAL_STATIC oal_bool_enum_uint8  dmac_scan_need_switch_home_channel(mac_device_stru *pst_mac_device)
{
    ...
    if (pst_scan_params->uc_scan_channel_interval != MAC_SCAN_CHANNEL_INTERVAL_DEFAULT
    ...
    if ((OAL_TRUE == pst_mac_device->st_scan_params.en_need_switch_back_home_channel) &&
        (0 == pst_mac_device->uc_scan_chan_idx % pst_scan_params->uc_scan_channel_interval))
    {
        return OAL_TRUE;
    }
...
OAL_STATIC oal_void  dmac_scan_switch_home_channel_work(mac_device_stru *pst_mac_device)
{
    ...
    if (pst_scan_params->us_scan_work_time_swith_home_channel != MAC_SCAN_WORK_TIME_ON_HOME_CHANNEL_DEFAULT
        && pst_scan_params->us_scan_work_time_swith_home_channel != MAC_SCAN_WORK_TIME_ON_HOME_CHANNEL_PERFORMANCE)
    {
        pst_scan_params->us_scan_work_time_swith_home_channel = MAC_SCAN_WORK_TIME_ON_HOME_CHANNEL_DEFAULT;
    }
    ...
    FRW_TIMER_CREATE_TIMER(&pst_mac_device->st_scan_timer,
                           ...
                           pst_scan_params->us_scan_work_time_swith_home_channel,
                           pst_mac_device,
                           OAL_FALSE,
                           OAM_MODULE_ID_DMAC,
                           pst_mac_device->ul_core_id);
