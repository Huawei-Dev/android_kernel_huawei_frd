...
/* 只1102需要tsf_restore逻辑 */
        ...
        )
    {
        OAL_DLIST_SEARCH_FOR_EACH(pst_dscr_entry, pst_tx_dscr_list_hdr)
        {
            ...
            if((OAL_TRUE == MAC_GET_CB_IS_VIPFRAME(pst_cb)) ||(MAC_DATA_ARP_REQ == uc_data_type)||(MAC_DATA_ARP_RSP == uc_data_type))
            {
                ...
                {
                    /* 软件拉高priority */
                    dmac_btcoex_wlan_priority_set(&(pst_dmac_vap->st_vap_base_info), 1, uc_coex_wl_time);
                }
            }
        }
    }
...
oal_uint32  dmac_tx_process_data_event(frw_event_mem_stru *pst_event_mem)
{
    ...
    while (pst_netbuf != OAL_PTR_NULL)
    {
        ...
        if((OAL_TRUE == MAC_GET_CB_IS_VIPFRAME(pst_tx_ctl)) || (MAC_DATA_ARP_REQ == uc_data_type) || (MAC_DATA_ARP_RSP == uc_data_type))
        {
