...
oal_uint32  dmac_vap_sta_reset(dmac_vap_stru *pst_dmac_vap)
{
    ...
    {
        OAM_WARNING_LOG1(pst_dmac_vap->st_vap_base_info.uc_device_id, OAM_SF_ANY, "{dmac_vap_sta_reset::pst_dmac_dev null.dev id [%d]}", pst_dmac_vap->st_vap_base_info.uc_device_id);
        return OAL_ERR_CODE_PTR_NULL;
    }

    if ((OAL_TRUE == pst_dmac_dev->en_state_in_sw_ctrl_mode) && (1 == pst_dmac_vap->st_vap_base_info.bit_rsp_soft_ctl_launcher))
    {
        pst_dmac_vap->st_vap_base_info.bit_rsp_soft_ctl_launcher = 0;
        en_launcher_vap_found = mac_vap_search_other_rsp_launcher(&pst_dmac_vap->st_vap_base_info);
...
oal_void dmac_vap_update_rsp_frm_rate(oal_uint8 uc_vap_id, oal_uint8 uc_protocol_mode, oal_uint8 uc_bandwidth, oal_uint8 uc_ref_rate)
{
    ...
    {
        hal_cfg_rsp_dyn_bw(OAL_TRUE, WLAN_BW_CAP_80M);
    }
