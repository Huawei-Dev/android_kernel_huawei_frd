...
OAL_STATIC oal_void  dmac_sta_up_rx_ch_switch(mac_vap_stru *pst_mac_vap, oal_uint8  *puc_frame_body,oal_uint16  us_framebody_len)
{
    ...
    {
        if (MAC_EID_CHANSWITCHANN == puc_frame_body[us_index])
        {
            dmac_ie_proc_ch_switch_ie(pst_mac_vap, &puc_frame_body[us_index], MAC_EID_CHANSWITCHANN);
        }
        else if (MAC_EID_SEC_CH_OFFSET == puc_frame_body[us_index])
        {
            if (MAC_SEC_CH_OFFSET_IE_LEN > puc_frame_body[us_index + 1])
            {
                OAM_WARNING_LOG1(0, OAM_SF_ANY, "{dmac_sta_up_rx_ch_switch::invalid sec chan offset ie len[%d]}", puc_frame_body[us_index + 1]);
                us_index += MAC_IE_HDR_LEN + puc_frame_body[us_index + 1];
                continue;
            }
            //l00311403TODO - 如果通道发生改变了，不需要切信道么?
            pst_mac_vap->st_ch_switch_info.en_new_bandwidth = mac_get_bandwidth_from_sco(puc_frame_body[us_index + MAC_IE_HDR_LEN]);
        }
        else if (MAC_EID_WIDE_BW_CH_SWITCH == puc_frame_body[us_index])
        {
            if (OAL_TRUE == mac_mib_get_VHTOptionImplemented(pst_mac_vap))
            {
                dmac_ie_proc_wide_bandwidth_ie(pst_mac_vap, &puc_frame_body[us_index]);
            }
        }
...
oal_void  dmac_send_2040_coext_mgmt_frame_sta(mac_vap_stru *pst_mac_vap)
{
    ...
    MAC_GET_CB_TX_USER_IDX(pst_tx_ctl) = MAC_INVALID_USER_INDEX;
