...
OAL_STATIC oal_uint32  dmac_config_sta_usr_info_syn(mac_vap_stru *pst_mac_vap, oal_uint8 uc_len, oal_uint8 *puc_param)
{
    ...
    oal_bool_enum_uint8      en_launcher_vap_found = OAL_FALSE;
    ...
    {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_TX, "{dmac_config_sta_usr_info_syn::pst_dmac_dev null.dev id [%d]}", pst_mac_vap->uc_device_id);
        return OAL_ERR_CODE_PTR_NULL;
    }

    if (OAL_TRUE == dmac_user_check_rsp_soft_ctl(pst_mac_vap, pst_mac_user))
    {
        pst_mac_vap->bit_rsp_soft_ctl_launcher = 1;
        if (OAL_FALSE == pst_dmac_dev->en_state_in_sw_ctrl_mode)
        {
            hal_cfg_rsp_dyn_bw(OAL_TRUE, pst_mac_user->en_cur_bandwidth);
    ...
    {
        /* 退出软控模式需要增加多VAP场景判断 */
        if ((OAL_TRUE == pst_dmac_dev->en_state_in_sw_ctrl_mode) && (1 == pst_mac_vap->bit_rsp_soft_ctl_launcher))
        {
            en_launcher_vap_found = mac_vap_search_other_rsp_launcher(pst_mac_vap);
            if (OAL_FALSE == en_launcher_vap_found)
            {
                hal_cfg_rsp_dyn_bw(OAL_FALSE, WLAN_BW_CAP_BUTT);
                pst_mac_vap->bit_rsp_soft_ctl_launcher = 0;
...
oal_uint32 dmac_config_set_ant(mac_vap_stru *pst_mac_vap, oal_uint8 uc_len, oal_uint8 *puc_param)
{
    oal_uint32 ul_result = DUAL_ANTENNA_ALG_CLOSE;
    hal_dual_antenna_switch(*puc_param, 1, &ul_result);
    OAM_WARNING_LOG2(0, OAM_SF_SMART_ANTENNA, "{dmac_config_set_ant::dual_antenna set to %d, result:%d.}",
                    *puc_param, ul_result);
    return ul_result;
}
...
#ifdef _PRE_WLAN_FEATURE_SMARTANT
    {WLAN_CFGID_SET_ANT,              {0, 0},         dmac_config_set_ant},
