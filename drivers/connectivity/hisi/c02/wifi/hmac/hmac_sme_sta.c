...
OAL_STATIC oal_void  hmac_cfg80211_prepare_scan_req_sta(mac_vap_stru *pst_mac_vap, mac_scan_req_stru *pst_scan_params, oal_int8 *puc_param)
{
    ...
    pst_scan_params->uc_scan_channel_interval = MAC_SCAN_CHANNEL_INTERVAL_DEFAULT;                          /* 间隔6个信道，切回工作信道工作一段时间 */
    pst_scan_params->us_scan_work_time_swith_home_channel = MAC_SCAN_WORK_TIME_ON_HOME_CHANNEL_DEFAULT;     /* 背景扫描时，返回工作信道工作的时间 */
    ...
    else if((OAL_SUCC == ul_ret) && (OAL_PTR_NULL != pst_mac_vap_temp) && !IS_LEGACY_VAP(pst_mac_vap_temp))
    {
        /* P2P处于关联状态，wlan 处于去关联状态,wlan每信道扫描时间40ms，每扫描2个信道回主信道60ms */
