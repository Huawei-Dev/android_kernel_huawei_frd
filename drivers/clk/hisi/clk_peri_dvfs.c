					       ...
					       unsigned long parent_rate)
{
	...
	if (IS_ERR_OR_NULL(friend_clk)) {
		/*if friend clk set up after dfclk. then during setup dfclk ,
			the first get dfclk rate would be get failed*/
		pr_info("[%s] %s get failed!\n", __func__, dfclk->link);
		return dfclk->rate;
	}
	pr_info("%s rate=%ld\n", __func__, clk_get_rate_nolock(friend_clk));
	rate = clk_get_rate_nolock(friend_clk);
	return rate;
}
...
#ifdef CONFIG_HISI_PERIDVFS
static int peri_volt_reduce(u32 id, u32 volt)
{
...
static int peri_dvfs_clk_prepare(struct clk_hw *hw)
{
	...
	*/
		...
		ret = peri_volt_reduce(dfclk->id, PERI_VOLT_0);
#endif
        return ret;
}
...
#ifdef CONFIG_HISI_PERIDVFS
	peri_volt_reduce(dfclk->id, PERI_VOLT_0);
#endif
