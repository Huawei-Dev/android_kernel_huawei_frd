...
#ifdef CONFIG_HISI_RTC_SECURE_FEATURE
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 13, 0))
	atfd_hisi_service_rtc_smc(RTC_REGISTER_FN_ID, PMU_RTC_XO_THRESHOLD_SET, ldata->pmu_rtc_xo_thrshd_pwroff_val, 0);
#endif
#else
...
#ifdef CONFIG_HISI_RTC_SECURE_FEATURE
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 13, 0))
	atfd_hisi_service_rtc_smc(RTC_REGISTER_FN_ID, PMU_RTC_WRITE, ldata->pmu_rtc_baseaddr, time);
#endif
#else
	hisi_pmu_rtc_write_bulk(ldata->pmu_rtc_baseaddr + PMU_RTC_LR, time);
#endif
...
static int hisi_rtc_probe(struct platform_device *pdev)
{
    ...
    root = of_find_compatible_node(pdev->dev.of_node, NULL, "hisilicon,soc-rtc");
...
#ifdef CONFIG_HISI_RTC_SECURE_FEATURE
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 13, 0))
    atfd_hisi_service_rtc_smc(RTC_REGISTER_FN_ID, PMU_RTC_ENABLE, prtc->pmu_rtc_baseaddr, 0);
#endif
#else
...
static int hisi_rtc_resume(struct device *dev)
{
	...
	if (device_may_wakeup(dev) && disable_irq_wake(ldata->soc_rtc_irq)) {
		dev_err(&ldata->rtc_dev->dev, "%s: failed to disable irq wake\n", __func__);
	}
