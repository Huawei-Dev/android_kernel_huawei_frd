                                             ...
                                             struct device_attribute *attr, const char *buf, size_t count)
{
     ...
     if(offset > 0 && cam_id < CAMERAFS_ID_MAX) {
...
static ssize_t hw_sensor_thermal_meter_show(struct device *dev,
                                            struct device_attribute *attr, char *buf)
{
     int ret = -1;
     ret = scnprintf(buf, PAGE_SIZE, "%d %d %d", thermal_meter[0], thermal_meter[1], thermal_meter[2]);
     return ret;
}

static struct device_attribute sensor_thermal_meter =
     __ATTR(thermal_meter, 0664, hw_sensor_thermal_meter_show, hw_sensor_thermal_meter_store);
#endif
...
static int __init camerafs_module_init(void)
{
	...
	if (IS_ERR(camerafs.pDevice)) {
        ...
        register_camerafs_attr(&sensor_thermal_meter);
#endif
