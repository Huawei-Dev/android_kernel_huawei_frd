...
static int vl53l0_gal_get_dt_data(struct platform_device *pdev, hw_vl53l0_t *laser)
{
    ...
    /* get ldo */
    laser_info->ldo_num = of_property_count_strings(of_node, "huawei,ldo-names");
    if(laser_info->ldo_num < 0) {
        cam_err("%s failed %d\n", __func__, __LINE__);
        goto fail;
    }
    cam_debug("ldo num = %d", laser_info->ldo_num);
    ...
    hwlaser_config_data_t* data)
{
    ...
    case HWCAM_LASER_POWERON:
        for (index = 0; index < s->laser_info->ldo_num; index++) {
            rc = regulator_set_voltage(s->laser_info->ldo[index].consumer, VAL_LASER_3V3, VAL_LASER_3V3);
            if (rc < 0) {
                cam_err("failed to set ldo[avdd] to %dV", VAL_LASER_3V3);
                return rc;
            }

            rc = regulator_bulk_enable(1, &s->laser_info->ldo[index]);
            if (rc < 0) {
                cam_err("failed to enable regulator %l", rc);
                return rc;
            }
        }
    ...
    case HWCAM_LASER_POWEROFF:
        ...
        for (index = 0; index < s->laser_info->ldo_num; index++) {
            rc = regulator_bulk_disable(1, &s->laser_info->ldo[index]);
            if (rc < 0) {
                cam_err("fail disable regulator %l\n", rc);
                return rc;
            }
        }
