...
int hw_sensor_power_up(sensor_t *s_ctrl)
{
    ...
    if (s_ctrl->p_atpowercnt){
        if (atomic_read(s_ctrl->p_atpowercnt)) {
            cam_info("%s (%d): sensor has already powered up. powercnt is %d", __func__, __LINE__,s_ctrl->p_atpowercnt->counter);
            return 0;
        }
    }else{
    ...
    if (s_ctrl->p_atpowercnt){
        ...
        cam_info("%s (%d): sensor  powered up finish powercnt is %d", __func__, __LINE__,s_ctrl->p_atpowercnt->counter);
    }else{
        atomic_set(&s_powered, 1);
        cam_info("%s (%d): sensor  powered up finish powercnt is %d", __func__, __LINE__,s_powered.counter);
    }
	return rc;
}
