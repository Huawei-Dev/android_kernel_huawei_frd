...
static int hisp150_get_a7isp_addr(void *cfg)
{
    ...
    s_hisp150.handle = hdl;
    table = ion_sg_table(s_hisp150.ion_client, s_hisp150.handle);
    if (IS_ERR_OR_NULL(table)) {
        cam_err("%s Failed : ion_sg_table.%d\n", __func__, PTR_ERR(table));
...
/*
 * the chip issue: Need keep isp aclk enable while ISP is accessing DDRC.
 * Enable aclk before ISP power on, diable it after ISP power off.
 */

static int hisp150_dependent_clock_enable(hisp150_t *hi)
{
	int rc = 0;

	cam_info("%s: enter. Enable aclk", __func__);

	rc = clk_prepare(hi->dt.aclk);
	if (rc) {
		cam_err("%s: isp aclk prepare failed.", __func__);
	}

	rc = clk_enable(hi->dt.aclk);
	if (rc) {
		cam_err("%s: isp aclk enable failed.", __func__);
	}

	return rc;
}

static int hisp150_dependent_clock_disable(hisp150_t *hi)
{
	cam_info("%s: enter. Disable aclk.", __func__);

	clk_disable(hi->dt.aclk);
	clk_unprepare(hi->dt.aclk);
...
static int hisp150_power_on(hisp_intf_t *i)
{
	...
	if (!atomic_read((&hi->opened))) {
		if (!hw_is_fpga_board()) {
				hisp150_dependent_clock_enable(hi);
				clock_enable = true;
...
FAILED_RET:
	...
	if(clock_enable){
		hisp150_dependent_clock_disable(hi);
	}
