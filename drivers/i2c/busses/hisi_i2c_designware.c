...
cs_gpio_err:
	...
	if (r) {
		dev_info(&pdev->dev, "node %s doesn't have clock-rate property!\n",
				pdev->dev.of_node->name);
	} else {
	...
	r = of_property_read_u32(pdev->dev.of_node, "speed-mode", &speed_mode);
	if (r) {
		dev_info(&pdev->dev, "no speed-mode property, use default!\n");
		...
		dev_info(&pdev->dev, "speed mode is %d!\n", speed_mode);
	}
