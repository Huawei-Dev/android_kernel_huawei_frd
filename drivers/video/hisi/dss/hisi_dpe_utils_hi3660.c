...
int set_dss_clk_rate(struct hisi_fb_data_type *hisifd, dss_clk_rate_t dss_clk_rate)
{
	...
	if ((dss_clk_rate.dss_pri_clk_rate != DEFAULT_DSS_CORE_CLK_08V_RATE) && (dss_clk_rate.dss_pri_clk_rate != DEFAULT_DSS_CORE_CLK_07V_RATE)) {
		HISI_FB_ERR("no support set dss_pri_clk_rate(%llu)!\n", dss_clk_rate.dss_pri_clk_rate);
		return -1;
	}

	if (dss_clk_rate.dss_pri_clk_rate == hisifd->dss_clk_rate.dss_pclk_dss_rate) {
		return ret;
	}
	...
	hisifd->dss_clk_rate.dss_pclk_dss_rate = dss_clk_rate.dss_pri_clk_rate;
