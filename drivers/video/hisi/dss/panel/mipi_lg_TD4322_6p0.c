...
static struct gpio_desc lcd_gpio_normal_cmds_sub1[] = {
	...
	{DTYPE_GPIO_OUTPUT, WAIT_TYPE_MS, 3,
		GPIO_LCD_RESET_NAME, &gpio_lcd_reset, 0},
	{DTYPE_GPIO_OUTPUT, WAIT_TYPE_MS, 160,
		GPIO_LCD_RESET_NAME, &gpio_lcd_reset, 1},
};
	...
	char *buf)
{
	...
	j = 0;
	for (i = j; i<ARRAY_SIZE(lg_display_bist_check_cmds); i++) {
		HISI_FB_INFO("TEST %d\n", i+1);
		mipi_dsi_cmds_tx(lg_display_bist_check_cmds[i], \
	...
	mipi_dsi_cmds_tx(lg_display_bist_check_end, \
		ARRAY_SIZE(lg_display_bist_check_end), mipi_dsi0_base);
...
static int mipi_lg_probe(struct platform_device *pdev)
{
	...
	pinfo->lcd_uninit_step_support = 0;
