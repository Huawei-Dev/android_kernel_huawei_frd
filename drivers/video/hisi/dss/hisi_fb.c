...
static int hisifb_dss_dirty_region_info_get(struct fb_info *info, void __user *argp)
{
	struct hisi_fb_data_type *hisifd = NULL;
	struct hisi_panel_info *pinfo = NULL;
	...
	pinfo->dss_region_info.xres = pinfo->xres;
	pinfo->dss_region_info.yres = pinfo->yres;
	pinfo->dss_region_info.lcd_xres = pinfo->xres;
	pinfo->dss_region_info.lcd_yres = pinfo->yres;
	pinfo->dss_region_info.arsr1p_used = 0;

	if (copy_to_user(argp, &(hisifd->panel_info.dss_region_info),
		sizeof(struct dss_arsr1p_info))) {
		HISI_FB_ERR("copy to user fail");
		return -EFAULT;
	}
...
static int hisi_fb_ioctl(struct fb_info *info, unsigned int cmd, unsigned long arg)
{
	...
	case HISIFB_DSS_ARSR1P_INFO_GET:
		ret = hisifb_dss_dirty_region_info_get(info, argp);
		break;
