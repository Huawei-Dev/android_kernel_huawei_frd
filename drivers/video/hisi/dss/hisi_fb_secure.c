...
static void hisifd_secure_layer_config(struct hisi_fb_data_type *hisifd, dss_layer_t *layer)
{
	...
	HISI_FB_DEBUG("chn_idx = %d, frame_no = %d \n", layer->chn_idx, hisifd->ov_req.frame_no);
	dss_base = hisifd->dss_base_phy;
	secure_ctrl->set_reg(dss_base + g_dss_module_base[layer->chn_idx][MODULE_DMA] + CH_SECU_EN, 0x1, 1, 0);
}
static void hisifd_secure_layer_deconfig(struct hisi_fb_data_type *hisifd, dss_layer_t *layer)
{
	...
	HISI_FB_DEBUG("chn_idx = %d, frame_no = %d \n", layer->chn_idx, hisifd->ov_req.frame_no);
	dss_base = hisifd->dss_base_phy;
	secure_ctrl->set_reg(dss_base + g_dss_module_base[layer->chn_idx][MODULE_DMA] + CH_SECU_EN, 0x0, 1, 0);
}
...
void hisifd_secure_ch_default_config(struct hisi_fb_data_type *hisifd)
{
	...
	for (ch_index = 0; ch_index < DSS_CHN_MAX-1; ch_index++) {
		...
		if ((secure_ctrl->set_reg != NULL) && (module_base != 0) && (dss_base != 0)) {
