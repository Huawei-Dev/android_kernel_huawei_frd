		...
		for (i = 0; i < pov_h_block->layer_nums; i++) {
			...
			if (layer->img.secure_mode == 1) {
				if (secure_ctrl->secure_layer_deconfig) {
					secure_ctrl->secure_layer_deconfig(secure_ctrl->hisifd, layer);
				}
			}
		}
		if (wb_layer4block->dst.secure_mode == 1) {
			HISI_FB_DEBUG("hisifb%d wb_layer.ch_indx = %d, frame_no = %d\n", hisifd->index, wb_layer4block->chn_idx, hisifd->ov_req.frame_no);
			secure_ctrl->set_reg(secure_ctrl->hisifd->dss_base_phy + g_dss_module_base[wb_layer4block->chn_idx][MODULE_DMA] \
				+ CH_SECU_EN, 0x0, 1, 0);
		}
	}
		...
		for (i = 0; i < pov_h_block->layer_nums; i++) {
			...
			if (layer->img.secure_mode == 1) {
				if (secure_ctrl->secure_layer_config) {
					secure_ctrl->secure_layer_config(secure_ctrl->hisifd, layer);
				}
			}
		}
		if (wb_layer4block->dst.secure_mode == 1) {
...
REDO:
	...
	if ((ret <= 0) ||(ret_cmdlist_state < 0)) {
		...
		hisi_remove_mctl_mutex(hisifd, pov_req, mctl_idx, cmdlist_idxs);
	#endif
