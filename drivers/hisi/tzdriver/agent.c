...
int TC_NS_set_nativeCA_hash(unsigned long arg)
{
	...
	if (buflen > SZ_4K) {
		TCERR("ERROR: file size[0x%x] too big\n", buflen);
		return -1;
	}

	buftotee = kzalloc(SZ_4K, GFP_KERNEL);
	if (NULL == buftotee)
		return -1;
...
int TC_NS_unregister_agent(unsigned int agent_id)
{
	...
	if (agent_id == AGENT_FS_ID || agent_id == AGENT_MISC_ID ||
	    ...
	    TCERR("system agent is not allowed to unregister\n");
	    return TEEC_ERROR_GENERIC;
	}
...
int TC_NS_alloc_exception_mem(void)
{
	...
	exception_mem = kzalloc(EXCEPTION_MEM_SIZE, GFP_KERNEL);
	if (!exception_mem)
		return TEEC_ERROR_GENERIC;
	...
	operation.params[0].value.b= virt_to_phys(exception_mem) >> 32;
	operation.params[1].value.a = EXCEPTION_MEM_SIZE;
...
static int def_tee_agent_run(struct tee_agent_kernel_ops *agent_instance)
{
	...
	agent_instance->agent_thread =
		kthread_run(def_tee_agent_work, agent_instance, "agent_%s",
			agent_instance->agent_name);
