...
void TEEK_Encode(TC_NS_ClientContext *cli_context,
		...
		if ((param_type[param_cnt] == TEEC_MEMREF_TEMP_INPUT)
			...
			if (param_type[param_cnt] == TEEC_MEMREF_WHOLE) {
				...
				cli_context->params[param_cnt].memref.
				    size_addr =
				    (char *)&operation->params[param_cnt].
				    memref.parent->size;
			} else {
				cli_context->params[param_cnt].memref.offset
				    =
			...
			if (operation->params[param_cnt].memref.parent->
			    is_allocated) {
				cli_context->params[param_cnt].memref.buffer =
				    (char *)operation->params[param_cnt].memref.
				    parent->buffer;
			} else {
				cli_context->params[param_cnt].memref.buffer
				    =
			...
			cli_context->params[param_cnt].value.a_addr
				= (uint32_t *)&operation->params[param_cnt].ionref.ion_share_fd;
			cli_context->params[param_cnt].value.b_addr
				= (uint32_t *)&operation->params[param_cnt].ionref.ion_size;
		}
	}
			     ...
			     uint32_t *returnOrigin)
{
	...
	case TEEC_LOGIN_IDENTIFY:
		...
		if (operation->params[3].tmpref.size > MAX_PACKAGE_NAME_LEN) {
			...
			memcpy(dev_file->pkg_name,
			       operation->params[3].tmpref.buffer,
			       operation->params[3].tmpref.size);
		}
        dev_file->pub_key_len = operation->params[2].tmpref.size;
        if(dev_file->pub_key_len > 0 && dev_file->pub_key_len < MAX_PUBKEY_LEN )
        {
            memcpy(dev_file->pub_key,
