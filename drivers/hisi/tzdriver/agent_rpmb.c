...
static int rpmb_agent_open_blkdev(struct tee_agent_kernel_ops *agent_instance,
				  int stage)
{
	struct block_device *bdev;
	 if (agent_instance->agent_data)
		return 0;
	 bdev = blkdev_get_by_path(RPMB_DEVICE_NAME,
...
{
	/*dont care the return value*/
	rpmb_agent_open_blkdev(agent_instance, 0);
	return 0;
}
...
static void get_devinfo(struct tee_agent_kernel_ops *agent_instance)
{
	...
	TCDEBUG("raw_rpmb_size_mult=0x%x, rel_wr_sec_cnt=0x%x\n",
	    ...
	    snprintf(trans_ctrl->args.get_devinfo.cid,
		     sizeof(trans_ctrl->args.get_devinfo.cid) + 1,
	    ...
	    trans_ctrl->args.get_devinfo.rel_wr_sec_cnt =
	    card->ext_csd.rel_sectors;
	 trans_ctrl->ret = 0;
}
...
static void update_dataptr(struct rpmb_ctrl_t *trans_ctrl)
{
	...
	for (i = 0; i < MMC_IOC_MAX_RPMB_CMD; i++) {
		...
		if (trans_ctrl->args.send_ioccmd.ioc_rpmb.cmds[i].data_ptr) {
			dst = (uint8_t *)
			trans_ctrl->args.send_ioccmd.buf + offset;
			datasize = trans_ctrl->args.send_ioccmd.
			ioc_rpmb.cmds[i].blksz *
			    trans_ctrl->args.send_ioccmd.ioc_rpmb.cmds[i].
			    blocks;
			/*update the data_ptr*/
			mmc_ioc_cmd_set_data(trans_ctrl->args.send_ioccmd.
					     ioc_rpmb.cmds[i], dst);
		}
	}
}
...
static void send_ioccmd(struct tee_agent_kernel_ops *agent_instance)
{
	...
	bdev = (struct block_device *)agent_instance->agent_data;
	 ...
	 rpmb_data =
	    kzalloc(sizeof(struct mmc_blk_ioc_rpmb_data), GFP_KERNEL);
	if (rpmb_data == NULL) {
	...
	ioc_rpmb = (struct mmc_ioc_rpmb *)&trans_ctrl->args.send_ioccmd;
		for (i = 0; i < MMC_IOC_MAX_RPMB_CMD; i++) {
			rpmb_data->data[i].buf =
				(unsigned char *)ioc_rpmb->cmds[i].data_ptr;
			rpmb_data->data[i].buf_bytes =
			ioc_rpmb->cmds[i].blocks * ioc_rpmb->cmds[i].blksz;
			memcpy(&rpmb_data->data[i].ic, &ioc_rpmb->cmds[i],
