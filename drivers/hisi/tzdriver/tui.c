...
/* ms  modify to  ten minutes for test */
#define TUI_DEFAULT_TIMEOUT	30000
...
int send_tui_msg_adapt(int type, int val, void *data)
{
	if (atomic_read(&tui_state) == TUI_STATE_UNUSED) {
		...
		/*send_tui_msg_config(type, 0, NULL);*/
	return tui_send_event(type);
}
				 ...
				 loff_t *ppos)
{
	...
	if (event_type == TUI_POLL_TP) {
		...
		tui_ctl->n2s.status = kstrtol(tokens, &endptr, base);
		tokens = strsep(&begins, ":");
		tui_ctl->n2s.x = kstrtol(tokens, &endptr, base);
		tokens = strsep(&begins, ":");
		tui_ctl->n2s.y = kstrtol(tokens, &endptr, base);
	}
