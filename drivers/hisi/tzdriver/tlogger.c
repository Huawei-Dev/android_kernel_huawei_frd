...
static size_t addr_start;
static size_t addr_end;
#define PER_LEN 1024
...
#define CHECK_BOUNDARY(X) \
	((((X) < addr_start) || ((X) > addr_end)) ? true : false)
...
start:
	...
	if (ring_flag != reader->r_ver /*ring_flag not same */) {
		if ((reader->r_off > log->w_off)
			...
			sub = (addr_end - reader->r_off) +
			    (log->w_off - addr_start);
		} else {
			reader->r_off = log->w_off;
			sub =
			    (addr_end - reader->r_off) + (log->w_off -
							  addr_start);
		}
		disjoin_flag = true;
	} else {
	...
	if (disjoin_flag && ((reader->r_off + log_len) > addr_end)) {
		...
		ret =
		    copy_to_user(buf, (void *)(reader->r_off),
				 (addr_end - reader->r_off));
		TLOGEBUG("read (addr_end-reader->r_off)=%llu ret = %llu\n",
			 (addr_end - reader->r_off), ret);

		ret =
		    copy_to_user(buf, (void *)(addr_start),
				 (log_len - (addr_end - reader->r_off)));

		reader->r_ver = ring_flag;
	} else {
		if (CHECK_BOUNDARY(reader->r_off)
		    || CHECK_BOUNDARY(reader->r_off + log_len)) {
			TLOGERR("2 CHECK_BOUNDARY error\n");
		}

		if (log_len)
			ret =
			    copy_to_user(buf, (void *)(reader->r_off), log_len);
	...
	if (disjoin_flag && ((reader->r_off + log_len) > addr_end))
		reader->r_off =
		    addr_start + (log_len - (addr_end - reader->r_off));
	else
...
static int tee_create_dir(char *path)
{
	...
	if (0 != fd) {
		...
		ret =
		    (int)sys_chown((const char __user *)path, ROOT_UID,
				   SYSTEM_GID);
		if (ret)
...
int tlogger_store_lastmsg(void)
{
	...
	ret =
	    (int)sys_chown((const char __user *)LOG_PATH_TEE_LOG_FILE, ROOT_UID,
			   SYSTEM_GID);
	if (ret)
		TLOGERR("Failed to chown last_teemsg\n");

	write_len =
	    vfs_write(filep, (char __user *)tmp_log_buffer,
		      RDR_MEM_LEN - OFFSET2, &pos);
	if (write_len < 0) {
...
static int __init tlogger_init(void)
{
	...
	real_off = ADDR_P;
	addr_offset = (size_t) (log_buffer - (char *)real_off);
