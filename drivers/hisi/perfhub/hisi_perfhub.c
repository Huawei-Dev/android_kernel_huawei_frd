...
static int bind_cpu_cluster(enum cpu_cluster e_cpu_cluster, pid_t pid)
{
	...
	case CPU_CLUSTER_LITTLE:
		for (cpu_no = LITTLE_CPU_START; cpu_no < BIG_CPU_START; cpu_no++) {
			cpumask_set_cpu(cpu_no, &mask);
		}
		break;
	case CPU_CLUSTER_BIG:
		for (cpu_no = BIG_CPU_START; cpu_no < CPU_TOTAL; cpu_no++) {
			cpumask_set_cpu(cpu_no, &mask);
		}
		break;
	default:
		for (cpu_no = LITTLE_CPU_START; cpu_no < CPU_TOTAL; cpu_no++) {
			cpumask_set_cpu(cpu_no, &mask);
		}
		break;
	}
...
{
	if (NULL == buf){
		pr_err("perfhub_show buf is NULL");
		return -EINVAL;
	}
...
static ssize_t perfhub_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if (NULL == buf){
		pr_err("perfhub_store buf is NULL");
		return -EINVAL;
	}
