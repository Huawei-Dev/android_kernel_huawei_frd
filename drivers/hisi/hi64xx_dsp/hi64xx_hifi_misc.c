...
#define HI64XX_TCM_FILE_NAME	"codec_tcm.bin"
#define HI64XX_OCRAM_FILE_NAME	"codec_ocram.bin"
				  ...
				  const unsigned int len)
{
	...
	hi64xx_write(dsp_priv->dsp_config.para_addr, (const unsigned int*)arg, len);
...
static int hi64xx_write_msg(const unsigned char *arg, const unsigned int len)
{
	...
	hi64xx_write(dsp_priv->dsp_config.msg_addr, (const unsigned int*)arg, len);
...
static void hi64xx_dsp_stop(void)
{
	...
	if (dsp_priv->dsp_is_running) {
		if (dsp_priv->dsp_config.dsp_ops.clear_msg_sn)
			dsp_priv->dsp_config.dsp_ops.clear_msg_sn();
...
static int create_dir(const char *path)
{
	...
	if (bbox_chown((const char *)path, ROOT_UID, SYSTEM_GID, false)) {
		HI64XX_DSP_ERROR("chown %s dir failed\n", path);
	}
...
static void hi64xx_hifi_dump_all_ram(char *pathname)
{
	int ret = 0;
	char * buf = NULL;
	unsigned int buf_size = 0;
	char ocram_file[DUMP_DIR_LEN] = {0};
	char tcm_file[DUMP_DIR_LEN] = {0};

	if (0 == dsp_priv->dsp_config.ocram_start_addr
		|| 0 == dsp_priv->dsp_config.itcm_start_addr
		|| 0 == dsp_priv->dsp_config.dtcm_start_addr) {
		HI64XX_DSP_ERROR("ram addr is not registe, do not dump\n");
		return;
	}

	buf_size = dsp_priv->dsp_config.ocram_size > (dsp_priv->dsp_config.dtcm_size \
		+ dsp_priv->dsp_config.itcm_size) ? dsp_priv->dsp_config.ocram_size : (dsp_priv->dsp_config.dtcm_size \
		+ dsp_priv->dsp_config.itcm_size);
	...
	hi64xx_misc_dump_bin(dsp_priv->dsp_config.ocram_start_addr, buf, dsp_priv->dsp_config.ocram_size);
	HI64XX_DSP_INFO("dump ocram end\n");

	snprintf(ocram_file, sizeof(ocram_file), "%s%s", pathname, HI64XX_OCRAM_FILE_NAME); /* [false alarm] */
	ret = rdr_audio_write_file(ocram_file, buf, dsp_priv->dsp_config.ocram_size);
	if (ret)
		HI64XX_DSP_ERROR("write ocram file fail, ret:%d\n", ret);

	memset(buf, 0, buf_size);
	hi64xx_misc_dump_bin(dsp_priv->dsp_config.itcm_start_addr, buf, dsp_priv->dsp_config.itcm_size);
	HI64XX_DSP_INFO("dump itcm end\n");
	hi64xx_misc_dump_bin(dsp_priv->dsp_config.dtcm_start_addr, buf + dsp_priv->dsp_config.itcm_size, \
		dsp_priv->dsp_config.dtcm_size);
	HI64XX_DSP_INFO("dump dtcm end\n");

	snprintf(tcm_file, sizeof(tcm_file), "%s%s", pathname, HI64XX_TCM_FILE_NAME); /* [false alarm] */
	ret = rdr_audio_write_file(tcm_file, buf, dsp_priv->dsp_config.itcm_size + dsp_priv->dsp_config.dtcm_size);
	if (ret)
		HI64XX_DSP_ERROR("write tcm file fail, ret:%d\n", ret);
...
static void hi64xx_hifi_dump(unsigned int dump_type)
{
	...
	if (HIFI_DUMP_TYPE_LOG_BIN == dump_type) {
		/* for debug purpose,  use "rdr_system_error" if there's a real error */
		rdr_audio_codec_dump(~0, dirname, NULL);
	} else if (HIFI_DUMP_TYPE_ALL_RAM == dump_type) {
		hi64xx_hifi_dump_all_ram(dirname);
	} else {
		HI64XX_DSP_ERROR("dump type is invalid, dump_type:%d\n", dump_type);
	}
#endif
}
...
static int hi64xx_func_fw_download(struct krn_param_io_buf *param)
{
	...
	if (wait_event_interruptible_timeout(dsp_priv->dsp_pwron_wq,
		...
		if (dsp_priv->dsp_config.run_state_addr)
			HI64XX_DSP_ERROR("dsp running state:0x%x\n", hi64xx_hifi_read_reg(dsp_priv->dsp_config.run_state_addr));
		hi64xx_dump_debug_info();

		/* can't get codec version,reset system */
		BUG_ON(hi64xx_error_detect());
		/* after retry 3 times, reset system */
		if (RELOAD_RETRY_MAX == reload_retry_count) {
			HI64XX_DSP_ERROR("codecdsp reload fail, dump all ocram and tcm, then reboot\n");
			hi64xx_hifi_dump(HIFI_DUMP_TYPE_ALL_RAM);
			BUG_ON(1);
		}
...
static int hi64xx_func_pwr_test(struct krn_param_io_buf *param)
{
	int ret = OK;
	DSP_IF_OPEN_REQ_STRU *dsp_pwr_test_req =
	...
	switch (dsp_pwr_test_req->uhwMsgId) {
	case ID_AP_DSP_PWR_TEST_WFI_IN:
		hi64xx_request_pll_resource(HI_FREQ_SCENE_PWR_TEST);
		ret = hi64xx_sync_write(param->buf_in, param->buf_size_in);
		break;
	case ID_AP_DSP_PWR_TEST_WFI_OUT:
		ret = hi64xx_sync_write(param->buf_in, param->buf_size_in);
		msleep(100);
		hi64xx_release_pll_resource(HI_FREQ_SCENE_PWR_TEST);
		break;
	case ID_AP_DSP_PWR_TEST_WFI_LOWPLL_IN:
		hi64xx_request_low_pll_resource(LOW_FREQ_SCENE_PWR_TEST);
		ret = hi64xx_sync_write(param->buf_in, param->buf_size_in);
		break;
	case ID_AP_DSP_PWR_TEST_WFI_LOWPLL_OUT:
		ret = hi64xx_sync_write(param->buf_in, param->buf_size_in);
		msleep(100);
		hi64xx_release_low_pll_resource(LOW_FREQ_SCENE_PWR_TEST);
		break;
	default:
		break;
	}
...
static struct cmd_func_pair sync_cmd_func_map[] = {
	...
	{ ID_AP_DSP_PWR_TEST_WFI_IN, hi64xx_func_pwr_test,	 "ID_AP_DSP_PWR_TEST_WFI_IN" },
	{ ID_AP_DSP_PWR_TEST_WFI_OUT,hi64xx_func_pwr_test,	 "ID_AP_DSP_PWR_TEST_WFI_OUT"},
	{ ID_AP_DSP_PWR_TEST_WFI_LOWPLL_IN, hi64xx_func_pwr_test,	 "ID_AP_DSP_PWR_TEST_WFI_LOWPLL_IN" },
	{ ID_AP_DSP_PWR_TEST_WFI_LOWPLL_OUT,hi64xx_func_pwr_test,	 "ID_AP_DSP_PWR_TEST_WFI_LOWPLL_OUT"},
				...
				size_t size, loff_t *data)
{
	...
	case HI64XX_HIFI_DUMP_ONLY_LOG:
		hi64xx_hifi_dump(HIFI_DUMP_TYPE_LOG_BIN);
		break;
	/* dump the whole of ocram and tcm */
	case HI64XX_HIFI_DUMP_ALL_RAM:
		hi64xx_hifi_dump(HIFI_DUMP_TYPE_ALL_RAM);
		break;
	default:
		break;
...
void hi64xx_misc_dump_bin(const unsigned int addr, char *buf, const size_t len)
{
	...
	HI64XX_DSP_INFO("start read mem\n");
	hi64xx_read(addr, buf, len);
	HI64XX_DSP_INFO("end read mem\n");
				...
				struct hi64xx_dsp_config *dsp_config)
{
	...
	if (ret) {
		HI64XX_DSP_ERROR("misc register failed \n");
	}
