...
#include <asm/unistd.h>//lint !e451
		  ...
		  const unsigned int size)
{
	...
	operation.params[0].value.a = image;
	operation.params[0].value.b = run_addr;
					...
					u32 vrl_type)
{
	...
	switch (vrl_type) {
	case PrimVRL:
		strncpy(image_name, PART_VRL, PART_NAMELEN);
		image_name[PART_NAMELEN - 1] = '\0';
		break;
	case BackVRL:
		strncpy(image_name, PART_VRL_BACKUP, PART_NAMELEN);
		image_name[PART_NAMELEN - 1] = '\0';
		break;
	default:
...
s32 get_imgname_and_headsize(BSP_CORE_TYPE_E ecoretype, char *image_name, u32 *head_size)
{
	...
	{
		case BSP_CCORE:
			strncpy(image_name, PART_MODEM, PART_NAMELEN);
			image_name[PART_NAMELEN - 1] = '\0';
			if (NULL != head_size) {
			...
			break;
		case BSP_HIFI:
			strncpy(image_name, PART_FW_HIFI, PART_NAMELEN);
			image_name[PART_NAMELEN - 1] = '\0';
			if (NULL != head_size) {
			...
			break;
		case BSP_BBE:
			strncpy(image_name, PART_MODEM_DSP, PART_NAMELEN);
			image_name[PART_NAMELEN - 1] = '\0';
			if (NULL != head_size) {
				*head_size = sizeof(struct image_head);
			}
