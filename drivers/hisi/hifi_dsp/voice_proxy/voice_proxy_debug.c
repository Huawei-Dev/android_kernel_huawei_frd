...
static int32_t debug_snd_cmd_list_node(uint16_t msg_id)
{
	...
	BUG_ON(ID_PROXY_VOICE_LTE_RX_NTF > msg_id);
	BUG_ON(ID_PROXY_VOICE_DATA_MSGID_BUT < msg_id);
...
static int32_t debug_read_mailbox_msg(void *command, int8_t *buf, int32_t *size)
{
	...
	case ID_VOICE_PROXY_WIFI_TX_NTF:
		...
		if (node) {
			...
			wifi_rx_notify = (struct voice_proxy_wifi_rx_notify *)node->list_data.data;
			...
			memcpy(wifi_tx_notify.data, wifi_rx_notify->data, wifi_rx_notify->data_len);
	...
	memcpy(buf, mail_buf, mail_buf_len);
	*size = mail_buf_len;
...
static int32_t debug_mailbox_thread(void *arg)
{
	...
	while (!kthread_should_stop()) {
		...
		if (likely(s_priv.debug_mailbox_wait_flag> 0)) {
			s_priv.debug_mailbox_wait_flag--;
		}
...
static int32_t debug_hifi_thread(void *arg)
{
	while (!kthread_should_stop()) {
		...
		msleep(s_priv.debug_hifi_sleep_time);
	}
...
static int32_t debug_tfagent_handle_msg(struct send_tfagent_data *volte_buf)
{
	...
	if ((s_priv.debug_tf_lost_enable) &&
	    (0 == (buf.id % s_priv.debug_tf_lost_index))) {
...
static int32_t debug_tfagent_thread(void *arg)
{
	...
	while (!kthread_should_stop()) {
		size = proxy_pull_data((int8_t *)&buf, sizeof(buf));
		loge("debug_tfagent_thread\n");
		loge("debug_test_buf %d, %s, %s", __LINE__, __FUNCTION__, __FILE__);
		debug_test_buf((int8_t*)&buf, sizeof(buf));
		if (size == sizeof(buf))
...
static const struct class_attribute debug_attrs[] = {
	__ATTR(ssw, 0664, NULL, debug_ssw),
	__ATTR(paraw, 0664, NULL, debug_paraw),
};
...
static int32_t debug_create_attrs(struct class *class)
{
	...
	for (i=0; i < (sizeof(debug_attrs)/sizeof(struct class_attribute)); i++) {
