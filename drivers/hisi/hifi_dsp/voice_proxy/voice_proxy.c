...
int32_t voice_proxy_mailbox_send_msg_cb(uint32_t mailcode, uint16_t msg_id, void *buf, uint32_t size)
{
	...
	ret = mailbox_send_msg(mailcode, buf, size);
	if (ret) {
...
static int32_t read_mailbox_msg_data_cb(void *mail_handle, int8_t *buf, int32_t *size)
{
	...
	BUG_ON(NULL == mail_handle);
	BUG_ON(NULL == buf);
	BUG_ON(NULL == size);

	ret = mailbox_read_msg_data(mail_handle, (char *)buf, (unsigned int *)size);
	if (ret) {
 ...
 * from callback.
 */
static int32_t write_thread_get_data(int8_t *data, int32_t *size, uint16_t *msg_id)
{
        int32_t ret = 0;
        int32_t i;
	struct voice_proxy_cmd_node *command = NULL;
...
static int voice_proxy_write_thread(void *arg)
{
	...
	int8_t *data = (int8_t *)kzalloc(size, GFP_ATOMIC);
...
static void handle_mail(void *usr_para, void *mail_handle, uint32_t mail_len)
{
	...
	rev_msg->mail_buf = kzalloc(VOICE_PROXY_LIMIT_PARAM_SIZE, GFP_ATOMIC);
	if (NULL == rev_msg->mail_buf) {
		loge("malloc mail_buf failed\n");
		goto ERR;
	}

	memset(rev_msg->mail_buf, 0, VOICE_PROXY_LIMIT_PARAM_SIZE);
	...
	if (rev_msg->mail_buf_len > mail_len) {
		loge("ReadMailData mail_size(%d) > mail_len(%d)\n", rev_msg->mail_buf_len, mail_len);
		goto ERR;
	}

	msg_para = (struct hifi_msg_para *)rev_msg->mail_buf;
	...
	for (i = 0; i < priv.msg_callbacks_size; i++) {
		if (message_callbacks[i].msg_id == msg_id) {
			message_callbacks[i].callback((int8_t *)rev_msg->mail_buf, rev_msg->mail_buf_len);
...
static int32_t register_mailbox_msg_cb(mb_msg_cb callback)
{
	...
	ret = mailbox_reg_msg_cb(MAILBOX_MAILCODE_HIFI_TO_ACPU_VOICE, callback, NULL);
	if (ret) {
