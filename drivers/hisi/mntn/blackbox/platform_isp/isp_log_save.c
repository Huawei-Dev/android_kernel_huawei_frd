...
static int isp_log_open(struct inode *inode, struct file *filp)
{
	...
	pr_info("[%s] usesecisp.%d, share_para.0x%p = 0x%p\n", __func__,
		usesecisp(), dev->share_para, dev->share_mem);
...
static int isp_log_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	pr_alert("%s: enter, paddr = 0x%llx, vaddr.0x%lx, size.0x%lx.(0x%lx - 0x%lx), _prot0x%lx\n",
