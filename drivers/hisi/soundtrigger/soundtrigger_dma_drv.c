...
DRV_DMA_CONFIG_STRU hi6403_soundtrigger_dma_fast_cfg[2] = {
	{.port = 0xe8051280, .config = 0x433220a7, .channel = 7},	/*hi6403 fast data left channel*/
	{.port = 0xe80512c0, .config = 0x43322077, .channel = 8},	/*hi6403 fast data right channel*/
};

DRV_DMA_CONFIG_STRU hi6403_soundtrigger_dma_normal_cfg[2] = {
	{.port = 0xe8051080, .config = 0x43322027, .channel = 3},	/*hi6403 normal data left channel*/
	{.port = 0xe80510c0, .config = 0x433220b7, .channel = 4},	/*hi6403 normal data right channel*/
};

DRV_DMA_CONFIG_STRU hi6402_soundtrigger_dma_fast_cfg[2] = {
	{.port = 0xe8051180, .config = 0x43322067, .channel = 7},	/*hi6402 fast data left channel*/
	{.port = 0xe80511c0, .config = 0x43322077, .channel = 8},	/*hi6402 fast data right channel*/
};

DRV_DMA_CONFIG_STRU hi6402_soundtrigger_dma_normal_cfg[2] = {
	{.port = 0xe8051280, .config = 0x433220a7, .channel = 3},	/*hi6402 normal data left channel*/
	{.port = 0xe80512c0, .config = 0x433220b7, .channel = 4},	/*hi6402 normal data right channel*/
};

struct soundtrigger_pcm_config hi6403_pcm_fast_cfg= {				/*fast trans pcm config*/
...
struct soundtrigger_pcm_config hi6403_pcm_normal_cfg= {				/*normal trans pcm config*/
	.channels = DMA_PORT_NUM,
...
struct soundtrigger_pcm_config hi6402_pcm_fast_cfg= {				/*fast trans pcm config*/
	.channels = DMA_PORT_NUM,
	.rate = FAST_TRAN_RATE,
	.frame_len = FAST_FRAME_LENGTH,
	.byte_count = BYTE_COUNT,
...
struct soundtrigger_pcm_config hi6402_pcm_normal_cfg= {				/*normal trans pcm config*/
	.channels = DMA_PORT_NUM,
	...
	for (pcm_channel = 0; pcm_channel < SOUNDTRIGGER_PCM_CHAN_NUM; pcm_channel++) {
		...
		if (pcm_channel == SOUNDTRIGGER_PCM_FAST) {
			if (CODEC_HI6402 == dma_drv_info->type) {
				memcpy(&(pcm_info->pcm_cfg), &hi6402_pcm_fast_cfg, sizeof(struct soundtrigger_pcm_config));
				for (dma_num = 0; dma_num < DMA_PORT_NUM; dma_num++) {
					pcm_info->channel[dma_num] = hi6402_soundtrigger_dma_fast_cfg[dma_num].channel;
				}
			} else {
				memcpy(&(pcm_info->pcm_cfg), &hi6403_pcm_fast_cfg, sizeof(struct soundtrigger_pcm_config));
				for (dma_num = 0; dma_num < DMA_PORT_NUM; dma_num++) {
					pcm_info->channel[dma_num] = hi6403_soundtrigger_dma_fast_cfg[dma_num].channel;
				}
			}
		}
		if (pcm_channel == SOUNDTRIGGER_PCM_NORMAL) {
			if (CODEC_HI6402 == dma_drv_info->type) {
				memcpy(&(pcm_info->pcm_cfg), &hi6402_pcm_normal_cfg, sizeof(struct soundtrigger_pcm_config));
				for (dma_num = 0; dma_num < DMA_PORT_NUM; dma_num++) {
					pcm_info->channel[dma_num] = hi6402_soundtrigger_dma_normal_cfg[dma_num].channel;
				}
			} else {
				memcpy(&(pcm_info->pcm_cfg), &hi6403_pcm_normal_cfg, sizeof(struct soundtrigger_pcm_config));
				for (dma_num = 0; dma_num < DMA_PORT_NUM; dma_num++) {
					pcm_info->channel[dma_num] = hi6403_soundtrigger_dma_normal_cfg[dma_num].channel;
				}
			}
		}
		...
		for (dma_num = 0; dma_num < DMA_PORT_NUM; dma_num++) {
			for (dma_cfg_num = 0; dma_cfg_num < PCM_SWAP_BUFFER_NUM; dma_cfg_num++) {
				if (pcm_channel == SOUNDTRIGGER_PCM_FAST) {
					if (CODEC_HI6402 == dma_drv_info->type) {
						pcm_info->dma_cfg[dma_num][dma_cfg_num]->config = hi6402_soundtrigger_dma_fast_cfg[dma_num].config;
						pcm_info->dma_cfg[dma_num][dma_cfg_num]->src_addr = hi6402_soundtrigger_dma_fast_cfg[dma_num].port;
					} else {
						pcm_info->dma_cfg[dma_num][dma_cfg_num]->config = hi6403_soundtrigger_dma_fast_cfg[dma_num].config;
						pcm_info->dma_cfg[dma_num][dma_cfg_num]->src_addr = hi6403_soundtrigger_dma_fast_cfg[dma_num].port;
					}
				}

				if (pcm_channel == SOUNDTRIGGER_PCM_NORMAL) {
					if (CODEC_HI6402 == dma_drv_info->type) {
						pcm_info->dma_cfg[dma_num][dma_cfg_num]->config = hi6402_soundtrigger_dma_normal_cfg[dma_num].config;
						pcm_info->dma_cfg[dma_num][dma_cfg_num]->src_addr = hi6402_soundtrigger_dma_normal_cfg[dma_num].port;
					} else {
						pcm_info->dma_cfg[dma_num][dma_cfg_num]->config = hi6403_soundtrigger_dma_normal_cfg[dma_num].config;
						pcm_info->dma_cfg[dma_num][dma_cfg_num]->src_addr = hi6403_soundtrigger_dma_normal_cfg[dma_num].port;
					}
				}
...
static int32_t dma_start(struct st_fast_status * fast_status)
{
	...
	if (CODEC_HI6402== dma_drv_info->type) {
		...
		//slimbus_param_set(SLIMBUS_DEVICE_HI6403, SLIMBUS_TRACK_VOICE_UP, &slimbus_params);

		slimbus_params.rate = 192000;
		logi("fast slimbus_params.rate[%d]\n",slimbus_params.rate);
		slimbus_params.channels = 1;
		slimbus_param_set(SLIMBUS_DEVICE_HI6403, SLIMBUS_TRACK_SOUND_TRIGGER, &slimbus_params);
		slimbus_track_configure(SLIMBUS_DEVICE_HI6403, SLIMBUS_TRACK_SOUND_TRIGGER, NULL);
		slimbus_track_activate(SLIMBUS_DEVICE_HI6403, SLIMBUS_TRACK_SOUND_TRIGGER);
	}
...
static int32_t dma_close(struct st_fast_status * fast_status)
{
	...
	if (CODEC_HI6402== dma_drv_info->type) {
		...
		slimbus_track_deactivate(SLIMBUS_DEVICE_HI6403, SLIMBUS_TRACK_SOUND_TRIGGER);
		slimbus_track_remove(SLIMBUS_DEVICE_HI6403, SLIMBUS_TRACK_SOUND_TRIGGER);
	}
