...
static int watermark_num = TRACK_MAX;
...
static int creat_trackerd(int nid)
{
	...
	if (ret >= 32)
		goto err;
	entry = proc_create_data(kbuf, 0664,
...
module_param_array_named(page_tracker_watermark,
	page_tracker_watermark, int, &watermark_num, S_IRUGO | S_IWUSR);
/*lint -e665 +esym(665,__module_param_call)*/
#ifdef PGTRACK_TEST
