                ...
                DICC_CPU_ID_ENUM_UINT8 enCpuId)
{
    ...
    for (usNodeLoop = 0; usNodeLoop < pstDiccChnCtrl->usChnBlkNum; usNodeLoop++)
    {
        ...
        /* 核间传递结点内存清零 */
        PSACORE_MEM_SET(pstPerNodeAddr->aucPriData, sizeof(pstPerNodeAddr->aucPriData), 0x0, sizeof(pstPerNodeAddr->aucPriData));
               ...
               DICC_CPU_ID_ENUM_UINT8 enCpuId)
{
    ...
    PSACORE_MEM_SET((VOID *)&stSyncPara, sizeof(DICC_CHAN_SYNC_PARA_STRU), 0x00, sizeof(DICC_CHAN_SYNC_PARA_STRU));
    ...
    if (0 == (*pulDiccMagicAddr))
    {
        ...
        PSACORE_MEM_SET((VOID *)pucStartAddr, DICC_QUEUE_MEM_SIZE, 0x00, DICC_QUEUE_MEM_SIZE);
                              ...
                              DICC_CPU_ID_ENUM_UINT8       enCpuId)
{
    ...
    PSACORE_MEM_CPY(&(stInitChanInfo.stInitPara), sizeof(stInitChanInfo.stInitPara),
                                  ...
                                  DICC_CPU_ID_ENUM_UINT8 enCpuId)
{
    ...
    if (VOS_NULL_PTR != pstDiccExcp)
    {
        ...
        for (ulTraceBlkLoop = 0; ulTraceBlkLoop < ucTraceBlkCnt; ulTraceBlkLoop++)
        {
            ...
            PSACORE_MEM_CPY(pstQueueData->aucPriData, pstQueueNode->ucPriDataLen, pstQueueNode->aucPriData, pstQueueNode->ucPriDataLen);
        }

        /* 将出错的通道的状态信息带上 */
        pstDFx = DICC_GET_CHAN_DFX(g_astChanRole[enCpuId].pastDICCMgmt);
        PSACORE_MEM_CPY(&(pstDiccExcp->stIsrDbg), sizeof(pstDiccExcp->stIsrDbg), &(pstDFx->stIsrDbg), sizeof(pstDiccExcp->stIsrDbg));
        PSACORE_MEM_CPY(&(pstDiccExcp->stChanStatus), sizeof(pstDiccExcp->stChanStatus), &(pstDFx->astChanStatus[pstTraceQueue->ucChanId]),
                                        ...
                                        DICC_CPU_ID_ENUM_UINT8 enCpuId)
{
    ...
    PSACORE_MEM_CPY(&(stInsertPara.stDscpBlk), sizeof((stInsertPara.stDscpBlk)), pstDscpBlk, sizeof((stInsertPara.stDscpBlk)));
    ...
    if (PS_TRUE != ulIsQFull)    /* 队列不满 */
    {
        ...
        /* 拷贝构造核间传递结点 */
        PSACORE_MEM_CPY((VOS_UINT8 *)pstQueueNode->aucPriData, pstDscpBlk->ucDataLen, pstDscpBlk->pucUserData, pstDscpBlk->ucDataLen);
    ...
    if (DICC_CHAN_RECVER_OPEN_MASK == (DICC_CHAN_RECVER_OPEN_MASK & pstDiccChnCtrl->ucStateMask))
    {
        if (0 != TTF_RING_Q_GetCnt(ulPid, (TTF_RING_Q_ST *)(&(pstDiccChnCtrl->stRingBuffer))))
        {
            ...
            if (DICC_MAGIC_NUM_VALUE != pstQueueNode->usMagicNum)
            {
                ...
                PSACORE_MEM_CPY(&(stMagicEvent.stQueueNode), sizeof(stMagicEvent.stQueueNode), (VOS_UINT8*)pstQueueNode, sizeof(stMagicEvent.stQueueNode));
            ...
            PSACORE_MEM_CPY(pucUsrData, pstQueueNode->ucPriDataLen, (VOS_UINT8 *)pstQueueNode->aucPriData, pstQueueNode->ucPriDataLen);
            ...
            PSACORE_MEM_SET((VOS_VOID *)pstQueueNode->aucPriData, sizeof(pstQueueNode->aucPriData), 0x0, sizeof(pstQueueNode->aucPriData));
        }
    }
    ...
    if (DICC_CHAN_RECVER_OPEN_MASK == (DICC_CHAN_RECVER_OPEN_MASK & pstDiccChnCtrl->ucStateMask))
    {
        ...
        *pusActiveRemoveCnt = 0;
        ...
        while (0 != ulActiveNeedCnt)
        {
            ...
            PSACORE_MEM_CPY(&aucUsrData[ulCopyLen], pstQueueNode->ucPriDataLen, (VOS_UINT8 *)pstQueueNode->aucPriData, pstQueueNode->ucPriDataLen);
                                        ...
                                        DICC_CPU_ID_ENUM_UINT8 enCpuId)
{
    ...
    while (ulActiveInsertCnt < pstBatDscpBlk->ucInsertCnt)
    {
        ...
        /* 拷贝构造核间传递结点 */
        PSACORE_MEM_CPY((VOS_UINT8 *)pstQueueNode->aucPriData,
