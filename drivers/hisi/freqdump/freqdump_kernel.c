...
#define LOADMONITOR_BUFF_SIZE			170
#define OK					0

#define FREQDUMP_DATA_PHY_ADDR_OFFSET 0x10000
#define FREQDUMP_DATA_PHY_ADDR_BEGIN  (HISI_SUB_RESERVED_BL31_SHARE_MEM_PHYMEM_BASE+FREQDUMP_DATA_PHY_ADDR_OFFSET)
...
#define FREQDUMP_LOADMONITOR_SVC_DISABLE		0xC700EE03a
/*lint -e750 +esym(750,*)*/
/****** all FREQDUMP service function id end ******/

int freqdump_atoi(char *str)
{
	int result = 0;
	bool symbol = false;
	if (str == NULL)
...
static int freqdump_node_dump(struct seq_file *s, void *p)
{
	...
	pos += snprintf(freqdump_buf+pos, FREQDUMP_BUFF_SIZE-pos, "%u,", freqdump_read_data->ddr_type);
	pos += snprintf(freqdump_buf+pos, FREQDUMP_BUFF_SIZE-pos, "%u,", freqdump_read_data->clkdiv2);
	pos += snprintf(freqdump_buf+pos, FREQDUMP_BUFF_SIZE-pos, "%u", freqdump_read_data->clkdiv9);
	ret = seq_printf(s, "%s\n", freqdump_buf);
	if (ret < 0) {
...
static int loadmonitor_node_dump(struct seq_file *s, void *p)
{
	...
	for (i = 0; i < MAX_MONITOR_IP_DEVICE; i++)
		pos += snprintf(loadmonitor_buf+pos, sizeof(loadmonitor_buf)-pos, "%u|", (u32)loadmonitor_load[i]); /* [false alarm]:snprintf */
	ret = seq_printf(s, "%s\n", loadmonitor_buf);
	if (ret < 0) {
...
static int __init freqdump_debugfs_init(void)
{
	...
	freqdump_virt_addr = ioremap(FREQDUMP_DATA_PHY_ADDR_BEGIN, FREQDUMP_DATA_PHY_ADDR_SIZE);
