...
static inline u32 ufshcd_get_intr_mask(struct ufs_hba *hba)
{
	...
	else if(hba->ufs_version == UFSHCI_VERSION_11)
		return INTERRUPT_MASK_ALL_VER_11;
	else
		return INTERRUPT_MASK_ALL_VER_20;
}
...
void ufshcd_prepare_utp_scsi_cmd_upiu(struct ufshcd_lrb *lrbp, u32 upiu_flags)
{
	...
	WARN(lrbp->cmd->cmd_len > MAX_CDB_SIZE,
	     "scsi(0x%x) cmd_len big than MAX_CDB_SIZE\n", lrbp->cmd->cmnd[0]);
...
static int ufshcd_probe_hba(struct ufs_hba *hba)
{
	...
	if (ret){
		pr_err("ufs link startup failed\n");
		hba->vops->dump_reg(hba);
		goto out;
	}
...
static struct scsi_host_template ufshcd_driver_template = {
	...
	.max_host_blocked	= 1,
	/* older linux kernel dont exist this two flags */
	/* release this after update scsi code */
	/*.use_blk_tags		= 1, */
	/*.track_queue_depth	= 1, */
};

/**
 * older linux kernel dont exist regulator_set_load, we stolen this from samsung
 * we must used this with double check
 */
#if 0
