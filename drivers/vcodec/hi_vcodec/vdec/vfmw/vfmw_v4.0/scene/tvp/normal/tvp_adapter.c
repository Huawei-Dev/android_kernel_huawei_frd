...
HI_S32 TVP_VDEC_GetChanImage( HI_S32 ChanID, IMAGE *pImage )
{
    ...
    {
        SecPrint(PRN_FATAL, "InvokeCommand HIVCODEC_CMD_ID_GET_IMAGE Failed!\n");
        OSAL_UP(&g_TvpSem);
        return VDEC_ERR;
    }
...
HI_S32 TVP_VDEC_ReleaseChanImage(HI_S32 ChanID, IMAGE *pImage )
{
    ...
    if (result != TEEC_SUCCESS)
    {
        SecPrint(PRN_FATAL, "InvokeCommand HIVCODEC_CMD_ID_RELEASE_IMAGE Failed!\n");
        OSAL_UP(&g_TvpSem);
        return VDEC_ERR;
    }
...
HI_S32 TVP_VDEC_MiddleWare(HI_VOID *pArgs)
{
    ...
    while (1)
    {
        ...
        {
            SecPrint(PRN_FATAL, "InvokeCommand HIVCODEC_CMD_ID_RUN_PROCESS Failed!\n");
            goto exit;
        }
...
HI_S32 TVP_VDEC_SyncExtBuffer(HI_S32 ChanID, EXTBUF_PARAM_S *pParam)
{
    ...
    pos = TVP_VDEC_FindExtBuffer(pArray, TargetPhyAddr);
...
HI_S32 TVP_VDEC_SecureControl(HI_S32 ChanID, VDEC_CID_E eCmdID, HI_VOID *pArgs, HI_U32 ArgLen)
{
    ...
    if (TEEC_SUCCESS != result)
    {
        SecPrint(PRN_FATAL, "TEEC_InvokeCommand HIVCODEC_CMD_ID_CONTROL Failed!\n");
        goto Exit_Entry;
    }
...
HI_VOID TVP_VDEC_HelpProc(HI_VOID)
{
    ...
    SecPrint(PRN_ALWS, "USAGE:echo [cmd] [para] >/proc/vfmw_sec\n");
...
HI_S32 TVP_VDEC_SendProcCommand(HI_U32 option, HI_U32 value)
{
    ...
    if (result != TEEC_SUCCESS)
    {
        SecPrint(PRN_ALWS, "InvokeCommand HIVCODEC_CMD_ID_WRITE_PROC Failed!\n");
        return VDEC_ERR;
    }
...
static HI_S32 TVP_VDEC_ReadProc(struct seq_file *p, HI_VOID *v)
{
    ...
    if (g_bSecEnvSetUp && g_SecureInstNum > 0)
    {
        ...
        if (result != TEEC_SUCCESS)
        {
            SecPrint(PRN_ALWS, "InvokeCommand HIVCODEC_CMD_ID_READ_PROC Failed!\n");
            OSAL_UP(&g_TvpSem);
            return VDEC_ERR;
        }
...
HI_S32 TVP_VDEC_SecureInit(VDEC_OPERATION_S *pArgs)
{
    ...
    {
        SecPrint(PRN_FATAL, "InvokeCommand HIVCODEC_CMD_ID_INIT failed, result_val=%d\n", operation.params[0].value.a);
        goto InitWithfree_2;
    }
...
HI_S32 TVP_VDEC_SecureExit(HI_VOID)
{
    ...
    if (result != TEEC_SUCCESS)
    {
        SecPrint(PRN_FATAL, "InvokeCommand HIVCODEC_CMD_ID_EXIT Failed!\n");
    }
