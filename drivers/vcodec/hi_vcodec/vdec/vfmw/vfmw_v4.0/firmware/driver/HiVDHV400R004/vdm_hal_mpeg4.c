...
/*  自用函数申明                                                        */
/************************************************************************/
SINT32 MP4HAL_CfgReg(MP4_DEC_PARAM_S *pMp4DecParam, SINT32 VdhId, UINT32 *pBaseAddr);
...
SINT32 MP4HAL_StartDec( MP4_DEC_PARAM_S *pMp4DecParam, SINT32 VdhId )
{
    ...
    /* 配置寄存器 */
    ret = MP4HAL_CfgReg(pMp4DecParam, VdhId, &StreamBaseAddr);
    ...
    /* 配置PIC DMSG */
    ret = MP4HAL_CfgDnMsg(pMp4DecParam, VdhId);

    if (VDMHAL_OK != ret)
    {
        dprint(PRN_FATAL, "MP4HAL_CfgDnMsg failed!\n");
        return VDMHAL_ERR;
    }

    /* 配置SLICE DMSG*/
    ret = MP4HAL_WriteSlicMsg(pMp4DecParam, VdhId, StreamBaseAddr);

    if (VDMHAL_OK != ret)
    {
        dprint(PRN_FATAL, "MP4HAL_WriteSlicMsg failed!\n");
        return VDMHAL_ERR;
    }
...
SINT32 MP4HAL_CfgReg(MP4_DEC_PARAM_S *pMp4DecParam, SINT32 VdhId, UINT32 *pBaseAddr)
{
    ...
    *pBaseAddr = stream_base_addr;
    ...
    //AVM_ADDR
    D32 = g_HwMem[VdhId].MsgSlotAddr[DN_MSG_SLOT_INDEX] & 0xFFFFFFF0;  // mpeg4 down msg
    WR_VREG( VREG_AVM_ADDR, D32, VdhId );
    dprint(PRN_VDMREG, "AVM_ADDR=0x%x\n", D32);

    //VAM_ADDR
    D32 = g_HwMem[VdhId].MsgSlotAddr[UP_MSG_SLOT_INDEX] & 0xFFFFFFF0;  // mpeg4 up msg
    ...
    D32 = 0;
    WR_VREG( VREG_FF_APT_EN, 0, VdhId );
