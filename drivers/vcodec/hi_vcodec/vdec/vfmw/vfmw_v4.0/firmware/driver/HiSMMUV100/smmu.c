...
VOID SMMU_InitGlobalReg(VOID)
{
    ...
    if (1 == gSmmuInitFlag)
    {
        ...
        SINT32 tmp = -1;
        RD_SMMU_COMMON_VREG(SMMU_SCR, tmp);
        dprint(PRN_ALWS, "SMMU_SCR = 0x%x\n", tmp);
        RD_SMMU_COMMON_VREG(SMMU_SMRx_NS, tmp);
        dprint(PRN_ALWS, "Before configure:SMMU_SMRx_NS = 0x%x\n", tmp);
        RD_SMMU_COMMON_VREG(SMMU_SMRx_NS+0x4, tmp);
        dprint(PRN_ALWS, "Before configure:SMMU_SMRx_NS+0x4 = 0x%x\n", tmp);
        ...
        for (i = 0; i < SMRx_ID_SIZE; i += 2)
        {
            ...
            RD_SMMU_COMMON_VREG(SMMU_SMRx_NS + i*0x4, tmp);
            dprint(PRN_ALWS, "SMMU_SMRx_NS + %d*0x4 = 0x%x\n",i,tmp);
