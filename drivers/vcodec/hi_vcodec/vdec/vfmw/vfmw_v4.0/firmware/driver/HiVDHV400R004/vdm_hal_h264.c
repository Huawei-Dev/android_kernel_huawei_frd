...
/*!
************************************************************************
*   函数原型 :  SINT32 Write_CabacTab(UADDR  mn_phy_addr)
*   功能描述 :  写消息池cabac g_CabacMN code tab
*   参数说明 :
*   返回值   ：
************************************************************************
*/
VOID Write_CabacTab(UADDR mn_phy_addr)
{
    SINT32 i, j;
    UINT32 *p;
...
{
    Write_CabacTab(pHwMem->H264MnAddr);
...
/*!
************************************************************************
*   函数原型 :  UINT32 *Get_VirAddr(UINT32 Dn，UINT32 *base_vir_addr)
*   功能描述 :  获取对应Dn_num所在行在消息池中的虚拟地址
*   参数说明 :
*   返回值   ：
************************************************************************
*/
UINT32 *Get_VirAddr(UINT32 Dn, UINT32 *pbase_vir_addr)
{
...
/*!
************************************************************************
*   函数原型 :  UINT32 *Get_PhyAddr(UINT32 Dn，UADDR base_phy_addr)
*   功能描述 :  获取对应Dn_num所在行在消息池中的物理地址
*   参数说明 :
*   返回值   ：
************************************************************************
*/
UINT32 Get_PhyAddr(UINT32 Dn, UADDR base_phy_addr)
{
    return (base_phy_addr + Dn * 4); // one line addr
}


SINT32 H264HAL_CutSliceChain(UADDR CurrMsgSlotPhy )
{
    ...
    Dn_num = 0;
    pSLOT = Get_VirAddr(Dn_num, pSLOT_BASE);
...
VOID Write_Qmatrix(UINT32 *qmatrix, UINT32 size, UADDR phy_addr, UINT32 *vir_addr)
{
    UINT32 i;
...
SINT32 H264HAL_SetPicMsg(H264_DEC_PIC_PARAM_S *pParam, VDMHAL_HWMEM_S *pHwMem,
                                  UADDR PicMsgSlotPhy, UADDR FstSlcMsgSlotPhy)
{
    ...
    Write_Qmatrix(&(pParam->Qmatrix[0]), 56, pSlot_phy + 4 * 0, pSLOT + 0);

    return 0;
}


SINT32 H264HAL_SetSliceMsg(H264_DEC_PIC_PARAM_S *pParam, VDMHAL_HWMEM_S *pHwMem, H264_DEC_SLICE_PARAM_S *pSlcPara,
                                    UADDR CurrMsgSlotPhy, UADDR  ForwardSlcMsgPhy, UADDR NextSlcMsgSlotPhy, SINT32 first_mb_in_slice, SINT32 last_mb_in_slice)
{
    ...
    Dn_num = 0;
    pSLOT = Get_VirAddr(Dn_num, pSLOT_BASE);
    pSLOT_phy = Get_PhyAddr(Dn_num, pSLOT_phy_base);
    ...
    if (g_SliceTypeForPMV[pSlcPara->slice_type] != 0) // P,B Slice
    {
            ...
            ((1 == pParam->weighted_bipred_idc) && (2 == g_SliceTypeForPMV[pSlcPara->slice_type])))
        {
            ...
            Dn_0 = 64;
            pSLOT_line0 = Get_VirAddr(Dn_0, pSLOT_BASE) ;
            pSLOT_line0_phy = Get_PhyAddr(Dn_0, pSLOT_phy_base);

            // D0~D31 of burst2, Dn_num = 2*64 + D0, cb wp
            Dn_1 = 128;
            pSLOT_line1 = Get_VirAddr(Dn_1, pSLOT_BASE) ;
            pSLOT_line1_phy = Get_PhyAddr(Dn_1, pSLOT_phy_base);

            // D0~D31  of  burst3, Dn_num = 3*64 + D0, cr wp
            Dn_2 = 192;
            pSLOT_line2 = Get_VirAddr(Dn_2, pSLOT_BASE) ;
            pSLOT_line2_phy = Get_PhyAddr(Dn_2, pSLOT_phy_base);
...
SINT32 H264HAL_StartDec(H264_DEC_PIC_PARAM_S *pParam, SINT32 VdhId)
{
    ...
    //BASIC_CFG0
    D32 = 0;
    ((BASIC_CFG0 *)(&D32))->mbamt_to_dec         = (pParam->mbtodec > MAX_MB_NUM_IN_PIC ? MAX_MB_NUM_IN_PIC - 1 : pParam->mbtodec - 1);
    ...
    ((BASIC_CFG1 *)(&D32))->ddr_stride           = pParam->imgstride >> 6;
    ((BASIC_CFG1 *)(&D32))->fst_slc_grp          = pParam->NewPicFlag;
    ((BASIC_CFG1 *)(&D32))->mv_output_en         = (pParam->nal_ref_idc != 0);
    ((BASIC_CFG1 *)(&D32))->uv_order_en          = 0;
    ((BASIC_CFG1 *)(&D32))->vdh_2d_en            = 1;
    ((BASIC_CFG1 *)(&D32))->max_slcgrp_num       = 2;
    ((BASIC_CFG1 *)(&D32))->compress_en          = pParam->Compress_en;
    ((BASIC_CFG1 *)(&D32))->ppfd_en              = 0;
#ifdef VFMW_MODULE_LOWDLY_SUPPORT
    ((BASIC_CFG1 *)(&D32))->line_num_output_en   = ModuleLowlyEnable;
#else
    ...
    //STREAM_BASE_ADDR
    D32 = 0;
    ((STREAM_BASE_ADDR *)(&D32))->stream_base_addr = pParam->stream_base_addr & 0xFFFFFFF0;
    ...
    if (1 == ModuleLowlyEnable)
    {
        ...
        WR_VREG(VREG_PART_DEC_OVER_INT_LEVEL, D32, VdhId );
    ...
    //YSTADDR_1D
    D32 = 0;
    ((YSTADDR_1D *)(&D32))->ystaddr_1d = (pParam->ImgPhyAddr[pParam->CurrPicImgIdc]) & 0xFFFFFFF0;
    WR_VREG( VREG_YSTADDR_1D, D32, VdhId );
    dprint(PRN_VDMREG, "YSTADDR_1D = 0x%x\n", D32);

    //YSTRIDE_1D
    D32 = 0;
    ((YSTRIDE_1D *)(&D32))->ystride_1d = pParam->imgstride;
    ...
    if (1 == ModuleLowlyEnable)
    {
        ...
        WR_VREG( VREG_LINE_NUM_STADDR, D32, VdhId );
    ...
    /*3.配置pic消息池*/
    H264HAL_SetPicMsg(pParam, pHwMem, pHwMem->MsgSlotAddr[DN_MSG_SLOT_INDEX], pHwMem->MsgSlotAddr[DN_MSG_SLOT_INDEX + 1]);
    /*4. 配置slice消息池*/
    ...
    if (pSlicePara->first_mb_in_slice != 0)
    {
        ...
        AddSlice0Flag = 1;
        prev_slcmsg_phy = pHwMem->MsgSlotAddr[DN_MSG_SLOT_INDEX];
        cur_slcmsg_phy = pHwMem->MsgSlotAddr[DN_MSG_SLOT_INDEX + 1];
        next_slcmsg_phy = pHwMem->MsgSlotAddr[DN_MSG_SLOT_INDEX + AddSlice0Flag + 1];
        pre_first_mb_in_slice = 0;

        if (H264HAL_SetSliceMsg(pParam, pHwMem, pSlicePara, cur_slcmsg_phy, prev_slcmsg_phy, next_slcmsg_phy, first_mb_in_slice, last_mb_in_slice) != VDMHAL_OK)
        {
    ...
    for (i = 0; i < pParam->SliceParaNum; i++)
    {
        ...
        prev_slcmsg_phy = pHwMem->MsgSlotAddr[DN_MSG_SLOT_INDEX + AddSlice0Flag + i];
        cur_slcmsg_phy = pHwMem->MsgSlotAddr[DN_MSG_SLOT_INDEX + AddSlice0Flag + 1 + i];
        ...
        if ((j == pParam->SliceParaNum) || (MSG_SLOT_NUM == (DN_MSG_SLOT_INDEX + 1 + AddSlice0Flag + i + 1)))
        {
        ...
        else
        {
            ...
            next_slcmsg_phy = pHwMem->MsgSlotAddr[DN_MSG_SLOT_INDEX + AddSlice0Flag + 1 + j];
        }
    ...
    {
        H264HAL_CutSliceChain(pHwMem->MsgSlotAddr[DN_MSG_SLOT_INDEX + 1 + pParam->SliceParaNum - 1 + AddSlice0Flag]);
