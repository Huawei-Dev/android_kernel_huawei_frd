...
/*!
************************************************************************
*   函数原型 :  UINT32 *HEVCGet_VirAddr(UINT32 Dn，UINT32 *base_vir_addr)
*   功能描述 :  获取对应Dn_num所在行在消息池中的虚拟地址
*   参数说明 :
*   返回值   ：
************************************************************************
*/
UINT32 *HEVCGet_VirAddr(UINT32 Dn, UINT32 *pbase_vir_addr)
{
...
/*!
************************************************************************
*   函数原型 :  UINT32 *HEVCGet_PhyAddr(UINT32 Dn，UADDR base_phy_addr)
*   功能描述 :  获取对应Dn_num所在行在消息池中的物理地址
*   参数说明 :
*   返回值   ：
************************************************************************
*/
UINT32 HEVCGet_PhyAddr(UINT32 Dn, UADDR base_phy_addr)
{
    return (base_phy_addr + Dn * 4); // one line addr
}


SINT32 HEVCHAL_CutSliceChain(UADDR CurrMsgSlotPhy )
{
    ...
    Dn_num = 0;
    pSLOT = HEVCGet_VirAddr(Dn_num, pSLOT_BASE);
...
SINT32 HEVCHAL_SetPicMsg(HEVC_DEC_PIC_PARAM_S *pParam, VDMHAL_HWMEM_S *pHwMem,
...
SINT32 HEVCHAL_SetSliceMsg(HEVC_DEC_PIC_PARAM_S *pPicParam, VDMHAL_HWMEM_S *pHwMem, HEVC_DEC_SLICE_PARAM_S *pSliceParam,
                                    UADDR CurrMsgSlotPhy, UADDR ForwardSlcMsgPhy, UADDR NextSlcMsgSlotPhy, SINT32 slice_segment_address, SINT32 SliceSegmentTileAddress, SINT32 end_ctb_in_slice_raster, SINT32 end_ctb_in_slice_tile)
{
    ...
    // get base vir addr and phy addr of the slot will write slice para of currslice
    pSLOT_BASE = (UINT32 *)(MEM_Phy2Vir(CurrMsgSlotPhy)); // slot addr
    VDMHAL_ASSERT_RET( pSLOT_BASE != NULL, "HEVCHAL_SetSliceMsg can not map slice msg virtual address!\n");
    ...
    Dn_num    = 0;
    pSLOT     = HEVCGet_VirAddr(Dn_num, pSLOT_BASE);
    pSLOT_phy = HEVCGet_PhyAddr(Dn_num, pSLOT_phy_base);
...
SINT32 HEVCHAL_CfgVdmReg(HEVC_DEC_PIC_PARAM_S *pPicParam, VDMHAL_HWMEM_S *pHwMem, SINT32 VdhId)
{
    UINT8  uv_order = 0;
    SINT32 D32;
    UINT32 datTmp;
    ...
    //STREAM_BASE_ADDR
    D32 = 0;
    ((STREAM_BASE_ADDR *)(&D32))->stream_base_addr = pPicParam->stream_base_addr & 0xFFFFFFF0;
    ...
    D32 = 0;
    ((YSTADDR_1D *)(&D32))->ystaddr_1d = (pPicParam->ImgPhyAddr[pPicParam->CurrPicImgIdc]) & 0xFFFFFF00;
    WR_VREG( VREG_YSTADDR_1D, D32, VdhId );

    //YSTRIDE_1D
    D32 = 0;
    ((YSTRIDE_1D *)(&D32))->ystride_1d = pPicParam->imgstride;
    WR_VREG( VREG_YSTRIDE_1D, D32, VdhId );

    //UVOFFSET_1D
    D32 = 0;
    ((UVOFFSET_1D *)(&D32))->uvoffset_1d = pPicParam->UVOffset;
...
SINT32 HEVCHAL_StartDec(HEVC_DEC_PIC_PARAM_S *pParam, SINT32 VdhId)
{
    ...
    /* 2.  配置寄存器(启动寄存器除外) */
    HEVCHAL_CfgVdmReg(pParam, pHwMem, VdhId);

    /*3.配置pic消息池*/
    HEVCHAL_SetPicMsg(pParam, pHwMem, pHwMem->MsgSlotAddr[DN_MSG_SLOT_INDEX], pHwMem->MsgSlotAddr[DN_MSG_SLOT_INDEX + 1]);
    /*4. 配置slice消息池*/
    ...
    if (pSlicePara->SliceSegmentTileAddress != 0)
    {
        ...
        next_slcmsg_phy = pHwMem->MsgSlotAddr[DN_MSG_SLOT_INDEX + add_slice0_flag + 1];

        if (HEVCHAL_SetSliceMsg(pParam, pHwMem, pSlicePara, cur_slcmsg_phy, prev_slcmsg_phy, next_slcmsg_phy, slice_segment_address, slice_segment_tile_address, end_ctb_in_slice_raster, end_ctb_in_slice_tile) != VDMHAL_OK)
        {
    ...
    for (i = 0; i < pParam->NumSlicePara; i++)
    {
        ...
        prev_slcmsg_phy = pHwMem->MsgSlotAddr[DN_MSG_SLOT_INDEX + add_slice0_flag + i];
        cur_slcmsg_phy = pHwMem->MsgSlotAddr[DN_MSG_SLOT_INDEX  + add_slice0_flag + 1 + i];
        ...
        if ((j == pParam->NumSlicePara) || (HEVC_MAX_SLOT_NUM == (DN_MSG_SLOT_INDEX + 1 + add_slice0_flag + i + 1)))
        {
        ...
        else
        {
            ...
            next_slcmsg_phy = pHwMem->MsgSlotAddr[DN_MSG_SLOT_INDEX + add_slice0_flag + 1 + j];
        }
    ...
    {
        HEVCHAL_CutSliceChain(pHwMem->MsgSlotAddr[DN_MSG_SLOT_INDEX + 1 + pParam->NumSlicePara + add_slice0_flag - 1]);
    }
    else
    {
