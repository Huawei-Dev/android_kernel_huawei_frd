...
/*****************************************************************************************
  原型    static UINT32 VDMHAL_CalcFsSize(SINT32 ChanID, SINT32 Width, SINT32 Height, SINT32 UserDec, VID_STD_E VidStd)
  功能    计算所需的每一帧的帧存大小
  参数    ChanID  通道号
  Width   帧图的宽
  Height  帧图的高
  ...
  返回值  所需要的帧存大小
 ******************************************************************************************/
static UINT32 VDMHAL_CalcFsSize(SINT32 ChanID, SINT32 Width, SINT32 Height, VID_STD_E VidStd)
{
    SINT32 ImgWidthInMb;
    ...
    {
        ColMbSize = 16 * 4;  //ONEMB_PMV_COLMB_ADDR_LEN;
    }
    else
    {
        ColMbSize = 8 * 4;  //ONEMB_PMV_COLMB_ADDR_LEN;
    }
...
/*****************************************************************************************
  原型    staitc SINT32 VDMHAL_FillVdmMemElement(SINT32 ChanID, VDMHAL_MEM_ARRANGE_S *pVdmMemArrange)
  功能    在已经分配好的空间中填充内存元素
  参数    ChanID 通道号
  pVdmMemArrange  VDM各个内存元素的空间划分
  返回值  成功返回OK（0），否则返回错误码
 ******************************************************************************************/
static UINT32 VDMHAL_CalcFsSize_Detail (SINT32 ChanID, SINT32 Width, SINT32 Height, VID_STD_E VidStd, SINT32* decSize, SINT32* pmvSize, SINT32* stride)
{
    SINT32 ImgWidthInMb;
    SINT32 ImgHeightInMb;
    SINT32 ImgHeightInPixAln;
...
static SINT32 VDMHAL_FillVdmMemElement(SINT32 ChanID, VDMHAL_MEM_ARRANGE_S *pVdmMemArrange)
{
    ...
    for (i = 0; i < s32TotalFrmNum; i++)
    {
        ...
        /*对齐*/
        s32Offset = ((s32PhyAddr + VHB_STRIDE_BYTE - 1) & (~(VHB_STRIDE_BYTE - 1))) - s32PhyAddr;
...
static SINT32 VDMHAL_FillVdmMemElement_Detail(SINT32 ChanID, VDMHAL_MEM_ARRANGE_S *pVdmMemArrange)
{
    SINT32 s32Ret = VDMHAL_OK;
    SINT32 i = 0;
    SINT32 s32TotalFrmNum = 0;
...
SINT32 VDMHAL_ReadMsgSlot(SINT32 *pDst, SINT32 *pSrc, UINT32 upmsg_size)
{
...
SINT32 VDMHAL_CfgRpMsg(VDMHAL_REPAIR_PARAM_S *pRepairParam, VDMHAL_HWMEM_S *pHwMem, SINT32 VdhId)
{
    ...
    if (0 == pRepairParam->FullRepair)
    {
        for (i = 0; i < pRepairParam->ValidGroupNum; i++)
        {
            ...
            D32 = 0;
            ((VDMRPMSG_D8 *)(&D32))->start_mbx = start_Mb % fld_image_w;
            ((VDMRPMSG_D8 *)(&D32))->start_mby = start_Mb / fld_image_w;
            ...
            D32 = 0;
            ((VDMRPMSG_D9 *)(&D32))->end_mbx = (end_Mb) % fld_image_w;
            ((VDMRPMSG_D9 *)(&D32))->end_mby = (end_Mb) / fld_image_w;
    ...
    if ( 1 == pRepairParam->FullRepair)
    {
        ...
        D32 = 0;
        ((VDMRPMSG_D8 *)(&D32))->start_mbx = 0;
        ((VDMRPMSG_D8 *)(&D32))->start_mby = 0;
        WR_MSGWORD( pMsgBase + 8, D32);
        dprint(PRN_VDMREG, "repair: start_mbx = 0x%x\n", 0);
        dprint(PRN_VDMREG, "repair: start_mby = 0x%x\n", 0);

        D32 = 0;
        ((VDMRPMSG_D9 *)(&D32))->end_mbx = fld_image_w - 1;
        ((VDMRPMSG_D9 *)(&D32))->end_mby = fld_image_h - 1;
...
SINT32 VDMHAL_CfgRpReg(VID_STD_E VidStd, VDMHAL_HWMEM_S *pHwMem, VOID *pDecParam, SINT32 VdhId)
{
    SINT32 *p32;
    SINT32 D32 = 0;
    ...
    D32 = 0;
    ((AVM_ADDR *)(&D32))->av_msg_addr = pHwMem->MsgSlotAddr[RP_MSG_SLOT_INDEX] & 0xFFFFFFF0;
...
SINT32 VDMHAL_IMP_OpenHAL(VDMHAL_OPENPARAM_S *pOpenParam)
{
    ...
    PhyAddr += (SED_TOP_ADDR_LEN + 0xF) &(~0xF);

    g_HwMem[VdhId].PmvTopAddr = PhyAddr;
    PhyAddr += (PMV_TOP_ADDR_LEN + 0xF) &(~0xF);

    g_HwMem[VdhId].RcnTopAddr = PhyAddr;
    PhyAddr += 0;//(RCN_TOP_ADDR_LEN + 0xF) &(~0xF);

    g_HwMem[VdhId].ItransTopAddr = PhyAddr;
    PhyAddr += (ITRANS_TOP_ADDR_LEN + 0xF) &(~0xF);

    g_HwMem[VdhId].DblkTopAddr = PhyAddr;
    PhyAddr += (DBLK_TOP_ADDR_LEN + 0xF) &(~0xF);
    ...
    if ((PhyAddr - MemBaseAddr) <= Size)
    {
        ...
        if ( VDMHAL_OK != H264HAL_InitHal(&g_HwMem[VdhId]) )
        {
            Ret = VDMHAL_ERR;
        }
        ...
        if ( VDMHAL_OK != HEVCHAL_InitHal(&g_HwMem[VdhId]) )
        {
            Ret = VDMHAL_ERR;
        }
        ...
        if ( VDMHAL_OK != MP2HAL_InitHal(g_HwMem[VdhId].MPEG2TabAddr, VdhId) )
        {
            Ret = VDMHAL_ERR;
        }
        ...
        if ( VDMHAL_OK != MP4HAL_InitHal(g_HwMem[VdhId].MPEG4TabAddr) )
        {
            Ret = VDMHAL_ERR;
        }
        ...
        if ( VDMHAL_OK != AVSHAL_InitHal(g_HwMem[VdhId].AVSTabAddr) )
        {
            Ret = VDMHAL_ERR;
        }
        ...
        if ( VDMHAL_OK != VC1HAL_InitHal(g_HwMem[VdhId].VC1TabAddr) )
        {
            Ret = VDMHAL_ERR;
        }
        ...
        if ( VDMHAL_OK != VP6HAL_InitHal(g_HwMem[VdhId].Vp6TabAddr) )
        {
            Ret = VDMHAL_ERR;
        }
        ...
        if ( VDMHAL_OK != VP8HAL_InitHal(g_HwMem[VdhId].Vp8TabAddr) )
        {
            Ret = VDMHAL_ERR;
        }
...
/*****************************************************************************************
  原型    SINT32 VDMHAL_ArrangeMem( VDMHAL_MEM_ARRANGE_S *pVdmMemArrange )
  功能    划分VDM所使用的空间。
...
SINT32 VDMHAL_IMP_ArrangeMem( UADDR MemAddr, SINT32 MemSize, SINT32 Width, SINT32 Height, SINT32 PmvNum, SINT32 FrameNum, ARRANGE_FLAG_E eFlag, VDMHAL_MEM_ARRANGE_S *pVdmMemArrange )
{
    ...
    if ((ChanID >= 0) && (1 == s_pstVfmwChan[ChanID]->stSynExtraData.stChanOption.u32DynamicFrameStoreAllocEn))
    {
        ...
        {
            NewFrameSize = VDMHAL_CalcFsSize_Detail(ChanID, Width, Height, VidStd, &DecSize, &PmvSize, &Stride);
        }
        else
        {
            NewFrameSize = VDMHAL_CalcFsSize(ChanID, Width, Height, VidStd);
        }
        ...
        else
        {
        	...
        	if(s_pstVfmwChan[ChanID]->stSynExtraData.stChanOption.u32OmxBypassMode)
        	{
        		ret = VDMHAL_FillVdmMemElement_Detail(ChanID, pVdmMemArrange);
        	}
			else
			{
				ret = VDMHAL_FillVdmMemElement(ChanID, pVdmMemArrange);
			}
        }
    ...
    {
        VDMHAL_ASSERT_RET(0 != MemAddr, "VDMHAL_ArrangeMem Mem addr is NULL");
        ...
        Offset = ((MemAddr + VHB_STRIDE_BYTE - 1) & (~(VHB_STRIDE_BYTE - 1))) - MemAddr;
        ...
        if (eFlag >= VALID_CHAN_ID_START)
        {
            ...
            {
                dprint(PRN_FATAL, "VDMHAL_ArrangeMem get ChanWidth/ChanHeight failed!\n");
                return VDMHAL_ERR;
            }
...
VOID VDMHAL_IMP_ResetVdm( SINT32 VdhId)
{
    ...
    {
        dprint(PRN_FATAL, "VDMHAL_ResetVdm: map vdm register fail, vir(pVdmResetVirAddr) = (%p), vir(pVdmResetOkVirAddr) = (%p)\n",
...
VOID VDMHAL_IMP_GlbReset( VOID )
{
    ...
    {
        dprint(PRN_FATAL, "VDMHAL_GlbReset: map vdm register fail, vir(pResetVirAddr) = (%p), vir(pResetOKVirAddr) = (%p)\n",
...
VOID VDMHAL_IMP_ClearIntState( SINT32 VdhId )
{
    ...
    if (1 == ModuleLowlyEnable)
    {
        ...
        {
            // 清除中断
            RD_VREG( VREG_VDH_STATE, dat, VdhId );

            //如果是行号中断则清行号中断
            if (2 == g_VdmDrvParam[VdhId].ChanIntState)
            {
                WR_VREG( VREG_INT_STATE, 0xFFFFFFF4, VdhId );
            }
            //如果是解码完成中断则清解码完成中断
            else if (3 == g_VdmDrvParam[VdhId].ChanIntState )
            {
                WR_VREG( VREG_INT_STATE, 0xFFFFFFF1, VdhId );
            }
        }
    }
    else
...
/*****************************************************************************************
  原型    SINT32 VDMHAL_PrepareDec( VID_STD_E VidStd, VOID *pDecParam )
...
SINT32 VDMHAL_IMP_PrepareDec( VID_STD_E VidStd, VOID *pDecParam, SINT32 VdhId )
{
    ...
    switch (VidStd)
    {
        ...
        case VFMW_H264:
            return H264HAL_StartDec( (H264_DEC_PIC_PARAM_S *)pDecParam, VdhId );
            break;
#endif
#ifdef VFMW_HEVC_SUPPORT

        case VFMW_HEVC:
            return HEVCHAL_StartDec( (HEVC_DEC_PIC_PARAM_S *)pDecParam, VdhId );
            break;
#endif
#ifdef VFMW_MPEG2_SUPPORT

        case VFMW_MPEG2:
            return MP2HAL_StartDec( (MP2_DEC_PARAM_S *)pDecParam, VdhId );
            break;
#endif
#ifdef VFMW_MPEG4_SUPPORT

        case VFMW_MPEG4:
            return MP4HAL_StartDec( (MP4_DEC_PARAM_S *)pDecParam, VdhId );
            break;
#endif
#ifdef VFMW_AVS_SUPPORT

        case VFMW_AVS:
            return AVSHAL_StartDec( (AVS_DEC_PARAM_S *)pDecParam, VdhId );
            break;
#endif
#ifdef VFMW_REAL8_SUPPORT

        case VFMW_REAL8:
            return RV8HAL_StartDec( (RV8_DEC_PARAM_S *)pDecParam, VdhId );
            break;
#endif
#ifdef VFMW_REAL9_SUPPORT

        case VFMW_REAL9:
            return RV9HAL_StartDec( (RV9_DEC_PARAM_S *)pDecParam, VdhId );
            break;
#endif
#ifdef VFMW_DIVX3_SUPPORT

        case VFMW_DIVX3:
            return DIVX3HAL_StartDec( (DIVX3_DEC_PARAM_S *)pDecParam, VdhId );
            break;
#endif
#ifdef VFMW_VC1_SUPPORT

        case VFMW_VC1:
            return VC1HAL_StartDec( (VC1_DEC_PARAM_S *)pDecParam, VdhId );
            break;
#endif
#ifdef VFMW_VP6_SUPPORT
        ...
        case VFMW_VP6A:
        case VFMW_VP6F:
            return VP6HAL_StartDec( (VP6_DEC_PARAM_S *)pDecParam, VdhId );
            break;
#endif
#ifdef VFMW_VP8_SUPPORT

        case VFMW_VP8:
            return VP8HAL_StartDec( (VP8_DEC_PARAM_S *)pDecParam, VdhId );
            break;
#endif
...
SINT32 VDMHAL_IMP_BackupInfo(BACKUP_INFO_S *pBackUpInfo)
{
    ...
    // Read Regs
    RD_VREG( VREG_BASIC_CFG1, g_BackUp[VdhId].BasicCfg1, VdhId );
    RD_VREG( VREG_VDH_STATE, g_BackUp[VdhId].VdmState, VdhId );

    if (1 == LowdlyFlag)
    {
        /* read up msg */
        DecSliceNum = (g_BackUp[VdhId].VdmState & 0x1FFFF);
        UpMsgPhyAddr = pHwMem->MsgSlotAddr[UP_MSG_SLOT_INDEX];
    ...
    {
        RD_VREG( VREG_SED_STA, g_BackUp[VdhId].SedSta, VdhId );
        RD_VREG( VREG_SED_END0, g_BackUp[VdhId].SedEnd0, VdhId );
    }

    RD_VREG( VREG_MB0_QP_IN_CURR_PIC, g_BackUp[VdhId].Mb0QpInCurrPic, VdhId );
    RD_VREG( VREG_SWITCH_ROUNDING, g_BackUp[VdhId].SwitchRounding, VdhId );

    // 低延迟下面五个不可回读
    if (0 == LowdlyFlag)
    {
        RD_VREG(VREG_DEC_CYCLEPERPIC, g_BackUp[VdhId].DecCyclePerPic, VdhId );
        RD_VREG(VREG_RD_BDWIDTH_PERPIC, g_BackUp[VdhId].RdBdwidthPerPic, VdhId );
        RD_VREG(VREG_WR_BDWIDTH_PERPIC, g_BackUp[VdhId].WrBdWidthPerPic, VdhId );
        RD_VREG(VREG_RD_REQ_PERPIC, g_BackUp[VdhId].RdReqPerPic, VdhId );
        RD_VREG(VREG_WR_REQ_PERPIC, g_BackUp[VdhId].WrReqPerPic, VdhId );
    }

    for (i = 0; i < 32; i++)
    {
        RD_VREG( VREG_LUMA_HISTORGRAM + i * 4, g_BackUp[VdhId].LumaHistorgam[i], VdhId );
    }

    // 低延迟下面两个不可回读
    if (0 == LowdlyFlag)
    {
        RD_VREG( VREG_LUMA_SUM_LOW, g_BackUp[VdhId].LumaSumLow, VdhId );
        RD_VREG( VREG_LUMA_SUM_HIGH, g_BackUp[VdhId].LumaSumHigh, VdhId );
    }
...
SINT32 VDMHAL_IMP_MakeDecReport(MAKE_DEC_REPORT_S *pMakeDecReport)
{
    ...
    VDMHAL_ReadMsgSlot((SINT32 *)(&g_UpMsg[VdhId][0]), pMsgBase, 4 * pDecReport->DecSliceNum);
...
/*****************************************************************************************
  原型    VOID VDMHAL_PrepareRepair(VDMHAL_REPAIR_PARAM_S *pRepareParam)
  功能    将一组参数配置到寄存器和消息池
...
#endif
            VDMHAL_CfgRpReg(VidStd, pHwMem, pDecParam, VdhId);
            VDMHAL_CfgRpMsg(pRepairParam, pHwMem, VdhId);
    ...
    else if (SECOND_REPAIR == RepairTime)
    {
        ...
        if ( (VFMW_AVS == VidStd) && (1 == ((AVS_DEC_PARAM_S *)pDecParam)->PicStruct) && (g_RepairParam[VdhId][1].ValidGroupNum > 0) )
        {
            ...
            if (g_RepairParam[VdhId][1].ValidGroupNum > 0)
            {
                ...
                VDMHAL_CfgRpReg(VidStd, pHwMem, pDecParam, VdhId);
                VDMHAL_CfgRpMsg(&(g_RepairParam[VdhId][1]), pHwMem, VdhId);
                // Start Repair 删除，真正的启动操作放在外面，以便与VDMDRV状态机置位相配合
            }
        }
        else
...
#endif 
    WR_VREG(VREG_BASIC_CFG0, D32, VdhId); /*[27:26]	work_mode 00：解码；01：修补；10：RPR（REAL协议特有）*/
