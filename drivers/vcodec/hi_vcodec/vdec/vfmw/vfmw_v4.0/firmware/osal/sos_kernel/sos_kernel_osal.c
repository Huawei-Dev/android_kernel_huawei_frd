...
SINT8* SOS_MapShare(UINT32 Phy, SINT32 Size, UINT32 SecMode, UINT32 IsCached)
{
    ...
    ret = mmap(Phy, Size, &VirAddr, SecMode, IsCached);
    if (ret != 0)
    {
        dprint(PRN_FATAL, "%s %d, mmap failed ret=%d, Phy=%x, Size=%d, SecMode=%d, IsCached=%d\n", __func__,__LINE__, ret, Phy, Size, SecMode, IsCached);
        return NULL;
    }
