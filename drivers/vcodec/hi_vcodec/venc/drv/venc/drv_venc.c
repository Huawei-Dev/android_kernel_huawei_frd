...
HI_VOID VENC_DRV_BoardInit(HI_VOID)
{
 ...
 //  b_Regular_down_flag = HI_FALSE;
    ...
    cur_handle_num = 0;
    HI_INFO_VENC("enable interframe power gate control!!");
#else
                          ...
                          VENC_CHN_INFO_S *pstVeInfo, struct file *pfile)
{
    ...
    HI_DBG_VENC("enter,phVencChn:0x%x, pstAttr->enVencType:%d\n",phVencChn,pstAttr->enVencType);
    ...
    {
        HI_ERR_VENC("channel:0x%x create  failed,i:%d\n", phVencChn, i);
        return s32Ret;
    }
	...
	HI_DBG_VENC("stRcAttrCfg.InFrmRate:%d\n",stRcAttrCfg.InFrmRate);
	HI_DBG_VENC("stRcAttrCfg.Gop:%d\n",stRcAttrCfg.Gop);
	HI_DBG_VENC("stRcAttrCfg.BitRate:%d\n",stRcAttrCfg.BitRate);
	HI_DBG_VENC("stRcAttrCfg.MinQp:%d\n",stRcAttrCfg.MinQp);
	HI_DBG_VENC("stRcAttrCfg.MaxQp:%d\n",stRcAttrCfg.MaxQp);
	HI_DBG_VENC("stRcAttrCfg.SkipFrameEn:%d\n",stRcAttrCfg.SkipFrameEn);
...
//	g_stVencChn[i].u32SliceSize                  = stEncCfg.SplitSize;
	...
	HI_INFO_VENC("create channel success phVencChn = 0x%x, frameRate = %d, biteRate = %d, Gop = %d\n",
                 phVencChn, stRcAttrCfg.InFrmRate, stRcAttrCfg.BitRate, stRcAttrCfg.Gop);
    return HI_SUCCESS;
...
HI_S32 VENC_DRV_DestroyChn( HI_HANDLE hVencChn)
{
    ...
    HI_DBG_VENC("enter %s, VencChn:0x%x \n", __func__, hVencChn);
	...
	HI_INFO_VENC("destroy channel success, hVencChn:0x%x\n", hVencChn);
    return HI_SUCCESS;
}
...
HI_S32 VENC_DRV_StartReceivePic(HI_HANDLE EncHandle)
{
    ...
    if (HI_SUCCESS == s32Ret)
    {
        ...
        HI_DBG_VENC("%s, start Chn %d/%#x. OK\n", __func__, u32VeChn, EncHandle);
	...
	HI_INFO_VENC("start receive picture, EncHandle : 0x%x!!\n", EncHandle);
    return s32Ret;
}
...
HI_S32 VENC_DRV_StopReceivePic(HI_HANDLE EncHandle)
{
    ...
    HI_INFO_VENC("venc drv stop receive picture, EncHandle:0x%x\n", EncHandle);
    return s32Ret;
}
