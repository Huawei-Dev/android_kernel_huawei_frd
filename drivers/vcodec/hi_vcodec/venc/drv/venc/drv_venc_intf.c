...
static long VENC_Ioctl(struct file *file, unsigned int ucmd, unsigned long uarg)
{
    ...
    switch (cmd)
    {
        case CMD_VENC_CREATE_CHN:
        {
            ...
            Ret = down_interruptible(&g_VencMutex);
            Ret = VENC_DRV_CreateChn(&hKernVencChn, &(pstCreateInfo->stAttr), &(pstCreateInfo->stVeInfo), file);
            ...
            up(&g_VencMutex);
            HI_DBG_VENC("%s, CMD_VENC_CREATE_CHN Ret:%d\n", __func__, Ret);
            break;
        }
        case CMD_VENC_DESTROY_CHN:
        {
            ...
            up(&g_VencMutex);
            HI_DBG_VENC("%s, CMD_VENC_DESTROY_CHN Ret:%d\n", __func__, Ret);
            break;
        }
        case CMD_VENC_START_RECV_PIC:
        {
            ...
            up(&g_VencMutex);
            HI_DBG_VENC("%s, CMD_VENC_START_RECV_PIC Ret:%d\n", __func__, Ret);
            break;
        }
        case CMD_VENC_STOP_RECV_PIC:
        {
            ...
            up(&g_VencMutex);
            HI_DBG_VENC("%s, CMD_VENC_STOP_RECV_PIC Ret:%d\n", __func__, Ret)	;
            break;
        }
        case CMD_VENC_SET_CHN_ATTR:
        {
            ...
            up(&g_VencMutex);
            HI_DBG_VENC("%s, CMD_VENC_SET_CHN_ATTR Ret:%d\n", __func__, Ret);
            break;
        }
        ...
        case CMD_VENC_QUEUE_FRAME: //both omxvenc & venc use!!
        {
            ...
            up(&g_VencMutex);
            HI_DBG_VENC("%s, CMD_VENC_QUEUE_FRAME Ret:%d\n", __func__, Ret);
            break;
        }
        case CMD_VENC_GET_MSG:
        {
            ...
            //HI_INFO_VENC("CMD_VENC_GET_MSG Ret:%d\n",Ret);
            break;
        }
        case CMD_VENC_QUEUE_STREAM:                    //juse omxvenc use this api
        {
            ...
            up(&g_VencMutex);
            HI_DBG_VENC("%s, CMD_VENC_QUEUE_STREAM Ret:%d\n", __func__, Ret);
            break;
        }
