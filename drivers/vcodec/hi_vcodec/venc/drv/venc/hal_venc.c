...
HI_VOID VeduHal_ReadReg_Venc( VeduEfl_EncPara_S *pEncPara )
{
	...
	HI_DBG_VENC("RCinfo: pEncPara->PicBits:%d pEncPara->I8x8Num:%d\n",pEncPara->PicBits,pEncPara->I8x8Num);
...
HI_VOID VENC_HAL_CfgReg( HI_HANDLE EncHandle )
{
	...
	if (b_Regular_down_flag == HI_TRUE)
	{
		...
		HI_DBG_VENC("%s, hardware power on!!!\n", __func__);
	}
#endif
	 ...
	 //if (pEncPara->stRc.FrmNumSeq < 2|| bResetFlag == 1 ||  EncHandle != h_pre_enc_handle)
    {
	    ...
	    HI_DBG_VENC("%s, allcfg reg TotalFrame:%d bResetFlag:%d EncHandle:0x%llx h_pre_enc_handle:0x%llx\n", __func__,
...
HI_VOID VeduHal_CfgReg( HI_HANDLE EncHandle  )
{
    ...
    int ii = 0;
    HI_U32* pAddr_SCR = NULL;
    HI_U32* pAddr_SMR = NULL;
#ifdef TEST_TIME
    ...
    pAllReg->VEDU_VCPI_REGS.VEDU_DOWN_FREQ_EN.bits.down_freq_en     = pEncPara->down_freq_en;
    ...
    HI_DBG_VENC("REFLD_CMD_CFG.bits.ddr_cross_idx = 0x%x\n", pEncPara->refld_ddr_cross_idx);
    // Ime  0x20620a00
    ...
    HI_DBG_VENC("RECST_DDR_CROSS.bits.ddr_cross_idx = 0x%x\n", pEncPara->recst_ddr_cross_idx);
...
//     pAllReg->VEDU_COMN0_REGS.COMN0_SMMU_SMR35_NS.bits.smr35_ptw_qos  =  pEncPara->comn0_smr35_ptw_qos;
     ...
     pAddr_SCR = &(pAllReg->VEDU_COMN0_REGS.COMN0_SMMU_SCR);
     pAddr_SMR = &(pAllReg->VEDU_COMN0_REGS.COMN0_SMMU_SMR0_NS);
#ifdef VENC_SMMU_QOS_PRINT
     HI_INFO_VENC("COMN0_SMMU_SCR = 0x%x\n",*pAddr_SCR);
     for (ii = 0; ii < 36; ii++)
     {
         HI_INFO_VENC("COMN0_SMMU_SMR0_NS+%d*0x4 = 0x%x\n",ii,pAddr_SMR[ii]);
     }
#endif
