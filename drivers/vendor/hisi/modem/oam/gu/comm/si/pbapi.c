                                ...
                                SI_UINT8                    *pucContent)
{
    ...
    /*lint -e534*/
    VOS_MemCpy_s(pMsg->aucContent, ucLength, pucContent, ucLength);
    /*lint +e534*/
...
VOS_UINT32 SI_PB_GetEccNumber(SI_PB_ECC_DATA_STRU *pstEccData)
{
    ...
    for(i=0,j=0; i<ulNum; i++)   /*根据数据结构最大长度循环*/
    {
        ...
        else                                /*转换当前记录内容*/
        {
            ...
            /*lint -e534*/
            VOS_MemCpy_s(pstEccData->astEccRecord[j].aucEccCode, gastPBContent[ucPBOffset].ucNumberLen,
            ...
            if (USIMM_CARD_USIM == gstPBInitState.enCardType)
            {
                ...
                /*lint -e534*/
                VOS_MemCpy_s(pstEccData->astEccRecord[j].AlphaIdentifier,
...
VOS_UINT32 SI_PB_GetXeccNumber(SI_PB_ECC_DATA_STRU *pstEccData)
{
    ...
    /*lint -e534*/
    VOS_MemSet_s(pstEccData, sizeof(SI_PB_ECC_DATA_STRU), 0, sizeof(SI_PB_ECC_DATA_STRU));
    /*lint +e534*/
    ...
    for (i = 0, j = 0; i < ulNum; i++)   /* 根据数据结构最大长度循环 */
    {
        ...
        else                                /* 转换当前记录内容 */
        {
            ...
            /*lint -e534*/
            VOS_MemCpy_s(pstEccData->astEccRecord[j].aucEccCode, gastPBContent[ucPBOffset].ucNumberLen, pucTemp, gastPBContent[ucPBOffset].ucNumberLen);
            /*lint +e534*/
