...
#define LPHY_FEATURE_DSDS_20 (LPHY_TRUE)
...
/*****************************************************************************
 数据结构说明  : OTDOA测量SNR门限结构体定义
*****************************************************************************/
typedef struct
{
	UINT16    ausToaSnrThrd[6];              /*TOA计算结果的SNR门限,对应的不同带宽20M,15M,10M,5M,3M,1.4M*/
	UINT16    ausRstdSnrThrd[6];			 /*RSTD计算结果为bad的SNR门限,对应的不同带宽20M,15M,10M,5M,3M,1.4M*/
}NV_OTDOA_SNR_THRD_STRU;
...
*****************************************************************************/
typedef struct{
    LPHY_ANTANNA_PORT_ENUM_UINT16  enAntennaPortsCount;                /*LPHY UE的天线中用来接收下行信号的天线数目*/
    LPHY_LTE_MODE_TYPE_ENUM_UINT16 enLteMode;                          /*LPHY 初始化时默认的复用模式*/
...
typedef struct
{
    UINT32         aulFineDlyPara[UL_ET_FINEDLY_PARA_NUM];
    UINT16         usInitFineDly;
    UINT16         usRsv;
...
typedef struct
{
    UINT16 usListeningPathExistFlg;
    UINT16 usRes;
...
typedef struct
{
    ...
    NV_LTE_RX_FREQ_COMP_STRU   	 stchan2FreqCompFac;
    RF_DCOC_CAL_STRU             stDcocTbl;
    RF_DCOC_CAL_STRU             stDcocTblListening;
...
{
    UINT16 usValidFlag;
    UINT16 usTxAntSel;  /*指示切换后用哪个天线发射*/
    INT16  sPowCmp[RF_TX_FREQ_NUM]; /*天线切换后差损补偿，每个频点对应一个*/
}LTE_TX_ANT_SEL_PATH_PARA_STRU;

typedef struct
{
    LTE_TX_ANT_SEL_PATH_PARA_STRU stTxAntSelPathPara[MAX_TX_CHAN_NUM];
}NV_LTE_TX_ANT_SEL_PARA_STRU;
...
typedef struct
{
	...
	UINT8  ucMrxVloAdj;
   	UINT8  ucMrxVgaGain;
	UINT8  usCountMin;
	UINT8  ucSampleSmoothLen;
	UINT8  ucAmamSmoothLen;
	UINT8  ucGrIndex;
   	UINT8  ucGrLen;
   	UINT8  ucGrShiftBit;
   	UINT16 usApcCode;
	UINT16 usDbbCode;
	UINT16 usAmamLineNum;
   	UINT16 usAmamMaxVcc;
   	UINT16 usAmamVccStep;
	UINT32 ulEclNdBLinear;
}NV_DPD_ECL_CAL_STRU;

/* DPD ECL等压缩线校准结果NV */
typedef struct
{
    UINT16 usSlopeVccVout;
	UINT16 usSlopeVinVout;
	INT32  sInterceptVccVout;
	INT32  sInterceptVinVout;
	INT16  sMrxGain; //有符号
	UINT16 usRsv;
}NV_DPD_ECL_PARA_STRU;
...
typedef struct
{
	...
	UINT16 usAmpMax;
	UINT16 usGainCoeff;
    NV_DPD_MRX_PARA_STRU  astDpdMrxPara[4];

}NV_DPD_LUT_CAL_PARA_STRU;

/* RFIC功率校准参数 */
typedef struct
{
	UINT16 usApcCode[16][2];  //由8->16
	UINT16 usDbbCode;
	UINT16 usBandWidth;
	UINT16 usApcNum;
	UINT16 usRsv;
}NV_DPD_POUT_DELTA_INFO_STRU;
...
typedef struct
{
	...
	UINT16 usPaprDb[2];
	UINT16 usMaxVcc;
	UINT16 usCfrAmaxPmax;
    UINT16 usRficSweepDB;
 	INT16  asPoutOffset[4][16];
	UINT16 usTempSensor[4][16];
	UINT16 usTempNum;
	UINT16 usRsv;
 	UINT16 usTargetPow[4];
 	UINT16 usApcCode[4];
    INT32  slMrxIData;
	INT32  slMrxQData;
    NV_DPD_POUT_DELTA_INFO_STRU stDpdPoutDeltaInfo;
  	NV_DPD_ECL_CAL_STRU  	   astDpdEclCalPara[2];
 	NV_DPD_LUT_CAL_PARA_STRU   astDpdLutCalPara[2];
 	INT16  asLutTabLowThre[4];                       /* 每个LUT表适用功率下限*/
  	INT16  asAptToDpdThre[2];            /*Apt向DPD切换的功率门限,区分PA高中档*/
}NV_DPD_LAB_PARA_STRU;

/* Pin offset温频补NV */
typedef struct
{
	INT8   aucPinOffset[4][16][16];
	UINT16 usTempSensor[4][16];
    INT16  sPinoffsetThre[4];
	UINT16 usTempNum;
	UINT16  usRsv;
}NV_DPD_LAB_PIN_OFFSET_STRU;


/* DPD产线NV */
typedef struct
{
   	NV_DPD_ECL_PARA_STRU 	astDpdEclPara[2];
    NV_DPD_RFIC_DELTA_STRU	stDpdRficDelta;
}NV_DPD_FAC_RESULT_STRU;
...
typedef struct
{
	...
	/*新增end*/
	INT16  sFreqComp[16];
	LTE_DPD_LUT_TABLE_STRU stDpdLutTable[128];
}NV_DPD_FAC_LUT_STRU;
...
typedef struct
{
    ...
    /* Start:DPD功能涉及NV */
	NV_DPD_LAB_PARA_STRU          stDpdLabPara;         /* DPD实验室参数 */
	NV_DPD_LAB_PIN_OFFSET_STRU    stDpdLabPinOffset;    /* DPD实验室Pin Offset温度频点补偿值*/
	NV_DPD_FAC_RESULT_STRU        stDpdFacResult;       /* DPD产线NV结果 */
	NV_DPD_FAC_LUT_STRU           stDpdFacLut0;         /* DPD产线LUT结果0 */
	NV_DPD_FAC_LUT_STRU           stDpdFacLut1;         /* DPD产线LUT结果1 */
	NV_DPD_FAC_LUT_STRU           stDpdFacLut2;         /* DPD产线LUT结果2 */
    NV_DPD_FAC_LUT_STRU           stDpdFacLut3;         /* DPD产线LUT结果3 */
    /* End:DPD功能涉及NV */
    ...
    NV_LTE_TX_ANT_SEL_PARA_STRU   stTxAntSel;
	//NV_RF_CA_RCCODE_STRU          stRfRCcode;           /*RFIC Code*/
#if (FEATURE_ON == FEATURE_TLPHY_ET)
	/* ETM使用的NV */
...
typedef struct
{
	UINT16 usBand;
	UINT16 usPathNum;
	NV_BAND_DL_CHAN_PARA_STRU stBandRxListenPara;
...
/*  NV_FE_PBAND_MIPI_INFO_STRU          stPBandMipiCfg;
    ...
    NV_OTDOA_SNR_THRD_STRU              stOtdoaSnrThd;
...
typedef struct
{
    ...
    NV_BAND_FE_DL_PARA_STRU     stThirdDLChPara;   /*下行侦听通道参数*/
