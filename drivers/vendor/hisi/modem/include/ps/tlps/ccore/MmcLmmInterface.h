...
enum MMC_LMM_PLMN_SRCH_TYPE_ENUM
{
    MMC_LMM_PLMN_SRCH_SPEC              = 0,                /* 指定的PLMN的搜索 */
     ...
     MMC_LMM_PLMN_SRCH_USER_LIST        = 1,                /* 非美国搜网定制的列表搜，可以是驻留态，也可以是非驻留态 */
    ...
    MMC_LMM_PLMN_SRCH_INTER_LIST_FULL_BAND  = 11,           /* 美国搜网定制驻留态的内部触发的list搜索,使用FULL Band进行搜索 */
    MMC_LMM_PLMN_SRCH_INTER_LIST_PREF_BAND  = 12,           /* 美国搜网定制驻留态的内部触发的list搜索,使用Prefer Band进行搜索 */
...
enum MMC_LMM_PLMN_SRCH_RLT_ENUM
{
    ...
    MMC_LMM_PLMN_SRCH_RLT_USER_LIST_SUCC                = 1,                    /* PLMN List搜索成功      */
    MMC_LMM_PLMN_SRCH_RLT_SPEC_FAIL                     = 2,                    /* 指定PLMN ID搜索失败    */
    MMC_LMM_PLMN_SRCH_RLT_USER_LIST_REJ                 = 3,                    /* 驻留态下列表搜网被非出服务区流程打断 */

    MMC_LMM_PLMN_SRCH_RLT_SPEC_NO_RF                    = 4,
    MMC_LMM_PLMN_SRCH_RLT_USER_LIST_NO_RF               = 5,
    ...
    MMC_LMM_PLMN_SRCH_RLT_INTER_LIST_FULL_BAND_SUCC     = 18,
    MMC_LMM_PLMN_SRCH_RLT_INTER_LIST_FULL_BAND_FAIL     = 19,
    MMC_LMM_PLMN_SRCH_RLT_INTER_LIST_FULL_BAND_NO_RF    = 20,
    MMC_LMM_PLMN_SRCH_RLT_INTER_LIST_FULL_BAND_REJ      = 21,

    MMC_LMM_PLMN_SRCH_RLT_INTER_LIST_PREF_BAND_SUCC     = 22,
    MMC_LMM_PLMN_SRCH_RLT_INTER_LIST_PREF_BAND_FAIL     = 23,
    MMC_LMM_PLMN_SRCH_RLT_INTER_LIST_PREF_BAND_NO_RF    = 24,
    MMC_LMM_PLMN_SRCH_RLT_INTER_LIST_PREF_BAND_REJ      = 25,
...
/*****************************************************************************
 枚举名    : MMC_LMM_BG_SRCH_HRPD_RLT_ENUM
 协议表格  :
 ASN.1描述 :
 枚举说明  : LMM BG搜索HRPD结果
*****************************************************************************/
enum MMC_LMM_BG_SRCH_HRPD_RLT_ENUM
{
    MMC_LMM_BG_SRCH_HRPD_RLT_SUCCESS    = 0,                /* 指定的PLMN搜索成功 */
    MMC_LMM_BG_SRCH_HRPD_RLT_FAIL       = 1,                /* 实际发起了搜网，但是搜索失败 */
    MMC_LMM_BG_SRCH_HRPD_RLT_ABORT      = 2,                /* 没有发起搜网，因为当前状态不允许，搜索被终止 */
    MMC_LMM_BG_SRCH_HRPD_RLT_BUTT
};
typedef VOS_UINT32  MMC_LMM_BG_SRCH_HRPD_RLT_ENUM_UINT32;
...
{
    MMC_LMM_BG_SPEC_SRCH_RLT_SUCCESS                        = 0,                /* bg spec指定的PLMN搜索成功 */
    MMC_LMM_BG_SPEC_SRCH_RLT_FAIL                           = 1,                /* bg spec实际发起了搜网，但是搜索失败 */
    MMC_LMM_BG_SPEC_SRCH_RLT_ABORT                          = 2,                /* bg spec没有发起搜网，因为当前状态不允许，搜索被终止 */
    MMC_LMM_BG_SPEC_SRCH_RLT_NO_RF                          = 3,                /* bg spec搜网过程中无资源可用 */
    MMC_LMM_BG_HISTORY_SRCH_RLT_SUCCESS                     = 4,                /* bg history指定的PLMN搜索成功 */
    MMC_LMM_BG_HISTORY_SRCH_RLT_FAIL                        = 5,                /* bg history实际发起了搜网，但是搜索失败 */
    MMC_LMM_BG_HISTORY_SRCH_RLT_ABORT                       = 6,                /* bg history没有发起搜网，因为当前状态不允许，搜索被终止 */
    MMC_LMM_BG_HISTORY_SRCH_RLT_NO_RF                       = 7,                /* bg history搜网过程中无资源可用 */
...
typedef struct
{
    ...
    /* 搜索结果 */
    /* 注:收到LRRC上报CSG_LIST搜FAIL时，LNAS将结果转为SUCC，并上报空列表 */
...
typedef struct
{
    ...
    MMC_LMM_RAT_TYPE_ENUM_UINT8          enRat;
    VOS_UINT8                            aucReserve[3];
}NAS_LMM_TAI_STRU;
...
/*****************************************************************************
 枚举名: MMC_LMM_BG_PLMN_TYPE_ENUM
 协议格式:
 ASN.1描述:
 枚举说明:
*****************************************************************************/
enum MMC_LMM_BG_PLMN_TYPE_ENUM
{
    MMC_LMM_BG_PLMN_SRCH_HISTORY        = 0,                /* 指定的PLMN的History搜索 */
    MMC_LMM_BG_PLMN_SRCH_SPEC           = 1,                /* 指定的PLMN的全Band搜索   */
    MMC_LMM_BG_PLMN_SRCH_BUTT
};
typedef VOS_UINT32 MMC_LMM_BG_PLMN_TYPE_ENUM_UINT32;
...
typedef struct
{
    ...
    MMC_LMM_PLMN_ID_STRU                astPlmnIdList[MMC_LMM_MAX_EQUPLMN_NUM];       /* PlMN标识 */
    MMC_LMM_RAT_TYPE_ENUM_UINT8         enRat;              /* 支持的接入技术 */
    VOS_UINT8                           aucRsv[3];

    MMC_LMM_BG_PLMN_TYPE_ENUM_UINT32    enBgType;           /*本次bg搜索类型 */
...
/*****************************************************************************
 结构名    : LMM_MMC_RX_ANT_INFO_STRU
 协议表格  :
 ASN.1描述 :
 结构说明  : LTE下的天线数以及各天线测量到的rsrp sinr值
*****************************************************************************/
typedef struct
{
    VOS_UINT8                           ucRxANTNum;           /*天线总数*/
    VOS_UINT8                           aucRev[3];
    VOS_INT16                           asRsrpRx[4];          /*1/8db精度*/
    VOS_INT32                           alSINRRx[4];          /*1db精度*/
} LMM_MMC_RX_ANT_INFO_STRU;
...
typedef struct
{
    ...
    LMM_MMC_RX_ANT_INFO_STRU                stRxAntInfo;                        /* RxAntInfo */
