 ...
 * 解决方案: 请确保传入的任务ID合法
 */
#define OS_ERRNO_CALLSTACK_TSK_ID_INVALID                  SRE_ERRNO_OS_ERROR(OS_MID_CALLSTACK, 0x04)
 ...
 */
#define OS_ERRNO_CALLSTACK_TSK_UNUSED                      SRE_ERRNO_OS_ERROR(OS_MID_CALLSTACK, 0x05)
 ...
 * 值: 0x02003506
 *
 * 解决方案: 请确保传入的任务ID是非当前运行的任务
 */
#define OS_ERRNO_CALLSTACK_TSK_IS_RUNNING                  SRE_ERRNO_OS_ERROR(OS_MID_CALLSTACK, 0x06)
 ...
 * <ul>
 * <li>只支持Cortex-AX平台。</li>
 * <li>只有任务当前不处在运行状态才能获取</li>
 * <li>缓冲区存储各级函数PC的顺序为从最底层函数到最上层函数。</li>
 * </ul>
 *
 * @param  puwMaxNum [IN/OUT] 类型#UINT32 *，最大调用层数，输入/输出参数。输入：缓冲区puwPCList可容纳的元素个数；输出：实际解析出的函数调用层数。
 * @param  puwPCList [OUT] 类型#UINT32 *，存储函数PC信息的缓冲区首地址，用于填写各级调用函数的PC。
 * @param  uwTaskPID [IN] 类型#UINT32 *，需要获取调用栈信息的任务的ID。
 * @retval #OS_ERRNO_CALLSTACK_PTR_NULL         0x02003501，获取调用栈信息时，输入的参数指针为NULL。
 * @retval #OS_ERRNO_CALLSTACK_MAX_NUM_VALID    0x02003502，获取调用栈信息时，输入的最大调用层数等于0或者大于10。
 ...
 * @retval #OS_ERRNO_CALLSTACK_TSK_IS_RUNNING   0x02003506，获取调用栈信息时，输入的任务ID为当前运行任务的ID。
 * @retval #SRE_OK                              0x00000000，获取调用栈信息成功。
 * @par 依赖:
 * <ul><li>sre_callstack.h：该接口声明所在的头文件。</li></ul>
 * @since RTOSck V100R005C00
 * @see NONE
 **/
extern UINT32 SRE_GetCallStackByTaskID(UINT32 *puwMaxDepth, UINT32 *puwPCList, UINT32 uwTaskPID);
