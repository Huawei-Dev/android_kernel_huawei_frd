...
typedef struct
{
    ...
    VOS_UINT8                             aucContent[4];   /* 此处仅形参，具体消息内容由以下结构体定义*/
...
/*****************************************************************************
结构名    : TLPHY_AGENT_RFIC_INFO_STRU
结构说明  :LPHY发送给RTT AGENT的RFIC信息
*****************************************************************************/
...
/*****************************************************************************
 ...
 结构说明  :RTT AGENT发送给TLPHY的OM重传消息确认结构体
*****************************************************************************/
typedef struct
{
...
/*****************************************************************************
结构名    : LPHY_AGENT_DSPWAKE_TIMEOUT_INFO_STRU
结构说明  :LPHY发送给RTT AGENG的DSP唤醒超时信息
*****************************************************************************/
typedef struct
{
    VOS_UINT32    ulTimeOutCnt;       /* 累计唤醒超时发生的次数*/
    VOS_UINT16    usLpcWakeType;      /* 本次唤醒方式:   LTE 正常唤醒 = 0x00,
                                                                                                        LTE 强制唤醒 = 0x01,
                                                                                                        TDS 正常唤醒 = 0x10,
                                                                                                        TDS 强制唤醒 = 0x11 */
    VOS_UINT16    usLastSleepType;    /* 本次唤醒前的睡眠方式: 1,浅睡；2，深睡 */                                                                                                                                       
    VOS_UINT32    ulSleepTime;        /* 本次唤醒前的实际睡眠时长，32k时钟周期个数 */
    VOS_UINT32    ulWakeupTime;       /* DSP 设置的睡眠时长，32k时钟周期个数 */
    VOS_UINT32    ulSwitchTime;       /* DSP 设置的时钟切换时长，32k时钟周期个数 */
    VOS_UINT32    ulLastSleepSlice;   /* 睡眠开始32k 时钟时间戳*/
    VOS_UINT32    ulWakeupStartSlice; /* 本次唤醒开始32k 时钟时间戳*/   
}LPHY_AGENT_DSPWAKE_TIMEOUT_INFO_STRU;
