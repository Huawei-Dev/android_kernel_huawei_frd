...
typedef enum
{
	...
	LPHY_REQ_AGC_BANDSCAN_POSTPWR_RPT_ENA
}LPHY_REQ_AGC_ENUM;
...
}LPHY_REQ_AGC_BANDSCAN_POSTPWR_RPT_ENA_STRU;
typedef struct
{
    AGC_OM_SWITCH_ENUM_UINT32    enReportEn;
...
typedef enum
{
	...
	LPHY_TRACE_AGC_BANDSCAN_POSTPWR_INFO
}LPHY_TRACE_AGC_ENUM;
...
typedef UINT08  AGC_MBMP_STATE_ENUM_UINT08;
...
#define AGC_MAX_SERV_FREQ_NUM					(LPHY_FEATURE_MAX_CARRIER_NUM+2)
#define AGC_MASTER_MODE_PCC_SCC_CA_BGS_FREQ_NUM (AGC_MAX_SERV_FREQ_NUM+3)  //cc + max ca number + bgs number
#define AGC_MAX_FREQ_NUM						(LPHY_FEATURE_MAX_INTER_EMU_NUM + AGC_MASTER_MODE_PCC_SCC_CA_BGS_FREQ_NUM)
#define AGC_FREQUENCY_NEAR_SPARE_IDX			(AGC_MAX_FREQ_NUM - 3)	/*STATIC FREQ IDX FOR BGS FREQ*/
#define AGC_BGS_DUAL_PSS_BUF_START_INDEX        (AGC_MAX_FREQ_NUM - 3)
...
#define AGC_MAX_BANDSCAN_FREQ                   (LRRC_LPHY_FREQ_RSSI_MAX_SCAN_NUM)

#define AGC_MAX_CONT_CA_NUM                     (2)
#define AGC_BGS_DUAL_PSS_BUF_NUM                (3)
...
typedef struct
{
    UINT16 usFreq;
    UINT16 usBand;
    INT16  asPostAntPwrdBm[LPHY_MAX_RX_ANTENNA_NUM];
}AGC_BAND_SCAN_PWR_STRU;
#define AGC_INVALID_BANDSCAN_FREQ_INDEX (AGC_MAX_BANDSCAN_FREQ)
typedef struct
{
    UINT16 usBand;
    UINT16 usLastBand;
    UINT16 usFreqNum;
    UINT16 usRsv;
    AGC_BAND_SCAN_PWR_STRU stBandScanGain[AGC_MAX_BANDSCAN_FREQ];
}AGC_BAND_SCAN_INFO_STRU;
...
typedef struct
{
    ...
    UINT16                          usBufIdx;
    UINT16           usCcIdx;
    AGC_CC_STATUS_ENUM_UINT16       enCcStatus;                   
}AGC_ONE_FREQ_RPT_STRU;

typedef struct
{
    AGC_ONE_FREQ_RPT_STRU astFreqInfo[18]; //共18个频点，HIDS 上报识别宏有问题，用数字来代替
}AGC_FREQ_RPT_STRU;
...
typedef struct
{
    ...
    UINT8                           ucValidFlg;     /*接口生效标志*/
    UINT8                           ucDualPssFlag;  /*双通道扫频的标志*/
    UINT8                           ucActFlag;      /*载波激活标志*/
    UINT8                           ucDeActFlag;    /*载波去激活标志*/
    UINT8                           ucChChangeFlg;  /*载波通道变化标志*/
    UINT8                           ucActHoldFlg;   /*载波因为激活 hold*/
    UINT8                           ucDeActHoldFlg; /*载波因为去激活 hold*/
