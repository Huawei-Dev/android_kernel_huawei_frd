...
*/
void ESS_ConfigureSchedulingInterrupt( ESS_Handle_t *puhInterrupt,
                                       void (*ESS_InstallHandler)( uint32_t *,
                                                                   ESS_LL_IF_STRU *,
                                                                   void (*) ( void *, uint32_t ),
                                                                   void   *)
                                       );
...
/** **************************************************************************
    Creates a Scheduling loop to which jobs can be added. The loop
    will count from 0 to uwMaxCounter - 1. This count is used by jobs
    to specify when they shall execute for the first time. Jobs added
    to the loop will execute every uwPeriod of events of the
    underlying loop (interrupt or loop handle)
...
*/
void ESS_ConfigureSchedulerLoop( ESS_Handle_t *punLoop,
                                 ESS_Handle_t unHandle,
                                 uint32_t uwPeriod,
...
/** **************************************************************************
    ...
    Adds a recurring job to a a work queue. The job will be executed
    each time the time defined by auwStart occurs. The job will be
    waiting until the counter of the loop handle to which the job has
    been added reaches the value specified by 'auwStart' (@sa
    ESS_Time_t).

    @note jobs can only be added to handles that are
    created by ESS_CreateSchedulingLoop!

    @note Jobs are per defeault added in the waiting state (waiting
    for the loop counter to reach the value specified in auwStart @sa
...
*/
void ESS_AddJob( ESS_Handle_t      *punJob,
                 ESS_Handle_t       unHandle,
...
/** **************************************************************************
    Adds an oneshot pfJobFun to a a work queue. The pfJobFun will be
...
*/
void ESS_AddOneshotJob( ESS_Handle_t      *punJob,
                        ESS_Handle_t       unHandle,
                        const ESS_Time_t   auwStart,
                        int32_t          swPriority,
                        ESS_Job_t          pfJobFun,
...
/** **************************************************************************
  ...
  @return       void
*/
void ESS_Delete(ESS_Handle_t *punHandle);
