...
struct NewEvent{
    enum UFSM_ERROR_MSG_ENUM eLogType;
...
#define pstLogFunctions pfLogFilter
...
*****************************************************************************/
/**
 * Context pointer to @ref UFSM_EVENT_HANDLER functions. Used as
 * instance pointer by function calls that is supposed to be called from
 * within a state machine.
 *
 * @sa UFSM_Error
 */
typedef void UFSM_Context_t;
 ...
 */
typedef UFSM_STATE_T (*UFSM_EVENT_HANDLER)( UFSM_Context_t*, void*, int32_t );

/**
 * Log state handler interface, mntn callback
 *
 * Every logging fsm state handler takes three arguments.
 * The first is FsmId, second is current state, third is
 * the new state.
 */
typedef void (*UFSM_LOG_STATE_HANDLER)( uint16_t, int32_t, int32_t );
...
typedef struct UFSM_FSM
{
    ...
    int32_t swLogId;
    size_t extraStorage;
    int (*pfLogFilter)(void *, void *, const union UFSM_LogInfo *);
...
/**
 * Registers an mntn handler to log fsm state.
 *
 * @param[in] pfLogStateHandler  Function that is called when an LogState.
 */
void UFSM_RegisterMntnLogStateHandler( UFSM_LOG_STATE_HANDLER pfLogStateHandler );
 ...
 * @sa UFSM_GetError
 */
void UFSM_Error(UFSM_Context_t *pstFsm, int32_t swErrorCode);
 ...
 * the init handler. Not the handle returned by UFSM_Register()
 *
 * @param[in] pstCtx      Pointer to a handle passed to the init handler during the registraion of the FSM.
 * @return The error code reported by a call to UFSM_Error() untill UFSM_HandleEvent()
 *         returns, after that it will report 0.
 *
 * @sa UFSM_Error
 */
int32_t UFSM_GetError(UFSM_Context_t *pstCtx);
...
/**
 * Get Current state from Context pointer.
 * @param[in] pstCtx Context pointer passed to statemachine handler function.
 *
 * @return Current state.
 */
int32_t UFSM_GetMyState(UFSM_Context_t *pstCtx);

size_t UFSM_GetMySize(UFSM_Context_t *pstCtx);
