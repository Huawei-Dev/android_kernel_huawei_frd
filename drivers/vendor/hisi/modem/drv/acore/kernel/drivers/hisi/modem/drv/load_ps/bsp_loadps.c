...
s32 bsp_loadps_reset_cb(DRV_RESET_CB_MOMENT_E eparam, s32 userdata)    /*lint !e830*/
{
    ...
    else
    {
        ...
        if(EN_LOADPS_DOING == g_loadpsMain.opState
            ||EN_LOADPS_SUSPEND == g_loadpsMain.opState)
        {
...
s32 load_ps_task(void* obj)
{
    ...
    while(1)
    {
        ...
        {
            /* 表示此任务要被睡眠状态*/
            loadps_trace(BSP_LOG_LEVEL_ERROR ," loadps g_loadpsMain.opState = EN_LOADPS_SUSPEND.\n");
            g_loadpsMain.opState = EN_LOADPS_SUSPEND;
            osl_sem_down(&g_loadpsMain.suspend_mutex);
            /*睡眠唤醒后就表示开始执行状态*/
            loadps_trace(BSP_LOG_LEVEL_ERROR ," loadps g_loadpsMain.opState = EN_LOADPS_DOING.\n");
            g_loadpsMain.opState = EN_LOADPS_DOING;
        }
...
static s32 modem_loadps_resume(struct device *dev)
{
    dev_info(dev,"%s:+\n",__func__);
    g_loadpsMain.eInitFlag = EN_LOADPS_INIT_FINISH;
    if(g_loadpsMain.opState == EN_LOADPS_SUSPEND)
    {
