...
{
#ifdef HI_PINTRL_FIX_REG_ADDR
	u32 base = 0;
        if(gpio <= 125)
            base = HI_PINTRL_REG_ADDR;
		else if(gpio >= 144 && gpio <= 155)
			base = HI_PINTRL_FIX_REG_ADDR;
		else if(gpio >= 160 && gpio <= 165)
			base = HI_PINTRL_MMC0_REG_ADDR;
		else if(gpio >= 168 && gpio <= 173)
			base = HI_PINTRL_MMC1_REG_ADDR;
        else if(gpio >= 178 && gpio <= 221)
            base = HI_PINTRL_SYS_REG_ADDR;
        else
			base = 0;
		return base;
#else
