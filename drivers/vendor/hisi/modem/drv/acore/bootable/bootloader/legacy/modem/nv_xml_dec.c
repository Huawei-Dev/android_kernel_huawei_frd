...
XML_RESULT_ENUM_UINT32 xml_write_nv_data(void)
{
    ...
    rreq.itemid = ref_info.itemid;
    rreq.modemid = xml_ctrl.card_type;
    rreq.pdata = (u8*)xml_ctrl.g_puclnvitem;
    rreq.size = ref_info.nv_len;
    rreq.offset = 0;

    returnval = nv_boot_write_to_mem(&rreq, &ref_info);
    if(returnval)
    {
        xml_write_error_log(__LINE__, usnvitemid, returnval);
        goto out;
