...
u32 bsp_emmc_read(const char *partition_name, u32 partition_offset, void* ptr_ram_addr, u32 length,u32* skip_len)
{
    ...
    u32 len;
    struct storage_operators *storage_ops = NULL;

	/*lint -save -e838*/
	storage_ops = (struct storage_operators *)get_operators(STORAGE_MODULE_NAME_STR);
	if(!storage_ops){
		cprintf("can not get storage_ops!\n");
		return 4;
	}
	/*lint -restore +e838*/
	//lint --e{605}
	...
	ret = storage_ops->storage_read(partition_name, (unsigned long long)sec_off, (void *)block_data, (unsigned long long)len);
	if (ret)
    {
