...
#ifdef SHM_SIZE_SEC_ICC
	memset((void *)(SHM_SEC_BASE_ADDR + SHM_OFFSET_SEC_ICC), 0, SHM_SIZE_SEC_ICC);/* [false alarm]:屏蔽Fortify错误 */
#endif
...
void modem_config_info(int coreid, void* pconfig)
{
	...
	if (coreid == MODEM_CONFIG_MCU) {
		...
		/*温保相对共享DDR偏移地址*/
		pmcuconfig->temprature.addr = (u32)(SHM_OFFSET_TEMPERATURE);
		pmcuconfig->temprature.size = (u32)(SHM_SIZE_TEMPERATURE);
		/*ICC相对共享DDR偏移地址*/
		pmcuconfig->icc_ccpu2mcu.addr = (u32)((unsigned long)ADDR_MCCORE_RECV - SHM_BASE_ADDR);
		pmcuconfig->icc_ccpu2mcu.size = (u32)(STRU_SIZE + ICC_MCCORE_SIZE);
		pmcuconfig->icc_mcu2ccpu.addr = (u32)((unsigned long)ADDR_MCCORE_SEND - SHM_BASE_ADDR);
		...
		pmcuconfig->modem_pm_stats.addr = (u32)(SHM_OFFSET_MODEM_SR_STAMP);
...
struct modem_dump_info modem_dump_infos[]={
    {"modem_ddr"   ,MDDR_FAMA(DDR_MCORE_ADDR)       ,DDR_MCORE_SIZE         },
    {"modem_share" ,MDDR_FAMA(DDR_SHARED_MEM_ADDR)  ,DDR_SHARED_MEM_SIZE    },
    {"modem_dump"  ,MDDR_FAMA(DDR_MNTN_ADDR)        ,DDR_MNTN_SIZE          },
    {"modem_dt"    ,MDDR_FAMA(DDR_MCORE_DTS_ADDR)   ,DDR_MCORE_DTS_SIZE     },
    {"tldsp_ddr"   ,MDDR_FAMA(DDR_TLPHY_IMAGE_ADDR) ,DDR_TLPHY_IMAGE_SIZE   },
#ifdef DDR_CBBE_IMAGE_ADDR
    {"xdsp_ddr"    ,MDDR_FAMA(DDR_CBBE_IMAGE_ADDR)  ,DDR_CBBE_IMAGE_SIZE    },
#endif
};

void modem_dump_regist(void)
{
    int i,num;
    struct mntn_ddrdump_operators *ddrdump_ops = NULL;

    num = sizeof(modem_dump_infos)/sizeof(modem_dump_infos[0]);
    /*lint -save -e838*//*838为ddrdump_ops未使用，但是以下明明使用了，所以此处注释掉*/
    ddrdump_ops = get_operators(DDRDUMP_MODULE_NAME_STR);
    /*lint -restore*/
    if(NULL == ddrdump_ops)
    {
        PRINT_ERROR("modem get mntn_ddrdump operators fail!\n");
        return;
    }
    for(i = 0; i < num; ++i){
        ddrdump_ops->ddrdump_register(modem_dump_infos[i].name, modem_dump_infos[i].base, modem_dump_infos[i].size);
    }
}
