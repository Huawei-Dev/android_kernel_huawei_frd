...
u32 nv_boot_dload_info_init(void)
{
    ...
    {
        printf("dload file is not exsit, nv_dload.nv_bin.magic_num = 0x%x\n", nv_dload.nv_bin.magic_num);
...
/*lint -restore +e801*/
    ...
    if(ret)
    {
        printf("[%s]:there is no nv useable packet to update, load nv directory!\n",__FUNCTION__);
    }
...
u32 nv_boot_write_to_mem(nv_wr_req *wreq, nv_item_info_s *item_info)
{
	...
	/*lint -save -e838*//*mdm_base明明使用了，此处仍报该变量的赋值没有使用，因此屏蔽*/
...
{
    /*lint -save -e835*//*835为NV_GLOBAL_CTRL_INFO_ADDR中有加0的地方，问题引入为bsp_shared_ddr.h中有加0的地方，所以此处注释掉*/
    nv_global_info_s* global_info = (nv_global_info_s*)NV_GLOBAL_INFO_ADDR;

    return (u8*)((unsigned long)NV_GLOBAL_CTRL_INFO_ADDR
...
u32 nv_readEx(u32 modem_id,u32 itemid,u32 offset,u8* pdata,u32 datalen)
{
	...
	/*lint -save -e835 -e838*//*835为NV_GLOBAL_CTRL_INFO_ADDR中有加0的地方，问题引入为bsp_shared_ddr.h中有加0的地方，所以此处注释掉,838为ret值未使用*/
    ret = nv_boot_search_byid(itemid,(u8*)NV_GLOBAL_CTRL_INFO_ADDR,&item_info,&file_info);
    if(ret)
    {
        printf("\n[%s]:can not find 0x%x , ret = 0x%x!\n",__FUNCTION__,itemid, ret);
        return BSP_ERR_NV_NO_THIS_ID;
    }
	/*lint -restore*/
    if((offset + datalen) > item_info.nv_len)
    {
        printf("\n[%s]:read len is too large nvid=0x%x itemlen=0x%x rdlen=0x%x offset=0x%x!\n",
                __FUNCTION__,itemid,item_info.nv_len,datalen,offset);
    ...
    ret = nv_read_from_mem(&rreq, &item_info);
    if(ret)
    {
        printf("\n[%s]:read nv from memory failed nvid=0x%x 0x%x!\n",__FUNCTION__,itemid, ret);
        return ret;
    }
