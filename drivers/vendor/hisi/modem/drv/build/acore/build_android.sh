...
case "$CFG_OS_ANDROID_PRODUCT_NAME" in
	...
	cd ${CFG_ANDROID_DIR}
	. build/envsetup.sh
	choosecombo release ${CFG_OS_ANDROID_PRODUCT_NAME} ${android_type}
	if [ -z "${ANDROID_MAKE_GOALS}" ]; then
		make BUILD_TINY_ANDROID=true -j${NumJobs}
		echo "make USE_MINGW=y fastboot factory_image_tool split_tool"
		make USE_MINGW=y fastboot factory_image_tool split_tool
	else
