...
)
{
    ...
    VOS_UINT8                           ucNumberType;
    VOS_UINT8                           aucAsciiNum[MN_CALL_MAX_CALLED_ASCII_NUM_LEN + 1];
    VOS_UINT8                           aucDisplayName[MN_CALL_DISPLAY_NAME_STRING_SZ + 1];
    VOS_UINT16                          usLength;
    AT_CLCC_MODE_ENUM_U8                enClccMode;
    ...
    ucNumberType    = AT_NUMBER_TYPE_UNKOWN;
    usLength        = 0;
    PS_MEM_SET(aucDisplayName, 0x00, sizeof(aucDisplayName));
     ...
     && ( ucNumOfCalls <=  AT_CALL_MAX_NUM))
    {
        for (ucTmp = 0; ucTmp < ucNumOfCalls; ucTmp++)
        {
            ...
            PS_MEM_CPY( aucDisplayName,
                        pstCallInfos->astCallInfos[ucTmp].stDisplayName.acDisplayName,
                        pstCallInfos->astCallInfos[ucTmp].stDisplayName.ucNumLen);

            aucDisplayName[pstCallInfos->astCallInfos[ucTmp].stDisplayName.ucNumLen] = '\0';
            ...
            if (MN_CALL_DIR_MO == pstCallInfos->astCallInfos[ucTmp].enCallDir)
            {
                ...
                else
                {
                    ...
                    usLength += (VOS_UINT16)At_sprintf(AT_CMD_MAX_LEN,
                                                       (VOS_CHAR *)pgucAtSndCodeAddr,
                                                       (VOS_CHAR *)pgucAtSndCodeAddr + usLength,
                                                       ",\"\",%d,\"%s\"",ucNumberType,aucDisplayName);
                }
            }
            else
            {
...
)
{
    ...
    /* 记录<RabId> */
    gastAtClientTab[ucIndex].ucPsRabId  = pstEvent->ucNewRabId;
