...
)
{
    ...
    for (ulLoop = 0; ulLoop < ulNum; ulLoop++)
    {
...
#if 0
        /* $GPGSA */
        else if (0 == VOS_MemCmp(pcTemp, AT_PARA_NMEA_GPGSA, AT_PARA_NMEA_MIN_LEN))
        {
            pstNmeaRep->bitGPGSA = VOS_TRUE;
        }
        /* $GPGSV */
        else if (0 == VOS_MemCmp(pcTemp, AT_PARA_NMEA_GPGSV, AT_PARA_NMEA_MIN_LEN))
        {
            pstNmeaRep->bitGPGSV = VOS_TRUE;
        }
        /* $GPGSV */
        else if (0 == VOS_MemCmp(pcTemp, AT_PARA_NMEA_GPGSV, AT_PARA_NMEA_MIN_LEN))
        {
        ...
        /* $GPVTG */
        else if (0 == VOS_MemCmp(pcTemp, AT_PARA_NMEA_GPVTG, AT_PARA_NMEA_MIN_LEN))
        {
            pstNmeaRep->bitGPVTG = VOS_TRUE;
        }
#endif
        else
        {
...
)
{
    ...
    if (AT_OK == ulResult)
    {
          ...
          || (TAF_SSA_LCS_MOLR_ENABLE_TYPE_NMEA_GAD == pstGetLcsMolrCnf->enEnable) )
        {
...
#if 0
            if (VOS_TRUE == pstGetLcsMolrCnf->stNmeaRep.bitGPGSA)
            {
                bNmeaFlg  = VOS_TRUE;
                usLength += (VOS_UINT16)At_sprintf(AT_CMD_MAX_LEN,
                                                   (VOS_CHAR *)pgucAtSndCodeAddr,
            ...
            if (VOS_TRUE == pstGetLcsMolrCnf->stNmeaRep.bitGPGSV)
            {
                bNmeaFlg  = VOS_TRUE;
                usLength += (VOS_UINT16)At_sprintf(AT_CMD_MAX_LEN,
                                                   (VOS_CHAR *)pgucAtSndCodeAddr,
            ...
            if (VOS_TRUE == pstGetLcsMolrCnf->stNmeaRep.bitGPVTG)
            {
                bNmeaFlg  = VOS_TRUE;
                usLength += (VOS_UINT16)At_sprintf(AT_CMD_MAX_LEN,
                                                   (VOS_CHAR *)pgucAtSndCodeAddr,
