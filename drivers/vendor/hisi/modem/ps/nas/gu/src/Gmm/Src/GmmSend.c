                                   ...
                                   )
{
    ...
    ucImsiRefreshStatus = NAS_MML_GetImsiRefreshStatus();
    ...
    case USIMM_GSM_EFLOCIGPRS_ID:
        ...
        /* IMSI Refresh, 且是USIMM_RESET类型的Refresh, 需要清空PTMSI */
        if ((VOS_TRUE == ucImsiRefreshStatus)
...
)
{
    ...
    pstMsg->ulAttachAttCounter            = g_GmmAttachCtrl.ucAttachAttmptCnt;
    pstMsg->ucDelEplmnFlg = VOS_TRUE;

    if ((RR_REL_CAUSE_RR_CONNECTION_FAILURE_NO_RF == g_GmmGlobalCtrl.ucRelCause)
     || (RR_REL_CAUSE_RR_CONNECTION_FAILURE_FAST_RETURN == g_GmmGlobalCtrl.ucRelCause))
    {
        NAS_NORMAL_LOG(WUEPS_PID_GMM, "NAS_GMM_SndMmCombinedAttachRejected: ucDelEplmnFlg set to FALSE");
        pstMsg->ucDelEplmnFlg = VOS_FALSE;
    }
...
)
{
    ...
    if ((RR_REL_CAUSE_RR_CONNECTION_FAILURE_NO_RF == g_GmmGlobalCtrl.ucRelCause)
     || (RR_REL_CAUSE_RR_CONNECTION_FAILURE_FAST_RETURN == g_GmmGlobalCtrl.ucRelCause))
    {
        NAS_NORMAL_LOG1(WUEPS_PID_GMM, "NAS_GMM_SndMmCombinedRauRejected: ucDelEplmnFlg set to FALSE", g_GmmGlobalCtrl.ucRelCause);
        pstMsg->ucDelEplmnFlg = VOS_FALSE;
    }
...
)
{
    ...
    NAS_GMM_UpdateDamNoSearchStatus(enActionType, enActionResult, enCause);
...
VOS_VOID NAS_GMM_SndMmRadioAccesCapaChangeInd(VOS_VOID)
{
    GMMMM_RADIO_ACCESS_CAP_CHANGE_IND_STRU                 *pstMsg          = VOS_NULL_PTR;
    GMMMM_RADIO_ACCESS_CAP_CHANGE_IND_STRU                 *pstInternalMsg  = VOS_NULL_PTR;
    VOS_UINT32                                              ulLen;

    /* 从内部消息队列中获取一个还没有使用的空间 */
    ulLen           = sizeof(GMMMM_RADIO_ACCESS_CAP_CHANGE_IND_STRU);

    pstInternalMsg  = (GMMMM_RADIO_ACCESS_CAP_CHANGE_IND_STRU*)NAS_MML_GetIntMsgSendBuf(ulLen);
    if ( VOS_NULL_PTR == pstInternalMsg)
    {
        NAS_ERROR_LOG(WUEPS_PID_GMM, "NAS_GMM_SndMmRadioAccesCapaChangeInd,ERROR:ALLOC BUFFER WRONG!");
        return;
    }
    ...
    pstMsg->stMsgHeader.ulReceiverPid     = WUEPS_PID_MM;
    pstMsg->stMsgHeader.ulLength          = ulLen - VOS_MSG_HEAD_LENGTH;
    pstMsg->stMsgHeader.ulMsgName         = GMMMM_RADIO_ACCESS_CAP_CHANEG_IND;
