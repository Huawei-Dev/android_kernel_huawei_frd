...
VOS_UINT8 GMM_Fill_IE_ClassMark3(
    VOS_UINT8                          *pClassMark3                             /* 填写ClassMark3 IE的首地址   */
)
{
    VOS_UINT8                           ucLen;
    NAS_UTRANCTRL_UTRAN_MODE_ENUM_UINT8 ucUtranMode;

    ucLen                               = 0;
    ucUtranMode                         = NAS_UTRANCTRL_GetCurrUtranMode();
    
    if (NAS_UTRANCTRL_UTRAN_MODE_FDD == ucUtranMode)
    {
        ucLen = GMM_Fill_IE_FDD_ClassMark3(pClassMark3);
    }
    else if (NAS_UTRANCTRL_UTRAN_MODE_TDD == ucUtranMode)
    {
        ucLen = GMM_Fill_IE_TDD_ClassMark3(pClassMark3);
    }
    else
    {
        NAS_NORMAL_LOG(WUEPS_PID_GMM, "GMM_Fill_IE_ClassMark3: invalid speech version");    
    }
...
NAS_MSG_STRU *Gmm_AttachRequestMsgMake(VOS_VOID)
{
    ...
    ucAttachType                        = (VOS_UINT8)NAS_GMM_GetAttachType();
    
    if ((GMM_COMBINED_GPRS_IMSI_ATTACH      == ucAttachType)
      ||(GMM_GPRS_ATTACH_WHILE_IMSI_ATTACH  == ucAttachType))
    {
    ...
    if (cVersion >= PS_PTL_VER_R9)
    {
        if (VOS_TRUE == NAS_MML_GetSupportSrvccFlg())
        {
            ...
            /* 填写classmark3 */
            /* 降圈复杂度 */
            ucNumber += GMM_Fill_IE_ClassMark3(&pAttachRequest->aucNasMsg[ucNumber]);            
...
VOS_VOID Gmm_DelPsLocInfoUpdateUsim(VOS_VOID)
{
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_GMM, "Gmm_DelPsLocInfoUpdateUsim: USIM card, update USIMM_USIM_EFPSLOCI_ID");
        Gmm_SndAgentUsimUpdateFileReq(USIMM_USIM_EFPSLOCI_ID);       /* 更新SIM卡信息                            */
    }
    else
    {
        NAS_NORMAL_LOG(WUEPS_PID_GMM, "Gmm_DelPsLocInfoUpdateUsim: SIM card, update USIMM_GSM_EFLOCIGPRS_ID");
    ...
    {                                                                           /* 原来的Key情报有效                        */
        ...
        if (NAS_MML_SIM_TYPE_USIM == NAS_MML_GetSimType())
        {
            NAS_NORMAL_LOG(WUEPS_PID_GMM, "Gmm_DelPsLocInfoUpdateUsim: USIM card, update USIMM_USIM_EFKEYSPS_ID");
            Gmm_SndAgentUsimUpdateFileReq(USIMM_USIM_EFKEYSPS_ID);              /* 通知AGENT删除PS域加密信息                */
        }
        else
        {
            NAS_NORMAL_LOG(WUEPS_PID_GMM, "Gmm_DelPsLocInfoUpdateUsim: SIM card, update USIMM_GSM_EFKCGPRS_ID");
            Gmm_SndAgentUsimUpdateFileReq(USIMM_GSM_EFKCGPRS_ID);               /* 通知AGENT删除CKSN      */
        }
                              ...
                              )
{
    ...
    if (RR_REL_CAUSE_RR_CONNECTION_FAILURE_NO_RF != g_GmmGlobalCtrl.ucRelCause)
    {
        ...
        NAS_NORMAL_LOG1(WUEPS_PID_GMM, "Gmm_AttachAttemptCounter: Not NO RF Scene, increase ucRauAttmptCnt", g_GmmRauCtrl.ucRauAttmptCnt);
    }
...
VOS_VOID Gmm_RcvSmEstablishReq(VOS_VOID  *pMsg)
{
    ...
    case GMM_REGISTERED_INITIATED:
        NAS_NORMAL_LOG(WUEPS_PID_GMM, "Gmm_RcvSmEstablishReq: GMM_REGISTERED_INITIATED state, set ucSmCnfFlg to TRUE");
    ...
    case GMM_DEREGISTERED_NORMAL_SERVICE:
        ...
        NAS_NORMAL_LOG(WUEPS_PID_GMM, "Gmm_RcvSmEstablishReq: GMM_DEREGISTERED_NORMAL_SERVICE state, set ucSmCnfFlg and ucFollowOnFlg to TRUE");
        g_GmmAttachCtrl.ucSmCnfFlg             = GMM_TRUE;                      /* 置给SM回EST_CNF标志为TRUE                */
        ...
        if (VOS_TRUE == NAS_GMM_IsNeedEnableLte_AttachPs())
        {
            ...
            {
                g_GmmGlobalCtrl.ucSpecProc = NAS_GMM_GetAttachProcType();
...
#endif
        NAS_NORMAL_LOG(WUEPS_PID_GMM, "Gmm_RcvSmEstablishReq, DEREGISTERED_NORMAL_SERVICE trigger ATT");
    ...
    case GMM_DEREGISTERED_PLMN_SEARCH:
        if (VOS_TRUE == NAS_MML_GetSimPsRegStatus())
        {
            ...
            {
                NAS_MML_SetPsAttachAllowFlg(VOS_TRUE);
                NAS_NORMAL_LOG(WUEPS_PID_GMM, "Gmm_RcvSmEstablishReq: GMM_DEREGISTERED_PLMN_SEARCH state, set ucSmCnfFlg and ucFollowOnFlg to TRUE");
                g_GmmAttachCtrl.ucSmCnfFlg             = GMM_TRUE;              /* 置给SM回EST_CNF标志为TRUE                */
                g_GmmGlobalCtrl.ucFollowOnFlg          = GMM_TRUE;              /* 设置follow on 标志                       */
                NAS_MML_SetPsServiceBufferStatusFlg(VOS_TRUE);
                            ...
                            )
{
    ...
    {                                                                           /* 等待触发RAU                              */
        ...
        {                                                                       /* ATTACH流程成功结束                       */
            ...
            NAS_NORMAL_LOG(WUEPS_PID_GMM, "Gmm_RcvAttachAcceptMsg trigger rau");
                                    ...
                                    )
{
    ...
    else
    {
       ...
       NAS_NORMAL_LOG(WUEPS_PID_GMM, "Gmm_RcvMmcGmmAttachReq_DeregInit, DEREGISTERED_NORMAL_SERVICE trigger ATT");
                                        ...
                                        )
{
    ...
    {                                                                           /* 用户模式为A，网络模式为I                 */
        ...
        {                                                                       /* 注册CS域                                 */
            ...
            NAS_NORMAL_LOG(WUEPS_PID_GMM, "Gmm_RcvMmcGmmAttachReq_ServReqInit trigger rau");
                                         ...
                                         )
{
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_GMM, "Gmm_RcvMmcGmmAttachReq_DeregNmlServ, trigger ATT");
                                            ...
                                            )
{
    ...
    NAS_NORMAL_LOG(WUEPS_PID_GMM, "Gmm_RcvMmcGmmAttachReq_DeregAtmpToAtch: trigger ATT");
                                       ...
                                       )
{
    ...
    {                                                                           /* 用户模式为A，网络模式为I                 */
        ...
        {                                                                       /* 注册CS域                                 */
            ...
            {
                NAS_NORMAL_LOG(WUEPS_PID_GMM, "Gmm_RcvMmcGmmAttachReq_RegNmlServ trigger rau");
                                          ...
                                          )
{
    ...
    NAS_NORMAL_LOG(WUEPS_PID_GMM, "Gmm_RcvMmcGmmAttachReq_RegAtmpToUpdt trigger rau");
                                            ...
                                            )
{
    ...
    {                                                                           /* 用户模式为A，网络模式为I                 */
        ...
        {                                                                       /* 注册CS域                                 */
            ...
            NAS_NORMAL_LOG(WUEPS_PID_GMM, "Gmm_RcvMmcGmmAttachReq_RegAtmpToUpdtMm trigger rau");
