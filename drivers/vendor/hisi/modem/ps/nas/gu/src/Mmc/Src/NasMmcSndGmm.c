...
VOS_VOID NAS_MMC_SndGmmPowerOffNotify(VOS_VOID)
{
    MMCGMM_POWER_OFF_NOTIFY_STRU       *pstMsg = VOS_NULL_PTR;

    /* 从内部消息队列中获取一个还没有使用的空间 */
    pstMsg  = (MMCGMM_POWER_OFF_NOTIFY_STRU*)NAS_MML_GetIntMsgSendBuf(sizeof(MMCGMM_POWER_OFF_NOTIFY_STRU));
    if ( VOS_NULL_PTR == pstMsg)
    {
        return;
    }
    ...
    pstMsg->MsgHeader.ulReceiverCpuId   = VOS_LOCAL_CPUID;
    pstMsg->MsgHeader.ulReceiverPid     = WUEPS_PID_GMM;
    pstMsg->MsgHeader.ulLength          = sizeof(MMCGMM_POWER_OFF_NOTIFY_STRU) - VOS_MSG_HEAD_LENGTH;

    /* 填写消息类别  */
    pstMsg->MsgHeader.ulMsgName         = MMCGMM_POWER_OFF_NOTIFY;              /* 填写消息名称                             */
