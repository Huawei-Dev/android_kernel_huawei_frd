    ...
    */
    ...
    else
    {
        ...
        if ( (VOS_TRUE != ulIsTestCard)
          || (NAS_CC_CALL_STATE_U19 != enCallState))
        {
...
)
{
    ...
    ucTiNonregularCustom = NAS_CC_GetTiNonregularCustom();
    ...
    *pulEntityId = NAS_CC_GetEntityByTi(*pucMsTi);
    ...
    /* IOT规避方案，第一路被叫，第二路主叫，第二路网络下发的connect时带的TI值是0
       TI在CC实体中会找不到，在此处手动修正TI值，即TI加1，然后再获取对应的CC实体
    */
    if ((NAS_CC_INVALID_ENTITY_ID == *pulEntityId)
     && (VOS_TRUE == ucTiNonregularCustom)
     && (NAS_CC_MSG_SETUP != *pucMsgType))
    {
        *pulEntityId = NAS_CC_GetEntityByTi(*pucMsTi + 1);

        if (NAS_CC_INVALID_ENTITY_ID != *pulEntityId)
        {
            NAS_CC_WARN_LOG("NAS_CC_CheckAirMsgValidity: TI Add 1.");

            NAS_CC_SetTiNonregularAddedFlg(VOS_TRUE);
