...
)
{
    ...
    for ( i = 0; i < 7; i++ )
    {
        ...
        if ( 0 != (g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].aucMMConnExtFlg[0]
                        & (0x01 << i)) )
        {
            ucCsSrvFlg  = VOS_TRUE;

            /* 通知LCS模块链接释放 */
            NAS_MM_SndLcsRelInd(i, enMmCcRelCause);
        }
        if ( 0 != (g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].aucMMConnExtFlg[1]
                        & (0x01 << i)) )
        {
            ucCsSrvFlg  = VOS_TRUE;

            /* 通知LCS模块链接释放 */
            NAS_MM_SndLcsRelInd(i + 8, enMmCcRelCause);
        }
    ...
    if ( NO_MM_CONN_ESTING != g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].ucMMConnEstingTI )
    {
        ucCsSrvFlg  = VOS_TRUE;

        NAS_MM_SndLcsRelInd(g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].ucMMConnEstingTI,
                            enMmCcRelCause);
        g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].RcvXXEstReq.ucFlg = MM_FALSE;
    }
    ...
    if ( MM_TRUE == g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].RcvXXEstReq.ucFlg )
    {
        ...
        NAS_MM_SndLcsRelInd(g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].RcvXXEstReq.ulTransactionId,
                            enMmCcRelCause);
        g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].RcvXXEstReq.ucFlg = MM_FALSE;
    }
    ...
    g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].ucMMConnEstingTI      = NO_MM_CONN_ESTING;
    ...
    g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].aucMMConnExtFlg[0]    = 0;
    g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].aucMMConnExtFlg[1]    = 0;
...
)
{
    ...
    if (NO_MM_CONN_ESTING != g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].ucMMConnEstingTI)
    {
        NAS_MM_SndLcsRelInd(g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].ucMMConnEstingTI, enMmCcRelCause);
...
VOS_VOID Mm_ComAbortCause6(VOS_VOID)
{
      ...
      && (MM_TRUE != ucMmConnFlg) )
    {
        ...
        Mm_ComSetMmState(RR_CONNECTION_RELEASE_NOT_ALLOWED);
        PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_NORMAL,
            "Mm_ComAbortCause6: MM STATUS is MM_RR_CONNECTION_RELEASE_NOT_ALLOWED.");
    ...
    else
    {
        ...
        Mm_ComSetMmState(WAIT_FOR_NETWORK_COMMAND);
        PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_NORMAL,
            "Mm_ComAbortCause6: MM STATUS is WAIT_FOR_NETWORK_COMMAND.");
        Mm_TimerStart(MM_TIMER_T3240);
    }
...
VOS_VOID Mm_ComAbortCause17()
{
      ...
      && (MM_TRUE != ucMmConnFlg) )
    {
        ...
        Mm_ComSetMmState(RR_CONNECTION_RELEASE_NOT_ALLOWED);
        PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_NORMAL,
            "Mm_ComAbortCause17: MM STATUS is MM_RR_CONNECTION_RELEASE_NOT_ALLOWED.");
        Mm_TimerStart(MM_TIMER_T3241);
    }
    else
    {
        ...
        Mm_ComSetMmState(WAIT_FOR_NETWORK_COMMAND);
        PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_NORMAL,
            "Mm_ComAbortCause17: MM STATUS is WAIT_FOR_NETWORK_COMMAND.");
        Mm_TimerStart(MM_TIMER_T3240);
    }
...
VOS_VOID Mm_ComRejectCause4(VOS_VOID)
{
    VOS_UINT8                           ucMmConnFlg;
      ...
      && (MM_TRUE != ucMmConnFlg) )
    {
        ...
        Mm_ComSetMmState(RR_CONNECTION_RELEASE_NOT_ALLOWED);
        PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_NORMAL,
            "Mm_ComRejectCause4: MM STATUS is MM_RR_CONNECTION_RELEASE_NOT_ALLOWED.");
        Mm_TimerStart(MM_TIMER_T3241);
    }
    else
    {
        ...
        Mm_ComSetMmState(WAIT_FOR_NETWORK_COMMAND);
        PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_NORMAL,
            "Mm_ComRejectCause4: MM STATUS is WAIT_FOR_NETWORK_COMMAND.");
        Mm_TimerStart(MM_TIMER_T3240);
    }
                    ...
                    )
{
    ...
    NAS_NORMAL_LOG(WUEPS_PID_MM, "Mm_T3240Expire: Call NAS_MM_ResetCsPagingFlgAndT3240Len()");
                    ...
                    )
 {
    ...
    {                                                                           /* 没有MM连接存在                           */
        ...
        if (RRMM_LCS_STATE_START == NAS_MM_GetRrLcsState())
        {
            ...
            Mm_ComSetMmState(RR_CONNECTION_RELEASE_NOT_ALLOWED);
            PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_NORMAL,
                "Mm_T3230Expire: MM STATUS is MM_RR_CONNECTION_RELEASE_NOT_ALLOWED.");
            Mm_TimerStart(MM_TIMER_T3241);
        }
        else
        {
            ...
            Mm_ComSetMmState(WAIT_FOR_NETWORK_COMMAND);
            PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_NORMAL,
                "Mm_T3230Expire: MM STATUS is WAIT_FOR_NETWORK_COMMAND.");
            Mm_TimerStart(MM_TIMER_T3240);
        }
    }
    ...
    {                                                                           /* 重建过程                                 */
        ...
        /* WAIT_FOR_NETWORK_COMMAND之后进入的状态 */
...
VOS_VOID Mm_ComCheckDelayMmConn_MM_IDLE_NORMAL_SERVICE(VOS_UINT8   ucLUFlg)
{
    ...
    else if ( MM_TRUE == g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].RcvXXEstReq.ucFlg )
    {
        NAS_MM_DealBufferedLcsEstReq();
    }
    else if ( MM_TRUE ==
    g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_SMS].RcvXXEstReq.ucFlg )
    {
...
{
    if (VOS_TRUE == NAS_MM_IsExistBufferService())
    {
        return MM_FALSE;
    }
...
VOS_UINT8 Mm_ComTiChk(VOS_VOID)
{
    if ( ((MM_CONST_NUM_8 >
        g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_CC].
                                RcvXXEstReq.ulTransactionId )
    ...
    if ( MM_TRUE == g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].RcvXXEstReq.ucFlg )
    {
        if ( ( MM_CONST_NUM_8 > g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].RcvXXEstReq.ulTransactionId )
          || ( MM_CONST_NUM_14 < g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].RcvXXEstReq.ulTransactionId ) )
        {
            /* TI不合法 */
            g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].RcvXXEstReq.ucFlg = MM_FALSE;
...
VOS_VOID Mm_ComLuOnly()
{
    ...
    if ( VOS_TRUE == NAS_MML_GetCsAttachAllowFlg())
    {
        ...
        {
            if ( (MM_CONST_NUM_7 > g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].RcvXXEstReq.ulTransactionId)
              || ( (MM_CONST_NUM_7 < g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].RcvXXEstReq.ulTransactionId)
                && (MM_CONST_NUM_15 > g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].RcvXXEstReq.ulTransactionId) ) )
            {
                /* 设置FOR */
                Msg.MmIeLuType.ucFOR = MM_IE_LU_TYPE_FOR_MASK;
            }
            else
            {
                /* 释放缓存的LCS建链请求 */
                ...
                /* 设置FOR */
                Msg.MmIeLuType.ucFOR = MM_CONST_NUM_0;
            }
        }
        else if ( MM_TRUE ==
            g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_SMS].RcvXXEstReq.ucFlg )
...
VOS_VOID Mm_ComInit ()
{
    ...
    gstMmTimer[MM_TIMER_T3241].ulTimerLen = MM_TIMER_T3241_VALUE;
    ...
    gstMmTimer[MM_TIMER_DELAY_RADIO_CAPA_TRIGED_LAU].ulTimerLen = MM_TIMER_DELAY_RADIO_CAPA_TRIGED_LAU_VALUE;

    gstMmTimer[MM_TIMER_DELAY_NETWORK_SEARCH_AFTER_EMC].ulTimerLen = (VOS_UINT32)(NAS_MML_GetTimerLenOfDelayNetworkSearch() * 1000);
...
VOS_VOID Mm_Nop(VOS_VOID* pMsg)
{
    ...
    PS_NAS_LOG3(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_WARNING,
        "Mm_Nop: MM State, Sender Pid, MsgName:",
        g_MmGlobalInfo.ucState,
        pstMsgHeader->ulSenderPid,
...
VOS_VOID  NAS_MM_RcvSysInfoPsRestrictRegLaiNoChangeU1(VOS_VOID)
{
    ...
    else
    {
        ...
        else if (VOS_TRUE == NAS_MM_IsRadioAccessCapNeedTriggerLau())
        {
            NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_RcvSysInfoPsRestrictRegLaiNoChangeU1: NAS_MM_IsRadioAccessCapNeedTriggerLau is true");
            NAS_MM_ProcBufferedEmgCallOrLau_RcvSysinfo(MM_FALSE, MM_IE_LUT_NORMAL_LU);
...
VOS_VOID    Mm_ComSysInfoHandleWithLaiNoChange(VOS_UINT8     ucUserSpecificSearchFlg)
{
    ...
    NAS_NORMAL_LOG(WUEPS_PID_MM, "Mm_ComSysInfoHandleWithLaiNoChange enter");
    ...
    if (NAS_MML_LOCATION_UPDATE_STATUS_UPDATED == NAS_MML_GetCsUpdateStatus())
    {
        ...
        if ( MM_ATT_NOT_NEED == g_MmGlobalInfo.ucAttFlg )
        {
            ...
            if (VOS_TRUE == NAS_MM_IsRadioAccessCapNeedTriggerLau())
            {
                NAS_NORMAL_LOG(WUEPS_PID_MM, "Mm_ComSysInfoHandleWithLaiNoChange:NAS_MM_IsRadioAccessCapNeedTriggerLau is true");
            ...
            else
            {
                ...
                if ( MM_TRUE == g_MmGlobalInfo.ucPowerOnFlg )
                {
                    ...
                    NAS_NORMAL_LOG(WUEPS_PID_MM, "Mm_ComSysInfoHandleWithLaiNoChange: U1 and MM_ATT_NOT_NEED,ucPowerOnFlg IS TRUE");
        ...
        else
        {
            if ( MM_TRUE == g_MmGlobalInfo.ucPowerOnFlg )
            {
                NAS_NORMAL_LOG(WUEPS_PID_MM, "Mm_ComRcvSysHandle: U1 and MM_ATT_NEED,ucPowerOnFlg is TRUE, TRIGGER LAU");
                ...
                g_MmGlobalInfo.LuInfo.ucT3212ExpiredFlg )
            {
                NAS_NORMAL_LOG(WUEPS_PID_MM, "Mm_ComRcvSysHandle: t3212 expire TRIGGER PERIODIC LAU");
            ...
            /* 由于发起LAU的时候停止了T3212，因此如果T3212没有运行，表示前面已经发起过 */
                  ...
                  && (MM_TIMER_STOP == gstMmTimer[MM_TIMER_T3213].ucTimerStatus))
            {
                NAS_NORMAL_LOG(WUEPS_PID_MM, "Mm_ComRcvSysHandle: U1:t3211,t3212,t3213 is stop TRIGGER LAU");
    ...
    {
        NAS_NORMAL_LOG2(WUEPS_PID_MM, "Mm_ComRcvSysHandle:ucUserSpecificSearchFlg is; g_MmGlobalInfo.ucPowerOnFlg is ;",ucUserSpecificSearchFlg,g_MmGlobalInfo.ucPowerOnFlg);
    ...
    {   /* MM 状态迁回到搜网前的状态 */
        ...
        if ((MM_TIMER_STOP == gstMmTimer[MM_TIMER_T3211].ucTimerStatus)
         ...
         && (0x0 != gstMmTimer[MM_TIMER_T3212].ulTimerLen))
        {
            NAS_NORMAL_LOG(WUEPS_PID_MM, "Mm_ComRcvSysHandle:NOT U1:t3211,t3212 is stop TRIGGER LAU");
        ...
        else if (MM_IDLE_PLMN_SEARCH == g_MmGlobalInfo.ucState)
        {
            ...
            {
                NAS_NORMAL_LOG(WUEPS_PID_MM, "Mm_ComRcvSysHandle:NOT U1:MM_IDLE_PLMN_SEARCH, t3212 is running");
            ...
            {
                NAS_NORMAL_LOG2(WUEPS_PID_MM, "Mm_ComRcvSysHandle:NOT U1:MM_IDLE_PLMN_SEARCH, t3212 is; t3211 is;",gstMmTimer[MM_TIMER_T3212].ucTimerStatus,gstMmTimer[MM_TIMER_T3211].ucTimerStatus);
...
VOS_VOID Mm_ComRcvSysHandle(VOS_UINT8     ucUserSpecificSearchFlg)
{
    ...
    NAS_NORMAL_LOG3(WUEPS_PID_MM, "Mm_ComRcvSysHandle:,g_MmGlobalInfo.ucNotCampONPreState is; ulLaiChangeFlag is ;NAS_MML_GetCsUpdateStatus is",g_MmGlobalInfo.ucNotCampONPreState, ulLaiChangeFlag, NAS_MML_GetCsUpdateStatus());

    /* 在前状态是attempt tO update时,和上一次驻留的LAI相比较;
       在前状态是limit service时,和上次成功驻留的LAI相比较 */
    if (MM_IDLE_PLMN_SEARCH == g_MmGlobalInfo.ucState)
    {
        NAS_NORMAL_LOG(WUEPS_PID_MM, "Mm_ComRcvSysHandle: MM state is MM_IDLE_PLMN_SEARCH");
        ...
        if (NAS_MML_LOCATION_UPDATE_STATUS_NOT_UPDATED == NAS_MML_GetCsUpdateStatus())
        {
            ...
            {
                NAS_NORMAL_LOG(WUEPS_PID_MM, "Mm_ComRcvSysHandle: NOT_UPDATED and LAI changed");
        ...
        else if (MM_IDLE_LIMITED_SERVICE == g_MmGlobalInfo.ucNotCampONPreState)
        {
            ...
            {
                NAS_NORMAL_LOG(WUEPS_PID_MM, "Mm_ComRcvSysHandle: ucNotCampONPreState is LIMITED SERVICE and Lai changed");
    ...
    if ( g_MmGlobalInfo.ucNtMod == g_MmGlobalInfo.ucNewNtMod )
    {
        if (NAS_MML_LOCATION_UPDATE_STATUS_UPDATED == NAS_MML_GetCsUpdateStatus())
        {
            ...
            {
                NAS_NORMAL_LOG(WUEPS_PID_MM, "Mm_ComRcvSysHandle: Network Mode not changed, STATUS UPDATED and LAi changed");
                        ...
                        )
{
    ...
    for ( i = 0 ; i < MM_CONN_CTRL_BUTT; i++ )
    {
        if ( MM_TRUE == g_MmGlobalInfo.ConnCtrlInfo[i].RcvXXEstReq.ucFlg )
        {
               ...
               && ( MM_CONST_NUM_15 > g_MmGlobalInfo.ConnCtrlInfo[i].RcvXXEstReq.ulTransactionId) )
            {
                ...
                else if ( MM_CONN_CTRL_LCS == i )
                {
                    /* 后续LCS支持LMU定位流程时需要判断修改CmSrvType为MM_IE_CM_SVC_TYPE_LCS */
                    ucCmSvsType = MM_IE_CM_SVC_TYPE_SS_ACT;
                    ucPdType    = MM_IE_PD_LOCATION_SRVC;
                }
                else
                {
                    /* 非SS/SMS消息，则异常处理 */
            ...
            {
                /* 通知LCS建链失败 */
                NAS_MM_SndLcsRelInd(g_MmGlobalInfo.ConnCtrlInfo[i].RcvXXEstReq.ulTransactionId,
                                    NAS_MMCM_REL_CAUSE_MM_INTER_ERR_TI_INVALID);
            }
            else
            {
      ...
      && (MM_TRUE != ucMmConnFlg) )
    {
        ...
        Mm_ComSetMmState(RR_CONNECTION_RELEASE_NOT_ALLOWED);
        PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_NORMAL,
            "Mm_ComFollowOnProc: MM STATUS is MM_RR_CONNECTION_RELEASE_NOT_ALLOWED.");
        Mm_TimerStart(MM_TIMER_T3241);
    }
    else
    {
        ...
        Mm_ComSetMmState(WAIT_FOR_NETWORK_COMMAND);
        PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_NORMAL,
            "Mm_ComFollowOnProc: MM STATUS is WAIT_FOR_NETWORK_COMMAND.");
        Mm_TimerStart(MM_TIMER_T3240);
    }
    ...
    case NAS_MML_REG_FAIL_CAUSE_IMSI_UNKNOWN_IN_VLR:
           ...
           || ( MMCMM_FORBIDDEN_PLMN_FOR_GPRS == g_MmGlobalInfo.MsCsInfo.ulCurFobidnFlg )))
        {
            ...
            */
            ...
            {
                NAS_NORMAL_LOG(WUEPS_PID_MM, "Mm_ComProcCauseProcess_Handling_MM_MM_CONN_PROC: cm rej#4 trigger lau");
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MM, "Mm_ComProcCauseProcess proc is", g_MmGlobalInfo.ucProc);
...
VOS_VOID Mm_TimerStart( VOS_UINT8   ucTimerId )
{
    ...
    if ( MM_TIMER_STOP ==  gstMmTimer[ucTimerId].ucTimerStatus )
    {
        ...
        else
        {
            ...
            if (0 == gstMmTimer[ucTimerId].ulTimerLen)
            {
                NAS_WARNING_LOG(WUEPS_PID_MM, "Mm_TimerStart:ulTimerLen is 0");
                return;
            }
...
)
{
    ...
    for ( i = 0; i < MM_TIMER_MAX; i++ )
    {
        ...
        if ( MM_TIMER_STOP != gstMmTimer[i].ucTimerStatus )
        {
            ...
            {
                NAS_MML_SetCsfbMtPagingTimerStatus(NAS_MML_TIMER_STATUS_STOP);
            }
    ...
    if ( MM_TIMER_STOP_ALL == ucTimerId )
    {
        ...
        for ( i = 0; i < MM_TIMER_MAX; i++ )
        {
            if ( MM_TIMER_STOP != gstMmTimer[i].ucTimerStatus )
            {
                ...
                Mm_StopRelTimer(WUEPS_PID_MM, i, &gstMmTimer[i].hTimer))
                {                                                               /* 停止失败：已经停止或Timer不存在          */
                    PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_WARNING, "Mm_TimerStop:WARNING: STOP ALL TIMER FAILURE!");
                }

                gstMmTimer[i].ucTimerStatus = MM_TIMER_STOP;
                PS_NAS_LOG1(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_INFO, "Mm_TimerStop: Stop All Timer:", i);
            }
        }
    }
...
VOS_VOID Mm_ComNetModeChange()
{
    ...
    NAS_NORMAL_LOG(WUEPS_PID_MM, "Mm_ComNetModeChange enter");
    ...
    if (NAS_MML_LOCATION_UPDATE_STATUS_UPDATED == NAS_MML_GetCsUpdateStatus())
    {
        if ( MM_TRUE == g_MmGlobalInfo.LuInfo.ucT3212ExpiredFlg )
        {
            ...
            NAS_NORMAL_LOG(WUEPS_PID_MM, "Mm_ComNetModeChange,updated and ucT3212ExpiredFlg is true");
        ...
        if (VOS_TRUE == NAS_MM_IsRadioAccessCapNeedTriggerLau())
        {
            NAS_NORMAL_LOG(WUEPS_PID_MM, "Mm_ComNetModeChange: U1,NAS_MM_IsRadioAccessCapNeedTriggerLau is true");
    ...
    switch (ucMmConnCtrl)
    {
        ...
        case MM_CONN_CTRL_SS:
            *pucMMConnEstingPD = MM_IE_PD_NON_CALL_RLT_SS_MSG;
            break;
        case MM_CONN_CTRL_LCS:
            *pucMMConnEstingPD = MM_IE_PD_LOCATION_SRVC;
            break;
    ...
    switch (ucMmConnCtrl)
    {
        ...
        case MM_CONN_CTRL_LCS:
            ...
            g_MmGlobalInfo.ConnCtrlInfo[ucMmConnCtrl].RcvXXEstReq.ulCallType
                = 0;
            g_MmGlobalInfo.ConnCtrlInfo[ucMmConnCtrl].RcvXXEstReq.ulTransactionId
                = NAS_MM_GetLcsEstReqTi();

            g_MmGlobalInfo.ConnCtrlInfo[ucMmConnCtrl].ucMMConnEstingTI
                = (VOS_UINT8)NAS_MM_GetLcsEstReqTi();
...
VOS_VOID NAS_MM_ClearConnCtrlInfo(VOS_UINT8 ucMMConnEstingPD)
{
    ...
    else if(MM_IE_PD_LOCATION_SRVC == ucMMConnEstingPD)
    {
        g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].RcvXXEstReq.ucFlg = MM_FALSE;
    }
    else
...
VOS_VOID NAS_MM_ProcEstCnfSuccessCauseAtState12(VOS_VOID)
{
    ...
    if ( (NO_MM_CONN_ESTING != g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_CC].ucMMConnEstingTI)
       ...
       ||(NO_MM_CONN_ESTING != g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].ucMMConnEstingTI)
...
)
{
    ...
    else if ( NO_MM_CONN_ESTING != g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].ucMMConnEstingTI )
    {
        /* LCS发起的RR连接的建立，通知LCS模块MM连接建立失败 */
        NAS_MM_SndLcsRelInd(g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].ucMMConnEstingTI,
                            enMmCcRelCause);
...
)
{
    ...
    {
        NAS_MM_RelBufferedServiceEstReq(enMmCcRelCause);
    }
...
{
    VOS_UINT8                          ucMmConnExistFlg;

    if (MM_TRUE == Mm_ComMmConnExist())
    {
        ucMmConnExistFlg = VOS_TRUE;
    }
    else
    {
        ucMmConnExistFlg = VOS_FALSE;
    }

    NAS_MML_SetCsServiceConnStatusFlg(ucMmConnExistFlg);
...
{
    VOS_UINT8                           ucBufferServiceExistFlg;

    ucBufferServiceExistFlg = (VOS_UINT8)NAS_MM_IsExistBufferService();

    if (VOS_TRUE == ucBufferServiceExistFlg)
    {
        NAS_INFO_LOG(WUEPS_PID_MM, "NAS_MM_UpdateCsServiceBufferStatusFlg(): Exist Buffered Cs Service!");
    }
    else
    {
        NAS_INFO_LOG(WUEPS_PID_MM, "NAS_MM_UpdateCsServiceBufferStatusFlg(): No Buffered Cs Service!");
    }

    NAS_MML_SetCsServiceBufferStatusFlg(ucBufferServiceExistFlg);
...
)
{
    ...
    switch (ulCause)
    {
        ...
        case NAS_MML_REG_FAIL_CAUSE_AUTH_REJ:
            ...
            NAS_NORMAL_LOG1(WUEPS_PID_MM, "NAS_MM_UpdateEquPlmnInfo: Delete EPLMN since ulCause is", ulCause);
        ...
        case NAS_MML_REG_FAIL_CAUSE_OTHER_CAUSE_REG_MAX_TIMES:

            NAS_NORMAL_LOG1(WUEPS_PID_MM, "NAS_MM_UpdateEquPlmnInfo: Do not delete EPLMN since ulCause is", ulCause);
            break;
        ...
        default:
           ...
           if ((VOS_TRUE == NAS_MML_IsCsLaiChanged())
            ...
            || (NAS_MML_LOCATION_UPDATE_STATUS_UPDATED != enCsUpdateStatus))
            {
                NAS_NORMAL_LOG3(WUEPS_PID_MM, "NAS_MM_UpdateEquPlmnInfo: Delete EPLMN IsLaiChanged, LuAttemptCnt, enCsUpdateStatus", NAS_MML_IsCsLaiChanged(), g_MmGlobalInfo.LuInfo.ucLuAttmptCnt, enCsUpdateStatus);
                NAS_MM_DeleteEPlmnList();
...
VOS_VOID  NAS_MM_ProcCsAcChgRegisterNetModeIIUpdated()
{
    ...
    if (MM_ATT_NOT_NEED == g_MmGlobalInfo.ucAttFlg)
    {
        if (MM_TRUE == g_MmGlobalInfo.LuInfo.ucT3212ExpiredFlg)
        {
            NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_ProcCsAcChgRegisterNetModeIIUpdated: not need att,T3212 expired trigger lau");
    ...
    else
    {
        ...
        if (MM_TRUE == g_MmGlobalInfo.ucPowerOnFlg)
        {
            NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_ProcCsAcChgRegisterNetModeIIUpdated: powerOn trigger lau");
        ...
        {
            NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_ProcCsAcChgRegisterNetModeIIUpdated: need att, T3212 trigger lau");
...
VOS_VOID NAS_MM_ProcCsAcChgRegisterNetModeIINotUpdated(VOS_VOID)
{
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_ProcCsAcChgRegisterNetModeIINotUpdated: PowerOn trigger lau");
     ...
     && (0x0 != gstMmTimer[MM_TIMER_T3212].ulTimerLen))
    {
        ...
        NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_ProcCsAcChgRegisterNetModeIINotUpdated: T3212/T3211 not runing trigger lau");
...
VOS_VOID NAS_MM_ProcCsAcChgRegisterNetModeII(VOS_VOID)
{
    ...
    if (VOS_TRUE == NAS_MML_IsCsLaiChanged())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_ProcCsAcChgRegisterNetModeII: lai change trigger lau");
...
VOS_UINT32  NAS_MM_IsLauNeededLaiNoChange_LInterSys(VOS_VOID)
{
    ...
    {
        NAS_NORMAL_LOG1(WUEPS_PID_MM, "g_MmGlobalInfo.enPreRatType not lte and is; ",g_MmGlobalInfo.enPreRatType);

        return VOS_FALSE;
    }

    /* DAM且T3212在运行，且当前驻留网络和上次失败的网络是同一个网络，则不触发LAU */
    if (VOS_TRUE == NAS_MM_IsDamNotNeedLau())
    {
        return VOS_FALSE;
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MM, "LInterSys tin is guti trigger lau");
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MM, "ucImsVoiceAvailFlg force lau ");
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MM, "camp plmn support dam and t3212 expired ");
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_IsLauNeededLaiNoChange_LInterSys: NAS_MML_LOCATION_UPDATE_STATUS_LOCATION_AREA_NOT_ALLOWED, need LAU");
        return VOS_TRUE;
    }
    ...
    if (NAS_MML_LOCATION_UPDATE_STATUS_NOT_UPDATED == NAS_MML_GetCsUpdateStatus())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_IsLauNeededLaiNoChange_LInterSys: NAS_MML_LOCATION_UPDATE_STATUS_NOT_UPDATED, need LAU");
        return VOS_TRUE;
    }

    NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_IsLauNeededLaiNoChange_LInterSys: return FALSE, no need LAU");
    return VOS_FALSE;
}
...
)
{
    ...
    */
    if (((NAS_MML_TIMER_START == enT3423Status)
     ...
     && (NAS_MML_TIN_TYPE_RAT_RELATED_TMSI == enTinType))
    {
        NAS_NORMAL_LOG2(WUEPS_PID_MM, "NAS_MM_IsT3423StatusChangeNeedRegist is true: enT3423Status, enLteCsServiceCfg",enT3423Status,enLteCsServiceCfg);
...
)
{
    ...
    */
    ...
    if (pstRplmnCfgInfo->enTinType != enTinType)
    {
        ...
        /* 可维可测，勾消息 */
        NAS_MM_LogTinTypeInfo(&stTinInfo);
    }
}
...
VOS_UINT32 NAS_MM_RcvSysInfoSurpportLte_PreProc(VOS_VOID)
{
    ...
    if (VOS_TRUE == NAS_MML_IsCsfbServiceStatusExist())
    {
        ...
        NAS_NORMAL_LOG1(WUEPS_PID_MM, "NAS_MML_IsCsfbServiceStatusExist: CsfbServiceStatus is",NAS_MML_GetCsfbServiceStatus());
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MML_GetDelayedCsfbLauFlg is TRUE");
    ...
    {
        /* 发起LAU流程 有紧急呼先进行紧急呼*/
        NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_RcvSysInfoSurpportLte_PreProc: LAU needed when LAI no changes");
    ...
    {
        /* 发起LAU流程 有紧急呼先进行紧急呼*/
        NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_RcvSysInfoSurpportLte_PreProc: ulT3423StatusChgNeedRegFlg is true, LAU needed");
        NAS_MM_ProcBufferedEmgCallOrLau_RcvSysinfo(ulLaiChangeFlag, MM_IE_LUT_PERIODIC_UPDATING);
...
)
{
    ...
    if (VOS_FALSE == ulLaiChangeFlag)
    {
        ...
        if (MM_CS_SERV_EMERGENCY_CALL_EXIST == ulEmergencyCallFlg)
        {
            ...
            NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_ProcBufferedEmgCallOrLau_RcvSysinfo:lai no change trigger emergency call");
        ...
        NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_ProcBufferedEmgCallOrLau_RcvSysinfo: lai NO change and no buffer emc then trigger LAU");
     ...
     && (MM_CS_SERV_EMERGENCY_CALL_EXIST == ulEmergencyCallFlg))
    {
        ...
        NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_ProcBufferedEmgCallOrLau_RcvSysinfo: lai change trigger lau");
    ...
    NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_ProcBufferedEmgCallOrLau_RcvSysinfo: lai change and no buffer emc then trigger LAU");
...
VOS_UINT32  NAS_MM_IsNeedLauCsSigRel( VOS_VOID )
{
    ...
    if (VOS_TRUE == NAS_MM_IsRadioAccessCapNeedTriggerLau())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_IsNeedLauCsSigRel: NAS_MM_IsRadioAccessCapNeedTriggerLau IS TRUE");
        return VOS_TRUE;
    }
...
VOS_VOID NAS_MM_LocalDetach(VOS_VOID)
{
    ...
    NAS_NORMAL_LOG1(WUEPS_PID_MM, "NAS_MM_LocalDetach MmState is",ulMmState);
...
VOS_UINT32 NAS_MM_IsNeedProcRadioCapChange(VOS_VOID)
{
    NAS_MML_CAMP_PLMN_INFO_STRU        *pstCampInfo       = VOS_NULL_PTR;
    NAS_MML_NETWORK_INFO_STRU          *pstNetWorkInfo    = VOS_NULL_PTR;
    NAS_MML_SIM_FORBIDPLMN_INFO_STRU   *pstForbidPlmnInfo = VOS_NULL_PTR;
    ...
    if ( NAS_MML_NET_MODE_I != pstCampInfo->enNetworkMode )
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MM_IsNeedProcRadioCapChange: not netmode I");
        return VOS_TRUE;
    }
    ...
    if ( VOS_FALSE == NAS_MML_GetPsAttachAllowFlg() )
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MM_IsNeedProcRadioCapChange: ps attach not allow");
    ...
    if ( VOS_TRUE == NAS_MML_GetPsRestrictRegisterFlg() )
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MM_IsNeedProcRadioCapChange: NAS_MML_GetPsRestrictRegisterFlg is true");
    ...
    if ( VOS_FALSE == NAS_MML_GetSimPsRegStatus())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MM_IsNeedProcRadioCapChange: NAS_MML_GetSimPsRegStatus is false");
    ...
    if ( VOS_FALSE == pstNetWorkInfo->stPsDomainInfo.ucPsSupportFlg )
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MM_IsNeedProcRadioCapChange: ucPsSupportFlg is false");
    ...
    if ( VOS_TRUE == ulRet )
    {
        ...
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MM_IsNeedProcRadioCapChange: curr plmn is in astForbGprsPlmnList");

        return VOS_TRUE;
    }

    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MM_IsNeedProcRadioCapChange: rslt is false.");
...
VOS_UINT32 NAS_MM_IsLauNeeded_NetModeChange(VOS_VOID)
{
    ...
    if (VOS_TRUE == NAS_MML_IsCsLaiChanged())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_IsLauNeeded_NetModeChange:NAS_MML_IsCsLaiChanged is true");
    ...
    {
        NAS_NORMAL_LOG1(WUEPS_PID_MM, "NAS_MM_IsLauNeeded_NetModeChange:rac not change",g_MmGlobalInfo.MsCsInfo.ucOldRac);
    ...
    if ( (MM_ATT_NEED  == g_MmGlobalInfo.ucAttFlg)
      && (MM_TRUE      == g_MmGlobalInfo.ucPowerOnFlg) )
    {
        NAS_NORMAL_LOG2(WUEPS_PID_MM, "NAS_MM_IsLauNeeded_NetModeChange:g_MmGlobalInfo.ucAttFlg is ;g_MmGlobalInfo.ucPowerOnFlg is",g_MmGlobalInfo.ucAttFlg,g_MmGlobalInfo.ucPowerOnFlg);
    ...
    {
        NAS_NORMAL_LOG2(WUEPS_PID_MM, "NAS_MM_IsLauNeeded_NetModeChange:ucGsAssociationFlg is ;NAS_MM_IsClassAAndNmoIMode is FALSE",g_MmSubLyrShare.GmmShare.ucGsAssociationFlg,g_MmGlobalInfo.ucNewNtMod);
...
VOS_UINT32 NAS_MM_IsEmergencyCallEndTrigLau(VOS_VOID)
{
    ...
    if ( (VOS_TRUE            == pstConnStatus->ucRrcStatusFlg)
      ...
      && ((MMCMM_NO_FORBIDDEN            == g_MmGlobalInfo.MsCsInfo.ulCurFobidnFlg)
       || (MMCMM_FORBIDDEN_PLMN_FOR_GPRS == g_MmGlobalInfo.MsCsInfo.ulCurFobidnFlg)))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_IsEmergencyCallEndTrigLau: return TRUE");
        return VOS_TRUE;
    }

    NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_IsEmergencyCallEndTrigLau: return FALSE");
    return VOS_FALSE;
...
)
{
    ...
    if ( MM_TRUE == g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_CC].RcvXXEstReq.ucFlg )
    {
        Mm_SndCcRelInd(g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_CC].RcvXXEstReq.ulTransactionId,
                       enRelCause);
        NAS_MM_ClearConnCtrlInfoWithBuffer(MM_CONN_CTRL_CC);
    }
    ...
    if ( MM_TRUE == g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_SS].RcvXXEstReq.ucFlg )
    {
        Mm_SndSsRelInd(g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_SS].RcvXXEstReq.ulTransactionId,
                       enRelCause);
        NAS_MM_ClearConnCtrlInfoWithBuffer(MM_CONN_CTRL_SS);
    }
    ...
    if ( MM_TRUE == g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_SMS].RcvXXEstReq.ucFlg )
    {
        Mm_SndSmsRelInd(g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_SMS].RcvXXEstReq.ulTransactionId,
                        enRelCause);
        NAS_MM_ClearConnCtrlInfoWithBuffer(MM_CONN_CTRL_SMS);
    }
...
VOS_VOID NAS_MM_RestartT3240_WaitForNetworkCommand(VOS_VOID)
{
    ...
    {
        /* NV2380定制打开，且当前有CS PAGING时，延长T3240定时器时长 */
        if ((VOS_TRUE == NAS_MML_GetExtendT3240LenFlg())
         && (VOS_TRUE == NAS_MM_GetCsPaingFlg()))
        {
            gstMmTimer[MM_TIMER_T3240].ulTimerLen = NAS_MML_GetCustomizedT3240Len();
            NAS_NORMAL_LOG1(WUEPS_PID_MM, "NAS_MM_RestartT3240_WaitForNetworkCommand: Set T3240 len", gstMmTimer[MM_TIMER_T3240].ulTimerLen);
        }
...
{
    NAS_MM_RelBufferedServiceEstReqExceptEmergencyCall(NAS_MMCM_REL_CAUSE_RR_CONN_FAIL_IMMEDIATE_ASSIGN_REJECT);

    /* 如果当前no rf，则紧急呼也直接给cc回复失败 */
    if (MM_TRUE == g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_CC].RcvXXEstReq.ucFlg)
    {
        if (VOS_FALSE == NAS_MML_GetRfAvailFlg())
        {
            Mm_SndCcRelInd(g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_CC].RcvXXEstReq.ulTransactionId,
                           NAS_MMCM_REL_CAUSE_RR_CONN_FAIL_IMMEDIATE_ASSIGN_REJECT);
            NAS_MM_ClearConnCtrlInfoWithBuffer(MM_CONN_CTRL_CC);
        }
    }
    ...
    {                                                               /* RAC为0的场合                             */
            ...
            g_MmGlobalInfo.MsCsInfo.ulCurFobidnFlg ) )
        {
                ...
                )
            {
                ...
                PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_INFO, "NAS_MM_ProcessLuAfterCsConnectNotExist_NetModeI:INFO: MmServiceState is MM_LIMITED_SERVICE");
                Mm_ComSetMmState(MM_IDLE_LIMITED_SERVICE);
                ...
                PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_NORMAL, "NAS_MM_ProcessLuAfterCsConnectNotExist_NetModeI:NORMAL: STATUS is MM_IDLE_LIMITED_SERVICE");
            }
        ...
        {
            PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_INFO, "NAS_MM_ProcessLuAfterCsConnectNotExist_NetModeI: unsupport gprs trigger lau");
           g_MmGlobalInfo.LuInfo.ucLuType = MM_IE_LUT_NORMAL_LU;    /* 设置LU类型                               */
           Mm_ComLuOnly();                                          /* LU REQ                                   */
           ucRet = MM_TRUE;
        }
...
{                                                                   /* 非A+I模式                                */
    ...
    {                                                               /* 如果没有CS域禁止的信息                   */
        ...
        {
            g_MmGlobalInfo.ucMmServiceState = MM_LIMITED_SERVICE;   /* 设置服务状态                             */
            PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_INFO, "NAS_MM_ProcessLuAfterCsConnectNotExist_OhterNetMode:INFO: MmServiceState is MM_LIMITED_SERVICE");
            Mm_ComSetMmState(MM_IDLE_LIMITED_SERVICE);
            ...
            PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_NORMAL, "NAS_MM_ProcessLuAfterCsConnectNotExist_OhterNetMode:NORMAL: STATUS is MM_IDLE_LIMITED_SERVICE");
        }
        else if ( MMCMM_FORBIDDEN_LA_FOR_REG_PRVS_SVS
            == ( MMCMM_FORBIDDEN_LA_FOR_REG_PRVS_SVS
                & g_MmGlobalInfo.MsCsInfo.ulCurFobidnFlg ) )
    ...
    {
        PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_INFO, "NAS_MM_ProcessLuAfterCsConnectNotExist_OhterNetMode: forbidden gprs trigger lau");
...
)
{
    ...
    NAS_NORMAL_LOG1(WUEPS_PID_MM, "NAS_MM_ProcessLuAfterCsConnectNotExist_T3212Expire: MM state", g_MmGlobalInfo.ucState);
    ...
    case MM_IDLE_NORMAL_SERVICE:
        ...
        NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_ProcessLuAfterCsConnectNotExist_T3212Expire: idle state need lau");

        Mm_ComLuOnly();                                                     /* 发送LU REQ                               */
        break;
    case MM_IDLE_ATTEMPTING_TO_UPDATE:

        NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_ProcessLuAfterCsConnectNotExist_T3212Expire: attempt to update state need lau");
...
VOS_UINT8 NAS_MM_ProcessLuAfterCsConnectNotExist(VOS_VOID)
{
    ...
    if ( g_MmGlobalInfo.ucNtMod != g_MmGlobalInfo.ucNewNtMod )
    {
        ...
        if  ( MM_NET_MODE_II == g_MmGlobalInfo.ucNtMod )
        {
            if (MM_TRUE == ucFlg)
            {
                NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_ProcessLuAfterCsConnectNotExist: NMOII trigger lau");
    ...
    NAS_NORMAL_LOG1(WUEPS_PID_MM, "NAS_MM_ProcessLuAfterCsConnectNotExist: returnVal is", ucRet);
...
VOS_VOID  NAS_MM_LogMmCtxInfo(VOS_VOID)
{
    ...
    pstMsg->stMmGlobalInfo.ConnCtrlInfo_LCS_RcvXXEstReq_ucFlg = g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].RcvXXEstReq.ucFlg;
...
VOS_VOID NAS_MM_RcvDelayRadioCapaTriggerLauExpired_MmIdleNormalService(VOS_VOID)
{

    /* 当前在L模式下，丢弃不处理 */
    if (NAS_MML_NET_RAT_TYPE_LTE == NAS_MML_GetCurrNetRatType())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC,"NAS_MM_RcvDelayRadioCapaTriggerLauExpired_MmIdleNormalService, current is lte mode,discard msgs");
        return;
    }
    /* 检测当前MMC正在做SYSCFG，则启动延时定时器 */
    if (VOS_TRUE == NAS_MML_GetSysCfgFlg())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MM_RcvDelayRadioCapaTriggerLauExpired_MmIdleNormalService mmc SyscfgFlg is ture");

        Mm_TimerStart(MM_TIMER_DELAY_RADIO_CAPA_TRIGED_LAU);
        return;
    }

    /* 检测当前的RAT能力是否发生改变，若发生改变，则触发LAU流程 */
    if (VOS_TRUE == NAS_MM_IsRadioAccessCapNeedTriggerLau())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC,"NAS_MM_RcvDelayRadioCapaTriggerLauExpired_MmIdleNormalService, radio access cap trigger lau");
        g_MmGlobalInfo.LuInfo.ucLuType = MM_IE_LUT_NORMAL_LU;
        Mm_ComLuOnly();
    }
...
VOS_VOID NAS_MM_RcvDelayRadioCapaTriggerLauExpired_WaitForLauRrConnnection(VOS_VOID)
{
    /* 检测当前的RAT能力是否发生改变，若发生改变，则启动延时定时器 */
    if (VOS_TRUE == NAS_MM_IsRadioAccessCapNeedTriggerLau())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC,"NAS_MM_RcvDelayRadioCapaTriggerLauExpired_WaitForLauRrConnnection, restart delay radio cap triger lau timer");
        Mm_TimerStart(MM_TIMER_DELAY_RADIO_CAPA_TRIGED_LAU);
    }
...
VOS_VOID NAS_MM_RcvDelayRadioCapaTriggerLauExpired(VOS_VOID)
{
    VOS_UINT32                          ulMmState;
    ...
    {
        case MM_IDLE_NORMAL_SERVICE:
            NAS_MM_RcvDelayRadioCapaTriggerLauExpired_MmIdleNormalService();
            break;

        case WAIT_FOR_RR_CONNECTION_MM_CONNECTION:
        case WAIT_FOR_RR_CONNECTION_LOCATION_UPDATING:
        case WAIT_FOR_RR_CONNECTION_IMSI_DETACH:
            NAS_MM_RcvDelayRadioCapaTriggerLauExpired_WaitForLauRrConnnection();
            break;

        default:
            NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MM_RcvDelayRadioCapaTriggerLauExpired discard expired msg");
            break;
    }
...
VOS_VOID NAS_MM_RcvMmTimerDelayNetworkSearchAfterEmcExpired(VOS_VOID)
{
    VOS_UINT8                           ucCsEmergencyServiceFlg;
    VOS_UINT8                           ucImsEmcCallExistFlg;

    ucCsEmergencyServiceFlg = NAS_MML_GetCsEmergencyServiceFlg();
    ...
    /* 紧急呼呼叫，则重启 */
    if ((VOS_TRUE == ucCsEmergencyServiceFlg)
     || (VOS_TRUE == ucImsEmcCallExistFlg))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_RcvMmTimerDelayNetworkSearchAfterEmcExpired:EMC TRIGER RESTART TIMER");
        Mm_TimerStart(MM_TIMER_DELAY_NETWORK_SEARCH_AFTER_EMC);
        NAS_MML_SetTimerStatusOfDelayNetworkSearchAfterEmc(NAS_MML_TIMER_STATUS_RUNNING);
    }
...
{
    VOS_UINT8                           ucCsfbTimerStatus;

    ucCsfbTimerStatus = NAS_MML_GetCsfbMtPagingTimerStatus();

    /* 存在CSMT标志，则需要携带CSMT标志 */
    if ( (NAS_MML_CSFB_SERVICE_STATUS_MT_EXIST == NAS_MML_GetCsfbServiceStatus())
      || (NAS_MML_TIMER_STATUS_RUNNING == ucCsfbTimerStatus) )
    {
        return VOS_TRUE;
    }
...
VOS_VOID NAS_MM_ResetCsPagingFlgAndT3240Len(VOS_VOID)
{
    NAS_MM_SetCsPaingFlg(VOS_FALSE);

    if (VOS_TRUE == NAS_MML_GetExtendT3240LenFlg())
    {
        gstMmTimer[MM_TIMER_T3240].ulTimerLen   = MM_TIMER_T3240_VALUE;
        NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_ResetCsPagingFlgAndT3240Len: Reset T3240 len");
    }
...
VOS_UINT8 NAS_MM_GetCsPaingFlg(VOS_VOID)
{
    return g_MmGlobalInfo.ucCsPagingFlg;
}


VOS_VOID NAS_MM_SetCsPaingFlg(
    VOS_UINT8                           ucCsPagingFlg
)
{
    g_MmGlobalInfo.ucCsPagingFlg = ucCsPagingFlg;
}
        ...
        if (WAIT_FOR_NETWORK_COMMAND != g_MmGlobalInfo.ucState)
        {
                  ...
                  && (MM_CONN_CTRL_SS         == ucMmConnCtrl))
            {
                ...
                return VOS_FALSE;
                  ...
                  && (MM_CONN_CTRL_LCS         == ucMmConnCtrl))
            {
                /* 当前一个MO 呼叫业务请求正在建立，如果收到LCS业务请求，拒绝当前LCS业务请求 */
                return VOS_FALSE;
            }
            else if ((MM_IE_PD_LOCATION_SRVC   == g_MmGlobalInfo.ucMMConnEstingPD)
                  && (MM_CONN_CTRL_CC         == ucMmConnCtrl))
            {
                /* 当前一个MO LCS业务请求正在建立，如果收到呼叫业务请求，拒绝当前呼叫业务请求 */
                return VOS_FALSE;
            }
            else
            {
...
)
{
    ...
    NAS_MML_SetCsfbServiceStatus(enTargetStatus);
...
VOS_UINT32 NAS_MM_IsNeedLauFirst_CsfbMoEmcExist(VOS_VOID)
{
    ...
    */
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_IsNeedLauFirst_CsfbMoEmcExist: return TRUE");
        return VOS_TRUE;
    }

    NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_IsNeedLauFirst_CsfbMoEmcExist: return FALSE");
    return VOS_FALSE;
}

VOS_UINT32 NAS_MM_IsDamNotNeedLau(VOS_VOID)
{
    NAS_MML_LAI_STRU                   *pstCurrCampLai = VOS_NULL_PTR;
    MM_LAI_STRU                         stCurLai;
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_IsDamNotNeedLau: NOT SUPPORT DAM return VOS_FALSE");
        return VOS_FALSE;
    }

    if (NAS_MML_LOCATION_UPDATE_STATUS_NOT_UPDATED != NAS_MML_GetCsUpdateStatus())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_IsDamNotNeedLau: MM_TIMER_T3212 NOT RUNNING return VOS_FALSE");
        return VOS_FALSE;
    }

    if (MM_TIMER_RUNNING != NAS_MM_QryTimerStatus(MM_TIMER_T3212))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_IsDamNotNeedLau: MM_TIMER_T3212 NOT RUNNING return VOS_FALSE");
        return VOS_FALSE;
    }

    if (VOS_TRUE != NAS_MM_IsSameLaiInfo(NAS_MM_GetAttemptUpdateLaiInfo(), &stCurLai))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_IsDamNotNeedLau: NAS_MM_IsSameLaiInfo return VOS_FALSE");
        return VOS_FALSE;
    }

    /* 当前DAM特性使能,且当前在U2状态，且当前T3212定时器在运行，且当前驻留的lai是上次记录的ATTEMPT UPDATE LAI,则不需要LAU */
    NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_IsDamNotNeedLau: return VOS_TRUE");
...
)
{
    ...
    NAS_NORMAL_LOG1(WUEPS_PID_MM, "NAS_MM_ProcT3213AttemptCounter,ucT3213AttmptCnt is",g_MmGlobalInfo.LuInfo.ucT3213AttmptCnt);
...
VOS_UINT32 NAS_MM_IsMmConnExist(
    NAS_MM_CONN_CTRL_ENUM_UINT8         enMmConnCtrl,
    VOS_UINT32                          ulTransactionId
)
{
    if (ulTransactionId < MM_CONST_NUM_8)
    {
        if (0 != (g_MmGlobalInfo.ConnCtrlInfo[enMmConnCtrl].aucMMConnExtFlg[0]
                        & (VOS_UINT8)(((VOS_UINT32)0x01) << ulTransactionId)))
        {
            return VOS_TRUE;
        }
    }
    else if (ulTransactionId < MM_CONST_NUM_16)
    {
        if (0 != (g_MmGlobalInfo.ConnCtrlInfo[enMmConnCtrl].aucMMConnExtFlg[1]
                        & (VOS_UINT8)(((VOS_UINT32)0x01) << (ulTransactionId - MM_CONST_NUM_8))))
        {
            return VOS_TRUE;
        }
    }
...
VOS_UINT32 NAS_MM_IsCsServiceBuffered(
    NAS_MM_CONN_CTRL_ENUM_UINT8         enMmConnCtrl,
    VOS_UINT32                          ulTransactionId
)
{
    if ( (MM_TRUE == g_MmGlobalInfo.ConnCtrlInfo[enMmConnCtrl].RcvXXEstReq.ucFlg)
      && (ulTransactionId == g_MmGlobalInfo.ConnCtrlInfo[enMmConnCtrl].RcvXXEstReq.ulTransactionId) )
    {
        return VOS_TRUE;
    }
...
VOS_VOID NAS_MM_ClearConnCtrlInfoWithBuffer(
    NAS_MM_CONN_CTRL_ENUM_UINT8         enMmConnCtrl
)
{
    if (g_MmGlobalInfo.ConnCtrlInfo[enMmConnCtrl].RcvXXEstReq.ulTransactionId
            == g_MmGlobalInfo.ConnCtrlInfo[enMmConnCtrl].ucMMConnEstingTI)
    {
        g_MmGlobalInfo.ConnCtrlInfo[enMmConnCtrl].ucMMConnEstingTI = NO_MM_CONN_ESTING;
    }

    g_MmGlobalInfo.ConnCtrlInfo[enMmConnCtrl].ucRat                         = NAS_MML_NET_RAT_TYPE_BUTT;
    g_MmGlobalInfo.ConnCtrlInfo[enMmConnCtrl].RcvXXEstReq.ulTransactionId   = NAS_MM_CM_TI_INVALID;
    g_MmGlobalInfo.ConnCtrlInfo[enMmConnCtrl].RcvXXEstReq.ulCallType        = 0;
    g_MmGlobalInfo.ConnCtrlInfo[enMmConnCtrl].RcvXXEstReq.ulCallPri         = 0;
    g_MmGlobalInfo.ConnCtrlInfo[enMmConnCtrl].RcvXXEstReq.ulCallMode        = 0;
    g_MmGlobalInfo.ConnCtrlInfo[enMmConnCtrl].RcvXXEstReq.ucFlg             = MM_FALSE;                                                     /* 设置不存在缓存的MM连接 */
...
VOS_VOID NAS_MM_UpdateMmConnByTi(
    NAS_MM_CONN_CTRL_ENUM_UINT8         enMmConnCtrl,
    VOS_UINT32                          ulTransactionId,
    NAS_MM_CONN_STATE_ENUM_UINT8        enMmConnState
)
{
    if (NAS_MM_CONN_EXIST == enMmConnState)
    {
        if (ulTransactionId < MM_CONST_NUM_8)
        {
            NAS_MM_GetConnCtrlInfo(enMmConnCtrl)->aucMMConnExtFlg[0] |=
                (VOS_UINT8)(((VOS_UINT32)0x01) << ulTransactionId);
        }
        else if (ulTransactionId < MM_CONST_NUM_16)
        {
            NAS_MM_GetConnCtrlInfo(enMmConnCtrl)->aucMMConnExtFlg[1] |=
                (VOS_UINT8)(((VOS_UINT32)0x01) << (ulTransactionId - MM_CONST_NUM_8));
        }
        else
        {
...
VOS_VOID NAS_MM_BuildLcsEstReqFromBuffer(
    LCS_MM_EST_REQ_STRU                *pstLcsEstReq
)
{
    PS_MEM_SET(pstLcsEstReq, 0, sizeof(LCS_MM_EST_REQ_STRU));

    pstLcsEstReq->ulSenderCpuId      = VOS_LOCAL_CPUID;
    pstLcsEstReq->ulSenderPid        = WUEPS_PID_LCS;
    pstLcsEstReq->ulReceiverCpuId    = VOS_LOCAL_CPUID;
    pstLcsEstReq->ulReceiverPid      = WUEPS_PID_MM;
    pstLcsEstReq->ulLength           = sizeof(LCS_MM_EST_REQ_STRU) - VOS_MSG_HEAD_LENGTH;
    pstLcsEstReq->ulMsgId            = ID_LCS_MM_EST_REQ;
    pstLcsEstReq->ulTi               = NAS_MM_GetLcsEstReqTi();

    return;
}


VOS_VOID NAS_MM_ClearAllMmConn(VOS_VOID)
{
    g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_CC].aucMMConnExtFlg[0] = MM_CONST_NUM_0;
    g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_CC].aucMMConnExtFlg[1] = MM_CONST_NUM_0;

    g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_SS].aucMMConnExtFlg[0] = MM_CONST_NUM_0;
    g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_SS].aucMMConnExtFlg[1] = MM_CONST_NUM_0;

    g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].aucMMConnExtFlg[0] = MM_CONST_NUM_0;
    g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].aucMMConnExtFlg[1] = MM_CONST_NUM_0;

    g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_SMS].aucMMConnExtFlg[0] = MM_CONST_NUM_0;
    g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_SMS].aucMMConnExtFlg[1] = MM_CONST_NUM_0;

    return;
}


RRMM_TRANSACTION_STATUS_ENUM_UINT8 NAS_MM_ConvertLcsStatusToTransactionStatus(
    NAS_LCS_STATUS_ENUM_UINT8           enLcsStatus
)
{
    if (NAS_LCS_STATUS_SETUP_SUCC == enLcsStatus)
    {
        return RRMM_TRANSACTION_CONNECTION_EST_SUCC;
    }

    return RRMM_TRANSACTION_FAIL;
}


VOS_VOID NAS_MM_UpdateLcsStatus(
    NAS_LCS_STATUS_ENUM_UINT8           enLcsStatus
)
{
    NAS_MML_MO_LCS_STATUS_ENUM_UINT8    enOldMoLcsStatus;

    enOldMoLcsStatus = NAS_MML_GetMoLcsStatus();

    /* 更新MO标志 */
    switch (enOldMoLcsStatus)
    {
        case NAS_MML_CSFB_MO_LCS_SETUP_START:
            if (NAS_LCS_STATUS_SETUP_SUCC == enLcsStatus)
            {
                NAS_MML_SetMoLcsStatus(NAS_MML_CSFB_MO_LCS_SETUP_SUCC);
            }
            else
            {
                NAS_MML_SetMoLcsStatus(NAS_MML_CSFB_MO_LCS_SETUP_FAIL);
            }
            break;
        ...
        case NAS_MML_MO_LCS_SETUP_START:
            if (NAS_LCS_STATUS_SETUP_SUCC == enLcsStatus)
            {
                NAS_MML_SetMoLcsStatus(NAS_MML_MO_LCS_SETUP_SUCC);
            }
            else
            {
                NAS_MML_SetMoLcsStatus(NAS_MML_MO_LCS_SETUP_FAIL);
            }
            break;
...
VOS_UINT32 NAS_MM_IsCsRegFailOtherCause(
    VOS_UINT32                          ulCause
)
{
    /* 假流程的cause不是PS other cause */
    if (ulCause >= NAS_MML_REG_FAIL_CAUSE_OTHER_CAUSE)
    {
        return VOS_FALSE;
    }
...
VOS_VOID NAS_MM_UpdateDamNoSearchStatus(
    VOS_UINT32                          ulLuResult,
    VOS_UINT32                          ulCause
)
{
    VOS_UINT32                          ulIsCsRegFailOtherCause;

    /* 假流程，直接return */
    if (ulCause >= NAS_MML_REG_FAIL_CAUSE_OTHER_CAUSE)
    {
        NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_CheckCsInDamNoSearchStatus: unreal procedure");
    ...
    if (VOS_FALSE == NAS_MML_IsPlmnSupportDam(NAS_MML_GetCurrCampPlmnId()))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_CheckCsInDamNoSearchStatus: not support dam");

        return;
    }

    ulIsCsRegFailOtherCause = NAS_MM_IsCsRegFailOtherCause(ulCause);

    /* LAU失败到达4次，原因值不是#17，则进入DAM不搜网状态 */
    if ( (MM_MMC_LU_RESULT_FAILURE == ulLuResult)
      && (g_MmGlobalInfo.LuInfo.ucLuAttmptCnt >= MM_CONST_NUM_4)
      && (VOS_TRUE == ulIsCsRegFailOtherCause)
      && (NAS_MML_REG_FAIL_CAUSE_NETWORK_FAILURE != ulCause) )
    {
        NAS_MML_SetCsDamNoSearchFlag(VOS_TRUE);
        NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_CheckCsInDamNoSearchStatus: Cs In Dam No Search Status");
    ...
    NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_CheckCsInDamNoSearchStatus: Cs Not In Dam No Search Status");
