...
VOS_UINT32 NAS_MMC_RcvMsccCsgSpecSearchReq_PlmnSelection_Init(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    ...
    NAS_MMC_PLMN_SELECTION_LIST_INFO_STRU                  *pstPlmnSelectionListInfo   = VOS_NULL_PTR;
    MSCC_MMC_CSG_SPEC_SEARCH_REQ_STRU                      *pstUserCsgSpecSearchReqMsg = VOS_NULL_PTR;
    ...
    pstUserCsgSpecSearchReqMsg   = (MSCC_MMC_CSG_SPEC_SEARCH_REQ_STRU*)pstMsg;
    ...
    /* 保存CSG手动搜网信息 */
    NAS_MMC_UpdateUserCsgSpecSearchInfo_PlmnSelection((NAS_MML_PLMN_WITH_RAT_STRU*)(&pstUserCsgSpecSearchReqMsg->stPlmnWithRat),
        pstUserCsgSpecSearchReqMsg->ulCsgId);

    /* 根据场景初始化搜网列表信息 */
    NAS_MMC_InitPlmnSelectionList(NAS_MMC_PLMN_SEARCH_SCENE_USER_CSG_SPEC_SEARCH,
        VOS_NULL_PTR,
        pstPlmnSelectionListInfo);
    ...
    {
        if ( VOS_TRUE == NAS_MMC_IsNeedEnableLte_PlmnSelection(NAS_MMC_PLMN_SEARCH_SCENE_USER_CSG_SPEC_SEARCH) )
        {
        ...
        {
            /* CSG指定搜上报失败，初始化用户指定CSG ID，恢复白名单均在如下退出状态机的函数中处理 */
    ...
    {
        /* 打印异常 */
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvMsccCsgSpecSearchReq_PlmnSelection_Init: Get Next Search Plmn fail at user spec search.");
...
VOS_UINT32  NAS_MMC_RcvMsccCsgSpecSearchAbortReq_PlmnSelection(
    VOS_UINT32                         ulEventType,
    struct MsgCB                      *pstMsg
)
{
...
#if (FEATURE_ON == FEATURE_LTE)
    /* 对特殊场景是否需要Disable LTE的处理放在此处 */
    if ( VOS_TRUE == NAS_MMC_IsNeedEnableLte_PlmnSelection(pstPlmnSearchReq->enPlmnSearchScene) )
    {
        if ((VOS_FALSE == NAS_MML_GetDisableLteRoamFlg())
         && (NAS_MML_LTE_CAPABILITY_STATUS_DISABLE_NOTIFIED_AS == enLteCapabilityStatus))
        {
...
)
{
    ...
    for (i = 0; i < pstInterPlmnSearchInfo->ulHighPlmnNum; i++)
    {
        ...
        {
            NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedSortPlmnListRcvRrcPlmnSearchCnfFail_PlmnSelection: High PLMN exist RPLMN");
        ...
        {
            NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedSortPlmnListRcvRrcPlmnSearchCnfFail_PlmnSelection: High PLMN exist HPLMN");

            NAS_MMC_SetExistRplmnOrHplmnFlag_PlmnSelection(VOS_TRUE);
            return VOS_FALSE;
        }

        if (VOS_FALSE == NAS_MML_ComparePlmnMcc(pstCurrSearchingPlmn->astPlmnId[0].stPlmnId.ulMcc, pstInterPlmnSearchInfo->astHighPlmnList[i].ulMcc))
        {
    ...
    for (i = 0; i < pstInterPlmnSearchInfo->ulLowPlmnNum; i++)
    {
        ...
        {
            NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedSortPlmnListRcvRrcPlmnSearchCnfFail_PlmnSelection: Low PLMN exist RPLMN");
        ...
        {
            NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedSortPlmnListRcvRrcPlmnSearchCnfFail_PlmnSelection: Low PLMN exist HPLMN");

            NAS_MMC_SetExistRplmnOrHplmnFlag_PlmnSelection(VOS_TRUE);
            return VOS_FALSE;
        }

        if (VOS_FALSE == NAS_MML_ComparePlmnMcc(pstCurrSearchingPlmn->astPlmnId[0].stPlmnId.ulMcc, pstInterPlmnSearchInfo->astLowPlmnList[i].stPlmnId.ulMcc))
        {
            ucSearchedPlmnAllSameWithCurrSearchingPlmn = VOS_FALSE;
        }
    }
    ...
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_IsNeedSortPlmnListRcvRrcPlmnSearchCnfFail_PlmnSelection: ucSearchedPlmnAllSameWithCurrSearchingPlmn is ", ucSearchedPlmnAllSameWithCurrSearchingPlmn);
...
)
{
    ...
    if (VOS_TRUE == ulIsNeedSortRoamPlmnList)
    {
        ...
        NAS_MMC_BuildRoamPlmnSelectionListBySearchedExistPlmnInfo(NAS_MML_NET_RAT_TYPE_WCDMA,
                     &stSearchedExistPlmnInfo, &stNewRoamPlmnSelectionList, pstPlmnSrchList);
...
)
{
    ...
    if (VOS_TRUE == ulIsNeedSortRoamPlmnList)
    {
        ...
        /* 设置UTRAN未完成HISTORY搜索流程 */
        NAS_MMC_UpdateHistorySearchedFlagWhenRoamSort_PlmnSelection(NAS_MML_NET_RAT_TYPE_WCDMA);
...
)
{
    ...
    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvSearchedPlmnInfoInd_PlmnSelection_WaitWasPlmnSearchCnf: Send MSCC PLMN SEARCH INFO IND");
     ...
     && (VOS_TRUE == ulIsNeedStopPlmnSearch))
    {
        ...
        */
        NAS_MMC_BuildRoamPlmnSelectionListBySearchedExistPlmnInfo(NAS_MML_NET_RAT_TYPE_WCDMA,
            &stSearchedExistPlmnInfo, &stNewRoamPlmnSelectionList, pstPlmnSrchList);
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvSearchedPlmnInfoInd_PlmnSelection_WaitWasPlmnSearchCnf: Sort PLMN and Abort UTRANCTRL PLMN SEARCH");
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvSearchedPlmnInfoInd_PlmnSelection_WaitWasPlmnSearchCnf: Send MMCMMC_INTER_SKIP_SEARCH_W_IND");
...
)
{
    ...
    if (VOS_TRUE == NAS_MMC_IsNeedStopPlmnSearchRcvRrcSearchedPlmnInfoInd_PlmnSelection(NAS_MML_NET_RAT_TYPE_GSM, pstSearchedPlmnInfoMsg))
    {
        ...
        */
        ...
        if (NAS_MMC_PLMN_SEARCH_TYPE_HISTORY == NAS_MMC_GetCurrSearchingType_PlmnSelection())
        {
            NAS_MMC_UpdateHistorySearchedFlagWhenRoamSort_PlmnSelection(NAS_MML_NET_RAT_TYPE_GSM);
        }
...
{
    NAS_MML_PLMN_LIST_WITH_RAT_STRU                        *pstCurrSearchingPlmn;
    ...
    VOS_UINT32                                              ulIsCtccSrchOutSideMcc;
#endif

#if (FEATURE_ON == FEATURE_UE_MODE_CDMA)
    NAS_MEM_SET_S(&stPlmnId, sizeof(stPlmnId), 0x00, sizeof(stPlmnId));

    ulIsCtccSrchOutSideMcc = VOS_FALSE;
    ...
    stCurrSearchRoamPlmnInfo.stPlmnWithRat.stPlmnId.ulMcc = pstCurrSearchingPlmn->astPlmnId[0].stPlmnId.ulMcc;
    stCurrSearchRoamPlmnInfo.stPlmnWithRat.stPlmnId.ulMnc = pstCurrSearchingPlmn->astPlmnId[0].stPlmnId.ulMnc;
    stCurrSearchRoamPlmnInfo.stPlmnWithRat.enRat          = pstCurrSearchingPlmn->astPlmnId[0].enRat;
    ...
    for (ulIndex = 0; ulIndex < pstSearchedPlmnInfoInd->ulTaiNum; ulIndex++)
    {
        ...
        {
            if ( (VOS_FALSE == NAS_MMC_IsChinaMainlandOrMacaoMcc(stGUNasPlmnId.ulMcc))
              && (NAS_MML_INVALID_MCC != stGUNasPlmnId.ulMcc) )
            {
                NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedStopPlmnSearchRcvLteSearchedPlmnInfoInd_PlmnSelection: CL MODE, CTCC SIM, FOREIGN MCC!");
                ulIsCtccSrchOutSideMcc = VOS_TRUE;
            }
        }
#endif
        ...
        for (i = 0; i < pstCurrSearchingPlmn->ulPlmnNum; i++)
        {
            if (VOS_TRUE == NAS_MML_CompareBcchPlmnwithSimPlmn(&pstCurrSearchingPlmn->astPlmnId[i].stPlmnId, &stGUNasPlmnId))
            {
                NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedStopPlmnSearchRcvLteSearchedPlmnInfoInd_PlmnSelection: searched plmn is in searching plmn list");
                return VOS_FALSE;
            }
        }
        ...
        if (VOS_FALSE == NAS_MML_IsPlmnMccBelongToTheSameCountry(pstCurrSearchingPlmn->astPlmnId[0].stPlmnId.ulMcc, stGUNasPlmnId.ulMcc))
        {
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedStopPlmnSearchRcvLteSearchedPlmnInfoInd_PlmnSelection: All Searched Plmn Disable Lte Roam");
...
#if (FEATURE_ON == FEATURE_UE_MODE_CDMA)
        NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_IsNeedStopPlmnSearchRcvLteSearchedPlmnInfoInd_PlmnSelection: ulIsCtccSrchOutSideMcc is ", ulIsCtccSrchOutSideMcc);

        if (VOS_TRUE == ulIsCtccSrchOutSideMcc)
        {
            return VOS_FALSE;
        }
#endif
    ...
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_IsNeedStopPlmnSearchRcvLteSearchedPlmnInfoInd_PlmnSelection: ulIsExistDiffMccWithCurrSearchingMcc is ", ulIsExistDiffMccWithCurrSearchingMcc);

    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_IsNeedStopPlmnSearchRcvLteSearchedPlmnInfoInd_PlmnSelection: ucAllSearchedPlmnForbiddenPlmnFlag is ", ucAllSearchedPlmnForbiddenPlmnFlag);

    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_IsNeedStopPlmnSearchRcvLteSearchedPlmnInfoInd_PlmnSelection: ucIsExistSearchedPlmnPrioCurrSearchingPlmn is ", ucIsExistSearchedPlmnPrioCurrSearchingPlmn);
    ...
    if (VOS_TRUE == NAS_MMC_IsNeedStopPlmnSearchRcvLteSearchedPlmnInfoInd_PlmnSelection(pstSearchedPlmnInfoInd))
    {
        ...
        */
        NAS_MMC_BuildRoamPlmnSelectionListBySearchedExistPlmnInfo(NAS_MML_NET_RAT_TYPE_LTE,
            &stExistRoamPlmnSelectionList, &stNewRoamPlmnSelectionList, pstPlmnSrchList);
        /* 把排过序的roam搜网列表插入搜网列表前面 */
        NAS_MMC_AddRoamPlmnSelectionListInPlmnSelectionList(&stNewRoamPlmnSelectionList, pstPlmnSrchList);
        ...
        if (NAS_MMC_PLMN_SEARCH_TYPE_HISTORY == NAS_MMC_GetCurrSearchingType_PlmnSelection())
        {
            NAS_MMC_UpdateHistorySearchedFlagWhenRoamSort_PlmnSelection(NAS_MML_NET_RAT_TYPE_LTE);
        }
...
VOS_VOID NAS_MMC_RcvLmmCsgSpecSearchCnf_PlmnSelection_WaitLmmPlmnSearchCnf(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    ...
    PS_MEM_SET(&stSearchedExistPlmnInfo, 0, sizeof(stSearchedExistPlmnInfo));
    PS_MEM_SET(&stNewRoamPlmnSelectionList, 0, sizeof(stNewRoamPlmnSelectionList));
    PS_MEM_SET(&stPlmnSearchInfo, 0, sizeof(stPlmnSearchInfo));
    ...
    /* 设置当前模式进行了全频搜网，CSG指定搜失败，接入层只会携带存在的CSG和非CSG网络，会做全频搜 */
    NAS_MMC_UpdateLmmCoverageFlg_PlmnSelection(pstLmmSrchCnfMsg);

    if (MMC_LMM_PLMN_SRCH_RLT_SPEC_CSG_SUCC == pstLmmSrchCnfMsg->enRlst)
    {
        NAS_EventReport(WUEPS_PID_MMC, NAS_OM_EVENT_PLMN_SELECTION_SUCCESS,
            VOS_NULL_PTR, NAS_OM_EVENT_NO_PARA);
    ...
    /* 如果当前为NO RF，则直接退状态机 */
    if (MMC_LMM_PLMN_SRCH_RLT_SPEC_CSG_NO_RF == pstLmmSrchCnfMsg->enRlst)
    {
        NAS_MMC_ProcNoRf_PlmnSelection(NAS_MML_NET_RAT_TYPE_LTE);
    ...
    /* CSG指定搜网失败，上报结果 */
    /* 不能清除duplicate rplmn和eplmn信息，否则如下场景有问题，驻留非csg 310410网络，
    csg指定搜310412 csg id 1成功注册成功，再次csg指定搜310412 csg id 2失败，此时无需清除
    duplicate rplmn信息，因为后续可能会搜310412成功驻留原csg小区，在该csg小区上丢网还需要使用
    duplicate rplmn信息回310410 */
    NAS_MMC_ProcUserCsgSearchResultFail_PlmnSelection();
...
#if (FEATURE_ON == FEATURE_CSG)
        case MMC_LMM_PLMN_SRCH_RLT_SPEC_CSG_SUCC:
        case MMC_LMM_PLMN_SRCH_RLT_SPEC_CSG_FAIL:
        case MMC_LMM_PLMN_SRCH_RLT_SPEC_CSG_NO_RF:

            /* L下CSG指定搜网的处理 */
            NAS_MMC_RcvLmmCsgSpecSearchCnf_PlmnSelection_WaitLmmPlmnSearchCnf(ulEventType, pstMsg);
            break;
#endif
        default:
...
)
{
    ...
    /* 如果是用户CSG指定搜场景的搜网定时器超时，直接按失败处理，恢复白名单和状态机入口消息 */
#if (FEATURE_ON == FEATURE_CSG)
    if (VOS_TRUE == NAS_MMC_IsUserCsgSpecSearchType_PlmnSelection(NAS_MMC_GetCurrFsmId(),NAS_MMC_GetCurrFsmEventType()))
    {
        NAS_MMC_ProcUserCsgSearchResultFail_PlmnSelection();
    }
#endif
...
VOS_VOID NAS_MMC_RcvLmmCsgSpecSearchCnf_PlmnSelection_WaitLmmPlmnStopCnf(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
...
)
{
    ...
    switch (pstLmmSrchCnfMsg->enRlst)
    {
        ...
        case MMC_LMM_PLMN_SRCH_RLT_SPEC_CSG_NO_RF:
            ...
            /* L下CSG指定搜网的处理 */
            NAS_MMC_RcvLmmCsgSpecSearchCnf_PlmnSelection_WaitLmmPlmnStopCnf(ulEventType, pstMsg);
            break;
#endif
        default:
...
)
{
    ...
    pstPlmnWithRat      = NAS_MMC_GetCurrSearchingPlmnList_PlmnSelection();
...
)
{
    ...
    则根据注册结果判断csg指定搜网是否成功 */
#if (FEATURE_ON == FEATURE_CSG)
    if (VOS_TRUE == NAS_MMC_IsUserCsgSpecSearchType_PlmnSelection(enCurrentFsmId,ulCurrentEventType))
    {
        if (VOS_TRUE ==  NAS_MMC_ProcUserCsgSpecSearchRegRslt_PlmnSelection(NAS_MMC_GetCsRegCause_PlmnSelection(), NAS_MMC_GetPsRegCause_PlmnSelection()))
        {
            /*更新入口消息为普通的搜网请求，后续有新的用户请求时，可以打断该状态机*/
            NAS_MMC_SaveCommonPlmnSrchEntryMsg_PlmnSelection();
        }
...
{
    NAS_MMC_PLMN_SELECTION_LIST_INFO_STRU                  *pstPlmnSelectionListInfo = VOS_NULL_PTR;
    NAS_MML_PLMN_LIST_WITH_RAT_STRU                        *pstCurrSearchingPlmnList = VOS_NULL_PTR;
    NAS_MML_PLMN_WITH_RAT_STRU                              stDestPlmn;
    VOS_UINT32                                              ulResult;
    ...
    pstCurrSearchingPlmnList    = NAS_MMC_GetCurrSearchingPlmnList_PlmnSelection();

    /* 更新当前网络的搜索状态为已注册过，在此轮搜网中，不会再搜此网络 */
    NAS_MMC_UpdatePlmnNetStatusInPlmnSelectionList(&(pstCurrSearchingPlmnList->astPlmnId[0]),
                                            NAS_MMC_NET_STATUS_SEARCHED_REGISTERED,
                                            pstPlmnSelectionListInfo);
...
VOS_UINT32 NAS_MMC_RcvMsccCsgSpecSearchReq_PlmnSelection_WaitEpsRegRsltInd(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    MSCC_MMC_CSG_SPEC_SEARCH_REQ_STRU                      *pstCsgSpecSearchMsg  = VOS_NULL_PTR;
    NAS_MML_REG_FAIL_CAUSE_ENUM_UINT16                      enCsRegCause;
    NAS_MML_REG_FAIL_CAUSE_ENUM_UINT16                      enPsRegCause;

    pstCsgSpecSearchMsg = (MSCC_MMC_CSG_SPEC_SEARCH_REQ_STRU*)pstMsg;

    /* 保存CSG手动搜网信息 */
    NAS_MMC_UpdateUserCsgSpecSearchInfo_PlmnSelection((NAS_MML_PLMN_WITH_RAT_STRU*)(&pstCsgSpecSearchMsg->stPlmnWithRat),
        pstCsgSpecSearchMsg->ulCsgId);

    /* 如果ims注册结果已经收到可以判定csg指定搜网成功或失败，如果ims注册结果未收到，继续等ims注册结果 */
    if (NAS_MMC_WAIT_REG_RESULT_IND_NULL != NAS_MMC_GetWaitRegRsltFlag_PlmnSelection())
    {
        /* 保存入口消息 */
        NAS_MMC_SaveCurEntryMsg(ulEventType, pstMsg);
...
)
{
...
#if (FEATURE_ON == FEATURE_CSG)
    MMC_LMM_CELL_TYPE_ENUM_UINT8        enCurrCampCellType;
    ...
    if (VOS_TRUE != pstImsVoiceNtf->ucRcvServiceChangeIndFlag)
    {
        return VOS_TRUE;
    }
    ...
    {
        /* csg小区上注册被拒导致的disable lte不需要disable lte */
            ...
            )
        {
            ...
            NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvMsccImsVoiceCapInd_PlmnSelection_WaitEpsRegRsltInd: MMC_LMM_DISABLE_LTE_REASON_LTE_VOICE_NOT_AVAILABLE");
...
VOS_UINT32 NAS_MMC_RcvMsccCsgSpecSearchReq_PlmnSelection_WaitEpsConnRelInd(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    ...
    /* 如果csg指定的PlmnId、RAT、csg id 和当前驻留的PlmnId、RAT、csg id相同，直接返回指定搜网结果 */
    enCsCause = NAS_MMC_GetCsRegCause_PlmnSelection();
    enPsCause = NAS_MMC_GetPsRegCause_PlmnSelection();

    if (VOS_TRUE == NAS_MMC_ProcUserCsgSpecSearchRegRslt_PlmnSelection(enCsCause, enPsCause))
    {
        return VOS_TRUE;
    }
...
{
    LMM_MMC_TAU_RESULT_IND_STRU                            *pstLmmTauIndMsg = VOS_NULL_PTR;
    VOS_UINT32                                              ulPeriodSearchGUFlag;
    NAS_MMC_ADDITIONAL_ACTION_ENUM_UINT8                    enAdditionalAction;
    ...
    ulPeriodSearchGUFlag = NAS_MMC_IsCsPsMode1NeedPeriodSearchGU();

    /* 附加操作为驻留当前网络 */
    if ( (VOS_FALSE == NAS_MMC_IsAdditionalActionTrigerPlmnSrch(enAdditionalAction))
      && (ulPeriodSearchGUFlag == VOS_FALSE))
    {
        /* 回复状态机退出成功 */
        NAS_MMC_SndRslt_PlmnSelection(NAS_MMC_PLMN_SELECTION_SUCC);
...
)
{
    ...
    if ((NAS_MML_NET_RAT_TYPE_WCDMA == enRatType)
     || (NAS_MML_NET_RAT_TYPE_GSM == enRatType))
    {
        /* 如果是CSG指定搜的场景，状态机退出结果处理中会自然回复失败，因此无需特殊处理 */
...
VOS_UINT32 NAS_MMC_RcvMsccCsgSpecSearchReq_PlmnSelection_WaitEpsRegRsltIndCsPsMode1ReCampLte(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    ...
    if (VOS_TRUE == NAS_MMC_ProcUserCsgSpecSearchRegRslt_PlmnSelection(enCsRegCause, enPsRegCause))
    {
        return VOS_TRUE;
    }
...
)
{
    ...
    if (VOS_FALSE  == pstImsVoiceNtf->ucAvail)
    {
        if (VOS_TRUE == NAS_MMC_IsNeedDisableLte_ImsVoiceNotAvail())
        {
            ...
            NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvMsccImsVoiceCapInd_PlmnSelection_WaitEpsRegRsltIndCsPsMode1ReCampLte: MMC_LMM_DISABLE_LTE_REASON_LTE_VOICE_NOT_AVAILABLE");
        }
    }
...
VOS_UINT32 NAS_MMC_RcvMsccCsgSpecSearchReq_PlmnSelection_WaitEpsRelIndCsPsMode1ReCampLte(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
...
#if (FEATURE_ON == FEATURE_CSG)
    /* 不是用户指定搜网时判断是否是csg指定搜网，csg指定搜网判断禁止逻辑同用户指定搜网 */
    if (VOS_FALSE == ulUserSpecPlmnSearch)
    {
        ulUserSpecPlmnSearch = NAS_MMC_IsUserCsgSpecSearchType_PlmnSelection(enCurrentFsmId,ulCurrentEventType);
    }
#endif
...
)
{
    ...
    if (VOS_TRUE == NAS_MMC_IsNeedSearchAvailPlmn_PlmnSelection())
    {
        ...
        for (i = 0; i < pstPrioRatList->ucRatNum; i++)
        {
            ...
            {
                NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_GetNextPrefBandSearchPlmn_PlmnSelection: RAT in Forb List ",
                    pstPrioRatList->aucRatPrio[i]);
                continue;
            }
            ...
            ulIsCmServiceExist = NAS_MML_GetCmServiceSetupFlg();
            /* 满足如下条件,则不需要搜索LTE的网络:
            ...
            */
            if (((VOS_FALSE                             == NAS_MML_GetSimPsRegStatus())
              ...
              || (VOS_TRUE == ulIsCmServiceExist))
             && (NAS_MML_NET_RAT_TYPE_LTE == pstPrioRatList->aucRatPrio[i]))
            {
                NAS_NORMAL_LOG4(WUEPS_PID_MMC, "NAS_MMC_GetNextPrefBandSearchPlmn_PlmnSelection:enLCapabilityStatus,enSimType,ulIsCmServiceExist,pstPrioRatList->aucRatPrio",
                    enLCapabilityStatus,
                    enSimType,
                    ulIsCmServiceExist,
                    pstPrioRatList->aucRatPrio[i]);
            ...
            {
                NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_GetNextPrefBandSearchPlmn_PlmnSelection: GSM RAT Forbidden");
                continue;
            }
            ...
            if (VOS_FALSE == NAS_MMC_GetAllBandSearch_PlmnSelection(pstPrioRatList->aucRatPrio[i]))
            {
                ...
                /* 发送搜网请求之前记录发送的PLMN列表 */
                NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_GetNextPrefBandSearchPlmn_PlmnSelection: not All Band Search Rat is ", pstPrioRatList->aucRatPrio[i]);
...
)
{
    ...
    if (VOS_TRUE == NAS_MMC_IsNeedSearchAvailPlmn_PlmnSelection())
    {
        ...
        for (i = 0; i < pstPrioRatList->ucRatNum; i++)
        {
            ...
            if (VOS_TRUE == NAS_MML_IsRatInForbiddenList(pstPrioRatList->aucRatPrio[i]))
            {
                NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_GetNextSpecSearchPlmn_PlmnSelection: RAT in Forb List ",
                    pstPrioRatList->aucRatPrio[i]);
                continue;
            }
            ...
            */
            if (((VOS_FALSE                             == NAS_MML_GetSimPsRegStatus())
             ...
             && (NAS_MML_NET_RAT_TYPE_LTE == pstPrioRatList->aucRatPrio[i]))
            {
                NAS_NORMAL_LOG4(WUEPS_PID_MMC, "NAS_MMC_GetNextSpecSearchPlmn_PlmnSelection:enLCapabilityStatus,enSimType,ulIsCmServiceExist,pstPrioRatList->aucRatPrio",
                    enLCapabilityStatus,
                    enSimType,
                    ulIsCmServiceExist,
                    pstPrioRatList->aucRatPrio[i]);
            ...
            if ( (VOS_TRUE == NAS_MML_GetGsmForbidFlg())
              && (NAS_MML_NET_RAT_TYPE_GSM == pstPrioRatList->aucRatPrio[i]) )
            {
                NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_GetNextSpecSearchPlmn_PlmnSelection: GSM RAT Forbidden");
                continue;
            }
...
)
{
    ...
    PS_MEM_SET(&stPlmnWithCsgIdList, 0, sizeof(stPlmnWithCsgIdList));
    ...
    {
#if (FEATURE_ON == FEATURE_CSG)
        if (VOS_TRUE == NAS_MMC_IsNeedSndCsgSpecSearchReq_PlmnSelection(pstDestPlmnList))
        {
            /* 通知L进行CSG指定搜网前先通知L CSG白名单为用户指定csg id，相当于对当前用户指定csg id进行锁定，
            后续csg指定搜网流程成功或失败再通知接入层CSG白名单全集 */
            stPlmnWithCsgIdList.ulPlmnWithCsgIdListNum = 1;
            NAS_MML_ConvertNasPlmnToLMMFormat(&stPlmnWithCsgIdList.astPlmnWithCsgIdInfo[0].stPlmnId,
                &pstPlmnWithCsgId->stPlmnWithRat.stPlmnId);
            stPlmnWithCsgIdList.astPlmnWithCsgIdInfo[0].ulCsgId = pstPlmnWithCsgId->ulCsgId;

            NAS_MMC_SndLmmCsgWhiteListNotify(&stPlmnWithCsgIdList);
            NAS_MMC_SndLmmCsgSpecSearchReq(pstDestPlmnList, NAS_MMC_GetCsgSpecSrchPlmnWithCsgId());
            NAS_MMC_SetSearchedCsgSpecSearchFlag_PlmnSelection(NAS_MML_NET_RAT_TYPE_LTE, VOS_TRUE);
            return;
        }
#endif
...
)
{
    ...
    ulRplmnExistFlg = NAS_MML_GetPrefRPlmn(enRatType, &stRPlmnId);
...
)
{
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedAddEhplmnWhenSearchRplmn_PlmnSelection: NAS_MMC_IsRPlmnFirstSearchWithHPlmn return FALSE, Do not add EHPLMN");
        return VOS_FALSE;
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedAddEhplmnWhenSearchRplmn_PlmnSelection: Not RPLMN, Do not add EHPLMN when searching");
        return VOS_FALSE;
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedAddEhplmnWhenSearchRplmn_PlmnSelection: ucSearchRplmnAndHplmnFlg is TRUE, Do not add EHPLMN when searching");
        return VOS_FALSE;
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedAddEhplmnWhenSearchRplmn_PlmnSelection: Power Up Scene, Add EHPLMN when searching");
        return VOS_TRUE;
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedAddEhplmnWhenSearchRplmn_PlmnSelection: AREA LOST Scene, Add EHPLMN when searching");
        return VOS_TRUE;
    }

    /* OOS场景都允许搜RPLMN带EHPLMN/HPLMN */
...
)
{
    ...
    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedAddEhplmnWhenSearchHplmn_PlmnSelection: Add EHPLMN when searching HPLMN");
    return VOS_TRUE;
}
...
/* 移到NasMmcPlmnSelectionStrategy.c文件中 */
...
)
{
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedAddEplmnWhenSearchRplmn_PlmnSelection: Dest Plmn is not in EPLMN LIST");
        return VOS_FALSE;
    }
      ...
      && (VOS_FALSE == ucSearchRplmnAndEplmnFlg))
    {
        ...
        {
            NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedAddEplmnWhenSearchRplmn_PlmnSelection: CSFB fail triggered PLMN SRCH, Add EPLMN");
            return VOS_TRUE;
        }

        if (VOS_TRUE == NAS_MMC_GetSpecSceneSrchRplmnCarryEplmnFlag(enPlmnSearchScene, ucIsRoam))
        {
            NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedAddEplmnWhenSearchRplmn_PlmnSelection: Should add EPLMN");
            return VOS_TRUE;
        }

    }

    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedAddEplmnWhenSearchRplmn_PlmnSelection: Do not add EPLMN");
...
)
{
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedAddDplmnOfSameCountryWhenSearchDPlmn_PlmnSelection: NV 2333 not active, return FALSE");
        return VOS_FALSE;
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedAddDplmnOfSameCountryWhenSearchDPlmn_PlmnSelection: Manual Selection, return FALSE");
        return VOS_FALSE;
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedAddDplmnOfSameCountryWhenSearchDPlmn_PlmnSelection: RAT not roam sorted, return FALSE");
        return VOS_FALSE;
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedAddDplmnOfSameCountryWhenSearchDPlmn_PlmnSelection: Not DPLMN, return FALSE");
        return VOS_FALSE;
    }
    ...
    if (VOS_TRUE == NAS_MMC_GetSrchUOplmnPriorToDplmnFlg())
    {
        ...
        {
            NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedAddDplmnOfSameCountryWhenSearchDPlmn_PlmnSelection: UPLMN, return FALSE");
            return VOS_FALSE;
        }
        ...
        {
            NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedAddDplmnOfSameCountryWhenSearchDPlmn_PlmnSelection: OPLMN, return FALSE");
            return VOS_FALSE;
        }
    }

    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedAddDplmnOfSameCountryWhenSearchDPlmn_PlmnSelection: return FALSE");
    return VOS_TRUE;
}
...
VOS_UINT32 NAS_MMC_IsNeedHistoryPlmnSearch_PlmnSelection(VOS_VOID)
{
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedHistoryPlmnSearch_PlmnSelection: Do not need History srch before Spec srch");
        return VOS_FALSE;
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedHistoryPlmnSearch_PlmnSelection: OOC scene and OOS History Search NV not avtive");
        return VOS_FALSE;
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedHistoryPlmnSearch_PlmnSelection: OOS scene and History srch not support in Curr phase");
        return VOS_FALSE;
    }
     ...
     || (NAS_MMC_PLMN_SEARCH_SCENE_CSFB_SERVIEC_REJ == enPlmnSearchScene))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC,
            "NAS_MMC_IsNeedHistoryPlmnSearch_PlmnSelection: Mark L mode NW as History Searched");
        NAS_MMC_SetHistorySearchedFlag_PlmnSelection(NAS_MML_NET_RAT_TYPE_LTE, VOS_TRUE);
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC,
            "NAS_MMC_IsNeedHistoryPlmnSearch_PlmnSelection: Searched all NW, Don't need History Search");
        return VOS_FALSE;
    }
...
VOS_UINT32 NAS_MMC_IsNeedPrefBandPlmnSearch_PlmnSelection(VOS_VOID)
{
    ...
    /* area lost时需要根据NV判断是要pref band搜还是spec搜，
       如果配置为0:当前模不搜，pref band特性打开时其他模做pref band，否则其他模做spec搜 */
    if ((NAS_MMC_PLMN_SEARCH_SCENE_AREA_LOST == enPlmnSearchScene)
...
VOS_UINT32 NAS_MMC_IsNeedSpecPlmnSearch_PlmnSelection(VOS_VOID)
{
    ...
    /* 避免NAS_MMC_PLMN_SEARCH_SCENE_PREFBAND_LIST_PLMN_SEARCH场景没有获取到下一个网络 */
    if (NAS_MMC_PLMN_SEARCH_SCENE_PREFBAND_LIST_PLMN_SEARCH == enPlmnSearchScene)
    {
        return VOS_FALSE;
    }
...
)
{
    ...
    if (VOS_FALSE == NAS_MMC_GetRoamPlmnSelectionSortActiveFlg())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedAddEhplmnWhenSeachDplmn_PlmnSelection: NV 2333 not active, Do not add EHPLMN");
        return VOS_FALSE;
    }
    ...
    if (VOS_FALSE == NAS_MMC_IsSpecRatListRoamSorted_PlmnSelection(pstPrioRatList) )
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedAddEhplmnWhenSeachDplmn_PlmnSelection: RAT not roam sorted, Do not add EHPLMN");
        return VOS_FALSE;
    }
    ...
    if (VOS_FALSE == NAS_MMC_IsPlmnIdWithRatInSimPlmnWithRegDomainList(pstDestPlmn,
                           pstDplmnNplmnCfgInfo->usDplmnListNum, pstDplmnNplmnCfgInfo->astDPlmnList))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedAddEhplmnWhenSeachDplmn_PlmnSelection: Not DPLMN, Do not add EHPLMN");
        return VOS_FALSE;
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedAddEhplmnWhenSeachDplmn_PlmnSelection: Auto Selection, Add EHPLMN");
        return VOS_TRUE;
    }

    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedAddEhplmnWhenSeachDplmn_PlmnSelection: Do not add EHPLMN");
...
)
{
    ...
    else if ( VOS_TRUE == NAS_MMC_IsNeedAddEhplmnWhenSearchHplmn_PlmnSelection(pstDestPlmn) )
    {
        ...
        NAS_MMC_AddEHPlmnInDestPlmnList(&stTempDestPlmnList, &stEHPlmnInfo);
    ...
    for (i = 0, j = 0; i < stTempDestPlmnList.ulPlmnNum; i++)
    {
        ...
        {
            NAS_WARNING_LOG3(WUEPS_PID_MMC, "NAS_MMC_BuildDestPlmnListWhenSearchPlmn_PlmnSelection:WARNING: Current Lte Plmn is not Allowed Srch ulMcc、 ulMnc and enRat are",
...
)
{
...
#if (FEATURE_ON == FEATURE_CSG)
    VOS_UINT32                                              ulUserCsgSpecSearch;
#endif
...
#if (FEATURE_ON == FEATURE_CSG)
    ulUserCsgSpecSearch = NAS_MMC_IsUserCsgSpecSearchType_PlmnSelection(enCurrentFsmId,ulCurrentEventType);
#endif
    ...
    NAS_MMC_SetCurrSearchingPlmnList_PlmnSelection(&stDestPlmnList);
...
)
{
    ...
    NAS_MMC_UpdateHistorySearchedFlagByInterPlmnSearchReq_PlmnSelection(pstPlmnSearchReq);

    NAS_MMC_UpdateNetStatusByHighPrioPlmnList_PlmnSelection(pstPlmnSearchReq, pstPlmnSelectionListInfo);

    if (VOS_TRUE == NAS_MMC_IsNeedDelCurrCampPlmn_PlmnSelection(pstPlmnSearchReq->enPlmnSearchScene, NAS_MML_GetCurrCampLai()))
    {
        /* 删除当前注册被拒的网络 */
...
)
{
    ...
    if (VOS_TRUE == NAS_MML_GetCmServiceSetupFlg())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_UpdatePlmnSearchList_PlmnSelection: Put L to the end of PLMN Search List");
...
VOS_VOID NAS_MMC_UpdateHistorySearchedFlgBasedOnAdditionalAction_PlmnSelection(
    NAS_MMC_ADDITIONAL_ACTION_ENUM_UINT8 enAdditionalAction
)
{
    NAS_MML_PLMN_RAT_PRIO_STRU                   *pstPrioRatList = VOS_NULL_PTR;
    ...
    if (NAS_MMC_ADDITIONAL_ACTION_SEARCH_SUITABLE_CELL_IN_SAME_PLMN != enAdditionalAction)
    {
        return;
    }
    ...
    if (NAS_MMC_PLMN_SEARCH_SCENE_AVAILABLE_TIMER_EXPIRED_HISTORY_PLMN_SEARCH == enPlmnSearchScene)
    {
        NAS_MMC_SetHistorySearchedFlag_PlmnSelection(enNetRatType, VOS_FALSE);
        NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_UpdateHistorySearchedFlgBasedOnAdditionalAction_PlmnSelection: Set curr rat has not performed history search", enNetRatType);
    }
    else
    {
        for (i = 0; i < pstPrioRatList->ucRatNum; i++)
        {
            enNetRatType = pstPrioRatList->aucRatPrio[i];
            NAS_MMC_SetHistorySearchedFlag_PlmnSelection(enNetRatType, VOS_TRUE);
        }
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_UpdateHistorySearchedFlgBasedOnAdditionalAction_PlmnSelection: Set all Rats has performed history search");
    }
...
VOS_VOID NAS_MMC_UpdateHistorySearchedFlagByInterPlmnSearchReq_PlmnSelection(
    NAS_MMC_INTER_PLMN_SEARCH_REQ_STRU                     *pstPlmnSearchReq
)
{
    VOS_UINT32                          i;
    ...
    /* 更新是否进行过history搜标志 */
    if (VOS_TRUE == NAS_MMC_IsPlmnSearchSceneInNeedSetHistorySearchedFlagSceneTbl(pstPlmnSearchReq->enPlmnSearchScene))
    {
...
VOS_VOID NAS_MMC_UpdateNetStatusByHighPrioPlmnList_PlmnSelection(
    NAS_MMC_INTER_PLMN_SEARCH_REQ_STRU                     *pstPlmnSearchReq,
    NAS_MMC_PLMN_SELECTION_LIST_INFO_STRU                  *pstPlmnSelectionListInfo
)
{
    NAS_MMC_PLMN_SELECTION_LIST_INFO_STRU                  *pstHighPrioList = VOS_NULL_PTR;
    NAS_MML_PLMN_WITH_RAT_STRU                              stPlmnWithRat;
    VOS_UINT32                                              i;
    VOS_UINT32                                              j;

    pstHighPrioList = NAS_MMC_GetHighPrioPlmnList();

    /* 如果列表类型不为BACKUP，不更新网络状态 */
    if (NAS_MMC_STORE_HIGH_PRIO_PLMN_SELECTION_LIST == NAS_MMC_GetPlmnSelectionListType())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_UpdateNetStatusByHighPrioPlmnList_PlmnSelection: Plmn Selection List Type is High Prio");
        return;
    }

    /* 如果列表类型为BACKUP，表示之前进行过NO RF或BG HISTORY搜，不是NO RF时，把快速指定搜成功注册被拒的网络更新成REGISTERED，避免重复注册 */
    if (NAS_MMC_PLMN_SEARCH_SCENE_RF_ENABLE == pstPlmnSearchReq->enPlmnSearchScene)
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_UpdateNetStatusByHighPrioPlmnList_PlmnSelection: Plmn Search Scene is RF ENABLE");
        return;
    }

    /* 可维可测，输出高优先级列表信息 */
    NAS_MMC_LogPlmnSelectionList(pstHighPrioList);

    for (i = 0; i < pstHighPrioList->usSearchPlmnNum; i++)
    {
        for (j = 0; j < pstHighPrioList->astPlmnSelectionList[i].ucRatNum; j++)
        {
            /* 在高优先级列表中的网络状态是REGISTERED，选网状态机中的网络状态不为NO EXIST时，把该PLMN更新成REGISTERED */

            if (NAS_MMC_NET_STATUS_SEARCHED_REGISTERED != pstHighPrioList->astPlmnSelectionList[i].astPlmnRatInfo[j].enNetStatus)
            {
                continue;
            }

            PS_MEM_SET(&stPlmnWithRat, 0, sizeof(NAS_MML_PLMN_WITH_RAT_STRU));
            stPlmnWithRat.enRat = pstHighPrioList->astPlmnSelectionList[i].astPlmnRatInfo[j].enRatType;
            stPlmnWithRat.stPlmnId.ulMcc    = pstHighPrioList->astPlmnSelectionList[i].stPlmnId.ulMcc;
            stPlmnWithRat.stPlmnId.ulMnc    = pstHighPrioList->astPlmnSelectionList[i].stPlmnId.ulMnc;

            if (NAS_MMC_NET_STATUS_SEARCHED_NO_EXIST != NAS_MMC_GetSpecPlmnWithRatNetStatusInPlmnSelectionList(&stPlmnWithRat,
                                                                                                            pstPlmnSelectionListInfo))
            {
                NAS_MMC_UpdatePlmnNetStatusInPlmnSelectionList(&stPlmnWithRat,
                                                               NAS_MMC_NET_STATUS_SEARCHED_REGISTERED,
                                                               pstPlmnSelectionListInfo);
            }
        }
...
)
{
    ...
    else if (NAS_MMC_ADDITIONAL_ACTION_SEARCH_SUITABLE_CELL_IN_SAME_PLMN == enAdditionalAction)
    {
        ...
        NAS_MMC_UpdateHistorySearchedFlgBasedOnAdditionalAction_PlmnSelection(enAdditionalAction);
    }
    else
    {
    }
...
VOS_VOID NAS_MMC_PerformAdditionalActionRegFinished_PlmnSelection()
{
    ...
    if ((VOS_TRUE == NAS_MMC_IsInterPlmnSearch_PlmnSelection())
     && (NAS_MMC_PLMN_SEARCH_SCENE_AVAILABLE_TIMER_EXPIRED_NCELL_SEARCH == enPlmnSearchScene))
    {
        stCurPlmn.stPlmnId = pstCurrSearchingPlmn->astPlmnId[0].stPlmnId;
        stCurPlmn.enRat    = pstCurrSearchingPlmn->astPlmnId[0].enRat;
        NAS_MMC_UpdatePlmnNetStatusInPlmnSelectionList(&stCurPlmn,
                                            NAS_MMC_NET_STATUS_SEARCHED_REGISTERED,
                                            pstPlmnSelectionListInfo);
    }
...
)
{
    switch (pstLmmAttachIndMsg->ulReqType)
    {
        ...
        case MMC_LMM_ATT_TYPE_EPS_EMERGENCY:
            ...
            NAS_MMC_ProcLmmEpsEmergencyAttachInd_PlmnSelection(pstLmmAttachIndMsg);
            break;
...
#if (FEATURE_ON == FEATURE_CSG)
                if ( (0 != ulAttemptCnt)
                    && (MMC_LMM_CELL_TYPE_CSG == enCurrCampCellType)
                    && (NAS_MMC_ADDITIONAL_ACTION_WAIT_REG_ATTEMPT == NAS_MMC_GetCsRegAdditionalAction_PlmnSelection())
                    && (NAS_MMC_ADDITIONAL_ACTION_WAIT_REG_ATTEMPT == NAS_MMC_GetPsRegAdditionalAction_PlmnSelection())
                    )
                {
                    enPsAdditionalAction = NAS_MMC_GetPsRegAdditionalAction_PlmnSelection();
                    enCsAdditionalAction = NAS_MMC_GetCsRegAdditionalAction_PlmnSelection();
    ...
    switch (pstLmmTauIndMsg->ulReqType)
    {
        ...
        case MMC_LMM_PS_PERIODIC_UPDATING:
            if (VOS_TRUE == NAS_MML_GetEpsEmcAttachFlg())
            {
                ...
                NAS_MMC_ProcLmmEpsEmergencyTauResultInd_PlmnSelection(pstLmmTauIndMsg);
            }
            else
            {
...
)
{
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_PlmnSelection,NAS_MML_RAT_CAPABILITY_STATUS_DISABLE return false");
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_PlmnSelection,NAS_MML_SIM_TYPE_SIM return false");
    ...
    if ( VOS_FALSE == NAS_MML_GetSimPsRegStatus() )
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_PlmnSelection,NAS_MML_GetSimPsRegStatus is false return false");
    ...
    if ( VOS_FALSE == NAS_MML_GetPsAttachAllowFlg() )
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_PlmnSelection,NAS_MML_GetPsAttachAllowFlg is false return false");
    ...
    {    
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_PlmnSelection,NAS_MMC_PLMN_SEARCH_SCENE_DISABLE_LTE is false return false");
    ...
    {    
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_PlmnSelection,dam and NAS_MML_GetAddDamPlmnInDisablePlmnWithRatListFlag is false,not enable lte");
    ...
    {
        /* 用户指定搜时需要ENABLE LTE */
        if (NAS_MMC_PLMN_SEARCH_SCENE_USER_SPEC_PLMN_SEARCH == enPlmnSearchScene)
        {
            return VOS_TRUE;
        }

        /* 紧急呼触发或LTE被#14拒绝导致的Disable LTE，不需要重新Enable LTE */
        ...
        {            
            NAS_NORMAL_LOG2(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_PlmnSelection,enDisableLteReason reason or emcflg,not enable lte",enDisableLteReason,ucEmcFlg);

            return VOS_FALSE;
        }

        /* bg history搜状态机快速指定搜注册被拒或pref band list/full band list状态机中注册被拒触发的Disable LTE也不需要Enable LTE */
        if (VOS_TRUE == NAS_MMC_GetLteEnabledFlgWhenFastSpecSrch())
        {        
            NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_PlmnSelection,NAS_MMC_GetLteEnabledFlgWhenFastSpecSrch is true,not enable lte");
            return VOS_FALSE;
        }

        
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_PlmnSelection, final enable lte");

        return VOS_TRUE;
    }

    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_PlmnSelection, final not enable lte",enLteCapabilityStatus);
...
VOS_VOID  NAS_MMC_ProcLmmEpsEmergencyAttachInd_PlmnSelection(
    LMM_MMC_ATTACH_IND_STRU            *pstLmmAttachIndMsg
)
{
    VOS_UINT32                                              ulEquPlmnNum;
...
VOS_VOID  NAS_MMC_ProcLmmEpsEmergencyTauResultInd_PlmnSelection(
    LMM_MMC_TAU_RESULT_IND_STRU        *pstLmmTauIndMsg
)
{
    VOS_UINT32                                              ulEquPlmnNum;
...
#if (FEATURE_ON == FEATURE_CSG)
    /* 用户CSG指定搜网场景 */
    if ( pstEntryMsg->ulEventType == NAS_BuildEventType(UEPS_PID_MSCC, ID_MSCC_MMC_CSG_SPEC_SEARCH_REQ) )
    {
        return NAS_MMC_PLMN_SEARCH_SCENE_USER_CSG_SPEC_SEARCH;
    }
#endif
...
)
{
    ...
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_IsNeedSortRoamPlmnSelectionList_PlmnSelection: enPlmnSearchScene is:", enPlmnSearchScene);

    ucIsOosScene        = NAS_MMC_IsOosPlmnSearchScene(enPlmnSearchScene);
...
)
{
    ...
    for (ulIndex = 0; ulIndex < stSearchedPlmnInfoMsg.ulAvailPlmnNum; ulIndex++)
    {
        ...
        for (i = 0; i < pstCurrSearchingPlmn->ulPlmnNum; i++)
        {
            if (VOS_TRUE == NAS_MML_CompareBcchPlmnwithSimPlmn(&pstCurrSearchingPlmn->astPlmnId[i].stPlmnId, (NAS_MML_PLMN_ID_STRU *)&stSearchedPlmnInfoMsg.astPlmnWithLacList[ulIndex].stPlmnId))
            {
                NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedStopPlmnSearchRcvRrcSearchedPlmnInfoInd_PlmnSelection: searched plmn is in searching plmn list");
        ...
        if (VOS_FALSE == NAS_MML_IsPlmnMccBelongToTheSameCountry(pstCurrSearchingPlmn->astPlmnId[0].stPlmnId.ulMcc, stSearchedPlmnInfoMsg.astPlmnWithLacList[ulIndex].stPlmnId.ulMcc))
        {
            ulIsExistDiffMccWithCurrSearchingMcc  = VOS_TRUE;
    ...
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_IsNeedStopPlmnSearchRcvRrcSearchedPlmnInfoInd_PlmnSelection: ulIsExistDiffMccWithCurrSearchingMcc is ", ulIsExistDiffMccWithCurrSearchingMcc);

    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_IsNeedStopPlmnSearchRcvRrcSearchedPlmnInfoInd_PlmnSelection: ucAllSearchedPlmnForbiddenPlmnFlag is ", ucAllSearchedPlmnForbiddenPlmnFlag);

    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_IsNeedStopPlmnSearchRcvRrcSearchedPlmnInfoInd_PlmnSelection: ucIsExistSearchedPlmnPrioCurrSearchingPlmn is ", ucIsExistSearchedPlmnPrioCurrSearchingPlmn);
...
)
{
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedSkipFddRcvRrcSearchedPlmnInfoInd_PlmnSelection: Not NAS_UTRANCTRL_UTRAN_SWITCH_MODE_AUTO, return FALSE");
        return VOS_FALSE;
    }
    ...
    for (i = 0; i < pstSearchedPlmnInfoMsg->ulAvailPlmnNum; i++)
    {
        ...
        {
            NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedSkipFddRcvRrcSearchedPlmnInfoInd_PlmnSelection: Return TRUE");
            return VOS_TRUE;
        }
    }

    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedSkipFddRcvRrcSearchedPlmnInfoInd_PlmnSelection: Return FALSE");
    return VOS_FALSE;
}
...
#if (FEATURE_ON == FEATURE_CSG)
    ulIsUserSpecSrch = NAS_MMC_IsUserCsgSpecSearchType_PlmnSelection(enCurrentFsmId,ulCurrentEventType);
    if ((VOS_TRUE == ulIsUserSpecSrch)
        && (VOS_TRUE == ulIsPlmnForbid))
    {
        return VOS_FALSE;
...
)
{
    ...
    switch(enPlmnSearchScene)
    {
        ...
        case NAS_MMC_PLMN_SEARCH_SCENE_AREA_LOST_ROAMING_CFG:
            return VOS_FALSE;
        ...
        case NAS_MMC_PLMN_SEARCH_SCENE_FULLBAND_LIST_PLMN_SEARCH:
            if (VOS_FALSE == pstPlmnListInfo->stSearchedType.ucSpecSearchedFlg)
            {
                return VOS_FALSE;
            }
            break;

        case NAS_MMC_PLMN_SEARCH_SCENE_USER_LIST:
            if (VOS_FALSE == pstPlmnListInfo->stSearchedType.ucSpecSearchedFlg)
            {
                return VOS_FALSE;
            }
...
VOS_UINT32 NAS_MMC_IsNeedDelCurrCampPlmn_PlmnSelection(
    NAS_MMC_PLMN_SEARCH_SCENE_ENUM_UINT32   enPlmnSearchScene,
...
)
{
    ...
    switch(enPlmnSearchScene)
    {
        /* inter list或user list退出状态机时是下面两种搜网场景 */
        case NAS_MMC_PLMN_SEARCH_SCENE_USER_LIST:
        case NAS_MMC_PLMN_SEARCH_SCENE_DISABLE_LTE:
             ...
             && (NAS_MMC_SPEC_PLMN_SEARCH_STOP == ucSpecPlmnSearchFlg))
            {
                ...
                {
                    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDelCurrCampPlmn_PlmnSelection: USER_LIST or DISABLE_LTE need Delete Curr Camp PLMN");
                    return VOS_TRUE;
                }
            }
           ...
           /* pref band list或full band list，是驻留态列表搜，退出状态机时是下面两种搜网场景 */
           case NAS_MMC_PLMN_SEARCH_SCENE_PREFBAND_LIST_PLMN_SEARCH:
           case NAS_MMC_PLMN_SEARCH_SCENE_FULLBAND_LIST_PLMN_SEARCH:

            if ((VOS_TRUE == NAS_MMC_IsNeedDelCurrCampPlmn_PlmnSelectionStrategy(pstLai))
             && (NAS_MMC_SPEC_PLMN_SEARCH_STOP == ucSpecPlmnSearchFlg))
            {
                NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDelCurrCampPlmn_PlmnSelection: PREFBAND_LIST or FULLBAND_LIST need Delete Curr Camp PLMN");
                return VOS_TRUE;
            }
    ...
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_IsNeedDelCurrCampPlmn_PlmnSelection: no need Delete Curr Camp PLMN ", enPlmnSearchScene);
...
VOS_UINT32 NAS_MMC_IsNeedProcUserCsgSpecSearchRegRslt_PlmnSelection(
    NAS_MML_REG_FAIL_CAUSE_ENUM_UINT16  enCsCause,
    NAS_MML_REG_FAIL_CAUSE_ENUM_UINT16  enPsCause
)
{
    ...
    /* 目前只支持LTE CSG,CS和PS两个域的注册结果是一起获取到的,CS或PS注册15号原因值时,
    会对当前网络再进行搜网，继续等注册结果 */
    if ( ((NAS_MML_REG_FAIL_CAUSE_NO_SUITABL_CELL       == enCsCause)
        && (NAS_MML_REG_FAIL_CAUSE_NO_SUITABL_CELL       == enPsCause))
        || ((NAS_MML_REG_FAIL_CAUSE_FORBIDDEN_LA_FOR_ROAM == enCsCause)
...
VOS_UINT32 NAS_MMC_IsUserCsgSpecSearchSucc_PlmnSelection(
    NAS_MML_REG_FAIL_CAUSE_ENUM_UINT16  enPsCause
)
{
    NAS_MML_NET_RAT_TYPE_ENUM_UINT8     enRatType;
    VOS_UINT32                          ucImsVoiceAvailFlag;
    VOS_UINT32                          ulIsCampOnUserSpecCsgIdFlag;
    NAS_MML_PLMN_WITH_CSG_ID_STRU       stUserCsgSpecPlmnWithCsgId;
    ...
    /* 注册结果为ims可用,或ims不可用不满足disable lte条件，
    向MMA发送用户CSG指定搜网成功 */
    PS_MEM_CPY(&stCurrCampPlmnWithCsgId.stPlmnId, NAS_MML_GetCurrCampPlmnId(), sizeof(stCurrCampPlmnWithCsgId.stPlmnId));
    enRatType                           = NAS_MML_GetCurrNetRatType();
    stCurrCampPlmnWithCsgId.ulCsgId     = NAS_MML_GetCurrCampCsgId();
    pstCsgSpecSrchPlmnWithCsgId         = NAS_MMC_GetCsgSpecSrchPlmnWithCsgId();

    stUserCsgSpecPlmnWithCsgId.ulCsgId  = pstCsgSpecSrchPlmnWithCsgId->ulCsgId;
    stUserCsgSpecPlmnWithCsgId.stPlmnId = pstCsgSpecSrchPlmnWithCsgId->stPlmnWithRat.stPlmnId;

    ulIsCampOnUserSpecCsgIdFlag         = VOS_FALSE;
    ucImsVoiceAvailFlag                 = NAS_MML_GetImsVoiceAvailFlg();

    if ((VOS_TRUE == NAS_MML_IsPlmnWithCsgIdInDestPlmnWithCsgIdList(&stUserCsgSpecPlmnWithCsgId, 1, &stCurrCampPlmnWithCsgId))
        && (enRatType == pstCsgSpecSrchPlmnWithCsgId->stPlmnWithRat.enRat))
    {
        ulIsCampOnUserSpecCsgIdFlag = VOS_TRUE;
    }
    ...
    if (((NAS_MML_REG_FAIL_CAUSE_NULL == enPsCause)
        ...
        &&(VOS_TRUE == ulIsCampOnUserSpecCsgIdFlag))
    {
        return VOS_TRUE;
    }
...
VOS_UINT32 NAS_MMC_ProcUserCsgSpecSearchRegRslt_PlmnSelection(
    NAS_MML_REG_FAIL_CAUSE_ENUM_UINT16  enCsCause,
    NAS_MML_REG_FAIL_CAUSE_ENUM_UINT16  enPsCause
    )
{
    ...
    if (VOS_FALSE == NAS_MMC_IsNeedProcUserCsgSpecSearchRegRslt_PlmnSelection(enCsCause, enPsCause))
    {
        return VOS_FALSE;
    }

    if (VOS_TRUE == NAS_MMC_IsUserCsgSpecSearchSucc_PlmnSelection(enPsCause))
    {
        ...
        /* 认为csg指定搜注册成功的csg id需要加入allowed csg list中 */
        if (VOS_TRUE == NAS_MMC_IsRegSuccNeedAddCsgIdInAllowedCsgList(&stCurrCampPlmnWithCsgId, NAS_MML_GetCurrCampCsgIdHomeNodeBName()))
        {
            ...
            /* 如果加入allowed csg list中的csg id在禁止csg列表中，则需要删除 */
            NAS_MMC_DeleteSpecCsgIdInForbiddenCsgIdList(&stCurrCampPlmnWithCsgId);
        ...
        {
            /* 如果驻留的是混合CSG ID，虽然不用添加到Allowed List，但是需要加入通知接入层的白名单 */
            if(VOS_TRUE == NAS_MMC_IsCampOnUserSpecHybridCsgId())
            {
                /* 通知L更新后的CSG白名单 */
                NAS_MMC_SndLmmCsgWhiteList();
            }
    ...
    /* 已获取到CS和PS的注册结果，且有明确原因值,上报失败 */
    NAS_MMC_ProcUserCsgSearchResultFail_PlmnSelection();

    /* csg注册失败需要获取下一个网络rplmn继续搜网，如果cs和ps的AdditionalAction为无需搜网，
    需要更新为需要搜网 */
    enCsAdditionalAction = NAS_MMC_GetCsRegAdditionalAction_PlmnSelection();
    enPsAdditionalAction = NAS_MMC_GetPsRegAdditionalAction_PlmnSelection();
    enAdditionalAction   = NAS_MMC_GetPrioAddtionalAction(enCsAdditionalAction, enPsAdditionalAction);
    ...
    if ((NAS_MMC_ADDITIONAL_ACTION_CELL_SELECTION == enAdditionalAction)
        ...
        || (NAS_MMC_ADDITIONAL_ACTION_ACCESS_BARRED == enAdditionalAction))
    {
        NAS_MMC_SetPsRegAdditionalAction_PlmnSelection(NAS_MMC_ADDITIONAL_ACTION_PLMN_SELECTION);
    }
...
VOS_UINT32 NAS_MMC_IsNeedSndCsgSpecSearchReq_PlmnSelection(
    NAS_MML_PLMN_LIST_WITH_RAT_STRU    *pstDestPlmnList
)
{
    NAS_MMC_PLMN_TYPE_ENUM_UINT8                            enPlmnType;
    ...
    pstPlmnSelectionList        = NAS_MMC_GetPlmnSelectionListInfo_PlmnSelection();
    enPlmnType                  = NAS_MMC_GetPlmnTypeInPlmnSelectionList(&(pstDestPlmnList->astPlmnId[0]), pstPlmnSelectionList);
    ucSearchedCsgSpecSearchFlag = NAS_MMC_GetSearchedCsgSpecSearchFlag_PlmnSelection(NAS_MML_NET_RAT_TYPE_LTE);

    if((VOS_TRUE == NAS_MMC_IsUserCsgSpecSearchType_PlmnSelection(NAS_MMC_GetCurrFsmId(), NAS_MMC_GetCurrFsmEventType()))
        && (NAS_MMC_PLMN_TYPE_USER_CSG_SPEC_PLMN == enPlmnType)
        && (VOS_FALSE == ucSearchedCsgSpecSearchFlag))
    {
        return VOS_TRUE;
    }
...
VOS_VOID NAS_MMC_ProcUserCsgSearchResultFail_PlmnSelection(VOS_VOID)
{
    /* CSG指定搜网失败，上报结果 */
    NAS_MMC_SndMsccUserCsgSpecSearchCnf(NAS_MSCC_PIF_USER_PLMN_SEARCH_RESULT_FAIL);
    ...
    /*更新入口消息为普通的搜网请求，后续搜网不再发CSG指定搜网类型，而是普通指定搜网类型 */
    NAS_MMC_SaveCommonPlmnSrchEntryMsg_PlmnSelection();
    NAS_MMC_InitCsgSpecSrchPlmnWithCsgId();
...
VOS_VOID  NAS_MMC_UpdateUserCsgSpecSearchInfo_PlmnSelection(
...
)
{
    ...
    pstPlmnWithCsgId->stPlmnWithRat.stPlmnId.ulMcc = pstPlmnIdWithRat->stPlmnId.ulMcc;
    pstPlmnWithCsgId->stPlmnWithRat.stPlmnId.ulMnc = pstPlmnIdWithRat->stPlmnId.ulMnc;
    pstPlmnWithCsgId->stPlmnWithRat.enRat          = pstPlmnIdWithRat->enRat;
    pstPlmnWithCsgId->ulCsgId                      = ulCsgId;

    /* 当前为CSG指定搜，则初始化选网状态机的禁止漫游LA信息变量 */
    PS_MEM_SET( NAS_MMC_GetForbRoamLaInfo_PlmnSelection(),
        0X0, sizeof(NAS_MML_FORBIDPLMN_ROAMING_LAS_INFO_STRU) );
