        ...
        {                                                                       /* 没有MM连接存在                           */
            ...
            if (RRMM_LCS_STATE_START == NAS_MM_GetRrLcsState())
            {
                ...
                PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_NORMAL,
                    "Mm_Cell_S10_E66: MM STATUS is MM_STATE_RR_CONNECTION_RELEASE_NOT_ALLOWED.");
            ...
            else
            {
                ...
                PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_NORMAL,
                    "Mm_Cell_S10_E66: MM STATUS is WAIT_FOR_NETWORK_COMMAND.");
...
VOS_VOID Mm_Cell_S22_E32(VOS_VOID* pMsg)
{
    ...
    case RRC_EST_ACCESS_BARRED:
        ...
        else
        {
            ...
            /* 立即指派拒绝后，需要将RetryFlg置为TRUE，这样后续小区重选后上报系统消息
               能够立即触发LAU,测试卡不生效 */
            if (VOS_FALSE == NAS_USIMMAPI_IsTestCard())
            {
                NAS_NORMAL_LOG(WUEPS_PID_MM, "Mm_Cell_S22_E32: RRC_EST_IMMEDIATE_ASSIGN_REJECT, Set ucRetryFlg to TRUE");
                g_MmGlobalInfo.LuInfo.ucRetryFlg = MM_TRUE;
            }
...
VOS_VOID Mm_Cell_S3_E52(VOS_VOID* pMsg)
{
    ...
    NAS_NORMAL_LOG(WUEPS_PID_MM, "Mm_Cell_S3_E52: trigger LAU");
    NAS_NORMAL_LOG(WUEPS_PID_MM, "Mm_Cell_S3_E52: trigger lau");
...
VOS_VOID Mm_Cell_S6_E54(VOS_VOID* pMsg)
{
    ...
    if (NAS_MML_NET_RAT_TYPE_GSM == NAS_MML_GetCurrNetRatType())
    {
        ...
        */
        ...
        {
            NAS_NORMAL_LOG(WUEPS_PID_MM, "Mm_Cell_S6_E54: trigger LAU");
            Mm_ComLuOnly();
            g_MmGlobalInfo.LuInfo.ucT3213AttmptCnt++;
        }
        ...
        {                                                                       /* 网络模式II                               */
            if (VOS_TRUE == NAS_MML_GetCsAttachAllowFlg())
            {
                ...
                {                                                               /* 如果LAI不变,更新状态为U1                 */
                    ...
                    {
                        PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_NORMAL, "Mm_Cell_S4_E2: need attach trigger lau");
                    
                        g_MmGlobalInfo.LuInfo.ucLuType = MM_IE_LUT_IMSI_ATTACH; /* 设置LU类型                               */
                        NAS_NORMAL_LOG(WUEPS_PID_MM, "Mm_Cell_S4_E2: LAI not change and U1, trigger LAU with IMSI ATTACH");
                        Mm_ComLuOnly();                                         /* 发送LU REQ                               */
                    }
                }
                else
                {
                    NAS_NORMAL_LOG(WUEPS_PID_MM, "Mm_Cell_S4_E2: lai change or U1 trigger lau");
                    
                    g_MmGlobalInfo.LuInfo.ucLuType = MM_IE_LUT_NORMAL_LU;   /* 设置LU类型                               */
                    NAS_NORMAL_LOG(WUEPS_PID_MM, "Mm_Cell_S4_E2: LAI change or not U1, trigger LAU");
                    Mm_ComLuOnly();                                         /* 发送LU REQ                               */
                }
            }
        }
        ...
        {                                                                       /* 网络模式II                               */
            if (VOS_TRUE == NAS_MML_GetCsAttachAllowFlg())
            {
                ...
                {                                                               /* 如果LAI不变,更新状态为U1                 */
                    ...
                    {
                        PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_NORMAL, "Mm_Cell_S19_E2: need attach trigger lau");
                ...
                {
                    PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_NORMAL, "Mm_Cell_S19_E2: lai change or NOT U1 trigger lau");
    ...
    else
    {
        ...
        {                                                                       /* 网络模式为I,且用户模式为A的场合          */
            ...
            else
            {
                ...
                if (MM_IDLE_ATTEMPTING_TO_UPDATE == g_MmGlobalInfo.ucState)
                {
                    NAS_NORMAL_LOG(WUEPS_PID_MM, "Mm_Cell_S3_E12: trigger LAU");
                    g_MmGlobalInfo.LuInfo.ucLuType = MM_IE_LUT_NORMAL_LU;
                    Mm_ComLuOnly();
                }
                else
    ...
    else
    {
        ...
        {                                                                       /* 网络模式为I,且用户模式为A的场合          */
              ...
              && (MM_IDLE_LIMITED_SERVICE == ulCurrMMState) )
            {
                ...
                {
                    NAS_NORMAL_LOG(WUEPS_PID_MM, "Mm_Cell_S4_E12: lai change or U1 trigger lau");
        ...
        {                                                                       /* 网络模式为I,且用户模式为A的场合          */
            ...
            if ( VOS_TRUE == NAS_MM_IsCsEnableLau() )
            {
                ...
                if ( (VOS_TRUE == NAS_MML_IsCsLaiChanged())
                  || (NAS_MML_LOCATION_UPDATE_STATUS_NOT_UPDATED == enUpdateStatus))
                {
                    NAS_NORMAL_LOG(WUEPS_PID_MM, "Mm_Cell_S174_E12: lai change or U1 trigger lau");
...
VOS_VOID    Mm_Cell_S6_E65( VOS_VOID* pMsg)
{
    ...
    else
    {
        ...
        if (MM_COM_SRVST_NORMAL_SERVICE == NAS_MM_GetLauUptNeededCsSrvStatus())
        {
            ...
            /* 正常服务发起建链 */
            NAS_MM_EstMmConnection(MM_CONN_CTRL_SS);
        }
        else
        {
            /* ==>A32D12640 */
    ...
    else
    {
        ...
        {                                                                       /* TI合法                                   */
            ...
            else
            {
                ...
                {
                    NAS_NORMAL_LOG(WUEPS_PID_MM, "Mm_Cell_S5_E24: NOT A+I trigger lau");
...
VOS_VOID    Mm_Cell_S5_E62( VOS_VOID*pMsg )
{
    ...
    {                                                                           /* 消息检查结果成功                         */
        ...
        {
            NAS_NORMAL_LOG(WUEPS_PID_MM, "Mm_Cell_S5_E62: NOT A+I trigger lau");
...
VOS_VOID    Mm_Cell_S5_E65( VOS_VOID*pMsg )
{
    ...
    {                                                                           /* 消息检查结果成功                         */
        ...
        {
            NAS_NORMAL_LOG(WUEPS_PID_MM, "Mm_Cell_S5_E65: NOT A+I trigger lau");
                    ...
                    )
{
    ...
    else if (NO_MM_CONN_ESTING != g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].ucMMConnEstingTI)
    {
        ...
        NAS_NORMAL_LOG1(WUEPS_PID_MM, "Mm_Cell_S12_E77:NORMAL: MM State: ", NAS_MM_GetMmState());
    }
    else if (NO_MM_CONN_ESTING !=
        g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_SMS].ucMMConnEstingTI)
    {                                                                   /* SMS发起的RR连接的建立                    */
    ...
    {
        NAS_MM_RelBufferedServiceEstReq(NAS_MMCM_REL_CAUSE_MM_INTER_ERR_WAIT_EST_CNF_TIME_OUT);
    }
...
VOS_VOID Mm_Cell_S6_E74(VOS_VOID *pRcvMsg)
{
     ...
     || ( RRC_NAS_T3122_EXPIRE == pRrMmAcInfoChgInd->ulAcChangeInd) )
    {
        ...
        else
        {
            ...
            {
                NAS_NORMAL_LOG1(WUEPS_PID_MM, "Mm_Cell_S6_E74: not A+I or PsRestriction trigger lau, PsRestriction is", ucPsRestrictionFlg);
...
/*******************************************************************************
  ...
  FUNCTION : 在MM CONNECTION ACTIVE状态下收到RRMM_DATA_IND(CM_SRV_ACC)的处理
  INPUT    : VOS_VOID            *pRcvMsg  收到的原语头指针
  OUTPUT   : VOS_VOID
  RETURN   : VOS_VOID
  NOTE     :
                    ...
                    )
{
    ...
    {
        /* 释放MM缓存的所有CM层的建链请求 */
        NAS_MM_RelBufferedServiceEstReq(NAS_MMCM_REL_CAUSE_MM_INTER_ERR_WAIT_CC_REEST_TIME_OUT);
    }
    ...
    {                                                                           /* 消息检查结果成功                         */
        ...
        {                                                                       /* 终止的MM连接建立是正在建立的             */
              ...
              && (MM_TRUE != ucMmConnFlg) )
            {
                ...
                Mm_ComSetMmState(MM_STATE_RR_CONNECTION_RELEASE_NOT_ALLOWED);
                PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_NORMAL,
                    "Mm_Cell_S9_E80: MM STATUS is MM_STATE_RR_CONNECTION_RELEASE_NOT_ALLOWED.");
                Mm_TimerStart(MM_TIMER_T3241);
            }
            else
            {
                ...
                Mm_ComSetMmState(WAIT_FOR_NETWORK_COMMAND);
                PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_NORMAL,
                    "Mm_Cell_S9_E80: MM STATUS is WAIT_FOR_NETWORK_COMMAND.");
                Mm_TimerStart(MM_TIMER_T3240);
            }
    ...
    {                                                                           /* 消息检查结果成功                         */
        ...
        {                                                                       /* 终止的MM连接建立是正在建立的             */
              ...
              && (MM_TRUE != ucMmConnFlg) )
            {
                ...
                Mm_ComSetMmState(MM_STATE_RR_CONNECTION_RELEASE_NOT_ALLOWED);
                PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_NORMAL,
                    "Mm_Cell_S9_E81: MM STATUS is MM_STATE_RR_CONNECTION_RELEASE_NOT_ALLOWED.");
                Mm_TimerStart(MM_TIMER_T3241);
            }
            else
            {
                ...
                Mm_ComSetMmState(WAIT_FOR_NETWORK_COMMAND);
                PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_NORMAL,
                    "Mm_Cell_S9_E81: MM STATUS is WAIT_FOR_NETWORK_COMMAND.");
                Mm_TimerStart(MM_TIMER_T3240);
            }
        }
        else
    ...
    {                                                                           /* 消息检查结果成功                         */
        ...
        {                                                                       /* 终止的MM连接建立是正在建立的             */
            ...
            g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_SS].aucMMConnExtFlg[0] ) )
            {
                ...
                NAS_MM_UpdateMmConnByTi(MM_CONN_CTRL_SS, g_MmSsAbortReq.ulTransactionId, NAS_MM_CONN_NOT_EXIST);
                  ...
                  && (MM_TRUE != ucMmConnFlg) )
                {
                    ...
                    Mm_ComSetMmState(MM_STATE_RR_CONNECTION_RELEASE_NOT_ALLOWED);
                    PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_NORMAL,
                        "Mm_Cell_S10_E80: MM STATUS is MM_STATE_RR_CONNECTION_RELEASE_NOT_ALLOWED.");
                    Mm_TimerStart(MM_TIMER_T3241);
                }
                else
                {
                    ...
                    Mm_ComSetMmState(WAIT_FOR_NETWORK_COMMAND);
                    PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_NORMAL,
                        "Mm_Cell_S10_E80: MM STATUS is WAIT_FOR_NETWORK_COMMAND.");
                    Mm_TimerStart(MM_TIMER_T3240);
                }
                    ...
                    )
{
    ...
    {                                                                           /* 消息检查结果成功                         */
        ...
        if ( 0 !=
            ...
            & ( 0x01 << ( g_MmSmsAbortReq.ulTransactionId - 8 ) ) ) )
        {                                                                       /* 终止的MM连接建立是正在建立的             */
            ...
            g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_SMS].aucMMConnExtFlg[0] ) )
            {
                  ...
                  && (MM_TRUE != ucMmConnFlg) )
                {
                    ...
                    Mm_ComSetMmState(MM_STATE_RR_CONNECTION_RELEASE_NOT_ALLOWED);
                    PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_NORMAL,
                        "Mm_Cell_S10_E81: MM STATUS is MM_STATE_RR_CONNECTION_RELEASE_NOT_ALLOWED.");
                    Mm_TimerStart(MM_TIMER_T3241);
                }
                else
                {
                    ...
                    Mm_ComSetMmState(WAIT_FOR_NETWORK_COMMAND);
                    PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_NORMAL,
                        "Mm_Cell_S10_E81: MM STATUS is WAIT_FOR_NETWORK_COMMAND.");
                    Mm_TimerStart(MM_TIMER_T3240);
                }
                    ...
                    )
{
    ...
    {                                                                           /* 没有存在的MM连接 */
        ...
        if (RRMM_LCS_STATE_START == NAS_MM_GetRrLcsState())
        {
            ...
            Mm_ComSetMmState(MM_STATE_RR_CONNECTION_RELEASE_NOT_ALLOWED);
            PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_NORMAL,
                "Mm_Cell_S27_E82: MM STATUS is MM_STATE_RR_CONNECTION_RELEASE_NOT_ALLOWED.");
            Mm_TimerStart(MM_TIMER_T3241);
        }
        else
        {
            ...
            Mm_ComSetMmState(WAIT_FOR_NETWORK_COMMAND);
            PS_NAS_LOG(WUEPS_PID_MM, VOS_NULL, PS_LOG_LEVEL_NORMAL,
                "Mm_Cell_S27_E82: MM STATUS is WAIT_FOR_NETWORK_COMMAND.");
            Mm_TimerStart(MM_TIMER_T3240);
        }
    }
    else
...
VOS_UINT32 NAS_MM_IsRadioAccessCapNeedTriggerLau( VOS_VOID )
{
    NAS_MML_PLMN_RAT_PRIO_STRU          stMsRatCapabilityList;
    NAS_MML_PLMN_RAT_PRIO_STRU          *pstMsPrioRatList    = VOS_NULL_PTR;

    /* NMO1下,MM不需要进行LAU */
    if (VOS_FALSE == NAS_MM_IsNeedProcRadioCapChange())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MM_IsRadioAccessCapNeedTriggerLau: CURRENT IS A+I");

        return VOS_FALSE;
    }

    /* 如果NV RatBalancing 没开，则返回FALSE */
    if (VOS_FALSE == NAS_MML_GetRatBalancingEnableFlg())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MM_IsRadioAccessCapNeedTriggerLau: RatBalancingEnableFlg is false" );

        return VOS_FALSE;
    }

    pstMsPrioRatList                    = NAS_MML_GetMsNotifyNwCsPrioRatList();

    /* 如果当前通知网络的接入技术为0,则出发保护不发起LAU或者RAU*/
    if (0 == pstMsPrioRatList->ucRatNum)
    {
    ...
    if (VOS_TRUE == NAS_MML_IsMsRatCapabilityChanged(&stMsRatCapabilityList, NAS_MML_GetMsNotifyNwCsPrioRatList()))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MML_IsMsRatCapabilityChanged is true");
        
        return VOS_TRUE;
    }

    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MML_IsMsRatCapabilityChanged is false");
...
VOS_VOID NAS_MM_RcvGmmRadioAccessCapChangeInd_MmIdleNormalService(
    VOS_VOID                            *pRcvMsg
)
{
    /* 当前在L模式下，丢弃不处理 */
    if (NAS_MML_NET_RAT_TYPE_LTE == NAS_MML_GetCurrNetRatType()) 
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC,"NAS_MM_RcvGmmRadioAccessCapChangeInd_MmIdleNormalService, current is lte mode,discard msgs");
        return;
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MM_RcvGmmRadioAccessCapChangeInd_MmIdleNormalService mmc SyscfgFlg is ture");
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_RcvGmmRadioAccessCapChangeInd_MmIdleNormalService: RAT change triggered LAU");
        g_MmGlobalInfo.LuInfo.ucLuType = MM_IE_LUT_NORMAL_LU;    /* 设置LU类型  */
        Mm_ComLuOnly();
    }
...
VOS_VOID NAS_MM_RcvGmmRadioAccessCapChangeInd_WaitForLauRrConnnection(
    VOS_VOID                            *pRcvMsg
)
{
...
VOS_VOID Mm_Cell_S3_E54(VOS_VOID* pMsg)
{
    ...
    if (NAS_MML_NET_RAT_TYPE_LTE != NAS_MML_GetCurrNetRatType())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MM, "Mm_Cell_S3_E54: trigger lau");
...
VOS_VOID NAS_MM_RcvMmcStartReq_RrConnRelNotAllow(VOS_VOID *pMsg)
{
    /* 消息检查失败，直接返回 */
    if (MM_TRUE != Mm_RcvMmcStartReq(pMsg))
    {
    ...
    NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_RcvMmcStartReq_RrConnRelNotAllow: MM STATUS is MM_STATE_NULL");
...
VOS_VOID NAS_MM_RcvMmcAttachReq_RrConnRelNotAllow(VOS_VOID *pMsg)
{
    /* 消息检查失败，直接返回 */
    if (MM_TRUE != Mm_RcvMmcAttachReq(pMsg))
    {
        return;
    }
...
VOS_VOID NAS_MM_RcvMmcSysinfoInd_RrConnRelNotAllow(VOS_VOID* pMsg)
{
    MMCMM_SYS_INFO_IND_STRU            *pstSysInfoMsg = VOS_NULL_PTR;
...
VOS_VOID NAS_MM_RcvGmmGprsDetachIni_RrConnRelNotAllow(VOS_VOID* pMsg)
{
    /* 消息检查失败，直接返回 */
    if (MM_TRUE != NAS_MM_RcvGmmGprsDetechIni(pMsg))
    {
        return;
    }
    ...
    /* 记录迁移之前的状态，并迁移到IMSI_DETACH_PENDING状态 */
    NAS_MM_SetMmPreState(NAS_MM_GetMmState());
    Mm_ComSetMmState(IMSI_DETACH_PENDING);

    NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_RcvGmmGprsDetachIni_RrConnRelNotAllow: MM STATUS is IMSI_DETACH_PENDING");

    return;
}


VOS_VOID NAS_MM_RcvGmmGprsDetechCmpl_RrConnRelNotAllow(VOS_VOID* pMsg)
{
    /* 消息检查失败，直接返回 */
    if (MM_TRUE != Mm_RcvGmmGprsDetechCmpl(pMsg))
    {
        return;
    }
     ...
     && (MM_NET_MODE_I == g_MmGlobalInfo.ucNtMod))
    {
          ...
          && (GMM_STATUS_DETACHED == g_MmSubLyrShare.GmmShare.ucAttachSta))
        {
            /* 类似于B模式的场合，设置为类似B模式 */
            g_MmGlobalInfo.ucLikeB = MM_TRUE;
            NAS_INFO_LOG(WUEPS_PID_MM, "NAS_MM_RcvGmmGprsDetechCmpl_RrConnRelNotAllow: MmLikeB is MM_TRUE");
        }
    }
...
VOS_VOID NAS_MM_RcvGmmNetworkDetachInd_RrConnRelNotAllow(VOS_VOID* pMsg)
{
    GMMMM_NETWORK_DETACH_IND_STRU       *pstNetWkDetachInd = VOS_NULL_PTR;

    if (MM_TRUE != NAS_MM_RcvGmmNetworkDetechInd(pMsg))
    {
        return;
    }
    ...
    if ( GMMMM_NET_DETACH_REATTACH == pstNetWkDetachInd->enDetachType )
    {
        NAS_MM_StartT3212Timer(NAS_MM_T3212_START_SCENE_NORMAL_START);
        g_MmGlobalInfo.ucLikeB = MM_TRUE;
        NAS_INFO_LOG(WUEPS_PID_MM, "NAS_MM_RcvGmmNetworkDetachInd_RrConnRelNotAllow: MmLikeB is MM_TRUE");
    }
    else if ( GMMMM_NET_DETACH_IMSI == pstNetWkDetachInd->enDetachType )
    {
        Mm_TimerStop(MM_TIMER_PROTECT_AGENT);
        ...
        /* 记录流程和原因值 */
        Mm_ComSaveProcAndCauseVal(MM_NET_DETACH_PROC, NAS_MML_REG_FAIL_CAUSE_NTDTH_IMSI);
    }
    else
    {
        switch ( pstNetWkDetachInd->ulDetachCause )
        {
            ...
            case NAS_MML_REG_FAIL_CAUSE_GPRS_SERV_AND_NON_GPRS_SERV_NOT_ALLOW:
                ...
                /* 记录流程和原因值 */
                Mm_ComSaveProcAndCauseVal(MM_NET_DETACH_PROC, (VOS_UINT16)pstNetWkDetachInd->ulDetachCause);

                /* 设置服务状态 */
                g_MmGlobalInfo.ucMmServiceState = MM_NO_IMSI;
                NAS_INFO_LOG(WUEPS_PID_MM, "NAS_MM_RcvGmmNetworkDetachInd_RrConnRelNotAllow: MmServiceState is MM_NO_IMSI");
...
VOS_VOID NAS_MM_RcvGmmActionRsltInd_RrConnRelNotAllow(VOS_VOID* pMsg)
{
    /* 此状态下处理同WAIT FOR NETWORK COMMAND状态 */
    Mm_Cell_S11_E14(pMsg);

    return;
}


VOS_VOID NAS_MM_RcvGmmLuInitiation_RrConnRelNotAllow(VOS_VOID* pMsg)
{
    /* 此状态下处理同WAIT FOR NETWORK COMMAND状态 */
    Mm_Cell_S9_E15(pMsg);
...
VOS_VOID NAS_MM_RcvGmmAuthFailureInd_RrConnRelNotAllow(VOS_VOID* pMsg)
{
    /* 此状态下处理同WAIT FOR NETWORK COMMAND状态 */
    Mm_Cell_S11_E16(pMsg);

    return;
}


VOS_VOID NAS_MM_RcvMmcModeChangeReq_RrConnRelNotAllow(VOS_VOID* pMsg)
{
    /* 此状态下处理同WAIT FOR NETWORK COMMAND状态 */
    Mm_Cell_S9_E20(pMsg);
...
VOS_VOID NAS_MM_RcvMmcCoverageLostInd_RrConnRelNotAllow(VOS_VOID* pMsg)
{
    MM_SERVICE_STATUS_ENUM_UINT8        enMmSrvStatus;
    /* 消息检查失败，直接返回 */
    if (MM_TRUE != Mm_RcvMmcCoverageLostInd(pMsg))
    {
        return;
    }
    ...
    /* 正常服务需要MM发起LAU时，启动T3212定时器 */
    enMmSrvStatus = NAS_MM_GetMmServiceState();
    if ( (VOS_TRUE == NAS_MM_IsDisableGprsCombineAttach())
      && (MM_NORMAL_SERVICE == enMmSrvStatus) )
    {
        NAS_MM_StartT3212Timer(NAS_MM_T3212_START_SCENE_NORMAL_START);
    }

    /* 切换到无服务状态 */
    NAS_MM_SetMmServiceState(MM_NO_SERVICE);
    NAS_INFO_LOG(WUEPS_PID_MM, "NAS_MM_RcvMmcCoverageLostInd_RrConnRelNotAllow: MmServiceState is MM_NO_SERVICE");

    /* 保存出服务区前的状态，在搜网成功后用于判断LAI是否改变 */
    g_MmGlobalInfo.ucNotCampONPreState = g_MmGlobalInfo.ucState;

    /* 记录切换之前的状态，切换MM状态为MM_IDLE_NO_CELL_AVAILABLE */
    NAS_MM_SetMmPreState(NAS_MM_GetMmState());
    Mm_ComSetMmState(MM_IDLE_NO_CELL_AVAILABLE);

    NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_RcvMmcCoverageLostInd_RrConnRelNotAllow: MM STATUS is MM_IDLE_NO_CELL_AVAILABLE");
...
VOS_VOID NAS_MM_RcvMmcDetachReq_RrConnRelNotAllow(VOS_VOID* pMsg)
{
    MMCMM_DETACH_REQ_STRU          *pstMmcDetachReq = VOS_NULL_PTR;
    MM_MSG_IMSI_DETACH_IND_STRU     stImsiDetachInd;

    PS_MEM_SET(&stImsiDetachInd, 0, sizeof(stImsiDetachInd));

    /* 消息检查失败，直接返回 */
    if (MM_TRUE != Mm_RcvMmcDetachReq(pMsg))
    {
        return;
    }

    pstMmcDetachReq = (MMCMM_DETACH_REQ_STRU*)pMsg;
    ...
    if ( VOS_TRUE == NAS_MM_IsNeedGmmProcUserDetach() )
    {
        ...
        if ( MM_RCV_AUTH_CNF_FLG == (g_MmGlobalInfo.ucRcvAgentFlg & MM_RCV_AUTH_CNF_FLG) )
        {
            NAS_NORMAL_LOG1(WUEPS_PID_MM, "NAS_MM_RcvMmcDetachReq_RrConnRelNotAllow: g_MmGlobalInfo.ucRcvAgentFlg: ", g_MmGlobalInfo.ucRcvAgentFlg);
            g_MmGlobalInfo.ucRcvAgentFlg &= ~MM_RCV_AUTH_CNF_FLG;
        ...
        /* 记录切换之前的状态，切换MM状态为IMSI_DETACH_PENDING */
        NAS_MM_SetMmPreState(NAS_MM_GetMmState());
        Mm_ComSetMmState(IMSI_DETACH_PENDING);

        NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_RcvMmcDetachReq_RrConnRelNotAllow: MM STATUS is IMSI_DETACH_PENDING");

        if (MMC_MM_PS_DETACH != pstMmcDetachReq->enDetachType)
        {
            /* 设置DETACH类型 */
            g_MmGlobalInfo.ucDetachType = MM_DETACH_CS_NORMAL;
        }
      ...
      || (MM_STATUS_ATTACHED != g_MmSubLyrShare.MmShare.ucCsAttachState) )
    {
        ...
        /* 判断等待标志是否存在，如果存在则发送MMCMM_DETACH CNF */
        if ( (MM_WAIT_CS_DETACH == g_MmGlobalInfo.stDetachInfo.enDetachType)
          || (MM_WAIT_CS_PS_DETACH == g_MmGlobalInfo.stDetachInfo.enDetachType))
        {
            NAS_MM_SndMmcDetachCnf(MM_DETACHED);
        ...
        /* 存储建链流程及原因值 */
        Mm_ComSaveProcAndCauseVal(MM_IMSI_DETACH, NAS_MML_REG_FAIL_CAUSE_NULL);

        /* 记录迁移之前的状态，并迁移到WAIT_FOR_NETWORK_COMMAND状态 */
        NAS_MM_SetMmPreState(NAS_MM_GetMmState());
        Mm_ComSetMmState(WAIT_FOR_NETWORK_COMMAND);
        NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_RcvMmcDetachReq_RrConnRelNotAllow: MM STATUS is WAIT_FOR_NETWORK_COMMAND");
    ...
    {
        /* 启动DETACH保护定时器，填写MobileID，发送IMSI_DEATCH_IND消息 */
        Mm_TimerStart(MM_TIMER_PROTECT_DETACH);
        NAS_MM_FillMobileID(&(stImsiDetachInd.MmIeMobileId.MobileID));
        Mm_ComMsgImsiDetachIniSndOfDataReq(&stImsiDetachInd);

        /* 记录迁移之前的状态，并迁移到WAIT_FOR_NETWORK_COMMAND状态 */
        NAS_MM_SetMmPreState(NAS_MM_GetMmState());
        Mm_ComSetMmState(IMSI_DETACH_INITIATED);
        NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_RcvMmcDetachReq_RrConnRelNotAllow: MM STATUS is IMSI_DETACH_INITIATED");
...
VOS_VOID NAS_MM_RcvMmcPowerOffReq_RrConnRelNotAllow(VOS_VOID* pMsg)
{
    MM_MSG_IMSI_DETACH_IND_STRU         stImsiDetachInd;
    ...
    /* 停止MM定时器,清空紧急呼号码列表 */
    (VOS_VOID)Mm_RcvMmcPowerOffReq(pMsg);
      ...
      && (MM_NET_MODE_I == g_MmGlobalInfo.ucNtMod) )
    {
          ...
          && (GMM_STATUS_ATTACHED == g_MmSubLyrShare.GmmShare.ucAttachSta) )
        {
            ...
            /* 记录迁移之前的状态，并迁移到WAIT FOR NETWORK COMMAND状态，启动T3240定时器 */
            NAS_MM_SetMmPreState(NAS_MM_GetMmState());
            Mm_ComSetMmState(WAIT_FOR_NETWORK_COMMAND);
            NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_RcvMmcPowerOffReq_RrConnRelNotAllow: MM STATUS is WAIT_FOR_NETWORK_COMMAND");
    ...
    if (MM_STATUS_ATTACHED == g_MmSubLyrShare.MmShare.ucCsAttachState)
    {
        if (MM_ATT_NOT_NEED == g_MmGlobalInfo.ucAttFlg)
        {
            /* 切换到无服务状态 */
            NAS_MM_SetMmServiceState(MM_NO_SERVICE);
            NAS_INFO_LOG(WUEPS_PID_MM, "NAS_MM_RcvMmcPowerOffReq_RrConnRelNotAllow: MmServiceState is MM_NO_SERVICE");
            ...
            /* 记录迁移之前的状态，并迁移到IMSI_DETACH_INITIATED状态 */
            NAS_MM_SetMmPreState(NAS_MM_GetMmState());
            Mm_ComSetMmState(IMSI_DETACH_INITIATED);
            NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_RcvMmcDetachReq_RrConnRelNotAllow: MM STATUS is IMSI_DETACH_INITIATED");
        }
...
VOS_VOID NAS_MM_RcvCcEstReq_RrConnRelNotAllow(VOS_VOID* pMsg)
{
    MM_MSG_CM_SVC_REQ_STRU              stCmSvcReq;

    PS_MEM_SET(&stCmSvcReq, 0x00, sizeof(stCmSvcReq));

    /* 消息检查失败，直接返回 */
    if (MM_TRUE != Mm_RcvCcEstReq(pMsg))
    {
        return;
    }
    ...
    if (MMCC_EMERGENCY_CALL == g_MmCcEstReq.ulCallType)
    {
        Mm_ComFillInCMSncReq(MM_IE_CM_SVC_TYPE_EMG_CALL_EST,
            MM_FALSE, &stCmSvcReq);                                             /* 填充CM SERVICE REQUEST结构体             */
    }
    else
    {
        /* 非紧急呼,当前小区CS域正常业务受限，则回复CC建链失败 */
        if (VOS_TRUE == NAS_MML_GetCsRestrictNormalServiceFlg())
        {
            Mm_SndCcRelInd(g_MmCcEstReq.ulTransactionId, NAS_MMCM_REL_CAUSE_MM_INTER_ERR_CS_ACCESS_BAR);
            return;
        ...
        /* 当前处于受限服务状态，则回复CC建链失败 */
        if (MM_TRUE == Mm_ComJudgeLimitSvc())
        {
            Mm_SndCcRelInd(g_MmCcEstReq.ulTransactionId, NAS_MMCM_REL_CAUSE_MM_INTER_ERR_UE_INVALID_STATE);
            return;
        }
    ...
    if (MM_TIMER_RUNNING == NAS_MM_QryTimerStatus(MM_TIMER_PROTECT_MT_CSFB_PAGING_PROCEDURE))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_RcvCcEstReq_RrConnRelNotAllow: CSFB MT exists, snd MM REL IND to CC to start redial proc");
    ...
    /* 发起MM链接请求 */
    Mm_ComMsgCmSvcReqSnd(&stCmSvcReq);                                          /* 向网侧发送建立MM连接的请求               */
    NAS_MM_UpdateConnCtrlInfo(MM_CONN_CTRL_CC);
...
VOS_VOID NAS_MM_RcvRrRelInd_RrConnRelNotAllow(VOS_VOID* pMsg)
{
    /* 停止T3241定时器 */
    Mm_TimerStop(MM_TIMER_T3241);
    ...
    Mm_ComSetMmState(WAIT_FOR_NETWORK_COMMAND);
    NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_RcvRrRelInd_RrConnRelNotAllow: MM STATUS is WAIT_FOR_NETWORK_COMMAND");

    /* 启动T3240定时器 */
    Mm_TimerStart(MM_TIMER_T3240);

    /* 调用WAIT FOR NETWORK COMMAND状态的处理 */
    Mm_Cell_S11_E33(pMsg);

    return;
}


VOS_VOID NAS_MM_RcvRrDataIndMmInfo_RrConnRelNotAllow(VOS_VOID* pMsg)
{
    if ( MM_LIMITED_SERVICE != g_MmGlobalInfo.ucMmServiceState )
    {
        Mm_TimerStop(MM_TIMER_T3212);                                         /* 停T3212                                  */
...
VOS_VOID NAS_MM_RcvMmcRelReq_RrConnRelNotAllow(VOS_VOID* pMsg)
{
    /* 停止T3241定时器 */
    Mm_TimerStop(MM_TIMER_T3241);
    ...
    /* 迁移状态到WAIT FOR NETWORK COMMAND */
    Mm_ComSetMmState(WAIT_FOR_NETWORK_COMMAND);
    NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_RcvMmcRelReq_RrConnRelNotAllow: MM STATUS is WAIT_FOR_NETWORK_COMMAND");

    /* 启动T3240定时器 */
    Mm_TimerStart(MM_TIMER_T3240);

    /* 调用WAIT FOR NETWORK COMMAND状态的处理 */
    Mm_Cell_S11_E49(pMsg);

    return;
}


VOS_VOID NAS_MM_RcvRrDataIndCc_RrConnRelNotAllow(VOS_VOID* pMsg)
{
    ...
    /* 向CC发送MMCC_EST_IND */
    Mm_SndCcEstInd(ucTi, pstRrDataInd->RcvNasMsg.ulNasMsgSize, pucCmMsg);

    /* 设置已经存在的MM连接的标志位 */
    NAS_MM_UpdateMmConnByTi(MM_CONN_CTRL_CC, ucTi, NAS_MM_CONN_EXIST);
    ...
    NAS_MM_SetMmPreState(NAS_MM_GetMmState());
    Mm_ComSetMmState(MM_CONNECTION_ACTIVE);
    NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_RcvRrDataIndCc_RrConnRelNotAllow: MM STATUS is MM_CONNECTION_ACTIVE");
...
VOS_VOID NAS_MM_RcvSmsEstReq_RrConnRelNotAllow(VOS_VOID* pMsg)
{
    MM_MSG_CM_SVC_REQ_STRU              stCmSvcReq;
    ...
    {
        /* Ti不合法，通知SMS链接释放 */
        Mm_SndSmsRelInd(g_MmSmsEstReq.ulTi, NAS_MMCM_REL_CAUSE_MM_INTER_ERR_TI_INVALID);
        return;
    }
    ...
    /* 记录正在建立的MM连接的PD */
    NAS_MM_SetMMConnEstingPD(MM_IE_PD_SMS_MSG);

    Mm_TimerStop(MM_TIMER_T3241);
    /* 迁移到WAIT FOR OUTGING MM CONNECTION状态，启动T3230定时器 */
    Mm_ComSetMmState(WAIT_FOR_OUTGOING_MM_CONNECTION);
    NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_RcvSmsEstReq_RrConnRelNotAllow: MM STATUS is WAIT_FOR_OUTGOING_MM_CONNECTION");
...
VOS_VOID NAS_MM_RcvSsEstReq_RrConnRelNotAllow(VOS_VOID* pMsg)
{
    MM_MSG_CM_SVC_REQ_STRU              stCmSvcReq;
    ...
    /* 记录正在建立的MM连接的PD */
    NAS_MM_SetMMConnEstingPD(MM_IE_PD_NON_CALL_RLT_SS_MSG);

    Mm_TimerStop(MM_TIMER_T3241);
    /* 迁移到WAIT FOR OUTGING MM CONNECTION状态，启动T3230定时器 */
    Mm_ComSetMmState(WAIT_FOR_OUTGOING_MM_CONNECTION);
    NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_RcvSsEstReq_RrConnRelNotAllow: MM STATUS is WAIT_FOR_OUTGOING_MM_CONNECTION");
    Mm_TimerStart(MM_TIMER_T3230);
...
VOS_VOID NAS_MM_RcvRrDataIndSs_RrConnRelNotAllow(VOS_VOID* pMsg)
{
    RRMM_DATA_IND_STRU      *pstRrDataInd   = VOS_NULL_PTR;
    VOS_UINT8               *pucCmMsg       = VOS_NULL_PTR;
    VOS_UINT8                ucTi;
    ...
    {
        NAS_WARNING_LOG(WUEPS_PID_MM, "NAS_MM_RcvRrDataIndSs_RrConnRelNotAllow: Invalid Transaction Id!");
        return;
    }
    ...
    /* 向SS发送MMSS_EST_IND */
    Mm_SndSsEstInd(ucTi, pstRrDataInd->RcvNasMsg.ulNasMsgSize, pucCmMsg);

    /* 设置已经存在的MM连接的标志位 */
    NAS_MM_UpdateMmConnByTi(MM_CONN_CTRL_SS, ucTi, NAS_MM_CONN_EXIST);
    ...
    NAS_MM_SetMmPreState(NAS_MM_GetMmState());
    Mm_ComSetMmState(MM_CONNECTION_ACTIVE);
    NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_RcvRrDataIndSs_RrConnRelNotAllow: MM STATUS is MM_CONNECTION_ACTIVE");
...
VOS_VOID NAS_MM_RcvRrDataIndSms_RrConnRelNotAllow(VOS_VOID* pMsg)
{
    RRMM_DATA_IND_STRU      *pstRrDataInd   = VOS_NULL_PTR;
    VOS_UINT8               *pucCmMsg       = VOS_NULL_PTR;
    VOS_UINT8                ucTi;
    ...
    if ( (MM_CONST_NUM_7 == ucTi)
      || (MM_CONST_NUM_15 <= ucTi) )
    {
        NAS_WARNING_LOG(WUEPS_PID_MM, "NAS_MM_RcvRrDataIndSms_RrConnRelNotAllow: Invalid Transaction Id!");
        return;
    }
    ...
    /* 向SS发送MMSMS_EST_IND */
    Mm_SndSmsEstInd(ucTi, pstRrDataInd->RcvNasMsg.ulNasMsgSize, pucCmMsg);

    /* 设置已经存在的MM连接的标志位 */
    NAS_MM_UpdateMmConnByTi(MM_CONN_CTRL_SMS, ucTi, NAS_MM_CONN_EXIST);
    ...
    NAS_MM_SetMmPreState(NAS_MM_GetMmState());
    Mm_ComSetMmState(MM_CONNECTION_ACTIVE);
    NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_RcvRrDataIndSms_RrConnRelNotAllow: MM STATUS is MM_CONNECTION_ACTIVE");
