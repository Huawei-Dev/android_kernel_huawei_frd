...
VOS_UINT32  NAS_MMC_IsExistCachedUserSearchMsg( VOS_VOID )
{
    ...
    ulExistFlg = NAS_MMC_IsExistCacheMsg(NAS_BuildEventType(UEPS_PID_MSCC, ID_MSCC_MMC_CSG_SPEC_SEARCH_REQ),
                                         &ulCacheMsgIdx);
    if ( VOS_TRUE == ulExistFlg )
    {
        return VOS_TRUE;
...
#if (FEATURE_ON == FEATURE_CSG)
    NAS_MMC_InitCsgBgSearchCtrlCtx(&(pstMmcCtx->stCsgBgSearchCtrl));
#endif
...
)
{
    ...
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_MsgProc: Receive MSG", pstMsgHeader->ulMsgName);
