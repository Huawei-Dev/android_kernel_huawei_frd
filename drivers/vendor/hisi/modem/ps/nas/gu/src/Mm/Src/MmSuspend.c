...
)
{
    ...
    /* MO CSFB流程中在L下恢复，认为CSFB失败，向CC回复建链失败，清缓存 */
    ...
    /* MO CSFB流程中在L下恢复，认为CSFB失败，向SS回复建链失败，清缓存 */
    ...
    /* MO CSFB流程中在L下恢复，认为CSFB失败，向LCS回复建链失败，清缓存 */
    if (NAS_MML_CSFB_SERVICE_STATUS_MO_NORMAL_LCS_EXIST == enCsfbServiceStatus)
    {
        if (MM_TRUE == g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].RcvXXEstReq.ucFlg)
        {
            NAS_MM_SndLcsRelInd(g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].RcvXXEstReq.ulTransactionId, enRelCause);
            NAS_MM_ClearConnCtrlInfo(MM_IE_PD_LOCATION_SRVC);
        }
...
{
    VOS_UINT8                           aucTimerId[2];

    if (VOS_FALSE == NAS_MML_IsPlmnSupportDam(NAS_MML_GetCurrCampPlmnId()))
    {
        if (NAS_MML_TIN_TYPE_RAT_RELATED_TMSI == NAS_MML_GetTinType())
        {
            /* ISR激活，停止除了CSFB MT保护定时器和T3212以外的其他定时器 */
            NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_ProcResumeIndBackToLTE: ISR active, Stop other MM timer except T3212 and CSFB MT PAGING timer");
            aucTimerId[0] = MM_TIMER_PROTECT_MT_CSFB_PAGING_PROCEDURE;
            aucTimerId[1] = MM_TIMER_T3212;
            NAS_MM_TimerStopExceptSpecialTimers(2, aucTimerId);
        }
        else
        {
 ...
 */
    ...
    else
    {
        if ( ((MMC_SUSPEND_CAUSE_PLMNSEARCH == g_MmGlobalInfo.stSuspendShare.ucSuspendCause)
          ...
          || ((MMC_SUSPEND_CAUSE_REDIRECTION == g_MmGlobalInfo.stSuspendShare.ucSuspendCause)
           && (NAS_MML_NET_RAT_TYPE_LTE == g_MmGlobalInfo.stSuspendShare.enDestSuspendRat)) )
        {
            /* oos流程比较长如果是紧急呼叫需要打断oos流程，如果FR回LTE过程中用户发起主叫也可能要打断异系统流程 */
            /*缓存该服务请求 */
