...
)
{
    ...
    if (VOS_TRUE == NAS_MMC_GetAbortFlag_InterSysCellResel())
    {
        ...
        {
            /* MMCC_EST_REQ与RRMM_SUSPEND_IND对冲场景，此时直接回复Suspend failure */
            /* 根据挂起的发起方，构造一条恢复消息 */
...
)
{
    ...
    /* 回复InterSysCellResel执行结果，现阶段只有重定向流程才可以收到 suspend rel cnf消息 */
    NAS_MMC_SndSuspendRsltMsg(MMC_SUSPEND_CAUSE_REDIRECTION, NAS_MMC_SUSPEND_ABORTED, NAS_MMC_SUSPEND_FAIL_CAUSE_BUTT, VOS_NULL_PTR);
