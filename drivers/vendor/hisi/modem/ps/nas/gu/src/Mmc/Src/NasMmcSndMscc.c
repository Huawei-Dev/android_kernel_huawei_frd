...
VOS_VOID NAS_MMC_SndMsccUserCsgSpecSearchCnf(
    NAS_MSCC_PIF_USER_PLMN_SEARCH_RESULT_ENUM_UINT8         enResult
)
{
    MMC_MSCC_CSG_SPEC_SEARCH_CNF_STRU                      *pstMsg = VOS_NULL_PTR;

    pstMsg = (MMC_MSCC_CSG_SPEC_SEARCH_CNF_STRU *)PS_ALLOC_MSG(WUEPS_PID_MMC,
                              sizeof(MMC_MSCC_CSG_SPEC_SEARCH_CNF_STRU) - VOS_MSG_HEAD_LENGTH);

    if( VOS_NULL_PTR == pstMsg )
    {
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_SndMsccUserCsgSpecSearchCnf():ERROR:Memory Alloc Error ");
        return;
    }

    /* 填充消息头 */
    PS_MEM_SET(((VOS_UINT8*)pstMsg) + VOS_MSG_HEAD_LENGTH, 0, sizeof(MMC_MSCC_CSG_SPEC_SEARCH_CNF_STRU) - VOS_MSG_HEAD_LENGTH);

    pstMsg->stMsgHeader.ulMsgName         = ID_MMC_MSCC_CSG_SPEC_SEARCH_CNF;
    pstMsg->stMsgHeader.ulSenderCpuId     = VOS_LOCAL_CPUID;
    pstMsg->stMsgHeader.ulSenderPid       = WUEPS_PID_MMC;
    pstMsg->stMsgHeader.ulReceiverCpuId   = VOS_LOCAL_CPUID;
    pstMsg->stMsgHeader.ulReceiverPid     = UEPS_PID_MSCC;
    pstMsg->stMsgHeader.ulLength          = sizeof(MMC_MSCC_CSG_SPEC_SEARCH_CNF_STRU) - VOS_MSG_HEAD_LENGTH;
...
VOS_VOID NAS_MMC_SndMsccUserCsgSpecSearchAbortCnf(VOS_VOID)
{
    MMC_MSCC_CSG_SPEC_SEARCH_ABORT_CNF_STRU              *pstMsg = VOS_NULL_PTR;

    pstMsg = (MMC_MSCC_CSG_SPEC_SEARCH_ABORT_CNF_STRU *)PS_ALLOC_MSG(WUEPS_PID_MMC,
                              sizeof(MMC_MSCC_CSG_SPEC_SEARCH_ABORT_CNF_STRU) - VOS_MSG_HEAD_LENGTH);

    if( VOS_NULL_PTR == pstMsg )
    {
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_SndMsccUserCsgSpecSearchAbortCnf():ERROR:Memory Alloc Error for pstPlmnListAbortCnf ");
        return;
    }

    /* 填充消息头 */
    PS_MEM_SET(((VOS_UINT8*)pstMsg) + VOS_MSG_HEAD_LENGTH, 0, sizeof(MMC_MSCC_CSG_SPEC_SEARCH_ABORT_CNF_STRU) - VOS_MSG_HEAD_LENGTH);

    pstMsg->stMsgHeader.ulMsgName         = ID_MMC_MSCC_CSG_SPEC_SEARCH_ABORT_CNF;
    pstMsg->stMsgHeader.ulSenderCpuId     = VOS_LOCAL_CPUID;
    pstMsg->stMsgHeader.ulSenderPid       = WUEPS_PID_MMC;
    pstMsg->stMsgHeader.ulReceiverCpuId   = VOS_LOCAL_CPUID;
    pstMsg->stMsgHeader.ulReceiverPid     = UEPS_PID_MSCC;
    pstMsg->stMsgHeader.ulLength          = sizeof(MMC_MSCC_CSG_SPEC_SEARCH_ABORT_CNF_STRU) - VOS_MSG_HEAD_LENGTH;

    PS_SEND_MSG(WUEPS_PID_MMC, pstMsg);
    return;
}

VOS_VOID NAS_MMC_SndMsccMsgCsgIdHomeNodeBNameInd(
    MMC_MSCC_CSG_ID_HOME_NODEB_NAME_IND_STRU               *pstCsgCellInfo
)
{
    MMC_MSCC_CSG_ID_HOME_NODEB_NAME_IND_STRU               *pstMsg = VOS_NULL_PTR;

    pstMsg = (MMC_MSCC_CSG_ID_HOME_NODEB_NAME_IND_STRU *)PS_ALLOC_MSG(WUEPS_PID_MMC,
                              sizeof(MMC_MSCC_CSG_ID_HOME_NODEB_NAME_IND_STRU) - VOS_MSG_HEAD_LENGTH);
    ...
    /* 填充消息头 */
    PS_MEM_SET(((VOS_UINT8*)pstMsg) + VOS_MSG_HEAD_LENGTH, 0, sizeof(MMC_MSCC_CSG_ID_HOME_NODEB_NAME_IND_STRU) - VOS_MSG_HEAD_LENGTH);
    pstMsg->stMsgHeader.ulMsgName         = ID_MMC_MSCC_CSG_ID_HOME_NODEB_NAME_IND;
    pstMsg->stMsgHeader.ulSenderCpuId     = VOS_LOCAL_CPUID;
    pstMsg->stMsgHeader.ulSenderPid       = WUEPS_PID_MMC;
    pstMsg->stMsgHeader.ulReceiverCpuId   = VOS_LOCAL_CPUID;
    pstMsg->stMsgHeader.ulReceiverPid     = UEPS_PID_MSCC;
    pstMsg->stMsgHeader.ulLength          = sizeof(MMC_MSCC_CSG_ID_HOME_NODEB_NAME_IND_STRU) - VOS_MSG_HEAD_LENGTH;

    /* 若传进来的消息为空指针则表示列表为空 */
    if (VOS_NULL_PTR == pstCsgCellInfo)
    {
        pstMsg->ucHomeNodeBNameLen = 0;

        PS_SEND_MSG(WUEPS_PID_MMC, pstMsg);
        return;
    }

    pstMsg->stPlmnId.ulMcc     = pstCsgCellInfo->stPlmnId.ulMcc;
    pstMsg->stPlmnId.ulMnc     = pstCsgCellInfo->stPlmnId.ulMnc;
    pstMsg->ulCsgId            = pstCsgCellInfo->ulCsgId;
    pstMsg->ucHomeNodeBNameLen = pstCsgCellInfo->ucHomeNodeBNameLen;
    PS_MEM_CPY(pstMsg->aucHomeNodeBName,
               pstCsgCellInfo->aucHomeNodeBName,
               NAS_MML_MIN(pstCsgCellInfo->ucHomeNodeBNameLen, MMC_LMM_MAX_HOME_NODEB_NAME_LEN));
...
#if (FEATURE_ON == FEATURE_CSG)
        pstSysInfoInd->stCellId.aucCellType[i] = pstCampPlmnInfo->stCampCellInfo.astCellInfo[i].enCellType;
#endif
    }
...
#if (FEATURE_ON == FEATURE_CSG)
    pstSysInfoInd->ulCsgId            = pstCampPlmnInfo->ulCsgId;
    pstSysInfoInd->ucHomeNodeBNameLen = pstCampPlmnInfo->stCsgIdHomeNodeBName.ucHomeNodeBNameLen;

    PS_MEM_CPY(pstSysInfoInd->aucHomeNodeBName,
        pstCampPlmnInfo->stCsgIdHomeNodeBName.aucHomeNodeBName,
        pstCampPlmnInfo->stCsgIdHomeNodeBName.ucHomeNodeBNameLen);
 #endif
    ...
    )
{
    ...
    pstSndMsg->stLteRssi.stRxAntInfo.ucRxANTNum  = pstRssiValueInfo->stRxAntInfo.ucRxANTNum;
    pstSndMsg->stLteRssi.stRxAntInfo.asRsrpRx[0] = pstRssiValueInfo->stRxAntInfo.asRsrpRx[0];
    pstSndMsg->stLteRssi.stRxAntInfo.asRsrpRx[1] = pstRssiValueInfo->stRxAntInfo.asRsrpRx[1];
    pstSndMsg->stLteRssi.stRxAntInfo.asRsrpRx[2] = pstRssiValueInfo->stRxAntInfo.asRsrpRx[2];
    pstSndMsg->stLteRssi.stRxAntInfo.asRsrpRx[3] = pstRssiValueInfo->stRxAntInfo.asRsrpRx[3];
    pstSndMsg->stLteRssi.stRxAntInfo.alSINRRx[0] = pstRssiValueInfo->stRxAntInfo.alSINRRx[0];
    pstSndMsg->stLteRssi.stRxAntInfo.alSINRRx[1] = pstRssiValueInfo->stRxAntInfo.alSINRRx[1];
    pstSndMsg->stLteRssi.stRxAntInfo.alSINRRx[2] = pstRssiValueInfo->stRxAntInfo.alSINRRx[2];
    pstSndMsg->stLteRssi.stRxAntInfo.alSINRRx[3] = pstRssiValueInfo->stRxAntInfo.alSINRRx[3];
...
VOS_VOID NAS_MMC_SndMsccBgSearchCnf(
    MMC_LMM_BG_SRCH_HRPD_RLT_ENUM_UINT32                    enRlst,
...
)
{
    ...
    switch (enRlst)
    {
        case MMC_LMM_BG_SRCH_HRPD_RLT_SUCCESS:
            pstMsg->enRslt = MSCC_MMC_BG_SRCH_RESULT_SUCCESS;
            break;
        case MMC_LMM_BG_SRCH_HRPD_RLT_FAIL:
            pstMsg->enRslt = MSCC_MMC_BG_SRCH_RESULT_FAIL;
            break;
        case MMC_LMM_BG_SRCH_HRPD_RLT_ABORT:
...
)
{
    ...
    pstMsg->stMsgHeader.ulReceiverCpuId   = VOS_LOCAL_CPUID;
    pstMsg->stMsgHeader.ulReceiverPid     = UEPS_PID_MSCC;
    pstMsg->stMsgHeader.ulLength          = sizeof(MMC_MSCC_PS_REG_CN_RSLT_IND_STRU) - VOS_MSG_HEAD_LENGTH;
