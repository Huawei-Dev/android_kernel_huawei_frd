...
VOS_VOID NAS_MMC_SndLmmPlmnListReq(
MMC_LMM_PLMN_SRCH_TYPE_ENUM_UINT32  enPlmnSearchScene
)
{
    /* 定义原语类型指针 */
    MMC_LMM_PLMN_SRCH_REQ_STRU         *pstMsg = VOS_NULL_PTR;
    ...
    pstMsg->ulMsgId                         = ID_MMC_LMM_PLMN_SRCH_REQ;
    pstMsg->ulOpId                          = 0;
    pstMsg->enSrchType                      = enPlmnSearchScene;
...
)
{
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_UTRANCTRL_ConvertNasUtranModeToLmmUtranMode(): CL mode, enUtranMode set to MMC_LMM_UTRAN_MODE_NULL");
...
)
{
    ...
    /* 和leixiantiao确认:normal驻留时，因为4G未开户被禁的PLMN返回允许注册；anycell驻留时4G未开户被禁的PLMN仍然返回不允许注册 */
    if (VOS_TRUE == NAS_IsPlmnIdWithRatInDestDisabledPlmnWithForbiddenPeriodList(ucAsAnyCampOn, pstPlmn))
    {
        return MMC_LMM_PLMN_NOT_ALLOW_REG;
    }
...
VOS_UINT32 NAS_IsPlmnIdWithRatInDestDisabledPlmnWithForbiddenPeriodList(
    VOS_UINT8                           ucAsAnyCampOn,
    MMC_LMM_PLMN_ID_STRU                *pstLmmPlmn
)
{
    ...
    /* 将PLMN ID进行转换 */
    NAS_MEM_SET_S(&stPlmnWithRat, sizeof(NAS_MML_PLMN_WITH_RAT_STRU), 0, sizeof(stPlmnWithRat));
    NAS_MMC_ConvertLmmPlmnToGUNasFormat(pstLmmPlmn, &(stPlmnWithRat.stPlmnId));
    stPlmnWithRat.enRat             = NAS_MML_NET_RAT_TYPE_LTE;
    ...
    for ( i = 0; i < ulIndex; i++ )
    {
        if ((VOS_TRUE               == NAS_MML_CompareBcchPlmnwithSimPlmn(&(stPlmnWithRat.stPlmnId), &(pstDisabledPlmnInfo->astDisabledRatPlmnId[i].stPlmnWithRat.stPlmnId)))
         && (stPlmnWithRat.enRat    == pstDisabledPlmnInfo->astDisabledRatPlmnId[i].stPlmnWithRat.enRat))
        {
            /* normal驻留时，因为4G未开户导致的被禁，允许LTE注册 */
            if ( (VOS_FALSE == ucAsAnyCampOn)
              && (MMC_LMM_DISABLE_LTE_REASON_LMM_NOTIFY_EUTRAN_NOT_ALLOW == pstDisabledPlmnInfo->astDisabledRatPlmnId[i].enDisableLteReason))
            {
                NAS_NORMAL_LOG3(WUEPS_PID_MMC, "NAS_IsPlmnIdWithRatInDestDisabledPlmnWithForbiddenPeriodList:Disable reason is eutran not allow, return FALSE,MCC,MNC,RAT",stPlmnWithRat.stPlmnId.ulMcc,stPlmnWithRat.stPlmnId.ulMnc,stPlmnWithRat.enRat);
                return VOS_FALSE;
            }

            NAS_NORMAL_LOG3(WUEPS_PID_MMC, "NAS_IsPlmnIdWithRatInDestDisabledPlmnWithForbiddenPeriodList IS TRUE,MCC,MNC,RAT",stPlmnWithRat.stPlmnId.ulMcc,stPlmnWithRat.stPlmnId.ulMnc,stPlmnWithRat.enRat);

            return VOS_TRUE;
        }
    }

    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_IsPlmnIdWithRatInDestDisabledPlmnWithForbiddenPeriodList: not in DisabledPlmnWithForbiddenPeriodList, return VOS_FALSE");
...
VOS_VOID  NAS_MMC_ConvertLmmPlmnWithCsgIdListToLmm(
    VOS_UINT32                          ulPlmnWithCsgIdNum,
    NAS_MML_PLMN_WITH_CSG_ID_STRU      *pstPlmnWithCsgIdList,
    MMC_LMM_PLMN_WITH_CSG_ID_STRU      *pstMmcLmmPlmnWithCsgIdList

)
{
    VOS_UINT32                          i;

    for ( i = 0 ; i <  NAS_MML_MIN(ulPlmnWithCsgIdNum, NAS_MML_MAX_PLMN_CSG_ID_NUM); i++ )
    {
        NAS_MML_ConvertNasPlmnToLMMFormat(&(pstMmcLmmPlmnWithCsgIdList[i].stPlmnId),
                                          &(pstPlmnWithCsgIdList[i].stPlmnId));

        pstMmcLmmPlmnWithCsgIdList[i].ulCsgId = pstPlmnWithCsgIdList[i].ulCsgId;
    }
...
VOS_VOID NAS_MMC_SndLmmCsgSpecSearchReq(
    NAS_MML_PLMN_LIST_WITH_RAT_STRU    *pstDestPlmnList,
    NAS_MML_PLMN_CSG_ID_WITH_RAT_STRU  *pstPlmnWithCsgId
)
{
    /* 定义原语类型指针 */
    MMC_LMM_PLMN_SRCH_REQ_STRU         *pstMsg = VOS_NULL_PTR;
    ...
    if ( VOS_NULL_PTR == pstMsg )
    {
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_SndLmmCsgSpecSearchReq():WARNING:PS_ALLOC_MSG FAIL");
        return;
    }
    ...
    /* 填写消息内容 */
    pstMsg->ulOpId                          = 0;
    pstMsg->enSrchType                      = MMC_LMM_PLMN_SRCH_USER_CSG_SPEC;
    pstMsg->bitOpSpecPlmn                   = VOS_TRUE;
    pstMsg->bitOpPlmnInfoCampFlg            = VOS_TRUE;
    pstMsg->enFftScanType                   = MMC_LMM_FFT_SCAN_TYPE_NONE;
    pstMsg->bitOpCsgIdListInfo              = VOS_TRUE;

    pstMsg->stPlmnWithCsgIdList.ulPlmnWithCsgIdListNum = 1;
    NAS_MML_ConvertNasPlmnToLMMFormat(&pstMsg->stPlmnWithCsgIdList.astPlmnWithCsgIdInfo[0].stPlmnId,
                                      &pstPlmnWithCsgId->stPlmnWithRat.stPlmnId);
    pstMsg->stPlmnWithCsgIdList.astPlmnWithCsgIdInfo[0].ulCsgId = pstPlmnWithCsgId->ulCsgId;
    ...
    if (VOS_OK != PS_SEND_MSG( WUEPS_PID_MMC, pstMsg ))
    {
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_SndLmmCsgSpecSearchReq():WARNING:SEND LMM PLMN SEARCH REQ FAIL");
        return;
    }
#else
...
VOS_VOID NAS_MMC_BuildSndLmmCsgWhiteListInfo(
    MMC_LMM_PLMN_WITH_CSG_ID_LIST_STRU *pstPlmnWithCsgIdList
)
{
    VOS_UINT32                                              ulLoop;
    VOS_UINT8                                               ucSndCsgWhiteListNum;
    NAS_MML_PLMN_WITH_CSG_ID_STRU                           stPlmnWithCsgId;
    NAS_MML_PLMN_WITH_CSG_ID_STRU                           astPlmnWithCsgIdInfo[NAS_MML_MAX_PLMN_CSG_ID_NUM];
    ...
    ucSndCsgWhiteListNum = 0;
    PS_MEM_SET(astPlmnWithCsgIdInfo, 0, sizeof(astPlmnWithCsgIdInfo));
    ...
    if ((0 == pstOperatorCsgList->ulPlmnWithCsgIdListNum)
      &&(0 != pstUeBasedOperatorCsgList->ulPlmnWithCsgIdListNum))
    {
        pstOperatorCsgList = NAS_MML_GetUeBasedOperatorCsgList();
    }

    /* 如果当前驻留的网络与用户csg指定搜网的CSG ID相同，且当前驻留混合小区，
       且该混合小区CSG ID不在CSG白名单中，需要加入CSG白名单并通知接入层*/
    pstCsgSpecSrchPlmnWithCsgId       = NAS_MMC_GetCsgSpecSrchPlmnWithCsgId();

    if (VOS_TRUE == NAS_MMC_IsCampOnUserSpecHybridCsgId())
    {
        /* 肯定不在csg白名单中，直接加入放在最前面 */
        astPlmnWithCsgIdInfo[ucSndCsgWhiteListNum].stPlmnId = pstCsgSpecSrchPlmnWithCsgId->stPlmnWithRat.stPlmnId;
        astPlmnWithCsgIdInfo[ucSndCsgWhiteListNum].ulCsgId  = pstCsgSpecSrchPlmnWithCsgId->ulCsgId;

        ucSndCsgWhiteListNum++;
    }

    /* 加入Allowed CSG List */
    ulLoopMax = NAS_MML_MIN(pstAllowedCsgList->ulPlmnWithCsgIdListNum, NAS_MML_MAX_PLMN_CSG_ID_NUM);

    for (ulLoop = 0; ulLoop < ulLoopMax; ulLoop++)
    {
        stPlmnWithCsgId = pstAllowedCsgList->astPlmnWithCsgIdInfo[ulLoop].stPlmnWithCsgId;

        /* 如果构造的csg白名单列表达到最大个数则后续不再添加 */
        if (ucSndCsgWhiteListNum >= NAS_MML_MAX_PLMN_CSG_ID_NUM)
        {
            pstPlmnWithCsgIdList->ulPlmnWithCsgIdListNum = NAS_MML_MAX_PLMN_CSG_ID_NUM;
            NAS_MMC_ConvertLmmPlmnWithCsgIdListToLmm(pstPlmnWithCsgIdList->ulPlmnWithCsgIdListNum, astPlmnWithCsgIdInfo, pstPlmnWithCsgIdList->astPlmnWithCsgIdInfo);
            return;
        }

        /* 已经在构造的csg 白名单列表中则无需重复添加，跳过 */
        if (VOS_TRUE == NAS_MML_IsPlmnWithCsgIdInDestPlmnWithCsgIdList(&stPlmnWithCsgId,
                                           ucSndCsgWhiteListNum, astPlmnWithCsgIdInfo))
        {
            continue;
        }

        astPlmnWithCsgIdInfo[ucSndCsgWhiteListNum].stPlmnId = stPlmnWithCsgId.stPlmnId;
        astPlmnWithCsgIdInfo[ucSndCsgWhiteListNum].ulCsgId  = stPlmnWithCsgId.ulCsgId;
    ...
    pstPlmnWithCsgIdList->ulPlmnWithCsgIdListNum = ucSndCsgWhiteListNum;
    NAS_MMC_ConvertLmmPlmnWithCsgIdListToLmm(pstPlmnWithCsgIdList->ulPlmnWithCsgIdListNum, astPlmnWithCsgIdInfo, pstPlmnWithCsgIdList->astPlmnWithCsgIdInfo);
...
VOS_VOID NAS_MMC_SndLmmCsgWhiteListNotify(
    MMC_LMM_PLMN_WITH_CSG_ID_LIST_STRU *pstPlmnWithCsgIdList
)
{
    /* 定义原语类型指针 */
    MMC_LMM_CSG_WHITE_LIST_NOTIFY_STRU *pstLmmCsgWhiteListMsg  = VOS_NULL_PTR;

#if (defined (NAS_STUB) || defined(__PS_WIN32_RECUR__))
    /* 申请内存  */
    pstLmmCsgWhiteListMsg = (MMC_LMM_CSG_WHITE_LIST_NOTIFY_STRU *)PS_ALLOC_MSG(WUEPS_PID_MMC,
                                               sizeof(MMC_LMM_CSG_WHITE_LIST_NOTIFY_STRU) - VOS_MSG_HEAD_LENGTH);
#else
    pstLmmCsgWhiteListMsg = (MMC_LMM_CSG_WHITE_LIST_NOTIFY_STRU *)NAS_MML_GetIntMsgSendBuf(sizeof(MMC_LMM_CSG_WHITE_LIST_NOTIFY_STRU));
#endif

    if( VOS_NULL_PTR == pstLmmCsgWhiteListMsg )
    {
        /* 内存申请失败 */
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SndLmmCsgWhiteListNotify,ERRORALLOC MEM FAIL");
        return ;
    }

    PS_MEM_SET( (VOS_INT8*)pstLmmCsgWhiteListMsg + VOS_MSG_HEAD_LENGTH, 0X0,
                sizeof(MMC_LMM_CSG_WHITE_LIST_NOTIFY_STRU) - VOS_MSG_HEAD_LENGTH );

    pstLmmCsgWhiteListMsg->ulReceiverCpuId                 = VOS_LOCAL_CPUID;
    pstLmmCsgWhiteListMsg->ulSenderCpuId                   = VOS_LOCAL_CPUID;
    pstLmmCsgWhiteListMsg->ulSenderPid                     = WUEPS_PID_MMC;
    pstLmmCsgWhiteListMsg->ulReceiverPid                   = PS_PID_MM;
    pstLmmCsgWhiteListMsg->ulLength                        = sizeof(MMC_LMM_CSG_WHITE_LIST_NOTIFY_STRU) - VOS_MSG_HEAD_LENGTH;
    pstLmmCsgWhiteListMsg->ulMsgId                         = ID_MMC_LMM_CSG_WHITE_LIST_NOTIFY;
    pstLmmCsgWhiteListMsg->ulOpId                          = 0;

    PS_MEM_CPY(&pstLmmCsgWhiteListMsg->stPlmnWithCsgIdList, pstPlmnWithCsgIdList, sizeof(pstLmmCsgWhiteListMsg->stPlmnWithCsgIdList));

#if (defined (NAS_STUB) || defined(__PS_WIN32_RECUR__))
    /* 调用VOS发送原语 */
    if( VOS_OK != PS_SEND_MSG( WUEPS_PID_MMC, pstLmmCsgWhiteListMsg ) )
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SndLmmCsgWhiteListNotify():ERROR:SEND MESSAGE FAILURE");
        return ;
    }
#else

    NAS_MML_LogMsgInfo((MSG_HEADER_STRU*)pstLmmCsgWhiteListMsg);

    /* 内部消息的发送 */
    NAS_MML_SndInternalMsg(pstLmmCsgWhiteListMsg);
#endif
...
VOS_VOID NAS_MMC_SndLmmCsgWhiteList(VOS_VOID)
{
    ...
    if (VOS_TRUE == NAS_MMC_IsNeedSndLmmCsgWhiteList())
    {
        NAS_MMC_BuildSndLmmCsgWhiteListInfo(&stPlmnWithCsgIdList);
...
)
{
    ...
    /* bg类型 */
    pstMsg->enBgType          = MMC_LMM_BG_PLMN_SRCH_SPEC;

    /* PLMN */
    NAS_MML_ConvertNasPlmnToLMMFormat(&stLmmPlmn, &pstDestPlmn->stPlmnId);
    pstMsg->ulPlmnIdNum = 1;
    PS_MEM_CPY(pstMsg->astPlmnIdList[0].aucPlmnId, stLmmPlmn.aucPlmnId, MMC_LMM_PLMN_ID_LEN);
    pstMsg->enRat = pstDestPlmn->enRat;
...
VOS_VOID NAS_MMC_SndLmmFastCsgSearchReq(
    NAS_MML_PLMN_ID_STRU               *pstDestPlmn,
...
)
{
    ...
    if ( VOS_NULL_PTR == pstMsg )
    {
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_SndLmmFastCsgSearchReq():WARNING:PS_ALLOC_MSG FIAL");
        return;
    }
    ...
    /* 快速指定的CSG的搜索 */
    pstMsg->enSrchType       = MMC_LMM_PLMN_SRCH_FAST_CSG_SPEC;
    ...
    pstMsg->stPlmnWithCsgIdList.ulPlmnWithCsgIdListNum = NAS_MML_MIN(ulCsgIdNum, MMC_LMM_MAX_PLMN_CSG_ID_NUM);
    for (i = 0; i < pstMsg->stPlmnWithCsgIdList.ulPlmnWithCsgIdListNum; i++)
    {
        pstMsg->stPlmnWithCsgIdList.astPlmnWithCsgIdInfo[i].stPlmnId =  pstMsg->stSpecPlmnId;
        pstMsg->stPlmnWithCsgIdList.astPlmnWithCsgIdInfo[i].ulCsgId =   pulCsgId[i];
    }
    ...
    if (VOS_OK != PS_SEND_MSG( WUEPS_PID_MMC, pstMsg ))
    {
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_SndLmmFastCsgSearchReq():WARNING:SEND LMM PLMN SEARCH REQ FIAL");
    }
#else
    NAS_MML_LogMsgInfo((MSG_HEADER_STRU*)pstMsg);
...
VOS_VOID NAS_MMC_SndLmmCsgBgSearchReq(
    NAS_MML_PLMN_WITH_RAT_STRU         *pstDestPlmn,
    VOS_UINT32                          ulCsgIdNum,
    VOS_UINT32                         *pulCsgId
)
{
    /* 定义原语类型指针 */
    MMC_LMM_CSG_BG_SEARCH_REQ_STRU     *pstMsg = VOS_NULL_PTR;
    MMC_LMM_PLMN_ID_STRU                stLmmPlmn;
...
#if (defined (NAS_STUB) || defined(__PS_WIN32_RECUR__))
    /* 申请内存  */
    pstMsg = (MMC_LMM_CSG_BG_SEARCH_REQ_STRU *)PS_ALLOC_MSG(WUEPS_PID_MMC,
                    sizeof(MMC_LMM_CSG_BG_SEARCH_REQ_STRU) - VOS_MSG_HEAD_LENGTH);
#else
    pstMsg = (MMC_LMM_CSG_BG_SEARCH_REQ_STRU *)NAS_MML_GetIntMsgSendBuf(sizeof(MMC_LMM_CSG_BG_SEARCH_REQ_STRU));
#endif
    ...
    pstMsg->ulSenderCpuId     = VOS_LOCAL_CPUID;
    pstMsg->ulSenderPid       = WUEPS_PID_MMC;
    pstMsg->ulLength          = sizeof(MMC_LMM_CSG_BG_SEARCH_REQ_STRU) - VOS_MSG_HEAD_LENGTH;
    pstMsg->ulMsgId           = ID_MMC_LMM_CSG_BG_SEARCH_REQ;
    pstMsg->ulOpId            = 0;

    NAS_MML_ConvertNasPlmnToLMMFormat(&stLmmPlmn, &pstDestPlmn->stPlmnId);
    PS_MEM_CPY(pstMsg->stPlmnWithRat.aucPlmnId, stLmmPlmn.aucPlmnId, MMC_LMM_PLMN_ID_LEN);
    pstMsg->stPlmnWithRat.enPlmnRat = pstDestPlmn->enRat;

    pstMsg->ulCsgIdNum = NAS_MML_MIN(ulCsgIdNum, MMC_LMM_MAX_PLMN_CSG_ID_NUM);
    PS_MEM_CPY(pstMsg->aulCsgId, pulCsgId, (pstMsg->ulCsgIdNum)*sizeof(VOS_UINT32));
...
VOS_VOID NAS_MMC_SndLmmBgHistoryPlmnSearchReq(
    NAS_MML_PLMN_LIST_WITH_RAT_STRU    *pstDestPlmnList
)
{
    /* 定义原语类型指针 */
    ...
    if ( VOS_NULL_PTR == pstMsg )
    {
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_SndLmmBgHistoryPlmnSearchReq():WARNING:PS_ALLOC_MSG FIAL");
        return;
    }
    ...
    /* bg类型 */
    pstMsg->enBgType          = MMC_LMM_BG_PLMN_SRCH_HISTORY;

    /* PLMN列表 */
    pstMsg->ulPlmnIdNum = NAS_MML_MIN(pstDestPlmnList->ulPlmnNum, MMC_LMM_MAX_EQUPLMN_NUM);
    pstMsg->enRat       = pstDestPlmnList->astPlmnId[0].enRat;
    for (i = 0; i < pstMsg->ulPlmnIdNum; i++)
    {
        NAS_MML_ConvertNasPlmnToLMMFormat(&stLmmPlmn, &pstDestPlmnList->astPlmnId[i].stPlmnId);
        PS_MEM_CPY(pstMsg->astPlmnIdList[i].aucPlmnId, stLmmPlmn.aucPlmnId, MMC_LMM_PLMN_ID_LEN);
    }


    /* forb Roam LA列表 */
    if (NAS_MML_NET_RAT_TYPE_LTE != pstDestPlmnList->astPlmnId[0].enRat)
    {
        /* L下背景搜GU的网络，需要将所有禁止漫游 LA 添加到ForbLa列表 */
        for ( i = 0; i < pstForbPlmnInfo->ucForbRoamLaNum; i++ )
        {
    ...
    if (VOS_OK != PS_SEND_MSG( WUEPS_PID_MMC, pstMsg ))
    {
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_SndLmmBgHistoryPlmnSearchReq():WARNING:SEND LMM BG PLMN SEARCH REQ FIAL");
    }
#else
