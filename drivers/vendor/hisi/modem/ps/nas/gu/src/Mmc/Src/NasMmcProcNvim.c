...
)
{
    ...
    */
    ...
    /* 可维可测，勾消息 */
    NAS_MMC_LogTinTypeInfo(&stTinInfo);
}
...
{
    NAS_MMC_NVIM_CAUSE18_ENABLE_LTE_SUPPORT_FLG_STRU stCause18EnableLteSupportFlg;

    PS_MEM_SET(&stCause18EnableLteSupportFlg, 0x00, sizeof(stCause18EnableLteSupportFlg));

    /* 读NV失败 */
    if (NV_OK != NV_Read(en_NV_Item_Cause18_Enable_Lte_Support_Flg,
                         &stCause18EnableLteSupportFlg, sizeof(NAS_MMC_NVIM_CAUSE18_ENABLE_LTE_SUPPORT_FLG_STRU)))
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC,
                      "NAS_MMC_ReadCause18EnableLteSupportFlgNvim(): en_NV_Item_Cause18_Enable_Lte_Support_Flg Error");
        return;
    }

    /*将NV内容赋值给全局变量*/
    if(VOS_TRUE == stCause18EnableLteSupportFlg.ucCause18EnableLteSupportFlg)
    {
        NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stLteCustomCfgInfo.ucCause18EnableLteSupportFlg = VOS_TRUE;
    }
    else
    {
        NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stLteCustomCfgInfo.ucCause18EnableLteSupportFlg = VOS_FALSE;
    }
...
VOS_VOID NAS_MMC_DeleteEPlmnList( VOS_VOID )
{
    ...
    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_DeleteEPlmnList: Delete EPLMN");
...
VOS_VOID NAS_MMC_ReadCustomSupplementOplmnInfoNvim( VOS_VOID )
{
    ...
    NAS_MML_SIM_FORMAT_PLMN_ID                              stSimPlmn;
    NAS_MML_PLMN_ID_STRU                                    stPlmn;
    NAS_MMC_NVIM_CUSTOM_SUPPLEMENT_OPLMN_INFO_STRU          stNvimSupplementOplmnInfo;
    NAS_MML_CUSTOM_SUPPLEMENT_OPLMN_INFO_STRU              *pstCustomSupplementOplmnInfo;

    PS_MEM_SET(&stNvimSupplementOplmnInfo, 0x00, sizeof(stNvimSupplementOplmnInfo));

    PS_MEM_SET(&stSimPlmn, 0x00, sizeof(stSimPlmn));

    pstCustomSupplementOplmnInfo = NAS_MML_GetCustomSupplementOplmnInfo();

    if ( NV_OK != NV_Read( en_NV_Item_Custom_Supplement_Oplmn_Info,
                            &stNvimSupplementOplmnInfo, sizeof(NAS_MMC_NVIM_CUSTOM_SUPPLEMENT_OPLMN_INFO_STRU)) )
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_ReadUserCfgExtOplmnInfoNvim:Read NV Failed");
        return;
    }

    if (stNvimSupplementOplmnInfo.ucOPlmnListNum > NAS_MML_MAX_CUSTOM_SUPPLEMENT_OPLMN_NUM )
    {
        stNvimSupplementOplmnInfo.ucOPlmnListNum  = NAS_MML_MAX_CUSTOM_SUPPLEMENT_OPLMN_NUM;
    }

    if (stNvimSupplementOplmnInfo.ucImsiPlmnListNum > NAS_MMC_NVIM_MAX_USER_CFG_IMSI_PLMN_NUM)
    {
        stNvimSupplementOplmnInfo.ucImsiPlmnListNum = NAS_MMC_NVIM_MAX_USER_CFG_IMSI_PLMN_NUM;
    }

    for ( i = 0; i < stNvimSupplementOplmnInfo.ucImsiPlmnListNum; i++ )
    {
        PS_MEM_CPY(stSimPlmn.aucSimPlmn, stNvimSupplementOplmnInfo.astImsiPlmnList[i].aucSimPlmn, NAS_MML_SIM_PLMN_ID_LEN);
        NAS_MMC_ConvertSimPlmnToNasPLMN(&stSimPlmn, &stPlmn);

        if (VOS_TRUE == NAS_MML_IsPlmnIdValid(&stPlmn))
        {
            pstCustomSupplementOplmnInfo->astImsiPlmnList[i]= stPlmn;
            pstCustomSupplementOplmnInfo->ucImsiPlmnListNum++;
        }
    }

    for ( i = 0; i < stNvimSupplementOplmnInfo.ucOPlmnListNum; i++ )
    {
        PS_MEM_CPY(stSimPlmn.aucSimPlmn, stNvimSupplementOplmnInfo.astOPlmnList[i].aucSimPlmn, NAS_MML_SIM_PLMN_ID_LEN);
        NAS_MMC_ConvertSimPlmnToNasPLMN(&stSimPlmn, &stPlmn);

        if (VOS_TRUE == NAS_MML_IsPlmnIdValid(&stPlmn))
        {
            pstCustomSupplementOplmnInfo->astOPlmnList[i]= stPlmn;
            pstCustomSupplementOplmnInfo->ucOplmnListNum++;
        }
    }
...
VOS_VOID NAS_MMC_UpdateLastImsi( VOS_VOID )
{
    ...
    if ( USIMM_API_SUCCESS != NAS_USIMMAPI_GetCardIMSI(aucLastImsi) )
    {
...
VOS_UINT32 NAS_MMC_IsNvimDuplicateRplmnAndEplmnListEqual(
    NAS_NVIM_CSG_DUPLICATED_RPLMN_AND_EPLMN_LIST_STRU      *pstNewDuplicateRplmnAndEplmnList,
    NAS_NVIM_CSG_DUPLICATED_RPLMN_AND_EPLMN_LIST_STRU      *pstOldDuplicateRplmnAndEplmnList
)
{
    VOS_UINT32                          i;

    /* 两个列表的个数不同，返回false */
    if (pstNewDuplicateRplmnAndEplmnList->ucNum != pstOldDuplicateRplmnAndEplmnList->ucNum)
    {
        return VOS_FALSE;
    }

    /* 两个列表的接入技术不同，返回false */
    if (pstNewDuplicateRplmnAndEplmnList->ucRat != pstOldDuplicateRplmnAndEplmnList->ucRat)
    {
        return VOS_FALSE;
    }

    /* duplicate rplmn 和Eplmn个数大于0,但duplicate Rplmn不同时，返回false */
    if (pstNewDuplicateRplmnAndEplmnList->ucNum > 0)
    {
        if (VOS_FALSE == NAS_MMC_IsInNvEplmnList(&(pstNewDuplicateRplmnAndEplmnList->astRplmnAndEplmnList[0]),
                                                 1,
                                                 pstOldDuplicateRplmnAndEplmnList->astRplmnAndEplmnList))
        {
            return VOS_FALSE;
        }
    }
    ...
    /* 其它duplicate Eplmn没有进行排序且有重复数据，需要对比2个列表才能确定相同 */
    for ( i = 1 ; i < pstNewDuplicateRplmnAndEplmnList->ucNum; i++ )
    {
        if (VOS_FALSE == NAS_MMC_IsInNvEplmnList(&(pstNewDuplicateRplmnAndEplmnList->astRplmnAndEplmnList[i]),
                                                 pstOldDuplicateRplmnAndEplmnList->ucNum,
                                                 pstOldDuplicateRplmnAndEplmnList->astRplmnAndEplmnList))
        {
            return VOS_FALSE;
        }
    ...
    for ( i = 1 ; i < pstOldDuplicateRplmnAndEplmnList->ucNum; i++ )
    {
        if (VOS_FALSE == NAS_MMC_IsInNvEplmnList(&(pstOldDuplicateRplmnAndEplmnList->astRplmnAndEplmnList[i]),
                                                 pstNewDuplicateRplmnAndEplmnList->ucNum,
                                                 pstNewDuplicateRplmnAndEplmnList->astRplmnAndEplmnList))
        {
            return VOS_FALSE;
        }
    }
...
VOS_UINT32 NAS_MMC_IsPlmnWithCsgIdInNvAllowedCsgList(
    NAS_NVIM_PLMN_WITH_CSG_ID_STRU     *pstPlmnWithCsgId,
    VOS_UINT8                           ucPlmnNum,
    NAS_NVIM_PLMN_WITH_CSG_ID_STRU     *pstDestPlmnWithCsgIdList
)
{
    VOS_UINT32                          i;
    ...
    for ( i = 0 ; i < ucPlmnNum ; i++ )
    {
        if ( (pstPlmnWithCsgId->stPlmnId.ucMcc[0] == pstDestPlmnWithCsgIdList[i].stPlmnId.ucMcc[0])
          && (pstPlmnWithCsgId->stPlmnId.ucMcc[1] == pstDestPlmnWithCsgIdList[i].stPlmnId.ucMcc[1])
          && (pstPlmnWithCsgId->stPlmnId.ucMcc[2] == pstDestPlmnWithCsgIdList[i].stPlmnId.ucMcc[2])
          && (pstPlmnWithCsgId->stPlmnId.ucMnc[0] == pstDestPlmnWithCsgIdList[i].stPlmnId.ucMnc[0])
          && (pstPlmnWithCsgId->stPlmnId.ucMnc[1] == pstDestPlmnWithCsgIdList[i].stPlmnId.ucMnc[1])
          && (pstPlmnWithCsgId->stPlmnId.ucMnc[2] == pstDestPlmnWithCsgIdList[i].stPlmnId.ucMnc[2])
          && (pstPlmnWithCsgId->ulCsgId           == pstDestPlmnWithCsgIdList[i].ulCsgId))
        {
            return VOS_TRUE;
        }
    }
...
VOS_UINT32 NAS_MMC_IsNvimAllowedCsgListEqual(
    NAS_NVIM_ALLOWED_CSG_LIST_STRU     *pstNewAllowedCsgList,
    NAS_NVIM_ALLOWED_CSG_LIST_STRU     *pstOldAllowedCsgList
)
{
    VOS_UINT32                          i;

    /* 两个列表的个数不同，返回false */
    if (pstNewAllowedCsgList->ucPlmnWithCsgIdNum != pstOldAllowedCsgList->ucPlmnWithCsgIdNum)
    {
        return VOS_FALSE;
    }

    /* 其它allowed csg list中的csg id没有进行排序且有重复数据，需要对比2个列表才能确定相同 */
    for ( i = 0 ; i < pstNewAllowedCsgList->ucPlmnWithCsgIdNum; i++ )
    {
        if (VOS_FALSE == NAS_MMC_IsPlmnWithCsgIdInNvAllowedCsgList(&(pstNewAllowedCsgList->astPlmnWithCsgIdList[i]),
                                                 pstOldAllowedCsgList->ucPlmnWithCsgIdNum,
                                                 pstOldAllowedCsgList->astPlmnWithCsgIdList))
        {
            return VOS_FALSE;
        }
    }

    for ( i = 0; i < pstOldAllowedCsgList->ucPlmnWithCsgIdNum; i++ )
    {
        if (VOS_FALSE == NAS_MMC_IsPlmnWithCsgIdInNvAllowedCsgList(&(pstOldAllowedCsgList->astPlmnWithCsgIdList[i]),
                                                 pstNewAllowedCsgList->ucPlmnWithCsgIdNum,
                                                 pstNewAllowedCsgList->astPlmnWithCsgIdList))
        {
            return VOS_FALSE;
        }
    }
...
VOS_VOID NAS_MMC_WriteCsgSpecSrchDuplicateRplmnAndEplmnNvim(VOS_VOID)
{
    ...
    NAS_NVIM_CSG_DUPLICATED_RPLMN_AND_EPLMN_LIST_STRU       stNewNvDuplicateRplmnAndEplmnList;
    NAS_NVIM_CSG_DUPLICATED_RPLMN_AND_EPLMN_LIST_STRU       stOldNvDuplicateRplmnAndEplmnList;
    NAS_MML_CSG_DUPLICATED_RPLMN_AND_EPLMN_LIST_STRU       *pstMmlDuplicateRplmnAndEplmnList = VOS_NULL_PTR;
    VOS_UINT32                                              i;
    VOS_UINT32                                              ulLength;

    PS_MEM_SET(&stNewNvDuplicateRplmnAndEplmnList, 0, sizeof(stNewNvDuplicateRplmnAndEplmnList));
    PS_MEM_SET(&stOldNvDuplicateRplmnAndEplmnList, 0, sizeof(stOldNvDuplicateRplmnAndEplmnList));
    PS_MEM_SET(&stCsgCtrlCfgNvim, 0, sizeof(stCsgCtrlCfgNvim));
    ulUpdateNvFlag                   = VOS_FALSE;
    (VOS_VOID)NV_GetLength(en_NV_Item_CSG_CTRL_CFG, &ulLength);
    pstMmlDuplicateRplmnAndEplmnList = NAS_MMC_GetCsgSpecSrchDuplicateRplmnAndEplmnInfo();

    stNewNvDuplicateRplmnAndEplmnList.ucNum = NAS_MML_MIN(pstMmlDuplicateRplmnAndEplmnList->ucPlmnNum,NAS_MML_MAX_EHPLMN_NUM);
    stNewNvDuplicateRplmnAndEplmnList.ucRat = pstMmlDuplicateRplmnAndEplmnList->enRat;

    for (i = 0; i < stNewNvDuplicateRplmnAndEplmnList.ucNum; i++)
    {
        NAS_MMC_ConvertMmlPlmnIdToNvimEquPlmn(&(pstMmlDuplicateRplmnAndEplmnList->astPlmnId[i]),
                                              &(stNewNvDuplicateRplmnAndEplmnList.astRplmnAndEplmnList[i]));
    ...
    {
        PS_MEM_CPY(&stOldNvDuplicateRplmnAndEplmnList, &stCsgCtrlCfgNvim.stDuplicatedRplmnAndEplmnList,
                   sizeof(stOldNvDuplicateRplmnAndEplmnList));

        if (VOS_FALSE == NAS_MMC_IsNvimDuplicateRplmnAndEplmnListEqual(&stNewNvDuplicateRplmnAndEplmnList,
                                                      &stOldNvDuplicateRplmnAndEplmnList))
        {
            ulUpdateNvFlag = VOS_TRUE;
        }
    }
    ...
    if ( VOS_TRUE == ulUpdateNvFlag )
    {
        /* 将新的duplicate rplmn和eplmn信息赋值给stCsgCtrlCfgNvim并写入nv */
        PS_MEM_CPY(&stCsgCtrlCfgNvim.stDuplicatedRplmnAndEplmnList, &stNewNvDuplicateRplmnAndEplmnList,
                   sizeof(stCsgCtrlCfgNvim.stDuplicatedRplmnAndEplmnList));

        if (NV_OK != NV_Write(en_NV_Item_CSG_CTRL_CFG,
                              &stCsgCtrlCfgNvim,
                              ulLength))
        {
            NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_WriteCsgSpecSrchDuplicateRplmnAndEplmnNvim:WARNING: en_NV_Item_CSG_CTRL_CFG Error");
        }
    }
...
VOS_VOID NAS_MMC_WriteAllowedCsgListNvim(VOS_VOID)
{
    VOS_UINT32                          ulUpdateNvFlag;
    NAS_NVIM_CSG_CTRL_CFG_STRU          stCsgCtrlCfgNvim;
    NAS_NVIM_ALLOWED_CSG_LIST_STRU      stNewNvAllowedCsgList;
    NAS_NVIM_ALLOWED_CSG_LIST_STRU      stOldNvAllowedCsgList;
    NAS_MML_PLMN_WITH_CSG_ID_LIST_STRU *pstMmlAllowedCsgList = VOS_NULL_PTR;
    VOS_UINT32                          i;
    VOS_UINT32                          ulLength;

    PS_MEM_SET(&stNewNvAllowedCsgList, 0, sizeof(stNewNvAllowedCsgList));
    PS_MEM_SET(&stOldNvAllowedCsgList, 0, sizeof(stOldNvAllowedCsgList));
    PS_MEM_SET(&stCsgCtrlCfgNvim, 0, sizeof(stCsgCtrlCfgNvim));
    (VOS_VOID)NV_GetLength(en_NV_Item_CSG_CTRL_CFG, &ulLength);
    ulUpdateNvFlag       = VOS_FALSE;
    pstMmlAllowedCsgList = NAS_MML_GetAllowedCsgList();


    stNewNvAllowedCsgList.ucPlmnWithCsgIdNum = (VOS_UINT8)NAS_MML_MIN(pstMmlAllowedCsgList->ulPlmnWithCsgIdListNum, NAS_NVIM_MAX_PLMN_CSG_ID_NUM);

    for (i = 0; i < stNewNvAllowedCsgList.ucPlmnWithCsgIdNum; i++)
    {
        stNewNvAllowedCsgList.astPlmnWithCsgIdList[i].ulCsgId = pstMmlAllowedCsgList->astPlmnWithCsgIdInfo[i].stPlmnWithCsgId.ulCsgId;

        /* MML格式的plmn id转换为NVIM_PLMN_VALUE_STRU格式的plmn id */
        NAS_MMC_ConvertMmlPlmnIdToNvimEquPlmn(&(pstMmlAllowedCsgList->astPlmnWithCsgIdInfo[i].stPlmnWithCsgId.stPlmnId),
                                              &(stNewNvAllowedCsgList.astPlmnWithCsgIdList[i].stPlmnId));

    }

    /* 读取NV中allowed csg list 信息 */
    if ( NV_OK == NV_Read(en_NV_Item_CSG_CTRL_CFG,
                          &stCsgCtrlCfgNvim, ulLength) )
    {
        PS_MEM_CPY(&stOldNvAllowedCsgList, &stCsgCtrlCfgNvim.stAllowedCsgList,
                   sizeof(stOldNvAllowedCsgList));

        if (VOS_FALSE == NAS_MMC_IsNvimAllowedCsgListEqual(&stNewNvAllowedCsgList,
                                                      &stOldNvAllowedCsgList))
        {
            ulUpdateNvFlag = VOS_TRUE;
        }
    }
    ...
    if ( VOS_TRUE == ulUpdateNvFlag )
    {
        /* 将新的allowed csg list信息赋值给stAllowedCsgList并写入nv */
        PS_MEM_CPY(&stCsgCtrlCfgNvim.stAllowedCsgList, &stNewNvAllowedCsgList,
                   sizeof(stCsgCtrlCfgNvim.stAllowedCsgList));

        if (NV_OK != NV_Write(en_NV_Item_CSG_CTRL_CFG,
                              &stCsgCtrlCfgNvim,
                              ulLength))
        {
            NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_WriteAllowedCsgListNvim:WARNING: en_NV_Item_CSG_CTRL_CFG Error");
        }
    }
...
VOS_VOID NAS_MMC_ReadCsgCfgNvim(VOS_VOID)
{
    ...
    NAS_MML_CSG_DUPLICATED_RPLMN_AND_EPLMN_LIST_STRU       *pstCsgDuplicateRplmnAndEplmnInfo = VOS_NULL_PTR;
    NAS_MML_PLMN_WITH_CSG_ID_LIST_STRU                     *pstAllowedCsgList                = VOS_NULL_PTR;
    NAS_MML_PLMN_WITH_CSG_ID_LIST_STRU                     *pstUeBasedOperatorCsgList        = VOS_NULL_PTR;
    ...
    pstCsgDuplicateRplmnAndEplmnInfo = NAS_MMC_GetCsgSpecSrchDuplicateRplmnAndEplmnInfo();
    pstAllowedCsgList                = NAS_MML_GetAllowedCsgList();
    pstUeBasedOperatorCsgList        = NAS_MML_GetUeBasedOperatorCsgList();
    /*需要使用UT开关，即UT时不设置为false*/
    //#if (VOS_WIN32 != VOS_OS_VER)
    ...
    /* 读取csg禁止网络相关配置并保存 */
    NAS_MML_SetCsgForbiddenPlmnPeriod(stCsgCtrlCfgNvim.stCsgForbiddenPlmnCfg.usCsgPlmnForbiddenPeriod * NAS_MML_ONE_THOUSAND_MILLISECOND);
    pstCsgForbiddenPlmnCfg->ucCauseNum = NAS_MML_MIN(stCsgCtrlCfgNvim.stCsgForbiddenPlmnCfg.ucCauseNum, NAS_NVIM_MAX_CSG_REJ_CAUSE_NUM);

    for (i = 0; i < pstCsgForbiddenPlmnCfg->ucCauseNum; i++)
    {
        if (0 != stCsgCtrlCfgNvim.stCsgForbiddenPlmnCfg.ausCause[i])
        {
            pstCsgForbiddenPlmnCfg->ausCause[i] = stCsgCtrlCfgNvim.stCsgForbiddenPlmnCfg.ausCause[i];
        }
    }

    pstCsgDuplicateRplmnAndEplmnInfo->ucPlmnNum = NAS_MML_MIN(stCsgCtrlCfgNvim.stDuplicatedRplmnAndEplmnList.ucNum, NVIM_MAX_EPLMN_NUM);
    pstCsgDuplicateRplmnAndEplmnInfo->enRat     = stCsgCtrlCfgNvim.stDuplicatedRplmnAndEplmnList.ucRat;

    if (0 < pstCsgDuplicateRplmnAndEplmnInfo->ucPlmnNum)
    {
        /* 设置duplicate Rplmn和Eplmn标识为有效 */
        NAS_MMC_SetCsgDuplicateRplmnAndEplmnValidFlag(VOS_TRUE);
    }

    /* 读取duplicate rplmn和eplmn信息并保存 */
    for (i = 0; i < pstCsgDuplicateRplmnAndEplmnInfo->ucPlmnNum; i++)
    {
        pstCsgDuplicateRplmnAndEplmnInfo->astPlmnId[i].ulMcc = (stCsgCtrlCfgNvim.stDuplicatedRplmnAndEplmnList.astRplmnAndEplmnList[i].ucMcc[0]
                            | ((VOS_UINT32)stCsgCtrlCfgNvim.stDuplicatedRplmnAndEplmnList.astRplmnAndEplmnList[i].ucMcc[1] << NAS_MML_OCTET_MOVE_EIGHT_BITS)
                            | ((VOS_UINT32)stCsgCtrlCfgNvim.stDuplicatedRplmnAndEplmnList.astRplmnAndEplmnList[i].ucMcc[2] << NAS_MML_OCTET_MOVE_SIXTEEN_BITS));

        pstCsgDuplicateRplmnAndEplmnInfo->astPlmnId[i].ulMnc =
                           (stCsgCtrlCfgNvim.stDuplicatedRplmnAndEplmnList.astRplmnAndEplmnList[i].ucMnc[0]
                           | ((VOS_UINT32)stCsgCtrlCfgNvim.stDuplicatedRplmnAndEplmnList.astRplmnAndEplmnList[i].ucMnc[1] << NAS_MML_OCTET_MOVE_EIGHT_BITS)
                           | ((VOS_UINT32)stCsgCtrlCfgNvim.stDuplicatedRplmnAndEplmnList.astRplmnAndEplmnList[i].ucMnc[2] << NAS_MML_OCTET_MOVE_SIXTEEN_BITS));
    }

    /* 读取nv中allowed csg list信息 */
    pstAllowedCsgList->ulPlmnWithCsgIdListNum = NAS_MML_MIN(stCsgCtrlCfgNvim.stAllowedCsgList.ucPlmnWithCsgIdNum, NAS_NVIM_MAX_PLMN_CSG_ID_NUM);

    for (i = 0; i < pstAllowedCsgList->ulPlmnWithCsgIdListNum; i++)
    {
        pstAllowedCsgList->astPlmnWithCsgIdInfo[i].stPlmnWithCsgId.stPlmnId.ulMcc = (stCsgCtrlCfgNvim.stAllowedCsgList.astPlmnWithCsgIdList[i].stPlmnId.ucMcc[0]
                            | ((VOS_UINT32)stCsgCtrlCfgNvim.stAllowedCsgList.astPlmnWithCsgIdList[i].stPlmnId.ucMcc[1] << NAS_MML_OCTET_MOVE_EIGHT_BITS)
                            | ((VOS_UINT32)stCsgCtrlCfgNvim.stAllowedCsgList.astPlmnWithCsgIdList[i].stPlmnId.ucMcc[2] << NAS_MML_OCTET_MOVE_SIXTEEN_BITS));

        pstAllowedCsgList->astPlmnWithCsgIdInfo[i].stPlmnWithCsgId.stPlmnId.ulMnc =  (stCsgCtrlCfgNvim.stAllowedCsgList.astPlmnWithCsgIdList[i].stPlmnId.ucMnc[0]
                           | ((VOS_UINT32)stCsgCtrlCfgNvim.stAllowedCsgList.astPlmnWithCsgIdList[i].stPlmnId.ucMnc[1] << NAS_MML_OCTET_MOVE_EIGHT_BITS)
                           | ((VOS_UINT32)stCsgCtrlCfgNvim.stAllowedCsgList.astPlmnWithCsgIdList[i].stPlmnId.ucMnc[2] << NAS_MML_OCTET_MOVE_SIXTEEN_BITS));

        pstAllowedCsgList->astPlmnWithCsgIdInfo[i].stPlmnWithCsgId.ulCsgId = stCsgCtrlCfgNvim.stAllowedCsgList.astPlmnWithCsgIdList[i].ulCsgId;

    }
    /* 读取nv中based csg list信息 */
    pstUeBasedOperatorCsgList->ulPlmnWithCsgIdListNum = NAS_MML_MIN(stCsgCtrlCfgNvim.stUeBasedOperatorCsgList.ucPlmnWithCsgIdNum, NAS_NVIM_MAX_PLMN_CSG_ID_NUM);

    for (i = 0; i < pstUeBasedOperatorCsgList->ulPlmnWithCsgIdListNum; i++)
    {
        pstUeBasedOperatorCsgList->astPlmnWithCsgIdInfo[i].stPlmnWithCsgId.stPlmnId.ulMcc = (stCsgCtrlCfgNvim.stUeBasedOperatorCsgList.astPlmnWithCsgIdList[i].stPlmnId.ucMcc[0]
                            | ((VOS_UINT32)stCsgCtrlCfgNvim.stUeBasedOperatorCsgList.astPlmnWithCsgIdList[i].stPlmnId.ucMcc[1] << NAS_MML_OCTET_MOVE_EIGHT_BITS)
                            | ((VOS_UINT32)stCsgCtrlCfgNvim.stUeBasedOperatorCsgList.astPlmnWithCsgIdList[i].stPlmnId.ucMcc[2] << NAS_MML_OCTET_MOVE_SIXTEEN_BITS));

        pstUeBasedOperatorCsgList->astPlmnWithCsgIdInfo[i].stPlmnWithCsgId.stPlmnId.ulMnc =  (stCsgCtrlCfgNvim.stUeBasedOperatorCsgList.astPlmnWithCsgIdList[i].stPlmnId.ucMnc[0]
                           | ((VOS_UINT32)stCsgCtrlCfgNvim.stUeBasedOperatorCsgList.astPlmnWithCsgIdList[i].stPlmnId.ucMnc[1] << NAS_MML_OCTET_MOVE_EIGHT_BITS)
                           | ((VOS_UINT32)stCsgCtrlCfgNvim.stUeBasedOperatorCsgList.astPlmnWithCsgIdList[i].stPlmnId.ucMnc[2] << NAS_MML_OCTET_MOVE_SIXTEEN_BITS));

        pstUeBasedOperatorCsgList->astPlmnWithCsgIdInfo[i].stPlmnWithCsgId.ulCsgId = stCsgCtrlCfgNvim.stUeBasedOperatorCsgList.astPlmnWithCsgIdList[i].ulCsgId;

    }

    /* 读取ASF和PSF定时器相关信息 */
    NAS_MML_SetCsgPeriodicSearchLen(stCsgCtrlCfgNvim.ulCsgPeriodicSearchPeriod*NAS_MML_ONE_THOUSAND_MILLISECOND*60);

    pstCsgASFCfg = NAS_MML_GetCsgAutonomousSrchCfg();

    pstCsgASFCfg->ucCsgAutonomousSrchFirstSrchTimeCount  = stCsgCtrlCfgNvim.stCsgAutonomousSrchCfg.ucCsgAutonomousSrchFirstSrchTimeCount;
    pstCsgASFCfg->usCsgAutonomousSrchFirstSrchTimeLen    = stCsgCtrlCfgNvim.stCsgAutonomousSrchCfg.usCsgAutonomousSrchFirstSrchTimeLen*NAS_MML_ONE_THOUSAND_MILLISECOND;
    pstCsgASFCfg->ucCsgAutonomousSrchSecondSrchTimeCount = stCsgCtrlCfgNvim.stCsgAutonomousSrchCfg.ucCsgAutonomousSrchSecondSrchTimeCount;
    pstCsgASFCfg->usCsgAutonomousSrchSecondSrchTimeLen   = stCsgCtrlCfgNvim.stCsgAutonomousSrchCfg.usCsgAutonomousSrchSecondSrchTimeLen*NAS_MML_ONE_THOUSAND_MILLISECOND;
...
VOS_VOID NAS_MMC_ReadOosBgNetworkSearchCustom(VOS_VOID)
{
    ...
    NAS_MMC_NVIM_OOS_BG_NETWORK_SEARCH_CUSTOM_STRU          stOosNsCustomCfg;
    VOS_UINT32                                              ulLength;

    pstCustomCfg = NAS_MML_GetCustomCfg();

    PS_MEM_SET(&stOosNsCustomCfg, 0x00, sizeof(stOosNsCustomCfg));

    ulLength             = 0;

    /* 先获取NV的长度 */
    (VOS_VOID)NV_GetLength(en_NV_Item_OOS_BG_NETWORK_SEARCH_CUSTOM, &ulLength);

    if (ulLength > sizeof(NAS_MMC_NVIM_OOS_BG_NETWORK_SEARCH_CUSTOM_STRU))
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_ReadOosBgNetworkSearchCustom(): get length Error");
        return;
    }

    /* 读NV失败 */
    if (NV_OK != NV_Read(en_NV_Item_OOS_BG_NETWORK_SEARCH_CUSTOM,
                         &stOosNsCustomCfg, ulLength))
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_ReadOosBgNetworkSearchCustom(): read nv error");
        return;
    }

    pstCustomCfg->stOosBgNetworkSearchCustom.ulDelayNetworkSearchTimerLenAfterEmc    = stOosNsCustomCfg.ulDelayNetworkSearchTimerLenAfterEmc;

    /* 初始化化时默认为FALSE；只有NV配置为TRUE时，才置成TRUE，NV为其他值时都认为不支持 */
    if (VOS_TRUE == stOosNsCustomCfg.ucBgHistorySupportFlg)
    {
        pstCustomCfg->stOosBgNetworkSearchCustom.ucBgHistorySupportFlg      = VOS_TRUE;
    }

    if (VOS_TRUE == stOosNsCustomCfg.ucPrefBandListSupportFlg)
    {
        pstCustomCfg->stOosBgNetworkSearchCustom.ucPrefBandListSupportFlg   = VOS_TRUE;
    }

    if (VOS_TRUE == stOosNsCustomCfg.ucFullBandListSupportFlg)
    {
        pstCustomCfg->stOosBgNetworkSearchCustom.ucFullBandListSupportFlg   = VOS_TRUE;
    }
...
{
    NAS_NVIM_EXTEND_T3240_LEN_CFG_STRU                      stExtendT3240LenCfg;
    VOS_UINT32                                              ulLength;

    PS_MEM_SET(&stExtendT3240LenCfg, 0x00, sizeof(stExtendT3240LenCfg));
    ...
    if (VOS_TRUE  == NAS_USIMMAPI_IsTestCard())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_ReadExtendT3240LenCfg():Test SIM, Do not extend T3240 len");
    ...
    /* 先获取NV的长度 */
    (VOS_VOID)NV_GetLength(en_NV_Item_EXTEND_T3240_LEN_CFG, &ulLength);

    if (ulLength > sizeof(NAS_NVIM_EXTEND_T3240_LEN_CFG_STRU))
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_ReadExtendT3240LenCfg(): get length Error");
        return;
    }

    /* 读NV失败 */
    if (NV_OK != NV_Read(en_NV_Item_EXTEND_T3240_LEN_CFG,
                         &stExtendT3240LenCfg, ulLength))
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_ReadExtendT3240LenCfg(): read nv error");
        return;
    }

    NAS_NORMAL_LOG2(WUEPS_PID_MMC, "NAS_MMC_ReadExtendT3240LenCfg(): ucExtendT3240LenFlg, usCustomizedT3240Len", stExtendT3240LenCfg.ucExtendT3240LenFlg, stExtendT3240LenCfg.usCustomizedT3240Len);

    /* 初始化化时默认为FALSE；只有NV配置为TRUE时，才置成TRUE，NV为其他值时都认为不支持 */
    if (VOS_TRUE == stExtendT3240LenCfg.ucExtendT3240LenFlg)
    {
        NAS_MML_SetExtendT3240LenFlg(VOS_TRUE);
        NAS_MML_SetCustomizedT3240Len(stExtendT3240LenCfg.usCustomizedT3240Len);
    }
...
VOS_VOID NAS_MMC_ReadDamCfgNvim(VOS_VOID)
{
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MML_SetEnableLteTrigPlmnSearchFlag IS TRUE");
    ...
    for (i = 0; i < stDamCfgInfo.stDamConfigPlmnPara.stDamPlmnList.ulPlmnNum; i++)
    {
        ...
        NAS_NORMAL_LOG3(WUEPS_PID_MMC, "NAS_MMC_ReadDamCfgNvim, DAMPLMN,ulMcc; ulMnc,TOTAL NUMBER IS;",stGUNasPlmn.ulMcc,stGUNasPlmn.ulMnc,pstDamPlmnInfoCfg->stDamPlmnList.ucPlmnNum);
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MML_SetAddDamPlmnInDisablePlmnWithRatListFlag IS TRUE");

        NAS_MML_SetAddDamPlmnInDisablePlmnWithRatListFlag(VOS_TRUE);
    }

    NAS_MML_SetDamActiveFlag(VOS_FALSE);
    if (VOS_TRUE == stDamCfgInfo.ucDamFlag[0] )
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_ReadDamCfgNvim, DAM ACTIVE FLAG IS TRUE");
        NAS_MML_SetDamActiveFlag(VOS_TRUE);
    }
...
VOS_VOID NAS_MMC_ReadImsRatSupportNvim(VOS_VOID)
{
    ...
    {
        NAS_MML_SetLteImsSupportFlg(VOS_TRUE);
    }
...
VOS_VOID NAS_MMC_ReadRefreshAllFileRestartFlagNvim(VOS_VOID)
{
    ...
    NAS_MEM_SET_S(&stRefreshAllFileRestartFlag, sizeof(stRefreshAllFileRestartFlag), 0x00, sizeof(TAF_NVIM_REFRESH_ALL_FILE_RESTART_CFG_STRU));

    /* 读取NV */
    if (NV_OK != NV_Read (en_NV_Item_REFRESH_ALL_FILE_RESTART_CFG, &stRefreshAllFileRestartFlag, sizeof(TAF_NVIM_REFRESH_ALL_FILE_RESTART_CFG_STRU)))
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_ReadRefreshAllFileRestartFlagNvim(): en_NV_Item_REFRESH_ALL_FILE_RESTART_CFG Error");
        return;
    }
    ...
    {
        /* 赋值到全局变量 */
        NAS_MML_SetRefreshAllFileRestartFlag(stRefreshAllFileRestartFlag.ucRefreshAllFileRestartFlag);
    }
...
{
    NAS_NVIM_ATT_ENS_CTRL_STRU          stNvAttEns;
    PS_MEM_SET(&stNvAttEns, 0x00, sizeof(stNvAttEns));

    /* 默认不使能 */
    NAS_MML_SetNetworkSelectionMenuEnableFlg(VOS_FALSE);
    NAS_MML_SetRatBalancingEnableFlg(VOS_FALSE);

    /* 读NV项en_NV_Item_Att_Ens_Ctrl_Para失败 */
    if (NV_OK != NV_Read(en_NV_Item_Att_Ens_Ctrl_Para, &stNvAttEns, sizeof(NAS_NVIM_ATT_ENS_CTRL_STRU)))
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_ReadAttEnsCustomNvim(): Read NV Fail");
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NetworkSelectionMenu IS Enable");
        NAS_MML_SetNetworkSelectionMenuEnableFlg(VOS_TRUE);
    }

    /* 置接入技术平衡控制标识 */
    if ( VOS_TRUE == stNvAttEns.ucRatBalancingFlg)
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "RatBalancing IS Enable");
        NAS_MML_SetRatBalancingEnableFlg(VOS_TRUE);
    }
...
{
    AI_MODEM_CFG_NVIM_STRU              stNvAiModemCfg;
    VOS_UINT32                          ulLength;

    ulLength = 0;
    PS_MEM_SET(&stNvAiModemCfg, 0x00, sizeof(stNvAiModemCfg));

    /* 默认不使能 */
    NAS_MML_SetAiModemActFlg(VOS_FALSE);

    (VOS_VOID)NV_GetLength(en_NV_Item_AI_MODEM_CFG, &ulLength);

    if (ulLength > sizeof(AI_MODEM_CFG_NVIM_STRU))
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_ReadAiModemCfgNvim():WARNING: en_NV_Item_AI_MODEM_CFG length Error");

        return;
    }

    /* 读NV项en_NV_Item_AI_MODEM_CFG失败 */
    if (NV_OK != NV_Read(en_NV_Item_AI_MODEM_CFG, &stNvAiModemCfg,
                sizeof(AI_MODEM_CFG_NVIM_STRU)))
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_ReadAiModemCfgNvim(): Read NV Fail");

        return;
    }

    if (VOS_TRUE == stNvAiModemCfg.ucActFlg)
    {
        NAS_MML_SetAiModemActFlg(VOS_TRUE);
        NAS_MML_SetOosPlmnSearchSleepCount(stNvAiModemCfg.stNasSrchCfg.ucOosSrchSleepCnt);
        NAS_MML_SetHighPrioRatHplmnSleepCount(stNvAiModemCfg.stNasSrchCfg.ucHighPrioSrchSleepCnt);
        NAS_MML_SetAiModemApplyArea(stNvAiModemCfg.ucApplyArea);

        if (   (stNvAiModemCfg.stTimeCfg.ucTimeStart <= NAS_MTC_TIME_TWENTY_FOUR_CLOCK)
            && (stNvAiModemCfg.stTimeCfg.ucTimeEnd <= NAS_MTC_TIME_TWENTY_FOUR_CLOCK))
        {
            NAS_MML_SetAiModemTimeStart(stNvAiModemCfg.stTimeCfg.ucTimeStart);
            NAS_MML_SetAiModemTimeEnd(stNvAiModemCfg.stTimeCfg.ucTimeEnd);
        }
        NAS_NORMAL_LOG4(WUEPS_PID_MMC, "AiModemSavePowerActFlg IS Enable",
            stNvAiModemCfg.stNasSrchCfg.ucOosSrchSleepCnt,
            stNvAiModemCfg.stNasSrchCfg.ucHighPrioSrchSleepCnt,
            stNvAiModemCfg.stTimeCfg.ucTimeStart,
            stNvAiModemCfg.stTimeCfg.ucTimeEnd);
    }
