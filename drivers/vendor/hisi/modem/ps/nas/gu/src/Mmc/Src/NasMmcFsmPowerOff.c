...
)
{
    ...
    /* 关机，清除之前记录的用户指定CSG ID信息 */
    NAS_MMC_InitCsgSpecSrchPlmnWithCsgId();

    /* 释放开机时为存储Allowed Csg List码流申请的空间 */
    NAS_MML_ClearAllowedCsgListRecordStream();
#endif
...
)
{
    ...
    NAS_MMC_SndMmPowerOffNotify();
    NAS_MMC_SndGmmPowerOffNotify();
...
)
{
    ...
    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvLmmSuspendCnf_PowerOff_WaitLmmSuspendCnf: Set srv status to NO SERVICE and reg state to 0 before sending POWER SAVE CNF");
    NAS_MMC_ChangeServiceState(NAS_MSCC_PIF_SRVDOMAIN_CS_PS, NAS_MMC_NO_SERVICE);
    NAS_MMC_ChangePsRegState(NAS_MML_REG_NOT_REGISTERED_NOT_SEARCH);
    NAS_MMC_ChangeCsRegState(NAS_MML_REG_NOT_REGISTERED_NOT_SEARCH);
