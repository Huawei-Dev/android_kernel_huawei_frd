...
VOS_VOID NAS_CC_ReadNvimInfo(VOS_VOID)
{
    ...
    NAS_CC_ReadNonregularServiceCustomNvim();
...
VOS_VOID NAS_CC_ReadNonregularServiceCustomNvim(VOS_VOID)
{
    NAS_NVIM_NONREGULAR_SERVICE_CUSTOM_STRU    stNonregularServiceCustom;

    NAS_MEM_SET_S(&stNonregularServiceCustom, sizeof(NAS_NVIM_NONREGULAR_SERVICE_CUSTOM_STRU), 0x00, sizeof(NAS_NVIM_NONREGULAR_SERVICE_CUSTOM_STRU));

    /* 设置默认值 */
    NAS_CC_SetTiNonregularCustom(VOS_FALSE);

    /* 读取NV */
    if (VOS_OK != NV_Read(en_NV_Item_Nonregular_Servic_Custom, &stNonregularServiceCustom, sizeof(stNonregularServiceCustom)))
    {
        NAS_CC_WARN_LOG("NAS_CC_ReadNonregularServiceCustomNvim():WARNING: en_NV_Item_Nonregular_Servic_Custom Error");

        return;
    }

    if (VOS_TRUE  == stNonregularServiceCustom.ucTiNonregularCustom)
    {
        NAS_CC_SetTiNonregularCustom(VOS_TRUE);
    }
