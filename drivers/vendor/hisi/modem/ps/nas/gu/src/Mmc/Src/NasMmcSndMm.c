...
VOS_VOID NAS_MMC_SndMmPowerOffNotify(VOS_VOID)
{
    MMCMM_POWER_OFF_NOTIFY_STRU        *pstMsg              = VOS_NULL_PTR;

    /* 从内部消息队列中获取一个还没有使用的空间 */
    pstMsg          = (MMCMM_POWER_OFF_NOTIFY_STRU *)NAS_MML_GetIntMsgSendBuf(sizeof(MMCMM_POWER_OFF_NOTIFY_STRU));
    if ( VOS_NULL_PTR == pstMsg)
    {
        return;
    }
    ...
    pstMsg->MsgHeader.ulReceiverCpuId   = VOS_LOCAL_CPUID;
    pstMsg->MsgHeader.ulReceiverPid     = WUEPS_PID_MM;
    pstMsg->MsgHeader.ulLength          = sizeof(MMCMM_POWER_OFF_NOTIFY_STRU) - VOS_MSG_HEAD_LENGTH;

    /* 填写消息类别 */
    pstMsg->MsgHeader.ulMsgName         = MMCMM_POWER_OFF_NOTIFY;
...
VOS_VOID NAS_MMC_SndMmImsSrvInfoNotify(
    MSCC_MMC_IMS_SRV_INFO_NOTIFY_STRU  *pstImsSrvInfo
)
{
    MMCMM_IMS_SRV_INFO_NOTIFY_STRU     *pstMsg = VOS_NULL_PTR;

    /* 申请消息内存分配 */
    pstMsg              = (MMCMM_IMS_SRV_INFO_NOTIFY_STRU *)NAS_MML_GetIntMsgSendBuf(sizeof(MMCMM_IMS_SRV_INFO_NOTIFY_STRU));
    ...
    pstMsg->stMsgHeader.ulLength          = sizeof(MMCMM_IMS_SRV_INFO_NOTIFY_STRU) - VOS_MSG_HEAD_LENGTH;
    pstMsg->stMsgHeader.ulMsgName         = MMCMM_IMS_SRV_INFO_NOTIFY;

    NAS_MEM_CPY_S(&pstMsg->stImsCallInfo, sizeof(pstMsg->stImsCallInfo), &pstImsSrvInfo->stImsCallInfo, sizeof(pstImsSrvInfo->stImsCallInfo));
