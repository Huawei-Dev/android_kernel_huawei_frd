...
VOS_VOID  NAS_MMC_SndApsPsRegStatusInd(
    MMC_APS_PS_REG_RESULT_ENUM_U32      enPsRegRslt,
    MMC_APS_PLMN_ID_STRU               *pstCurPlmn
)
{
    MMC_APS_PS_REG_STATUS_IND_STRU     *pstMsg      = VOS_NULL_PTR;

    pstMsg = (MMC_APS_PS_REG_STATUS_IND_STRU *)PS_ALLOC_MSG_WITH_HEADER_LEN(WUEPS_PID_MMC,
                        sizeof(MMC_APS_PS_REG_STATUS_IND_STRU));

    if ( VOS_NULL_PTR == pstMsg )
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SndApsPsRegStatusInd: memery alloc failed.");
    ...
    pstMsg->stMsgHeader.ulSenderPid     = WUEPS_PID_MMC;
    pstMsg->stMsgHeader.ulReceiverPid   = WUEPS_PID_TAF;
    pstMsg->stMsgHeader.ulLength        = sizeof(MMC_APS_PS_REG_STATUS_IND_STRU) - VOS_MSG_HEAD_LENGTH;
    pstMsg->stMsgHeader.ulMsgName       = ID_MMC_APS_PS_REG_STATUS_IND;
    pstMsg->enPsRegResult               = enPsRegRslt;

    PS_MEM_CPY(&pstMsg->stPlmnId, pstCurPlmn,sizeof(pstMsg->stPlmnId));

    if ( VOS_OK != PS_SEND_MSG(WUEPS_PID_MMC, pstMsg) )
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SndApsPsRegStatusInd: send message failed.");
    }
