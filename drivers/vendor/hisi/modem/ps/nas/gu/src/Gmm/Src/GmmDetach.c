...
)
{
          ...
          )
    {
        ...
        NAS_NORMAL_LOG(WUEPS_PID_GMM, "NAS_GMM_ProcDetachReq_RegNmlServ: waitForSrv RauCause trigger RAU");
...
VOS_VOID Gmm_RcvMmcPowerOffNotify(
    VOS_VOID                           *pMsg                                    /* 原语指针                                 */
)
{
    if (VOS_TRUE == NAS_MML_IsNeedClearTmsiPtmsi())
    {
        ...
        NAS_NORMAL_LOG(WUEPS_PID_GMM, "Gmm_RcvMmcPowerOffNotify: Delete PTMSI");
    }
    if (GMM_NULL == NAS_GMM_GetGmmState())
    {
        PS_NAS_LOG(WUEPS_PID_GMM, VOS_NULL, PS_LOG_LEVEL_INFO, "Gmm_RcvMmcPowerOffNotify: GMM state is null");
        return;
    }

    if (GMM_UEID_P_TMSI_SIGNATURE == (g_GmmGlobalCtrl.UeInfo.UeId.ucUeIdMask & GMM_UEID_P_TMSI_SIGNATURE))
    {
        /* DETACH成功且使用了P_TMSI_SIGNATURE       */
...
VOS_VOID Gmm_Com_Cause2Detach(VOS_VOID)
{
    ...
    {                                                                           /* 当前状态为REGISTERED_INITIATED           */
        NAS_NORMAL_LOG(WUEPS_PID_GMM, "Gmm_Com_Cause2Detach: REGISTERED_INITIATED trigger ATT");
    ...
    {                                                                           /* 当前状态为RAU_INITIATED                  */
        NAS_NORMAL_LOG(WUEPS_PID_GMM, "Gmm_Com_Cause2Detach: RAU_INITIATED state trigger RAU");
...
VOS_VOID Gmm_NetInitRequiredDetach(VOS_VOID)
{
    ...
    else
    {
        ...
        {
            NAS_NORMAL_LOG(WUEPS_PID_GMM, "Gmm_NetInitRequiredDetach: trigger ATT");
...
VOS_VOID Gmm_NetInitImsiDetach(VOS_VOID)
{
    ...
    {                                                                           /* CS域注册                                 */
        ...
        {
            NAS_NORMAL_LOG(WUEPS_PID_GMM, "Gmm_NetInitImsiDetach: trigger RAU");
