...
)
{
    ...
    if ( NAS_MMC_POWEROFF_RSLT_POWER_OFF == pstPowerOffRslt->enRslt )
    {
        ...
        /* 如果CSG手动搜索网络与RPLMN不同标识为TRUE且当前驻留L CSG小区，
           且duplicate RPLMN和EPLMN有效，则将duplicate RPLMN和EPLMN信息写入新增NV项*/
...
)
{
    ...
    /* CSG自主搜索定时器和周期搜索定时器以及自主搜索计数的相关处理 */
...
)
{
    ...
    /* 不在on plmn时搜到CS紧急呼叫，发起anycell搜网 */
...
#if (FEATURE_ON == FEATURE_CSG)
    /* 通知L白名单，如果之前是Hybrid驻留，则不能再自动回到原小区，此处无条件配置一次以保证 */
...
#if (FEATURE_ON == FEATURE_CSG)
    /* UE成功驻留在L非CSG PLMN1的小区，后同系统重选到L CSG PLMN2的小区，MMC在ON PLMN
    收到L系统消息判断如果当前支持CSG功能且当前驻留的CSG小区，当前驻留网络与RPLMN不同，
    且之前驻留非CSG小区，则更新duplicate RPLMN为PLMN1，如果PLMN存在有效的EPLMN，EPLMN也许保存在duplicate EPLMN中*/
...
VOS_UINT32 NAS_MMC_IsAvailTimerExpiredTriggerBgHistorySearch_L1Main(VOS_VOID)
{
    ...
    */
    ...
    /* 如果美国搜网定制bg history特性未打开，不需要发起bg history搜 */
...
VOS_VOID NAS_MMC_PerformBufferedPlmnSearch_L1Main(VOS_VOID)
{
    ...
    /* NAS_MMC_RcvTiAvailTimerExpired_PreProc有RRC链接缓存搜网请求时，没有区分是history/pref band/full band搜，
       此处调用NAS_MMC_ProcTiAvailableTimerExpiredTriggerPlmnSearch_L1Main进行细分是哪种类型的搜网 */
    if ((NAS_MMC_PLMN_SEARCH_SCENE_AVAILABLE_TIMER_EXPIRED == enPlmnSearchScene)
     && (VOS_TRUE == ulIsNeedPlmnSearch))
    {
        /* 和available超时的处理相同:根据配置决定是否进行(bg)history/pref band(list)/full band(list)/list搜网 */
...
)
{
    ...
    if (MMC_LMM_ATT_RSLT_SUCCESS == pstLmmAttachIndMsg->ulAttachRslt)
    {
        ...
        /*注意: 需要先更新EPLMN LIST，再处理注册状态的上报，因为NAS_MMC_UpdatePsRegStatePsRegSucc()函数中会根据EPLMN LIST上报注册状态 */
        /* 更新EPLMN */
        if ( VOS_TRUE == pstLmmAttachIndMsg->bitOpEplmn)
        {
            ulEquPlmnNum        = pstLmmAttachIndMsg->stEplmnList.ulPlmnNum;
...
VOS_UINT32 NAS_MMC_ProcTiAvailableTimerExpired_HistorySearch_L1Main(VOS_VOID)
{
    ...
    /* 灭屏，静止，NV开关打开且在夜里会进入低功耗模式，低功耗模式睡眠次数不够情况下重启定时器不搜网 */
    ...
    /* 有CM业务存在时，周期定时器超时需要做SPEC搜而不是History搜 */
...
VOS_UINT32 NAS_MMC_ProcTiAvailableTimerExpired_PrefBandSearch_L1Main(VOS_VOID)
{
    ...
    /* 是否需要做BG history搜，如果需要，bg history搜失败再做list pref band搜 */
    if (VOS_TRUE == NAS_MMC_IsAvailTimerExpiredTriggerBgHistorySearch_L1Main())
    {
    ...
    {
        /* 如果当前第一阶段总控定时器不在运行，如在第二阶段搜网，为降低功耗，
           HISTORY搜时不用发LIST搜网，继续启动定时器,到PREFBAND搜时，则触发list搜 */
...
VOS_UINT32 NAS_MMC_ProcTiAvailableTimerExpired_NormalSearch_L1Main(VOS_VOID)
{
    ...
    /* 是否需要做BG history搜，如果需要，bg history搜失败再做list full band搜 */
    if (VOS_TRUE == NAS_MMC_IsAvailTimerExpiredTriggerBgHistorySearch_L1Main())
    {
...
)
{
    ...
    /* 如果未搜索完成，则需要在处理完ABORT或FAIL后再找机会执行背景搜 */
    ...
    {
        /* 如果已经搜索完成，则不再启动尝试定时器，背景搜列表解锁，本次搜索按失败处理 */
    ...
    /* 若状态机退出结果为NAS_MMC_BG_PLMN_SEARCH_FAIL，有以下几种情况:
       1、无下一个CSG网络且回VPLMN失败； 2、注册被拒导致卡无效；
...
)
{
    ...
    switch(pstBgPlmnSearchRsltCnfMsg->enRslt)
    {
        ...
        /* 如果状态机退出结果为 NAS_MMC_BG_PLMN_SEARCH_SUCC，
           表明没有下一个可搜索高优先级网络且驻留在高优先级网络或vplmn上的情况，
           bg history场景如果不提供正常服务不需要搜网 */
...
)
{
    ...
    /* 如果当前驻留网络被#12拒绝或ACCESS BAR， 则不需要搜网，也不需要启available定时器 */
    if ( (NAS_MMC_ADDITIONAL_ACTION_CELL_SELECTION == enPrioAdditionalAction)
    ...
    switch(pstBgPlmnSearchRsltCnfMsg->enBgPlmnSearchScene)
    {
        case NAS_MMC_BG_PLMN_SEARCH_SCENE_AVAILABLE_TIMER_EXPIRED_HISTORY_PLMN_SEARCH:
            ...
            /* 非正常服务不需要搜网,同bg搜失败不需要搜网的处理相同,启available定时器 */
            if (VOS_FALSE == NAS_MMC_IsNormalServiceStatus())
            {
                NAS_MMC_RcvBgPlmnSearchRsltCnfFailNoTrigPlmnSrch_OnPlmn(pstBgPlmnSearchRsltCnfMsg->enBgPlmnSearchScene,
                                                                        pstBgPlmnSearchRsltCnfMsg->astSearchedPlmnListInfo);
...
)
{
    ...
    if (VOS_FALSE == NAS_MMC_IsNeedAnycell_OnPlmn())
    {
        ...
        /* bg history场景，驻留态，不需要搜网，启available定时器 */
        if ((NAS_MMC_AS_CELL_CAMP_ON == NAS_MMC_GetAsCellCampOn())
