...
VOS_UINT8 NAS_GMM_IsCcoInStateRauInit(VOS_VOID)
{
    VOS_UINT8                 ucIsCcoInStateRauInit;

    ucIsCcoInStateRauInit = VOS_FALSE;
    ...
    {
        ucIsCcoInStateRauInit = VOS_TRUE;
    }

    return ucIsCcoInStateRauInit;
}

VOS_UINT8 NAS_GMM_IsInterSysProcWcdmaHandoverToLte(VOS_VOID)
{
...
VOS_VOID NAS_GMM_ProcInterruptedDetachReqRcvSuspendInd_DeregInit(VOS_VOID* pRcvMsg)
{
    MMCGMM_SUSPEND_IND_ST              *pstSuspendIndMsg;

    pstSuspendIndMsg = (MMCGMM_SUSPEND_IND_ST*)pRcvMsg;

    if (GMM_AGENT_USIM_AUTHENTICATION_CNF_FLG == (g_GmmReqCnfMng.ucCnfMask & GMM_AGENT_USIM_AUTHENTICATION_CNF_FLG))
    {
        Gmm_TimerStop(GMM_TIMER_PROTECT);
        g_GmmReqCnfMng.ucCnfMask &= ~GMM_AGENT_USIM_AUTHENTICATION_CNF_FLG;
    }

    /* 注册过程中收到Detach Req，GSM假流程 RRMM_EST_CNF会导致异系统后PS域不会再发起注册，因此需要Local Detach */
    if ((VOS_FALSE == NAS_MML_GetPsAttachAllowFlg())
     || (NAS_MML_NET_RAT_TYPE_LTE == pstSuspendIndMsg->ucSuspendDestination))
    {
...
VOS_VOID NAS_GMM_ProcInterRatCellReselectFlg(NAS_MML_NET_RAT_TYPE_ENUM_UINT8 enCurrNetType)
{
    if ((NAS_MML_NET_RAT_TYPE_WCDMA == enCurrNetType)
     && (gstGmmSuspendCtrl.ucPreRat != enCurrNetType))
    {
...
VOS_VOID NAS_GMM_ProcGuResumeSucc(NAS_MML_NET_RAT_TYPE_ENUM_UINT8 enCurrNetType)
{
    ...
    if (gstGmmSuspendCtrl.ucPreRat == enCurrNetType)
    {
        ...
        /* 已经设置PsAttachAllowFlg后出现异系统失败，又回到GSM后，这时要迁状态到Dereg.NormalService */
        if (GMM_DEREGISTERED_INITIATED == gstGmmSuspendCtrl.ucPreState)
        {
            if (VOS_FALSE == ucPsAttachAllowFlg)
            {
    ...
    if (VOS_TRUE == NAS_GMM_IsDeregisterState(gstGmmSuspendCtrl.ucPreState))
    {
    ...
    else if (VOS_TRUE == NAS_GMM_IsCcoInStateRauInit())
    {
        Gmm_TimerStart(GMM_TIMER_T3330);
    }
    else if (VOS_TRUE == NAS_GMM_IsInterSysProcWcdmaHandoverToLte())
    {
    ...
    else if (GMM_DEREGISTERED_INITIATED == gstGmmSuspendCtrl.ucPreState)
    {
        GMM_ResumeTimer(GMM_TIMER_RESUME);
    ...
    NAS_GMM_ProcInterRatCellReselectFlg(enCurrNetType);
}


VOS_VOID NAS_GMM_ProcGuResumeFail(VOS_VOID)
{
    VOS_UINT8                 ucPsAttachAllowFlg;
...
VOS_VOID NAS_GMM_ProcGuResumeResultGrpsSuspension(NAS_MML_NET_RAT_TYPE_ENUM_UINT8 enCurrNetType)
{
    VOS_UINT8                 ucPsAttachAllowFlg;
...
VOS_VOID GMM_RcvMmcSuspendInd(VOS_VOID* pRcvMsg)
{
    ...
    switch (g_GmmGlobalCtrl.ucState)
    {
        ...
        case GMM_DEREGISTERED_INITIATED:
            NAS_GMM_ProcInterruptedDetachReqRcvSuspendInd_DeregInit(pSuspendIndMsg);
            break;
        default:
            break;
    }
...
VOS_VOID GMM_RcvMmcResumeInd(VOS_VOID* pRcvMsg)
{
    ...
    switch (pResumeIndMsg->ucPsResumeResult)
    {
        case MMC_RESUME_RESULT_SUCCESS:
            NAS_GMM_ProcGuResumeSucc(enCurrNetType);
            break;

        case MMC_RESUME_RESULT_FAILURE:
            NAS_GMM_ProcGuResumeFail();
            break;

        case MMC_RESUME_GPRS_SUSPENSION:
            NAS_GMM_ProcGuResumeResultGrpsSuspension(enCurrNetType);
            break;
...
VOS_VOID GMM_ResumeSuccess()
{
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_GMM, "GMM_ResumeSuccess: T3212 expired trigger RAU, ");
...
VOS_VOID NAS_GMM_ProcResumeToLTE(VOS_VOID)
{
    ...
    if ((VOS_TRUE == NAS_GMM_IsDeregisterState(gstGmmSuspendCtrl.ucPreState))
     || (GMM_DEREGISTERED_INITIATED == gstGmmSuspendCtrl.ucPreState))
    {
