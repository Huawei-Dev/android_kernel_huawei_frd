...
)
{
    ...
    if (i >= NAS_MMC_MAX_TIMER_NUM)
    {
        NAS_INFO_LOG1(WUEPS_PID_MMC,"NAS_MMC_GetRemainTimerLen: Timer already stopped!", (VOS_INT32)enTimerId);
        *pulRemainTimeLen = 0;
        return;
    }
    ...
    if (VOS_NULL_PTR == pstMmcTimerCtx[i].hTimer)
    {
        NAS_INFO_LOG1(WUEPS_PID_MMC,"NAS_MMC_GetRemainTimerLen: Timer handle is null!", (VOS_INT32)enTimerId);
        *pulRemainTimeLen = 0;
        return;
    }

    if (VOS_OK != VOS_GetRelTmRemainTime(&(pstMmcTimerCtx[i].hTimer), &ulRemainTick))
    {
        NAS_INFO_LOG1(WUEPS_PID_MMC,"NAS_MMC_GetRemainTimerLen: Get remain time fail!", (VOS_INT32)enTimerId);
        *pulRemainTimeLen = 0;
        return;
    }

    *pulRemainTimeLen = ulRemainTick * TI_NAS_MMC_1_TICK_10_MS;
    return;
}
