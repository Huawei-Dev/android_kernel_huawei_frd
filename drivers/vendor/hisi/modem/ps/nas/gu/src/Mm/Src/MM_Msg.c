                               ...
                               )
{
    ...
    for ( i = MM_CONST_NUM_0; i < MM_CONST_NUM_2; i++ )
    {
        for ( j = MM_CONST_NUM_0; j < MM_CONST_NUM_7; j++ )
        {
            ...
            if ( MM_CONST_NUM_0 != (g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].aucMMConnExtFlg[i]
                                        & (0x01 << j)) )
            {
                /* 统计MM连接的个数 */
                ucCnt++;
                                  ...
                                  )
{
    ...
    if ( (MM_TRUE != Mm_ComMmConnExist())
      && (NO_MM_CONN_ESTING != g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_CC].ucMMConnEstingTI)
      && (MM_TRUE == g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_CC].ucEstingCallTypeFlg) )
    {
...
)
{
    ...
    while ( usIndex < usMsgSize )
    {
        ...
        {
            NAS_NORMAL_LOG(WUEPS_PID_MMC, "Mm_ComMsgMmInfoRcv:Operator name in MM INFO diff from NV, need write NV");
             ...
             && (NAS_NVIM_MAX_OPER_LONG_NAME_LEN > (pstCurrOperatorName->aucOperatorNameLong[0])))
            {
                ...
                {
                    NAS_WARNING_LOG(WUEPS_PID_MMC, "Mm_ComMsgMmInfoRcv:Write NV fail.");
                }
            }
            else
            {
                NAS_WARNING_LOG(WUEPS_PID_MMC, "Mm_ComMsgMmInfoRcv: Operator name length out of range.");
            }
        }
...
VOS_UINT32  MM_ComCheckNoOtherMmConnection()
{
    if ( (MM_CONST_NUM_0 ==  g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_SMS].aucMMConnExtFlg[0])
      ...
      && (MM_CONST_NUM_0 ==  g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].aucMMConnExtFlg[0])
      && (MM_CONST_NUM_0 ==  g_MmGlobalInfo.ConnCtrlInfo[MM_CONN_CTRL_LCS].aucMMConnExtFlg[1])
                        ...
                        )
{
    ...
    else
    {
        ...
        case MM_IE_PD_NON_CALL_RLT_SS_MSG:                                          /* SS的消息                                 */
            if (LCS_MM_LCS_NW_MSG == NAS_LCS_IsLcsNwMsg(pMsg->RcvNasMsg.aucNasMsg, pMsg->RcvNasMsg.ulNasMsgSize))
            {
                ucEventId = MM_EVENT_ID_LCS_MESSAGE;
            }
            else
            {
                               ...
                               )
{
    ...
    else
    {
                                                ...
                                                &pMsg->MmIeMobileId.MobileID) )
        {
            ...
            if ( (RRC_REL_CAUSE_RR_DRIECT_SIGN_CONN_EST == NAS_MML_GetPsRelCause())
              && (NAS_MML_NET_RAT_TYPE_WCDMA == NAS_MML_GetCurrNetRatType()) )
            {
                Mm_SndRrEstReq( RRC_EST_CAUSE_CALL_RE_ESTABLISH, MM_FALSE,
                                                    usMsgSize, pucSndMsg );
            }
...
{
    if (VOS_TRUE == NAS_MML_IsNeedClearTmsiPtmsi())
    {
        ...
        NAS_WARNING_LOG(WUEPS_PID_MM, "NAS_MM_RcvMmcPowerOffNotify: delete TMSI" );
    }

    if (MM_STATE_NULL == NAS_MM_GetMmState())
    {
        NAS_WARNING_LOG(WUEPS_PID_MM, "NAS_MM_RcvMmcPowerOffNotify: MM state is NULL" );
        return;
    }
...
{
    MMCMM_IMS_SRV_INFO_NOTIFY_STRU     *pstImsSrvInfo;

    pstImsSrvInfo = (MMCMM_IMS_SRV_INFO_NOTIFY_STRU *)pstRcvMsg;

    if (MN_CALL_TYPE_EMERGENCY != pstImsSrvInfo->stImsCallInfo.enCallType)
    {
        NAS_MML_SetImsNormalCallExistFlg(pstImsSrvInfo->stImsCallInfo.ucImsCallFlg);
    }

    if ((VOS_TRUE == pstImsSrvInfo->stImsCallInfo.ucImsCallFlg)
     && (MN_CALL_TYPE_EMERGENCY == pstImsSrvInfo->stImsCallInfo.enCallType))
    {
        /* IMS呼叫存在，并且calltype是紧急呼，则设置IMS紧急呼存在标志 */
        NAS_MML_SetImsEmcCallExistFlg(VOS_TRUE);
        ...
        if (0 != NAS_MML_GetTimerLenOfDelayNetworkSearch())
        {
    ...
    if ((VOS_TRUE == NAS_MML_GetImsEmcCallExistFlg())
     && (VOS_FALSE == pstImsSrvInfo->stImsCallInfo.ucImsCallFlg))
    {
        /* IMS呼叫结束，清除IMS紧急呼存在标志 */
        NAS_MML_SetImsEmcCallExistFlg(VOS_FALSE);
...
VOS_VOID NAS_MM_RcvSysInfoAccessBar_ProcBufferedCsService(VOS_VOID)
{
     ...
     && (VOS_FALSE == ucRestrictNormalService))
    {
        ...
        else if (NAS_MML_CSFB_SERVICE_STATUS_MO_NORMAL_LCS_EXIST == enCsfbServiceStatus)
        {
            /* 记录delay lau标识，呼叫结束后收到系统消息需要做lau */
            NAS_MML_SetDelayedCsfbLauFlg(VOS_TRUE);
...
)
{
    ...
    if ( MMCC_SESSION_TYPE_BUTT != pstRcvMsg->enSessionType )
    {
        ...
        /* 紧急呼开启后启动运行定时器，在收到available定时器超时时候不触发缓存搜网 */
        if (RRC_NAS_SESSION_TYPE_CS_MO_EMERGENCY_CALL == enRrcSessionType)
        {
            if (0 != NAS_MML_GetTimerLenOfDelayNetworkSearch())
            {
                Mm_TimerStart(MM_TIMER_DELAY_NETWORK_SEARCH_AFTER_EMC);
...
)
{
    ...
    if ( MMCC_SESSION_TYPE_BUTT != pstRcvMsg->enSessionType )
    {
        ...
        /* 紧急呼结束后启不搜网定时器等待紧急呼回呼 */
        if (RRC_NAS_SESSION_TYPE_CS_MO_EMERGENCY_CALL == enRrcSessionType)
        {
            /* 如果定时器正在运行要先停止，每次紧急呼结束之后都重启定时器  */
            if (MM_TIMER_RUNNING == NAS_MM_QryTimerStatus(MM_TIMER_DELAY_NETWORK_SEARCH_AFTER_EMC) )
            {
                Mm_TimerStop(MM_TIMER_DELAY_NETWORK_SEARCH_AFTER_EMC);
            }
...
)
{
    ...
    if ( (VOS_TRUE == NAS_MML_GetMoCallSetupFlg())
     && (VOS_FALSE == NAS_MML_GetMoSsSetupFlg()) )
    {
...
)
{
    ...
    if ( (VOS_TRUE == NAS_MML_GetMoSsSetupFlg())
     && (VOS_FALSE == NAS_MML_GetMoCallSetupFlg()) )
    {
