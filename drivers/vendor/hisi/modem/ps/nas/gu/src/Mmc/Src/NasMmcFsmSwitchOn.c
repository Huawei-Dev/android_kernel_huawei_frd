...
)
{
    ...
    if ( VOS_TRUE == NAS_MML_GetSimPresentStatus())
    {
        ...
        {
            NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvStartReq_SwitchOn_Init: imsi change is true");

            NAS_TRACE_HIGH("NAS_MMC_RcvStartReq_SwitchOn_Init: imsi change is true");
            ...
            /* IMSI发生变更，清除前面从NV中读取的duplicate Rplmn和Allowed List */
            NAS_MMC_InitDuplicateRplmnAndAllowedList_SwitchOn();
#endif
        }
...
VOS_VOID  NAS_MMC_ProcReadUsimFilesComplete_SwitchOn(VOS_VOID)
{
    NAS_MML_SIM_MS_IDENTITY_STRU       *pstMsIdentity       = VOS_NULL_PTR;
    ...
    NAS_MMC_InsertNvimSupplementOplmnInfo(NAS_MML_GetCustomSupplementOplmnInfo());
...
VOS_UINT32  NAS_MMC_RcvUsimQueryFileRsp_SwitchOn_WaitSimFilesCnf(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    ...
    NAS_MMC_RcvAgentUsimQueryCsgFileCnf(pstRcvMsg, VOS_FALSE);
    ...
    NAS_MMC_ProcReadUsimFilesComplete_SwitchOn();
...
)
{
    ...
    PS_MEM_SET(aucImsi,0xFF,sizeof(aucImsi));
...
)
{
    ...
    if (MMC_START_FAILURE == pstRrStartCnfMsg->ulResult)
    {
        ...
        NAS_MML_SoftReBoot(NAS_MML_REBOOT_SCENE_MMC_WAIT_GAS_START_CNF_FAIL);
...
)
{
    ...
    NAS_MML_SoftReBoot(NAS_MML_REBOOT_SCENE_MMC_WAIT_GAS_START_CNF_EXPIRED);
...
)
{
    ...
    if (MMC_LMM_SUCC != pstRrStartCnfMsg->ulRst)
    {
        ...
        NAS_MML_SoftReBoot(NAS_MML_REBOOT_SCENE_MMC_WAIT_LMM_START_CNF_FAIL);
...
)
{
    ...
    NAS_MML_SoftReBoot(NAS_MML_REBOOT_SCENE_MMC_WAIT_LMM_START_CNF_EXPIRED);
...
)
{
    ...
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_IsSimRplmnInNvRplmn_SwitchOn: ulExistSamePlmn is;",ulExistSamePlmn);
...
VOS_VOID NAS_MMC_UpdateNvRplmnWithSimRplmn_SwitchOn(VOS_VOID)
{
    ...
    {
        ulRplmnExistFlg = NAS_MML_GetPrefRPlmn(NAS_MML_NET_RAT_TYPE_LTE, &stRplmn);
    }
    else
#endif
    {
     ...
     || (VOS_FALSE  == ulRplmnExistFlg))
    {
        ...
        NAS_NORMAL_LOG2(WUEPS_PID_MMC, "NAS_MMC_UpdateNvRplmnWithSimRplmn_SwitchOn: imsi change is;ulRplmnExistFlg is;  G/W RPLMN in nv is initiated",NAS_MMC_IsImsiChange_SwitchOn(),ulRplmnExistFlg);
    ...
    for ( i = 0 ; i < pstPrioRatList->ucRatNum; i++ )
    {
        ...
        if ((VOS_FALSE  == NAS_MML_IsPlmnIdValid(pstNVRplmn))
         ...
         || (VOS_FALSE  == pstRplmnCfgInfo->ucMultiRATRplmnFlg))
        {
            NAS_NORMAL_LOG2(WUEPS_PID_MMC, "NAS_MMC_UpdateNvRplmnWithSimRplmn_SwitchOn: ulExistSamePlmn is;ucMultiRATRplmnFlg is;  use Usim RPLMN ",ulExistSamePlmn,pstRplmnCfgInfo->ucMultiRATRplmnFlg);
...
)
{
    ...
    {
        NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_UpdateLastRplmnRat_LastRplmnRatInvalid_SwitchOn pstRplmnCfgInfo->enLastRplmnRat IS ;",pstRatOrder->aucRatPrio[0]);
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_UpdateLastRplmnRat_LastRplmnRatInvalid_SwitchOn: last rplmn rat is lte");
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MML_IsWcdmaPrioGsm is true and enLastRplmnRat is WCDMA");
...
VOS_VOID NAS_MMC_UpdateEplmn_SwitchOn(VOS_VOID)
{
    ...
    if  (VOS_FALSE == NAS_MMC_GetLastRplmn_SwitchOn(&stTmpPlmn))
    {
        ...
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_UpdateEplmn_SwitchOn: eplmn is setted and valid is true");
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_UpdateEplmn_SwitchOn: rplmn is not in eplmn list");
...
VOS_VOID NAS_MMC_InitUserSpecPlmnId_SwitchOn(VOS_VOID)
{
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_GetLastRplmn_SwitchOn: is false");
        ...
        NAS_NORMAL_LOG3(WUEPS_PID_MMC, "stUserPlmn MCC IS ;MNC IS; ucLastRplmnExistFlag IS;",stUserPlmn.stPlmnId.ulMcc,stUserPlmn.stPlmnId.ulMnc,ucLastRplmnExistFlag);
    ...
    {
        NAS_NORMAL_LOG2(WUEPS_PID_MMC, "NAS_MMC_InitUserSpecPlmnId_SwitchOn pstUserPlmnInfo MCC IS ;MNC IS; ",pstUserPlmnInfo->stPlmnId.ulMcc,pstUserPlmnInfo->stPlmnId.ulMnc);
    ...
    NAS_NORMAL_LOG2(WUEPS_PID_MMC, "NAS_MMC_SetUserSpecPlmnId: MCC IS ;MNC IS;",stUserPlmn.stPlmnId.ulMcc,stUserPlmn.stPlmnId.ulMnc);
...
)
{
    ...
    switch(pstRplmnCfgInfo->enLastRplmnRat)
    {
        ...
        case NAS_MML_NET_RAT_TYPE_GSM:
            /* 获取GU的RPLMN */
            ulRplmnExistFlg   = NAS_MML_GetPrefRPlmn(NAS_MML_NET_RAT_TYPE_WCDMA, &stRplmn);

            NAS_NORMAL_LOG2(WUEPS_PID_MMC, "NAS_MMC_GetLastRplmn_SwitchOn ulRplmnExistFlg IS ;pstRplmnCfgInfo->enLastRplmnRat IS; ",ulRplmnExistFlg,pstRplmnCfgInfo->enLastRplmnRat);

            if (VOS_FALSE == ulRplmnExistFlg)
            {
                return VOS_FALSE;
            }

            pstLasRplmn->stPlmnId.ulMcc = stRplmn.ulMcc;
            pstLasRplmn->stPlmnId.ulMnc = stRplmn.ulMnc;
...
{
    NAS_MMC_RcvStartCnf_SwitchOn_ForbiddenCustImsRatCapability();
    ...
    if (VOS_TRUE == NAS_MML_IsPlatformSupportLte())
    {
        ...
        {
#if (FEATURE_ON == FEATURE_ECALL)
            NAS_MMC_RcvStartCnf_SwitchOn_ForbiddenEcallOnlyLteCapability();
#endif
        }
    }
#endif
...
VOS_VOID NAS_MMC_RcvStartCnf_SwitchOn_ForbiddenCustImsRatCapability (VOS_VOID)
{
...
VOS_VOID NAS_MMC_RcvStartCnf_SwitchOn_ForbiddenEcallOnlyLteCapability (VOS_VOID)
{
    if (NAS_MML_ECALL_ONLY == NAS_MML_GetCallMode())
    {
        /* Ecall Only时，前面已经将PS置为不可用 */
...
VOS_VOID NAS_MMC_ReadNvimInfo_SwitchOn()
{
    ...
    NAS_MMC_ReadCause18EnableLteSupportFlgNvim();
    ...
    NAS_MMC_ReadOosBgNetworkSearchCustom();

    NAS_MMC_ReadOosPlmnSearchStrategyCfgNvim();

    NAS_MMC_ReadExtendT3240LenCfg();
    ...
    NAS_MMC_ReadAttEnsCustomNvim();

    NAS_MMC_ReadCustomSupplementOplmnInfoNvim();

    NAS_MMC_ReadAiModemCfgNvim();

    NAS_MMC_ReadRefreshAllFileRestartFlagNvim();
...
VOS_VOID NAS_MMC_InitLastRplmnRat_SwitchOn(VOS_VOID)
{
    ...
    if (VOS_TRUE   == NAS_MMC_IsImsiChange_SwitchOn())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_InitLastRplmnRat_SwitchOn: imsi change and senLastRplmnRat is BUTT");
...
VOS_VOID NAS_MMC_InitDuplicateRplmnAndAllowedList_SwitchOn(VOS_VOID)
{
    VOS_UINT32                                              i;
    ...
    /* 初始化duplicate Rplmn和Eplmn为空 */
    NAS_MMC_InitCsgSpecSrchDuplicateRplmnAndEplmnInfo();

    /* 初始化Allowed List列表为空 */
    PS_MEM_SET(pstAllowedCsgList, 0, sizeof(NAS_MML_PLMN_WITH_CSG_ID_LIST_STRU));

    for (i = 0; i < NAS_NVIM_MAX_PLMN_CSG_ID_NUM; i++)
    {
        pstAllowedCsgList->astPlmnWithCsgIdInfo[i].stPlmnWithCsgId.stPlmnId.ulMcc = NAS_MML_INVALID_MCC;
        pstAllowedCsgList->astPlmnWithCsgIdInfo[i].stPlmnWithCsgId.stPlmnId.ulMnc = NAS_MML_INVALID_MNC;
        pstAllowedCsgList->astPlmnWithCsgIdInfo[i].enCsgDisplayIndicator          = NAS_MML_CSG_ID_DISPLAY_IND_BUTT;
    }
...
VOS_UINT32 NAS_MMC_IsSimCardUsedOtherUe_SwitchOn(VOS_VOID)
{
    ...
    pulPtmsiValue = (VOS_UINT32*)aucPtmsi;
    pulTmsiValue  = (VOS_UINT32*)aucTmsi;

    NAS_INFO_LOG2(WUEPS_PID_MMC,"NAS_MMC_IsSimCardUsedOtherUe_SwitchOn. SIM Ptmsi. Tmsi",*pulPtmsiValue,*pulTmsiValue);

    pulPtmsiValue = (VOS_UINT32*)NAS_MMC_GetLastPtmsi_SwitchOn();
    pulTmsiValue  = (VOS_UINT32*)NAS_MMC_GetLastTmsi_SwitchOn();

    NAS_INFO_LOG2(WUEPS_PID_MMC,"NAS_MMC_IsSimCardUsedOtherUe_SwitchOn. nv Ptmsi. Tmsi",*pulPtmsiValue,*pulTmsiValue);
