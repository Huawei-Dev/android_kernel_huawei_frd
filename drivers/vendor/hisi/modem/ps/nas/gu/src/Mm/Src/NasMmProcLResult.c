...
)
{
    ...
    NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_RcvLmmAttachInd: Cs Not In Dam No Search Status");

    /* 如果是单独EPS注册或紧急ATTACH */
    if ( (MMC_LMM_ATT_TYPE_EPS_ONLY == pstLmmAttachInd->ulReqType)
      || (MMC_LMM_ATT_TYPE_EPS_EMERGENCY == pstLmmAttachInd->ulReqType) )
    {
...
)
{
    ...
    NAS_MML_SetCsDamNoSearchFlag(VOS_FALSE);
    NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_RcvLmmTauInd: Cs Not In Dam No Search Status");
    ...
    if (MMC_LMM_CELL_STATUS_NORMAL == pstLmmSysInfo->stLteSysInfo.enCellStatusInd)
    {
        ...
        if (VOS_FALSE == NAS_MML_IsPlmnSupportDam(NAS_MML_GetCurrCampPlmnId()))
        {
            if (NAS_MML_TIN_TYPE_RAT_RELATED_TMSI == NAS_MML_GetTinType())
            {
                ...
                NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_RcvLmmSysInfoInd: ISR active, Stop other MM timer except T3212 and CSFB MT PAGING timer");
...
)
{
    ...
    if (VOS_TRUE   == NAS_MML_IsImsCallExist())
    {
        return MM_EVENT_ID_INVALID;
    }
    ...
    NAS_MML_SetCsfbMtPagingTimerStatus(NAS_MML_TIMER_STATUS_RUNNING);
