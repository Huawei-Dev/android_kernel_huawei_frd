...
#ifdef DMT
        ulReturnCode = VOS_RegisterPIDInfo(WUEPS_PID_LCS,
                                    (Init_Fun_Type)NAS_LCS_PidInit,
                                    (Msg_Fun_Type)NAS_LCS_TaskEntry);
        if( VOS_OK != ulReturnCode )
        {
            return VOS_ERR;
        }
...
VOS_VOID NAS_CC_ProcessSpecTypeBufferMsg(VOS_UINT32 ulEventType)
{
    ...
    while (VOS_TRUE == NAS_CC_GetNextSpecEventTypeCachedMsg(ulEventType, pstEntryMsg))
    {
        ...
        /* 异常保护，如果循环达到了最大次就不再循环，避免死循环 */
        if (ucCount >= NAS_CC_MAX_MSG_QUEUE_NUM)
        {
            break;
        }
