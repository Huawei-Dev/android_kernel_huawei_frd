...
VOS_VOID NAS_CC_TiNonRegularProc(VOS_UINT8 *pucCcMsg)
{
    ...
    if (VOS_FALSE == NAS_CC_GetTiNonregularCustom())
    {
        NAS_WARNING_LOG(WUEPS_PID_CC, "NAS_CC_TiNonRegularProc: TiNonregularCustom close.");

        return;
    }

    /* 不是仪器测试出错场景，不改TI值 */
    if (VOS_FALSE == NAS_CC_GetTiNonregularAddedFlg())
    {
        NAS_WARNING_LOG(WUEPS_PID_CC, "NAS_CC_TiNonRegularProc: TiNonregularAddedFlg is false.");
    ...
    for (i = 0; i < NAS_CC_MAX_ENTITY_NUM; i++)
    {
        if (VOS_TRUE == f_astCcEntities[i].bUsed)
        {
            if (1 == ((f_astCcEntities[i].ucTi >> 3) & 0x1))
            {
                ucMtNum++;
            }
            else
      ...
      && (1 == ucMoNum) )
    {
        if (1 == ucTi)
        {
            ...
            NAS_WARNING_LOG(WUEPS_PID_CC, "NAS_CC_TiNonRegularProc: change ti from 1 to 0.");
        }
    }
