...
)
{
    ...
    {                                                                           /* 申请 实体,PDP Addr,Ti成功                */
        ...
        {                                                                       /* APN存在                                  */
            stJudgmentPara.ucApnLen = (VOS_UINT8)pstMsg->stUnpackApn.ulApnLen;
            PS_MEM_CPY(stJudgmentPara.aucApnName,
                       pstMsg->stUnpackApn.aucApnAddr,
                       pstMsg->stUnpackApn.ulApnLen);

            if( NAS_BACKOFF_SUCC != NAS_BACKOFF_OperateJudgment(NAS_BACKOFF_NETRAT_GU, &stJudgmentPara) )
            {
                /* BACK-OFF算法不允许发起该APN的PDP激活请求 */
                SM_SndTafSmPdpActivateRej(ucCr,
                                          SM_TAF_CAUSE_SM_BACKOFF_ALG_NOT_ALLOWED,
                                          VOS_NULL_PTR);                        /* 回拒绝消息                               */
            ...
            g_SmPdpAddrApn.aPdpAddrlist[ucAddrIndex].stUnPackPdpApn.ulApnLength
                = pstMsg->stUnpackApn.ulApnLen;                                 /* 设定UnpackApn数据长度                 */
            PS_MEM_CPY(g_SmPdpAddrApn.aPdpAddrlist[ucAddrIndex].stUnPackPdpApn.aucApnValue,
                       pstMsg->stUnpackApn.aucApnAddr,
                       pstMsg->stUnpackApn.ulApnLen);                           /* 存储UnpackApn数据                     */
