...
)
{
    ...
    {
        NAS_INFO_LOG(WUEPS_PID_MMC, "NAS_MMC_BuildForbLaInAsSpecPlmnSearchReqMsg():ucForbRoamLaNum is 0");
        return;
    }
...
)
{
    ...
    NAS_MML_TIMER_STATUS_ENUM_UINT8                 enCsfbMtTimerStatus;
    ...
    if ( (VOS_TRUE == ucCsfbMoFlag)
      || (NAS_MML_TIMER_STATUS_RUNNING == enCsfbMtTimerStatus) )
    {
        pstMsg->enCsfbFlg = PS_TRUE;
    }
    else
...
VOS_VOID NAS_MMC_SndAsPlmnListReq(
    VOS_UINT32                          ulReceiverPid,
    RRC_PLMN_SEARCH_TYPE_ENUM_UINT32    enPlmnSearchScene
)
{
    RRMM_PLMN_SEARCH_REQ_STRU          *pstMsg = VOS_NULL_PTR;
    ...
    pstMsg->MsgHeader.ulMsgName           = RRMM_PLMN_SEARCH_REQ;
    pstMsg->MsgHeader.ulLength            = sizeof(RRMM_PLMN_SEARCH_REQ_STRU) - VOS_MSG_HEAD_LENGTH;
    pstMsg->ulSearchType                  = enPlmnSearchScene;
...
VOS_VOID NAS_MMC_SndAsBgSpecPlmnSearchReq(
    VOS_UINT32                          ulReceiverPid,
...
)
{
    ...
    if (VOS_NULL_PTR == pstMsg)
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SndAsBgSpecPlmnSearchReq: memery alloc failed.");
    ...
    pstMsg->enBgType                  = RRMM_BG_PLMN_SRCH_SPEC;

    pstMsg->ulPlmnIdNum               = 1;
    pstMsg->astPlmnIdList[0].ulMcc    = pstPlmnId->stPlmnId.ulMcc;
    pstMsg->astPlmnIdList[0].ulMnc    = pstPlmnId->stPlmnId.ulMnc;
    ...
    switch (pstPlmnId->enRat)
    {
        case NAS_MML_NET_RAT_TYPE_WCDMA:
            ...
            pstMsg->enRat = RRC_NAS_RAT_WCDMA_FDD;
            break;

        case NAS_MML_NET_RAT_TYPE_GSM:

            pstMsg->enRat = RRC_NAS_RAT_GSM;
            break;
#if (FEATURE_ON == FEATURE_LTE)
        case NAS_MML_NET_RAT_TYPE_LTE:

            pstMsg->enRat = RRC_NAS_RAT_LTE;
            break;
#endif

        default:

            NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_SndAsBgSpecPlmnSearchReq: Unexpected rat type.");
            break;
    }
    ...
    if (VOS_OK != NAS_UTRANCTRL_SndAsMsg(WUEPS_PID_MMC, (struct MsgCB **)&pstMsg))
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SndAsBgSpecPlmnSearchReq: send message failed.");
    }
...
VOS_VOID NAS_MMC_SndAsFastCsgSearchReq(
    VOS_UINT32                          ulReceiverPid,
    NAS_MML_PLMN_ID_STRU               *pstPlmnId,
...
{
    /* GU下暂不支持CSG快速指定搜，如有需要后续可在此扩展 */
    return;
}


VOS_VOID NAS_MMC_SndAsCsgBgSearchReq(
    VOS_UINT32                          ulReceiverPid,
    NAS_MML_PLMN_WITH_RAT_STRU         *pstPlmnId,
    VOS_UINT32                          ulCsgIdNum,
    VOS_UINT32                         *pulCsgId
...
{
    RRMM_CSG_BG_SEARCH_REQ_STRU         *pstMsg = VOS_NULL_PTR;

    pstMsg = (RRMM_CSG_BG_SEARCH_REQ_STRU *)PS_ALLOC_MSG_WITH_HEADER_LEN(
                                        WUEPS_PID_MMC,
                                        sizeof(RRMM_CSG_BG_SEARCH_REQ_STRU));
    if (VOS_NULL_PTR == pstMsg)
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SndAsCsgBgSearchReq: memery alloc failed.");
        return;
    }

    PS_MEM_SET((VOS_UINT8*)pstMsg + VOS_MSG_HEAD_LENGTH, 0,
                sizeof(RRMM_CSG_BG_SEARCH_REQ_STRU) - VOS_MSG_HEAD_LENGTH);

    pstMsg->MsgHeader.ulReceiverCpuId    = VOS_LOCAL_CPUID;
    pstMsg->MsgHeader.ulReceiverPid      = ulReceiverPid;

    pstMsg->MsgHeader.ulMsgName          = RRMM_CSG_BG_SEARCH_REQ;

    pstMsg->stPlmnWithRat.stPlmnId.ulMcc = pstPlmnId->stPlmnId.ulMcc ;
    pstMsg->stPlmnWithRat.stPlmnId.ulMnc = pstPlmnId->stPlmnId.ulMnc ;

    /* 当前只支持搜1个CSG ID，但不排除以后可能支持多个 */
    pstMsg->ulCsgIdNum                   = NAS_MML_MIN(ulCsgIdNum, RRC_NAS_MAX_PLMN_CSG_ID_NUM);
    ...
    switch (pstPlmnId->enRat)
    {
        case NAS_MML_NET_RAT_TYPE_WCDMA:
            ...
            pstMsg->stPlmnWithRat.ulRat = RRC_NAS_RAT_WCDMA_FDD;
            break;

        case NAS_MML_NET_RAT_TYPE_GSM:

            pstMsg->stPlmnWithRat.ulRat = RRC_NAS_RAT_GSM;
            break;
#if (FEATURE_ON == FEATURE_LTE)
        case NAS_MML_NET_RAT_TYPE_LTE:

            pstMsg->stPlmnWithRat.ulRat = RRC_NAS_RAT_LTE;
            break;
#endif

        default:

            NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_SndAsCsgBgSearchReq: Unexpected rat type.");
            break;
    }

    if (VOS_OK != NAS_UTRANCTRL_SndAsMsg(WUEPS_PID_MMC, (struct MsgCB **)&pstMsg))
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SndAsCsgBgSearchReq: send message failed.");
    }
...
VOS_VOID NAS_MMC_SndAsBgHistoryPlmnSearchReq(
    VOS_UINT32                          ulReceiverPid,
...
)
{
    ...
    /* 申请内存  */
    pstMsg = (RRMM_BG_PLMN_SEARCH_REQ_ST *)NAS_MMC_ALLOC_MSG_WITH_HDR(sizeof(RRMM_BG_PLMN_SEARCH_REQ_ST));
    if ( VOS_NULL_PTR == pstMsg )
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SndAsBgHistoryPlmnSearchReq:ERROR: Memory Alloc Error for pMsg");
        return;
    }
    ...
    NAS_MMC_CFG_MSG_HDR(pstMsg,
                        ulReceiverPid,
                        RRMM_BG_PLMN_SEARCH_REQ,
                        sizeof(RRMM_BG_PLMN_SEARCH_REQ_ST) - VOS_MSG_HEAD_LENGTH);


    /* 消息内容 */
    pstMsg->enBgType                  = RRMM_BG_PLMN_SRCH_HISTORY;

    pstMsg->ulPlmnIdNum               = NAS_MML_MIN(pstPlmnIdList->ulPlmnNum, RRC_MAX_EQUPLMN_NUM) ;

    for ( ulIndex = 0 ; ulIndex < pstMsg->ulPlmnIdNum; ulIndex++ )
    {
        pstMsg->astPlmnIdList[ulIndex].ulMcc          = pstPlmnIdList->astPlmnId[ulIndex].stPlmnId.ulMcc;
        pstMsg->astPlmnIdList[ulIndex].ulMnc          = pstPlmnIdList->astPlmnId[ulIndex].stPlmnId.ulMnc;
    }

    /* 把MMC内部的RAT转换为和AS接口中的类型 */
    switch (pstPlmnIdList->astPlmnId[0].enRat)
    {
        case NAS_MML_NET_RAT_TYPE_WCDMA:
        ...
        default:
            ...
            NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SndAsBgHistoryPlmnSearchReq: Unexpected rat type.");
            break;
    }
    ...
    if (VOS_OK != NAS_UTRANCTRL_SndAsMsg(WUEPS_PID_MMC, (struct MsgCB **)&pstMsg))
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SndAsBgHistoryPlmnSearchReq: send message failed.");
    }
