...
)
{
    ...
    pstInternalMsg->enListPlmnSearchScene     = enListPlmnSearchScene;

    NAS_MMC_InitSearchedPlmnListInfo(pstInternalMsg->astInterPlmnSearchInfo);

    for (i = 0; i < NAS_MML_MIN(ucRatNum, NAS_MML_MAX_RAT_NUM); i++)
    {
        PS_MEM_CPY(&(pstInternalMsg->astInterPlmnSearchInfo[i]), &pstSearchedPlmnListInfo[i], sizeof(NAS_MMC_SEARCHED_PLMN_LIST_INFO_STRU));
    }
...
)
{
    ...
    /* 从内部消息队列中获取一个还没有使用的空间 */
    ulLen           = sizeof(NAS_MMC_INTER_BG_SEARCH_REQ_STRU);

    pstInternalMsg  = (NAS_MMC_INTER_BG_SEARCH_REQ_STRU*)NAS_MML_GetIntMsgSendBuf(ulLen);
    if ( VOS_NULL_PTR == pstInternalMsg)
    {
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_SndInterBgSearchReq:pstInternalMsg IS VOS_NULL_PTR ");
        return;
    }

    PS_MEM_SET((VOS_UINT8*)pstInternalMsg + VOS_MSG_HEAD_LENGTH, 0,
              sizeof(NAS_MMC_INTER_BG_SEARCH_REQ_STRU) - VOS_MSG_HEAD_LENGTH);

    pstInternalMsg->MsgHeader.ulSenderCpuId   = VOS_LOCAL_CPUID;
    pstInternalMsg->MsgHeader.ulSenderPid     = WUEPS_PID_MMC;
    pstInternalMsg->MsgHeader.ulReceiverCpuId = VOS_LOCAL_CPUID;
    pstInternalMsg->MsgHeader.ulReceiverPid   = WUEPS_PID_MMC;
    pstInternalMsg->MsgHeader.ulMsgName       = MMCMMC_INTER_BG_SEARCH_REQ;
    pstInternalMsg->MsgHeader.ulLength        = sizeof(NAS_MMC_INTER_BG_SEARCH_REQ_STRU) - VOS_MSG_HEAD_LENGTH;
...
)
{
    ...
    {
        /* 异常打印 */
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_SndBgPlmnSearchRslt:pstBgRsltCnf IS VOS_NULL_PTR ");
    ...
    /* 填充消息内容 */
    pstInternalMsg->enRslt                      = pstBgRsltCnf->enRslt;
    pstInternalMsg->ulNeedPlmnSearch            = pstBgRsltCnf->ulNeedPlmnSearch;
    pstInternalMsg->ulSearchCompleted           = pstBgRsltCnf->ulSearchCompleted;

    pstInternalMsg->enBgPlmnSearchScene         = pstBgRsltCnf->enBgPlmnSearchScene;

    pstInternalMsg->enFsmBgSearchType           = pstBgRsltCnf->enFsmBgSearchType;

    pstInternalMsg->enCsRegAdditionalAction     = pstBgRsltCnf->enCsRegAdditionalAction;
    pstInternalMsg->enPsRegAdditionalAction     = pstBgRsltCnf->enPsRegAdditionalAction;

    /* 填充已搜索到的网络信息 */
    PS_MEM_CPY(pstInternalMsg->astSearchedPlmnListInfo, pstBgRsltCnf->astSearchedPlmnListInfo,
...
)
{
    ...
    RRMM_SUSPEND_IND_ST                *pstEntryMsg     = VOS_NULL_PTR;
    VOS_UINT32                          ulLen;

    pstEntryMsg     = (RRMM_SUSPEND_IND_ST*)(NAS_MMC_GetCurrEntryMsg()->aucEntryMsgBuffer);
    ...
    pstMsg->enSuspendDestination        = pstEntryMsg->ucSuspendDestination;

    /* F态和非F态出服务区搜网时，接入层都会上报列表 */
    if (NAS_MMC_SUSPEND_FAIL_CAUSE_AREA_LOST  == enSuspendFailCause)
    {
        if (VOS_NULL_PTR != pstListInfo)
        {
            /* 根据消息中携带的搜网信息更新搜网列表 */
...
)
{
    ...
    NAS_TRACE_HIGH("enGetGeoRslt is %u, ulMcc is %x ;ulMnc is %x ", enGetGeoRslt, pstMsg->stGetGeoPlmn.ulMcc, pstMsg->stGetGeoPlmn.ulMnc);
