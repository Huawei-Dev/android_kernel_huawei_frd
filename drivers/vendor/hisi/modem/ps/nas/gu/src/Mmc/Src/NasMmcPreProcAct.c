...
*****************************************************************************/
#if 1
extern  VOS_UINT8 g_RefreshFileContent[];
#endif
    ...
    if (VOS_TRUE == NAS_MMC_IsNeedDisableLte_ModeChange())
    {
        ...
        else
        {
            ...
            if (VOS_FALSE == ulDisableLteRoamFlg)
            {
                ...
                NAS_WARNING_LOG1(WUEPS_PID_MMC, "NAS_MMC_RcvMsccModeChange_PreProc:Reason.", NAS_MML_GetDisableLteReason());
...
VOS_VOID NAS_MMC_DeleteEplmnActingHplmnChange_PreProc(VOS_VOID)
{
    NAS_MML_EQUPLMN_INFO_STRU          *pstEquPlmnInfo      = VOS_NULL_PTR;
    NAS_MML_PLMN_ID_STRU               *pstCurrPlmnId       = VOS_NULL_PTR;
    NAS_MML_EQUPLMN_INFO_STRU           stSndEquPlmnInfo;

    PS_MEM_SET(&stSndEquPlmnInfo, 0x0, sizeof(stSndEquPlmnInfo));
    ...
    pstEquPlmnInfo->astEquPlmnAddr[0].ulMcc = pstCurrPlmnId->ulMcc;
    pstEquPlmnInfo->astEquPlmnAddr[0].ulMnc = pstCurrPlmnId->ulMnc;
...
)
{
    ...
    VOS_UINT32                              ulFileId;
    NAS_MML_PLMN_ID_STRU                    stOldHplmn;
    NAS_MML_HPLMN_TYPE_ENUM_UINT8           ucHplmnType;
    ...
    *pucBufferRefresh = VOS_FALSE;
    ...
    PS_MEM_SET(&stOldHplmn, 0x0, sizeof(NAS_MML_PLMN_ID_STRU));

    /* 如果搜索控制NV未打开，直接返回 */
    if (VOS_FALSE == NAS_MML_GetActingHplmnSupportFlg())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC,"NAS_MMC_ProcActingHplmnRefresh_PreProc():NAS_MML_GetActingHplmnSupportFlg IS FALSE");
        return;
    }

    /* 如果EHPLMN存在，即使为无效值，也不更新ACTING hplmn */
    if (PS_USIM_SERVICE_AVAILIABLE == NAS_USIMMAPI_IsServiceAvailable(NAS_USIM_SVR_EHPLMN))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC,"NAS_MMC_ProcActingHplmnRefresh_PreProc():EHPLMN exit not refresh acting hplmn");
        return;
    }
    ...
    ucHplmnType = NAS_MML_GetHPlmnType();
    if ( (NAS_MML_HPLMN_TYPE_ACTING_HPLMN == ucHplmnType)
      || (NAS_MML_HPLMN_TYPE_HPLMN        == ucHplmnType)  )
    {
        PS_MEM_CPY(&stOldHplmn, &(pstEHplmnList->astEhPlmnInfo[0].stPlmnId), sizeof(NAS_MML_PLMN_ID_STRU));
    }
    ...
    {
        /* refresh acting hplmn 成功，并且和原acting hplmn 不同 */
        if (VOS_FALSE == NAS_MML_CompareBcchPlmnwithSimPlmn(&stOldHplmn, &(pstEHplmnList->astEhPlmnInfo[0].stPlmnId)))
        {
            NAS_MMC_DeleteEplmnActingHplmnChange_PreProc();
        }
    }

    NAS_MMC_UpdateCsRegRoamState();
    NAS_MMC_UpdatePsRegRoamState();

    /* 高优先级表的类型为bg history或back up时，不能刷新 */
    if (NAS_MMC_STORE_HIGH_PRIO_PLMN_SELECTION_LIST == NAS_MMC_GetPlmnSelectionListType())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_ProcActingHplmnRefresh_PreProc: Plmn Selection List Type is HIGH_PRIO_PLMN_SELECTION_LIST");
    ...
    NAS_MMC_InitCurHighRatHplmnSleepCount();
...
VOS_UINT32  NAS_MMC_ProcActingRcvRefreshInd_PreProc(
    VOS_UINT32                          ulFileId,
...
)
{
    ...
    || (USIMM_ATTUSIM_EFACTINGHPLMN_ID == ulFileId))
     && (USIMM_GUTL_APP == enAppType))
    {
        NAS_MMC_ProcActingHplmnRefresh_PreProc(pucBufferRefresh);
        return VOS_TRUE;
    }
...
#if (FEATURE_ON == FEATURE_CSG)
    /* 如果更新的文件ID为Operator CSG List，且卡中该文件存在，则重新读卡文件 */
    if (USIMM_USIM_EFOCSGL_ID == ulFileId)
    {
        ...
        {
            /* 动态申请更新Operater Csg List使用的缓存空间 */
            if ( VOS_NULL_PTR != NAS_MML_AllocOperaterCsgListCache() )
            {
                ...
                NAS_MMC_SetCsgRefreshSimFileReadFlg(NAS_MML_READ_OPERATOR_CSG_LIST_FILE_ID_FLG);
                NAS_MMC_SetCsgRefreshSimFileReadFlg(NAS_MML_READ_OPERATOR_CSG_TYPE_FILE_ID_FLG);
                NAS_MMC_SetCsgRefreshSimFileReadFlg(NAS_MML_READ_OPERATOR_CSG_HOME_NODEB_NAME_FILE_ID_FLG);
...
)
{
    ...
    NAS_MML_SetUsimRefreshFlag(VOS_TRUE);

    pucOldImsi = NAS_MML_GetSimImsi();
    if (USIMM_API_SUCCESS == NAS_USIMMAPI_GetCardIMSI(aucNewImsi))
    {
    ...
    for (i = 0; i < pstRefreshFileContent->usEfNum; i++ )
    {
        ...
        /*  降圈复杂度，将可以直接continue的部分单独提出一个函数*/
        if(VOS_TRUE == NAS_MMC_ProcActingRcvRefreshInd_PreProc( ulFileId,
                                                                pstRefreshFileContent->astRefreshFile[i].enAppType,
                                                               &ucBufferRefreshFlg))
        {
            continue;
        }
...
VOS_UINT32 NAS_MMC_RcvUsimRefreshTypeInd_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    USIMM_STKREFRESH_TYPE_IND_STRU                    *pstUsimRefreshTypeInd;

    pstUsimRefreshTypeInd = (USIMM_STKREFRESH_TYPE_IND_STRU *)pstMsg;

    NAS_MML_SetUsimRefreshType(pstUsimRefreshTypeInd->enRefreshType);
...
)
{
    ...
    if ( VOS_TRUE == NAS_MMC_IsNeedDisableLte_DetachPs())
    {
        ...
        if (VOS_FALSE == ulDisableLteRoamFlg)
        {
            NAS_MMC_SndDisableLteNotify(MMC_LMM_DISABLE_LTE_REASON_PS_DETACH);
...
)
{
    ...
    if (NAS_MML_LTE_CAPABILITY_STATUS_DISABLE_UNNOTIFY_AS == NAS_MML_GetLteCapabilityStatus())
    {
        ...
        if (VOS_FALSE == NAS_MML_GetDisableLteRoamFlg())
        {
            ...
            NAS_WARNING_LOG1(WUEPS_PID_MMC, "NAS_MMC_RcvLmmSuspendCnf_PreProc:Reason.", NAS_MML_GetDisableLteReason());
    ...
    else
    {
        ...
        */
        ...
        {
            NAS_MML_DisableLteCommProc(NAS_MML_LTE_CAPABILITY_STATUS_DISABLE_NOTIFIED_AS,
                                       MMC_LMM_DISABLE_LTE_REASON_EMERGENCY_CALL);                     
...
)
{
    ...
    stLteCellSignInfo.stRxAntInfo.ucRxANTNum  = pStSignReportIndMsg->stRxAntInfo.ucRxANTNum;
    stLteCellSignInfo.stRxAntInfo.asRsrpRx[0] = ((NAS_MMC_RSRP_UNVALID == pStSignReportIndMsg->stRxAntInfo.asRsrpRx[0])
                                                 ? NAS_MMC_RSRP_UNVALID : pStSignReportIndMsg->stRxAntInfo.asRsrpRx[0] / NAS_MMC_SIGNAL_DIVISOR);
    stLteCellSignInfo.stRxAntInfo.asRsrpRx[1] = ((NAS_MMC_RSRP_UNVALID == pStSignReportIndMsg->stRxAntInfo.asRsrpRx[1])
                                                 ? NAS_MMC_RSRP_UNVALID : pStSignReportIndMsg->stRxAntInfo.asRsrpRx[1] / NAS_MMC_SIGNAL_DIVISOR);
    stLteCellSignInfo.stRxAntInfo.asRsrpRx[2] = ((NAS_MMC_RSRP_UNVALID == pStSignReportIndMsg->stRxAntInfo.asRsrpRx[2])
                                                 ? NAS_MMC_RSRP_UNVALID : pStSignReportIndMsg->stRxAntInfo.asRsrpRx[2] / NAS_MMC_SIGNAL_DIVISOR);
    stLteCellSignInfo.stRxAntInfo.asRsrpRx[3] = ((NAS_MMC_RSRP_UNVALID == pStSignReportIndMsg->stRxAntInfo.asRsrpRx[3])
                                                 ? NAS_MMC_RSRP_UNVALID : pStSignReportIndMsg->stRxAntInfo.asRsrpRx[3] / NAS_MMC_SIGNAL_DIVISOR);
    stLteCellSignInfo.stRxAntInfo.alSINRRx[0] = pStSignReportIndMsg->stRxAntInfo.alSINRRx[0];
    stLteCellSignInfo.stRxAntInfo.alSINRRx[1] = pStSignReportIndMsg->stRxAntInfo.alSINRRx[1];
    stLteCellSignInfo.stRxAntInfo.alSINRRx[2] = pStSignReportIndMsg->stRxAntInfo.alSINRRx[2];
    stLteCellSignInfo.stRxAntInfo.alSINRRx[3] = pStSignReportIndMsg->stRxAntInfo.alSINRRx[3];
...
VOS_UINT32  NAS_MMC_RcvLmmAttachInd_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    LMM_MMC_ATTACH_IND_STRU            *pstLmmAttachIndMsg = VOS_NULL_PTR;
    MMC_APS_PS_REG_RESULT_ENUM_U32      enPsRegRslt;

    pstLmmAttachIndMsg  = (LMM_MMC_ATTACH_IND_STRU*)pstMsg;
    enPsRegRslt         = MMC_APS_PS_REG_RESULT_BUTT;

    if ( MMC_LMM_ATT_RSLT_SUCCESS == pstLmmAttachIndMsg->ulAttachRslt )
    {
        enPsRegRslt = MMC_APS_PS_REG_RESULT_SUCCESS;
    }
    else
    {
        enPsRegRslt = MMC_APS_PS_REG_RESULT_FAILURE;
    }

    /* 需要往APS报PS域注册结果状态 */
    NAS_MMC_SndApsPsRegStatusInd(enPsRegRslt, (MMC_APS_PLMN_ID_STRU *)NAS_MML_GetCurrCampPlmnId());

    return VOS_FALSE;
}


VOS_UINT32  NAS_MMC_RcvLmmTauResultInd_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    ...
    if ( (MMC_LMM_TAU_RSLT_SUCCESS == pstLmmTauIndMsg->ulTauRst)
      || (MMC_LMM_TAU_RSLT_T3402_RUNNING_EPS_ONLY_SUCC == pstLmmTauIndMsg->ulTauRst))
    {
        enPsRegRslt = MMC_APS_PS_REG_RESULT_SUCCESS;
    }
    else
...
VOS_UINT32  NAS_MMC_RcvLmmSearchedPlmnInfoInd_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                      *pstMsg
)
{
...
)
{
    ...
    if (VOS_TRUE == NAS_MMC_IsPsEmcAnyCellSearchScene())
    {
        NAS_MMC_SndMsccSysCfgCnf(NAS_MSCC_PIF_SYS_CFG_SET_FAILURE_OTHER);
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvTafSysCfgReq_PreProc: is ps emc any cell searching");
        return VOS_TRUE;
    }
...
)
{
    ...
    if (VOS_TRUE == NAS_MMC_IsPsEmcAnyCellSearchScene())
    {
        ...
        stPlmnId.ulMnc     = NAS_MML_INVALID_MNC;
        NAS_MMC_SndMsccAcqCnf(NAS_MSCC_PIF_ACQ_RESULT_FAILURE, &stPlmnId, 0);
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvTafAcqReq_PreProc: is ps emc any cell searching");
        return VOS_TRUE;
    }
...
)
{
    ...
    if (VOS_TRUE == NAS_MMC_IsPsEmcAnyCellSearchScene())
    {
        ...
        stPlmnId.ulMnc     = NAS_MML_INVALID_MNC;
        NAS_MMC_SndMsccRegCnf(MMC_LMM_CL_REG_STATUS_BUTT, NAS_MSCC_PIF_REG_RESULT_ACQ_FAILURE, &stPlmnId, 0x0);
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvTafRegReq_PreProc: is ps emc any cell searching");
        return VOS_TRUE;
    }
...
)
{
...
#if (FEATURE_ON == FEATURE_CSG)
    /* 用户指定PLMN搜索，清除之前记录的用户指定CSG ID信息 */
    NAS_MMC_InitCsgSpecSrchPlmnWithCsgId();
#endif
    ...
    {
        NAS_MMC_SndMsccUserSpecPlmnSearchRej();
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvMsccUserSpecPlmnSearch_PreProc: is ps emc any cell searching");
        return VOS_TRUE;
    }
...
)
{
    ...
    {
        NAS_MMC_SndMsccUserSpecPlmnSearchCnf(NAS_MSCC_PIF_USER_PLMN_SEARCH_RESULT_REJ);
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvUserReselReq_PreProc: is ps emc any cell searching");
        return VOS_TRUE;
    }
...
)
{
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvTiAvailTimerExpired_PreProc: User Spec Plmn is Forbidden Plmn and ulState is L1_STATE_ON_PLMN");
        return VOS_TRUE;
    }
    ...
    if ((VOS_TRUE == NAS_MML_IsLteOnlyMode(NAS_MML_GetMsPrioRatList()))
     ...
     && (NAS_MMC_L1_STA_OOC != ulState))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvTiAvailTimerExpired_PreProc: User Spec Plmn is Forbidden Plmn for GPRS and ulState is L1_ON_PLMN and LTE ONLY");
        return VOS_TRUE;
    }
#endif
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvTiAvailTimerExpired_PreProc: ulRrcConnExistFlag is VOS_TRUE");
...
VOS_UINT32 NAS_MMC_IsNeedRestartAvailableTimer( VOS_VOID )
{
    ...
    NAS_MMC_SPEC_PLMN_SEARCH_STATE_ENUM_UINT8               enPlmnSrchState;
    VOS_UINT32                          ulIsPsEmcAnyCellSearchScene;
    VOS_UINT8                           ucImsCallFlg;

    enCurrNetRat    = NAS_MML_GetCurrNetRatType();
    enPlmnSrchState = NAS_MMC_GetSpecPlmnSearchState();
    ulIsPsEmcAnyCellSearchScene         = NAS_MMC_IsPsEmcAnyCellSearchScene();
    ucImsCallFlg                        = NAS_MML_IsImsCallExist();
    ...
    /* 紧急呼结束后不搜网定时器正在运行，重启定时器；不搜网定时器定时器超时后不立即搜网，靠available超时触发 */
    if ((NAS_MML_TIMER_STATUS_RUNNING == NAS_MML_GetTimerStatusOfDelayNetworkSearchAfterEmc())
     && (NAS_MMC_SPEC_PLMN_SEARCH_STOP == enPlmnSrchState))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedRestartAvailableTimer: Delay Network Search After Emc Timer is runing.");

        return VOS_TRUE;
    }

    if ((VOS_TRUE == ucImsCallFlg)
     || (VOS_TRUE == ulIsPsEmcAnyCellSearchScene))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedRestartAvailableTimer: ims call exist or ps emc any cell search");
...
)
{
    ...
    {
        NAS_Mmc_SndMsccPlmnListRej();
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvTafPlmnListReq_PreProc: is ps emc any cell searching");
        return VOS_TRUE;
    }
    ...
    /* 判断当前是否需要打断当前状态机 或进行消息的缓存 */
    return NAS_MMC_ProcMsgPrio(NAS_BuildEventType(UEPS_PID_MSCC, ID_MSCC_MMC_PLMN_LIST_REQ),
                               pstMsg);
...
)
{
    ...
    {
        NAS_MMC_SndMsccCsgListSearchRej();
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvMsccCsgListSearchReq_PreProc: is ps emc any cell searching");
        return VOS_TRUE;
    }
...
)
{
    ...
    {
        NAS_MMC_SndMsccCsgListSearchRej();
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvTiPeriodTryingUserCsgListExpired_PreProc: is ps emc any cell searching");
        return VOS_TRUE;
    }
...
VOS_UINT32  NAS_MMC_RcvPeriodDeleteForbiddenCsgIdTimerExpired_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    if (VOS_TRUE == NAS_MMC_DeleteForbiddenPeriodExpiredCsgIdInForbiddenCsgIdList())
    {
        /* 通知L接入层CSG白名单 */
...
VOS_UINT32  NAS_MMC_RcvCsgIdHomeNodeBNameInd_PreProc(
    VOS_UINT32                                              ulEventType,
    struct MsgCB                                           *pstMsg
)
{
    LMM_MMC_CSG_ID_HOME_NODEB_NAME_IND_STRU                *pstCsgIdWithHNB = VOS_NULL_PTR;
    MMC_MSCC_CSG_ID_HOME_NODEB_NAME_IND_STRU                stCsgCellInfo;
    ...
    PS_MEM_SET(&stCsgCellInfo, 0, sizeof(MMC_MSCC_CSG_ID_HOME_NODEB_NAME_IND_STRU));
    ...
    pstCsgIdWithHNB = (LMM_MMC_CSG_ID_HOME_NODEB_NAME_IND_STRU*)pstMsg;
    ...
    if (VOS_TRUE == NAS_MML_IsCsgIdInOperatorCsgIdList(&stPlmnWithCsgId))
    {
        ...
        NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_RcvCsgIdHomeNodeBNameInd_PreProc,ucHomeNodebNameIndication is :",ucHomeNodebNameIndication);
        ...
        if (0x00 == ucHomeNodebNameIndication)
        {
            ...
            NAS_MMC_ConvertLmmPlmnToGUNasFormat(&pstCsgIdWithHNB->stPlmnWithCsgId.stPlmnId, &stPlmnId);

            stCsgCellInfo.stPlmnId.ulMcc              = stPlmnId.ulMcc;
            stCsgCellInfo.stPlmnId.ulMnc              = stPlmnId.ulMnc;
            stCsgCellInfo.ulCsgId                     = pstCsgIdWithHNB->stPlmnWithCsgId.ulCsgId;
            stCsgCellInfo.ucHomeNodeBNameLen          = pstCsgIdWithHNB->stHomeNodeBName.ucHomeNodeBNameLen;

            PS_MEM_CPY(&(stCsgCellInfo.aucHomeNodeBName[0]),
                   pstCsgIdWithHNB->stHomeNodeBName.aucHomeNodeBName,
                   NAS_MML_MIN(pstCsgIdWithHNB->stHomeNodeBName.ucHomeNodeBNameLen, MMC_LMM_MAX_HOME_NODEB_NAME_LEN));

            NAS_MMC_SndMsccMsgCsgIdHomeNodeBNameInd(&stCsgCellInfo);
        }
    }
    return VOS_TRUE;
}

VOS_UINT32  NAS_MMC_IsNeedCsgSpecSearchReq_PreProc(
    MSCC_MMC_CSG_SPEC_SEARCH_REQ_STRU  *pstCsgSpecSearchReq
)
{
    NAS_MML_PLMN_WITH_RAT_STRU          stUserSpecPlmnWithRat;

    stUserSpecPlmnWithRat.stPlmnId.ulMcc = pstCsgSpecSearchReq->stPlmnWithRat.stPlmnId.ulMcc;
    stUserSpecPlmnWithRat.stPlmnId.ulMnc = pstCsgSpecSearchReq->stPlmnWithRat.stPlmnId.ulMnc;
    stUserSpecPlmnWithRat.enRat          = pstCsgSpecSearchReq->stPlmnWithRat.enRatType;

    /* 判断用户CSG指定的Plmn是否有效，无效，返回false不满足搜网条件 */
    if (VOS_FALSE == NAS_MML_IsPlmnIdValid(&stUserSpecPlmnWithRat.stPlmnId))
    {
        return VOS_FALSE;
    }
    ...
    if ( VOS_FALSE == NAS_MML_IsUsimStausValid() )
    {
        /* 卡不在位或卡无效的处理,返回false不满足搜网条件 */
        return VOS_FALSE;
    }

    /* 该PLMN ID是否在锁网锁卡的PLMN 列表中,返回false不满足搜网条件 */
    if (VOS_TRUE == NAS_MML_IsBcchPlmnIdInLockPlmnList(&stUserSpecPlmnWithRat.stPlmnId))
    {
        return VOS_FALSE;
    }

    /* 指定PLMN 不允许漫游, 返回false不满足搜网条件 */
    if ( VOS_TRUE != NAS_MML_IsPlmnIdRoamingAllowed(&stUserSpecPlmnWithRat.stPlmnId) )
    {
        return VOS_FALSE;
    }

    /* 指定网络在禁止接入技术网络中，返回false不满足搜网条件,因为NAS_MMC_AppendPlmnInPlmnSelectionList
       函数不会将禁止接入技术的网络加入搜网列表 */
    if (VOS_TRUE == NAS_MML_IsPlmnIdWithRatInDestDisabledPlmnWithForbiddenPeriodList(&stUserSpecPlmnWithRat))
    {
        return VOS_FALSE;
    }

    /* 紧急pdn连接存在，返回false不满足搜网条件 */
    if (VOS_TRUE == NAS_MML_GetEmcPdpStatusFlg())
    {
        return VOS_FALSE;
    }
...
VOS_UINT32  NAS_MMC_RcvMsccUserCsgSpecSearchReq_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    MSCC_MMC_CSG_SPEC_SEARCH_REQ_STRU  *pstCsgSpecSrchReq = VOS_NULL_PTR;
    NAS_MMC_FSM_ID_ENUM_UINT32          enFsmId;
    NAS_MML_PLMN_ID_STRU               *pstUserSpecPlmnId = VOS_NULL_PTR;
    NAS_MML_NET_RAT_TYPE_ENUM_UINT8     enCurrRatType;
    VOS_UINT32                          ulRslt;
    ...
    pstCsgSpecSrchReq = (MSCC_MMC_CSG_SPEC_SEARCH_REQ_STRU *)pstMsg;
    pstUserSpecPlmnId = (NAS_MML_PLMN_ID_STRU*)(&pstCsgSpecSrchReq->stPlmnWithRat.stPlmnId);

    /* 当前不支持CSG功能，直接返回rej */
    if (VOS_FALSE == NAS_MML_IsSupportLteCsg())
    {
        NAS_MMC_SndMsccUserCsgSpecSearchCnf(NAS_MSCC_PIF_USER_PLMN_SEARCH_RESULT_REJ);
        return VOS_TRUE;
    }
    ...
    /* 当前驻留plmn和csg id和csg指定搜网的plmn和csg id相同，直接返回成功 */
    /* 如果重选状态机中收到用户CSG指定搜，用户CSG指定的网络和当前驻留的网络相同，
       并且当前驻留的网络能提供正常服务，则不打断重选状态机，直接回复搜网成功 */
    if ( ((NAS_MMC_L1_STA_ON_PLMN == NAS_MMC_GetFsmTopState())
       && (NAS_MMC_FSM_L1_MAIN == enFsmId))
      || (NAS_MMC_FSM_INTER_SYS_CELLRESEL == enFsmId) )
    {
        /* 判断用户指定的网络和当前驻留的网络是否相同,CSG ID相同 */
        ulRslt = NAS_MML_CompareBcchPlmnwithSimPlmn(NAS_MML_GetCurrCampPlmnId(),
                                                    pstUserSpecPlmnId);

        enCurrRatType = NAS_MML_GetCurrNetRatType();
        if ((pstCsgSpecSrchReq->ulCsgId == NAS_MML_GetCurrCampCsgId())
         && (VOS_TRUE == ulRslt)
         && (pstCsgSpecSrchReq->stPlmnWithRat.enRatType == enCurrRatType) )
        {
            /* 当前处在正常服务状态且未进行过搜网，否则在高优先级指定搜网时
               或者快速指定搜时，被用户指定搜打断，若用户指定的网络和先前驻留的
               网络相同，高优先级搜网状态机退出后，会不进行用户指定搜 */
            ...
            {
                /* 向MSCC发送搜网成功 */
                NAS_MMC_SndMsccUserCsgSpecSearchCnf(NAS_MSCC_PIF_USER_PLMN_SEARCH_RESULT_SUCCESS);
                return VOS_TRUE;
            }
        }
    }
    ...
    /* 不满足搜网条件直接返回rej */
    if (VOS_FALSE == NAS_MMC_IsNeedCsgSpecSearchReq_PreProc(pstCsgSpecSrchReq))
    {
        NAS_MMC_SndMsccUserCsgSpecSearchCnf(NAS_MSCC_PIF_USER_PLMN_SEARCH_RESULT_REJ);
        return VOS_TRUE;
    }
    ...
    {
        NAS_MMC_SndMsccUserCsgSpecSearchCnf(NAS_MSCC_PIF_USER_PLMN_SEARCH_RESULT_REJ);
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvMsccUserCsgSpecSearchReq_PreProc: is ps emc any cell searching");
        return VOS_TRUE;
    }
...
VOS_UINT32  NAS_MMC_RcvMsccUserCsgSpecSearchAbortReq_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    ...
    /* 用户CSG指定搜网消息*/
    ulCacheEventType     = NAS_BuildEventType(UEPS_PID_MSCC, ID_MSCC_MMC_CSG_SPEC_SEARCH_REQ);
    ulExistFlg           = NAS_MMC_IsExistCacheMsg(ulCacheEventType, &ulCacheMsgIdx);
    ulCurrentFsmId       = NAS_MMC_GetCurrFsmId();
    ...
    if (VOS_TRUE == ulExistFlg )
    {
        ...
        /* 向MSCC回复CSG指定搜网终止回复 */
        NAS_MMC_SndMsccUserCsgSpecSearchAbortCnf();
        return VOS_TRUE;
    }
    ...
    {
        /* 异常打印 */
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvMsccUserCsgSpecSearchAbortReq_PreProc:ERROR");
...
VOS_UINT32  NAS_MMC_RcvTiAutonomousCsgIdSearchExpired_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    NAS_MMC_PLMN_SELECTION_LIST_INFO_STRU                  *pstASFCsgIdSearchList = VOS_NULL_PTR;
    NAS_MML_CSG_AUTONOMOUS_SEARCH_CFG_STRU                 *pstCsgAsfCfg = VOS_NULL_PTR;

    /* 如果CSG背景搜网列表被锁定，说明当前正在执行CSG背景搜流程，本次不再发起ASF搜索 */
    if (NAS_MMC_USE_CSG_SEARCH_LIST_PROC_NULL != NAS_MMC_GetUseCsgIdSearchListProc())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvTiAutonomousCsgIdSearchExpired_PreProc: CSG BG SEARCH is on going");
        return VOS_TRUE;
    }

    /* 不满足发起ASF的条件，直接丢弃消息 */
    if (VOS_FALSE == NAS_MMC_IsNeedStartCsgIdASF())
    {
        return VOS_TRUE;
    }

    /* 获取CSG背景搜网列表 */
    pstASFCsgIdSearchList = NAS_MMC_GetCsgIdBgSearchList();

    /* 锁定CSG背景搜列表 */
    NAS_MMC_SetUseCsgIdSearchListProc(NAS_MMC_USE_CSG_SEARCH_LIST_PROC_ASF_SEARCH);

    /* 初始化CSG自主搜索列表 */
    NAS_MMC_InitPlmnSelectionList(NAS_MMC_PLMN_SEARCH_SCENE_ASF_CSG_SEARCH,
                                  VOS_NULL_PTR,
                                  pstASFCsgIdSearchList);

    pstCsgAsfCfg = NAS_MML_GetCsgAutonomousSrchCfg();

    /* 计数加1 */
    NAS_MMC_AddCsgASFTryNum();

    /* 若达到一阶段搜索最大次数，则进入第2阶段 */
    if (pstCsgAsfCfg->ucCsgAutonomousSrchFirstSrchTimeCount <= NAS_MMC_GetCsgASFTryNum())
    {
        NAS_MMC_SetCsgASFPhaseNum(NAS_MMC_CSG_ASF_PHASE_TWO);
    }

    /* 无法进行CSG自主搜索时,启动10s周期性尝试CSG搜索定时器 */
...
VOS_UINT32  NAS_MMC_RcvTiPeriodCsgIdSearchExpired_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    NAS_MMC_PLMN_SELECTION_LIST_INFO_STRU                  *pstPSFCsgIdSearchList = VOS_NULL_PTR;

    /* 如果CSG背景搜网列表被锁定，说明当前正在执行CSG背景搜流程，本次不再发起ASF搜索 */
    if (NAS_MMC_USE_CSG_SEARCH_LIST_PROC_NULL != NAS_MMC_GetUseCsgIdSearchListProc())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvTiPeriodCsgIdSearchExpired_PreProc: CSG BG SEARCH is on going");
        return VOS_TRUE;
    }

    /* 不满足发起PSF的条件，直接丢弃消息 */
    if (VOS_FALSE == NAS_MMC_IsNeedStartCsgIdPSF())
    {
        return VOS_TRUE;
    }

    /* 获取CSG背景搜网列表 */
    pstPSFCsgIdSearchList = NAS_MMC_GetCsgIdBgSearchList();

    /* 锁定CSG背景搜列表 */
    NAS_MMC_SetUseCsgIdSearchListProc(NAS_MMC_USE_CSG_SEARCH_LIST_PROC_PSF_SEARCH);

    /* 初始化CSG自主搜索列表 */
    NAS_MMC_InitPlmnSelectionList(NAS_MMC_PLMN_SEARCH_SCENE_PSF_CSG_SEARCH,
                                  VOS_NULL_PTR,
                                  pstPSFCsgIdSearchList);

    /* 无法进行CSG自主搜索时,启动10s周期性尝试CSG自主搜索定时器 */
    if (VOS_FALSE == NAS_MMC_IsEnableBgPlmnSearch_PreProc())
    {
        (VOS_VOID)NAS_MMC_StartTimer(TI_NAS_MMC_PERIOD_TRYING_CSG_ID_SEARCH, TI_NAS_MMC_PERIOD_TRYING_CSG_ID_SEARCH_LEN);
        return VOS_TRUE;
...
VOS_UINT32  NAS_MMC_RcvTiPeriodTryingCsgIdSearchExpired_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    /* 如果当前是PSF流程中 */
    if (NAS_MMC_USE_CSG_SEARCH_LIST_PROC_PSF_SEARCH == NAS_MMC_GetUseCsgIdSearchListProc())
    {
        /* 不满足发起PSF的条件，直接丢弃消息 */
        if (VOS_FALSE == NAS_MMC_IsNeedStartCsgIdPSF())
        {
    ...
    /* 无法进行CSG周期搜索时,启动10s周期性尝试CSG搜索定时器 */
    if (VOS_FALSE == NAS_MMC_IsEnableBgPlmnSearch_PreProc())
    {
        (VOS_VOID)NAS_MMC_StartTimer(TI_NAS_MMC_PERIOD_TRYING_CSG_ID_SEARCH, TI_NAS_MMC_PERIOD_TRYING_CSG_ID_SEARCH_LEN);
        return VOS_TRUE;
...
VOS_VOID NAS_MMC_UpdateAllowedCsgListUsimFileRecordsInBatches(
...
)
{
    ...
    NAS_MML_CSG_FILE_RECORD_STRU       *pstRecordInfo       = VOS_NULL_PTR;
    NAS_MML_ACSGL_RECORD_STREAM_STRU   *pstRecordStream     = VOS_NULL_PTR;

    PS_MEM_SET(&stSetFileInfo, 0x00, sizeof(NAS_USIMM_UPDATEFILE_INFO_STRU));
    pstRecordInfo = &(NAS_MML_GetAllowedCsgListFilesInfo()->stCsgListRecordInfo);
    pstRecordStream = NAS_MML_GetAllowedCsgListRecordStream();

    if (pstRecordInfo->ucTotalRecordNum <= (ucStartRecordNum + NAS_MML_READ_SIMFILE_ONCE_MAXNUM))
    {
        ucEndRecord = pstRecordInfo->ucTotalRecordNum;
    }
    else
    {
        ucEndRecord = ucStartRecordNum + NAS_MML_READ_SIMFILE_ONCE_MAXNUM;
    }

    for ( i = ucStartRecordNum; i < ucEndRecord; i++ )
    {
        if (VOS_NULL_PTR != pstRecordStream->apstCsgListRecordInfo[i])
        {
            NAS_COMM_BUILD_USIM_SET_FILE_INFO( &stSetFileInfo,
                                                USIMM_GUTL_APP,
                                                USIMM_USIM_EFACSGL_ID,
                                                (i+1),
                                                pstRecordInfo->ucRecordLen,
                                                pstRecordStream->apstCsgListRecordInfo[i]->aucEf);
...
VOS_UINT32 NAS_MMC_IsCsgIdDeleteAfterRefresh_PreProc(
    NAS_MML_PLMN_WITH_CSG_ID_LIST_STRU                     *pstOperatorCsgListCache)
{
    ...
    NAS_MML_PLMN_WITH_CSG_ID_STRU                          *pstPlmnWithCsgId        = VOS_NULL_PTR;
    VOS_UINT32                                              i;
    VOS_UINT32                                              j;
    VOS_UINT32                                              ulFoundFlag;

    pstOperatorCsgList                  = NAS_MML_GetOperatorCsgList();

    if (pstOperatorCsgListCache->ulPlmnWithCsgIdListNum < pstOperatorCsgList->ulPlmnWithCsgIdListNum)
    {
        return VOS_TRUE;
    }

    for ( i = 0; i < NAS_MML_MIN(pstOperatorCsgList->ulPlmnWithCsgIdListNum, NAS_MML_MAX_PLMN_CSG_ID_NUM); i++ )
    {
        pstPlmnWithCsgId = &(pstOperatorCsgList->astPlmnWithCsgIdInfo[i].stPlmnWithCsgId);
        ulFoundFlag = VOS_FALSE;

        for (j = 0; j < NAS_MML_MIN(pstOperatorCsgListCache->ulPlmnWithCsgIdListNum, NAS_MML_MAX_PLMN_CSG_ID_NUM); j++)
        {
            if ((VOS_TRUE == NAS_MML_CompareBcchPlmnwithSimPlmn(&pstPlmnWithCsgId->stPlmnId, &(pstOperatorCsgListCache->astPlmnWithCsgIdInfo[j].stPlmnWithCsgId.stPlmnId)))
              && (pstPlmnWithCsgId->ulCsgId == pstOperatorCsgListCache->astPlmnWithCsgIdInfo[j].stPlmnWithCsgId.ulCsgId))
            {
                ulFoundFlag = VOS_TRUE;
                break;
            }
        }

        if(VOS_FALSE == ulFoundFlag)
        {
            return VOS_TRUE;
        }
    }
...
VOS_UINT32 NAS_MMC_UpdateCsgInfoAfterRefresh_PreProc(VOS_VOID)
{
    NAS_MML_PLMN_WITH_CSG_ID_LIST_STRU                     *pstOperatorCsgListCache = VOS_NULL_PTR;
    NAS_MML_PLMN_WITH_CSG_ID_LIST_STRU                     *pstOperatorCsgList      = VOS_NULL_PTR;
    ...
    VOS_UINT32                                              i;
    VOS_UINT32                                              ulRet;
    VOS_UINT32                                              ulIsAdded;
    VOS_UINT32                                              ulIsDeleted;

    pstSimInfo                          = NAS_MML_GetSimInfo();
    pstOperatorCsgListCache             = NAS_MML_GetOperatorCsgListCache();
    pstOperatorCsgList                  = NAS_MML_GetOperatorCsgList();
    pstAllowedCsgList                   = NAS_MML_GetAllowedCsgList();
    pstForbiddenCsgList                 = NAS_MML_GetForbiddenCsgList();
    ulIsAdded                           = VOS_FALSE;
    ulIsDeleted                         = VOS_FALSE;

    if (VOS_NULL_PTR == pstOperatorCsgListCache)
    {
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_UpdateCsgInfoAfterRefresh:ERROR: pstOperatorCsgListCache is NULL");
        return VOS_TRUE;
    }

    /* 新Operator Csg List为空，清空sim信息及EFACSGL文件 */
    if (0 == pstOperatorCsgListCache->ulPlmnWithCsgIdListNum)
    {
        NAS_MML_ClearSimCsgInfo(&(pstSimInfo->stSimCsgInfo));
        ulIsDeleted = VOS_TRUE;

        /* 保存新的EFOCSGL */
        PS_MEM_CPY(pstOperatorCsgList, pstOperatorCsgListCache, sizeof(NAS_MML_PLMN_WITH_CSG_ID_LIST_STRU));
    }
    else
    {
        /* 查找新增CSG ID，更新CSG Info, */
        for ( i = 0; i < NAS_MML_MIN(pstOperatorCsgListCache->ulPlmnWithCsgIdListNum, NAS_MML_MAX_PLMN_CSG_ID_NUM); i++ )
        {
            pstPlmnWithCsgId = &(pstOperatorCsgListCache->astPlmnWithCsgIdInfo[i].stPlmnWithCsgId);
            if ( VOS_FALSE == NAS_MML_IsCsgIdInOperatorCsgIdList(pstPlmnWithCsgId) )
            {
                ulIsAdded = VOS_TRUE;

                /* 停止所有CSG 自主搜和周期搜相关的定时器 */
                ...
                (VOS_VOID)NAS_MMC_StartTimer(TI_NAS_MMC_PERIOD_CSG_ID_SEARCH_TIMER, TI_NAS_MMC_PERIOD_TRYING_CSG_ID_UNAVAILABLE_SEARCH_LEN);
                break;
            }
        }

        /* 是否有删除CSG ID */
        ulIsDeleted = NAS_MMC_IsCsgIdDeleteAfterRefresh_PreProc(pstOperatorCsgListCache);

        /* 保存新的EFOCSGL */
        PS_MEM_CPY(pstOperatorCsgList, pstOperatorCsgListCache, sizeof(NAS_MML_PLMN_WITH_CSG_ID_LIST_STRU));

        /* AT&T要求Alloewed Csg List是Operator Csg List子集 */
        i = 0;
        pstAllowedCsgList->ulPlmnWithCsgIdListNum = NAS_MML_MIN(pstAllowedCsgList->ulPlmnWithCsgIdListNum, NAS_MML_MAX_PLMN_CSG_ID_NUM);
        while (i < pstAllowedCsgList->ulPlmnWithCsgIdListNum)
        {
            pstPlmnWithCsgId = &(pstAllowedCsgList->astPlmnWithCsgIdInfo[i].stPlmnWithCsgId);
            if ( VOS_FALSE == NAS_MML_IsCsgIdInOperatorCsgIdList(pstPlmnWithCsgId) )
            {
                PS_MEM_MOVE(&(pstAllowedCsgList->astPlmnWithCsgIdInfo[i]),
                            &(pstAllowedCsgList->astPlmnWithCsgIdInfo[i+1]),
                             (pstAllowedCsgList->ulPlmnWithCsgIdListNum-i-1)*sizeof(NAS_MML_PLMN_WITH_CSG_ID_INFO_STRU));
        ...
        /* 查找Forbidden Csg Lis中不在新的Operator Csg List中的CSG ID ,并删除 */
        i = 0;
        pstForbiddenCsgList->ulForbiddenCsgIdNum = NAS_MML_MIN(pstForbiddenCsgList->ulForbiddenCsgIdNum, NAS_MML_MAX_PLMN_CSG_ID_NUM);
        while (i < pstForbiddenCsgList->ulForbiddenCsgIdNum)
        {
            pstPlmnWithCsgId = &(pstForbiddenCsgList->astForbiddenCsgIdPeriodInfo[i].stPlmnWithCsgIdInfo);
            if ( VOS_FALSE == NAS_MML_IsCsgIdInOperatorCsgIdList(pstPlmnWithCsgId) )
            {
                PS_MEM_MOVE(&(pstForbiddenCsgList->astForbiddenCsgIdPeriodInfo[i]),
                            &(pstForbiddenCsgList->astForbiddenCsgIdPeriodInfo[i+1]),
                             (pstForbiddenCsgList->ulForbiddenCsgIdNum-i-1) * sizeof(NAS_MML_CSG_ID_FORBIDDEN_PERIOD_INFO_STRU));

                pstForbiddenCsgList->ulForbiddenCsgIdNum--;

                /* 删除完CSG ID，移动后，原来的最后的CSG ID的位置所有参数被初始化 */
                PS_MEM_SET(&(pstForbiddenCsgList->astForbiddenCsgIdPeriodInfo[pstForbiddenCsgList->ulForbiddenCsgIdNum]), 0, sizeof(NAS_MML_CSG_ID_FORBIDDEN_PERIOD_INFO_STRU));
    ...
    /* 释放动态申请的空间 */
    NAS_MML_FreeOperaterCsgListCache();

    if ((VOS_TRUE ==ulIsAdded)
     || (VOS_TRUE == ulIsDeleted))
    {
        ulRet = NAS_USIMMAPI_IsServiceAvailable(NAS_USIM_SVR_OP_CSG);
        if (PS_USIM_SERVICE_AVAILIABLE == ulRet)
        {
            /* 卡文件存在写则重写卡文件EFACSGL */
            NAS_MMC_UpdateAllowedCsgListUsimFileRecordsInBatches(0);
        ...
        /* 搜网 */
        ulRet = NAS_MMC_CsgIdUnavailablePlmnSearch_PreProc();
...
VOS_UINT32  NAS_MMC_RcvUsimQueryFileCnf_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    USIMM_QUERYFILE_CNF_STRU           *pstRcvMsg = VOS_NULL_PTR;
    USIMM_DEF_FILEID_ENUM_UINT32        enEfId;
    VOS_UINT32                          ulRslt;

    pstRcvMsg                           = (USIMM_QUERYFILE_CNF_STRU*)pstMsg;
    ...
    /* 目前只对USIMM_USIM_EFOCSGL_ID及相关的消息做预处理，用于CSG refresh */
    if ( USIMM_USIM_EFOCSGL_ID != enEfId
      && USIMM_USIM_EFOCSGT_ID != enEfId
      && USIMM_USIM_EFOHNBN_ID != enEfId )
    {
        return VOS_FALSE;
    }

    NAS_MMC_RcvAgentUsimQueryCsgFileCnf(pstRcvMsg, VOS_TRUE);

    /* 未收齐向agient要求的响应，继续等待 */
    if (NAS_MML_READ_USIM_FILE_FLG_NULL != NAS_MMC_GetCsgRefreshSimFileReadFlg())
    {
        return VOS_TRUE;
    }
    ...
    /* 更新CSG相关sim卡信息 */
    ulRslt = NAS_MMC_UpdateCsgInfoAfterRefresh_PreProc();

    return ulRslt;
}


VOS_UINT32  NAS_MMC_RcvUsimReadFileCnf_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    ...
    /* 目前只对USIMM_USIM_EFOCSGL_ID及相关的消息预处理，用于CSG refresh */
    switch ( enEfId )
    {
        case USIMM_USIM_EFOCSGL_ID:
            NAS_MMC_RcvAgentUsimReadFileCnf_EFOCSGL_FILE_RECORD(pstRcvMsg, VOS_TRUE);
            break;

        case USIMM_USIM_EFOCSGT_ID:
            NAS_MMC_RcvAgentUsimReadFileCnf_EFCSGT_FILE_RECORD(pstRcvMsg, USIMM_USIM_EFOCSGT_ID, VOS_TRUE);
            break;

        case USIMM_USIM_EFOHNBN_ID:
            NAS_MMC_RcvAgentUsimReadFileCnf_EFHNBN_FILE_RECORD(pstRcvMsg, USIMM_USIM_EFOHNBN_ID, VOS_TRUE);
            break;
...
VOS_UINT32  NAS_MMC_RcvTiReadSimFilesExpired_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    /* 异常打印 */
    NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvTiReadSimFilesExpired_PreProc:ENTERED");
    ...
    /* 清除等待接收SIM卡文件标志位 */
    NAS_MMC_ClearAllCsgRefreshSimFileReadFlg();
...
VOS_VOID  NAS_MMC_ProcCsgSetFileCnf_PreProc(
...
)
{
    ...
    if (NAS_MMC_FSM_SWITCH_ON == NAS_MMC_GetCurrFsmId())
    {
        if (USIMM_SW_OK != pUsimSetFileCnf->stCmdResult.ulResult )
        {
            NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_ProcCsgSetFileCnf_PreProc: NAS_MMC_IsCsPsMode1NeedPeriodSearchGU return false");
        }
        return ;
    }
    ...
    else
    {
        ...
        {
            NAS_MMC_UpdateAllowedCsgListUsimFileRecordsInBatches(pUsimSetFileCnf->ucRecordNum);
        }
    }
...
)
{
    ...
    {
        NAS_Mmc_SndMsccPlmnListRej();
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvTiPeriodTryingUserPlmnListExpired_PreProc: is ps emc any cell searching");
        return VOS_TRUE;
    }
...
)
{
    ...
    if (NAS_MMC_TIMER_STATUS_RUNING == NAS_MMC_GetTimerStatus(TI_NAS_MMC_AVAILABLE_TIMER))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvTiPeriodTryingInterPlmnListExpired_PreProc: TI_NAS_MMC_AVAILABLE_TIMER is running");
        return VOS_TRUE;
    }
    ...
    if ( VOS_TRUE == NAS_MMC_IsNormalServiceStatus())
    {
        ...
        if (NAS_MML_NET_RAT_TYPE_LTE == enCurRatType)
        {
            ...
            {
                NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvTiPeriodTryingInterPlmnListExpired_PreProc: NAS_MMC_IsCsPsMode1NeedPeriodSearchGU return false");
                return VOS_TRUE;
            }
        }
        else
        {
            ...
            {
                NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvTiPeriodTryingInterPlmnListExpired_PreProc: CS&PS no need plmn srch");
                return VOS_TRUE;
            }
        }
#else
    ...
    if (VOS_FALSE == NAS_MML_IsUsimStausValid())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvTiPeriodTryingInterPlmnListExpired_PreProc: Usim Status is invalid");
        return VOS_TRUE;
    }
...
)
{
    VOS_UINT8                           ucCsSigConnStatusFlg;
    NAS_MMC_AS_CELL_CAMP_ON_ENUM_UINT8  enCampStaus;
    NAS_MMC_INTER_PLMN_LIST_REQ_STRU   *pstInterListReq = VOS_NULL_PTR;

    /* 不允许列表搜时，直接返回 */
    if (VOS_FALSE == NAS_MMC_IsEnableInterPlmnList_PreProc())
    {
        return VOS_TRUE;
    }


    pstInterListReq             = (NAS_MMC_INTER_PLMN_LIST_REQ_STRU *)pstMsg;
    enCampStaus                 = NAS_MMC_GetAsCellCampOn();
    ucCsSigConnStatusFlg        = NAS_MML_GetCsSigConnStatusFlg();
    ...
    if ((VOS_TRUE                    == NAS_MML_GetCsServiceExistFlg())
     ...
     || (VOS_TRUE                    == ucCsSigConnStatusFlg))
    {
        if ((NAS_MMC_PLMN_LIST_SEARCH_SCENE_PREFBAND_PLMN_SEARCH == pstInterListReq->enListPlmnSearchScene)
         || (NAS_MMC_PLMN_LIST_SEARCH_SCENE_FULLBAND_PLMN_SEARCH == pstInterListReq->enListPlmnSearchScene))
        {
            /* 启动定时器TI_NAS_MMC_AVAILABLE_TIMER,否则搜网场景会丢失 */
            (VOS_VOID)NAS_MMC_StartTimer(TI_NAS_MMC_AVAILABLE_TIMER, NAS_MMC_GetNextAvailableTimerValue());
...
VOS_UINT32 NAS_MMC_IsEnableInterPlmnList_PreProc(VOS_VOID)
{
    VOS_UINT32                          ulSingleDomainNeedPlmnSrch;
    ...
    if (NAS_MMC_TIMER_STATUS_RUNING == NAS_MMC_GetTimerStatus(TI_NAS_MMC_PERIOD_TRYING_USER_PLMN_LIST))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsEnableInterPlmnList_PreProc: TI_NAS_MMC_PERIOD_TRYING_USER_PLMN_LIST is running");
        return VOS_FALSE;
    }
    ...
    if (NAS_MMC_TIMER_STATUS_RUNING == NAS_MMC_GetTimerStatus(TI_NAS_MMC_PERIOD_TRYING_USER_CSG_LIST_SEARCH))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsEnableInterPlmnList_PreProc: TI_NAS_MMC_PERIOD_TRYING_USER_CSG_LIST_SEARCH is running");
        return VOS_FALSE;
    }
#endif
    ...
    if (VOS_TRUE == NAS_MMC_IsNormalServiceStatus())
    {
        ...
        if (NAS_MML_NET_RAT_TYPE_LTE == NAS_MML_GetCurrNetRatType())
        {
            if (VOS_FALSE == NAS_MMC_IsCsPsMode1NeedPeriodSearchGU())
            {
                NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsEnableInterPlmnList_PreProc: NAS_MMC_IsCsPsMode1NeedPeriodSearchGU return false");
                return VOS_FALSE;
            }
        }
        else
        {
            if ((VOS_FALSE == NAS_MMC_IsSingleDomainRegFailNeedPlmnSrch(NAS_MSCC_PIF_SRVDOMAIN_CS))
             && (VOS_FALSE == ulSingleDomainNeedPlmnSrch))
            {
                NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsEnableInterPlmnList_PreProc: CS&PS no need plmn serch");
                return VOS_FALSE;
            }
        }
#else
    ...
    if (VOS_FALSE == NAS_MML_IsUsimStausValid())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsEnableInterPlmnList_PreProc: Usim Status is invalid");
        return VOS_FALSE;
    }
...
VOS_UINT32 NAS_MMC_IsEnableBgHistoryPlmnSearch_PreProc( VOS_VOID )
{
    ...
    if (NAS_MMC_TIMER_STATUS_RUNING == NAS_MMC_GetTimerStatus(TI_NAS_MMC_PERIOD_TRYING_USER_PLMN_LIST))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsEnableBgHistoryPlmnSearch_PreProc: Period Trying User Plmn List Timer is Running");
        return VOS_FALSE;
    }
    ...
    if (NAS_MMC_TIMER_STATUS_RUNING == NAS_MMC_GetTimerStatus(TI_NAS_MMC_PERIOD_TRYING_USER_CSG_LIST_SEARCH))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsEnableBgHistoryPlmnSearch_PreProc: Period Trying User CSG List Timer is Running");
        return VOS_FALSE;
    }
#endif
    ...
    if (NAS_MMC_AS_CELL_NOT_CAMP_ON == NAS_MMC_GetAsCellCampOn())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsEnableBgHistoryPlmnSearch_PreProc: As Cell is not camp on");
        return VOS_FALSE;
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsEnableBgHistoryPlmnSearch_PreProc: connect exist");
        return VOS_FALSE;
    }

    if ((VOS_TRUE == pstConnStatus->ucPsServiceBufferFlg)
     || (VOS_TRUE == pstConnStatus->ucCsServiceBufferFlg))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsEnableBgHistoryPlmnSearch_PreProc: ServiceBuffer exist");
        return VOS_FALSE;
    }
    ...
    if( NAS_MMC_FSM_L1_MAIN != NAS_MMC_GetCurrFsmId())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsEnableBgHistoryPlmnSearch_PreProc: Current Fsm Id is not L1_MAIN");
        return VOS_FALSE;
    }
...
VOS_UINT32 NAS_MMC_IsEnableBgPlmnSearch_PreProc( VOS_VOID )
{
    ...
    if (NAS_MMC_TIMER_STATUS_RUNING == NAS_MMC_GetTimerStatus(TI_NAS_MMC_AVAILABLE_TIMER))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsEnableBgPlmnSearch_PreProc: Available Timer is Running");
        return VOS_FALSE;
    }
    ...
    if (NAS_MMC_TIMER_STATUS_RUNING == NAS_MMC_GetTimerStatus(TI_NAS_MMC_PERIOD_TRYING_USER_PLMN_LIST))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsEnableBgPlmnSearch_PreProc: Period Trying User Plmn List Timer is Running");
        return VOS_FALSE;
    }
    ...
    if (NAS_MMC_TIMER_STATUS_RUNING == NAS_MMC_GetTimerStatus(TI_NAS_MMC_PERIOD_TRYING_USER_CSG_LIST_SEARCH))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsEnableBgPlmnSearch_PreProc: Period Trying User CSG List Timer is Running");
        return VOS_FALSE;
    }
#endif
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsEnableBgPlmnSearch_PreProc: Period Trying Inter List Timer is Running");
        return VOS_FALSE;
    }
    ...
    if (NAS_MMC_AS_CELL_NOT_CAMP_ON == NAS_MMC_GetAsCellCampOn())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsEnableBgPlmnSearch_PreProc: As Cell is not camp on");
        return VOS_FALSE;
    }
    ...
    if (NAS_MMC_TIMER_STATUS_RUNING == NAS_MMC_GetTimerStatus(TI_NAS_MMC_PERIOD_TRYING_HIGH_PRIO_PLMN_SEARCH))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsEnableBgPlmnSearch_PreProc: Period Trying High Prio Plmn Srch Timer is Running");
        return VOS_FALSE;
    }
    ...
    if ((VOS_TRUE == pstConnStatus->ucPsServiceBufferFlg)
     || (VOS_TRUE == pstConnStatus->ucCsServiceBufferFlg))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsEnableBgPlmnSearch_PreProc: ServiceBuffer exist");
        return VOS_FALSE;
    }
    ...
    if( NAS_MMC_FSM_L1_MAIN != NAS_MMC_GetCurrFsmId())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsEnableBgPlmnSearch_PreProc: Current Fsm Id is not L1_MAIN");
        return VOS_FALSE;
    }
...
VOS_UINT32 NAS_MMC_ProcTiHplmnTimerExpired_PreProc(VOS_VOID)
{
    ...
    if ( VOS_FALSE == NAS_MMC_IsEnableBgPlmnSearch_PreProc() )
    {
        /* 如果当前NO RF,不启动TI_NAS_MMC_PERIOD_TRYING_HIGH_PRIO_PLMN_SEARCH定时器，直接退状态机 */
        if (VOS_FALSE == NAS_MML_GetRfAvailFlg())
        {
            NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_ProcTiHplmnTimerExpired_PreProc: RfAvailFlg = VOS_FALSE");
            return VOS_TRUE;
        }
        ...
        if ( NAS_MMC_FSM_BG_PLMN_SEARCH == NAS_MMC_GetCurrFsmId())
        {
            NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_ProcTiHplmnTimerExpired_PreProc: Bg Plmn Search Is Running");
            return VOS_TRUE;
        }

        /* 列表类型如果是NO RF备份的选网列表，或BG History备份的高优先级列表，在选网状态机中需要使用，不能初始化成高优先级列表 */
        if (NAS_MMC_STORE_HIGH_PRIO_PLMN_SELECTION_LIST == NAS_MMC_GetPlmnSelectionListType())
        {
            /* 初始化高优先级搜网列表 */
            NAS_MMC_InitPlmnSelectionList(NAS_MMC_PLMN_SEARCH_SCENE_HIGH_PRIO_PLMN_SEARCH,
    ...
    /* 如果驻留在用户指定的CSG ID上，则不发起高优先级搜索 */
    if (VOS_TRUE == NAS_MMC_IsCurrCampUserSpecCsgId())
    {
        return VOS_TRUE;
    }
#endif
...
)
{
    ...
    enPsRegRslt  = (GMM_MMC_ACTION_RESULT_SUCCESS == pstPsRegRslt->enActionResult) ? MMC_APS_PS_REG_RESULT_SUCCESS : MMC_APS_PS_REG_RESULT_FAILURE;
    ...
    if (VOS_TRUE == NAS_MMC_IsNeedEnableLte_EnableLteTimerExp())
    {
         ...
         && (VOS_FALSE == ulDisablteRoamFlg))
        {
             ...
             && (NAS_MML_NET_RAT_TYPE_LTE == pstPrioRatList->aucRatPrio[0]))
            {
                ...
                if (VOS_TRUE == NAS_MML_GetCsServiceExistFlg())
                {
                    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvEnableLteExpired_PreProc: NAS_MMC_SetEnableLteTimerExpireFlag is true");
            ...
            NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvEnableLteExpired_PreProc: LTE REENABLE_NOTIFIED_AS and disable lte reason is reset");
        }
        else
        {
            NAS_NORMAL_LOG2(WUEPS_PID_MMC, "NAS_MMC_RcvEnableLteExpired_PreProc: ulDisablteRoamFlg and NAS_MML_GetLteCapabilityStatus not need enable lte",ulDisablteRoamFlg,NAS_MML_GetLteCapabilityStatus());
        }
...
)
{
    ...
    pstCLAssociatedInfo->ul1xCampedMcc          = pstCLAssociatedInfoNtfMsg->ul1xCampedMcc;
...
VOS_UINT32 NAS_MMC_RcvMsccCurrGeoInfoNtf_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    MSCC_MMC_CURR_GEO_INFO_NTF_STRU     *pstCurGeoInd;

    pstCurGeoInd = (MSCC_MMC_CURR_GEO_INFO_NTF_STRU *)pstMsg;

    NAS_MMC_SndCssCurrGeoInd(pstCurGeoInd->ulIsPowerOn, pstCurGeoInd->ulPlmnNum, &(pstCurGeoInd->astPlmnId[0]));
...
VOS_UINT32 NAS_MMC_RcvMsccEplmnInfoNtf_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
#if (FEATURE_ON == FEATURE_LTE)
    NAS_MML_EQUPLMN_INFO_STRU           stSndEquPlmnInfo;
    MSCC_MMC_EPLMN_INFO_NTF_STRU       *pstEplmnInfoNtf;
    VOS_UINT32                          i;
    VOS_UINT32                          ulEplmnNum;

    PS_MEM_SET(&stSndEquPlmnInfo, 0, sizeof(NAS_MML_EQUPLMN_INFO_STRU));

    pstEplmnInfoNtf = (MSCC_MMC_EPLMN_INFO_NTF_STRU *)pstMsg;
    ulEplmnNum      = NAS_COMM_MIN(pstEplmnInfoNtf->ulEplmnNum, MSCC_MMC_MAX_EPLMN_ID_NUM);

    for (i = 0; i < ulEplmnNum; i++)
    {
        stSndEquPlmnInfo.astEquPlmnAddr[i].ulMcc = pstEplmnInfoNtf->astEplmnList[i].ulMcc;
        stSndEquPlmnInfo.astEquPlmnAddr[i].ulMnc = pstEplmnInfoNtf->astEplmnList[i].ulMnc;

        stSndEquPlmnInfo.ucEquPlmnNum++;
    }

    if (0 < stSndEquPlmnInfo.ucEquPlmnNum)
    {
        NAS_MMC_BuildSndLmmEquPlmnInfo(&stSndEquPlmnInfo);
    }
...
{
    VOS_UINT32                                              ulCurrState;
    NAS_MML_PLMN_LIST_WITH_RAT_STRU                        *pstCurrSearchingPlmn = VOS_NULL_PTR;
    VOS_UINT32                                              ulIsCurrSearchingPlmnHplmn;
    VOS_UINT32                                              ulUserAutoReselActiveFlg;
    NAS_MMC_FSM_ID_ENUM_UINT32                              enFsmId;
    VOS_UINT32                                              ulIsUsimStatusValid;
    ...
    pstCurrSearchingPlmn       = NAS_MMC_GetCurrSearchingPlmnList_PlmnSelection();
    ulIsCurrSearchingPlmnHplmn = NAS_MML_ComparePlmnIdWithHplmn(&pstCurrSearchingPlmn->astPlmnId[0].stPlmnId);
    ulIsUsimStatusValid        = NAS_MML_IsUsimStausValid();
...
)
{
    ...
    NAS_MMC_TIMER_STATUS_ENUM_U8        enUserPlmnListTimerStatus;
    NAS_MMC_TIMER_STATUS_ENUM_U8        enInterPlmnListTimerStatus;
     ...
     && (NAS_MMC_FSM_L1_MAIN    == enFsmId))
    {
        ...
        enUserPlmnListTimerStatus   = NAS_MMC_GetTimerStatus(TI_NAS_MMC_PERIOD_TRYING_USER_PLMN_LIST);
        enInterPlmnListTimerStatus  = NAS_MMC_GetTimerStatus(TI_NAS_MMC_PERIOD_TRYING_INTER_PLMN_LIST);
        if ( (VOS_TRUE == NAS_MMC_IsCsPsMode1NeedPeriodSearchGU())
          && ( (NAS_MMC_TIMER_STATUS_RUNING == enAvaiStatus)
            ...
            || (enUserPlmnListTimerStatus == NAS_MMC_TIMER_STATUS_RUNING)
            || (enInterPlmnListTimerStatus == NAS_MMC_TIMER_STATUS_RUNING) ) ) /* available定时器超时后，LTE存在链接，则会缓存搜网 */
        {
        ...
        NAS_WARNING_LOG4(WUEPS_PID_MMC, "NAS_MMC_RcvMsccImsVoiceCapInd_PreProc: MMC_LMM_DISABLE_LTE_REASON_LTE_VOICE_NOT_AVAILABLE 1.",
                        enAvaiStatus,enPlmnSearchScene,enUserPlmnListTimerStatus,enInterPlmnListTimerStatus);
        ...
        if (NAS_MML_NET_RAT_TYPE_LTE != NAS_MML_GetCurrNetRatType())
        {
            ...
            NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvMsccImsVoiceCapInd_PreProc: MMC_LMM_DISABLE_LTE_REASON_LTE_VOICE_NOT_AVAILABLE 2");
...
)
{
    ...
    /* 如果驻留在用户指定的CSG ID上，则不发起高优先级接入技术搜索 */
    if (VOS_TRUE == NAS_MMC_IsCurrCampUserSpecCsgId())
    {
        return VOS_TRUE;
    }
    ...
    (VOS_VOID)NAS_MMC_AddCurHighRatHplmnSleepCount();
    if (VOS_TRUE == NAS_MMC_IsNeedSkipHighPrioRatSearch())
    {
        (VOS_VOID)NAS_MMC_StartTimer(TI_NAS_MMC_HIGH_PRIO_RAT_HPLMN_TIMER, NAS_MMC_GetHighPrioRatHplmnTimerLen());
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvTiHighPrioRatHplmnSrchTimerExpired_PreProc: Save power mode. Restart high prio timer and not search.");
        return VOS_TRUE;
    }
    else
    {
...
VOS_UINT32 NAS_MMC_RcvMmcInterBgSearchReq_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    ...
    if (VOS_FALSE == NAS_MML_IsUsimStausValid())
    {
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvMmcInterBgSearchReq_PreProc: usim status invalid.");
        return VOS_TRUE;
    }

    /* 无法进行高优先级搜索时,启动available定时器，启周期性尝试定时器搜网场景会丢失 */
    if ( VOS_FALSE == NAS_MMC_IsEnableBgHistoryPlmnSearch_PreProc() )
    {
        (VOS_VOID)NAS_MMC_StartTimer(TI_NAS_MMC_AVAILABLE_TIMER, NAS_MMC_GetNextAvailableTimerValue());
...
)
{
    switch (enMsccMmcSrvType)
    {
        ...
        case NAS_MSCC_PIF_SRV_TYPE_PS_MO_EMERGENCY_CALL:
...
)
{
    ...
    NAS_MMC_AS_CELL_CAMP_ON_ENUM_UINT8  enCampOnType;
    NAS_MML_NET_RAT_TYPE_ENUM_UINT8     enCurrNetRatType;

    pstSrvAcqReq        = (MSCC_MMC_SRV_ACQ_REQ_STRU*)pstMsg;
    enFsmId             = NAS_MMC_GetCurrFsmId();
    ulState             = NAS_MMC_GetFsmTopState();
    enCampOnType        = NAS_MMC_GetAsCellCampOn();
    enCurrNetRatType    = NAS_MML_GetCurrNetRatType();
    ...
    if ((NAS_MMC_FSM_L1_MAIN           == enFsmId)
     && (NAS_MMC_L1_STA_ON_PLMN        == ulState))
    {
        /* 如果是CS紧急呼叫触发搜网当前处于on plmn则直接回复搜网结果为成功 */
        ...
        /* 如果是PS域紧急呼叫触发的搜网且当前驻留在L下则直接回复搜网结果为成功 */
        if ((NAS_MML_NET_RAT_TYPE_LTE                   == enCurrNetRatType)
         && (NAS_MMC_AS_CELL_CAMP_ON                    == enCampOnType)
...
)
{
    ...
    /* 将ImsSrvInfoNotify消息发给MM，触发MM启动延迟搜网的2分钟定时器 */
    NAS_MMC_SndMmImsSrvInfoNotify(pstRcvMsg);
...
/* DMT用例中有很多用例在还未开机时就先下发了AT命令去清空FPLMN列表，而实际上SIM卡
   未初始化完成时，是不能对FPLMN列表进行操作的。对于DMT用例我们并不太关注是否写卡
   成功，故对于DMT用例还是保留原来的函数接口去处理 */
#ifdef DMT
...
)
{
    ...
    if (NAS_MSCC_PIF_CFPLMN_TYPE_ADD_ONE == pstCFPlmnReqMsg->enCmdType)
    {
        ...
        {
            /* 上报设置结果　*/
            NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvMsccCFPlmnSetReq_PreProc: Added PLMN is HPLMN, return ERROR");
            NAS_MMC_SndMsccCFPlmnSetCnf(NAS_MSCC_PIF_CFPLMN_OPER_RESULT_FAILURE);
            return VOS_TRUE;
        }
        ...
        {
            NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvMsccCFPlmnSetReq_PreProc: Add failure, return FAILURE");
            NAS_MMC_SndMsccCFPlmnSetCnf(NAS_MSCC_PIF_CFPLMN_OPER_RESULT_FAILURE);
          ...
          || (NAS_MSCC_PIF_CFPLMN_TYPE_DEL_ALL == pstCFPlmnReqMsg->enCmdType))
    {
        ...
        if (NAS_MSCC_PIF_CFPLMN_TYPE_DEL_ALL == pstCFPlmnReqMsg->enCmdType)
        {
            if (0 != pstForbidPlmnList->ucUsimForbPlmnNum)
            {
                if (0 != pstForbidPlmnList->ucForbPlmnNum)
                {
                    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvMsccCFPlmnSetReq_PreProc: Delete all FPLMN list");
                    pstForbidPlmnList->ucForbPlmnNum = 0;
                    ulChgFlag = VOS_TRUE;
                }
            }
            ...
            {
                NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvMsccCFPlmnSetReq_PreProc: Delete all FPLMN list failure because ucUsimForbPlmnNum is 0");
                ulChgFlag = VOS_FALSE;
            }
        }
        else
        ...
        {
            NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_RcvMsccCFPlmnSetReq_PreProc: Del failure, return FAILURE");
            NAS_MMC_SndMsccCFPlmnSetCnf(NAS_MSCC_PIF_CFPLMN_OPER_RESULT_FAILURE);
...
)
{
	...
	#if(FEATURE_ON == FEATURE_CSG)
        else if (USIMM_USIM_EFACSGL_ID   == ulEfId)
        {
            NAS_MMC_ProcCsgSetFileCnf_PreProc(pUsimSetFileCnf);
            return VOS_TRUE;
        }
	#endif
        return VOS_FALSE;
...
#if (FEATURE_ON == FEATURE_UE_MODE_CDMA)
        stPlmnPriClassListInfo.astHighPlmnPriClassInfo[i].ucAllowSrchInCLMode = (VOS_UINT8)NAS_MMC_IsCurrLtePlmnAllowSrchInCLMode(&stTmpPlmnId);
#endif
    }
...
#if (FEATURE_ON == FEATURE_UE_MODE_CDMA)
        stPlmnPriClassListInfo.astLowPlmnPriClassInfo[i].ucAllowSrchInCLMode = (VOS_UINT8)NAS_MMC_IsCurrLtePlmnAllowSrchInCLMode(&stTmpPlmnId);
#endif
    }
...
)
{
    ...
    {
        NAS_MMC_SndMsccBgSearchCnf(MMC_LMM_BG_SRCH_HRPD_RLT_ABORT, 0, VOS_NULL_PTR);
        return VOS_TRUE;
    }
    ...
    if (VOS_RATMODE_HRPD != pstMsccMmcBgSearchReq->enRatMode)
    {
        ...
        NAS_MMC_SndMsccBgSearchCnf(MMC_LMM_BG_SRCH_HRPD_RLT_FAIL, 0, VOS_NULL_PTR);
...
VOS_UINT32  NAS_MMC_IsCurrCampUserSpecCsgId(VOS_VOID)
{
    NAS_MML_CELL_TYPE_ENUM_UINT8        enCurrCampCellType;
    VOS_UINT32                          ulCsgId;
    NAS_MML_PLMN_ID_STRU               *pstCurrCampPlmnId        = VOS_NULL_PTR;
    NAS_MML_PLMN_CSG_ID_WITH_RAT_STRU  *pstUserSpecPlmnWithCsgId = VOS_NULL_PTR;

    enCurrCampCellType                  = NAS_MML_GetCurrCampCellType();
    ulCsgId                             = NAS_MML_GetCurrCampCsgId();
    pstCurrCampPlmnId                   = NAS_MML_GetCurrCampPlmnId();
    pstUserSpecPlmnWithCsgId            = NAS_MMC_GetCsgSpecSrchPlmnWithCsgId();

    /* 当前不支持CSG功能，直接返回FALSE */
    if (VOS_FALSE == NAS_MML_IsSupportLteCsg())
    {
        return VOS_FALSE;
    }
    ...
    /* 宏驻留，返回FALSE */
    if (NAS_MML_CELL_TYPE_MACRO == enCurrCampCellType)
    {
        return VOS_FALSE;
    }

    /* CSG ID非法，返回FALSE */
    if (NAS_MML_INVALID_CSG_ID_VALUE == ulCsgId)
    {
        return VOS_FALSE;
    }

    /* PLMN不同，返回FALSE */
    if (VOS_TRUE != NAS_MML_CompareBcchPlmnwithSimPlmn(pstCurrCampPlmnId, &(pstUserSpecPlmnWithCsgId->stPlmnWithRat.stPlmnId)))
    {
        return VOS_FALSE;
    }

    /* CSG ID不同，返回FALSE */
    if (ulCsgId != pstUserSpecPlmnWithCsgId->ulCsgId)
    {
        return VOS_FALSE;
    }
...
VOS_UINT32 NAS_MMC_CsgIdUnavailablePlmnSearch_PreProc( VOS_VOID )
{
    NAS_MML_PLMN_WITH_CSG_ID_STRU                           stPlmnWithCsgId;
    NAS_MML_PLMN_WITH_CSG_ID_STRU                           stUserSpecCsgIdInfo;
    NAS_MML_CAMP_PLMN_INFO_STRU                            *pstCurCampInfo      = VOS_NULL_PTR;
    NAS_MML_PLMN_ID_STRU                                   *pstCurrPlmnId       = VOS_NULL_PTR;
    NAS_MML_PLMN_CSG_ID_WITH_RAT_STRU                      *pstUserSpecCsgId    = VOS_NULL_PTR;

    pstCurCampInfo                        = NAS_MML_GetCurrCampPlmnInfo();
    pstCurrPlmnId                         = NAS_MML_GetCurrCampPlmnId();
    pstUserSpecCsgId                      = NAS_MMC_GetCsgSpecSrchPlmnWithCsgId();

    stPlmnWithCsgId.stPlmnId.ulMcc        = pstCurrPlmnId->ulMcc;
    stPlmnWithCsgId.stPlmnId.ulMnc        = pstCurrPlmnId->ulMnc;
    stPlmnWithCsgId.ulCsgId               = pstCurCampInfo->ulCsgId ;

    stUserSpecCsgIdInfo.stPlmnId.ulMcc    = pstUserSpecCsgId->stPlmnWithRat.stPlmnId.ulMcc;
    stUserSpecCsgIdInfo.stPlmnId.ulMnc    = pstUserSpecCsgId->stPlmnWithRat.stPlmnId.ulMnc;
    stUserSpecCsgIdInfo.ulCsgId           = pstUserSpecCsgId->ulCsgId ;
    ...
    if (VOS_FALSE == NAS_MML_IsCsgIdInCsgWhiteList(&stUserSpecCsgIdInfo, VOS_NULL_PTR, NAS_MML_INVALID_CSG_ID_VALUE))
    {
        NAS_MMC_InitCsgSpecSrchPlmnWithCsgId();
    }
    ...
    {
        if (VOS_FALSE == NAS_MML_IsCsgIdInCsgWhiteList(&stPlmnWithCsgId, VOS_NULL_PTR, NAS_MML_INVALID_CSG_ID_VALUE))
        {
            return VOS_FALSE;
        }
        return VOS_TRUE;
    ...
    {
        if (VOS_TRUE == NAS_MML_IsWhiteCsgListEmpty())
        {
            /* 停止所有CSG 自主搜和周期搜相关的定时器 */
            NAS_MMC_StopTimer(TI_NAS_MMC_AUTONOMOUS_CSG_ID_SEARCH_TIMER);
            NAS_MMC_StopTimer(TI_NAS_MMC_PERIOD_CSG_ID_SEARCH_TIMER);
