...
VOS_UINT32 g_aulFsmEntryEventType[] =
{
    ...
    NAS_BuildEventType(UEPS_PID_MSCC, ID_MSCC_MMC_CSG_SPEC_SEARCH_REQ),

    NAS_BuildEventType(VOS_PID_TIMER, TI_NAS_MMC_AUTONOMOUS_CSG_ID_SEARCH_TIMER),
    NAS_BuildEventType(VOS_PID_TIMER, TI_NAS_MMC_PERIOD_CSG_ID_SEARCH_TIMER),
    NAS_BuildEventType(VOS_PID_TIMER, TI_NAS_MMC_PERIOD_TRYING_CSG_ID_SEARCH),
#endif
    ...
    NAS_BuildEventType(WUEPS_PID_MMC, MMCMMC_INTER_BG_SEARCH_REQ),
...
)
{
    ...
    switch ( pstEntryMsg->ulEventType)
    {
        ...
        case NAS_BuildEventType(UEPS_PID_MSCC, ID_MSCC_MMC_CSG_SPEC_SEARCH_REQ):
#endif
            ulCacheFlg = VOS_TRUE;
...
VOS_VOID NAS_MMC_SetLastTmsi_SwitchOn(VOS_UINT8 *pucTmsi)
{
    ...
    pulTmsiValue = (VOS_UINT32*)pstNasMmcCtx->stCurFsm.unFsmCtx.stSwitchOnCtx.aucLastTmsi;

    NAS_INFO_LOG1(WUEPS_PID_MMC, "NAS_MMC_SetLastTmsi_SwitchOn,tmsi ", *pulTmsiValue);
}
...
VOS_VOID  NAS_MMC_InitFsmCtx_PlmnSelection(VOS_VOID)
{
    ...
    NAS_MML_PLMN_LIST_WITH_RAT_STRU                         stInvalidPlmnIdList;
    VOS_UINT32                                              i;
    ...
    PS_MEM_SET(&stInvalidPlmnIdList, 0x0, sizeof(stInvalidPlmnIdList));

    stInvalidPlmnIdList.ulPlmnNum   = 0;

    for (i = 0; i < NAS_MML_MAX_EHPLMN_NUM; i++)
    {
        stInvalidPlmnIdList.astPlmnId[i].stPlmnId.ulMcc    = NAS_MML_INVALID_MCC;
        stInvalidPlmnIdList.astPlmnId[i].stPlmnId.ulMnc    = NAS_MML_INVALID_MNC;
        stInvalidPlmnIdList.astPlmnId[i].enRat             = NAS_MML_NET_RAT_TYPE_BUTT;
    }

    NAS_MMC_SetCurrSearchingPlmnList_PlmnSelection(&stInvalidPlmnIdList);
...
VOS_VOID  NAS_MMC_InitFsmCtx_BgPlmnSearch()
{
    ...
    PS_MEM_SET(&stInvalidPlmnIdList, 0, sizeof(NAS_MML_PLMN_LIST_WITH_RAT_STRU));
    stInvalidPlmnIdList.ulPlmnNum   = 0;
    for (i = 0; i < NAS_MML_MAX_EHPLMN_NUM; i++)
    {
        stInvalidPlmnIdList.astPlmnId[i].stPlmnId.ulMcc = stInvalidPlmnId.ulMcc;
        stInvalidPlmnIdList.astPlmnId[i].stPlmnId.ulMnc = stInvalidPlmnId.ulMnc;
        stInvalidPlmnIdList.astPlmnId[i].enRat          = NAS_MML_NET_RAT_TYPE_BUTT;
    }
    NAS_MMC_SetCurHighPrioPlmnList_BgPlmnSearch(&stInvalidPlmnIdList);

    /* 初始化VPlmn信息 */
    stInvalidLai.stPlmnId.ulMcc     = stInvalidPlmnId.ulMcc;
    stInvalidLai.stPlmnId.ulMnc     = stInvalidPlmnId.ulMnc;
    stInvalidLai.enCampPlmnNetRat   = NAS_MML_NET_RAT_TYPE_BUTT;
    stInvalidLai.aucLac[0]          = NAS_MML_LAC_LOW_BYTE_INVALID;
    stInvalidLai.aucLac[1]          = NAS_MML_LAC_HIGH_BYTE_INVALID;
    stInvalidLai.ucRac              = NAS_MML_RAC_INVALID;
    NAS_MMC_SetPreCampLai_BgPlmnSearch(&stInvalidLai);
    ...
    /* 设置当前不是CSG背景搜 */
    NAS_MMC_SetCsgSearchingFlag_BgPlmnSearch(VOS_FALSE);

    /* 初始化当前正在搜索的CSG ID为无效值 */
    NAS_MMC_SetCurSearchCsgId_BgPlmnSearch(&stInvalidPlmnId,
                                        NAS_MML_INVALID_CSG_ID_VALUE,
                                        NAS_MML_NET_RAT_TYPE_BUTT);
#endif

    NAS_MMC_InitPreServiceInfo_BgPlmnSearch();

    NAS_MMC_InitSearchRatInfo_BgPlmnSearch();
...
VOS_VOID  NAS_MMC_InitFsmCtx_PlmnList(VOS_VOID)
{
    ...
    NAS_MMC_SetAbortBySearchedPlmnFlg_PlmnList(VOS_FALSE);
...
)
{
    ...
    pstPlmnSearchCtrl->stCLAssociatedInfoNtf.ul1xCampedMcc             = NAS_MML_INVALID_MCC;
...
{
    NAS_MML_PLMN_WITH_RAT_STRU          stAbortListSearchPlmn;

    NAS_MEM_SET_S(&stAbortListSearchPlmn, sizeof(NAS_MML_PLMN_WITH_RAT_STRU),
                  0, sizeof(NAS_MML_PLMN_WITH_RAT_STRU));

    stAbortListSearchPlmn.enRat          = NAS_MML_NET_RAT_TYPE_BUTT;
    stAbortListSearchPlmn.stPlmnId.ulMcc = NAS_MML_INVALID_MCC;
    stAbortListSearchPlmn.stPlmnId.ulMnc = NAS_MML_INVALID_MNC;

    NAS_MMC_SetAbortListSearchPlmn(&stAbortListSearchPlmn);
...
{
    return &(NAS_MMC_GetMmcCtxAddr()->stPlmnSearchCtrl.stAbortListSearchPlmn);
}
...
{
    NAS_MMC_GetMmcCtxAddr()->stPlmnSearchCtrl.stAbortListSearchPlmn = *pstAbortListSearchPlmn;
...
VOS_VOID NAS_MMC_ResetOosPlmnSearchTimerCount_OnPlmn(VOS_VOID)
{
    ...
    /* Reset Oos Search Sleep Count */
    NAS_MMC_GetMmcCtxAddr()->stCurFsm.unFsmCtx.stL1MainCtx.ulCurOosSearchSleepCount        = 0;
...
)
{
    ...
    NAS_NORMAL_LOG2(WUEPS_PID_MMC, "NAS_MMC_SetCsRegAdditionalAction_L1Main,enCsAddtionalAction is; and ps is; ",enCsAddtionalAction, NAS_MMC_GetPsRegAdditionalAction_L1Main());
...
)
{
    ...
    NAS_NORMAL_LOG2(WUEPS_PID_MMC, "NAS_MMC_SetPsRegAdditionalAction_L1Main,enPsAddtionalAction is; and cs is; ",enPsAddtionalAction,NAS_MMC_GetCsRegAdditionalAction_L1Main());
...
VOS_UINT8 NAS_MMC_GetWaitRegRsltFlag_PlmnSelection(VOS_VOID)
{
    ...
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_GetWaitRegRsltFlag_PlmnSelection: ucWaitRegRsltFlag;1:cs;2:ps,4:ims ",g_stNasMmcCtx.stCurFsm.unFsmCtx.stPlmnSelectionCtx.stRegRlstInfo.ucWaitRegRsltFlag);
...
VOS_VOID NAS_MMC_SetWaitRegRsltFlag_PlmnSelection(VOS_UINT8 ucWaitFlag)
{
    ...
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_SetWaitRegRsltFlag_PlmnSelection: ucWaitRegRsltFlag;1:cs;2:ps,4:ims ",g_stNasMmcCtx.stCurFsm.unFsmCtx.stPlmnSelectionCtx.stRegRlstInfo.ucWaitRegRsltFlag);
}
...
NAS_MMC_ADDITIONAL_ACTION_ENUM_UINT8 NAS_MMC_GetCsRegAdditionalAction_PlmnSelection(VOS_VOID)
{
    ...
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_GetCsRegAdditionalAction_PlmnSelection: enCsRegAdditionalAction", g_stNasMmcCtx.stCurFsm.unFsmCtx.stPlmnSelectionCtx.stRegRlstInfo.enCsRegAdditionalAction);
...
)
{
    ...
    NAS_NORMAL_LOG2(WUEPS_PID_MMC, "NAS_MMC_SetCsRegAdditionalAction_PlmnSelection,enCsAddtionalAction is; and ps is; ",enCsAddtionalAction, NAS_MMC_GetPsRegAdditionalAction_PlmnSelection());
...
NAS_MMC_ADDITIONAL_ACTION_ENUM_UINT8 NAS_MMC_GetPsRegAdditionalAction_PlmnSelection(VOS_VOID)
{
    ...
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_GetPsRegAdditionalAction_PlmnSelection: enPsRegAdditionalAction", g_stNasMmcCtx.stCurFsm.unFsmCtx.stPlmnSelectionCtx.stRegRlstInfo.enPsRegAdditionalAction);
...
)
{
    ...
    NAS_NORMAL_LOG2(WUEPS_PID_MMC, "NAS_MMC_SetPsRegAdditionalAction_PlmnSelection,enPsAddtionalAction is; and cs is; ",enPsAddtionalAction,NAS_MMC_GetCsRegAdditionalAction_PlmnSelection());
...
)
{
    ...
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SetHistorySearchedFlag_PlmnList,ERROR:FsmId Error");
        return;
    }

    /* 查找列表搜状态机上下文中与enRat相同的接入技术数组 */
    for (i = 0 ; i < NAS_MML_MAX_RAT_NUM; i++)
    {
        if (enRat == g_stNasMmcCtx.stCurFsm.unFsmCtx.stPlmnListCtx.astPlmnSearchInfo[i].enRatType)
        {
            g_stNasMmcCtx.stCurFsm.unFsmCtx.stPlmnListCtx.astPlmnSearchInfo[i].stSearchedType.ucHistorySearchedFlg = ucHistorySearchedFlag;

            return;
        }
    }

    NAS_WARNING_LOG1(WUEPS_PID_MMC, "NAS_MMC_SetHistorySearchedFlag_PlmnList, enRat not found, enRat is.", enRat);
...
VOS_VOID NAS_MMC_SetPrefbandSearchedFlag_PlmnList(
    NAS_MML_NET_RAT_TYPE_ENUM_UINT8     enRat,
    VOS_UINT8                           ucPrefbandSearchedFlag
)
{
    VOS_UINT32                          i;
    ...
    if (NAS_MMC_FSM_PLMN_LIST!= NAS_MMC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SetPrefbandSearchedFlag_PlmnList,ERROR:FsmId Error");
        return;
    }
    ...
    for (i = 0 ; i < NAS_MML_MAX_RAT_NUM; i++)
    {
        if (enRat == g_stNasMmcCtx.stCurFsm.unFsmCtx.stPlmnListCtx.astPlmnSearchInfo[i].enRatType)
        {
            g_stNasMmcCtx.stCurFsm.unFsmCtx.stPlmnListCtx.astPlmnSearchInfo[i].stSearchedType.ucPrefBandSearchedFlg = ucPrefbandSearchedFlag;

            return;
        }
    }

    NAS_WARNING_LOG1(WUEPS_PID_MMC, "NAS_MMC_SetPrefbandSearchedFlag_PlmnList, enRat not found, enRat is.", enRat);
...
VOS_VOID NAS_MMC_SetFullbandSearchedFlag_PlmnList(
    NAS_MML_NET_RAT_TYPE_ENUM_UINT8     enRat,
    VOS_UINT8                           ucFullbandSearchedFlag
)
{
    VOS_UINT32                          i;
    ...
    if (NAS_MMC_FSM_PLMN_LIST!= NAS_MMC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SetFullbandSearchedFlag_PlmnList,ERROR:FsmId Error");
        return;
    }
    ...
    for (i = 0 ; i < NAS_MML_MAX_RAT_NUM; i++)
    {
        if (enRat == g_stNasMmcCtx.stCurFsm.unFsmCtx.stPlmnListCtx.astPlmnSearchInfo[i].enRatType)
        {
            g_stNasMmcCtx.stCurFsm.unFsmCtx.stPlmnListCtx.astPlmnSearchInfo[i].stSearchedType.ucSpecSearchedFlg = ucFullbandSearchedFlag;

            return;
        }
    }

    NAS_WARNING_LOG1(WUEPS_PID_MMC, "NAS_MMC_SetFullbandSearchedFlag_PlmnList, enRat not found, enRat is.", enRat);
...
)
{
    ...
    if (NAS_MMC_FSM_PLMN_SELECTION != NAS_MMC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_UpdateHistorySearchedFlagWhenRoamSort_PlmnSelection,ERROR:FsmId Error");

        return;
    }

    NAS_MMC_SetHistorySearchedFlag_PlmnSelection(enCurrRat, VOS_FALSE);

    /* 更新其他未进行过漫游排序的接入技术为未进行过历史搜 */
    for (i = 0; i < pstPrioRatList->ucRatNum; i++)
    {
        enRat = pstPrioRatList->aucRatPrio[i];

        if (VOS_FALSE == NAS_MMC_GetSearchedRoamPlmnSortedFlag_PlmnSelection(enRat))
        {
            NAS_MMC_SetHistorySearchedFlag_PlmnSelection(enRat, VOS_FALSE);
        }
    }
}
...
VOS_VOID NAS_MMC_InitSearchRatInfo_PlmnSelection(VOS_VOID)
{
    ...
    for (i = 0; i < NAS_MML_MAX_RAT_NUM; i++ )
    {
        ...
        pstSearchRatInfo[i].ucSearchedCsgSpecSearchFlag = VOS_FALSE;
        PS_MEM_SET(pstSearchRatInfo[i].aucReserved, 0, sizeof(pstSearchRatInfo[i].aucReserved));
...
)
{
    ...
    if ( NAS_MMC_FSM_PLMN_SELECTION != NAS_MMC_GetCurrFsmId() )
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SetCurrSearchingPlmnList_PlmnSelection,ERROR:FsmId Error");
        return;
    }

    g_stNasMmcCtx.stCurFsm.unFsmCtx.stPlmnSelectionCtx.stCurrSearchingPlmnList = *pstPlmnIdList;
}
...
NAS_MML_PLMN_LIST_WITH_RAT_STRU* NAS_MMC_GetCurrSearchingPlmnList_PlmnSelection(VOS_VOID)
{
    ...
    if ( NAS_MMC_FSM_PLMN_SELECTION != NAS_MMC_GetCurrFsmId() )
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_GetCurrSearchingPlmnList_PlmnSelection,ERROR:FsmId Error");
    }

    return &(g_stNasMmcCtx.stCurFsm.unFsmCtx.stPlmnSelectionCtx.stCurrSearchingPlmnList);
}
...
VOS_UINT32 NAS_MMC_IsPsEmcAnyCellSearchScene(VOS_VOID)
{
    ...
    if ((NAS_MMC_FSM_ANYCELL_SEARCH == enFsmId)
     && (MMCMMC_INTER_ANYCELL_SEARCH_REQ == NAS_ExtractMsgNameFromEvtType(pstEntryMsg->ulEventType))
     && (NAS_MMC_ANYCELL_SEARCH_SCENE_PS_EMC_TRIG_PLMN_SEARCH == pstInterAnycellSearchReq->enAnycellSearchScene))
    {
        return VOS_TRUE;
    }
...
NAS_MML_NET_RAT_TYPE_ENUM_UINT8 NAS_MMC_GetNextSearchRat_AnyCellSearch(VOS_VOID)
{
    ...
    for ( i = 0; i < ucRatNum; i++)
    {
        ...
        {
            NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_GetNextSearchRat_AnyCellSearch:Do not search L when L disabled");
            continue;
        }

        /* 有CSFB业务存在时，ANYCELL搜需要忽略L模 */
        if ((VOS_TRUE == NAS_MML_GetCmServiceSetupFlg())
         && (NAS_MML_NET_RAT_TYPE_LTE   == pstAnyCellSearchCtx->stMmcPlmnRatPrio.aucRatPrio[i]))
        {
            NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_GetNextSearchRat_AnyCellSearch:Do not search L when CSFB service exists");
            continue;
        }
#endif
...
)
{
    ...
    /* INIT不会调用此函数，如果找到下一个要搜的RAT就会变成非驻留，pref band list和full band list不允许在非驻留态搜网 */
    if ((VOS_TRUE == NAS_MMC_IsPrefBandListSearchScene_PlmnList())
     || (VOS_TRUE == ulIsFullBandScene))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_GetNextSearchingRat_PlmnList: NORMAL: is PrefBand or FullBand Scene!");
...
)
{
    ...
    if ( NAS_MMC_FSM_PLMN_LIST != NAS_MMC_GetCurrFsmId())
    {
       NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SetAbortBySearchedPlmnFlg_PlmnList,ERROR:FsmId Error");
    }

    g_stNasMmcCtx.stCurFsm.unFsmCtx.stPlmnListCtx.ucAbortBySearchedPlmnFlg = ucAbortBySearchedPlmnFlg;
...
VOS_UINT32 NAS_MMC_GetAbortBySearchedPlmnFlg_PlmnList(VOS_VOID)
{
    ...
    if ( NAS_MMC_FSM_PLMN_LIST != NAS_MMC_GetCurrFsmId())
    {
       NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_GetAbortBySearchedPlmnFlg_PlmnList,ERROR:FsmId Error");
    }

    return g_stNasMmcCtx.stCurFsm.unFsmCtx.stPlmnListCtx.ucAbortBySearchedPlmnFlg;
}
...
{
    NAS_MMC_SEARCHED_PLMN_LIST_INFO_STRU         *pstSearchedListInfo = VOS_NULL_PTR;
    VOS_UINT32                                    ulIndex;

    pstSearchedListInfo = NAS_MMC_GetSearchedPlmnListInfo_PlmnList();

    /* 在searched plmn info list中找到指定的enRat */
    for (ulIndex = 0; ulIndex < NAS_MML_MAX_RAT_NUM; ulIndex++)
    {
        if (enRat == pstSearchedListInfo[ulIndex].enRatType)
        {
            break;
        }
    }

    if (ulIndex >= NAS_MML_MAX_RAT_NUM)
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SetSearchedTypeFlgByPlmnSearchCnfSucc_PlmnList,ERROR:Rat not found");
        return;
    }


    if (RRC_PLMN_SEARCH_RLT_INTER_LIST_PREF_BAND_SUCC == enRrcPlmnSrchRslt)
    {
        /* list pref band 搜成功，之前一定进行过BG history搜，所以ucHistorySearchedFlg和ucPrefBandSearchedFlg
           标志为true，ucSpecSearchedFlg标志位false */
        pstSearchedListInfo[ulIndex].stSearchedType.ucHistorySearchedFlg  = VOS_TRUE;
        pstSearchedListInfo[ulIndex].stSearchedType.ucPrefBandSearchedFlg = VOS_TRUE;
        pstSearchedListInfo[ulIndex].stSearchedType.ucSpecSearchedFlg     = VOS_FALSE;
    }
    else
    {
        pstSearchedListInfo[ulIndex].stSearchedType.ucHistorySearchedFlg  = VOS_TRUE;
        pstSearchedListInfo[ulIndex].stSearchedType.ucPrefBandSearchedFlg = VOS_TRUE;
        pstSearchedListInfo[ulIndex].stSearchedType.ucSpecSearchedFlg     = VOS_TRUE;
    }
...
)
{
    ...
    if (MMC_LMM_PLMN_SRCH_RLT_INTER_LIST_PREF_BAND_SUCC == enLmmPlmnSrchRslt)
    {
        /* list pref band 搜成功，之前一定进行过BG history搜，所以ucHistorySearchedFlg和ucPrefBandSearchedFlg
           标志为true，ucSpecSearchedFlg标志位false */
        pstSearchedListInfo[ulIndex].stSearchedType.ucHistorySearchedFlg  = VOS_TRUE;
...
)
{
    ...
    if (ulIndex >= NAS_MML_MAX_RAT_NUM)
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SetSpecRatSearchedTypeByAreaLostInd_PlmnList,Rat not found");
        return;
    }


    /* 根据Aread Lost Ind 结果更新指定rat的搜网类型 */
    pstSearchedListInfo[ulIndex].stSearchedType.ucHistorySearchedFlg  = pstAreaLostInd->stSearchedType.ucHistorySearchedFlg;
    pstSearchedListInfo[ulIndex].stSearchedType.ucPrefBandSearchedFlg = pstAreaLostInd->stSearchedType.ucPrefBandSearchedFlg;
    pstSearchedListInfo[ulIndex].stSearchedType.ucSpecSearchedFlg     = pstAreaLostInd->stSearchedType.ucSpecSearchedFlg;
...
)
{
    ...
    if (ulIndex >= NAS_MML_MAX_RAT_NUM)
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SetSpecRatSearchedTypeByLmmAreaLostInd_PlmnList,Rat not found");
        return;
    }
...
)
{
    ...
    NAS_NORMAL_LOG2(WUEPS_PID_MMC, "NAS_MMC_SetCsRegAdditionalAction_PlmnList,enCsAddtionalAction is; and ps is; ",enCsAddtionalAction, NAS_MMC_GetCsRegAdditionalAction_PlmnList());
...
)
{
    ...
    NAS_NORMAL_LOG2(WUEPS_PID_MMC, "NAS_MMC_SetPsRegAdditionalAction_PlmnList,enPsAddtionalAction is; and cs is; ",enPsAddtionalAction, NAS_MMC_GetCsRegAdditionalAction_PlmnList());
...
)
{
    ...
    if (NAS_MMC_FSM_PLMN_LIST != NAS_MMC_GetCurrFsmId())
    {
       NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_InitSpecRatSearchedPlmnListInfo_PlmnList,ERROR:FsmId Error");
    ...
    for (i = 0; i < NAS_MML_MAX_RAT_NUM; i++)
    {
        if (enSpecRat == pstListInfo[i].enRatType)
        {
...
VOS_VOID NAS_MMC_InitSearchedPlmnListInfo_PlmnList(VOS_VOID)
{
    ...
    {
        pstListInfo[i].enRatType        = pstRatPrio->aucRatPrio[i];
        ...
        /* 是否进行过history/pref band/full band搜网标志更新  */
        pstListInfo[i].stSearchedType.ucHistorySearchedFlg  = VOS_FALSE;
        pstListInfo[i].stSearchedType.ucPrefBandSearchedFlg = VOS_FALSE;
        pstListInfo[i].stSearchedType.ucSpecSearchedFlg     = VOS_FALSE;
    }

    for ( i= pstRatPrio->ucRatNum; i < NAS_MML_MAX_RAT_NUM; i++ )
    {
...
VOS_VOID NAS_MMC_InitHighPrioPlmnSearchCtx(NAS_MMC_HIGH_PRIO_PLMN_SEARCH_CTRL_CTX_STRU *pstHighPrioPlmnSearchCtrl)
{
    ...
    pstHighPrioPlmnSearchCtrl->ucCurHighRatHplmnSleepCount                     = 0;
    ...
    for (i = 0; i < NAS_MMC_MAX_PLMN_NUM_IN_SELECTION_LIST; i++)
    {
        ...
        pstHighPrioPlmnSearchCtrl->stHighPrioPlmnSearchListInfo.astPlmnSelectionList[i].ulCsgId        = NAS_MML_INVALID_CSG_ID_VALUE;
...
{
    return &(g_stNasMmcCtx.stCurFsm.unFsmCtx.stBgPlmnSearchCtx);
}


NAS_MMC_RAT_SEARCH_INFO_STRU* NAS_MMC_GetSearchRatInfo_BgPlmnSearch(VOS_VOID)
{
    return &(g_stNasMmcCtx.stCurFsm.unFsmCtx.stBgPlmnSearchCtx.astSearchRatInfo[0]);
}
...
)
{
    ...
    NAS_NORMAL_LOG2(WUEPS_PID_MMC, "NAS_MMC_SetCsRegAdditionalAction_BgPlmnSearch,enCsAddtionalAction is; and ps is; ",enCsAddtionalAction, NAS_MMC_GetCsRegAdditionalAction_BgPlmnSearch());
...
)
{
    ...
    NAS_NORMAL_LOG2(WUEPS_PID_MMC, "NAS_MMC_SetPsRegAdditionalAction_BgPlmnSearch,enPsAddtionalAction is; and cs is; ",enPsAddtionalAction, NAS_MMC_GetCsRegAdditionalAction_BgPlmnSearch());
...
NAS_MML_PLMN_LIST_WITH_RAT_STRU* NAS_MMC_GetCurHighPrioPlmnList_BgPlmnSearch(VOS_VOID)
{
    ...
    if (NAS_MMC_FSM_BG_PLMN_SEARCH != NAS_MMC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_GetCurHighPrioPlmnList_BgPlmnSearch,ERROR:FsmId Error");
    }

    return &(g_stNasMmcCtx.stCurFsm.unFsmCtx.stBgPlmnSearchCtx.stCurHighPrioPlmnList);
}
...
{
    NAS_MMC_SERVICE_INFO_CTX_STRU      stPreSrvInfo;

    PS_MEM_SET(&stPreSrvInfo, 0, sizeof(NAS_MMC_SERVICE_INFO_CTX_STRU));

    stPreSrvInfo.enCsCurrService    = NAS_MMC_NO_SERVICE;
    stPreSrvInfo.enPsCurrService    = NAS_MMC_NO_SERVICE;

    NAS_MMC_SetPreServiceInfo_BgPlmnSearch(&stPreSrvInfo);
...
{
    NAS_MML_PLMN_RAT_PRIO_STRU                             *pstRatPrio          = VOS_NULL_PTR;
    NAS_MMC_FSM_BG_PLMN_SEARCH_CTX_STRU                    *pstBgPlmnSearchCtx  = VOS_NULL_PTR;
    VOS_UINT32                          i;

    pstRatPrio          = NAS_MML_GetMsPrioRatList();
    pstBgPlmnSearchCtx  = NAS_MMC_GetBgPlmnSearchCtx();

    /* syscfg支持的rat初始化成有效值 */
    for (i = 0; i < pstRatPrio->ucRatNum; i++)
    {
        pstBgPlmnSearchCtx->astSearchRatInfo[i].enRatType                     = pstRatPrio->aucRatPrio[i];
        pstBgPlmnSearchCtx->astSearchRatInfo[i].ucSearchAllBand               = VOS_FALSE;
        pstBgPlmnSearchCtx->astSearchRatInfo[i].enCoverageType                = NAS_MMC_COVERAGE_TYPE_BUTT;
        pstBgPlmnSearchCtx->astSearchRatInfo[i].ucSearchRplmnAndHplmnFlg      = VOS_FALSE;
        pstBgPlmnSearchCtx->astSearchRatInfo[i].ucSearchRplmnAndEplmnFlg      = VOS_FALSE;
        pstBgPlmnSearchCtx->astSearchRatInfo[i].ucSearchedRoamPlmnSortedFlag  = VOS_FALSE;
        pstBgPlmnSearchCtx->astSearchRatInfo[i].ucHistorySearchedFlag         = VOS_FALSE;
        pstBgPlmnSearchCtx->astSearchRatInfo[i].ucSearchDplmnAndHplmnFlg      = VOS_FALSE;
    }

    /* syscfg不支持的rat初始化成无效值 */
    for (i = pstRatPrio->ucRatNum; i < NAS_MML_MAX_RAT_NUM; i++)
    {
        pstBgPlmnSearchCtx->astSearchRatInfo[i].enRatType                     = NAS_MML_NET_RAT_TYPE_BUTT;
        pstBgPlmnSearchCtx->astSearchRatInfo[i].ucSearchAllBand               = VOS_FALSE;
        pstBgPlmnSearchCtx->astSearchRatInfo[i].enCoverageType                = NAS_MMC_COVERAGE_TYPE_BUTT;
        pstBgPlmnSearchCtx->astSearchRatInfo[i].ucSearchRplmnAndHplmnFlg      = VOS_FALSE;
        pstBgPlmnSearchCtx->astSearchRatInfo[i].ucSearchRplmnAndEplmnFlg      = VOS_FALSE;
...
)
{
    ...
    if (NAS_MMC_FSM_BG_PLMN_SEARCH != NAS_MMC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SetCurHighPrioPlmnList_BgPlmnSearch,ERROR:FsmId Error");
        return;
    }

    NAS_MMC_GetBgPlmnSearchCtx()->stCurHighPrioPlmnList   = *pstPlmnIdList;
}
...
NAS_MML_LAI_STRU* NAS_MMC_GetPreCampLai_BgPlmnSearch(VOS_VOID)
{
    ...
    if (NAS_MMC_FSM_BG_PLMN_SEARCH != NAS_MMC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_GetPreCampLai_BgPlmnSearch,ERROR:FsmId Error");
    }

    return &(NAS_MMC_GetBgPlmnSearchCtx()->stPreCampLai);
}
...
)
{
    ...
    if (NAS_MMC_FSM_BG_PLMN_SEARCH != NAS_MMC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SetPreCampLai_BgPlmnSearch,ERROR:FsmId Error");
        return;
    }

    PS_MEM_CPY(&(NAS_MMC_GetBgPlmnSearchCtx()->stPreCampLai), pstLai, sizeof(NAS_MML_LAI_STRU));
...
)
{
    ...
    pstLai  = NAS_MMC_GetPreCampLai_BgPlmnSearch();
    ...
    PS_MEM_SET(pstPlmnWithRat->aucReserve, 0, sizeof(pstPlmnWithRat->aucReserve));

    pstPlmnWithRat->stPlmnId.ulMcc    = pstLai->stPlmnId.ulMcc;
    pstPlmnWithRat->stPlmnId.ulMnc    = pstLai->stPlmnId.ulMnc;
    pstPlmnWithRat->enRat             = pstLai->enCampPlmnNetRat;
...
)
{
    ...
    NAS_MMC_GetPreCampLai_BgPlmnSearch()->stPlmnId = *pstPlmnId;
    NAS_MMC_GetPreCampLai_BgPlmnSearch()->enCampPlmnNetRat  = enRat;
...
)
{
    ...
    g_stNasMmcCtx.stCurFsm.unFsmCtx.stBgPlmnSearchCtx.stPreCampLai.enCampPlmnNetRat = enRat;
}
...
VOS_UINT8 NAS_MMC_GetCsgSearchingFlag_BgPlmnSearch(VOS_VOID)
{
    ...
    if (NAS_MMC_FSM_BG_PLMN_SEARCH != NAS_MMC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_GetCsgSearchingFlag_BgPlmnSearch,ERROR:FsmId Error");
    }

    return NAS_MMC_GetMmcCtxAddr()->stCurFsm.unFsmCtx.stBgPlmnSearchCtx.stCsgSearchInfo.ucCsgSearchingFlag;
}
...
)
{
    ...
    if (NAS_MMC_FSM_BG_PLMN_SEARCH != NAS_MMC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SetCurSearchCsgId_BgPlmnSearch,ERROR:FsmId Error");
        return;
    }

    NAS_MMC_GetMmcCtxAddr()->stCurFsm.unFsmCtx.stBgPlmnSearchCtx.stCsgSearchInfo.ucCsgSearchingFlag = ucCsgSearchingFlag;
}
...
NAS_MML_PLMN_CSG_ID_WITH_RAT_STRU* NAS_MMC_GetCurSearchCsgId_BgPlmnSearch(VOS_VOID)
{
    ...
    if (NAS_MMC_FSM_BG_PLMN_SEARCH != NAS_MMC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_GetCurSearchCsgId_BgPlmnSearch,ERROR:FsmId Error");
    }

    return &(NAS_MMC_GetMmcCtxAddr()->stCurFsm.unFsmCtx.stBgPlmnSearchCtx.stCsgSearchInfo.stCurrSearchCsgIdWithRat);
}
...
)
{
    ...
    NAS_MMC_GetMmcCtxAddr()->stCurFsm.unFsmCtx.stBgPlmnSearchCtx.stCsgSearchInfo.stCurrSearchCsgIdWithRat.stPlmnWithRat.stPlmnId = *pstPlmnId;
    NAS_MMC_GetMmcCtxAddr()->stCurFsm.unFsmCtx.stBgPlmnSearchCtx.stCsgSearchInfo.stCurrSearchCsgIdWithRat.ulCsgId                = ulCsgId;
    NAS_MMC_GetMmcCtxAddr()->stCurFsm.unFsmCtx.stBgPlmnSearchCtx.stCsgSearchInfo.stCurrSearchCsgIdWithRat.stPlmnWithRat.enRat    = enRat;
}
#endif
...
)
{
    ...
    if (NAS_MMC_FSM_BG_PLMN_SEARCH != NAS_MMC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SetPreServiceInfo_BgPlmnSearch,ERROR:FsmId Error");
        return;
    }

    NAS_MMC_GetBgPlmnSearchCtx()->stPreServiceInfo.enCsCurrService  = pstServiceInfo->enCsCurrService;
    NAS_MMC_GetBgPlmnSearchCtx()->stPreServiceInfo.enPsCurrService  = pstServiceInfo->enPsCurrService;
...
NAS_MMC_SERVICE_INFO_CTX_STRU *NAS_MMC_GetPreServiceInfo_BgPlmnSearch(VOS_VOID)
{
    ...
    if (NAS_MMC_FSM_BG_PLMN_SEARCH != NAS_MMC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_GetPreServiceInfo_BgPlmnSearch,ERROR:FsmId Error");
    }

    return &(NAS_MMC_GetBgPlmnSearchCtx()->stPreServiceInfo);
...
)
{
    ...
    if (NAS_MMC_FSM_BG_PLMN_SEARCH != NAS_MMC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SetHistorySearchedFlag_BgPlmnSearch,ERROR:FsmId Error");
        return;
    }

    /* 查找BG状态机上下文中与enRat相同的接入技术数组 */
    for (i = 0; i < NAS_MML_MAX_RAT_NUM; i++)
    {
        if (enRat == (g_stNasMmcCtx.stCurFsm.unFsmCtx.stBgPlmnSearchCtx.astSearchRatInfo[i].enRatType))
        {
            NAS_MMC_GetBgPlmnSearchCtx()->astSearchRatInfo[i].ucHistorySearchedFlag = ucHistorySearchedFlag;
            return;
        }
    }

    NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SetHistorySearchedFlag_BgPlmnSearch,ERROR:enRatType Error");
...
)
{
    ...
    /* 如果当前状态机不是BG PLMN SEARCH */
    if (NAS_MMC_FSM_BG_PLMN_SEARCH != NAS_MMC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_GetHistorySearchedFlag_BgPlmnSearch,ERROR:FsmId Error");
    ...
    for (i = 0; i < NAS_MML_MAX_RAT_NUM; i++)
    {
        if (enRat == (NAS_MMC_GetBgPlmnSearchCtx()->astSearchRatInfo[i].enRatType))
        {
            return NAS_MMC_GetBgPlmnSearchCtx()->astSearchRatInfo[i].ucHistorySearchedFlag;
        }
    }

    NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_GetHistorySearchedFlag_BgPlmnSearch,ERROR:enRatType Error");
...
)
{
    ...
    if (NAS_MMC_FSM_BG_PLMN_SEARCH != NAS_MMC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SetSearchRplmnAndHplmnFlg_BgPlmnSearch,ERROR:FsmId Error");
        return;
    }
    ...
    for (i = 0; i < NAS_MML_MAX_RAT_NUM; i++)
    {
        if (enRat == (NAS_MMC_GetBgPlmnSearchCtx()->astSearchRatInfo[i].enRatType))
        {
            NAS_MMC_GetBgPlmnSearchCtx()->astSearchRatInfo[i].ucSearchRplmnAndHplmnFlg = ucSearchRplmnAndHplmnFlg;
            return;
        }
    }

    NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SetSearchRplmnAndHplmnFlg_BgPlmnSearch,ERROR:enRatType Error");
...
)
{
    ...
    if (NAS_MMC_FSM_BG_PLMN_SEARCH != NAS_MMC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_GetSearchRplmnAndHplmnFlg_BgPlmnSearch,ERROR:FsmId Error");
    ...
    for (i = 0; i < NAS_MML_MAX_RAT_NUM; i++)
    {
        if (enRat == (NAS_MMC_GetBgPlmnSearchCtx()->astSearchRatInfo[i].enRatType))
        {
            return NAS_MMC_GetBgPlmnSearchCtx()->astSearchRatInfo[i].ucSearchRplmnAndHplmnFlg;
        }
    }

    NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_GetSearchRplmnAndHplmnFlg_BgPlmnSearch,ERROR:enRatType Error");
...
)
{
    ...
    if (NAS_MMC_FSM_BG_PLMN_SEARCH != NAS_MMC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SetSearchDplmnAndHplmnFlg_BgPlmnSearch,ERROR:FsmId Error");
        return;
    }
    ...
    for (i = 0; i < NAS_MML_MAX_RAT_NUM; i++)
    {
        if (enRat == (NAS_MMC_GetBgPlmnSearchCtx()->astSearchRatInfo[i].enRatType))
        {
            NAS_MMC_GetBgPlmnSearchCtx()->astSearchRatInfo[i].ucSearchDplmnAndHplmnFlg = ucSearchDplmnAndHplmnFlg;
            return;
        }
    }

    NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SetSearchDplmnAndHplmnFlg_BgPlmnSearch,ERROR:enRatType Error");
...
)
{
    ...
    if (NAS_MMC_FSM_BG_PLMN_SEARCH != NAS_MMC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_GetSearchDplmnAndHplmnFlg_BgPlmnSearch,ERROR:FsmId Error");
    ...
    for (i = 0; i < NAS_MML_MAX_RAT_NUM; i++)
    {
        if (enRat == (NAS_MMC_GetBgPlmnSearchCtx()->astSearchRatInfo[i].enRatType))
        {
            return NAS_MMC_GetBgPlmnSearchCtx()->astSearchRatInfo[i].ucSearchDplmnAndHplmnFlg;
        }
    }

    NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_GetSearchDplmnAndHplmnFlg_BgPlmnSearch,ERROR:enRatType Error");
...
)
{
    ...
    if (NAS_MMC_FSM_BG_PLMN_SEARCH != NAS_MMC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SetSearchRplmnAndEplmnFlg_BgPlmnSearch,ERROR:FsmId Error");
        return;
    }
    ...
    for (i = 0; i < NAS_MML_MAX_RAT_NUM; i++)
    {
        if (enRat == (NAS_MMC_GetBgPlmnSearchCtx()->astSearchRatInfo[i].enRatType))
        {
            NAS_MMC_GetBgPlmnSearchCtx()->astSearchRatInfo[i].ucSearchRplmnAndEplmnFlg = ucSearchRplmnAndEplmnFlg;
            return;
        }
    }

    NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SetSearchRplmnAndEplmnFlg_BgPlmnSearch,ERROR:enRatType Error");
...
)
{
    ...
    if (NAS_MMC_FSM_BG_PLMN_SEARCH != NAS_MMC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_GetSearchRplmnAndEplmnFlg_BgPlmnSearch,ERROR:FsmId Error");
    ...
    for (i = 0; i < NAS_MML_MAX_RAT_NUM; i++)
    {
        if (enRat == (NAS_MMC_GetBgPlmnSearchCtx()->astSearchRatInfo[i].enRatType))
        {
            return NAS_MMC_GetBgPlmnSearchCtx()->astSearchRatInfo[i].ucSearchRplmnAndEplmnFlg;
        }
    }

    NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_GetSearchRplmnAndEplmnFlg_BgPlmnSearch,ERROR:enRatType Error");
...
VOS_UINT8 NAS_MMC_IsNeedGoToNextPhaseByHistory( VOS_VOID )
{
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedGoToNextPhaseByHistory: Go to next phase.");
...
{
    return (NAS_MMC_GetMmcCtxAddr()->stPlmnSearchCtrl.stCsgSpecSearchInfo.ucDuplicateRplmnAndEplmnValidFlag);
}
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_SetCsgDuplicateRplmnAndEplmnValidFlag,ucDuplicateRplmnAndEplmnValidFlag is ;",ucDuplicateRplmnAndEplmnValidFlag);

    NAS_MMC_GetMmcCtxAddr()->stPlmnSearchCtrl.stCsgSpecSearchInfo.ucDuplicateRplmnAndEplmnValidFlag = ucDuplicateRplmnAndEplmnValidFlag;
    return;
}
...
{
    return &(NAS_MMC_GetMmcCtxAddr()->stPlmnSearchCtrl.stCsgSpecSearchInfo.stUserCsgSpecSrchPlmnWithCsgId);
}
...
VOS_VOID NAS_MMC_InitCsgSpecSrchPlmnWithCsgId( VOS_VOID )
{
    ...
    PS_MEM_SET(pstPlmnWithCsgId, 0, sizeof(NAS_MML_PLMN_CSG_ID_WITH_RAT_STRU));

    pstPlmnWithCsgId->stPlmnWithRat.stPlmnId.ulMcc = NAS_MML_INVALID_MCC;
    pstPlmnWithCsgId->stPlmnWithRat.stPlmnId.ulMnc = NAS_MML_INVALID_MNC;
    pstPlmnWithCsgId->stPlmnWithRat.enRat          = NAS_MML_NET_RAT_TYPE_BUTT;
    pstPlmnWithCsgId->ulCsgId                      = NAS_MML_INVALID_CSG_ID_VALUE;
...
)
{
    ...
    if (NAS_MMC_FSM_PLMN_SELECTION != NAS_MMC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_GetSearchedCsgSpecSearchFlag_PlmnSelection,ERROR:FsmId Error");
    }
    ...
    for (i = 0 ; i < NAS_MML_MAX_RAT_NUM; i++)
    {
        if (enRat == (g_stNasMmcCtx.stCurFsm.unFsmCtx.stPlmnSelectionCtx.astSearchRatInfo[i].enRatType))
        {
            return g_stNasMmcCtx.stCurFsm.unFsmCtx.stPlmnSelectionCtx.astSearchRatInfo[i].ucSearchedCsgSpecSearchFlag;
        }
    }

    NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_GetSearchedCsgSpecSearchFlag_PlmnSelection,ERROR:enRatType Error");
...
)
{
    ...
    if (NAS_MMC_FSM_PLMN_SELECTION != NAS_MMC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_SetSearchedCsgSpecSearchFlag_PlmnSelection,ERROR:FsmId Error");
        return;
    }
    ...
    for (i = 0 ; i < NAS_MML_MAX_RAT_NUM; i++)
    {
        if (enRat == (g_stNasMmcCtx.stCurFsm.unFsmCtx.stPlmnSelectionCtx.astSearchRatInfo[i].enRatType))
        {
            g_stNasMmcCtx.stCurFsm.unFsmCtx.stPlmnSelectionCtx.astSearchRatInfo[i].ucSearchedCsgSpecSearchFlag = ucSearchedCsgSpecSearchFlag;
            return;
        }
    }
}
...
{
    return &(NAS_MMC_GetMmcCtxAddr()->stPlmnSearchCtrl.stCsgSpecSearchInfo.stDuplicateRplmnAndEplmnInfo);
}


NAS_MML_NET_RAT_TYPE_ENUM_UINT8 NAS_MMC_GetCsgSpecSrchDuplicateRplmnRat(VOS_VOID)
{
    return NAS_MMC_GetMmcCtxAddr()->stPlmnSearchCtrl.stCsgSpecSearchInfo.stDuplicateRplmnAndEplmnInfo.enRat;
}
...
VOS_VOID NAS_MMC_InitCsgSpecSrchDuplicateRplmnAndEplmnInfo( VOS_VOID )
{
    ...
    /* 设置duplicate Rplmn和Eplmn标识为无效 */
    NAS_MMC_SetCsgDuplicateRplmnAndEplmnValidFlag(VOS_FALSE);

    pstDuplicateRplmnAndEplmn = NAS_MMC_GetCsgSpecSrchDuplicateRplmnAndEplmnInfo();

    PS_MEM_SET(pstDuplicateRplmnAndEplmn, 0, sizeof(NAS_MML_CSG_DUPLICATED_RPLMN_AND_EPLMN_LIST_STRU));

    for (ulIndex = 0; ulIndex < NAS_MML_MAX_EHPLMN_NUM; ulIndex++)
    {
        pstDuplicateRplmnAndEplmn->astPlmnId[ulIndex].ulMcc = NAS_MML_INVALID_MCC;
        pstDuplicateRplmnAndEplmn->astPlmnId[ulIndex].ulMnc = NAS_MML_INVALID_MNC;
    }

    pstDuplicateRplmnAndEplmn->enRat = NAS_MML_NET_RAT_TYPE_BUTT;
...
{
    return (NAS_MMC_GetMmcCtxAddr()->stCsgBgSearchCtrl.enUseCsgIdSearchListProc);
}
...
{
    (NAS_MMC_GetMmcCtxAddr()->stCsgBgSearchCtrl.enUseCsgIdSearchListProc) = enUseCsgIdSearchListProc;
     return;
}
...
{
    return &(NAS_MMC_GetMmcCtxAddr()->stCsgBgSearchCtrl.stCsgIdSearchListInfo);
}


NAS_MMC_CSG_ASF_PHASE_ENUM_UINT8 NAS_MMC_GetCsgASFPhaseNum(VOS_VOID)
{
    return NAS_MMC_GetMmcCtxAddr()->stCsgBgSearchCtrl.enCsgAsfPhaseNum;
}
...
{
    (NAS_MMC_GetMmcCtxAddr()->stCsgBgSearchCtrl.enCsgAsfPhaseNum) = enCsgAsfPhaseNum;
     return;
}

VOS_UINT8 NAS_MMC_GetCsgASFTryNum(VOS_VOID)
{
    return (NAS_MMC_GetMmcCtxAddr()->stCsgBgSearchCtrl.ucCsgAsfTryNum);
}


VOS_VOID NAS_MMC_ResetCsgASFPhaseNum(VOS_VOID)
{
    NAS_MMC_SetCsgASFPhaseNum(NAS_MMC_CSG_ASF_PHASE_ONE);
    return;
}
...
{
    (NAS_MMC_GetMmcCtxAddr()->stCsgBgSearchCtrl.ucCsgAsfTryNum) = 0;
    return;
}

VOS_VOID NAS_MMC_AddCsgASFTryNum(VOS_VOID)
{
    (NAS_MMC_GetMmcCtxAddr()->stCsgBgSearchCtrl.ucCsgAsfTryNum) += 0x01;
    return;
}
...
)
{
    ...
    pstCsgBgSearchCtrl->stCsgIdSearchListInfo.usSearchPlmnNum = 0;
    pstCsgBgSearchCtrl->stCsgIdSearchListInfo.aucReserve[0]   = 0;
    pstCsgBgSearchCtrl->stCsgIdSearchListInfo.aucReserve[1]   = 0;

    for (i = 0; i < NAS_MMC_MAX_PLMN_NUM_IN_SELECTION_LIST; i++)
    {
        pstCsgBgSearchCtrl->stCsgIdSearchListInfo.astPlmnSelectionList[i].stPlmnId.ulMcc = NAS_MML_INVALID_MCC;
        pstCsgBgSearchCtrl->stCsgIdSearchListInfo.astPlmnSelectionList[i].stPlmnId.ulMnc = NAS_MML_INVALID_MNC;
        pstCsgBgSearchCtrl->stCsgIdSearchListInfo.astPlmnSelectionList[i].enPlmnType     = NAS_MMC_PLMN_TYPE_BUTT;
        pstCsgBgSearchCtrl->stCsgIdSearchListInfo.astPlmnSelectionList[i].ucRatNum       = 0;
        pstCsgBgSearchCtrl->stCsgIdSearchListInfo.astPlmnSelectionList[i].aucReserve[0]  = 0;
        pstCsgBgSearchCtrl->stCsgIdSearchListInfo.astPlmnSelectionList[i].aucReserve[1]  = 0;
        pstCsgBgSearchCtrl->stCsgIdSearchListInfo.astPlmnSelectionList[i].ulCsgId        = NAS_MML_INVALID_CSG_ID_VALUE;

        for (j = 0; j < NAS_MML_MAX_RAT_NUM; j++)
        {
            pstCsgBgSearchCtrl->stCsgIdSearchListInfo.astPlmnSelectionList[i].astPlmnRatInfo[j].enRatType   = NAS_MML_NET_RAT_TYPE_BUTT;
            pstCsgBgSearchCtrl->stCsgIdSearchListInfo.astPlmnSelectionList[i].astPlmnRatInfo[j].enNetStatus = NAS_MMC_NET_STATUS_BUTT;
            pstCsgBgSearchCtrl->stCsgIdSearchListInfo.astPlmnSelectionList[i].astPlmnRatInfo[j].enQuality   = NAS_MMC_NET_QUALITY_BUTT;
            pstCsgBgSearchCtrl->stCsgIdSearchListInfo.astPlmnSelectionList[i].astPlmnRatInfo[j].ucReserve   = 0;
            pstCsgBgSearchCtrl->stCsgIdSearchListInfo.astPlmnSelectionList[i].astPlmnRatInfo[j].lRscp       = NAS_MML_UTRA_RSCP_UNVALID;
        }
    }
...
{
    return (NAS_MMC_GetMmcCtxAddr()->stPlmnSearchCtrl.ulCsgSimRefreshReadSimFileFlg);
}
...
{
    NAS_MMC_GetMmcCtxAddr()->stPlmnSearchCtrl.ulCsgSimRefreshReadSimFileFlg |= enSimFileCnfFlg;
}
...
{
    NAS_MMC_GetMmcCtxAddr()->stPlmnSearchCtrl.ulCsgSimRefreshReadSimFileFlg = NAS_MML_READ_USIM_FILE_FLG_NULL;
}
...
)
{
    ...
    {
        NAS_MMC_ClearCsgRefreshSimFileReadFlg(enEfId);
    }
    else
    {
        NAS_MMC_ClearWaitSimFilesCnfFlg_SwitchOn(enEfId);
    }
}
...
)
{
    ...
    NAS_NORMAL_LOG2(WUEPS_PID_MMC, "NAS_MMC_SetDisabledLtePlmnId: ",pstPlmnId->ulMcc,pstPlmnId->ulMnc);
...
{
    g_stNasMmcCtx.stBackUpSearchInfo.ucLteEnabledFlgWhenFastSpecSrch    = ucLteEnabledFlg;
    return;
}
...
{
    return (g_stNasMmcCtx.stBackUpSearchInfo.ucLteEnabledFlgWhenFastSpecSrch);
}
...
VOS_UINT32  NAS_MMC_GetCurOosSearchSleepCount_L1Main(VOS_VOID)
{
    ...
    if (NAS_MMC_FSM_L1_MAIN != NAS_MMC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_GetCurOosSearchSleepCount_L1Main,ERROR:FsmId Error");
    }

    return g_stNasMmcCtx.stCurFsm.unFsmCtx.stL1MainCtx.ulCurOosSearchSleepCount;
}
...
VOS_VOID    NAS_MMC_ResetCurOosSearchSleepCount_L1Main(VOS_VOID)
{
    ...
    if (NAS_MMC_FSM_L1_MAIN != NAS_MMC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_ResetCurOosSearchSleepCount_L1Main,ERROR:FsmId Error");

        return;
    }

    g_stNasMmcCtx.stCurFsm.unFsmCtx.stL1MainCtx.ulCurOosSearchSleepCount = 0;
}
...
VOS_VOID    NAS_MMC_AddCurOosSearchSleepCount_L1Main(VOS_VOID)
{
    ...
    if (NAS_MMC_FSM_L1_MAIN != NAS_MMC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_AddCurOosSearchSleepCount_L1Main,ERROR:FsmId Error");

        return;
    }

    g_stNasMmcCtx.stCurFsm.unFsmCtx.stL1MainCtx.ulCurOosSearchSleepCount++;
}


VOS_UINT8 NAS_MMC_GetCurHighRatHplmnSleepCount(VOS_VOID)
{
    return g_stNasMmcCtx.stHighPrioPlmnSearchCtrl.ucCurHighRatHplmnSleepCount;
}


VOS_VOID NAS_MMC_AddCurHighRatHplmnSleepCount(VOS_VOID)
{
    g_stNasMmcCtx.stHighPrioPlmnSearchCtrl.ucCurHighRatHplmnSleepCount++;
...
{
    g_stNasMmcCtx.stHighPrioPlmnSearchCtrl.ucCurHighRatHplmnSleepCount = 0;
...
{
    VOS_UINT8                           ucNvActFlag;
    VOS_UINT8                           ucNvSleepCount;
    VOS_UINT32                          ulCurrSleepCount;

    ucNvActFlag             = NAS_MML_GetAiModemActFlg();
    ucNvSleepCount          = NAS_MML_GetOosPlmnSearchSleepCount();
    ulCurrSleepCount        = NAS_MMC_GetCurOosSearchSleepCount_L1Main();

    /*第二阶段，无CSFB CALL，NV开关打开，睡眠次数不到并且是低功耗模式才跳过历史搜*/
    if (  (NAS_MMC_OOS_PHASE_TWO == NAS_MMC_GetOosPhaseNum())
       && (VOS_FALSE             == NAS_MML_GetCsfbMoSetupFlg())
       && (VOS_TRUE              == ucNvActFlag)
       && (ulCurrSleepCount      <  ucNvSleepCount)
       && (VOS_TRUE              == NAS_MML_IsSavePowerMode()))
    {
        NAS_NORMAL_LOG2(WUEPS_PID_MMC, "NAS_MMC_IsNeedSkipHistorySearch: YES", ucNvSleepCount, ulCurrSleepCount);

        return VOS_TRUE;
    }
    else
    {
        NAS_NORMAL_LOG3(WUEPS_PID_MMC, "NAS_MMC_IsNeedSkipHistorySearch: NO", ucNvActFlag, ucNvSleepCount, ulCurrSleepCount);
...
VOS_UINT8 NAS_MMC_IsNeedSkipHighPrioRatSearch(VOS_VOID)
{
    ...
    ucNvActFlag             = NAS_MML_GetAiModemActFlg();
    ucNvSleepCount          = NAS_MML_GetHighPrioRatHplmnSleepCount();
    ulCurrSleepCount        = NAS_MMC_GetCurHighRatHplmnSleepCount();

    /*第二阶段，NV开关打开，睡眠次数不到并且是低功耗模式才跳过高优先级背景搜*/
    if (  (ulCount           >= pstHighRatHplmnTimerCfg->ulFirstSearchTimeCount)
       && (VOS_TRUE          == ucNvActFlag)
       && (ulCurrSleepCount  <  ucNvSleepCount)
       && (VOS_TRUE          == NAS_MML_IsSavePowerMode()))
    {
        NAS_NORMAL_LOG3(WUEPS_PID_MMC, "NAS_MMC_IsNeedSkipHighPrioRatSearch: YES",
            ulCount, ucNvSleepCount, ulCurrSleepCount);

        return VOS_TRUE;
    }
    else
    {
        NAS_NORMAL_LOG4(WUEPS_PID_MMC, "NAS_MMC_IsNeedSkipHighPrioRatSearch: No",
            ulCount, ucNvActFlag, ucNvSleepCount, ulCurrSleepCount);
