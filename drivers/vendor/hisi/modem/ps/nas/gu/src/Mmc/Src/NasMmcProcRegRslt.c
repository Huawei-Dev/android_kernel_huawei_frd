...
NAS_MMC_COMBINED_EPS_REG_REJ_PROC_ENTITY             gastMmcProcCombinedAttachEpsRegFailCause[] =
{
    ...
    {NAS_EMM_CAUSE_NOT_AUTHORIZED_FOR_THIS_CSG,                       NAS_MMC_ProcCombinedEpsRegFailCause25NotAuthorizedForThisCsg},
#endif
...
#if (FEATURE_ON == FEATURE_CSG)
    {NAS_EMM_CAUSE_NOT_AUTHORIZED_FOR_THIS_CSG,                       NAS_MMC_ProcEpsRegFailCause25NotAuthorizedForThisCsg},
#endif
...
#if (FEATURE_ON == FEATURE_CSG)
    {NAS_EMM_CAUSE_NOT_AUTHORIZED_FOR_THIS_CSG,                       NAS_MMC_ProcEpsDetachCause25NotAuthorizedForThisCsg},
#endif
};
...
NAS_MMC_ADDITIONAL_ACTION_ENUM_UINT8 NAS_MMC_ProcCsRegFailAttemptCounterMaxTimes(VOS_VOID)
{
    ...
    if (VOS_TRUE == NAS_MML_GetCsDamNoSearchFlag())
    {
        enAction = NAS_MMC_ADDITIONAL_ACTION_NORMAL_CAMP_ON;
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_ProcCsRegFailAttemptCounterMaxTimes: DAM no search status!");
    }
    /* 最大次数失败被拒是否立即触发搜网，VDF定制打开或DT单域搜网定制打开时搜网；不打开时，等待PS的注册结果 */
    else if ((VOS_TRUE == NAS_MML_GetCsRejSearchSupportFlg())
     || (VOS_TRUE == ucSingleDomainSrchFlag))
    {
...
NAS_MMC_ADDITIONAL_ACTION_ENUM_UINT8 NAS_MMC_ProcPsRegFailCause7GprsServNotAllow(VOS_VOID)
{
    ...
    if (VOS_TRUE == NAS_MMC_IsNeedDisableL_PsFailCause7PsServNotAllow())
    {
        ...
        NAS_MML_DisableLteCommProc(NAS_MML_LTE_CAPABILITY_STATUS_DISABLE_NOTIFIED_AS,
                                   MMC_LMM_DISABLE_LTE_REASON_PS_USIM_INVALID);        
    }
#endif
    return NAS_MMC_ADDITIONAL_ACTION_SINGLE_DOMAIN_ANYCELL;
}
...
NAS_MMC_ADDITIONAL_ACTION_ENUM_UINT8 NAS_MMC_ProcPsRegFailAttemptCounterMaxTimes(VOS_VOID)
{
    ...
    if (VOS_TRUE == NAS_MML_GetPsDamNoSearchFlag())
    {
        enAction = NAS_MMC_ADDITIONAL_ACTION_NORMAL_CAMP_ON;
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_ProcPsRegFailAttemptCounterMaxTimes: DAM no search status!");
    }
    /* SOR打开手动模式搜网时候，返回LIMIT CAMP ON */
    else if ( (VOS_TRUE == NAS_MML_GetCsRejSearchSupportFlg())
...
VOS_VOID NAS_MMC_ProcCombinedEpsRegFailCause25NotAuthorizedForThisCsg(
    NAS_MMC_ADDITIONAL_ACTION_ENUM_UINT8                   *penPsRegAdditionalAction,
    NAS_MMC_ADDITIONAL_ACTION_ENUM_UINT8                   *penCsRegAdditionalAction
)
{
    ...
    ulCsgSpecSearchFlg = NAS_MMC_IsUserCsgSpecSearchType_PlmnSelection(enCurrentFsmId,ulCurrentEventType);

    /* 在CSG小区上收到#25,
       1. 如果是CSG指定搜网过程且存在duplicate plmn, 则要根据搜网列表进行下个搜网;
       2. 如果是CSG指定搜但不存在duplicat plmn或者不是CSG指定搜，则要尝试在当前plmn搜网
       在非CSG小区上收到#25， 按照abnormal cause处理*/
    if (MMC_LMM_CELL_TYPE_CSG == enCurrCampCellType)
    {
        NAS_MMC_ChangeServiceState(NAS_MSCC_PIF_SRVDOMAIN_CS_PS, NAS_MMC_LIMITED_SERVICE);
        NAS_MMC_ChangePsRegState(NAS_MML_REG_NOT_REGISTERED_SEARCHING);
        NAS_MMC_ChangeCsRegState(NAS_MML_REG_NOT_REGISTERED_SEARCHING);

        if ((VOS_TRUE == NAS_MMC_GetCsgDuplicateRplmnAndEplmnValidFlag())
         && (VOS_TRUE == ulCsgSpecSearchFlg))
        {
            *penCsRegAdditionalAction = NAS_MMC_ADDITIONAL_ACTION_PLMN_SELECTION;
            *penPsRegAdditionalAction = NAS_MMC_ADDITIONAL_ACTION_PLMN_SELECTION;
        }
...
)
{
    ...
    else
    {
        ...
        if ( VOS_TRUE == NAS_MMC_IsNeedDisableL_CombinedRegRsltFailMaxTimes() )
        {
            ...
            NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_ProcCombinedEpsRegFailCause19EsmFailure: MMC_LMM_DISABLE_LTE_REASON_LTE_VOICE_NOT_AVAILABLE");
...
)
{
    ...
    /* 如果当前驻留在CSG小区上，不认为卡无效，只将此CSG ID从Allowed CSG List中删除，继续搜当前PLMN的其他网络 */
    if (NAS_MML_CELL_TYPE_CSG == NAS_MML_GetCurrCampCellType())
    {
        /* 从Allowed CSG List中删除操作可以通过NV配置，在后面会集中处理 */
...
)
{
    ...
    else
    {
        ...
        if ( VOS_TRUE == NAS_MMC_IsNeedDisableL_CombinedRegRsltFailMaxTimes() )
        {
            ...
            NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_ProcCombinedEpsRegFailCause301TimerTimeOut: MMC_LMM_DISABLE_LTE_REASON_LTE_VOICE_NOT_AVAILABLE");
...
)
{
    ...
    /* 选网状态机和背景搜状态机场景下，需要触发搜网，不然会限制驻留在L上，而PS不可用 */
    if ((NAS_MMC_FSM_PLMN_SELECTION == enFsmId)
     || (NAS_MMC_FSM_BG_PLMN_SEARCH == enFsmId))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_ProcCombinedEpsRegFailCausePsAttachNotAllow: Set CS&PS to NAS_MMC_ADDITIONAL_ACTION_PLMN_SELECTION");
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_ProcCombinedEpsRegFailCausePsAttachNotAllow: Set CS&PS to NAS_MMC_ADDITIONAL_ACTION_LIMITED_CAMP_ON");
...
NAS_MMC_ADDITIONAL_ACTION_ENUM_UINT8 NAS_MMC_ProcEpsRegFailCause25NotAuthorizedForThisCsg(VOS_VOID)
{
    NAS_MMC_FSM_ID_ENUM_UINT32              enCurrentFsmId;
    MMC_LMM_CELL_TYPE_ENUM_UINT8            enCurrCampCellType;
    VOS_UINT32                              ulCurrentEventType;
    ...
    ulCurrentEventType      = NAS_MMC_GetCurrFsmEventType();
    enCurrCampCellType      = NAS_MML_GetCurrCampCellType();
    enRegRsltCause          = NAS_MML_REG_FAIL_CAUSE_NULL;
    enRegRsltType           = NAS_MMC_REG_RSULT_TYPE_BUTT;
    enAction                = NAS_MMC_ADDITIONAL_ACTION_BUTT;
    ulAttemptCnt            = 0;
    ulCsgSpecSearchFlg      = NAS_MMC_IsUserCsgSpecSearchType_PlmnSelection(enCurrentFsmId,ulCurrentEventType);
...
VOS_VOID NAS_MMC_ProcEpsDetachCause25NotAuthorizedForThisCsg(
    NAS_MMC_ADDITIONAL_ACTION_ENUM_UINT8                   *penCsRegAdditionalAction,
    NAS_MMC_ADDITIONAL_ACTION_ENUM_UINT8                   *penPsRegAdditionalAction
)
{
    ...
    {
        NAS_MMC_ProcEpsDetachCauseAbnormalCause(penCsRegAdditionalAction, penPsRegAdditionalAction);
    }
}
#endif
...
VOS_UINT32 NAS_MMC_IsNeedDisableL_RegFailCause16AttmptCntMaxTimes(VOS_VOID)
{
    ...
    if (VOS_TRUE == NAS_MML_IsLteOnlyMode(NAS_MML_GetMsPrioRatList()))
    {
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_RegFailCause16AttmptCntMaxTimes: LTE ONLY.");
        return VOS_FALSE;
    }
    ...
    {
        NAS_WARNING_LOG2(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_RegFailCause16AttmptCntMaxTimes: ims flag, Uemode.", NAS_MML_GetImsVoiceAvailFlg(), enLUeMode);
        return VOS_FALSE;
    }
    ...
    if (NAS_MML_GetLte3gppRelVersion() < NAS_MML_3GPP_REL_R11)
    {
        NAS_WARNING_LOG1(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_RegFailCause16AttmptCntMaxTimes: Rel .", NAS_MML_GetLte3gppRelVersion());
        return VOS_TRUE;
    }

    if (NAS_MML_PERSISTENT_BEARER_STATE_NOT_EXIST == NAS_MML_GetPersistentBearerState())
    {
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_RegFailCause16AttmptCntMaxTimes: NAS_MML_PERSISTENT_BEARER_STATE_NOT_EXIST.");

        return VOS_TRUE;
    }

    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_RegFailCause16AttmptCntMaxTimes: result is false");
...
NAS_MMC_ADDITIONAL_ACTION_ENUM_UINT8 NAS_MMC_ProcEpsServiceRequestFailCause18CsDomainNotAvailable(VOS_VOID)
{
    ...
    if (VOS_TRUE == NAS_MMC_IsNeedDisableL_ServiceReqFailCause18CsDomainNotAvailable())
    {
        ...
        else
        {
            ...
            (VOS_VOID)NAS_MMC_StartTimer(TI_NAS_MMC_WAIT_ENABLE_LTE_TIMER, NAS_MML_GetCsPsMode1EnableLteTimerLen());
            NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_ProcEpsServiceRequestFailCause18CsDomainNotAvailable: MMC_LMM_DISABLE_LTE_REASON_LTE_VOICE_NOT_AVAILABLE");
        }
...
VOS_VOID NAS_MMC_ProcLteCsgRegFail(
    NAS_MML_REG_FAIL_CAUSE_ENUM_UINT16  enRegFailCause
)
{
    NAS_MML_PLMN_WITH_CSG_ID_STRU       stPlmnWithCsgId;

    PS_MEM_SET(&stPlmnWithCsgId, 0, sizeof(stPlmnWithCsgId));
    PS_MEM_CPY(&stPlmnWithCsgId.stPlmnId, NAS_MML_GetCurrCampPlmnId(), sizeof(stPlmnWithCsgId.stPlmnId));
    stPlmnWithCsgId.ulCsgId = NAS_MML_GetCurrCampCsgId();

    if (VOS_TRUE == NAS_MMC_IsRegFailNeedAddCsgIdInForbiddenCsgIdList(&stPlmnWithCsgId, enRegFailCause))
    {
        NAS_MMC_AddCsgIdInForbiddenCsgIdList(&stPlmnWithCsgId, NAS_MML_GetCsgForbiddenPlmnPeriod(), enRegFailCause);
    }

    /* 判断是否需要将CSG网络从Allowed CSG List中删除 */
    if (VOS_TRUE == NAS_MMC_IsRegFailNeedDeleteCsgIdInAllowedCsgList(&stPlmnWithCsgId, enRegFailCause))
    {
        NAS_MMC_DeleteCsgIdInPlmnWithCsgIdList(&stPlmnWithCsgId, NAS_MML_GetAllowedCsgList());
...
)
{
    ...
    else
    {
            ...
            )
        {
            ...
            NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_ProcCombinedEpsRegFailCauseAbnormalCause: MMC_LMM_DISABLE_LTE_REASON_LTE_VOICE_NOT_AVAILABLE");
...
)
{
    ...
    else
    {
        ...
        if ( VOS_TRUE == NAS_MMC_IsNeedDisableL_CombinedRegRsltFailMaxTimes() )
        {
            ...
            NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_ProcCombinedEpsRegFailCause401OtherCause: MMC_LMM_DISABLE_LTE_REASON_LTE_VOICE_NOT_AVAILABLE");
...
#if (FEATURE_ON == FEATURE_CSG)
        NAS_MMC_ProcLteCsgRegFail(enPsRegRsltCause);
#endif
    }
...
#if (FEATURE_ON == FEATURE_CSG)
        NAS_MMC_ProcLteCsgRegFail(enRegRsltCause);
#endif
...
)
{
        ...
        )
    {
        ...
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_ProcLmmServiceRsltInd: MMC_LMM_DISABLE_LTE_REASON_LTE_VOICE_NOT_AVAILABLE");
...
#if (FEATURE_ON == FEATURE_CSG)
    /* L下注册成功，判断是否需要将注册成功网络加入Allowed CSG List */
    PS_MEM_SET(&stPlmnWithCsgId, 0, sizeof(stPlmnWithCsgId));
    PS_MEM_CPY(&stPlmnWithCsgId.stPlmnId, NAS_MML_GetCurrCampPlmnId(), sizeof(stPlmnWithCsgId.stPlmnId));
    stPlmnWithCsgId.ulCsgId = NAS_MML_GetCurrCampCsgId();

    if (VOS_TRUE == NAS_MMC_IsRegSuccNeedAddCsgIdInAllowedCsgList(&stPlmnWithCsgId, NAS_MML_GetCurrCampCsgIdHomeNodeBName()))
    {
        NAS_MMC_AddCsgIdInfoInAllowedCsgList(&stPlmnWithCsgId, NAS_MML_GetCurrCampCsgIdHomeNodeBName());

        /* 如果加入allowed csg list中的csg id在禁止csg列表中，则需要删除 */
        NAS_MMC_DeleteSpecCsgIdInForbiddenCsgIdList(&stPlmnWithCsgId);
    ...
    case MMC_LMM_TAU_RSLT_FAILURE:
        if (VOS_FALSE == pstLmmTauIndMsg->bitOpAtmpCnt)
        {
            if (NAS_MMC_FSM_L1_MAIN == NAS_MMC_GetCurrFsmId())
            {
                ...
                /* 当前驻留的PLMN是Forbidden PLMN，LMM后续不会发起注册，启available定时器 */
                if (MMC_LMM_PLMN_ALLOW_REG != Nas_PlmnIdIsForbid(&stLmmPlmnId))
                {
                    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_ProcLmmCombinedTauResult: curr cmamp plmn is not allow reg");
                    ...
                    /* available定时器没在运行时，启available定时器周期搜网 */
...
)
{
    ...
    {
        NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_GetPrioAddtionalAction: Prior action enCsAdditionalAction", enCsAdditionalAction);
        return enCsAdditionalAction;
    }
    else
    {
        NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_GetPrioAddtionalAction: Prior action enPsAdditionalAction", enPsAdditionalAction);
...
VOS_UINT32 NAS_MMC_IsNeedDisableL_CombinedRegRsltFailMaxTimes()
{
    ...
    NAS_MML_LTE_UE_OPERATION_MODE_ENUM_UINT8                enLUeMode;
    VOS_UINT32                                              ulIsCurrDamPlmn;

    pstPlmnId       = NAS_MML_GetCurrCampPlmnId();
    enLUeMode       = NAS_MML_GetLteUeOperationMode();
    ulIsCurrDamPlmn = NAS_MML_IsPlmnSupportDam(pstPlmnId);
    
    /* LTE only */
    if (VOS_TRUE == NAS_MML_IsLteOnlyMode(NAS_MML_GetMsPrioRatList()))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_CombinedRegRsltFailMaxTimes:lte only mode,not need disable lte");
        return VOS_FALSE;
    }
    
    /* 如果是DAM网络，则在非CSPS1和CSPS2时才不需要DISABLE lte */
    if ( VOS_TRUE == ulIsCurrDamPlmn )
    {   
        if ( (NAS_MML_LTE_UE_OPERATION_MODE_CS_PS_1 == enLUeMode) 
          || (NAS_MML_LTE_UE_OPERATION_MODE_CS_PS_2 == enLUeMode) )
        {
            NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_CombinedRegRsltFailMaxTimes:dam plmn and enLUeMode trigger need disable lte",enLUeMode);
            return VOS_TRUE;
        }
        
        NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_CombinedRegRsltFailMaxTimes:dam plmn and enLUeMode trigger not need disable lte",enLUeMode);

        return VOS_FALSE;        
    }

    /* 非DAM网络，是CS/PS mode 1，则需要DISABLE LTE */
    if (NAS_MML_LTE_UE_OPERATION_MODE_CS_PS_1 == enLUeMode)
    {
        NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_CombinedRegRsltFailMaxTimes:not dam plmn and enLUeMode is csps1 trigger need disable lte",enLUeMode);
        return VOS_TRUE;
    }

    /* 非DAM网络，不是CS/PS mode 1，则不需要DISABLE LTE */    
    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_CombinedRegRsltFailMaxTimes:not dam and not csps1 trigger not need disable lte");
...
VOS_UINT32 NAS_MMC_IsNeedDisableL_EpsDetachCause14GprsNotAllowInThisPlmn(VOS_VOID)
{
    ...
    if (NAS_MML_GetLte3gppRelVersion() < NAS_MML_3GPP_REL_R11)
    {
        NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_EpsDetachCause14GprsNotAllowInThisPlmn: rel version < r11 not need disable lte",NAS_MML_GetLte3gppRelVersion());
        return VOS_FALSE;
    }
    ...
    if (NAS_MML_LTE_UE_OPERATION_MODE_CS_PS_1 != enLUeMode)
    {
        NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_EpsDetachCause14GprsNotAllowInThisPlmn: enLUeMode not csps mode1",enLUeMode);
        return VOS_FALSE;
    }
    ...
    if (VOS_TRUE == NAS_MML_IsLteOnlyMode(NAS_MML_GetMsPrioRatList()))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_EpsDetachCause14GprsNotAllowInThisPlmn: lte only");
        return VOS_FALSE;
    }
    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_EpsDetachCause14GprsNotAllowInThisPlmn: final need disable lte");
...
VOS_UINT32 NAS_MMC_IsNeedDisableL_CombinedRegRsltSucc()
{
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_CombinedRegRsltSucc: NAS_MMC_IsNeedDisableL_CombinedRegSuccAdditionUpdateRslt need disable lte");
        return VOS_TRUE;
    }

    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_CombinedRegRsltSucc: final not need disable lte");
...
)
{
    ...
    if (NAS_MML_LTE_UE_OPERATION_MODE_CS_PS_1 != enLUeMode)
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_CombinedRegSuccAdditionUpdateRslt, not cspsmode1,not need disable lte");
        return VOS_FALSE;
    }
    ...
    if (VOS_TRUE == NAS_MML_IsLteOnlyMode(NAS_MML_GetMsPrioRatList()))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_CombinedRegSuccAdditionUpdateRslt, lte only mode,not need disable lte");
        return VOS_FALSE;
    }

    if (VOS_TRUE == NAS_MML_GetImsVoiceAvailFlg())
    {    
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_CombinedRegSuccAdditionUpdateRslt, ims voice available,not need disable lte");
        return VOS_FALSE;
    }
    ...
    {
        NAS_NORMAL_LOG3(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_CombinedRegSuccAdditionUpdateRslt, ucAdditionUpdRsltSupportCsfbFlag,enLteCsServiceCfg,enVoiceDomain need disable lte",ucAdditionUpdRsltSupportCsfbFlag,enLteCsServiceCfg,enVoiceDomain);
        
        return VOS_TRUE;
    }

    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_CombinedRegSuccAdditionUpdateRslt: final return false");
...
VOS_UINT32 NAS_MMC_IsNeedDisableL_RegFailCause22AttmptCntMaxTimes(VOS_VOID)
{
    ...
    if (VOS_TRUE == NAS_MML_IsLteOnlyMode(NAS_MML_GetMsPrioRatList()))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_CombinedRegRsltOnlyEpsSucc: LTE only not need disable lte");
        return VOS_FALSE;
    }
    ...
    if ((VOS_TRUE                               == NAS_MML_GetImsVoiceAvailFlg())
     || (NAS_MML_LTE_UE_OPERATION_MODE_CS_PS_1  != enLUeMode))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_RegFailCause22AttmptCntMaxTimes: not csps1 or ims voice not avail,so not need disable lte");
        return VOS_FALSE;
    }
    ...
    if (VOS_FALSE == NAS_MML_IsImsiHplmnInDamImsiPlmnList())
    {
        ...
        {
            NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_RegFailCause22AttmptCntMaxTimes: not dam imsi and rel version < r11  need disable lte");
            return VOS_TRUE;
        }

        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_RegFailCause22AttmptCntMaxTimes: not dam imsi and rel version >= r11 not need disable lte");
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_RegFailCause22AttmptCntMaxTimes: plmn support dam need disable lte");
        return VOS_TRUE;
    }

    if (enLteRelVersion < NAS_MML_3GPP_REL_R11)
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_RegFailCause22AttmptCntMaxTimes: plmn not support dam and rel < r11,need disable lte");
        return VOS_TRUE;
    }

    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_RegFailCause22AttmptCntMaxTimes: plmn not support dam and rel >= r11,not need disable lte");
    return VOS_FALSE;
}
...
)
{
    ...
    if (NAS_MML_LTE_UE_OPERATION_MODE_CS_PS_1 != enLUeMode)
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_CombinedRegRsltOnlyEpsSucc: not csps1 not need disable lte");
        return VOS_FALSE;
    }
...
VOS_UINT32 NAS_MMC_IsNeedDisableL_ServiceReqFailCause18CsDomainNotAvailable(VOS_VOID)
{
    ...
    if (NAS_MML_LTE_UE_OPERATION_MODE_CS_PS_1 != enLUeMode)
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_ServiceReqFailCause18CsDomainNotAvailable: not csps1 not need disable lte");
        return VOS_FALSE;
    }
    ...
    if (VOS_TRUE == NAS_MML_IsLteOnlyMode(NAS_MML_GetMsPrioRatList()))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_ServiceReqFailCause18CsDomainNotAvailable: LTE only not need disable lte");
        return VOS_FALSE;
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_ServiceReqFailCause18CsDomainNotAvailable: csfb service not exist not need disable lte");
        return VOS_FALSE;
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_ServiceReqFailCause18CsDomainNotAvailable: test card not need disable lte");
        return VOS_TRUE;
    }

    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_ServiceReqFailCause18CsDomainNotAvailable: final not need disable lte");
...
)
{
    ...
    if ( (NAS_MML_LTE_UE_OPERATION_MODE_CS_PS_1 != enLUeMode) 
      && (NAS_MML_LTE_UE_OPERATION_MODE_CS_PS_2 != enLUeMode) )
    {
        NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_ServiceRsltFail: ue mode cause not need disable lte",enLUeMode);
        return VOS_FALSE;
    }
    ...
    if (VOS_TRUE == NAS_MML_IsLteOnlyMode(NAS_MML_GetMsPrioRatList()))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_ServiceRsltFail: LTE ONLY cause not need disable lte");
        return VOS_FALSE;
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_ServiceRsltFail: curr camp plmn not support dam cause not need disable lte");
        return VOS_FALSE;
    }

    if (VOS_FALSE == ucIsReachMaxTimes)
    {
        NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_ServiceRsltFail: ucIsReachMaxTimes cause not need disable lte",ucIsReachMaxTimes);
        return VOS_FALSE;
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_ServiceRsltFail: congestion cause need disable lte");
        return VOS_TRUE;
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_ServiceRsltFail: other cause need disable lte");
        return VOS_TRUE;
    }

    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_ServiceRsltFail: enRegRsltCause not need disable lte",enRegRsltCause);
...
VOS_UINT32 NAS_MMC_IsNeedDisableL_EpsOnlyRegSucc()
{
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_EpsOnlyRegSucc: CS NOT ok not need disable lte");
        return VOS_FALSE;
    }
    ...
    if (NAS_MML_LTE_UE_OPERATION_MODE_CS_PS_1 != enLUeMode)
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_EpsOnlyRegSucc: not csps1 not need disable lte");
        return VOS_FALSE;
    }
    ...
    if (VOS_TRUE == NAS_MML_IsLteOnlyMode(NAS_MML_GetMsPrioRatList()))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_EpsOnlyRegSucc: lte only mode not need disable lte");
        return VOS_FALSE;
    }
    ...
    if (NAS_MML_MAX_PS_REG_FAIL_CNT == NAS_MMC_GetRegRsltAttemptCounter(NAS_MMC_GetRegRsltType()))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_EpsOnlyRegSucc:max attempt counter need disable lte");
        return VOS_TRUE;
    }
    ...
    if (VOS_TRUE == NAS_MML_IsPlmnIdInCsDomainUnAvailPlmnList(NAS_MML_GetCurrCampPlmnId()))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_EpsOnlyRegSucc: like in #18 list need disable lte");
        return VOS_TRUE;
    }

    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_EpsOnlyRegSucc: final not need disable lte");
...
VOS_UINT32 NAS_MMC_IsCsPsMode1NeedPeriodSearchGU(VOS_VOID)
{
    ...
    if (VOS_TRUE == NAS_MML_IsLteOnlyMode(pstRatOrder))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsCsPsMode1NeedPeriodSearchGU: LTE ONLY mode,result is false ");
        return VOS_FALSE;
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsCsPsMode1NeedPeriodSearchGU: curr is not lte,result is false ");
        return VOS_FALSE;
    }
    ...
    if ((VOS_TRUE                               == NAS_MML_GetImsVoiceAvailFlg())
     || (NAS_MML_LTE_UE_OPERATION_MODE_CS_PS_1  != enLUeMode))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsCsPsMode1NeedPeriodSearchGU: not csps1 or ims ok,result is false ");
        return VOS_FALSE;
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsCsPsMode1NeedPeriodSearchGU: NAS_MMC_IsNeedDisableL_CombinedRegSuccAdditionUpdateRslt is true,result is true");
        return VOS_TRUE;
    }
...
VOS_UINT32 NAS_MMC_IsNeedDisableL_PsFailCause7PsServNotAllow(VOS_VOID)
{
    ...
    if ((VOS_FALSE == NAS_MML_GetCsAttachAllowFlg())
     || (VOS_FALSE == ucSimCsRegStatus))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_PsFailCause7PsServNotAllow: CS NOT ok not need disable lte");
        return VOS_FALSE;
    }
    ...
    if (NAS_MML_LTE_CAPABILITY_STATUS_DISABLE_NOTIFIED_AS == NAS_MML_GetLteCapabilityStatus())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_PsFailCause7PsServNotAllow: lte already disabled and notfy as not need disable lte");
        return VOS_FALSE;
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_PsFailCause7PsServNotAllow: platform not support lte not need disable lte");
    ...
    if (VOS_TRUE == NAS_MML_IsLteOnlyMode(NAS_MML_GetMsPrioRatList()))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_PsFailCause7PsServNotAllow: lte only not need disable lte");
        
        return VOS_FALSE;
    }

    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_PsFailCause7PsServNotAllow: final need disable lte");
...
VOS_UINT32 NAS_MMC_IsEutranNotAllowedNeedDisableL(VOS_VOID)
{
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsEutranNotAllowedNeedDisableL: lte cap already disabled not need disable lte");
    ...
    if (VOS_TRUE == NAS_MML_IsLteOnlyMode(NAS_MML_GetMsPrioRatList()))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsEutranNotAllowedNeedDisableL: lte only not need disable lte");
        
        return VOS_FALSE;
    }

    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsEutranNotAllowedNeedDisableL: need disable lte");
...
VOS_UINT32 NAS_MMC_IsNeedDisableL_LteRejCause14(VOS_VOID)
{
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_LteRejCause14: NAS_MML_GetLteRejCause14Flg false not need disable lte");
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_LteRejCause14: curr plmn is vplmn not need disable lte");
    ...
    if (VOS_TRUE == NAS_MML_IsLteOnlyMode(NAS_MML_GetMsPrioRatList()))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_LteRejCause14: HOME PLMN, LE ONLY not need disable lte");
        return VOS_FALSE;
    }

    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedDisableL_LteRejCause14: need disable lte");
...
VOS_UINT32 NAS_MMC_IsNeedEnableLte_CsRegFailCause2ImsiUnknownInHlr(VOS_VOID)
{
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_CsRegFailCause2ImsiUnknownInHlr: LTE FORBIDDEN STATUS not need enable lte");
        return VOS_FALSE;
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_CsRegFailCause2ImsiUnknownInHlr: LTE not disable not need enable lte");
        return VOS_FALSE;
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_CsRegFailCause2ImsiUnknownInHlr: usim card or ps reg false need enable lte");
        return VOS_TRUE;
    }

    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_CsRegFailCause2ImsiUnknownInHlr: not need enable lte");
...
VOS_UINT32 NAS_MMC_IsNeedEnableLte_CsRegFailCause256AuthRej(VOS_VOID)
{
    ...
    if (NAS_MML_RAT_CAPABILITY_STATUS_DISABLE == NAS_MML_GetLteForbiddenStatusFlg())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_CsRegFailCause256AuthRej: LTE FORBIDDEN STATUS not need enable lte");
        return VOS_FALSE;
    }
    ...
    if ((NAS_MML_LTE_CAPABILITY_STATUS_DISABLE_UNNOTIFY_AS != enLteCapabilityStatus)
     && (NAS_MML_LTE_CAPABILITY_STATUS_DISABLE_NOTIFIED_AS != enLteCapabilityStatus))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_CsRegFailCause256AuthRej: LTE not disable not need enable lte");
        return VOS_FALSE;
    }
    ...
    if ( (VOS_FALSE == NAS_MML_GetSimPsRegStatus())
      || (NAS_MML_SIM_TYPE_USIM == enSimType) )
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_CsRegFailCause256AuthRej: usim card or ps reg false need enable lte");
        return VOS_TRUE;
    }

    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_CsRegFailCause256AuthRej: not need enable lte");
...
VOS_UINT32 NAS_MMC_IsNeedEnableLte_PsRegFailCause256AuthRej(VOS_VOID)
{
    ...
    if (NAS_MML_RAT_CAPABILITY_STATUS_DISABLE == NAS_MML_GetLteForbiddenStatusFlg())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_PsRegFailCause256AuthRej: LTE FORBIDDEN not need enable lte");
        return VOS_FALSE;
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_PsRegFailCause256AuthRej: sim card not need enable lte");
        return VOS_FALSE;
    }
    ...
    if ((NAS_MML_LTE_CAPABILITY_STATUS_DISABLE_UNNOTIFY_AS == enLteCapabilityStatus)
     || (NAS_MML_LTE_CAPABILITY_STATUS_DISABLE_NOTIFIED_AS == enLteCapabilityStatus))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_PsRegFailCause256AuthRej: lte cap disabled need enable lte");
        return VOS_TRUE;
    }

    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_PsRegFailCause256AuthRej: not need enable lte");
...
VOS_UINT32 NAS_MMC_IsNeedEnableLte_PsRegFailCause3IllegalMs(VOS_VOID)
{
    ...
    if (NAS_MML_RAT_CAPABILITY_STATUS_DISABLE == NAS_MML_GetLteForbiddenStatusFlg())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_PsRegFailCause3IllegalMs: LTE forbidden not need enable lte");
        return VOS_FALSE;
    }
    ...
    if (NAS_MML_SIM_TYPE_SIM == NAS_MML_GetSimType())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_PsRegFailCause3IllegalMs: sim card not need enable lte");
        return VOS_FALSE;
    }
    ...
    if ((NAS_MML_LTE_CAPABILITY_STATUS_DISABLE_UNNOTIFY_AS == enLteCapabilityStatus)
     || (NAS_MML_LTE_CAPABILITY_STATUS_DISABLE_NOTIFIED_AS == enLteCapabilityStatus))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_PsRegFailCause3IllegalMs: lte is disabled need enable lte");
        return VOS_TRUE;
    }

    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_PsRegFailCause3IllegalMs: not need enable lte");
...
VOS_UINT32 NAS_MMC_IsNeedEnableLte_PsRegFailCause8GprsAndNonGprsNotAllow(VOS_VOID)
{
    ...
    if (NAS_MML_RAT_CAPABILITY_STATUS_DISABLE == NAS_MML_GetLteForbiddenStatusFlg())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_PsRegFailCause8GprsAndNonGprsNotAllow:forbidden lte not need enable lte");
        return VOS_FALSE;
    }
    ...
    if (NAS_MML_SIM_TYPE_SIM == NAS_MML_GetSimType())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_PsRegFailCause8GprsAndNonGprsNotAllow:sim card not need enable lte");
    ...
    if ((NAS_MML_LTE_CAPABILITY_STATUS_DISABLE_UNNOTIFY_AS == enLteCapabilityStatus)
     || (NAS_MML_LTE_CAPABILITY_STATUS_DISABLE_NOTIFIED_AS == enLteCapabilityStatus))
    {
        NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_PsRegFailCause8GprsAndNonGprsNotAllow:lte disabled need enable lte",enLteCapabilityStatus);
        return VOS_TRUE;
    }

    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_PsRegFailCause8GprsAndNonGprsNotAllow:not need enable lte");
...
)
{
    ...
    if (NAS_MML_RAT_CAPABILITY_STATUS_DISABLE == NAS_MML_GetLteForbiddenStatusFlg())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_CmServiceRejectInd: LTE FORBIDDEN STATUS not need enable lte");
        return VOS_FALSE;
    }
    ...
    if ((NAS_MML_LTE_CAPABILITY_STATUS_DISABLE_UNNOTIFY_AS != enLteCapabilityStatus)
     && (NAS_MML_LTE_CAPABILITY_STATUS_DISABLE_NOTIFIED_AS != enLteCapabilityStatus))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_CmServiceRejectInd: LTE not disabled not need enable lte");
        return VOS_FALSE;
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_CmServiceRejectInd: sim card not need enable lte");
        return VOS_FALSE;
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_CmServiceRejectInd: enCmServiceRejCause is #6 need enable lte");
        return VOS_TRUE;
    }

    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_CmServiceRejectInd: not need enable lte");
...
)
{
    ...
    if (NAS_MML_RAT_CAPABILITY_STATUS_DISABLE == NAS_MML_GetLteForbiddenStatusFlg())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_MmAbortInd: LTE FORBIDDEN STATUS not need enable lte");
        return VOS_FALSE;
    }
    ...
    if ((NAS_MML_LTE_CAPABILITY_STATUS_DISABLE_UNNOTIFY_AS != enLteCapabilityStatus)
     && (NAS_MML_LTE_CAPABILITY_STATUS_DISABLE_NOTIFIED_AS != enLteCapabilityStatus))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_MmAbortInd: LTE not disabled not need enable lte");
        return VOS_FALSE;
    }
    ...
    if (NAS_MML_SIM_TYPE_SIM == enSimType)
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_MmAbortInd: sim card not need enable lte");
        return VOS_FALSE;
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_MmAbortInd: enAbortCause #6 need enable lte");
        return VOS_TRUE;
    }

    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_MmAbortInd: not need enable lte");
...
VOS_UINT32  NAS_MMC_IsNeedEnableLte_EnableLteTimerExp(VOS_VOID)
{
    ...
    if (NAS_MML_RAT_CAPABILITY_STATUS_DISABLE == NAS_MML_GetLteForbiddenStatusFlg())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_EnableLteTimerExp: NAS_MML_GetLteForbiddenStatusFlg is disable s");
        return VOS_FALSE;
    }
     ...
     && (NAS_MML_LTE_CAPABILITY_STATUS_DISABLE_NOTIFIED_AS != enLteCapabilityStatus))
    {
        ...
        NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_EnableLteTimerExp: enLteCapabilityStatus not need enable",enLteCapabilityStatus);
        return VOS_FALSE;
    }
    ...
    {
        NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_EnableLteTimerExp: enSimType not need enable",enSimType);
    ...
    {
        NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_EnableLteTimerExp: enDisableLteReason need enable ltes",enDisableLteReason);

        return VOS_TRUE;
    }

    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MMC_IsNeedEnableLte_EnableLteTimerExp: enDisableLteReason not need enable ltes",enDisableLteReason);
