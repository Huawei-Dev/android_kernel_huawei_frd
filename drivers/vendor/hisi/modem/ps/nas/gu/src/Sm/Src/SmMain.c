...
VOS_VOID SM_T3380Expire(VOS_UINT8 ucCntxtIndex)
{
    ...
    NAS_MNTN_ACTIVE_FAIL_TYPE_ENUM_UINT32   enActFailType;
    SM_TAF_CAUSE_ENUM_UINT16                enSmTafCause;
    VOS_UINT8                               ucAddrIndex;
    VOS_UINT8                               ucOmCause;
    ...
    enSmTafCause    = SM_TAF_CAUSE_SM_MAX_TIME_OUT;
    ucOmCause       = NAS_OM_SM_REJ_CAUSE_TIMER_TIMEOUT;
    enActFailType   = NAS_MNTN_ACTIVE_FAIL_TYPE_ACTIVE_TIMEOUT;
    PS_MEM_SET(&stJudgmentPara, 0x00, sizeof(NAS_BACKOFF_JUDGMENT_PARA_STRU));
    PS_MEM_SET(&stMaintainPara, 0x00, sizeof(NAS_BACKOFF_MAINTAIN_PARA_STRU));

    ucAddrIndex = g_SmEntity.aPdpCntxtList[ucCntxtIndex].ucAddrIndex;

    /* 存在APN，调用BACK-OFF算法进行维护 */
    if ( 0 != g_SmPdpAddrApn.aPdpAddrlist[ucAddrIndex].stUnPackPdpApn.ulApnLength )
    {
        stMaintainPara.stApnInfo.ucApnLen = (VOS_UINT8)g_SmPdpAddrApn.aPdpAddrlist[ucAddrIndex].stUnPackPdpApn.ulApnLength;
        PS_MEM_CPY(stMaintainPara.stApnInfo.aucApnName,
                   g_SmPdpAddrApn.aPdpAddrlist[ucAddrIndex].stUnPackPdpApn.aucApnValue,
                   g_SmPdpAddrApn.aPdpAddrlist[ucAddrIndex].stUnPackPdpApn.ulApnLength);

        stMaintainPara.enPdpResult = NAS_BACKOFF_PDP_ACTIVE_RESULT_TIMER_EXP;
        stMaintainPara.ucCnCause   = NAS_BACKOFF_PDP_CAUSE_NULL;

        NAS_BACKOFF_OperateMaintain(NAS_BACKOFF_NETRAT_GU, &stMaintainPara);
    }

    g_SmEntity.aPdpCntxtList[ucCntxtIndex].TimerInfo.ucExpireTimes++;           /* 超时的次数加1                            */

    /* 如果未达到最大次数，且有APN，用BACK-OFF算法进行判断，如果不满足发起条件，
       直接设置超时次数为最大，错误码为BACK-OFF算法不允许发起 */
    if ( (SM_MAX_EXPIRE_TIMES > g_SmEntity.aPdpCntxtList[ucCntxtIndex].TimerInfo.ucExpireTimes)
      && (0 != g_SmPdpAddrApn.aPdpAddrlist[ucAddrIndex].stUnPackPdpApn.ulApnLength) )
    {
        stJudgmentPara.ucApnLen = (VOS_UINT8)g_SmPdpAddrApn.aPdpAddrlist[ucAddrIndex].stUnPackPdpApn.ulApnLength;
        PS_MEM_CPY(stJudgmentPara.aucApnName,
                   g_SmPdpAddrApn.aPdpAddrlist[ucAddrIndex].stUnPackPdpApn.aucApnValue,
                   g_SmPdpAddrApn.aPdpAddrlist[ucAddrIndex].stUnPackPdpApn.ulApnLength);

        if( NAS_BACKOFF_SUCC != NAS_BACKOFF_OperateJudgment(NAS_BACKOFF_NETRAT_GU, &stJudgmentPara) )
        {
            g_SmEntity.aPdpCntxtList[ucCntxtIndex].TimerInfo.ucExpireTimes = SM_MAX_EXPIRE_TIMES;
            enSmTafCause  = SM_TAF_CAUSE_SM_BACKOFF_ALG_NOT_ALLOWED;
            ucOmCause     = NAS_OM_SM_REJ_CAUSE_BACKOFF_ALG_NOT_ALLOWED;
            enActFailType = NAS_MNTN_ACTIVE_FAIL_TYPE_BACKOFF_ALG_NOT_ALLOWED;
        }
    }
    ...
    {                                                                           /* 达到最大的次数(SM_MAX_EXPIRE_TIMES)      */
        NAS_MNTN_RecordPdpActiveFail(enActFailType,
                                     ucCntxtIndex,
...
VOS_VOID  Sm_PowerOff(VOS_VOID)
{
    ...
    for (ucCntxtIndex = 0; ucCntxtIndex < SM_MAX_NSAPI_NUM; ucCntxtIndex++)
    {
        ...
        /* 释放UNPACK APN消息 */
        pstPdpAddrInfo->stUnPackPdpApn.ulApnLength = 0;
        PS_MEM_SET(pstPdpAddrInfo->stUnPackPdpApn.aucApnValue,
                   0x00,
                   sizeof(pstPdpAddrInfo->stUnPackPdpApn.aucApnValue));
