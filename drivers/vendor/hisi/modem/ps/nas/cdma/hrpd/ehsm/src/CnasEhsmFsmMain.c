...
)
{
    ...
    CNAS_EHSM_MNTN_LogPcReplayFixedContext();
#endif
...
VOS_UINT32 CNAS_EHSM_RcvHsmSuspendInd_Inactive(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    ...
    if (VOS_TRUE == CNAS_EHSM_GetHandOverFailFlag())
    {
        CNAS_WARNING_LOG1(UEPS_PID_EHSM, "CNAS_EHSM_RcvHsmSuspendInd_Inactive: L2C fail", ulLtePdnNum);

        CNAS_EHSM_SetHandOverFailFlag(VOS_FALSE);

        if (0 != ulLtePdnNum)
        {
            /* L2C过程中，如果恢复PDN有过失败，EHSM直接通知APS去激活 */
            ...
            for (ulLoop = 0; ulLoop < CNAS_EHSM_MAX_PDN_NUM; ulLoop++)
            {
                if (VOS_TRUE == pstLtePdnBearInfo[ulLoop].ucInUsed)
                {
                    CNAS_EHSM_SndApsLteHandOverToEhrpdInd(CNAS_EHSM_INVALID_PDN_ID,
                                                          pstLtePdnBearInfo[ulLoop].ulEpsbId,
    ...
    if (CNAS_EHSM_INVALID_CID != ucCid)
    {
        switch (pstSynPdnInfo->enPdnOption)
        {
            ...
            case EHSM_ESM_PDN_OPT_MODIFIED:
                ...
                {
                    CNAS_EHSM_RecodeErrorLogInfo(pstSynPdnInfo->ulAttachFlag,
                                                 pstSynPdnInfo->enPdnOption,
                                                 pstSynPdnInfo->ulBitCid,
                                                 pstSynPdnInfo->ulEpsbId,
                                                 CNAS_EHSM_ERROR_CODE_INACTIVE);
                }
                break;
            ...
            case EHSM_ESM_PDN_OPT_DISCONNECTED:
                ...
                for (ucLteIndex = 0; ucLteIndex < CNAS_EHSM_MAX_PDN_NUM; ucLteIndex++)
                {
                    if ((pstEhsmCtx->astLtePdnBearInfo[ucLteIndex].ucInUsed == VOS_TRUE)
                      &&(pstEhsmCtx->astLtePdnBearInfo[ucLteIndex].ucCid == ucCid))
                    {
                        break;
                    }
                }
...
VOS_UINT32 CNAS_EHSM_RcvApsPdnActivateReq_Inactive(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    ...
    if (EHSM_HSM_SESSION_TYPE_EHRPD != CNAS_EHSM_GetSessionType())
    {
        CNAS_EHSM_SndApsPdnActivateCnf(pstPdnActReq->ucCid, EHSM_APS_CAUSE_NOT_SUPPORT_EHRPD);

        return VOS_TRUE;
    }

    /* 如果当前Session Type还维持之前的EHRPD,主要见于模式切换流程中，HRPD被挂起 */
...
VOS_UINT32 CNAS_EHSM_RcvCttfEhrpdDetachInd_Inactive(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    /* 场景: L2C过程中收到PPPC DETACH_IND消息 */

    VOS_UINT32                          ulNum;

    ulNum = CNAS_EHSM_GetLteActivePdnNum();

    /* if ehrpd is master, notify ESM clear all pdn */
    if ((CNAS_EHSM_HRPD_WORK_MODE_MASTER == CNAS_EHSM_GetHrpdWorkMode())
     && (0 != ulNum))
    {
        CNAS_EHSM_SndEsmClearAllNtf();
...
VOS_UINT32 CNAS_EHSM_RcvEsmClearAllBearerNotify_Inactive(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    ...
    /* 由LTE通知用户，不需要EHSM通知APS */
    return VOS_FALSE;
}



VOS_UINT32 CNAS_EHSM_RcvCttfEhrpdReconnInd_Inactive(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    CNAS_WARNING_LOG(UEPS_PID_EHSM, "CNAS_EHSM_RcvApsReconnReq_Inactive: ERR");
...
VOS_UINT32 CNAS_EHSM_RcvCttfEhrpdDetachInd_Suspend(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
...
VOS_UINT32 CNAS_EHSM_RcvEsmClearAllBearer_Suspend(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
...
VOS_UINT32 CNAS_EHSM_RcvCttfEhrpdReconnInd_Suspend(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
...
)
{
    ...
    /* 如果当前已经没有active的pdn，状态迁到inactive */
...
VOS_UINT32 CNAS_EHSM_RcvCttfEhrpdPdnDiscInd_Suspend(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
...
)
{
    ...
    {
        enPsRatType = CNAS_CCB_GetCurrPsRatType();

        if ((0 == CNAS_EHSM_GetLteActivePdnNum())
         && (CNAS_CCB_PS_RATTYPE_LTE ==  enPsRatType))
        {
            CNAS_EHSM_RecodeErrorLogInfo(0,
                                         0,
                                         0,
                                         0,
                                         CNAS_EHSM_ERROR_CODE_NO_SYNC);
...
)
{
    ...
    if (ucLteIndex >= CNAS_EHSM_MAX_PDN_NUM)
    {
        CNAS_ERROR_LOG1(UEPS_PID_EHSM, "CNAS_EHSM_RcvEsmSyncPdnInfoInd_Suspend, Lte Pdn Entity Overflow", ucLteIndex);
        return VOS_TRUE;
    }
    ...
    {
        CNAS_ERROR_LOG1(UEPS_PID_EHSM, "CNAS_EHSM_RcvEsmSyncPdnInfoInd_Suspend, Attachflag is not True", pstPdnInfo->ulAttachFlag);

        CNAS_EHSM_RecodeErrorLogInfo(pstPdnInfo->ulAttachFlag,
                                     pstPdnInfo->enPdnOption,
                                     pstPdnInfo->ulBitCid,
                                     pstPdnInfo->ulEpsbId,
                                     CNAS_EHSM_ERROR_CODE_SUSPEND);
        return VOS_TRUE;
    }
    else
    {
        /* Clear all Lte PDN info */
        CNAS_EHSM_ClearAllLteLocalPdnInfo();
...
)
{
    ...
    if (CNAS_EHSM_HRPD_WORK_MODE_MASTER != CNAS_EHSM_GetHrpdWorkMode())
    {
        CNAS_EHSM_SndApsReconnCnf(EHSM_APS_CAUSE_EHRPD_SUSPEND,pstApsReconnReq->ucPdnId, pstApsReconnReq->ucCid);
        CNAS_WARNING_LOG(UEPS_PID_EHSM, "CNAS_EHSM_RcvApsReConnReq_Active: EHRPD is Slave");
        return VOS_TRUE;
    }
    ...
    if (VOS_TRUE == CNAS_EHSM_GetAirLinkExistFlag())
    {
        ...
        {
            CNAS_WARNING_LOG(UEPS_PID_EHSM, "CNAS_EHSM_RcvApsReConnReq_Active, Ehrpd State not match with Air Link Status!");
...
)
{
    ...
    /* L2C过程中，如果还存在未恢复的PDN，EHSM直接通知APS去激活 */
    pstLtePdnBearInfo = CNAS_EHSM_GetEhsmCtxAddr()->astLtePdnBearInfo;
...
VOS_UINT32 CNAS_EHSM_RcvCttfEhrpdReconnInd_Active(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    ...
    {
        CNAS_WARNING_LOG(UEPS_PID_EHSM, "CNAS_EHSM_RcvCttfEhrpdReconnInd_Active: Non Dormant State");
    }

    if (CNAS_EHSM_HRPD_WORK_MODE_MASTER != CNAS_EHSM_GetHrpdWorkMode())
    {
        CNAS_WARNING_LOG(UEPS_PID_EHSM, "CNAS_EHSM_RcvCttfEhrpdReconnInd_Active: EHRPD is Slave");
        return VOS_TRUE;
    }
    /* Init activating Sub FSM */
    CNAS_EHSM_InitFsmL2(CNAS_EHSM_FSM_ACTIVATING);
...
VOS_UINT32 CNAS_EHSM_RcvRrmStatusInd_Inactive(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
