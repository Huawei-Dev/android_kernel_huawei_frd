...
{
    ID_MSCC_HSD_POWER_OFF_REQ,                  /* 0x3002 */
    ID_MSCC_HSD_POWER_SAVE_REQ,                 /* 0x301c */
    ID_MSCC_HSD_DATA_CALL_REDIAL_SYS_ACQ_REQ,   /* 0x3009 */
    ID_CAS_CNAS_HRPD_SUSPEND_IND,               /* 0x5116 */
    ID_CAS_CNAS_HRPD_NETWORK_LOST_IND           /* 0x5108 */
};

/* CNAS_HSD_AVOID_SESSION_NEGOTIATION_TIMEOUT随counter值变化的avoid time */
VOS_UINT8                               g_aucAvoidSETTimeLen[CNAS_HSD_AVOID_MAX_PHASE_NUM] =
{
    0, 1, 2, 4, 8, 8, 8, 16
};

/* CNAS_HSD_AVOID_MAX_ACCESS_PROBES_FAILURE随counter值变化的avoid time */
VOS_UINT8                               g_aucAvoidMAPFTimeLen[CNAS_HSD_AVOID_MAX_PHASE_NUM] =
{
    0, 1, 1, 2, 2, 2, 4, 4
};

/* CNAS_HSD_AVOID_BUSY_OR_GENERAL_FAILURE随counter值变化的avoid time */
VOS_UINT8                               g_aucAvoidBOGFTimeLen[CNAS_HSD_AVOID_MAX_PHASE_NUM] =
{
    0, 0, 0, 1, 1, 1, 1, 1
};

/* CNAS_HSD_AVOID_TCA_TIMEOUT随counter值变化的avoid time */
VOS_UINT8                               g_aucAvoidTcaTimeLen[CNAS_HSD_AVOID_MAX_PHASE_NUM] =
{
    0, 0, 0, 1, 2, 4, 8, 8
};
...
)
{
    ...
    NAS_MEM_SET_S(&(pstSysAcqCtrl->st1xSysInfo.st1xSys), sizeof(CNAS_PRL_1X_SYSTEM_STRU), 0, sizeof(CNAS_PRL_1X_SYSTEM_STRU));

    NAS_MEM_SET_S(&(pstSysAcqCtrl->stMruList), sizeof(CNAS_HSD_HRPD_MRU_LIST_STRU), 0, sizeof(CNAS_HSD_HRPD_MRU_LIST_STRU));

    NAS_MEM_SET_S(&(pstSysAcqCtrl->stAvoidFreqList), sizeof(CNAS_HSD_AVOID_FREQ_LIST_STRU), 0, sizeof(CNAS_HSD_AVOID_FREQ_LIST_STRU));

    NAS_MEM_SET_S(&(pstSysAcqCtrl->stHistorySysList), sizeof(CNAS_HSD_HISTORY_SYS_LIST_STRU), 0, sizeof(CNAS_HSD_HISTORY_SYS_LIST_STRU));

    NAS_MEM_SET_S(&(pstSysAcqCtrl->stSpmSysList), sizeof(CNAS_HSD_SPM_SYS_LIST_STRU), 0, sizeof(CNAS_HSD_SPM_SYS_LIST_STRU));
    ...
    NAS_MEM_SET_S(&(pstSysAcqCtrl->stHighPriority), sizeof(CNAS_HSD_HIGHPRIORITY_STRU), 0, sizeof(CNAS_HSD_HIGHPRIORITY_STRU));
    NAS_MEM_SET_S(&(pstSysAcqCtrl->stCasOhmHrpdSys), sizeof(CNAS_PRL_HRPD_SYSTEM_STRU), 0, sizeof(CNAS_PRL_HRPD_SYSTEM_STRU));
    NAS_MEM_SET_S(&(pstSysAcqCtrl->stNetwkLostSysRec), sizeof(CNAS_HSD_NETWORK_LOST_SYS_RECORD_STRU), 0, sizeof(CNAS_HSD_NETWORK_LOST_SYS_RECORD_STRU));

    /* 锁频参数 */
    CNAS_HSD_InitCFreqLockValue();

    NAS_MEM_SET_S(&(pstSysAcqCtrl->stSyncFreq), sizeof(CNAS_CAS_HRPD_FREQENCY_CHANNEL_STRU), 0, sizeof(CNAS_CAS_HRPD_FREQENCY_CHANNEL_STRU));
...
)
{
    ...
    pstSysAssistInfo->enSessionRlt       = HSM_HSD_SESSION_NEG_RSLT_ENUM_BUTT;
...
)
{
    switch (enFsmId)
    {
        ...
        case CNAS_HSD_FSM_BG_SRCH:
            pstCurFsm->ulState                  = CNAS_HSD_BG_SRCH_STA_INIT;
            pstCurFsm->pstFsmDesc               = CNAS_HSD_GetBgSrchFsmDescAddr();
            break;
...
)
{
    ...
    NAS_MEM_CPY_S(pstCurFsm, sizeof(CNAS_HSD_FSM_CTX_STRU), pstNewFsm, sizeof(CNAS_HSD_FSM_CTX_STRU));
...
)
{
    ...
    NAS_MEM_CPY_S((pstMsgQueue->pastCacheMsg[ulOffset]),
                  ulMsgLen,
                  pstMsg,
                  ulMsgLen);
    ...
    if (0 < pstMsgQueue->ucCacheMsgNum)
    {
        ...
        if (ucIndex < (CNAS_HSD_MAX_CACHE_MSG_QUEUE_NUM - 1))
        {
            ...
            NAS_MEM_MOVE_S(&(pstMsgQueue->pastCacheMsg[ucIndex]),
                           (CNAS_HSD_MAX_CACHE_MSG_QUEUE_NUM - ucIndex) * sizeof(VOS_UINT32),
...
)
{
    ...
    NAS_MEM_CPY_S(&pstEntryMsg->aucMsgBuffer[0], CNAS_HSD_MAX_MSG_BUFFER_LEN, pstMsg, ulLen );
...
VOS_UINT8 *CNAS_HSD_GetNextIntMsg(VOS_VOID)
{
    ...
    if (0 < ucIntMsgNum)
    {
        ...
        {
            /* 前移内部消息 */
            NAS_MEM_MOVE_S(&(CNAS_HSD_GetIntMsgQueueAddr()->pastIntMsg[0]),
                           CNAS_HSD_MAX_INT_MSG_QUEUE_NUM * sizeof(VOS_UINT8 *),
...
VOS_VOID CNAS_HSD_UpdateMru0(CNAS_PRL_HRPD_SYSTEM_STRU *pstCurSysId)
{
    ...
    NAS_MEM_SET_S(&stMruItem, sizeof(CNAS_PRL_HRPD_SYSTEM_STRU), 0, sizeof(CNAS_PRL_HRPD_SYSTEM_STRU));
    stMruItem.stFreq = pstCurSysId->stFreq;

    /* MRU中只存SubnetID中的高6字节 */
    NAS_MEM_CPY_S(stMruItem.aucSubnet,
                  CNAS_PRL_BYTES_IN_SUBNET,
                  pstCurSysId->aucSubnet + (CNAS_PRL_BYTES_IN_SUBNET - CNAS_HSD_CHECK_BYTES_SUBNET_ID),
    ...
    for (i = 0; i < pstMruList->ucCurrMruNum; i++)
    {
        ...
        if (VOS_TRUE == CNAS_PRL_CmpFreqValue(&(pstMruList->astHrpdSys[i].stFreq), &(stMruItem.stFreq)))
        {
                                                    ...
                                                    stMruItem.aucSubnet, CNAS_HSD_CHECK_BYTES_SUBNET_ID))
            {
                ...
                if (0 != i)
                {
                    NAS_MEM_MOVE_S(&pstMruList->astHrpdSys[1],
                                   (CNAS_HSD_MAX_MRU_SYS_NUM - 1) *(sizeof(CNAS_PRL_HRPD_SYSTEM_STRU)),
                                   pstMruList->astHrpdSys,
                                   (i)*(sizeof(CNAS_PRL_HRPD_SYSTEM_STRU)));
                }
    ...
    /* 赋值MRU[0] */
    NAS_MEM_CPY_S(pstMruList->astHrpdSys, sizeof(CNAS_PRL_HRPD_SYSTEM_STRU), &stMruItem, sizeof(CNAS_PRL_HRPD_SYSTEM_STRU));
...
)
{
    ...
    if (0 < CNAS_HSD_GetMruListNum())
    {
        ...
        NAS_MEM_CPY_S(pstMru0SysId->aucSubnet, CNAS_PRL_BYTES_IN_SUBNET, pstMruList->astHrpdSys[0].aucSubnet, CNAS_PRL_BYTES_IN_SUBNET);
...
{
    NAS_MEM_SET_S(pstOocScheduleInfo, sizeof(CNAS_HSD_OOC_SCHEDULE_INFO_STRU), 0, sizeof(CNAS_HSD_OOC_SCHEDULE_INFO_STRU));
...
VOS_VOID CNAS_HSD_InitOocCtxInfo(VOS_VOID)
{
    ...
    NAS_MEM_SET_S(&pstOocScheduleInfo->stOocCtxInfo, sizeof(CNAS_HSD_OOC_CTX_INFO_STRU), 0, sizeof(CNAS_HSD_OOC_CTX_INFO_STRU));
...
VOS_VOID CNAS_HSD_UpdateHistorySysList(CNAS_PRL_HRPD_SYSTEM_STRU *pstCurrSystem)
{
    ...
    {
        NAS_MEM_MOVE_S(&(pstHistorySysList->stCampedHrpdSysInfo[1]),
    ...
    NAS_MEM_CPY_S(&(pstHistorySysList->stCampedHrpdSysInfo[0]),
...
CNAS_HSD_SPM_SYS_LIST_STRU* CNAS_HSD_GetSpmSysList(VOS_VOID)
{
    return &(CNAS_HSD_GetHsdCtxAddr()->stSysAcqCtrl.stSpmSysList);
}


VOS_VOID CNAS_HSD_UpdateSpmSysList(CAS_CNAS_HRPD_OHM_SPM_CHANNEL_STRU *pstSpmChannel)
{
    ...
    pstSpmSysList->ucSpmNum = CNAS_MIN(pstSpmChannel->ucChannelNum, CNAS_HSD_SPM_LIST_MAX_NUM);

    for (i = 0; i < pstSpmSysList->ucSpmNum; i++)
    {
        pstSpmSysList->astSpmFreq[i].enBandClass    = pstSpmChannel->astChannelRec[i].usBandClass;
        pstSpmSysList->astSpmFreq[i].usChannel      = pstSpmChannel->astChannelRec[i].usChannelNumber;
    }
...
VOS_VOID CNAS_HSD_UpdateHrpdSysInfo(CNAS_PRL_HRPD_SYSTEM_STRU *pstSystem)
{
    ...
    NAS_MEM_CPY_S(pstHrpdSystem, sizeof(CNAS_PRL_HRPD_SYSTEM_STRU), pstSystem, sizeof(CNAS_PRL_HRPD_SYSTEM_STRU));
...
VOS_VOID CNAS_HSD_ClearHrpdSysInfo(VOS_VOID)
{
    ...
    NAS_MEM_SET_S(pstHrpdSystem, sizeof(CNAS_PRL_HRPD_SYSTEM_STRU), 0, sizeof(CNAS_PRL_HRPD_SYSTEM_STRU));
...
{
    NAS_MEM_SET_S(pstHighFreq, sizeof(CNAS_HSD_HIGHPRIORITY_STRU), 0, sizeof(CNAS_HSD_HIGHPRIORITY_STRU));
...
HSM_HSD_SESSION_NEG_RSLT_ENUM_UINT8 CNAS_HSD_GetSessionRlt(VOS_VOID)
{
    return CNAS_HSD_GetHsdCtxAddr()->stSysAssistInfo.enSessionRlt;
}


VOS_VOID CNAS_HSD_UpdateSessionRlt(HSM_HSD_SESSION_NEG_RSLT_ENUM_UINT8  enSessionRlt)
{
    CNAS_HSD_GetHsdCtxAddr()->stSysAssistInfo.enSessionRlt = enSessionRlt;
...
VOS_VOID CNAS_HSD_InitCFreqLockValue(VOS_VOID)
{
    ...
    NAS_MEM_SET_S(pstCnasHsdCFreqLockValue, sizeof(CNAS_HSD_CFREQ_LOCK_SET_PARA_STRU), 0x00, sizeof(CNAS_HSD_CFREQ_LOCK_SET_PARA_STRU));
...
CNAS_HSD_AVOID_SCHEDULE_INFO_STRU* CNAS_HSD_GetAvoidScheduleInfoAddr(VOS_VOID)
{
    return &(CNAS_HSD_GetHsdCtxAddr()->stSysAcqCtrl.stAvoidScheduleInfo);
}


VOS_VOID CNAS_HSD_InitAvoidScheduleInfo(VOS_VOID)
{
    ...
    /* counter始终为0 */
    pstAvoidScheduleInfo->ausAvoidTimerLen[CNAS_HSD_AVOID_PROTOCAL_MISMATCH][0]             = CNAS_HSD_AVOID_TIME_60_SEC;
    pstAvoidScheduleInfo->ausAvoidTimerLen[CNAS_HSD_AVOID_REDIRECTION][0]                   = CNAS_HSD_AVOID_TIME_30_SEC;
    pstAvoidScheduleInfo->ausAvoidTimerLen[CNAS_HSD_AVOID_SESSION_NEGOTIATION_FAILURE][0]   = CNAS_HSD_AVOID_TIME_60_SEC * 10;
    pstAvoidScheduleInfo->ausAvoidTimerLen[CNAS_HSD_AVOID_PERSISTENCE_TEST_FAILURE][0]      = CNAS_HSD_AVOID_TIME_60_SEC * 7;
    pstAvoidScheduleInfo->ausAvoidTimerLen[CNAS_HSD_AVOID_AUTHEN_OR_BILLING_FAILURE][0]     = CNAS_HSD_AVOID_TIME_60_SEC * 10;
    pstAvoidScheduleInfo->ausAvoidTimerLen[CNAS_HSD_AVOID_FAIL_CONTINUE_THREE_TIMES][0]     = CNAS_HSD_AVOID_TIME_60_SEC;
    pstAvoidScheduleInfo->ausAvoidTimerLen[CNAS_HSD_AVOID_PREFER_CH_NOT_VALID][0]           = CNAS_HSD_AVOID_TIME_60_SEC;
    pstAvoidScheduleInfo->ausAvoidTimerLen[CNAS_HSD_AVOID_CONN_OPEN_FAIL][0]                = CNAS_HSD_AVOID_TIME_60_SEC;

    /* counter会变 */
    for (i = 0; i < CNAS_HSD_AVOID_MAX_PHASE_NUM; i++)
    {
        pstAvoidScheduleInfo->ausAvoidTimerLen[CNAS_HSD_AVOID_SESSION_NEGOTIATION_TIMEOUT][i]   = CNAS_HSD_AVOID_TIME_60_SEC * g_aucAvoidSETTimeLen[i];
        pstAvoidScheduleInfo->ausAvoidTimerLen[CNAS_HSD_AVOID_MAX_ACCESS_PROBES_FAILURE][i]     = CNAS_HSD_AVOID_TIME_60_SEC * g_aucAvoidMAPFTimeLen[i];
        pstAvoidScheduleInfo->ausAvoidTimerLen[CNAS_HSD_AVOID_BUSY_OR_GENERAL_FAILURE][i]       = CNAS_HSD_AVOID_TIME_60_SEC * g_aucAvoidBOGFTimeLen[i];
        pstAvoidScheduleInfo->ausAvoidTimerLen[CNAS_HSD_AVOID_TCA_TIMEOUT][i]                   = CNAS_HSD_AVOID_TIME_60_SEC * g_aucAvoidTcaTimeLen[i];
    }
...
VOS_VOID CNAS_HSD_InitMatchedHrpdSysIndexList(VOS_VOID)
{
    ...
    NAS_MEM_SET_S(pstMatchedHrpdSysIndex->ausMatchedHrpdSysIndexList,
                  CNAS_PRL_MAX_MATCHED_HRPD_SYSTEM_NUM * (sizeof(VOS_UINT16)),
                  0,
                  CNAS_PRL_MAX_MATCHED_HRPD_SYSTEM_NUM * (sizeof(VOS_UINT16)));
...
VOS_VOID CNAS_HSD_InitFsmCtx_BgSrch(VOS_VOID)
{
    CNAS_HSD_CTX_STRU                  *pstHsdCtx = VOS_NULL_PTR;
    ...
    /* 检查当前状态机是否时CNAS_HSD_FSM_INTER_SYS状态机 */
    if (CNAS_HSD_FSM_BG_SRCH != CNAS_HSD_GetCurrFsmId())
    {
        /* 异常事件 */
        CNAS_WARNING_LOG(UEPS_PID_HSD, "CNAS_HSD_InitFsmCtx_BgSrch:ENTERED");

        return;
    }

    pstHsdCtx->stCurFsm.unFsmCtx.stBgSrchCtx.enCurAcqScene      = CNAS_HSD_SYS_ACQ_SCENE_BUTT;
    pstHsdCtx->stCurFsm.unFsmCtx.stBgSrchCtx.enNeedSrchFlag     = CNAS_HSD_NOT_NEED_REACQ;
    pstHsdCtx->stCurFsm.unFsmCtx.stBgSrchCtx.ucStopBgSrchInd    = VOS_FALSE;

    return;
}


VOS_VOID CNAS_HSD_SetSrchFlag_BgSrch(
    CNAS_HSD_SYS_ACQ_SCENE_ENUM_UINT32                      enCurAcqScene,
    CNAS_HSD_REACQ_FALG_ENUM_UINT32                         enNeedSrchFlag
)
{
    /* 如果当前状态机不是CNAS_HSD_FSM_BG_SRCH */
    if (CNAS_HSD_FSM_BG_SRCH != CNAS_HSD_GetCurrFsmId())
    {
        /* 异常事件 */
        CNAS_WARNING_LOG(UEPS_PID_HSD, "CNAS_HSD_SetSrchFlag_BgSrch: Current state is incorrect.");

        return;
    }

    CNAS_HSD_GetCurFsmAddr()->unFsmCtx.stBgSrchCtx.enCurAcqScene  = enCurAcqScene;
    CNAS_HSD_GetCurFsmAddr()->unFsmCtx.stBgSrchCtx.enNeedSrchFlag = enNeedSrchFlag;

    return;
}


CNAS_HSD_REACQ_FALG_ENUM_UINT32 CNAS_HSD_GetSrchFlag_BgSrch(VOS_VOID)
{
    /* 如果当前状态机不是CNAS_HSD_FSM_BG_SRCH */
    if (CNAS_HSD_FSM_BG_SRCH != CNAS_HSD_GetCurrFsmId())
    {
        /* 异常事件 */
        CNAS_WARNING_LOG(UEPS_PID_HSD, "CNAS_HSD_GetSrchFlag_BgSrch: Current state is incorrect.");
    }

    return CNAS_HSD_GetCurFsmAddr()->unFsmCtx.stBgSrchCtx.enNeedSrchFlag;
}


CNAS_HSD_SYS_ACQ_SCENE_ENUM_UINT32 CNAS_HSD_GetSrchScene_BgSrch(VOS_VOID)
{
    /* 如果当前状态机不是CNAS_HSD_FSM_BG_SRCH */
    if (CNAS_HSD_FSM_BG_SRCH != CNAS_HSD_GetCurrFsmId())
    {
        /* 异常事件 */
        CNAS_WARNING_LOG(UEPS_PID_HSD, "CNAS_HSD_GetSrchScene_BgSrch: Current state is incorrect.");
    }

    return CNAS_HSD_GetCurFsmAddr()->unFsmCtx.stBgSrchCtx.enCurAcqScene;
}


VOS_VOID CNAS_HSD_SetStopBgSrchInd_BgSrch(
...
)
{
    ...
    {
        /* 异常事件 */
        CNAS_WARNING_LOG(UEPS_PID_HSD, "CNAS_HSD_SetStopBgSrchInd_BgSrch: Current state is incorrect.");

        return;
    }

    CNAS_HSD_GetCurFsmAddr()->unFsmCtx.stBgSrchCtx.ucStopBgSrchInd  = ucStopBgSrchInd;

    return;
}


VOS_UINT8 CNAS_HSD_GetStopBgSrchInd_BgSrch(VOS_VOID)
{
    /* 如果当前状态机不是CNAS_HSD_FSM_BG_SRCH */
    if (CNAS_HSD_FSM_BG_SRCH != CNAS_HSD_GetCurrFsmId())
    {
        /* 异常事件 */
        CNAS_WARNING_LOG(UEPS_PID_HSD, "CNAS_HSD_GetStopBgSrchInd_BgSrch: Current state is incorrect.");
    }

    return CNAS_HSD_GetCurFsmAddr()->unFsmCtx.stBgSrchCtx.ucStopBgSrchInd;
}



VOS_VOID CNAS_HSD_SetSysAcqNoRfProtectTimerExpiredFlag(
    VOS_UINT32                          ulIsSysAcqNoRfTimerExpired
)
{
    CNAS_HSD_GetHsdCtxAddr()->stSysAcqCtrl.stSysAcqSrlteInfo.ulIsSysAcqNoRfTimerExpired = ulIsSysAcqNoRfTimerExpired;
    return;
}


VOS_UINT32 CNAS_HSD_GetSysAcqNoRfProtectTimerExpiredFlag(VOS_VOID)
{
    return CNAS_HSD_GetHsdCtxAddr()->stSysAcqCtrl.stSysAcqSrlteInfo.ulIsSysAcqNoRfTimerExpired;
}


VOS_UINT32 CNAS_HSD_GetSysAcqNoRfProtectTimerLen(VOS_VOID)
{
    return CNAS_HSD_GetHsdCtxAddr()->stSysAcqCtrl.stSysAcqSrlteInfo.ulHrpdSysAcqNoRfProtectTimerLen;
}

VOS_VOID CNAS_HSD_SetSysAcqNoRfProtectTimerLen(
    VOS_UINT32                          ulHrpdSysAcqNoRfProtectTimerLen
)
{
    CNAS_HSD_GetHsdCtxAddr()->stSysAcqCtrl.stSysAcqSrlteInfo.ulHrpdSysAcqNoRfProtectTimerLen = ulHrpdSysAcqNoRfProtectTimerLen*60*1000;
    return;
}


VOS_UINT32 CNAS_HSD_GetWaitCasSyncCnfTimerLen(VOS_VOID)
{
    return CNAS_HSD_GetHsdCtxAddr()->stSysAcqCtrl.stSysAcqSrlteInfo.ulWaitCasSystemSyncCnfTimerLen;
}

VOS_VOID CNAS_HSD_SetWaitCasSyncCnfTimerLen(
    VOS_UINT32                          ulWaitCasSystemSyncCnfTimerLen
)
{
    CNAS_HSD_GetHsdCtxAddr()->stSysAcqCtrl.stSysAcqSrlteInfo.ulWaitCasSystemSyncCnfTimerLen = ulWaitCasSystemSyncCnfTimerLen;
    return;
}
