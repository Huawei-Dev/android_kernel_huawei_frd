...
)
{
    ...
    if (CNAS_EHSM_AUTH_TYPE_CHAP == pstLocalPdnBearInfo->enAuthType)
    {
        ...
        if (0 == pstLocalPdnBearInfo->ucPwdLen)
        {
            CNAS_WARNING_LOG(UEPS_PID_EHSM, "CNAS_EHSM_EncodePco: Incorrect Para");
        ...
        {
            I1_TAF_InitMsgDigestFiveCtxt(&stMdCtx);
            I1_TAF_UpdateMsgDigestFiveCtxt(&stMdCtx, &(stConfigHead.ucIdentifier), sizeof(VOS_UINT8));
            I1_TAF_UpdateMsgDigestFiveCtxt(&stMdCtx, pstLocalPdnBearInfo->aucPwd, pstLocalPdnBearInfo->ucPwdLen);
#ifdef DMT
            I1_TAF_UpdateMsgDigestFiveCtxt(&stMdCtx, aucDmtAuthChapChalVal, CNAS_EHSM_CHAP_CHALLENGE_LEN);
#else
            I1_TAF_UpdateMsgDigestFiveCtxt(&stMdCtx, aucAuthChapChalVal, CNAS_EHSM_CHAP_CHALLENGE_LEN);
#endif
            I1_TAF_EndMsgDigestFive(aucAuthChapRespVal, &stMdCtx);

        }
#if (3 == MULTI_MODEM_NUMBER)
        else if (MODEM_ID_2 == CNAS_CCB_GetCdmaModeModemId())
        {
            I2_TAF_InitMsgDigestFiveCtxt(&stMdCtx);
            I2_TAF_UpdateMsgDigestFiveCtxt(&stMdCtx, &(stConfigHead.ucIdentifier), sizeof(VOS_UINT8));
            I2_TAF_UpdateMsgDigestFiveCtxt(&stMdCtx, pstLocalPdnBearInfo->aucPwd, pstLocalPdnBearInfo->ucPwdLen);
#ifdef DMT
            I2_TAF_UpdateMsgDigestFiveCtxt(&stMdCtx, aucDmtAuthChapChalVal, CNAS_EHSM_CHAP_CHALLENGE_LEN);
#else
            I2_TAF_UpdateMsgDigestFiveCtxt(&stMdCtx, aucAuthChapChalVal, CNAS_EHSM_CHAP_CHALLENGE_LEN);
#endif
            I2_TAF_EndMsgDigestFive(aucAuthChapRespVal, &stMdCtx);
        }
#endif
#endif
        else
        {
            ...
            CNAS_WARNING_LOG(UEPS_PID_EHSM, "CNAS_EHSM_EncodePco invalid modemId!");
        }
