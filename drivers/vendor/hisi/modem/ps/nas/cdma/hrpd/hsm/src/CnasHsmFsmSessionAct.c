...
)
{
    ...
    /* Session激活失败，上报CHR信息 */
    if (CNAS_HSM_SESSION_ACTIVE_RESULT_SUCC != enRslt)
    {
        NAS_ERRLOG_CCB_SetHrpdSessionFailEndTime(VOS_GetSlice());
        NAS_ERRLOG_CCB_SetHrpdSessionFailCause(enFailReason);

        if (VOS_TRUE == CNAS_HSM_IsSessionFailReasonNeedReportErrLog(enFailReason))
        {
            /* 上报Session激活失败CHR信息 */
            CNAS_HSM_ErrLogReportSessionFail();
        }
    }
    /*  CHR信息上报后，清空 */
...
PS_BOOL_ENUM_UINT8 CNAS_HSM_IsNeedNegEhrpd(
    CNAS_CAS_HRPD_SCP_ACT_TYPE_ENUM_UINT8                   enScpActType
)
{
...
)
{
    ...
    {
        enEhrpdFlag = CNAS_HSM_IsNeedNegEhrpd(enScpActType);
    }

#if (FEATURE_PTM == FEATURE_ON)
    /* 保存Session激活相关信息 */
    NAS_ERRLOG_CCB_SetHrpdSessionFailStartTime(VOS_GetSlice());
    NAS_ERRLOG_CCB_SetHrpdSessionFailActReason(pstSessionActiveReq->enSessionActiveReason);
    NAS_ERRLOG_CCB_SetHrpdSessionFailActType(enScpActType);
    NAS_ERRLOG_CCB_SetHrpdSessionFailSupportEhrpdFlg(enEhrpdFlag);
#endif
...
VOS_UINT32 CNAS_HSM_RcvHrmDataInd_SessionAct_WaitScpActiveCnf(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
...
VOS_UINT32 CNAS_HSM_RcvHrmDataInd_SessionAct_WaitPaRatModeNtf(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
