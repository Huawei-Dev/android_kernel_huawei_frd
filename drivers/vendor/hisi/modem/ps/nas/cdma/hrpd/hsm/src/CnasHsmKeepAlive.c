...
)
{
    ...
    if (CNAS_HSM_HRPD_CONN_STATUS_OPEN == CNAS_HSM_GetConnStatus())
    {
        if (VOS_TRUE == CNAS_HSM_GetClearKATimerInConnOpenFlg())
        {
            ...
            CNAS_HSM_SetTsmpCloseRemainTime(usTsmpClose * CNAS_HSM_MIN_TO_SEC);
...
)
{
     ...
     && (VOS_FALSE == ucCardChangedFlg))
    {
        ...
        ulIsSupportEhrpd        = CNAS_HSM_IsSupportEhrpd();

        if ((CNAS_HSM_SESSION_TYPE_EHRPD == enLastSessionType)
         && (VOS_FALSE == ulIsSupportEhrpd))
        {
            return VOS_FALSE;
        }
        ...
        if ((CNAS_HSM_SESSION_TYPE_HRPD == enLastSessionType)
         && (VOS_TRUE == ulIsSupportEhrpd))
        {
            /* 如果之前在LTE成功驻留，优先尝试EHRPD,直接进行初始协商 */
            if (VOS_TRUE == CNAS_HSM_GetRegLteSuccFlag())
            {
                return VOS_FALSE;
            }

            /*
                 如果NV中配的开关机后恢复ehrpd能力标记为VOS_TRUE，则在开机后如果平台能力支持ehrpd且关机前的会话类型为hrpd，则开机后需要重新协商,
                 否则开机后直接recover hrpd会话。
              */
            if (VOS_FALSE != CNAS_HSM_GetRecoverEhrpdAvailFlg())
            {
                return VOS_FALSE;
            }
        }
...
VOS_VOID CNAS_HSM_ProcKeepAliveTimerFwdTrafDataNotRcvd(VOS_VOID)
{
    ...
    if (CNAS_HSM_SMP_KEEP_ALIVE_LIMIT == ucKeepAliveReqSndCount)
    {
        ...
        else
        {
            ...
            CNAS_HSM_ErrLogReportSessionExcepDeact(NAS_ERR_LOG_SESSION_EXCEP_DEACT_CAUSE_BY_KEEPALIVE_TIMEOUT,
    ...
    else
    {
        ...
        CNAS_HSM_SndHrmSessionBeginNtf(HSM_HRM_SESSION_TYPE_MO_KEEP_ALIVE);

        CNAS_HSM_SndKeepAliveReqMsg(VOS_TRUE);
