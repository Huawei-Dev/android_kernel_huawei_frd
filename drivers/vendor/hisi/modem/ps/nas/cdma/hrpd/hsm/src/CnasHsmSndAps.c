...
VOS_VOID CNAS_HSM_SndApsSessionNegStatusInd(
    VOS_UINT8                           ucSessionNegStatus
)
{
    HSM_APS_SESSION_NEG_STATUS_IND_STRU                    *pstSessionNegStatusInd = VOS_NULL_PTR;
    VOS_UINT32                                              ulLen;

    ulLen = sizeof(HSM_APS_SESSION_NEG_STATUS_IND_STRU) - VOS_MSG_HEAD_LENGTH;

    pstSessionNegStatusInd = (HSM_APS_SESSION_NEG_STATUS_IND_STRU *)PS_ALLOC_MSG(UEPS_PID_HSM, ulLen);
    if (CNAS_HSM_IS_NULL_PTR(pstSessionNegStatusInd))
    {
        return;
    }

    NAS_MEM_SET_S(((VOS_UINT8 *)pstSessionNegStatusInd + VOS_MSG_HEAD_LENGTH),
                  ulLen,
                  0,
                  ulLen);

    CNAS_CFG_MSG_HDR(pstSessionNegStatusInd,
                     UEPS_PID_HSM,
                     CNAS_CCB_ConvertPidBasedOnModemId(CNAS_CCB_GetCdmaModeModemId(), WUEPS_PID_TAF),
                     ID_HSM_APS_SESSION_NEG_STATUS_IND,
                     ulLen);

    pstSessionNegStatusInd->ucSessionNegStatus = ucSessionNegStatus; 

    CNAS_MNTN_LogMsgInfo((MSG_HEADER_STRU *)pstSessionNegStatusInd);

    (VOS_VOID)PS_SEND_MSG(UEPS_PID_HSM, pstSessionNegStatusInd);
