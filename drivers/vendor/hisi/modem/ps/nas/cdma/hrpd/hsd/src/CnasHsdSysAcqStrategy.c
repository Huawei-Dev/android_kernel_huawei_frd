...
)
{
    ...
    CNAS_HSD_HRPD_SCAN_FREQ_ITEM_STRU   stHsdChannelInfo = {0};
    VOS_UINT16                          i;
    VOS_UINT16                          usIndex;
    ...
    /* mem move */
    NAS_MEM_MOVE_S(&pstScanList->pstScanFreqItem[usScanListPos],
                   (pstScanList->usTotalFreqNum - usScanListPos) * (sizeof(CNAS_HSD_HRPD_SCAN_FREQ_ITEM_STRU)),
                   &pstScanList->pstScanFreqItem[usScanListPos + 1],
                   (usIndex - usScanListPos) * (sizeof(CNAS_HSD_HRPD_SCAN_FREQ_ITEM_STRU)));

    /* 赋值最后一个 */
    NAS_MEM_CPY_S(&pstScanList->pstScanFreqItem[usIndex],
                  sizeof(CNAS_HSD_HRPD_SCAN_FREQ_ITEM_STRU),
                  &stHsdChannelInfo,
                  sizeof(CNAS_HSD_HRPD_SCAN_FREQ_ITEM_STRU));
...
)
{
    ...
    /* 存储当前匹配的1X系统*/
    NAS_MEM_CPY_S(CNAS_HSD_GetScanListMatched1xSystemInfo(),
...
)
{
    ...
    if (CNAS_HSD_SYS_MODE_HYBRID == enModeType)
    {
        ...
        else
        {
            ...
            NAS_MEM_SET_S(pstGeoListInfo, sizeof(CNAS_PRL_MULTI_MATCHED_GEO_LIST_INFO_STRU), 0, sizeof(CNAS_PRL_MULTI_MATCHED_GEO_LIST_INFO_STRU));
...
)
{
    ...
    /* 清零 */
    NAS_MEM_SET_S(pstScanList->pstScanFreqItem,
                  usTotalFreqNum * sizeof(CNAS_HSD_HRPD_SCAN_FREQ_ITEM_STRU),
                  0,
                  usTotalFreqNum * sizeof(CNAS_HSD_HRPD_SCAN_FREQ_ITEM_STRU));
...
)
{
    ...
    NAS_MEM_SET_S(pstSysList->pstHrpdSysItem,
                  usSysRecNum * (sizeof(CNAS_HSD_HRPD_SYS_REC_ITEM_STRU)),
                  0,
                  usSysRecNum * (sizeof(CNAS_HSD_HRPD_SYS_REC_ITEM_STRU)));
...
{
    CNAS_HSD_HRPD_SCAN_FREQ_LIST_STRU  stTempScanList = {0};
    CNAS_HSD_HRPD_MRU_LIST_STRU       *pstMRUList;
    ...
    NAS_MEM_SET_S(&stTempScanList, sizeof(CNAS_HSD_HRPD_SCAN_FREQ_LIST_STRU), 0, sizeof(CNAS_HSD_HRPD_SCAN_FREQ_LIST_STRU));
    ...
    /* 清零 */
    NAS_MEM_SET_S(stTempScanList.pstScanFreqItem,
                  usFreqNum * sizeof(CNAS_HSD_HRPD_SCAN_FREQ_ITEM_STRU),
                  0,
                  usFreqNum * sizeof(CNAS_HSD_HRPD_SCAN_FREQ_ITEM_STRU));
...
)
{
    ...
    for (i = usCurrIndex; i < usAvailFreqNum; i++)
    {
        ...
        /* 拷贝到可用列表中 */
        NAS_MEM_CPY_S(&pstFreq[ulHrpdFreqNum],
...
)
{
    ...
    NAS_MEM_SET_S(&stGeoListInfo,
                  sizeof(CNAS_PRL_MATCHED_GEO_LIST_INFO_STRU),
                  0,
                  sizeof(CNAS_PRL_MATCHED_GEO_LIST_INFO_STRU));
...
)
{
    ...
    NAS_MEM_SET_S(pstSrcMostMatchGeoListInfo,
                  sizeof(CNAS_PRL_MULTI_MATCHED_GEO_LIST_INFO_STRU),
                  0,
                  sizeof(CNAS_PRL_MULTI_MATCHED_GEO_LIST_INFO_STRU));

    NAS_MEM_SET_S(pstDestMostMatchGeoListInfo,
                  sizeof(CNAS_PRL_MULTI_MATCHED_GEO_LIST_INFO_STRU),
                  0,
                  sizeof(CNAS_PRL_MULTI_MATCHED_GEO_LIST_INFO_STRU));
    ...
    if (0 == PS_MEM_CMP(pstDestMostMatchGeoListInfo, pstSrcMostMatchGeoListInfo, sizeof(CNAS_PRL_MULTI_MATCHED_GEO_LIST_INFO_STRU)))
    {
...
)
{
    ...
    */
    ...
    NAS_MEM_SET_S(pstMatchedGeoList, sizeof(CNAS_PRL_MULTI_MATCHED_GEO_LIST_INFO_STRU), 0, sizeof(CNAS_PRL_MULTI_MATCHED_GEO_LIST_INFO_STRU));
    ...
    for (ucIndex = 0; ucIndex < pstMatchedGeoList->ucGeoNum; ucIndex++)
    {
        ...
        if (enGeoMostMatchLvl < pstMulTIMatchedGeo->enGeoMostMatchLvl)
        {
            ...
            ucMostMatchGeoNum = 0;
            NAS_MEM_SET_S(aucGeoIndex, CNAS_PRL_MAX_MATCHING_GEO_NUM, 0, CNAS_PRL_MAX_MATCHING_GEO_NUM);
