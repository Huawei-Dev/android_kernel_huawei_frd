...
VOS_VOID CNAS_EHSM_MNTN_LogErrorInfo(VOS_VOID)
{
    CNAS_EHSM_MNTN_LOG_ERROR_INFO_STRU   *pstMsg = VOS_NULL_PTR;

    pstMsg = (CNAS_EHSM_MNTN_LOG_ERROR_INFO_STRU*)PS_MEM_ALLOC(UEPS_PID_EHSM,
                             sizeof(CNAS_EHSM_MNTN_LOG_ERROR_INFO_STRU));
    if (VOS_NULL_PTR == pstMsg)
    {
        CNAS_ERROR_LOG(UEPS_PID_EHSM, "CNAS_EHSM_MNTN_LogErrorInfo: Alloc Memory Failed!");
        return;
    }
    ...
    pstMsg->ulSenderCpuId   = VOS_LOCAL_CPUID;
    pstMsg->ulSenderPid     = UEPS_PID_EHSM;
    pstMsg->ulLength        = sizeof(CNAS_EHSM_MNTN_LOG_ERROR_INFO_STRU) - VOS_MSG_HEAD_LENGTH;
    pstMsg->enMsgId         = ID_CNAS_EHSM_MNTN_LOG_ERROR_INFO_IND;

    NAS_MEM_CPY_S(&pstMsg->stErrorInfo,
                  sizeof(CNAS_EHSM_ERROR_INFO_TOTAL_STRU),
                  &CNAS_EHSM_GetEhsmCtxAddr()->stErrorLogInfo,
                  sizeof(CNAS_EHSM_ERROR_INFO_TOTAL_STRU));
...
VOS_VOID CNAS_EHSM_MNTN_LogPcReplayFixedContext(VOS_VOID)
{
    CNAS_EHSM_CTX_STRU                                     *pstEhsmCtx = VOS_NULL_PTR;
    CNAS_EHSM_PC_REPLAY_FIXED_CONTEXT_MSG_STRU                        stFixedCtxData;

    pstEhsmCtx = CNAS_EHSM_GetEhsmCtxAddr();

    NAS_MEM_SET_S(&stFixedCtxData,
                  sizeof(stFixedCtxData) ,
                  0,
                  sizeof(stFixedCtxData));

    stFixedCtxData.ulAutoAttachFlag = pstEhsmCtx->ulAutoAttachFlag;
    stFixedCtxData.ulExpireTimerLenOfRetryConn = pstEhsmCtx->stRetryCtrlInfo.stRetryConnEstInfo.ulExpireTimerLen;
    stFixedCtxData.ulExpireTimerLenOfRetryPdn  = pstEhsmCtx->stRetryCtrlInfo.stRetryPdnSetupInfo.ulExpireTimerLen;
    stFixedCtxData.usMaxNoOfRetryConn          = pstEhsmCtx->stRetryCtrlInfo.stRetryConnEstInfo.usMaxNoOfRetry;
    stFixedCtxData.usMaxNoOfRetryPdn           = pstEhsmCtx->stRetryCtrlInfo.stRetryPdnSetupInfo.usMaxNoOfRetry;

    CNAS_MNTN_LogPcReplayFixedContext(sizeof(CNAS_EHSM_PC_REPLAY_FIXED_CONTEXT_MSG_STRU), UEPS_PID_EHSM, (VOS_UINT8 *)&stFixedCtxData);
