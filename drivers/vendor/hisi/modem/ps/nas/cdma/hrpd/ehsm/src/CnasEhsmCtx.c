...
)
{
    ...
    CNAS_EHSM_SaveCacheMsgInMsgQueue(ulEventType, pstMsg, enAbortType);
...
)
{
    ...
    /* 如果是需要立即执行的缓存消息，需要前插到缓存队列的最前面 */
    if (CNAS_EHSM_ABORT_FSM_IMMEDIATELY == enAbortType)
    {
        /* 缓存队列中的消息依次后移 */
        for (ulIndex = pstMsgQueue->ucCacheMsgNum; ulIndex > 0; ulIndex--)
        {
            pstMsgQueue->pastCacheMsg[ulIndex] = pstMsgQueue->pastCacheMsg[ulIndex - 1];
        }
    ...
    {
        /* 保存当前的消息到队列的最后面 */
        ulIndex = pstMsgQueue->ucCacheMsgNum;
    }

    pstMsgQueue->pastCacheMsg[ulIndex]
            = (VOS_UINT8 *)PS_MEM_ALLOC(UEPS_PID_EHSM, pstMsgHeader->ulLength + VOS_MSG_HEAD_LENGTH);

    if (VOS_NULL_PTR == pstMsgQueue->pastCacheMsg[ulIndex])
    {
        return;
    }

    NAS_MEM_CPY_S(pstMsgQueue->pastCacheMsg[ulIndex],
...
{
    CNAS_EHSM_GetEhsmCtxAddr()->enWorkMode = enWorkMode;
...
{
    return CNAS_EHSM_GetEhsmCtxAddr()->enWorkMode;
}
...
)
{
    ...
    pstLocalPdnBearInfo->enAttachType   = CNAS_EHSM_ATTACH_TYPE_INITIAL;
...
)
{
    ...
    pstLocalPdnBearInfo->enAttachType   = CNAS_EHSM_ATTACH_TYPE_HANDOVER;
...
)
{
     ...
     && (CNAS_EHSM_CAUSE_SUCCESS == enCause))
    {
       ...
       {
            pstPdnInfo->stPcoIpv4Item.bitOpPriDns = VOS_TRUE;
       ...
       else if (2 == stMsgIE.ucIpv4DnsSerNum)
       {
            ...
            NAS_MEM_CPY_S(&(pstPdnInfo->stPcoIpv4Item.aucPriDns[0]),
                          ...
                          &(stMsgIE.astIpv4DnsServer[0].aucIpV4Addr[0]),
                          CNAS_EHSM_IPV4_ADDR_LEN);
            pstPdnInfo->stPcoIpv4Item.bitOpSecDns= VOS_TRUE;
       ...
       {
            pstPdnInfo->stPcoIpv6Item.bitOpPriDns = VOS_TRUE;
       ...
       else if (2 == stMsgIE.ucIpv6DnsSerNum)
       {
            ...
            NAS_MEM_CPY_S(&(pstPdnInfo->stPcoIpv6Item.aucPriDns[0]),
                          ...
                          &(stMsgIE.astIpv6DnsServer[0].aucIpV6Addr[0]),
                          CNAS_EHSM_IPV6_ADDR_LEN);
            pstPdnInfo->stPcoIpv6Item.bitOpSecDns= VOS_TRUE;
...
{
    CNAS_EHSM_RETRY_PDN_BEAR_INFO_STRU                     *pstRetryPdnInfo = VOS_NULL_PTR;
    CNAS_EHSM_RETRY_PDN_BEAR_INFO_STRU                      stTmpApnInfo = {0};
    VOS_UINT32                                              ulLoop;
...
)
{
    ...
    pstPdnInfo = &(CNAS_EHSM_GetEhsmCtxAddr()->astLocalPdnBearInfo[ucPdnId]);
    return pstPdnInfo->ucRetryTotalCnt;
}
...
)
{
    ...
    pstPdnInfo = &(CNAS_EHSM_GetEhsmCtxAddr()->astLocalPdnBearInfo[ucPdnId]);
    return pstPdnInfo->enAttachType;
}
...
{
    CNAS_EHSM_ERROR_INFO_TOTAL_STRU          *pstErrorInfo = VOS_NULL_PTR;

    pstErrorInfo = &(CNAS_EHSM_GetEhsmCtxAddr()->stErrorLogInfo);

    NAS_MEM_SET_S(pstErrorInfo,
                  sizeof(CNAS_EHSM_ERROR_INFO_TOTAL_STRU),
                  0x00,
                  sizeof(CNAS_EHSM_ERROR_INFO_TOTAL_STRU));
}
...
VOS_VOID CNAS_EHSM_AllocErrorLog(VOS_VOID)
{
    ...
    if (pstErrorInfo->ulCount < CNAS_EHSM_MAX_ERROR_LOG_INFO_NUM)
    {
        pstErrorInfo->ulCount ++;
    }

    /* 分配不到时，把最近的一次异常信息刷新到最后一个记录中 */
    return;
}
...
)
{
    ...
    /* 分配一个新的记录，分配不到时，把最近的一次异常信息刷新到最后一个记录中 */
    CNAS_EHSM_AllocErrorLog();
    ulIndex = (VOS_UINT32)(pstErrorInfo->ulCount - 1);

    pstErrorInfo->astErrorInfo[ulIndex].ulTime            = VOS_GetTick();
    pstErrorInfo->astErrorInfo[ulIndex].enErrorCode       = enErrorCode;
    pstErrorInfo->astErrorInfo[ulIndex].enFsmId           = CNAS_EHSM_GetCurrFsmId();
    pstErrorInfo->astErrorInfo[ulIndex].ulState           = CNAS_EHSM_GetFsmTopState();
    pstErrorInfo->astErrorInfo[ulIndex].ulParentEventType = CNAS_EHSM_GetCurFsmAddr()->stEntryMsg.ulEventType;
    pstErrorInfo->astErrorInfo[ulIndex].enEhrpdWorkMode   = CNAS_EHSM_GetHrpdWorkMode();
    pstErrorInfo->astErrorInfo[ulIndex].enEhrpdState      = CNAS_EHSM_GetEhrpdState();
    pstErrorInfo->astErrorInfo[ulIndex].ucLteAttachFlag   = (VOS_UINT8)ulAttachFlag;
    pstErrorInfo->astErrorInfo[ulIndex].ucLteOption       = (VOS_UINT8)ulPdnOption;
    pstErrorInfo->astErrorInfo[ulIndex].ulBitCid          = ulBitCid;
    pstErrorInfo->astErrorInfo[ulIndex].ulEpsbId          = ulEpsbId;
...
)
{
    ...
    pstEhsmCtx->ucHandOverFailFlag = ucHandoverFail;
}
...
VOS_UINT8 CNAS_EHSM_GetHandOverFailFlag(VOS_VOID)
{
    ...
    return pstEhsmCtx->ucHandOverFailFlag;
}


#if (FEATURE_ON == FEATURE_PTM)

VOS_VOID CNAS_EHSM_GetEhsmErrInfo(
    VOS_UINT32                         *pulErrCnt,
    NAS_ERR_LOG_EHSM_ERROR_INFO_STRU   *pstErrInfo
)
{
    CNAS_EHSM_ERROR_INFO_TOTAL_STRU    *pstEhsmErrorInfo = VOS_NULL_PTR;
    VOS_UINT32                          ulIndex;


    pstEhsmErrorInfo    = &(CNAS_EHSM_GetEhsmCtxAddr()->stErrorLogInfo);
    *pulErrCnt          = pstEhsmErrorInfo->ulCount;

    if (NAS_ERR_LOG_MAX_CDMA_PS_DISC_RECORD_NUM < *pulErrCnt)
    {
        *pulErrCnt = NAS_ERR_LOG_MAX_CDMA_PS_DISC_RECORD_NUM;
    }

    if (0 != *pulErrCnt)
    {
        /* 上报所有的ERROR LOG 记录 */
        for (ulIndex = 0; ulIndex < *pulErrCnt; ulIndex++)
        {
            pstErrInfo[ulIndex].ulTime             = pstEhsmErrorInfo->astErrorInfo[ulIndex].ulTime;
            pstErrInfo[ulIndex].enFsmId            = pstEhsmErrorInfo->astErrorInfo[ulIndex].enFsmId;
            pstErrInfo[ulIndex].enErrorCode        = pstEhsmErrorInfo->astErrorInfo[ulIndex].enErrorCode;
            pstErrInfo[ulIndex].ulState            = pstEhsmErrorInfo->astErrorInfo[ulIndex].ulState;
            pstErrInfo[ulIndex].ulParentEventType  = pstEhsmErrorInfo->astErrorInfo[ulIndex].ulParentEventType;
            pstErrInfo[ulIndex].enEhrpdWorkMode    = pstEhsmErrorInfo->astErrorInfo[ulIndex].enEhrpdWorkMode;
            pstErrInfo[ulIndex].enEhrpdState       = pstEhsmErrorInfo->astErrorInfo[ulIndex].enEhrpdState;
            pstErrInfo[ulIndex].ucLteAttachFlag    = pstEhsmErrorInfo->astErrorInfo[ulIndex].ucLteAttachFlag;
            pstErrInfo[ulIndex].ucLteOption        = pstEhsmErrorInfo->astErrorInfo[ulIndex].ucLteOption;
            pstErrInfo[ulIndex].ulBitCid           = pstEhsmErrorInfo->astErrorInfo[ulIndex].ulBitCid;
            pstErrInfo[ulIndex].ulEpsbId           = pstEhsmErrorInfo->astErrorInfo[ulIndex].ulEpsbId;
        }

        /* CHR查询后清除当前的ERROR log */
        NAS_MEM_SET_S(pstEhsmErrorInfo,
                      sizeof(CNAS_EHSM_ERROR_INFO_TOTAL_STRU),
                      0,
                      sizeof(CNAS_EHSM_ERROR_INFO_TOTAL_STRU));
    }
...
VOS_VOID CNAS_EHSM_GetEhsmPndInfoRecord(
    APS_EHSM_PDP_INFO_RECORD_IN_EHSM_STRU  *pstPdnInfo

)
{
    VOS_UINT32                          ulEhrpdPdn;
    VOS_UINT32                          ulLtePdn;

    ulEhrpdPdn  = CNAS_EHSM_GetLocalAllocatedPdnIdNum();
    ulLtePdn    = CNAS_EHSM_GetLteActivePdnNum();

    /* 当前LTE的承载个数不为0时，需要去除重复统计的 */
    if (ulLtePdn != 0)
    {
        pstPdnInfo->ulPdnNum = ulEhrpdPdn + (ulLtePdn - 1);
    }
    else
    {
        pstPdnInfo->ulPdnNum = ulEhrpdPdn;
    }

    if (pstPdnInfo->ulPdnNum > EHSM_APS_MAX_PDN_NUM)
    {
        pstPdnInfo->ulPdnNum = EHSM_APS_MAX_PDN_NUM;
    }
