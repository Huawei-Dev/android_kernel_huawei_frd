...
)
{
    ...
    {
        CNAS_HSM_SndCasHrpdConnCloseReq(VOS_FALSE);
...
VOS_UINT32 CNAS_HSM_RcvHrmDataInd_ConnMnmt_WaitConnCloseInd(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
...
VOS_UINT32 CNAS_HSM_RcvHrmDataInd_ConnMnmt_WaitConnOpenInd(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
...
VOS_UINT32 CNAS_HSM_RcvCasHrpdCasStatusInd_ConnMnmt_WaitConnOpenInd(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    ...
    CAS_CNAS_HRPD_CAS_STATUS_IND_STRU                      *pstHrpdCasStatus;

    pstHrpdCasStatus    = (CAS_CNAS_HRPD_CAS_STATUS_IND_STRU *)pstMsg;

    /* 进入该函数的场景: 建链先发给cas了，后续需要搜网，cas会直接rel all，不会上报建链结果或者掉网
       前面由于前面是IDLE态，cas会上报INIT，通过该消息退状态机 */
    if (CAS_CNAS_HRPD_CAS_STATUS_ENUM_INIT != pstHrpdCasStatus->enCasStatus)
    {
        return CNAS_HSM_FSM_EVENT_HANDLED;
    }
    ...
    /* 如果不是在搜网，后续会上报network lost ind，此时直接退出，不处理 */
    if (VOS_FALSE == ucHrpdNetWorkSrchingFlg)
    {
        return CNAS_HSM_FSM_EVENT_HANDLED;
    }
    ...
    if (CNAS_HSM_CONN_MNMT_TRIGGER_BY_APS_OPEN_CONN == enTiggerScene)
    {
        ...
        CNAS_HSM_SndApsOrigCnf(ucCallId, HSM_APS_CAUSE_CAN_NOT_EST_CONN);
    }
    else if (CNAS_HSM_CONN_MNMT_TRIGGER_BY_EHSM_OPEN_CONN == enTiggerScene)
    {
...
VOS_UINT32 CNAS_HSM_RcvEhsmEhrpdNotAvailableInd_ConnMnmt_WaitConnOpenInd(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    ...
    /* 如果缓存中有OHM消息，则清除缓存中的OHM消息，因为后续在回退的时候会进行重新协商，此处如果不清除
        OHM消息，则会引起一些不必要的操作(例如UATI流程，去激活流程等)，浪费空口资源 */
    ulHsmEventType = CNAS_BuildEventType(UEPS_PID_HSD, (VOS_UINT16)ID_HSD_HSM_OVERHEAD_MSG_IND);
...
VOS_UINT32 CNAS_HSM_RcvCasHrpdCasStatusInd_ConnMnmt_WaitConnCloseInd(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    ...
    /* 进入该函数的场景: 断链先发给cas了，后续需要搜网，cas会直接rel all，不会上报断链结果或者掉网
       前面由于前面是IDLE态，cas会上报INIT，通过该消息退状态机 */
    if (CAS_CNAS_HRPD_CAS_STATUS_ENUM_INIT != pstHrpdCasStatus->enCasStatus)
    {
        return CNAS_HSM_FSM_EVENT_HANDLED;
