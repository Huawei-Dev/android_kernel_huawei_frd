...
CNAS_HSM_SESSION_DEACT_REASON_PROC_TBL_STRU g_astHsmSessionDeactReasonProcTbl[] =
{
    ...
    {   CNAS_HSM_SESSION_DEACT_REASON_CAS_NEG_RSLT_IND_SCP_IRAT_TO_LTE,
        {0, 0, 0},
        CNAS_HSM_ProcSessionRetryDeactReasonCasNegRsltIndScpIratToLte,
    },
    ...
    {   CNAS_HSM_SESSION_DEACT_REASON_AMP_FAIL_UATI_SNP_DATA_CNF_NO_RF,
        {0, 0, 0},
        CNAS_HSM_ProcSessionRetryDeactReasonAmpFailUatiSnpDataCnfNoRf,
    },
...
)
{
    ...
    if ((CNAS_HSM_HRPD_CAS_STATUS_ENUM_INIT != enConvertedCasStatus)
     && ((RRM_PS_TASK_TYPE_HRPD_SESSION_NEG  == pstRrmStatusInd->enTaskType)
      || (RRM_PS_TASK_TYPE_HRPD_UATI         == pstRrmStatusInd->enTaskType)))
    {
        /* 启动Session激活保护定时器 */
        CNAS_HSM_StartTimer(TI_CNAS_HSM_UATI_AND_SESSION_ACT_PROTECT_TIMER,
                            CNAS_HSM_GetSessionActTimerLen());
...
)
{
    ...
    else if (CNAS_HSM_INTERNAL_SESSION_CLOSE_IND_SCENE_RCV_CAS_NEG_RSLD_IND == pstMsgInd->enScene)
    {
        ...
        /* 此处场景为cas上报session neg rslt消息触发hsm在预处理发送的内部session close消息打断uati过程或者session协商过程,
         * 目前在cnas发起的session协商收到cas的No Rf scp act失败协商结果, 注册资源等待资源可用后再次发起协商
         * 这里如果cas主动进行session协商失败的原因值为No Rf同样进行注册资源, 待资源可用后发起session协商处理
         */
         if (CAS_CNAS_HRPD_SCP_NEG_RSLT_NO_RF == pstMsgInd->stSessionNegRsltInfo.enResult)
         {
           CNAS_HSM_SndRrmRegisterInd(RRM_PS_TASK_TYPE_HRPD_SESSION_NEG, VOS_RATMODE_HRPD);
         }
...
)
{
    ...
    CNAS_HSM_SetSendSessionCloseFlg(VOS_FALSE);
...
VOS_UINT32 CNAS_HSM_RcvHrmDataInd_Open(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
...
)
{
    ...
    if (pstPaAccessAuthCtrlInfo->ucCount >= CNAS_HSM_AN_AUTH_MAX_REJECT_CNT)
    {
...
)
{
    ...
    switch (pstUatiResult->enRslt)
    {
        ...
        case CNAS_HSM_UATI_REQUEST_RESULT_CANCEL :
            ...
            /* 目前CANCEL原因值有两个场景:
                  1)发送UATI req时发现当前存在TCH信道，此时退出状态机等网侧指派UATI assignment；
                  2)Open态UATI过程中，收到Ohm消息指示UE回退到原小区，此时退出UATI状态机；
               */

            break;
        case CNAS_HSM_UATI_REQUEST_RESULT_NO_RF :

            /* 只有open态 uati req消息发送失败结果为no rf时返回此原因值，此时在open态等待资源可用后再发起uati流程 */
            CNAS_HSM_SetOpenUatiRegResFlg(VOS_TRUE);
...
)
{
    ...
    /* 由于page没有预处理函数，若此时有缓存的page消息没有处理，需要判断是否有网，且搜网时不应该将该消息发送到cas */
    if ((CNAS_HSM_HRPD_CAS_STATUS_ENUM_INIT == enConvertedCasStatus)
...
)
{
    ...
    switch (pstMsgInd->enScene)
    {
        ...
        case CNAS_HSM_INTERNAL_SESSION_CLOSE_IND_SCENE_RCV_CAS_NEG_RSLD_IND:
            ...
            /* 此处场景为cas上报session neg rslt消息触发hsm在预处理发送的内部session close消息,
             * 目前在cnas发起的session协商收到cas的No Rf scp act失败协商结果, 注册资源等待资源可用后再次发起协商
             * 如果cas主动进行session协商失败的原因值为No Rf同样进行注册资源, 待资源可用后发起session协商处理
             */
            if (CAS_CNAS_HRPD_SCP_NEG_RSLT_NO_RF == pstMsgInd->stSessionNegRsltInfo.enResult)
            {
                CNAS_HSM_SndRrmRegisterInd(RRM_PS_TASK_TYPE_HRPD_SESSION_NEG, VOS_RATMODE_HRPD);
...
VOS_UINT32 CNAS_HSM_RcvRrmStatusInd_Open(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    ...
    /* 正常如果cas状态进入init说明掉网了，如果掉网hsm会去注册资源，所以就不会再收到rrm的status消息，
     * 这里仍然加上cas状态的判断，作为保护 */
...
)
{
    ...
    CNAS_HSM_SndHrmSessionBeginNtf(HSM_HRM_SESSION_TYPE_UATI);
...
)
{
    ...
    /* 如果正在keep alive定时器超时处理过程中, 终止keep alive超时处理 */
...
)
{
    ...
    {
        CNAS_NORMAL_LOG(UEPS_PID_HSM, "CNAS_HSM_RcvHsdSystemAcqInd_Slave(): IsFirstSysAcq is true");
    }

    /* clear coverity warning */
    if (CNAS_HSM_SESSION_STATUS_OPEN == enCurSessionStatus)
    {
        CNAS_NORMAL_LOG(UEPS_PID_HSM, "CNAS_HSM_RcvHsdSystemAcqInd_Slave(): Cur Session Status is open");
    }
...
)
{
    ...
    if (VOS_TRUE == ucIsFirstSysAcq)
    {
        CNAS_NORMAL_LOG(UEPS_PID_HSM, "CNAS_HSM_RcvHsdIratFromLteNtf_Slave(): IsFirstSysAcq is true");
    }

    /* clear coverity warning */
    if (CNAS_HSM_SESSION_STATUS_OPEN == enCurSessionStatus)
    {
        CNAS_NORMAL_LOG(UEPS_PID_HSM, "CNAS_HSM_RcvHsdIratFromLteNtf_Slave(): Cur Session Status is open");
    }
...
)
{
    ...
    if ((VOS_TRUE == CNAS_HSM_IsSupportEhrpd())
...
VOS_UINT8 CNAS_HSM_IsSectorIdEqual(
    VOS_UINT8      *pstSrcSectorId,
    VOS_UINT8      *pstDestSectorId,
    VOS_UINT8       ucSectorIdLen
)
{
    VOS_UINT32 ulLoop;

    for (ulLoop = 0; ulLoop < ucSectorIdLen; ulLoop++)
    {
        if (pstSrcSectorId[ulLoop] != pstDestSectorId[ulLoop])
        {
            return VOS_FALSE;
        }
    }
...
VOS_VOID CNAS_HSM_ProcSessionRetryDeactReasonScpNoRf(VOS_VOID)
{
    ...
    CNAS_HSM_SndHsdSessionNegResultInd(HSM_HSD_SESSION_NEG_RSLT_NO_RF,
                                       VOS_FALSE,
                                       CNAS_HSM_SESSION_RELEASE_TYPE_BUTT);
}
...
VOS_VOID CNAS_HSM_ProcSessionRetryDeactReasonAmpFailUatiSnpDataCnfNoRf(VOS_VOID)
{
    CNAS_HSM_ResetSessionRetryParameters();
...
VOS_VOID CNAS_HSM_ProcSessionRetryDeactReasonCasNegRsltIndScpIratToLte(VOS_VOID)
{
    CNAS_HSM_ResetSessionRetryParameters();
