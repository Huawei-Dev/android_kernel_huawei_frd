...
)
{
    ...
    if (CNAS_BuildEventType(WUEPS_PID_TAF, ID_APS_EHSM_PDN_DEACTIVATE_REQ) == pstEntryMsg->ulEventType)
    {
        ...
        if (pstPdnDeactivatReq->ucPdnId != ucPdnId)
        {
            return VOS_FALSE;
        }
    }
...
)
{
    ...
    CNAS_EHSM_ProcDetach_Deactivating(EHSM_APS_CAUSE_SUCCESS);
...
VOS_UINT32 CNAS_EHSM_RcvEhsmAbortReq_Deactivating_WaitDetachCnf(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    ...
    if ((CNAS_BuildEventType(UEPS_PID_HSM, ID_HSM_EHSM_SUSPEND_IND) == pstRcvAbortMsg->ulEventType)
     || (CNAS_BuildEventType(UEPS_PID_HSM, ID_HSM_EHSM_SESSION_INFO_IND) == pstRcvAbortMsg->ulEventType)
     || (CNAS_BuildEventType(WUEPS_PID_TAF, ID_APS_EHSM_LOC_DEACTIVATE_NOTIFY) == pstRcvAbortMsg->ulEventType))
    {
        CNAS_EHSM_StopTimer(TI_CNAS_EHSM_WAIT_DETACH_CNF);
    ...
    if (CNAS_BuildEventType(MSPS_PID_PPPC, ID_CTTF_CNAS_EHRPD_DETACH_IND) == pstRcvAbortMsg->ulEventType)
    {
        CNAS_EHSM_StopTimer(TI_CNAS_EHSM_WAIT_DETACH_CNF);
    ...
    /* POWER OFF时加abort标记 等收到cnf消息时候判定*/
...
VOS_UINT32 CNAS_EHSM_RcvHsmConnectInd_Deativating_WaitConnEstRetry(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
...
{
    CNAS_EHSM_SndCttfEhrpdDetachInd();
    CNAS_EHSM_ProcDetach_Deactivating(EHSM_APS_CAUSE_TIME_OUT);
    return VOS_TRUE;
}
...
)
{
    ...
    /* 通知APS去激活成功 */
    CNAS_EHSM_SndApsPdnDeactivateInd(ucPdnId, ucCid, EHSM_APS_CAUSE_SUCCESS);
...
)
{
     ...
     || (CNAS_BuildEventType(WUEPS_PID_TAF, ID_APS_EHSM_LOC_DEACTIVATE_NOTIFY) == pstRcvAbortMsg->ulEventType))
    {
        ...
        if (VOS_TRUE == CNAS_EHSM_GetSendPdnDeactFlag_Deactivating())
        {
            ...
            ucCid      = CNAS_EHSM_GetCidByPdnId(ucPdnId);
            pstPdnInfo = CNAS_EHSM_GetEhrpdLocPdnAddr(ucPdnId);
            ...
            CNAS_EHSM_SndEsmSyncEhrpdPdnInfoInd(pstPdnInfo, 0, EHSM_ESM_PDN_OPT_DISCONNECTED);
        ...
        /* 在等建链结果时，但还未通知PPP去激活，被打断 */
        CNAS_EHSM_ProcAbortMsg_Deactivating_WaitEsting(EHSM_APS_CAUSE_EHRPD_SUSPEND);
        return VOS_TRUE;
...
)
{
     ...
     || (CNAS_BuildEventType(WUEPS_PID_TAF, ID_APS_EHSM_LOC_DEACTIVATE_NOTIFY) == pstRcvAbortMsg->ulEventType))
    {
        ...
        if (VOS_TRUE == CNAS_EHSM_GetSendPdnDeactFlag_Deactivating())
        {
            ...
            CNAS_EHSM_SndApsPdnDeactivateInd(ucPdnId, pstPdnDeactReq->ucCid, EHSM_APS_CAUSE_SUCCESS);
...
VOS_UINT32 CNAS_EHSM_RcvCttfEhrpdDetachCnf_Deactivating_WaitConnEstRetry(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
...
VOS_UINT32 CNAS_EHSM_RcvTiCttfEhrpdDetachCnf_Deactivating_WaitConnEstRetry(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
...
VOS_VOID CNAS_EHSM_ProcDetach_Deactivating(
    VOS_UINT32                          enCause
)
{
    CNAS_EHSM_CTX_STRU                 *pstEhsmCtx         = VOS_NULL_PTR;
...
VOS_VOID CNAS_EHSM_ProcAbortMsg_Deactivating_WaitEsting(
    VOS_UINT32                          enCause
)
{
    CNAS_EHSM_CTX_STRU                 *pstEhsmCtx         = VOS_NULL_PTR;
    ...
    /*在wait detach cnf过程中不会再次收到power off req消息，可能的打断为网测的detach和hsm suspend*/
    if (CNAS_BuildEventType(UEPS_PID_HSD, ID_HSD_EHSM_POWER_OFF_REQ) == pstEntryMsg->ulEventType)
    {
        /*the situation of wait conn est time out is same as that conn est cnf fail*/
        for (ulIndex = 0; ulIndex < CNAS_EHSM_MAX_PDN_NUM; ulIndex++)
