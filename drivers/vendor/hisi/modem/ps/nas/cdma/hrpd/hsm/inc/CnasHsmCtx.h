...
#define CNAS_HSM_UAIT_COMPLETE_MAX_RETRY_CNT                (3) /* 发送Uati Complete后，收到Snp Data Cnf指示失败时，Uati Complete的重试次数*/
#define CNAS_HSM_HARDWAREID_RESP_MAX_RETRY_CNT              (3) /* 发送HardWareId Resp后，收到Snp Data Cnf指示失败时，HardWareId Resp的重试次数 */
#define CNAS_HSM_KEEP_ALIVE_REQ_MAX_RETRY_CNT               (3) /* 发送Keep Alive Req后，收到Snp Data Cnf指示失败时，Keep Alive Req的重试次数 */
#define CNAS_HSM_KEEP_ALIVE_RSP_MAX_RETRY_CNT               (3) /* 发送Keep Alive Resp后，收到Snp Data Cnf指示失败时，Keep Alive Resp的重试次数 */
#define CNAS_HSM_SESSION_CLOSE_MAX_RETRY_CNT                (3) /* 发送Session Close后，收到Snp Data Cnf指示失败时，Session Close的重试次数 */
...
/** ****************************************************************************
 * Name        : CNAS_HSM_HRPD_CONN_STATUS_ENUM
 *
 * Description :
 *******************************************************************************/
enum CNAS_HSM_HRPD_CONN_STATUS_ENUM
{
    CNAS_HSM_HRPD_CONN_STATUS_CLOSE,
    CNAS_HSM_HRPD_CONN_STATUS_OPEN,

    CNAS_HSM_HRPD_CONN_STATUS_BUTT
};
...
enum CNAS_HSM_SESSION_TYPE_ENUM
{
...
typedef struct
{
    ...
    VOS_UINT8                                               ucAbortFlg;              /* deactive流程中是否收到终止要求,VOS_TRUE:收到, VOS_FALSE:未收到 */
    CNAS_HSM_SESSION_DEACT_REVISE_TIMER_SCENE_ENUM_UINT8    enReviseTimerScene;      /* 需要启动修正定时器的场景 */
    VOS_UINT8                                               ucSuspendFlg;            /* deactive流程中是否收suspend ind,VOS_TRUE:收到, VOS_FALSE:未收到 */
...
typedef struct
{
    ...
    VOS_UINT8                                               ucCount;
    VOS_UINT8                                               aucReserve[3];
} CNAS_HSM_PA_ACCESS_AUTH_CTRL_STRU;
...
typedef struct
{
    ...
    VOS_UINT8                                               ucUatiReqRetryTimesWhenUatiAssignTimerExpireInAmpOpen;/* 在open态时当UATI req发送成功后等网侧UATI assign超时
                                                                                                                                   后的重试次数,默认不重试，最大重试4次 */
    VOS_UINT8                                               ucSendSessionCloseFlg;          /* 保存一次Session有效期间内是否发送过Session Close消息 */
    ...
    CNAS_HSM_PA_ACCESS_AUTH_CTRL_STRU                       stPaAccessAuthCtrlInfo;

    VOS_UINT8                                               aucSectorIdOfLastUatiReq[CNAS_HSM_SUBNET_ID_MAX_LENGTH];/* 发送UATI req时的sector Id */
...
typedef struct
{
    ...
    VOS_UINT8                                               ucHardWareIdRspRetryTimes;         /* 发送HardWareId Rsp后，收到Snp Data Cnf指示失败时，HardWareId Resp的重试次数 */
    VOS_UINT8                                               ucKeepAliveReqRetryTimes;          /* 发送Keep Alive Req后，收到Snp Data Cnf指示失败时，Keep Alive Req的重试次数 */
    VOS_UINT8                                               ucKeepAliveRspRetryTimes;          /* 发送Keep Alive Rsp后，收到Snp Data Cnf指示失败时，Keep Alive Resp的重试次数 */
    VOS_UINT8                                               ucSessionCloseRetryTimes;          /* 发送Session Close后，收到Snp Data Cnf指示失败时，Session Close的重试次数 */

    VOS_UINT8                                               ucStoreANKeepAliveReqTransId;        /* 保存网络侧发送过来的KeepAlive Req TransId,用于KeepAlive Rsp消息重传 */
    VOS_UINT8                                               aucReserve[2];
    CNAS_HSM_HARDWARE_ID_RESPONSE_MSG_STRU                  stStoreHardWareIdRsp;              /* 保存上次给CTTF发送的HardWareId Rsp消息,用于HardWareId Rsp消息重传 */
    CNAS_HSM_SESSION_CLOSE_MSG_STRU                         stStoreSessionClose;               /* 保存上次给CTTF发送的Session Close消息,用于Session Close消息重传 */
}CNAS_HSM_RETRANSMIT_CTRL_STRU;


typedef struct
{
    VOS_UINT8                           ucOpenUatiRegResFlg;                       /* open态uati过程no rf失败注册资源标记 */
    VOS_UINT8                           aucRsv[3];
}CNAS_HSM_RES_REGISTER_CTRL_STRU;
...
typedef struct
{
    ...
    CNAS_HSM_RETRANSMIT_CTRL_STRU                           stRetransmitCtrlInfo;          /* 在AC_OR_RTC信道上发送的消息重传控制结构体 */

    CNAS_HSM_RES_REGISTER_CTRL_STRU                         stResRegisterCtrlInfo;          /* no rf情况下, 注册资源记录 */
}CNAS_HSM_CTX_STRU;
