...
)
{
    ...
    switch (enAttachReason)
    {
        case CNAS_EHSM_ATTACH_REASON_INITIAL_ATTACH:
        {
            ...
            pstMsg->ucRetry     = CNAS_EHSM_GetPdnRetryTotal(ucPdnId);
        ...
        {
            ucLtePdnIndex  = CNAS_EHSM_GetLtePdnIndexWithMinEspbId();
            pstLtePdnInfo  = &(CNAS_EHSM_GetEhsmCtxAddr()->astLtePdnBearInfo[ucLtePdnIndex]);
