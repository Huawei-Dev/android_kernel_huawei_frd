...
#define CNAS_HLU_FILL_HRM_REQ_MSG(pstHrmDataReq, pstMsgTxInfo, enProtType, enDataRequestType) \
{ \
    (pstHrmDataReq)->enMsgId            = ID_HLU_HRM_DATA_REQ; \
    (pstHrmDataReq)->stDataReqCommContent.enProtocolType     = enProtType; \
    (pstHrmDataReq)->stDataReqCommContent.enInstanceType     = CTTF_HRPD_INSTANCE_INUSE; \
    (pstHrmDataReq)->stDataReqCommContent.enAddressType      = (pstMsgTxInfo)->enAddressType; \
    (pstHrmDataReq)->stDataReqCommContent.enChannelType      = (pstMsgTxInfo)->enChannelType; \
    (pstHrmDataReq)->stDataReqCommContent.enDeliveryType     = (pstMsgTxInfo)->enDeliveryType; \
    (pstHrmDataReq)->stDataReqCommContent.ucPriority         = (pstMsgTxInfo)->ucPriority; \
    (pstHrmDataReq)->stDataReqCommContent.enNeedRslt         = CTTF_HRPD_BEST_EFFORT_NEED_RSLT; \
    (pstHrmDataReq)->enDataReqType                           = enDataRequestType; \
}
...
VOS_VOID CNAS_HLU_ProcHrmDataInd
(
    SNPITF_MSG_DATA_STRU               *pstMsgData
);

VOS_VOID CNAS_HLU_SndMsgToHrm
(
    VOS_UINT8                           ucMsgId
);
