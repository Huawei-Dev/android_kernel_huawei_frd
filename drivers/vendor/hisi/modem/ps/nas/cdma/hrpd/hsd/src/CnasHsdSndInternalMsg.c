...
)
{
    ...
    NAS_MEM_SET_S(pstMsg->astFreq,
                  sizeof(CNAS_PRL_FREQENCY_CHANNEL_STRU) * CNAS_HSD_INTERNAL_SYSTEM_ACQ_REQ_CHAN_MAX_NUM,
                  0,
                  sizeof(CNAS_PRL_FREQENCY_CHANNEL_STRU) * CNAS_HSD_INTERNAL_SYSTEM_ACQ_REQ_CHAN_MAX_NUM);
    ...
    {
        NAS_MEM_CPY_S(pstMsg->astFreq,
                      sizeof(CNAS_PRL_FREQENCY_CHANNEL_STRU) * CNAS_HSD_INTERNAL_SYSTEM_ACQ_REQ_CHAN_MAX_NUM,
                      pstFreq,
                      sizeof(CNAS_PRL_FREQENCY_CHANNEL_STRU) * ulFreqNum);
    }
...
VOS_VOID CNAS_HSD_SndBgSrcgRslt(
    CNAS_HSD_SYS_ACQ_SCENE_ENUM_UINT32  enCurAcqScene,
    CNAS_HSD_REACQ_FALG_ENUM_UINT32     enNeedSrchFlag
)
{
    CNAS_HSD_HSD_BG_SRCH_RSLT_CNF_STRU                   *pstMsg;
    VOS_UINT32                                              ulLen;

    /* 从内部消息队列中获取一个还没有使用的空间 */
    ulLen   = sizeof(CNAS_HSD_HSD_BG_SRCH_RSLT_CNF_STRU);

    pstMsg  = (CNAS_HSD_HSD_BG_SRCH_RSLT_CNF_STRU *)PS_MEM_ALLOC(UEPS_PID_HSD, ulLen);
    ...
    pstMsg->ulReceiverCpuId             = VOS_LOCAL_CPUID;
    pstMsg->ulReceiverPid               = UEPS_PID_HSD;
    pstMsg->enMsgId                     = ID_CNAS_HSD_HSD_BG_SRCH_RSLT_CNF;
    pstMsg->usOpId                      = 0;
    pstMsg->ulLength                    = sizeof(CNAS_HSD_HSD_BG_SRCH_RSLT_CNF_STRU) - VOS_MSG_HEAD_LENGTH;
    pstMsg->enCurAcqScene               = enCurAcqScene;
    pstMsg->enNeedSrchFlag              = enNeedSrchFlag;
