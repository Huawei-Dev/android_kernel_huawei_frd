...
)
{
    ...
    /* 先发了建断链，在发搜网，cas收到搜网后会直接rel all，不会回复建断链结果，也不会有掉网消息上报，hsm根据后续
       上报的status ind消息，退当前状态机 */
...
)
{
    ...
    /* Open态 uati req no rf失败, 已经注册资源, 等待资源可用后继续发起uati流程,
       此时收到连接建立请求不能发起建链流程 */
...
)
{
    ...
    {
        /* 目前在建链状态下发送断链，cas的处理是缓存当前断链请求，等建链成功后上报建链指示以及处理断链请求。
           对于建链被power save等打断的场景，hsm会下发断链请求到cas，并切换到CNAS_HSM_SS_CONN_MNMT_WAIT_HRPD_CONN_CLOSE_IND，
           后续由于cas会上报建链结果，如果在此状态下直接上报链路状态到aps/ehsm，后续收到断链或者掉网消息后，判断要不要给上层
           上报断链结果是根据断链是由aps/ehsm/power save的场景给发送断链指示，但是目前该打断场景aps/ehsm的场景触发的建链，
           直接走到状态机中的else分支，导致hsm不会上报断链结果到aps/ehsm.
           针对该场景，在CNAS_HSM_SS_CONN_MNMT_WAIT_HRPD_CONN_CLOSE_IND收到conn open ind消息,hsm直接丢弃不处理。
           但是这样修改会导致短时间内上下层链路状态不一致，而且hsm保持的链路状态和cas状态不一致 */
...
)
{
    ...
    /* 掉网后恢复驻留会上报OHM, 根据OHM消息决定是否需要继续发起UATI流程 */
...
)
{
          ...
          || (CNAS_HSM_SS_CONN_MNMT_WAIT_HRPD_CONN_CLOSE_IND == enSubState))
    {
        /* 将ID_CNAS_HSM_HSM_SESSION_CLOSE_IND消息的场景设置成IN_CONN_MNMT
          ...
          * CNAS_HSM_SESSION_DEACT_REASON_INTERNAL_SESSION_CLOSE_IN_CONN_MNMT，
          * 在deactive流程中通过该原因值判断是否已经发送过SessionClose到网络
          * 在一次Session有效期间内，若已向网侧发送过Session Close，则不再发送
          */
...
)
{
    ...
    /* 失败原因为CAN_NOT_TX和GENERIC_ERROR                                 -- 不再重发,进入时隙模式
        失败原因为SLP_RESET、AC_CHANNEL_NOT_EXIST、LINK_ACQUIRED和CONN_INIT -- 重发
        失败原因为CONN_CLOSE和RELEASE_ALL，若CNAS维护的CAS状态为INIT        --不再重发,进入时隙模式
        失败原因为CONN_CLOSE和RELEASE_ALL，若CNAS维护的CAS状态为IDLE或CONN  --重发                  */
    switch (enSnpDataCnfFailReason)
    {
        case CTTF_HRPD_SNP_DATA_FAIL_REASON_CAN_NOT_TX:
