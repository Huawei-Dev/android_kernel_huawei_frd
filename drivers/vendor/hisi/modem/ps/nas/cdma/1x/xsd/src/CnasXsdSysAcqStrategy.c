...
CNAS_XSD_SCAN_CHANNEL_LIST_BUILD_TBL_STRU                   g_astCnasXsdScanChanListBuildTbl[] =
{
    ...
    {CNAS_XSD_SYS_ACQ_SCENE_SYSTEM_LOST_WITH_LTE_OR_DO_CONN,    CNAS_XSD_BuildScanChanList_SystemLostWithLteOrDoConn},

    {CNAS_XSD_SYS_ACQ_SCENE_SERVICE_ACQ,    CNAS_XSD_BuildScanChanList_ServiceAcqSearch},
};
...
VOS_VOID CNAS_XSD_AllocMemoryForGeoSysRecordSrchListWithRoamInd(
...
)
{
    ...
    /* 计算匹配的GEO列表中最大频点个数 */
    usMaxAcqSysIdNum = CNAS_XSD_CalcPrlSysRecListTotalFreqNum(usMostMatchIndex, pstMatchedSysRecord);

    /* 取当前等级漫游系统，进行计数 */
    for (i = usMostMatchIndex; i < usMatchedSysRecNum; i++)
    {
        if (CNAS_PRL_SYS_ROAMING_STATUS_OFF == pstMatchedSysRecord[i].enRoamingInd)
        {
            usMaxAcqSysIdNum = usMaxAcqSysIdNum + CNAS_XSD_CalcPrlSysRecListTotalFreqNum(1, &pstMatchedSysRecord[i]);
        }
    }
...
VOS_VOID CNAS_XSD_BuildGeoSysRecordSrchListByLevel(
    VOS_UINT16                                              usMatchedSysRecNum,
    CNAS_PRL_EXT_SYS_RECORD_STRU                           *pstMatchedSysRecord,
    CNAS_XSD_GEO_SYS_RECORD_SRCH_LIST_STRU                 *pstGeoSysRecSrchList,
    VOS_UINT16                                              usLevel,
    VOS_UINT8                                               ucIsNonRoam
)
{
    VOS_UINT32                          i;
    VOS_UINT32                          j;
    ...
    NAS_MEM_SET_S(&stAcqRecFreqInfo, sizeof(CNAS_PRL_ACQ_REC_FREQ_INFO_STRU), 0x00, sizeof(CNAS_PRL_ACQ_REC_FREQ_INFO_STRU));

    /* 根据Level和ROAM进行排序，非ROAM系统记录在前，ROAM系统记录在后 */
    /* 先加入非漫游系统 */
    for (i = 0; i < usMatchedSysRecNum; i++)
    {
        if (usLevel != pstMatchedSysRecord[i].usPriLvl)
        {
            CNAS_ERROR_LOG4(UEPS_PID_XSD, "CNAS_XSD_BuildGeoSysRecordSrchListByLevel Level Err", 9, 1, 1, 1);
        ...
        /* 获取PRL中获取表指针 */
        pstPrlAcqRec = CNAS_PRL_GetSpecifiedPrlAcqRecord(pstMatchedSysRecord[i].usAcqIndex);

        /* 空指针进行保护 */
        if (VOS_NULL_PTR == pstPrlAcqRec)
        {
            CNAS_ERROR_LOG4(UEPS_PID_XSD, "CNAS_XSD_BuildGeoSysRecordSrchListByLevel Null Ptr", 9, 1, 1, 2);

            continue;
        }

        if (VOS_TRUE == ucIsNonRoam)
        {
            if (CNAS_PRL_SYS_ROAMING_STATUS_OFF != pstMatchedSysRecord[i].enRoamingInd)
            {
                continue;
        ...
        {
            NAS_MEM_SET_S(&stAcqRecFreqInfo, sizeof(stAcqRecFreqInfo), 0x00, sizeof(CNAS_PRL_ACQ_REC_FREQ_INFO_STRU));
...
VOS_VOID CNAS_XSD_BuildGeoSysRecordSrchListWithRoamIndStrategy(
...
{
    VOS_UINT32                          i;
    VOS_UINT16                          usCurLevel;
    VOS_UINT16                          usOldLevel;
    VOS_UINT16                          usTotalNum;
    VOS_UINT16                          usOldLevelLength;
    VOS_UINT16                          usAllocNum;

    usOldLevel                          = 0xFFFF;
    usOldLevelLength                    = 0;
    usCurLevel                          = 0xFFFF;
    usTotalNum                          = 0;
    usAllocNum                          = usMostMatchedIndex;

    if (CNAS_PRL_SYS_ROAMING_STATUS_OFF != enCurRoamingInd)
    {
        usAllocNum                      = usMatchedSysRecNum;
    }

    /* 内存分配时，仅分配GEO中高优先级的系统频点+当前同步到的系统频点 */
    CNAS_XSD_AllocMemoryForGeoSysRecordSrchListWithRoamInd(usAllocNum,
                                                           pstMatchedSysRecord,
    ...
    if (VOS_NULL_PTR == pstGeoSysRecSrchList->pstAcqSysInfo)
    {
        CNAS_ERROR_LOG4(UEPS_PID_XSD, "CNAS_XSD_BuildGeoSysRecordSrchListWithRoamIndStrategy NULL PTR", 9, 1, 0, 1);

        return;
    }

    /* 获取GEO中最match的level */
    usCurLevel = (pstMatchedSysRecord + usMostMatchedIndex)->usPriLvl;

    for (i = 0; i < usMatchedSysRecNum; i++)
    {
        /* 当前等级的系统数量计数 */
        if (usOldLevel == pstMatchedSysRecord[i].usPriLvl)
        {
            usOldLevelLength++;
        ...
        if (usCurLevel == usOldLevel)
        {
            CNAS_ERROR_LOG4(UEPS_PID_XSD, "CNAS_XSD_BuildGeoSysRecordSrchListWithRoamIndStrategy Err Match List", 9, 1, 0, 2);

            break;
        }

        /* 等级高于最Match的等级的处理 */
        if (0 != usOldLevelLength)
        {
            /* 根据当前Level进行构表，先加入非漫游系统 */
            CNAS_XSD_BuildGeoSysRecordSrchListByLevel(usOldLevelLength,
                                                     &pstMatchedSysRecord[i - usOldLevelLength],
                                                      pstGeoSysRecSrchList,
                                                      usOldLevel,
                                                      VOS_TRUE);

            /* 根据当前Level进行构表，加入漫游系统 */
            CNAS_XSD_BuildGeoSysRecordSrchListByLevel(usOldLevelLength,
                                                     &pstMatchedSysRecord[i - usOldLevelLength],
                                                      pstGeoSysRecSrchList,
                                                      usOldLevel,
        ...
        usOldLevel = pstMatchedSysRecord[i].usPriLvl;
        usOldLevelLength = 1;
    }
    ...
    {
        CNAS_ERROR_LOG4(UEPS_PID_XSD, "CNAS_XSD_BuildGeoSysRecordSrchListWithRoamIndStrategy Total>=Max", 9, 1, 0, 3);
    ...
    pstGeoSysRecSrchList->pstAcqSysInfo[usTotalNum].usLevel                     = usCurLevel;
    pstGeoSysRecSrchList->pstAcqSysInfo[usTotalNum].enRoamingInd                = enCurRoamingInd;
...
)
{
    ...
    */
    ...
    CNAS_XSD_AddMultiMru0InScanChanList(pstScanChanList);
...
)
{
    ...
    enOocScanStrategy = pstOocScheduleInfo->stStrategyCfg.enOocScanStrategy;
...
{
    /* 目前来看,该场景与system lost场景构造列表逻辑一致；由于现在SYSTEM LOST场景
       在SCAN LIST搜索列表前，插入多个MRU0，所以其他使用system lost场景的地方，均
       应该使用原来的SYSTEM LOST场景搜网，不能使用现有的SYSTEM LOST场景 */
    CNAS_XSD_OOC_SCAN_STRATEGY_ENUM_UINT8                   enOocScanStrategy;
    CNAS_XSD_OOC_SCHEDULE_INFO_STRU                        *pstOocScheduleInfo = VOS_NULL_PTR;
    CNAS_PRL_FREQENCY_CHANNEL_STRU                         *pstOrigFreq        = VOS_NULL_PTR;
    CNAS_PRL_1X_SYSTEM_STRU                                 stMru0SysId;
    ...
    */
    ...
    for (i = 0; i < pstGeoListInfo->ucGeoNum; i++)
    {
        ...
        for (j = pstGeoListInfo->astGeoInfoList[i].usGeoFirstSysRecIndex; j <= usSameOrHighIndex; j++)
        {
            ...
            /* HomeSidNid列表中的Neg系统不加入到构建的频点列表中 */
...
)
{
    ...
    CNAS_PRL_1X_SYSTEM_STRU                                 stOrigSysInfo = {0};
...
VOS_VOID CNAS_XSD_RemoveExpiredFreqFromAvoidList(VOS_VOID)
{
    ...
    VOS_UINT8                                               ucNextAvoidFreqNum;

    ucNextAvoidFreqNum                                      = CNAS_XSD_GetAvoidFreqListNum();
    ...
    for (i = 0; i < CNAS_MIN(ucNextAvoidFreqNum, CNAS_XSD_MAX_AVOID_FREQ_NUM); i++)
    {
...
)
{
    ...
    *pusDstChanNum = 0;
    ...
    /* scan list个数为0，直接退出 */
    if (0 == pstScanChanList->usTotalNum - usCurScanChanIndex)
    {
        return;
    }
    ...
    usChanScanNum = CNAS_XSD_GetMaxSyncFreqNumOfScanList(pstScanChanList->usTotalNum - usCurScanChanIndex);
...
)
{
    ...
    *pusScanChanNum = 0;
    ...
    usMaxFreqNum = CNAS_XSD_GetMaxSyncFreqNumOfGeoSearchList();

    for (i = 0; ((i < pstGeoSysRecSrchList->usTotalNum) && (*pusScanChanNum < usMaxFreqNum)); i++)
    {
...
VOS_UINT32 CNAS_XSD_IsCurFreqSynced(
    CNAS_PRL_FREQENCY_CHANNEL_STRU     *pstFreq,
    CNAS_XSD_SCAN_CHANNEL_LIST_STRU    *pstScanChanList
)
{
    ...
    for (i = 0; i < pstScanChanList->usTotalNum; i++)
    {
        if ((pstFreq->enBandClass == pstScanChanList->pstScanChanInfo[i].stFreqChan.enBandClass)
         && (pstFreq->usChannel   == pstScanChanList->pstScanChanInfo[i].stFreqChan.usChannel)
         && (CNAS_XSD_SCAN_CHAN_STATUS_NO_SYNCED != pstScanChanList->pstScanChanInfo[i].enChanStatus))
        {
            return VOS_TRUE;
        }
    }
...
VOS_VOID CNAS_XSD_AddMultiMru0InScanChanList(
    CNAS_XSD_SCAN_CHANNEL_LIST_STRU    *pstScanChanList
)
{
    CNAS_PRL_1X_SYSTEM_STRU                                 stMru0SysId;
    CNAS_XSD_SCAN_CHANNEL_INFO_STRU                         stPreCampChanInfo = {{0}};
    VOS_UINT16                                              usMru0Times;
    VOS_UINT16                                              i;

    usMru0Times        = 0;

    if (VOS_FALSE == CNAS_XSD_GetSwitchOnOocMru0EnableFlg())
    {
        return;
    }
    ...
    usMru0Times = CNAS_XSD_GetSwitchOnOocMru0Times();

    NAS_MEM_MOVE_S(pstScanChanList->pstScanChanInfo + usMru0Times,
                   (pstScanChanList->usTotalNum)*sizeof(CNAS_XSD_SCAN_CHANNEL_INFO_STRU),
                   pstScanChanList->pstScanChanInfo,
                   (pstScanChanList->usTotalNum)*sizeof(CNAS_XSD_SCAN_CHANNEL_INFO_STRU));

    /*先前驻留频点信息 */
    NAS_MEM_SET_S(&stPreCampChanInfo, sizeof(stPreCampChanInfo), 0, sizeof(stPreCampChanInfo));
    stPreCampChanInfo.stFreqChan.enBandClass = stMru0SysId.stFreq.enBandClass;
    stPreCampChanInfo.stFreqChan.usChannel   = stMru0SysId.stFreq.usChannel;
    stPreCampChanInfo.enChanStatus           = CNAS_XSD_SCAN_CHAN_STATUS_NO_SYNCED;

    for (i = 0;i < usMru0Times; i++)
    {
        *(pstScanChanList->pstScanChanInfo + i) = stPreCampChanInfo;
    }

    pstScanChanList->usTotalNum += usMru0Times;
}
...
)
{
    ...
    pstGeoSysRecSrchList->pstAcqSysInfo[usIndex].enRoamingInd    = pstAcqSysInfo->enRoamingInd;
...
)
{
    ...
    if (CNAS_PRL_SYS_RECORD_TYPE_1X_IS95 == pstSysRecord->enRecordType)
    {
        ...
        stTmpAcqSysInfo.enRoamingInd    = pstSysRecord->enRoamingInd;
...
VOS_UINT32 CNAS_XSD_IsNeedInsertMultiMru0(
    CNAS_XSD_SYS_ACQ_SCENE_ENUM_UINT32                      enSysAcqScene
)
{
    /* 搜网场景不是丢网或者开机场景的话，则直接返回VOS_FALSE,这时不需要多插入MRU0 */
    if ( (CNAS_XSD_SYS_ACQ_SCENE_SYSTEM_LOST != enSysAcqScene)
      && (CNAS_XSD_SYS_ACQ_SCENE_SWITCH_ON != enSysAcqScene) )
    {
        return VOS_FALSE;
    }
...
)
{
    ...
    if (VOS_TRUE == CNAS_XSD_IsNeedInsertMultiMru0(enSysAcqScene))
    {
        usMaxScanListSize += CNAS_XSD_GetSwitchOnOocMru0Times();
    }
...
)
{
    ...
    NAS_MEM_SET_S(&stTmpSysId, sizeof(CNAS_PRL_1X_SYSTEM_STRU), 0x0, sizeof(CNAS_PRL_1X_SYSTEM_STRU));
...
)
{
    ...
    switch (enScanScene)
    {
        ...
        case CNAS_XSD_SYS_ACQ_SCENE_SYSTEM_LOST_WITH_LTE_OR_DO_CONN:
            enSysAcqMode = CNAS_XSD_SYSTEM_ACQUIRE_MODE_BEST_PREF;
            break;
...
VOS_UINT32 CNAS_XSD_IsAcquireAvoidSysNeeded(
    CNAS_XSD_SYS_ACQ_SCENE_ENUM_UINT32  enScanScene
)
{
    CNAS_XSD_SYSTEM_ACQUIRE_MODE_ENUM_UINT8                 enSysAcqMode;
...
)
{
    ...
    for (i = 0; i < usPreSrchChanNum; i++)
    {
        NAS_MEM_SET_S(&stTmpScanChanInfo, sizeof(CNAS_XSD_SCAN_CHANNEL_INFO_STRU), 0x0, sizeof(CNAS_XSD_SCAN_CHANNEL_INFO_STRU));
...
)
{
    ...
    if ((CNAS_XSD_OOC_SCAN_NORMAL == pstOocScheduleInfo->stStrategyCfg.enOocScanStrategy)
     || (CNAS_XSD_OOC_SCAN_BUTT   == pstOocScheduleInfo->stStrategyCfg.enOocScanStrategy))
    {
        return VOS_FALSE;
    }
...
VOS_VOID CNAS_XSD_BuildScanChanList_SystemLostWithLteOrDoConn(
    VOS_UINT16                          usPreSrchChanNum,
    CNAS_PRL_FREQENCY_CHANNEL_STRU     *pstPreSrchChanArray,
    CNAS_XSD_SCAN_CHANNEL_LIST_STRU    *pstScanChanList
)
{
    ...
    /* 将MRU[0]中的频点信息增加到pstScanChanListInfo中 */
    if (VOS_TRUE == CNAS_XSD_GetMru0SysId(&stMru0SysId))
    {
        NAS_MEM_SET_S(&stTmpScanChanInfo, sizeof(stTmpScanChanInfo), 0x0, sizeof(CNAS_XSD_SCAN_CHANNEL_INFO_STRU));
...
VOS_UINT16 CNAS_XSD_GetMaxSyncFreqNumOfScanList(
    VOS_UINT16                          usChanScanNum
)
{
    ...
    if ((CNAS_XSD_SYS_ACQ_SCENE_SYSTEM_LOST_WITH_LTE_OR_DO_CONN == enCurAcqScene)
     && ((VOS_TRUE                                              == ulHrpdConnExistFlag)
        || ((CNAS_CCB_VERSION_SRLTE                             == CNAS_CCB_IsVersionSrlte())
         && (VOS_TRUE                                           == ulLteConnExistFlag))))
    {
        usChanScanNum = CNAS_XSD_MAX_FREQ_NUM_WHEN_LTE_DO_CONN;
    }
    else
    {
        /* 最多一次取32个频点发给AS进行同步 */
    ...
    return usChanScanNum;
}

VOS_UINT16 CNAS_XSD_GetMaxSyncFreqNumOfGeoSearchList( VOS_VOID )
{
    VOS_UINT32                          ulHrpdConnExistFlag;
    VOS_UINT32                          ulLteConnExistFlag;
    CNAS_XSD_SCAN_CHANNEL_INFO_STRU    *pstScanChanInfo;
    VOS_UINT32                          ulIsExistSvcAcq;

    ulLteConnExistFlag      = CNAS_XSD_GetLteConnExistFlag();
    ulHrpdConnExistFlag     = CNAS_XSD_GetHrpdConnExistFlag();
    pstScanChanInfo         = CNAS_XSD_GetEmcCallRedialScanChanListAddr()->pstScanChanInfo;
    ulIsExistSvcAcq         = CNAS_XSD_GetSrvAcqFlg();

    if ((VOS_NULL_PTR       != CNAS_XSD_GetCallRedialScanChanListAddr()->pstScanChanInfo)
     || (VOS_NULL_PTR       != pstScanChanInfo)
     || (VOS_TRUE           == ulIsExistSvcAcq))
    {
        /* 如果有呼叫重播搜网，或者业务下发的搜网请求，下一轮geo搜索要一次下发最大频点个数 */
        return CNAS_CAS_1X_MAX_FREQ_NUM;
    }

    /* 如果没有呼叫重播搜网，查看是否为LTE\HRPD连接态，如果是，下一轮sync req一次携带一个频点；否则一次下发最大频点个数32 */
    if((VOS_TRUE                == ulHrpdConnExistFlag)
    || ((CNAS_CCB_VERSION_SRLTE == CNAS_CCB_IsVersionSrlte())
     && (VOS_TRUE               == ulLteConnExistFlag)))
    {
        return CNAS_XSD_MAX_FREQ_NUM_WHEN_LTE_DO_CONN;
    }
    else
    {
        return CNAS_CAS_1X_MAX_FREQ_NUM;
...
VOS_VOID CNAS_XSD_BuildScanChanList_ServiceAcqSearch(
    VOS_UINT16                          usPreSrchChanNum,
    CNAS_PRL_FREQENCY_CHANNEL_STRU     *pstPreSrchChanArray,
    CNAS_XSD_SCAN_CHANNEL_LIST_STRU    *pstScanChanList
)
