...
)
{
    ...
    NAS_MEM_CPY_S(pstCurFsm, sizeof(CNAS_XREG_FSM_CTX_STRU), pstNewFsm, sizeof(CNAS_XREG_FSM_CTX_STRU));
...
)
{
    ...
    NAS_MEM_CPY_S( pstEntryMsg->aucMsgBuffer, CNAS_XREG_MAX_MSG_BUFFER_LEN, pstMsg, ulLen );
...
)
{
    ...
    if (pstMsgQueue->ucCacheMsgNum >= CNAS_XREG_MAX_CACHE_MSG_QUEUE_NUM)
    {
        ...
        NAS_MEM_SET_S(pstMsgQueue, sizeof(CNAS_XREG_CACHE_MSG_QUEUE_STRU), VOS_NULL, sizeof(CNAS_XREG_CACHE_MSG_QUEUE_STRU));
    }
    ...
    NAS_MEM_CPY_S(pstMsgQueue->pastCacheMsg[ulOffset], ulMsgLen, pucMsg, ulMsgLen);
