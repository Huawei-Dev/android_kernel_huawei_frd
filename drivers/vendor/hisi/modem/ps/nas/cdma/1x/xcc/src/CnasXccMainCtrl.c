...
CNAS_XCC_MAIN_CTRL_PROC_STRU g_astCnasXccMainCtrlProcTab[] = {
    ...
    NAS_ACT_TBL_ITEM(MSP_PID_DIAG_APP_AGENT,
                     ...
                     CNAS_XCC_RcvAcpuOmErrLogCtrlInd_MainCtrl),
#endif

#ifdef __PS_WIN32_RECUR__
        NAS_ACT_TBL_ITEM(UEPS_PID_XCC,
                         ID_CNAS_MNTN_LOG_PC_REPLAY_FIXED_CONTEXT,
                         CNAS_XCC_RestorePcReplayFixedContext_MainCtrl),
#endif
...
)
{
    ...
    switch (pstFsmCtx->enFsmId)
    {
        ...
        case CNAS_XCC_FSM_MO_CALLING:
            ...
            if ((CNAS_XCC_MO_CALLING_STA_WAIT_LMM_ESR_END_IND == pstFsmCtx->ulState)
        ...
        case CNAS_XCC_FSM_MT_CALLING:
            ...
            if ((CNAS_XCC_MT_CALLING_STA_WAIT_LMM_ESR_END_IND == pstFsmCtx->ulState)
             && (CNAS_CCB_VERSION_SRLTE                       == CNAS_CCB_IsVersionSrlte())
             && (VOS_TRUE                                     == ulIsSyscfgSupportLte))
            {
                /* 此时链路未建立，不需要发送release request */
...
)
{
    ...
    NAS_ERR_LOG_1X_SERVICE_TYPE_ENUM_UINT32                 enErrlogServiceType;
#endif
    ...
    *pucAbortIndex = 0;
    ...
    {
#if (FEATURE_PTM == FEATURE_ON)
        CNAS_XCC_ProcSameServiceExistedCollisionErrLogInfo(*pucAbortIndex, pstMsg->enSo);
#endif
    ...
    {
        enErrlogServiceType = CNAS_XCC_ConvertSoTo1xErrLogServiceType(pstMsg->enSo);
        /* 支持并发业务，保存新的被叫信息 */
        NAS_ERRLOG_CCB_SavePagingServiceType(enErrlogServiceType);

        NAS_ERRLOG_CCB_SaveReceivedPagingTimer(enErrlogServiceType, VOS_GetSlice());
    }
#endif
...
)
{
    ...
    *pucAbortIndex = 0;
    ...
    if (VOS_TRUE == CNAS_XCC_GetSpecTypeInstance(CNAS_XCC_SERVICE_TYPE_VOICE_CALL, pucAbortIndex))
    {
        ...
        {
            return TAF_XCC_CAUSE_EXIST_VOICE_CALL;
        }
...
)
{
    ...
    CNAS_XCC_CALL_INSTANCE_LIST_STRU                        stIndexList = {0};

    NAS_MEM_SET_S(&stIndexList, sizeof(CNAS_XCC_CALL_INSTANCE_LIST_STRU), 0x00, sizeof(CNAS_XCC_CALL_INSTANCE_LIST_STRU));

    CNAS_XCC_GetAllActiveCallInstanceIndex(&stIndexList);

    pstXccCtxAddr = CNAS_XCC_GetCtxAddr();

    if (0 == stIndexList.ucInstanceNum)
    {
        return VOS_FALSE;
    }

    /* 如果当前状态机不在L1 MAIN状态或不在conversation状态，不要发送privacy mode */
    if ((CNAS_XCC_FSM_L1_MAIN         != pstXccCtxAddr->astFsmEntity[stIndexList.aucInstance[0]].stCurFsm.enFsmId)
     || (CNAS_XCC_L1_STA_CONVERSATION != pstXccCtxAddr->astFsmEntity[stIndexList.aucInstance[0]].stCurFsm.ulState))
    {
        return VOS_FALSE;
    }

    /* 根据CAS约定，只有在SO协商完成后，才可发送 */
    if (VOS_FALSE == pstXccCtxAddr->astFsmEntity[stIndexList.aucInstance[0]].stCallInfoCtx.ucIsSoNegCmplt)
    {
        return VOS_FALSE;
    }

    /* 如果当前的用户请求的privacy mode等于协商后的privacy mode, 不需要发送 */
    if (enPrivacyMode == pstXccCtxAddr->astFsmEntity[stIndexList.aucInstance[0]].stCallInfoCtx.enNegPrivacyMode)
    {
        return VOS_FALSE;
    }
...
)
{
    ...
    CNAS_XCC_LogPcReplayFixedContext();
#endif
...
{
    CNAS_XCC_CALL_INSTANCE_LIST_STRU    stCallInstanceList = {0};
    CNAS_XCC_CTX_STRU                  *pstXccCtxAddr   = VOS_NULL_PTR;
    VOS_UINT8                           i;
    VOS_UINT8                           ucIndex;
...
)
{
    ...
    /* check是否处于Call Back模式 */
    if (VOS_TRUE == CNAS_CCB_GetEmcCbState())
    {
        /* 如果有关机消息缓存，通知APS呼叫失败 */
        CNAS_XCC_SndApsOrigDataCallCnf(pstDataCallReq->ucCallId,
                                       TAF_XCC_FAIL_LAYER_L3,
                                       TAF_XCC_CAUSE_EMC_CALL_BACK_MODE,
                                       CNAS_XCC_INVALID_CON_REF,
                                       CNAS_XCC_INVAILD_CON_ID,
                                       0);
...
)
{
    ...
    CAS_CNAS_1X_PAGING_IND_STRU                            *pst1xPagingInd = VOS_NULL_PTR;
#if (FEATURE_ON == FEATURE_PTM)
    NAS_ERR_LOG_1X_SERVICE_TYPE_ENUM_UINT32                 enServiceType;
#endif

    pst1xPagingInd = (CAS_CNAS_1X_PAGING_IND_STRU *)pstMsg;
    ...
    ucAbortIndex = 0;
    ucAbortFlag  = VOS_FALSE;
    if (VOS_FALSE == CNAS_XCC_IsPaginMsgAllowed(pst1xPagingInd, &ucAbortIndex, &ucAbortFlag))
    {
        return VOS_TRUE;
    }
#if (FEATURE_PTM == FEATURE_ON)
    ...
    {
        /* 保存新的被叫信息 */
        enServiceType = CNAS_XCC_ConvertSoTo1xErrLogServiceType(pst1xPagingInd->enSo);
        
        NAS_ERRLOG_CCB_SavePagingServiceType(enServiceType);

        NAS_ERRLOG_CCB_SaveReceivedPagingTimer(enServiceType, VOS_GetSlice());
    }
#endif
...
)
{
    ...
    else if (CNAS_XCC_SERVICE_TYPE_DATA_CALL == enPagingService)
    {
        ...
        {
#if (FEATURE_PTM == FEATURE_ON)
            /* 主叫由于记录了呼叫标记，在下面的逻辑中会上报呼叫异常，这里只处理被叫*/
            if (VOS_FALSE == CNAS_XCC_IsCurServiceMo(*pucAbortIndex))
            {
                 CNAS_XCC_ProcVoiceServiceExistedCollisionErrLogInfo(*pucAbortIndex, NAS_ERR_LOG_1X_MT_SERVICE_TYPE_DATA_CALL);
            }          
#endif
        ...
        {
#if (FEATURE_PTM == FEATURE_ON)
            CNAS_XCC_ProcCsmsServiceExistedCollisionErrLogInfo(*pucAbortIndex, NAS_ERR_LOG_1X_MT_SERVICE_TYPE_DATA_CALL);
#endif
        ...
        enModemId = CNAS_CCB_GetCdmaModeModemId();
        ...
        {
            ucCsCallExistFlg = I1_TAF_SDC_GetCsCallExistFlg();
        }
#if (3 == MULTI_MODEM_NUMBER)
        else if (MODEM_ID_2 == enModemId)
        {
            ucCsCallExistFlg = I2_TAF_SDC_GetCsCallExistFlg();
        }
#endif
#endif
        else
        {
            CNAS_WARNING_LOG(UEPS_PID_XCC, "CNAS_XCC_ProcessPagingCollision invalid modemId!");
        }

        CNAS_NORMAL_LOG2(UEPS_PID_XCC, "CNAS_XCC_ProcessPagingCollision Cur Modem Id:", (VOS_UINT32)enModemId, (VOS_UINT32)ucCsCallExistFlg);

        /* 有语音重拨存在丢弃数据被叫 */
        if (VOS_TRUE == ucCsCallExistFlg)
        {
#if (FEATURE_PTM == FEATURE_ON)
            NAS_ERRLOG_CCB_SavePagingServiceType(NAS_ERR_LOG_1X_MT_SERVICE_TYPE_DATA_CALL);

            NAS_ERRLOG_CCB_SaveReceivedPagingTimer(NAS_ERR_LOG_1X_MT_SERVICE_TYPE_DATA_CALL, VOS_GetSlice());

            NAS_ERRLOG_CCB_SaveMtServiceExceptionCause(NAS_ERR_LOG_1X_MT_SERVICE_TYPE_DATA_CALL, 
                                                       NAS_ERR_LOG_1X_MT_SERVICE_EXCEPTION_CAUSE_EXIST_MO_VOICE);

            CNAS_XCC_Report1xMtServiceExceptionErrLogInfo(NAS_ERR_LOG_1X_MT_SERVICE_TYPE_DATA_CALL);

            NAS_ERRLOG_CCB_Init1xMtServiceExceptionRecord(NAS_ERR_LOG_1X_MT_SERVICE_TYPE_DATA_CALL);
#endif
            ucRslt = VOS_FALSE;
        }
        ...
        {
#if (FEATURE_PTM == FEATURE_ON)
            CNAS_XCC_ProcAgpsServiceExistedCollisionErrLogInfo(*pucAbortIndex, NAS_ERR_LOG_1X_MT_SERVICE_TYPE_DATA_CALL);
#endif
    ...
    else if (CNAS_XCC_SERVICE_TYPE_SMS == enPagingService)
    {
        ...
        {
#if (FEATURE_PTM == FEATURE_ON)
            CNAS_XCC_ProcVoiceServiceExistedCollisionErrLogInfo(*pucAbortIndex, NAS_ERR_LOG_1X_MT_SERVICE_TYPE_SMS);
#endif
        ...
        {
#if (FEATURE_PTM == FEATURE_ON)
            CNAS_XCC_ProcCdataServiceExistedCollisionErrLogInfo(*pucAbortIndex, NAS_ERR_LOG_1X_MT_SERVICE_TYPE_SMS);
#endif
        ...
        {
#if (FEATURE_PTM == FEATURE_ON)
            CNAS_XCC_ProcAgpsServiceExistedCollisionErrLogInfo(*pucAbortIndex, NAS_ERR_LOG_1X_MT_SERVICE_TYPE_SMS);
#endif
    ...
    else if (CNAS_XCC_SERVICE_TYPE_AGPS_CALL == enPagingService)
    {
        ...
        {
#if (FEATURE_PTM == FEATURE_ON)
            CNAS_XCC_ProcVoiceServiceExistedCollisionErrLogInfo(*pucAbortIndex, NAS_ERR_LOG_1X_MT_SERVICE_TYPE_AGPS_CALL);
#endif
            ucRslt = VOS_FALSE;
        }
        ...
        {
#if (FEATURE_PTM == FEATURE_ON)
            CNAS_XCC_ProcCdataServiceExistedCollisionErrLogInfo(*pucAbortIndex, NAS_ERR_LOG_1X_MT_SERVICE_TYPE_AGPS_CALL);
#endif
            ucRslt = VOS_FALSE;
        }
        ...
        {
#if (FEATURE_PTM == FEATURE_ON)
            CNAS_XCC_ProcCsmsServiceExistedCollisionErrLogInfo(*pucAbortIndex, NAS_ERR_LOG_1X_MT_SERVICE_TYPE_AGPS_CALL);
#endif
            ucRslt = VOS_FALSE;
        }
    }
...
#if (FEATURE_ON == FEATURE_PTM)
    if ((CAS_CNAS_1X_TERMINATE_REASON_REDIRECTION   != pstCallTerminateInd->enTerminateReason)
     && (CAS_CNAS_1X_TERMINATE_REASON_NW_NORMAL_END != pstCallTerminateInd->enTerminateReason))
    {
        enXccCause = CNAS_XCC_ConvertTerminateReasonToTafCause(pstCallTerminateInd->enTerminateReason);

        CNAS_XCC_ProcMtServiceTerminateIndExceptionErrLogInd(CNAS_XCC_MapXccCauseToErrLogCause(enXccCause), pstCallTerminateInd); 
    }     
#endif
...
VOS_UINT32 CNAS_XCC_RcvAcpuOmErrLogCtrlInd_MainCtrl(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    ...
    {
        NAS_ERRLOG_CCB_SetXccMainActiveReportFlag(pstRcvMsg->ucAlmStatus);
    }
    ...
    {
        NAS_ERRLOG_CCB_SetXccMainActiveReportLevel(pstRcvMsg->ucAlmLevel);
    }
...
VOS_VOID CNAS_XCC_ProcMtServiceTerminateIndExceptionErrLogInd(
    NAS_ERR_LOG_1X_MT_SERVICE_EXCEPTION_CAUSE_ENUM_UINT32   enErrLogCallCause,
    CAS_CNAS_1X_CALL_TERMINATE_IND_STRU                    *pst1xCallTerminateInd
)
{
    CNAS_XCC_CALL_INSTANCE_LIST_STRU                        stCallInstanceList;
    NAS_ERR_LOG_1X_SERVICE_TYPE_ENUM_UINT32                 enServiceType;
    CNAS_XCC_CTX_STRU                                      *pstXccCtxAddr   = VOS_NULL_PTR;
    CNAS_XCC_FSM_CTX_STRU                                  *pstFsmCtx       = VOS_NULL_PTR;
    VOS_UINT8                                               i;
    VOS_UINT8                                               ucIndex;
    ...
    if (PS_TRUE == pst1xCallTerminateInd->enTerminateAll)
    {
        for (i = 0; i < CNAS_MIN(stCallInstanceList.ucInstanceNum, CNAS_XCC_MAX_FSM_ENTITY_NUM); i++)
        {
            ...
            pstFsmCtx = &(pstXccCtxAddr->astFsmEntity[ucIndex].stCurFsm);

            /*依次释放处于被叫的状态机状态下的被叫 */
            if (CNAS_XCC_FSM_MT_CALLING == pstFsmCtx->enFsmId)
            {
                enServiceType = CNAS_XCC_ConvertSoTo1xErrLogServiceType(pstXccCtxAddr->astFsmEntity[ucIndex].stCallInfoCtx.enCurrentSo);           

                NAS_ERRLOG_CCB_SaveMtServiceExceptionCause(enServiceType, enErrLogCallCause);

                NAS_ERRLOG_CCB_SaveMtServiceExceptionXccState(enServiceType, pstFsmCtx->ulState);

                CNAS_XCC_Report1xMtServiceExceptionErrLogInfo(enServiceType);

                NAS_ERRLOG_CCB_Init1xMtServiceExceptionRecord(enServiceType);
            }
    ...
    {
        if (VOS_TRUE == CNAS_XCC_GetCallInstanceIndexByConnId(pst1xCallTerminateInd->ucConnectId, &ucIndex))
        {
            pstFsmCtx = &(pstXccCtxAddr->astFsmEntity[ucIndex].stCurFsm);
...
VOS_VOID CNAS_XCC_LogPcReplayFixedContext(VOS_VOID)
{
    CNAS_XCC_CTX_STRU                                      *pstCnasXccCtx = VOS_NULL_PTR;
    CNAS_XCC_PC_REPLAY_FIXED_CONTEXT_MSG_STRU               stFixedCtxData;

    pstCnasXccCtx      = CNAS_XCC_GetCtxAddr();

    NAS_MEM_SET_S(&stFixedCtxData, sizeof(stFixedCtxData), 0, sizeof(stFixedCtxData));

    NAS_MEM_CPY_S(&(stFixedCtxData.stCallNvimCfg), sizeof(CNAS_XCC_CALL_NVIM_CFG_STRU), &(pstCnasXccCtx->stCallNvimCfg), sizeof(CNAS_XCC_CALL_NVIM_CFG_STRU));
    NAS_MEM_CPY_S(&(stFixedCtxData.stPagingRspSoCfg), sizeof(CNAS_XCC_1X_PAGING_RSP_SO_CFG_STRU), &(pstCnasXccCtx->stPagingRspSoCfg), sizeof(CNAS_XCC_1X_PAGING_RSP_SO_CFG_STRU));

    CNAS_MNTN_LogPcReplayFixedContext(sizeof(CNAS_XCC_PC_REPLAY_FIXED_CONTEXT_MSG_STRU), UEPS_PID_XCC, (VOS_UINT8 *)&stFixedCtxData);
}

#ifdef __PS_WIN32_RECUR__

VOS_UINT32 CNAS_XCC_RestorePcReplayFixedContext_MainCtrl(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    CNAS_MNTN_LOG_PC_REPLAY_CONTEXT_STRU                   *pstRcvMsg      = VOS_NULL_PTR;
    CNAS_XCC_PC_REPLAY_FIXED_CONTEXT_MSG_STRU                        *pstFixedCtxMsg = VOS_NULL_PTR;
    CNAS_XCC_CTX_STRU                                      *pstCnasXccCtx  = VOS_NULL_PTR;

    pstCnasXccCtx   = CNAS_XCC_GetCtxAddr();
    pstRcvMsg       = (CNAS_MNTN_LOG_PC_REPLAY_CONTEXT_STRU*)pstMsg;
    pstFixedCtxMsg  = (CNAS_XCC_PC_REPLAY_FIXED_CONTEXT_MSG_STRU*)pstRcvMsg->aucDataCtx;

    NAS_MEM_CPY_S(&(pstCnasXccCtx->stCallNvimCfg), sizeof(CNAS_XCC_CALL_NVIM_CFG_STRU), &(pstFixedCtxMsg->stCallNvimCfg), sizeof(CNAS_XCC_CALL_NVIM_CFG_STRU));
    NAS_MEM_CPY_S(&(pstCnasXccCtx->stPagingRspSoCfg), sizeof(CNAS_XCC_1X_PAGING_RSP_SO_CFG_STRU), &(pstFixedCtxMsg->stPagingRspSoCfg), sizeof(CNAS_XCC_1X_PAGING_RSP_SO_CFG_STRU));
