...
/* CNAS XSD PC 回放CTX */
CNAS_XSD_REPLAY_CTX_STRU                g_stCnasXsdReplayCtx;
#endif
...
{
    CNAS_XSD_AVOID_FREQ_LIST_STRU      *pstAvoidList = VOS_NULL_PTR;

    pstAvoidList = CNAS_XSD_GetAvoidFreqListAddr();

    pstAvoidList->ucNextAcqIndex = 0;
...
)
{
    ...
    NAS_MEM_SET_S(&(pstSysAcqCtrl->stMru0StrategyCfg), sizeof(pstSysAcqCtrl->stMru0StrategyCfg), 0, sizeof(CNAS_XSD_MRU0_SWITCH_ON_OOC_STRATEGY_CFG_STRU));
    ...
    CNAS_XSD_SetSystemAcquiredAbortedCause(NAS_ERR_LOG_1X_SEARCH_RSLT_BUTT);
    ...
    CNAS_XSD_SetHrpdConnExistFlag(VOS_FALSE);
    CNAS_XSD_SetLteConnExistFlag(VOS_FALSE);
    ...
    CNAS_XSD_SetLteOrDoConn1xSysAcqSyncDelayTimerLen(TI_CNAS_XSD_SYS_ACQ_SYNC_DELAY_TIMER_LEN);
    CNAS_XSD_SetLteOrDoConn1xSysAcqSyncDelayFlag(VOS_FALSE);
    CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.ulSrlteSyncOneChanAddTimerLen = TI_CNAS_XSD_SRLTE_CAS_SYNC_ONE_CHAN_ADD_TIMER_LEN;
}
...
)
{
    ...
    /* 异常场景处理 */
    if (ucNextSrchIndex < pstAvoidSysList->ucNextAcqIndex)
    {
        pstAvoidSysList->ucNextAcqIndex = (VOS_UINT8)CNAS_MIN((pstAvoidSysList->ucNextAcqIndex + 1), CNAS_XSD_MAX_AVOID_FREQ_NUM);

        return;
    }

    pstAvoidSysList->ucNextAcqIndex = (VOS_UINT8)CNAS_MIN(ucNextSrchIndex, CNAS_XSD_MAX_AVOID_FREQ_NUM);
...
{
    CNAS_XSD_Log1xAvoidSysAcqState(ucFlg);

    /* 如果当前状态机不是CNAS_XSD_FSM_SYS_ACQ */
    if (CNAS_XSD_FSM_SYS_ACQ != CNAS_XSD_GetCurrFsmId())
    {
        /* 异常事件 */
        CNAS_WARNING_LOG(UEPS_PID_XSD, "CNAS_XSD_SetAvoidSysAcqListFlg_SysAcq: Wrong state ENTERED");

        return;
    }

    g_stCnasXsdCtx.stCurFsm.unFsmCtx.stSysAcqCtx.ucIsAvoidSysAcqList = ucFlg;
...
VOS_UINT8 CNAS_XSD_GetAvoidSysAcqListFlg_SysAcq(VOS_VOID)
{
    ...
    {
        /* 异常事件 */
        CNAS_WARNING_LOG(UEPS_PID_XSD, "CNAS_XSD_GetAvoidSysAcqListFlg_SysAcq: Wrong state ENTERED");

        return VOS_FALSE;
    }

    CNAS_XSD_Log1xAvoidSysAcqState(g_stCnasXsdCtx.stCurFsm.unFsmCtx.stSysAcqCtx.ucIsAvoidSysAcqList);

    return g_stCnasXsdCtx.stCurFsm.unFsmCtx.stSysAcqCtx.ucIsAvoidSysAcqList;
}
...
)
{
    ...
    {
        /* 异常事件 */
        CNAS_WARNING_LOG(UEPS_PID_XSD, "CNAS_XSD_SetOocTotalTimerExpiredFlg_SysAcq: Wrong state ENTERED");

        return;
    }

    g_stCnasXsdCtx.stCurFsm.unFsmCtx.stSysAcqCtx.ucOocTotalTimerExpiredFlg = ucFlg;
...
VOS_UINT8 CNAS_XSD_GetOocTotalTimerExpiredFlg_SysAcq(VOS_VOID)
{
    ...
    {
        /* 异常事件 */
        CNAS_WARNING_LOG(UEPS_PID_XSD, "CNAS_XSD_GetOocTotalTimerExpiredFlg_SysAcq: Wrong state ENTERED");
    }

    return g_stCnasXsdCtx.stCurFsm.unFsmCtx.stSysAcqCtx.ucOocTotalTimerExpiredFlg;
}
...
)
{
    ...
    CNAS_XSD_InitOosSysAcqStrategyCfgInfo(&pstOocScheduleInfo->stStrategyCfg);
}
...
{
    NAS_MEM_SET_S(pstSysAcqStrategyCfg, sizeof(CNAS_XSD_OOS_SYS_ACQ_STRTEGY_CFG_STRU), 0x0, sizeof(CNAS_XSD_OOS_SYS_ACQ_STRTEGY_CFG_STRU));

    pstSysAcqStrategyCfg->usMru0SearchTimerLen          = CNAS_XSD_DEFAULT_OOS_SYS_ACQ_MRUO_SEARCH_SLEEP_TIME_LEN;
    pstSysAcqStrategyCfg->enOocScanStrategy             = CNAS_XSD_OOC_SCAN_REACQ_0_1_2_3_4S;
    pstSysAcqStrategyCfg->uc1xOocDoTchPhase1TimerLen    = CNAS_XSD_DEFAULT_OOS_SYS_ACQ_DO_TCH_PHASE_ONE_SLEEP_TIME_LEN;
    pstSysAcqStrategyCfg->uc1xOocDoTchPhase2TimerLen    = CNAS_XSD_DEFAULT_OOS_SYS_ACQ_DO_TCH_PHASE_TWO_SLEEP_TIME_LEN;

    pstSysAcqStrategyCfg->ucPhaseNum = CNAS_XSD_DEFAULT_OOS_SYS_ACQ_TOTAL_PHASE_NUM;

    pstSysAcqStrategyCfg->astPhaseCfgInfo[0].usTotalTimeLen = 0;
    pstSysAcqStrategyCfg->astPhaseCfgInfo[0].usSleepTimeLen = CNAS_XSD_DEFAULT_OOS_SYS_ACQ_PHASE_ONE_SLEEP_TIME_LEN;
    pstSysAcqStrategyCfg->astPhaseCfgInfo[0].ucSrchNum      = CNAS_XSD_DEFAULT_OOS_SYS_ACQ_PHASE_ONE_MAX_NUM_ONE_CYCLE;

    pstSysAcqStrategyCfg->astPhaseCfgInfo[1].usTotalTimeLen = 0;
    pstSysAcqStrategyCfg->astPhaseCfgInfo[1].usSleepTimeLen = CNAS_XSD_DEFAULT_OOS_SYS_ACQ_PHASE_TWO_SLEEP_TIME_LEN;
    pstSysAcqStrategyCfg->astPhaseCfgInfo[1].ucSrchNum      = CNAS_XSD_DEFAULT_OOS_SYS_ACQ_PHASE_TWO_MAX_NUM_ONE_CYCLE;

    pstSysAcqStrategyCfg->astPhaseCfgInfo[2].usTotalTimeLen = 0;
    pstSysAcqStrategyCfg->astPhaseCfgInfo[2].usSleepTimeLen = CNAS_XSD_DEFAULT_OOS_SYS_ACQ_PHASE_THREE_SLEEP_TIME_LEN;
    pstSysAcqStrategyCfg->astPhaseCfgInfo[2].ucSrchNum      = CNAS_XSD_DEFAULT_OOS_SYS_ACQ_PHASE_THREE_MAX_NUM_ONE_CYCLE;

    pstSysAcqStrategyCfg->astPhaseCfgInfo[3].usTotalTimeLen = 0;
    pstSysAcqStrategyCfg->astPhaseCfgInfo[3].usSleepTimeLen = CNAS_XSD_DEFAULT_OOS_SYS_ACQ_PHASE_FOUR_SLEEP_TIME_LEN;
    pstSysAcqStrategyCfg->astPhaseCfgInfo[3].ucSrchNum      = CNAS_XSD_DEFAULT_OOS_SYS_ACQ_PHASE_FOUR_MAX_NUM_ONE_CYCLE;

    pstSysAcqStrategyCfg->astPhaseCfgInfo[4].usTotalTimeLen = 0;
    pstSysAcqStrategyCfg->astPhaseCfgInfo[4].usSleepTimeLen = CNAS_XSD_DEFAULT_OOS_SYS_ACQ_PHASE_FIVE_SLEEP_TIME_LEN;
    pstSysAcqStrategyCfg->astPhaseCfgInfo[4].ucSrchNum      = CNAS_XSD_DEFAULT_OOS_SYS_ACQ_PHASE_FIVE_MAX_NUM_ONE_CYCLE;

    pstSysAcqStrategyCfg->astPhaseCfgInfo[5].usTotalTimeLen = 0;
    pstSysAcqStrategyCfg->astPhaseCfgInfo[5].usSleepTimeLen = CNAS_XSD_DEFAULT_OOS_SYS_ACQ_PHASE_SIX_SLEEP_TIME_LEN;
    pstSysAcqStrategyCfg->astPhaseCfgInfo[5].ucSrchNum      = CNAS_XSD_DEFAULT_OOS_SYS_ACQ_PHASE_SIX_MAX_NUM_ONE_CYCLE;
}
...
{
    return &(CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.stOocScheduleInfo.stStrategyCfg);
}
...
{
    return CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.ucIsConsiderRoamIndInPRL;
}
...
{
    CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.ucIsConsiderRoamIndInPRL  = ucIsConsiderRoamIndInPRLFlg;
...
{
    NAS_MEM_SET_S(CNAS_XSD_GetEmcCallBackCandidateFreqInfo(),
                  sizeof(CNAS_XSD_EMC_CALLBACK_CANDIDATE_FREQ_INFO_STRU),
                  0x00,
                  sizeof(CNAS_XSD_EMC_CALLBACK_CANDIDATE_FREQ_INFO_STRU));
...
)
{
    ...
    NAS_MEM_SET_S(&stNvimCdma1XCustomPrefChan,
    ...
    if (NV_OK != NV_Read(en_NV_Item_CDMA_1X_CUSTOMIZE_PREF_CHANNELS_INFO,
                         ...
                         sizeof(CNAS_NVIM_CDMA_1X_CUSTOMIZE_PREF_CHANNELS_STRU)))
    {
        CNAS_ERROR_LOG(UEPS_PID_XSD, "CNAS_XSD_IsCurFreqInCdma1XCustomPrefChannels: Read Nvim Failed");

        return VOS_TRUE;
    }

    stNvimCdma1XCustomPrefChan.usFreqNum = (VOS_UINT16)CNAS_MIN(stNvimCdma1XCustomPrefChan.usFreqNum, CNAS_XSD_MAX_CDMA_1X_CUSTOM_PREF_CHANNELS_NUM);

    for (i = 0; i < stNvimCdma1XCustomPrefChan.usFreqNum; i++)
    {
        if ((stNvimCdma1XCustomPrefChan.astFreqList[i].usBandClass == (VOS_UINT16)pstFreq->enBandClass)
         && (stNvimCdma1XCustomPrefChan.astFreqList[i].usChannel == pstFreq->usChannel))
        {
            return VOS_TRUE;
        }
    }
...
{
    CNAS_XSD_CDMA_1X_CUSTOM_PREF_CHANNELS_STRU             *pstFreqList = VOS_NULL_PTR;

    pstFreqList = CNAS_XSD_GetCdma1XCustomPrefChannels();

    if (CNAS_XSD_MAX_CDMA_1X_CUSTOM_PREF_CHANNELS_NUM <= pstFreqList->usFreqNum)
    {
        CNAS_ERROR_LOG(UEPS_PID_XSD, "CNAS_XSD_AddCurFreqToCdma1XCustomPrefChannels: FULL!");

        return VOS_FALSE;
    }

    pstFreqList->astFreqList[pstFreqList->usFreqNum].enBandClass = pstFreq->enBandClass;
    pstFreqList->astFreqList[pstFreqList->usFreqNum].usChannel   = pstFreq->usChannel;
    pstFreqList->usFreqNum++;
...
{
    return CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.stMru0StrategyCfg.ucEnable;
}


VOS_VOID CNAS_XSD_SetSwitchOnOocMru0EnableFlg(VOS_UINT8 ucEnable)
{
    CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.stMru0StrategyCfg.ucEnable = ucEnable;
}


VOS_UINT16 CNAS_XSD_GetSwitchOnOocMru0Times(VOS_VOID)
{
    return CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.stMru0StrategyCfg.usMru0Times;
}


VOS_VOID CNAS_XSD_SetSwitchOnOocMru0Times(VOS_UINT16 usTimes)
{
    CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.stMru0StrategyCfg.usMru0Times = usTimes;
}
...
{
    CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.enOosSpecialSdReason = enOosSpecialSdReason;
}


NAS_MSCC_PIF_1X_OOS_CAUSE_ENUM_UINT32 CNAS_XSD_GetOosSpecialSdReason(VOS_VOID)
{
    return CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.enOosSpecialSdReason;
}
...
{
    CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.enAbortedCause = enAbortedCause;
}


NAS_ERR_LOG_1X_SEARCH_RSLT_ENUM_UINT32 CNAS_XSD_GetSystemAcquiredAbortedCause(VOS_VOID)
{
    return CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.enAbortedCause;
}
...
{
    CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.stLteHrpdConnStatus.ucHrpdConnExistFlg = ucHrpdConnExistFlag;
    return;
}
...
{
    return CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.stLteHrpdConnStatus.ucHrpdConnExistFlg;
}
...
{
    CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.stLteHrpdConnStatus.ucLteEpsConnExistFlg = ucLteConnExistFlag;
    return;
}
...
{
    return CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.stLteHrpdConnStatus.ucLteEpsConnExistFlg;
}
...
{
    return CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.stSysAcqNoRfProtectTimerInfo.ul1xSysAcqNoRfProtectTimerLen;
}
...
{
    CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.stSysAcqNoRfProtectTimerInfo.ul1xSysAcqNoRfProtectTimerLen = ul1xSysAcqNoRfProtectTimerLen*60*1000;
    return;
}
...
{
    CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.stSysAcqNoRfProtectTimerInfo.ul1xRedirNoRfProtectTimerLen = ul1xRedirNoRfProtectTimerLen*60*1000;
    return;
}

VOS_UINT32 CNAS_XSD_GetRedirNoRfProtectTimerLen(VOS_VOID)
{
    return CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.stSysAcqNoRfProtectTimerInfo.ul1xRedirNoRfProtectTimerLen;
}


VOS_UINT32 CNAS_XSD_GetLteOrDoConn1xSysAcqSyncDelayTimerLen(VOS_VOID)
{
    return CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.st1xSysAcqSyncDelayInfo.ul1xSysAcqDelayTimerLen;
}
...
{
    CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.st1xSysAcqSyncDelayInfo.ul1xSysAcqDelayTimerLen = ul1xSysAcqDelayTimerLen;
    return;
}

VOS_UINT8 CNAS_XSD_GetLteOrDoConn1xSysAcqSyncDelayFlag(VOS_VOID)
{
    return CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.st1xSysAcqSyncDelayInfo.uc1xSysAcqSyncDelayEnable;
}
...
{
    CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.st1xSysAcqSyncDelayInfo.uc1xSysAcqSyncDelayEnable = uc1xSysAcqDelayFlag;
    return;
}
...
{
    VOS_UINT16                          usDelaySyncFreqNum;

    NAS_MEM_SET_S( CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.st1xSysAcqSyncDelayInfo.austSysAcqSyncDelayFreqInfo,
                   sizeof(CNAS_PRL_FREQENCY_CHANNEL_STRU)*CNAS_XSD_MAX_FREQ_NUM_WHEN_LTE_DO_CONN,
                   0,
                   sizeof(CNAS_PRL_FREQENCY_CHANNEL_STRU)*CNAS_XSD_MAX_FREQ_NUM_WHEN_LTE_DO_CONN);

    if (0 == usFreqNum)
    {
        CNAS_WARNING_LOG(UEPS_PID_XSD, "CNAS_XSD_SaveLteOrDoConn1xSysAcqSyncDelayFreq: save sync delay freq num is 0");
        return;
    }
    usDelaySyncFreqNum = CNAS_MIN(usFreqNum, CNAS_XSD_MAX_FREQ_NUM_WHEN_LTE_DO_CONN);

    CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.st1xSysAcqSyncDelayInfo.us1xSysAcqSyncDelayFreqNum = usDelaySyncFreqNum;

    NAS_MEM_CPY_S( CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.st1xSysAcqSyncDelayInfo.austSysAcqSyncDelayFreqInfo,
                   sizeof(CNAS_PRL_FREQENCY_CHANNEL_STRU)*CNAS_XSD_MAX_FREQ_NUM_WHEN_LTE_DO_CONN,
                   pstNextSysAcqFreqInfo,
                   sizeof(CNAS_PRL_FREQENCY_CHANNEL_STRU)*usDelaySyncFreqNum);
   return;
}

CNAS_XSD_1X_SYS_ACQ_SYNC_DELAY_INFO_WHEN_LTE_OR_DO_CONN_STRU* CNAS_XSD_GetLteOrDoConn1xSysAcqSyncDelayInfoAddr(VOS_VOID)
{
    return &(CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.st1xSysAcqSyncDelayInfo);
}
...
{
    return &(g_stCnasXsdReplayCtx);
}


CNAS_XSD_REPLAY_CACHE_MSG_QUEUE_STRU* CNAS_XSD_ReplayGetCacheMsgQueueAddr(VOS_VOID)
{
    return &(CNAS_XSD_ReplayGetCtxAddr()->stCacheMsg);
}

VOS_VOID CNAS_XSD_ReplayInitCacheMsgQueue(VOS_VOID)
{
    CNAS_XSD_REPLAY_CACHE_MSG_QUEUE_STRU                   *pstReplayCacheMsgQueue = VOS_NULL_PTR;
    VOS_UINT32                                              i;

    pstReplayCacheMsgQueue = CNAS_XSD_ReplayGetCacheMsgQueueAddr();

    for (i = 0; i < CNAS_XSD_REPLAY_MAX_CACHE_MSG_QUEUE_NUM; i++)
    {
        if (VOS_NULL_PTR != pstReplayCacheMsgQueue->pastCacheMsg[i])
        {
            PS_MEM_FREE(UEPS_PID_XSD, pstReplayCacheMsgQueue->pastCacheMsg[i]);
            pstReplayCacheMsgQueue->pastCacheMsg[i] = VOS_NULL_PTR;
        }
    }

    pstReplayCacheMsgQueue->ucCacheMsgNum = 0;
}
...
VOS_VOID CNAS_XSD_ReplayInitCtx(VOS_VOID)
{
    ...
    pstReplayCtx->ucPreCfgParaRcvd    = VOS_FALSE;
    pstReplayCtx->usPrlAcqRecRcvdNum  = 0;
    pstReplayCtx->usPrlSysRecRcvdNum  = 0;

    NAS_MEM_SET_S(pstReplayCtx->aucReserve, sizeof(pstReplayCtx->aucReserve), 0, sizeof(pstReplayCtx->aucReserve));

    /* 初始化缓存队列 */
    CNAS_XSD_ReplayInitCacheMsgQueue();
}
...
{
    CNAS_XSD_REPLAY_CACHE_MSG_QUEUE_STRU                   *pstRepalyMsgQueue   = VOS_NULL_PTR;
    MSG_HEADER_STRU                                        *pstMsgHeader  = VOS_NULL_PTR;

    pstMsgHeader = (MSG_HEADER_STRU*)pstMsg;

    pstRepalyMsgQueue  = CNAS_XSD_ReplayGetCacheMsgQueueAddr();

    if (CNAS_XSD_REPLAY_MAX_CACHE_MSG_QUEUE_NUM <= pstRepalyMsgQueue->ucCacheMsgNum)
    {
        CNAS_ERROR_LOG(UEPS_PID_XSD, "CNAS_XSD_RepalySaveCacheMsgInMsgQueue:No Empty buffer");

        return;
    }

    pstRepalyMsgQueue->pastCacheMsg[pstRepalyMsgQueue->ucCacheMsgNum]
            = (VOS_UINT8 *)PS_MEM_ALLOC(UEPS_PID_XSD, pstMsgHeader->ulLength + VOS_MSG_HEAD_LENGTH);

    if (VOS_NULL_PTR == pstRepalyMsgQueue->pastCacheMsg[pstRepalyMsgQueue->ucCacheMsgNum])
    {
        return;
    }

    NAS_MEM_CPY_S(pstRepalyMsgQueue->pastCacheMsg[pstRepalyMsgQueue->ucCacheMsgNum],
                  pstMsgHeader->ulLength + VOS_MSG_HEAD_LENGTH,
                  pstMsgHeader,
                  pstMsgHeader->ulLength + VOS_MSG_HEAD_LENGTH);

    pstRepalyMsgQueue->ucCacheMsgNum++;
}
...
{
    CNAS_XSD_REPLAY_CACHE_MSG_QUEUE_STRU       *pstMsgQueue = VOS_NULL_PTR;

    if (CNAS_XSD_REPLAY_MAX_CACHE_MSG_QUEUE_NUM <= ucIndex)
    {
        return;
    }

    pstMsgQueue = CNAS_XSD_ReplayGetCacheMsgQueueAddr();
    ...
    if (0 < pstMsgQueue->ucCacheMsgNum)
    {
        ...
        if (ucIndex < (CNAS_XSD_REPLAY_MAX_CACHE_MSG_QUEUE_NUM - 1))
        {
            NAS_MEM_MOVE_S(&(pstMsgQueue->pastCacheMsg[ucIndex]),
                            (pstMsgQueue->ucCacheMsgNum - ucIndex) * sizeof(VOS_UINT8 *),
                           &(pstMsgQueue->pastCacheMsg[ucIndex + 1]),
...
{
    CNAS_XSD_REPLAY_CACHE_MSG_QUEUE_STRU      *pstReplayMsgQueue = VOS_NULL_PTR;
    MSG_HEADER_STRU                    *pstCacheMsgHdr = VOS_NULL_PTR;
    REL_TIMER_MSG                      *pstTimerMsg;

    pstReplayMsgQueue = CNAS_XSD_ReplayGetCacheMsgQueueAddr();

    if ( 0 == pstReplayMsgQueue->ucCacheMsgNum )
    {
        return VOS_FALSE;
    }

    /* 按照先入先出原则,默认取第1个缓存内容 */
    pstCacheMsgHdr = (MSG_HEADER_STRU *)(pstReplayMsgQueue->pastCacheMsg[0]);
    ...
    /* 清除index为0的cache消息 */
    CNAS_XSD_ReplayClearCacheMsg(0);
