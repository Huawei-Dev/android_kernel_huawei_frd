...
)
{
    ...
    else if ( CNAS_XCC_MO_CALLING_RESULT_FAIL_CALL_ENDED == pstMoCallingRsltCnf->enRslt )
    {
        ...
        if (VOS_TRUE == CNAS_XCC_GetAlreadyNtfLmmCallStartFlag())
        {
            /* 清除标记*/
            CNAS_XCC_SetAlreadyNtfLmmCallStartFlag(VOS_FALSE);

            /* 通知LTE呼叫结束 */
            CNAS_XCC_SndLmmCallEndNtf();
        }
...
)
{
    ...
    CNAS_XCC_SERVICE_TYPE_ENUM_UINT32           enOldServiceType;
    ...
    /* 将SO转换成SERVICE TYPE */
    CNAS_XCC_ConvertSoToServiceType(pstCallInfoCtx->enCurrentSo, &enOldServiceType);
    CNAS_XCC_ConvertSoToServiceType(pstUpdateCallInfoInd->enSo, &enServiceType);

    if (enOldServiceType == enServiceType)
    {
        pstCallInfoCtx->enCurrentSo = pstUpdateCallInfoInd->enSo;
    }

    CNAS_XCC_UpdateSrIdUsed(pstCallInfoCtx->enCurrentSo,
...
)
{
    ...
    CNAS_XCC_CALL_INFO_CTX_STRU        *pstCallInfo = VOS_NULL_PTR;
    CNAS_XCC_REJ_ORDER_INFO_STRU        stRejInfo = {0};
    VOS_UINT8                           aucPduData[CNAS_XCC_MAX_DATA_SDU_OCTET_NUM];
