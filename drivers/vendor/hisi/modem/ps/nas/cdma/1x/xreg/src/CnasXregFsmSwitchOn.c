...
VOS_VOID CNAS_XREG_ReadNvimInfo_SwitchOn(VOS_VOID)
{
    ...
    CNAS_NVIM_HOME_MCC_INFO_STRU       *pstHomeMccInfo;
    ...
    pstHomeMccInfo = CNAS_XREG_GetHomeMccAddr();

    if (NV_OK != NV_Read(en_NV_Item_HOME_MCC_INFO, pstHomeMccInfo, sizeof(CNAS_NVIM_HOME_MCC_INFO_STRU)))
    {
        CNAS_ERROR_LOG(UEPS_PID_XREG, "CNAS_XREG_ReadNvimInfo_SwitchOn:NV_Read Failed");

        /* NV读取失败则采用默认值 */
        pstHomeMccInfo->ulMccNum  = 2;

        pstHomeMccInfo->aulMcc[0] = 1540;  /* 460 */
        pstHomeMccInfo->aulMcc[1] = 1364;  /* 455 */
    }
