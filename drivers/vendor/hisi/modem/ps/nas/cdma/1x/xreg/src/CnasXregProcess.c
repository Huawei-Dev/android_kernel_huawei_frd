...
CNAS_NVIM_HOME_MCC_INFO_STRU            g_stCnasXregHomeMccInfo;
...
VOS_UINT32 CNAS_XREG_DSCHDataTrans(CAS_CNAS_1X_DSCH_DATA_IND_STRU *pstDschData)
{
    ...
    /* 初始化内存 */
    NAS_MEM_SET_S(&stDecodeData, sizeof(stDecodeData), 0, sizeof(stDecodeData));
...
CNAS_NVIM_HOME_MCC_INFO_STRU* CNAS_XREG_GetHomeMccAddr(VOS_VOID)
{
    return &g_stCnasXregHomeMccInfo;
}


VOS_UINT32 CNAS_XREG_IsHomeMcc(
    VOS_UINT32                          ulMcc
)
{
    VOS_UINT8                           i;

    for (i = 0; i < g_stCnasXregHomeMccInfo.ulMccNum; i++)
    {
        if (ulMcc == g_stCnasXregHomeMccInfo.aulMcc[i])
        {
            return VOS_TRUE;
        }
    }
...
VOS_UINT32 CNAS_XREG_IsPowerupReg(
    VOS_UINT32                          ulRatMode,
    VOS_UINT32                          ulMcc
)
{
    /* 只判断HRPD和LTE是否注册成功 */
    if ((VOS_RATMODE_HRPD != ulRatMode)
     && (VOS_RATMODE_LTE != ulRatMode))
    {
        return VOS_FALSE;
    }

    /* 判断是否在国外网络 */
    if (VOS_FALSE == CNAS_XREG_IsHomeMcc(ulMcc))
    {
        return VOS_FALSE;
    }
...
VOS_UINT32 CNAS_XREG_IsRoamReg(
    VOS_UINT32                          ulRatMode,
    VOS_UINT32                          ulMcc
)
{
    CNAS_CCB_MS_SYS_CFG_INFO_STRU      *pstSysCfg;
    VOS_UINT32                          i;

    /* 漫游到国外后不用考虑CDMA状态 */
    if ((VOS_RATMODE_1X == ulRatMode)
     || (VOS_RATMODE_HRPD == ulRatMode))
    {
        return VOS_FALSE;
    }

    /* 漫游到国外后不会有1X存在 */
    pstSysCfg = CNAS_CCB_GetCurMsSysCfgAddr();

    for (i = 0 ; i < pstSysCfg->ucRatNum; i++)
    {
        if (VOS_RATMODE_1X == pstSysCfg->aulRatPrio[i])
        {
            return VOS_FALSE;
        }
    }
    ...
    /* 判断是否在国外 */
    if (VOS_TRUE == CNAS_XREG_IsHomeMcc(ulMcc))
    {
        return VOS_FALSE;
    }
