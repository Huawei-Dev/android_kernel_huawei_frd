...
{
    XSD_MSCC_1X_UE_STATUS_IND_STRU     *pstUeStatus  = VOS_NULL_PTR;

    /* 分配消息 */
    pstUeStatus = (XSD_MSCC_1X_UE_STATUS_IND_STRU *)PS_ALLOC_MSG_WITH_HEADER_LEN(UEPS_PID_XSD,
                                                                                 sizeof(XSD_MSCC_1X_UE_STATUS_IND_STRU));
    ...
    pstUeStatus->stMsgHeader.ulSenderPid        = UEPS_PID_XSD;
    pstUeStatus->stMsgHeader.ulReceiverCpuId    = VOS_LOCAL_CPUID;
    pstUeStatus->stMsgHeader.ulReceiverPid      = CNAS_CCB_ConvertPidBasedOnModemId(CNAS_CCB_GetCdmaModeModemId(), UEPS_PID_MSCC);
    pstUeStatus->stMsgHeader.ulLength           = sizeof(XSD_MSCC_1X_UE_STATUS_IND_STRU) - VOS_MSG_HEAD_LENGTH;

    pstUeStatus->stMsgHeader.ulMsgName          = ID_XSD_MSCC_1X_UE_STATUS_IND;
    pstUeStatus->enUeMainState                  = enUeMainState;
    pstUeStatus->enUeSubState                   = enUeSubState;
    pstUeStatus->enHighPrioServiceType          = enHighPrioServiceType;
...
)
{
    ...
    pst1xNoSrvInd->en1xOosCause                  = en1xOosCause;
...
{
    XSD_MSCC_PRL_HEADER_INFO_IND_STRU                      *pstPrlHeaderInfoInd  = VOS_NULL_PTR;
    CNAS_PRL_HEADER_INFO_STRU                              *pstPrlHeadInfo       = VOS_NULL_PTR;

    pstPrlHeadInfo = CNAS_PRL_GetPrlHeaderInfoAddr();

    /* 分配消息 */
    pstPrlHeaderInfoInd = (XSD_MSCC_PRL_HEADER_INFO_IND_STRU *)PS_ALLOC_MSG_WITH_HEADER_LEN(UEPS_PID_XSD,
                                                    sizeof(XSD_MSCC_PRL_HEADER_INFO_IND_STRU));
    if (VOS_NULL_PTR == pstPrlHeaderInfoInd)
    {
        return;
    }

    NAS_MEM_SET_S(((VOS_UINT8*)pstPrlHeaderInfoInd) + VOS_MSG_HEAD_LENGTH,
                  sizeof(XSD_MSCC_PRL_HEADER_INFO_IND_STRU) - VOS_MSG_HEAD_LENGTH,
                  0x00,
                  sizeof(XSD_MSCC_PRL_HEADER_INFO_IND_STRU) - VOS_MSG_HEAD_LENGTH);

    /* 填充消息头 */
    pstPrlHeaderInfoInd->stMsgHeader.ulSenderCpuId     = VOS_LOCAL_CPUID;
    pstPrlHeaderInfoInd->stMsgHeader.ulSenderPid       = UEPS_PID_XSD;
    pstPrlHeaderInfoInd->stMsgHeader.ulReceiverCpuId   = VOS_LOCAL_CPUID;
    pstPrlHeaderInfoInd->stMsgHeader.ulReceiverPid     = CNAS_CCB_ConvertPidBasedOnModemId(CNAS_CCB_GetCdmaModeModemId(), UEPS_PID_MSCC);
    pstPrlHeaderInfoInd->stMsgHeader.ulLength          = sizeof(XSD_MSCC_PRL_HEADER_INFO_IND_STRU) - VOS_MSG_HEAD_LENGTH;
    pstPrlHeaderInfoInd->stMsgHeader.ulMsgName         = ID_XSD_MSCC_PRL_HEADER_INFO_IND;

    /* 填充消息内容 */
    pstPrlHeaderInfoInd->stPrlHeader.enPrlSrcType           = CNAS_CCB_GetPrlSrcType();
    pstPrlHeaderInfoInd->stPrlHeader.ucSsprPRev             = pstPrlHeadInfo->enSsprPRev;
    pstPrlHeaderInfoInd->stPrlHeader.ucPreferOnly           = pstPrlHeadInfo->ucPreferOnly;
    pstPrlHeaderInfoInd->stPrlHeader.ucDefRoamInd           = pstPrlHeadInfo->enDefRoamInd;
    pstPrlHeaderInfoInd->stPrlHeader.usPrListSize           = pstPrlHeadInfo->usPrListSize;
    pstPrlHeaderInfoInd->stPrlHeader.usPrListId             = pstPrlHeadInfo->usPrListId;
    pstPrlHeaderInfoInd->stPrlHeader.usNumAcqRecs           = pstPrlHeadInfo->usNumAcqRecs;
    pstPrlHeaderInfoInd->stPrlHeader.usNumSubnetRecs        = pstPrlHeadInfo->usNumSubnetRecs;
    pstPrlHeaderInfoInd->stPrlHeader.usNumSysRecs           = pstPrlHeadInfo->usNumSysRecs;
    pstPrlHeaderInfoInd->stPrlHeader.usAcqTabBitOffset      = pstPrlHeadInfo->usAcqTabBitOffset;
    pstPrlHeaderInfoInd->stPrlHeader.ulComSubnetBitOffset   = pstPrlHeadInfo->ulComSubnetBitOffset;
    pstPrlHeaderInfoInd->stPrlHeader.ulComSubnetBitSize     = pstPrlHeadInfo->ulComSubnetBitSize;

    CNAS_MNTN_LogMsgInfo((MSG_HEADER_STRU*)pstPrlHeaderInfoInd);

    /* 发送消息 */
    (VOS_VOID)PS_SEND_MSG(UEPS_PID_XSD, pstPrlHeaderInfoInd);
