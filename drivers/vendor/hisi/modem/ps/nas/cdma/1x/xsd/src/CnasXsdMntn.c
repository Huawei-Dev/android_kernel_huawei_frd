...
)
{
    ...
    pstMsg->usMru0SearchTimerLen        = pstOocTimerScheduleInfo->stCtrlInfo.usMru0SearchTimerLen;
    pstMsg->enChanRepeatScanStrategy    = pstOocTimerScheduleInfo->stCtrlInfo.enChanRepeatScanStrategy;
    pstMsg->ucPhaseNum                  = pstOocTimerScheduleInfo->stCtrlInfo.ucPhaseNum;
    pstMsg->uc1xOocDoTchPhase1TimerLen  = pstOocTimerScheduleInfo->stCtrlInfo.uc1xOocDoTchPhase1TimerLen;
    pstMsg->uc1xOocDoTchPhase2TimerLen  = pstOocTimerScheduleInfo->stCtrlInfo.uc1xOocDoTchPhase2TimerLen;

    for (i = 0; i < pstOocTimerScheduleInfo->stCtrlInfo.ucPhaseNum; i++)
    {
        pstMsg->astOocTimerInfo[i].usSleepTimeLen = pstOocTimerScheduleInfo->astSysAcqPhaseInfo[i].usSleepTimeLen;
        pstMsg->astOocTimerInfo[i].ucSrchNum      = pstOocTimerScheduleInfo->astSysAcqPhaseInfo[i].ucSrchNum;
        pstMsg->astOocTimerInfo[i].usTotalTimeLen = pstOocTimerScheduleInfo->astSysAcqPhaseInfo[i].usTotalTimeLen;
    }
...
)
{
    ...
    pstMsg->ucNextAcqIndex              = pstAvoidFreqList->ucNextAcqIndex;
...
{
    CNAS_XSD_LOG_DRX_TIMER_STATUS_IND  *pstMsg = VOS_NULL_PTR;

    pstMsg = (CNAS_XSD_LOG_DRX_TIMER_STATUS_IND *)PS_MEM_ALLOC(UEPS_PID_XSD,
                                                  sizeof(CNAS_XSD_LOG_DRX_TIMER_STATUS_IND));
    ...
    NAS_MEM_SET_S(pstMsg, sizeof(CNAS_XSD_LOG_DRX_TIMER_STATUS_IND), 0x00, sizeof(CNAS_XSD_LOG_DRX_TIMER_STATUS_IND));
    ...
    pstMsg->ulReceiverPid               = UEPS_PID_XSD;
    pstMsg->ulSenderCpuId               = VOS_LOCAL_CPUID;
    pstMsg->enMsgId                     = ID_CNAS_XSD_MNTN_LOG_DRX_TIMER_STATUS_IND;
    pstMsg->ulLength                    = sizeof(CNAS_XSD_LOG_DRX_TIMER_STATUS_IND) - VOS_MSG_HEAD_LENGTH;

    pstMsg->ucDrxTimerStatus            = ucDrxTimerStatus;

    CNAS_MNTN_TraceIntMsg(UEPS_PID_XSD, sizeof(CNAS_XSD_LOG_DRX_TIMER_STATUS_IND), (VOS_VOID *)pstMsg);
...
{
    CNAS_XSD_LOG_1X_AVOID_SYS_ACQ_STRU *pstMsg = VOS_NULL_PTR;

    pstMsg = (CNAS_XSD_LOG_1X_AVOID_SYS_ACQ_STRU *)PS_MEM_ALLOC(UEPS_PID_XSD,
                                                            sizeof(CNAS_XSD_LOG_1X_AVOID_SYS_ACQ_STRU));
    ...
    NAS_MEM_SET_S(pstMsg, sizeof(CNAS_XSD_LOG_1X_AVOID_SYS_ACQ_STRU), 0x00, sizeof(CNAS_XSD_LOG_1X_AVOID_SYS_ACQ_STRU));
    ...
    pstMsg->ulReceiverPid               = UEPS_PID_XSD;
    pstMsg->ulSenderCpuId               = VOS_LOCAL_CPUID;
    pstMsg->enMsgId                     = ID_CNAS_XSD_MNTN_LOG_1X_AVOID_SYS_ACQ;
    pstMsg->ulLength                    = sizeof(CNAS_XSD_LOG_1X_AVOID_SYS_ACQ_STRU) - VOS_MSG_HEAD_LENGTH;
    pstMsg->ulIsAvoidSysAcq             = (VOS_UINT32)ucFlg;

    CNAS_MNTN_TraceIntMsg(UEPS_PID_XSD, sizeof(CNAS_XSD_LOG_1X_AVOID_SYS_ACQ_STRU), (VOS_VOID *)pstMsg);
...
{
    CNAS_XSD_PC_REPLAY_FIXED_CONTEXT_MSG_STRU                         stFixedCtxData;
    ...
    NAS_MEM_CPY_S(&(stFixedCtxData.stMruList), sizeof(CNAS_XSD_MRU_LIST_STRU),
                  pstMruList, sizeof(CNAS_XSD_MRU_LIST_STRU));

    NAS_MEM_CPY_S(&(stFixedCtxData.stStrategyCfg), sizeof(CNAS_XSD_OOS_SYS_ACQ_STRTEGY_CFG_STRU),
                  &(pstOocScheduleInfo->stStrategyCfg), sizeof(CNAS_XSD_OOS_SYS_ACQ_STRTEGY_CFG_STRU));

    NAS_MEM_CPY_S(&(stFixedCtxData.stTestConfig), sizeof(CNAS_XSD_TEST_CONFIG_STRU),
                  pstTestConfig, sizeof(CNAS_XSD_TEST_CONFIG_STRU));

    NAS_MEM_CPY_S(&(stFixedCtxData.stAddAvoidListCfg), sizeof(CNAS_XSD_ADD_AVOID_LIST_CFG_STRU),
                  pstAddAvoidListCfg, sizeof(CNAS_XSD_ADD_AVOID_LIST_CFG_STRU));

    NAS_MEM_CPY_S(&(stFixedCtxData.stNegPrefSysCmpCtrl), sizeof(CNAS_XSD_NEG_PREF_SYS_CMP_CTRL_STRU),
                  pstNegPrefSysCmpCtrl, sizeof(CNAS_XSD_NEG_PREF_SYS_CMP_CTRL_STRU));

    NAS_MEM_CPY_S(&(stFixedCtxData.stCallBackCfg), sizeof(CNAS_XSD_CALLBACK_CFG_STRU),
                  pstCallBackCfg, sizeof(CNAS_XSD_CALLBACK_CFG_STRU));

    NAS_MEM_CPY_S(&(stFixedCtxData.stCdma1XCustomPrefChan), sizeof(CNAS_XSD_CDMA_1X_CUSTOM_PREF_CHANNELS_STRU),
                  pstCdma1XCustomPrefChan, sizeof(CNAS_XSD_CDMA_1X_CUSTOM_PREF_CHANNELS_STRU));

    NAS_MEM_CPY_S(&(stFixedCtxData.stAvoidScheduInfo), sizeof(CNAS_XSD_AVOID_SCHEDULE_INFO_STRU),
                  pstAvoidScheduleInfo, sizeof(CNAS_XSD_AVOID_SCHEDULE_INFO_STRU));

    NAS_MEM_CPY_S(&(stFixedCtxData.stMru0StrategyCfg), sizeof(CNAS_XSD_MRU0_SWITCH_ON_OOC_STRATEGY_CFG_STRU),
                  &(CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.stMru0StrategyCfg), sizeof(CNAS_XSD_MRU0_SWITCH_ON_OOC_STRATEGY_CFG_STRU));

    NAS_MEM_CPY_S(&(stFixedCtxData.stSysAcqNoRfProtectTimerInfo), sizeof(CNAS_XSD_1X_SYS_ACQ_NO_RF_PROTECT_TIMER_INFO_STRU),
                  &(CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.stSysAcqNoRfProtectTimerInfo), sizeof(CNAS_XSD_1X_SYS_ACQ_NO_RF_PROTECT_TIMER_INFO_STRU));

    NAS_MEM_CPY_S(&(stFixedCtxData.stOperLockSysWhiteList), sizeof(CNAS_CCB_OPER_LOCK_SYS_WHITE_LIST_STRU),
                  pstOperLockSysWhiteList, sizeof(CNAS_CCB_OPER_LOCK_SYS_WHITE_LIST_STRU));

    NAS_MEM_CPY_S(&(stFixedCtxData.stCustFreqList), sizeof(CNAS_CCB_CTCC_CUSTOMIZE_FREQ_LIST_STRU),
                  pstCustFreqList, sizeof(CNAS_CCB_CTCC_CUSTOMIZE_FREQ_LIST_STRU));

    NAS_MEM_CPY_S(&(stFixedCtxData.stCdmaStandardChan), sizeof(CNAS_CCB_CDMA_STANDARD_CHANNELS_STRU),
                  pstCdmaStandardChan, sizeof(CNAS_CCB_CDMA_STANDARD_CHANNELS_STRU));

    NAS_MEM_CPY_S(&(stFixedCtxData.stCustomCfg), sizeof(CNAS_CCB_CUSTOM_CFG_INFO_STRU),
                  &(CNAS_CCB_GetCcbCtxAddr()->stCustomCfg), sizeof(CNAS_CCB_CUSTOM_CFG_INFO_STRU));

    NAS_MEM_CPY_S(stFixedCtxData.aucEmcRedialTimes, sizeof(stFixedCtxData.aucEmcRedialTimes), pstEmcRedialSysAcqCfg->aucRedialTimes, sizeof(pstEmcRedialSysAcqCfg->aucRedialTimes));

    stFixedCtxData.ucIsConsiderRoamIndInPRL  = CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.ucIsConsiderRoamIndInPRL;
    stFixedCtxData.uc1xSysAcqSyncDelayEnable = CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.st1xSysAcqSyncDelayInfo.uc1xSysAcqSyncDelayEnable;
    stFixedCtxData.ul1xSysAcqDelayTimerLen   = CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.st1xSysAcqSyncDelayInfo.ul1xSysAcqDelayTimerLen;
    stFixedCtxData.ulUeSupportedBand         = CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.unUeSupportedBand.ulBand;

    CNAS_MNTN_LogPcReplayFixedContext(sizeof(CNAS_XSD_PC_REPLAY_FIXED_CONTEXT_MSG_STRU), UEPS_PID_XSD, (VOS_UINT8 *)&stFixedCtxData);
}

VOS_VOID CNAS_XSD_LogPcReplayPreCfgParaSwitchOn(VOS_VOID)
{
    CNAS_XSD_LOG_PRE_PARA_AHEAD_SWITCH_ON_STRU              stPrePara;

    NAS_MEM_SET_S(&stPrePara, sizeof(stPrePara), 0, sizeof(stPrePara));

    stPrePara.ulIsPrlValid    = CNAS_PRL_GetPrlInfoValidFlag();

    CNAS_MNTN_LogPcReplayPreCfgParaSwitchOn(sizeof(stPrePara), UEPS_PID_XSD, (VOS_UINT8 *)&stPrePara);
