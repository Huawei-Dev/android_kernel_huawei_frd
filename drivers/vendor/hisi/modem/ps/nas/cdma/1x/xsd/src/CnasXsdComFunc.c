...
CNAS_XSD_ERRLOG_1X_OOS_CAUSE_STRU       g_astCNasXsdErrLog1xOosCauseMapTbl[] =
{
    {NAS_ERR_LOG_1X_OOS_CAUSE_SD_RESCAN,                    CAS_CNAS_1X_SD_RESCAN_IND},

    {NAS_ERR_LOG_1X_OOS_CAUSE_SD_PROTO_MISMATCH,            CAS_CNAS_1X_SD_PROTO_MISMATCH_IND},

    {NAS_ERR_LOG_1X_OOS_CAUSE_SD_SYS_LOST,                  CAS_CNAS_1X_SD_SYS_LOST_IND},

    {NAS_ERR_LOG_1X_OOS_CAUSE_SD_LOCK,                      CAS_CNAS_1X_SD_LOCK_IND},

    {NAS_ERR_LOG_1X_OOS_CAUSE_SD_UNLOCK,                    CAS_CNAS_1X_SD_UNLOCK_IND},

    {NAS_ERR_LOG_1X_OOS_CAUSE_SD_ACCESS_DENIED,             CAS_CNAS_1X_SD_ACCESS_DENIED_IND},

    {NAS_ERR_LOG_1X_OOS_CAUSE_SD_ACCT_BLOCKED,              CAS_CNAS_1X_SD_ACCT_BLOCKED_IND},

    {NAS_ERR_LOG_1X_OOS_CAUSE_SD_NDSS_OFF,                  CAS_CNAS_1X_SD_NDSS_OFF_IND},

    {NAS_ERR_LOG_1X_OOS_CAUSE_SD_ERR,                       CAS_CNAS_1X_SD_ERR_IND},

    {NAS_ERR_LOG_1X_OOS_CAUSE_SD_REGIST_REJ,                CAS_CNAS_1X_SD_REGIST_REJ_IND},

    {NAS_ERR_LOG_1X_OOS_CAUSE_SD_RELEASE,                   CAS_CNAS_1X_SD_RELEASE_IND},

    {NAS_ERR_LOG_1X_OOS_CAUSE_SD_RESELECTION,               CAS_CNAS_1X_SD_RESELECTION_IND},

    {NAS_ERR_LOG_1X_OOS_CAUSE_SD_NEW_SYSTEM,                CAS_CNAS_1X_SD_NEW_SYSTEM_IND},

    {NAS_ERR_LOG_1X_OOS_CAUSE_SD_MAX_ACCESS_FAILURE,        CAS_CNAS_1X_SD_MAX_ACCESS_FAILURE},

    {NAS_ERR_LOG_1X_OOS_CAUSE_SD_ABNORMAL,                  CAS_CNAS_1X_SD_ABNORMAL_IND},

    {NAS_ERR_LOG_1X_OOS_CAUSE_SD_ACQUISITION_FAIL,          CAS_CNAS_1X_SD_ACQUISITION_FAIL},

    {NAS_ERR_LOG_1X_OOS_CAUSE_NO_RF,                        CAS_CNAS_1X_SD_NO_RF},
};
...
)
{
    ...
    CNAS_PRL_ACQ_REC_FREQ_INFO_STRU     stDstFreqInfo = {0};
    VOS_UINT8                           i;
...
)
{
    ...
    {
        stSysInfo.ucRoamingInd          = CNAS_PRL_SYS_ROAMING_STATUS_OFF;
        stCurCampedSysInfo.ucRoamingInd = CNAS_PRL_SYS_ROAMING_STATUS_OFF;
    }
    ...
    if (VOS_FALSE == ulIsNegSys)
    {
        ...
        /* 如果是中国电信PLMN,加入到中国电信常用频点NV */
        if (VOS_TRUE == CNAS_XSD_IsNeedRecordAs1xCustomChannels(&stCurCampedSysInfo))
        {
            if (VOS_TRUE == CNAS_XSD_AddCurFreqToCdma1XCustomPrefChannels(&stCurSysInfo.stFreq))
            {
                CNAS_XSD_WriteCdma1XPrefChannelsNvim(&stCurSysInfo.stFreq);
            }
        }
...
)
{
    ...
    /* 当前系统不在PRL表中，且在白名单中，则认为是Available系统 */
    /* HOME SID/NID 只影响系统表的漫游指示属性，不影响Available系统的判断 */
...
VOS_VOID CNAS_XSD_QuitCallBackMode( VOS_VOID )
{
    ...
    CNAS_CCB_SetEmcCbState(CNAS_CCB_EMC_CB_STATE_NULL);
...
)
{
    CNAS_PRL_MATCHED_GEO_LIST_INFO_STRU stGeoListInfo;
    VOS_UINT32                          ulIsNegSys;
    CNAS_XSD_CFREQ_LOCK_SET_PARA_STRU  *pstFreqLockPara = VOS_NULL_PTR;

    CNAS_PRL_Get1xSysGeoList(pstCurSysInfo, &stGeoListInfo);

    /* 锁频状态下，若锁的SID/NID与当前系统SID/NID匹配，跳过PRL表，直接上报正常服务 */
    /* 获取锁频信息 */
    pstFreqLockPara = CNAS_XSD_GetFreqLockAddr();

    if (VOS_TRUE == CNAS_XSD_GetFreqLockMode())
    {
        if (VOS_TRUE == CNAS_PRL_Is1xSysIdMatched(pstCurSysInfo->usSid,
                                                  pstCurSysInfo->usNid,
    ...
    if (0 == stGeoListInfo.ucGeoNum)
    {
        /* 不在PRL表中时，仅PREF ONLY=FALSE,且在白名单中，才认为适合驻留 */
        ...
        /* HOME SID/NID 只影响漫游属性，不影响是否适合驻留的判断 */

        return VOS_TRUE;
    }

    /* 当前系统在PRL表中，非Negative系统，且在白名单中，才认为适合驻留 */
    ...
    /* HOME SID/NID只影响系统的漫游指示属性，不作为允许驻留的依据 */
...
)
{
    ...
    else
    {
        ...
        /* HOME SID/NID只影响系统的漫游指示属性，不影响Neg系统的判断 */
    }
...
)
{
    ...
    if ((CAS_CNAS_1X_SD_REDIR_IND              == enSdReason)
     || (CAS_CNAS_1X_SD_SYSTEM_RESELECTION_IND == enSdReason))
    {
        return CNAS_XSD_GetOosSpecialSdReason();
    }

    for (ulCnt = 0; ulCnt < (sizeof(g_astCNasXsdErrLog1xOosCauseMapTbl)/sizeof(CNAS_XSD_ERRLOG_1X_OOS_CAUSE_STRU)); ulCnt++)
    {
        if (g_astCNasXsdErrLog1xOosCauseMapTbl[ulCnt].enSdReason == enSdReason)
        {
            return g_astCNasXsdErrLog1xOosCauseMapTbl[ulCnt].en1xOosCause;
        }
    }

    return NAS_MSCC_PIF_1X_OOS_CAUSE_BUTT;
}
...
{
    if (CNAS_XSD_SYS_ACQ_SCENE_CALL_RELEASED == enCurAcqScene)
    {
        return NAS_MSCC_PIF_1X_OOS_CAUSE_CALL_RELEASE_ACQ_FAIL;
    }
    else if (CNAS_XSD_SYS_ACQ_SCENE_EMC_CALL_RELEASED == enCurAcqScene)
    {
        return NAS_MSCC_PIF_1X_OOS_CAUSE_ENC_CALL_RELEASE_ACQ_FAIL;
    }
    else if (CNAS_XSD_SYS_ACQ_SCENE_SWITCH_ON == enCurAcqScene)
    {
        return NAS_MSCC_PIF_1X_OOS_CAUSE_SWITCH_ON_SEARCH_FAIL;
    }
    else
    {
        return NAS_MSCC_PIF_1X_OOS_CAUSE_BUTT;
...
{
    /* 非中国电信网络 */
    if ((CNAS_XSD_MCC_CHINA != pstCampSysInfo->ulMcc)
    && (CNAS_XSD_MCC_MACAO  != pstCampSysInfo->ulMcc))
    {
        return VOS_FALSE;
    }

    /* 已经存在于列表中 */
    if (VOS_TRUE == CNAS_XSD_IsCurFreqInCdma1XCustomPrefChannels(&pstCampSysInfo->stSystem.stFreq))
    {
        return VOS_FALSE;
    }
...
VOS_VOID CNAS_XSD_RecordChr1xOosSrchInfo(VOS_VOID)
{
    ...
    /* 搜网记录数加1 */
    NAS_ERRLOG_CCB_InCrease1xOosSrchInfoRecordCnt();

    /* 对当前索引对应的记录结构体进行初始化 */
    ulIndex = NAS_ERRLOG_CCB_Calc1xOosSrchInfoRecordIndex();
    NAS_ERRLOG_CCB_Init1xOosSrchInfoRecord(ulIndex);

    /* 设置当前搜网记录的索引值 */
    NAS_ERRLOG_CCB_Set1xOosSrchInfoRecordIndex(ulIndex, NAS_ERRLOG_CCB_Get1xOosSrchInfoRecordCnt());

    /* 设置当前搜网记录的开始时间 */
    NAS_ERRLOG_CCB_Set1xOosSrchInfoStartTime(ulIndex, VOS_GetSlice());
...
{
    VOS_UINT32                                              ulIndex;
    NAS_ERR_LOG_1X_SEARCH_RSLT_ENUM_UINT32                  en1xSearchRslt;

    /* 记录当前搜网记录的结束时间 */
    ulIndex = NAS_ERRLOG_CCB_Calc1xOosSrchInfoRecordIndex();
    NAS_ERRLOG_CCB_Set1xOosSrchInfoEndTime(ulIndex, VOS_GetSlice());

    if (CNAS_XSD_SYSTEM_ACQUIRED_RESULT_SUCCESS == pstSysAcqRslt->enRslt)
    {
        en1xSearchRslt = NAS_ERR_LOG_1X_SEARCH_RSLT_SUCCESS;
    }
    else if (CNAS_XSD_SYSTEM_ACQUIRED_RESULT_ABORTED == pstSysAcqRslt->enRslt)
    {
        en1xSearchRslt = (VOS_UINT32)CNAS_XSD_GetSystemAcquiredAbortedCause();
    }
    else
    {
        if (CNAS_XSD_SYS_ACQ_SCENE_NO_RF == pstSysAcqRslt->enCurAcqScene)
        {
            /* 如果搜网结果携带的当前搜网场景是NO_RF，需要识别出来，记录为NO_RF */
            en1xSearchRslt = NAS_ERR_LOG_1X_SEARCH_RSLT_NO_RF;
        }
        else
        {
            en1xSearchRslt = NAS_ERR_LOG_1X_SEARCH_RSLT_FAILURE;
        }
    }

    /* 记录当前搜网记录的搜网结果 */
    NAS_ERRLOG_CCB_Set1xOosSrchInfoSearchRslt(ulIndex, en1xSearchRslt);
...
)
{
    ...
    VOS_UINT32                          ulSrlteSyncOneChanAddTimerLen;
    VOS_UINT32                          ulOneChanSrchTimerLen;
    VOS_UINT32                          ulSrlteOneChanSyncTimerLen;

    ulOneChanSrchTimerLen = TI_CNAS_XSD_ONE_CHAN_SRCH_TIMES * TI_CNAS_XSD_SRCH_ONE_CHAN_TIMER_LEN;

    if (CNAS_CCB_VERSION_SRLTE == CNAS_CCB_IsVersionSrlte())
    {
        /*SRLTE:物理层搜索单个频点时长为330ms，每个频点尝试4次搜索，每个频点同步时长为(1+10)s，对CNAS来说，单个频点sync时长= 330ms*4+11=12.32s
         *所以，ulWaitCasSyncCnfTimerLen = FreqNum * 单个频点sync时长 + cas延时ulSrlteSyncOneChanAddTimerLen + cnas延时5s */
        ulSrlteSyncOneChanAddTimerLen   = CNAS_XSD_GetXsdCtxAddr()->stSysAcqCtrl.ulSrlteSyncOneChanAddTimerLen;
        ulSrlteOneChanSyncTimerLen      = ulSrlteSyncOneChanAddTimerLen + TI_CNAS_XSD_SVLTE_CAS_SYNC_ONE_CHAN_TIMER_LEN;

        ulWaitCasSyncCnfTimerLen = ulSyncFreqNum * (ulOneChanSrchTimerLen + ulSrlteOneChanSyncTimerLen) + ulSrlteSyncOneChanAddTimerLen + TI_CNAS_XSD_WAIT_CAS_SYNC_CNF_APPEND_TIME_LEN;
    }
    else
    {
        /*SVLTE:物理层搜索单个频点时长为330ms，每个频点尝试4次搜索，每个频点同步时长为1s，对CNAS来说，单个频点sync时长= 330ms*4+1=2.32s
         *所以，ulWaitCasSyncCnfTimerLen = FreqNum * 单个频点sync时长 + cas资源申请保护时长 + cnas延时5s */
        ulWaitCasSyncCnfTimerLen = ulSyncFreqNum * (ulOneChanSrchTimerLen + TI_CNAS_XSD_SVLTE_CAS_SYNC_ONE_CHAN_TIMER_LEN) + TI_CNAS_XSD_SVLTE_CAS_SYNC_APPEND_TIME_LEN + TI_CNAS_XSD_WAIT_CAS_SYNC_CNF_APPEND_TIME_LEN;
    }

    return ulWaitCasSyncCnfTimerLen;
}
