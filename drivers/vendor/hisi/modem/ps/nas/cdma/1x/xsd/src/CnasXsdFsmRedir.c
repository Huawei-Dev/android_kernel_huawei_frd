...
)
{
    ...
    {
        if (CNAS_TIMER_STATUS_RUNNING == CNAS_GetSpecifiedTimerStatus(UEPS_PID_XSD, TI_CNAS_XSD_REDIR_NO_RF_PROTECT_TIMER, 0))
        {
            CNAS_XSD_StopTimer(TI_CNAS_XSD_REDIR_NO_RF_PROTECT_TIMER);
        }
        /* SRLTE模式下，1x搜网不申请资源，起保护定时器 */
        CNAS_XSD_StartTimer(TI_CNAS_XSD_REDIR_NO_RF_PROTECT_TIMER, CNAS_XSD_GetRedirNoRfProtectTimerLen());
    }

    CNAS_NORMAL_LOG1(UEPS_PID_XSD, "CNAS_XSD_RcvXsdRedirInd_Redir_Init: Curr Version Type is", CNAS_CCB_IsVersionSrlte());
...
)
{
    ...
    if (VOS_TRUE == CNAS_XSD_GetNdssIndFlag())
    {
        ...
        /* NDSS OFF结束，降低Session保护类型 */
        CNAS_XSD_SndCasEndSessionNotify(CNAS_CAS_1X_SESSION_TYPE_TCH_REL_NETWORK_SEARCH);
...
)
{
    ...
    {
        /***********************************************************************************************
         * 如果当前是SRLTE，且回原系统失败原因值是NO RF,重新发回原系统请求;否则退出状态机
         **********************************************************************************************/
        if (CNAS_CCB_VERSION_SRLTE == CNAS_CCB_IsVersionSrlte())
        {
            if (CNAS_TIMER_STATUS_RUNNING == CNAS_GetSpecifiedTimerStatus(UEPS_PID_XSD, TI_CNAS_XSD_REDIR_NO_RF_PROTECT_TIMER, 0))
            {
                ...
                CNAS_XSD_StartTimer(TI_CNAS_XSD_WAIT_CAS_SYSTEM_SYNC_CNF,
                                    CNAS_XSD_GetWaitCasSystemSyncCnfTimerLen(usDstChanNum));
                return VOS_TRUE;
            }
        }
...
)
{
    ...
    CNAS_XSD_SetCurChannelScanIndex_Redir(usCurScanListIndex + usUpdateSyncFailFreqNum);
