...
)
{
    ...
    /* 记录当前呼叫类型为被叫*/
    pstCallInfoCtx->ucIsMoCall = VOS_FALSE;
...
)
{
    ...
    if (VOS_TRUE == CNAS_XCC_IsNeedNtfLmmVoiceCall())
    {
            ...
            /* 通知LMM开始ESR流程 */
            CNAS_XCC_SndLmmCallStartNtf(XCC_LMM_CALL_TYPE_MT);

            /* 状态迁移到CNAS_XCC_MT_CALLING_STA_WAIT_LMM_ESR_END_IND */
            CNAS_XCC_SetCurrFsmState(CNAS_XCC_MT_CALLING_STA_WAIT_LMM_ESR_END_IND);
...
)
{
    ...
    /* 停止TI_CNAS_XCC_WAIT_ESR_END_IND */
...
#if (FEATURE_ON == FEATURE_PTM)
        enXccCause = CNAS_XCC_ConvertEstReasonToTafCause(pstEstCnfMsg->enEstRslt);

        CNAS_XCC_ProcMtServiceExceptionErrLogInfo(CNAS_XCC_ConvertSoTo1xErrLogServiceType(pstCallInfoCtx->enCurrentSo),
                                                  CNAS_XCC_MapXccCauseToErrLogCause(enXccCause),
                                                  CNAS_XCC_MT_CALLING_STA_WAIT_AS_EST_CNF);
#endif
...
)
{
    ...
    CNAS_XCC_ProcMtServiceExceptionErrLogInfo(CNAS_XCC_ConvertSoTo1xErrLogServiceType(pstCallInfoCtx->enCurrentSo),
                                              ...
                                              CNAS_XCC_MT_CALLING_STA_WAIT_AS_CALL_INIT);
#endif
...
)
{
    ...
    CNAS_XCC_ProcMtServiceExceptionErrLogInfo(CNAS_XCC_ConvertSoTo1xErrLogServiceType(pstCallInfoCtx->enCurrentSo),
                                              ...
                                              CNAS_XCC_MT_CALLING_STA_WAIT_FOR_ORDER);
#endif
...
)
{
    ...
    if (XCC_XCALL_RESULT_SUCC != pstIncomingCallRsp->enRslt)
    {
        /* 通知接入层REL REQ */
        CNAS_XCC_SndCasCallRelReq(pstCallInfo->ucConnectId,
                                  pstCallInfo->ucConRef,
        ...
        {            
            CNAS_XCC_ProcMtServiceExceptionErrLogInfo(NAS_ERR_LOG_1X_MT_SERVICE_TYPE_VOICE_CALL,
                                                      NAS_ERR_LOG_1X_MT_SERVICE_EXCEPTION_CAUSE_INCOMING_RSP_FAIL,
                                                      CNAS_XCC_MT_CALLING_STA_WAIT_FOR_INCOMING_RSP);
        }
#endif
...
)
{
    ...
    /* INCOMING RSP结果失败，通知CAS，退状态机 */
    if (XCC_APS_RESULT_SERVICE_INACTIVE == pstIncomingCallRsp->enRslt)
    {
        /* 通知接入层REL REQ */
        CNAS_XCC_SndCasCallRelReq(pstCallInfo->ucConnectId,
                                  pstCallInfo->ucConRef,
    ...
    if (XCC_APS_RESULT_NORMAL_RELEASE == pstIncomingCallRsp->enRslt)
    {
        /* 通知接入层REL REQ */
        CNAS_XCC_SndCasCallRelReq(pstCallInfo->ucConnectId,
                                  pstCallInfo->ucConRef,
...
#if (FEATURE_ON == FEATURE_PTM)
    CNAS_XCC_ProcMtServiceExceptionErrLogInfo(CNAS_XCC_ConvertSoTo1xErrLogServiceType(pstCallInfo->enCurrentSo),
...
)
{
    ...
    CNAS_XCC_ProcMtServiceExceptionErrLogInfo(CNAS_XCC_ConvertSoTo1xErrLogServiceType(pstCallInfoCtx->enCurrentSo),
                                              ...
                                              CNAS_XCC_MT_CALLING_STA_WAIT_FOR_ANSWER);
#endif
...
)
{
    ...
    CNAS_XCC_ProcMtServiceExceptionErrLogInfo(CNAS_XCC_ConvertSoTo1xErrLogServiceType(pstCallInfo->enCurrentSo), 
                                              ...
                                              CNAS_XCC_MT_CALLING_STA_WAIT_CONNECT_L2_ACK);
#endif
...
)
{
    ...
    NAS_ERRLOG_CCB_SaveMtServiceExceptionXccState(enService, enMtCallingStatus);
