...
)
{
    ...
    CNAS_XSD_LogPcReplayPreCfgParaSwitchOn();
#endif
...
#ifndef __PS_WIN32_RECUR__
    CNAS_XSD_LogPcReplayFixedContext();
#endif
    ...
    /* 在CCB中存储syscfg信息 */
    CNAS_CCB_SetCurMsSysCfg(pstStartReq->ucSuppRatNum, pstStartReq->aenRatMode);
    ...
    if (VOS_TRUE == CNAS_XSD_GetNoCardModeCfgFlg())
    {
        CNAS_XSD_ReadCbtPrlNvim();

        CNAS_XSD_SndMsccPrlHeaderInfoInd();
    ...
    else
    {
        ...
        CNAS_XSD_LogPrlSourceType(CNAS_CCB_GetCcbCtxAddr()->enPrlSrcType);
    }
...
VOS_VOID  CNAS_XSD_ReadNvimInfo_SwitchOn(VOS_VOID)
{
    ...
    CNAS_XSD_Read1xOosSysAcqStrategyCfgNvim();
    ...
    CNAS_XSD_ReadMru0SwitchOocStrategyCfgNvim();

    CNAS_XSD_Read1xPRLRoamIndStrategyNvim();

    CNAS_XSD_Read1xDoSysAcqNoRfProtectTimerCfgNvim();

    CNAS_XSD_ReadLteOrDoConn1xSysAcqSyncDelayInfoNvim();

    CNAS_XSD_ReadSrlteSyncOneChanAddTimerLen();
...
VOS_VOID CNAS_XSD_ReadDefaultPrl(VOS_VOID)
{
    ...
    if (VOS_TRUE == ulRslt)
    {
        CNAS_XSD_LogPrlSourceType(CNAS_CCB_PRL_TYPE_HARD_CODE);
        CNAS_CCB_SetPrlSrcType(CNAS_CCB_PRL_TYPE_HARD_CODE);
    }
