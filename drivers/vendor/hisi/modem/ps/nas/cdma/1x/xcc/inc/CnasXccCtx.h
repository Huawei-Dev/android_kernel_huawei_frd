...
typedef struct
{
    ...
    **************************************************************************/
    ...
    VOS_UINT8                                               ucIsAlreadyNtfLmmCallStart;  /* SRLTE下标记是否已经通知过LMM呼叫开始 */
    VOS_UINT8                                               aucRsvd[3];
}CNAS_XCC_CTX_STRU;

/*****************************************************************************
 Structure      : CNAS_XCC_PC_REPLAY_FIXED_CONTEXT_MSG_STRU
 Description    : PC回放工程，所有XCC相关的固定全局变量通过消息结构发送
 Message origin :
*****************************************************************************/
typedef struct
{
    CNAS_XCC_CALL_NVIM_CFG_STRU                             stCallNvimCfg;
    CNAS_XCC_1X_PAGING_RSP_SO_CFG_STRU                      stPagingRspSoCfg;
}CNAS_XCC_PC_REPLAY_FIXED_CONTEXT_MSG_STRU;
