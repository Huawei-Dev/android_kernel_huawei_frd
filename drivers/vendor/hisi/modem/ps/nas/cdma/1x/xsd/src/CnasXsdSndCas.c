...
)
{
    ...
    /* 填充消息 */
    NAS_MEM_SET_S((VOS_UINT8 *)pstStartrReq + VOS_MSG_HEAD_LENGTH,
                  sizeof(CNAS_CAS_1X_START_REQ_STRU) - VOS_MSG_HEAD_LENGTH,
                  0x00,
                  sizeof(CNAS_CAS_1X_START_REQ_STRU) - VOS_MSG_HEAD_LENGTH);

    pstStartrReq->ulSenderCpuId         = VOS_LOCAL_CPUID;
    pstStartrReq->ulSenderPid           = UEPS_PID_XSD;
...
VOS_VOID CNAS_XSD_SndCasOocNtf(VOS_VOID)
{
    CNAS_CAS_1X_OOC_NTF_STRU           *pstOocNtf  = VOS_NULL_PTR;

    /* 分配消息 */
    pstOocNtf = (CNAS_CAS_1X_OOC_NTF_STRU *)PS_ALLOC_MSG(UEPS_PID_XSD,
                sizeof(CNAS_CAS_1X_OOC_NTF_STRU) - VOS_MSG_HEAD_LENGTH);
    ...
    /* 填充消息 */
    pstOocNtf->ulReceiverPid       = UEPS_PID_1XCASM;
    pstOocNtf->enMsgId             = ID_CNAS_CAS_1X_OOC_NTF;
    pstOocNtf->usOpId              = 0;

    CNAS_MNTN_LogMsgInfo((MSG_HEADER_STRU*)pstOocNtf);

    /* 发送消息 */
    (VOS_VOID)PS_SEND_MSG(UEPS_PID_XSD, pstOocNtf);
