...
VOS_VOID CNAS_PRL_AddCTCCCustomizeFreqListIntoAcqRec(VOS_VOID)
{
    ...
    /* 申请内存 */
    ulSize                             = (CNAS_CCB_MAX_FREQ_NUM) * sizeof(CNAS_PRL_FREQENCY_CHANNEL_STRU) + sizeof(CNAS_PRL_ACQ_RECORD_STRU);
    pstAcqRecord                       = (CNAS_PRL_ACQ_RECORD_STRU *)CNAS_PRL_Malloc(ulSize);
    ...
    pstAcqRecord->enAcqType                = CNAS_PRL_ACQ_TYPE_GENERIC_ACQ_RECORD_FOR_HRPD;
    pstAcqRecord->ucAcqValid               = VOS_TRUE;
    pstAcqRecord->u.stHrpdSys.ucNumOfChans = (VOS_UINT8)(pstCustFreqList->usFreqNum);

    for (i = 0;i < pstCustFreqList->usFreqNum;i++)
    {
        pstAcqRecord->u.stHrpdSys.astFreq[i].usChannel   = pstCustFreqList->astFreqList[i].usChannel;
        pstAcqRecord->u.stHrpdSys.astFreq[i].enBandClass = CNAS_PRL_BAND_CLASS_TYPE_0;
    }
}
...
)
{
    ...
    if ((usFirstPrlListSize + CNAS_PRL_PRL_LIST_SIZE_BYTE_LEN) < usSrcLen)
    {
        ...
        /* PRL表size至少要大于两个字节，为后续处理做保护 */
        if ((usSecondPrlListSize > (sizeof(VOS_UINT16) + sizeof(VOS_UINT16)))
         && ((usFirstPrlListSize + usSecondPrlListSize) <= usSrcLen))
        {
            return VOS_TRUE;
        }
    }
