...
)
{
    ...
    CNAS_MNTN_LogReport(ulPid, pcString, enLevel, pstPara, pcFileName);
...
{
    XSD_MSCC_HANDSET_INFO_QRY_CNF_STRU                      stHandSetInfo = {{0}};
...
)
{
    ...
    if (0 == pstPara->ulParaNum)
    {
        (VOS_VOID)DIAG_LogReport( DIAG_GEN_LOG_MODULE(VOS_GetModemIDFromPid(ulPid), DIAG_MODE_1X, (enLevel)), \
                      (ulPid), pcFileName, pstPara->ulLine, "%s \r\n", (pcString) );
    }
    else if (1 == pstPara->ulParaNum)
    {
        (VOS_VOID)DIAG_LogReport( DIAG_GEN_LOG_MODULE(VOS_GetModemIDFromPid(ulPid), DIAG_MODE_1X, (enLevel)), \
                      (ulPid), pcFileName, pstPara->ulLine, "%s, %d \r\n", (pcString), (VOS_INT32)(pstPara->lPara1) );
    }
    else if (2 == pstPara->ulParaNum)
    {
        (VOS_VOID)DIAG_LogReport( DIAG_GEN_LOG_MODULE(VOS_GetModemIDFromPid(ulPid), DIAG_MODE_1X, (enLevel)), \
                      (ulPid), pcFileName, pstPara->ulLine, "%s, %d, %d \r\n", (pcString), (VOS_INT32)(pstPara->lPara1), (VOS_INT32)(pstPara->lPara2) );
    }
    else if (3 == pstPara->ulParaNum)
    {
        (VOS_VOID)DIAG_LogReport( DIAG_GEN_LOG_MODULE(VOS_GetModemIDFromPid(ulPid), DIAG_MODE_1X, (enLevel)), \
                      (ulPid), pcFileName, pstPara->ulLine, "%s, %d, %d, %d \r\n", (pcString), (VOS_INT32)(pstPara->lPara1), (VOS_INT32)(pstPara->lPara2), (VOS_INT32)(pstPara->lPara3) );
    }
    else if (4 == pstPara->ulParaNum)
    {
        (VOS_VOID)DIAG_LogReport( DIAG_GEN_LOG_MODULE(VOS_GetModemIDFromPid(ulPid), DIAG_MODE_1X, (enLevel)), \
                      (ulPid), pcFileName, pstPara->ulLine, "%s, %d, %d, %d, %d \r\n", (pcString), (VOS_INT32)(pstPara->lPara1), (VOS_INT32)(pstPara->lPara2), (VOS_INT32)(pstPara->lPara3), (VOS_INT32)(pstPara->lPara4) );
    }
    else
    {
    }
...
VOS_VOID CNAS_MNTN_LogPcReplayFixedContext(
    VOS_UINT32                          ulDataLength,
    VOS_UINT32                          ulPid,
    VOS_UINT8                          *pucData
)
{
    CNAS_MNTN_LOG_PC_REPLAY_CONTEXT_STRU                   *pstMsg = VOS_NULL_PTR;

    if (CNAS_MNTN_MSG_MAX_SIZE < ulDataLength)
    {
        CNAS_ERROR_LOG(ulPid, "CNAS_MNTN_LogPcReplayFixedContext: CNAS_MNTN_MSG_MAX_SIZE < ulDataLength");
        return;
    }

    pstMsg = (CNAS_MNTN_LOG_PC_REPLAY_CONTEXT_STRU*)PS_MEM_ALLOC(ulPid,
                             sizeof(CNAS_MNTN_LOG_PC_REPLAY_CONTEXT_STRU) + ulDataLength - 4);
    if (VOS_NULL_PTR == pstMsg)
    {
        return;
    }
    ...
    pstMsg->ulSenderCpuId   = VOS_LOCAL_CPUID;
    pstMsg->ulSenderPid     = ulPid;
    pstMsg->ulLength        = sizeof(CNAS_MNTN_LOG_PC_REPLAY_CONTEXT_STRU) - VOS_MSG_HEAD_LENGTH + ulDataLength - 4;
    pstMsg->enMsgId         = ID_CNAS_MNTN_LOG_PC_REPLAY_FIXED_CONTEXT;

    NAS_MEM_CPY_S(pstMsg->aucDataCtx, ulDataLength, pucData, ulDataLength);
...
VOS_VOID CNAS_MNTN_LogPcReplayPreCfgParaSwitchOn(
    VOS_UINT32                          ulDataLength,
    VOS_UINT32                          ulPid,
    VOS_UINT8                          *pucData
)
{
    ...
    pstMsg->ulSenderPid     = ulPid;
    pstMsg->ulLength        = sizeof(CNAS_MNTN_LOG_PC_REPLAY_CONTEXT_STRU) - VOS_MSG_HEAD_LENGTH + ulDataLength - 4;
    pstMsg->enMsgId         = ID_CNAS_MNTN_LOG_PC_REPLAY_PRE_CFG_PARA_SWITCH_ON;
