...
CNAS_CCB_NATIONAL_COUNTRY_CODE_INFO     g_astCnasCcbCountryList[] =
{
    {13568, 14335},     /* China 13568 */

    {25600, 26111},     /* China 25600 */

    {11296, 11311},     /* Macao/Macau : 11296 */
};
...
VOS_VOID CNAS_CCB_InitCcbCtx(VOS_VOID)
{
    ...
    CNAS_CCB_SetCasState(CNAS_CCB_1X_CAS_STATE_BUTT);

    CNAS_CCB_Init1xSysInfo();

    CNAS_CCB_InitMsSysCfgInfo();
    CNAS_CCB_InitHrpdNetWorkSrchInfo();

    CNAS_CCB_SetPrlSrcType(CNAS_CCB_PRL_TYPE_BUTT);
}
...
CNAS_CCB_1X_CALL_STATE_ENUM_UINT8 CNAS_CCB_Get1xVoiceCallState(VOS_VOID)
{
    ...
    for (ulIndex = 0; ulIndex < CNAS_CCB_1X_CALL_STATE_MAX_NUM; ulIndex++)
    {
        if (CNAS_CCB_1X_SO_TYPE_VOICE == CNAS_CCB_GetCcbCtxAddr()->ast1xCallState[ulIndex].en1xSoType)
        {
            return CNAS_CCB_GetCcbCtxAddr()->ast1xCallState[ulIndex].en1xCallState;
        }
    }
...
)
{
    ...
    for (ulIndex = 0; ulIndex < CNAS_CCB_1X_CALL_STATE_MAX_NUM; ulIndex++)
    {
        if (CNAS_CCB_1X_SO_TYPE_PS == CNAS_CCB_GetCcbCtxAddr()->ast1xCallState[ulIndex].en1xSoType)
        {
            return CNAS_CCB_GetCcbCtxAddr()->ast1xCallState[ulIndex].en1xCallState;
        }
    }
...
)
{
    ...
    for (ulIndex = 0; ulIndex < CNAS_CCB_1X_CALL_STATE_MAX_NUM; ulIndex++)
    {
        if (CNAS_CCB_1X_SO_TYPE_SMS== CNAS_CCB_GetCcbCtxAddr()->ast1xCallState[ulIndex].en1xSoType)
        {
            return CNAS_CCB_GetCcbCtxAddr()->ast1xCallState[ulIndex].en1xCallState;
        }
    }
...
)
{
    switch (en1xSoType)
    {
        case CNAS_CCB_1X_SO_TYPE_VOICE:
            CNAS_CCB_GetCcbCtxAddr()->ast1xCallState[0].en1xSoType    = en1xSoType;
            CNAS_CCB_GetCcbCtxAddr()->ast1xCallState[0].en1xCallState = en1xCallState;
            break;
        case CNAS_CCB_1X_SO_TYPE_SMS:
            CNAS_CCB_GetCcbCtxAddr()->ast1xCallState[1].en1xSoType    = en1xSoType;
            CNAS_CCB_GetCcbCtxAddr()->ast1xCallState[1].en1xCallState = en1xCallState;
            break;
        case CNAS_CCB_1X_SO_TYPE_PS:
            CNAS_CCB_GetCcbCtxAddr()->ast1xCallState[2].en1xSoType    = en1xSoType;
            CNAS_CCB_GetCcbCtxAddr()->ast1xCallState[2].en1xCallState = en1xCallState;
            break;
        case CNAS_CCB_1X_SO_TYPE_LOOPBACK:
            CNAS_CCB_GetCcbCtxAddr()->ast1xCallState[3].en1xSoType    = en1xSoType;
            CNAS_CCB_GetCcbCtxAddr()->ast1xCallState[3].en1xCallState = en1xCallState;
            break;
        case CNAS_CCB_1X_SO_TYPE_AGPS:
            CNAS_CCB_GetCcbCtxAddr()->ast1xCallState[4].en1xSoType    = en1xSoType;
            CNAS_CCB_GetCcbCtxAddr()->ast1xCallState[4].en1xCallState = en1xCallState;
            break;
...
{
    NAS_MEM_SET_S(&(CNAS_CCB_GetCcbNwInfoAddr()->st1xSysInfo),
                  sizeof(CNAS_CCB_1X_SYS_INFO_STRU),
                  0x7F,
                  sizeof(CNAS_CCB_1X_SYS_INFO_STRU));
...
VOS_UINT32 CNAS_CCB_IsCurCallStateIdle( VOS_VOID )
{
    ...
    for (ulIndex = 0; ulIndex < CNAS_CCB_1X_CALL_STATE_MAX_NUM; ulIndex++)
    {
        if (CNAS_CCB_1X_CALL_STATE_IDLE != pstCcbCtx->ast1xCallState[ulIndex].en1xCallState)
        {
            return VOS_FALSE;
        }
    }
...
CNAS_CCB_MS_SYS_CFG_INFO_STRU *CNAS_CCB_GetCurMsSysCfgAddr(VOS_VOID)
{
    ...
    return &(CNAS_CCB_GetCcbCtxAddr()->stMsSysCfgInfo);
}
...
{
    CNAS_CCB_MS_SYS_CFG_INFO_STRU      *pstMsSyscfg = VOS_NULL_PTR;
    VOS_UINT8                           i;
    VOS_UINT8                           ucMinRatNum;

    ucMinRatNum = NAS_COMM_MIN(ucRatNum, CNAS_CCB_MAX_RAT_NUM);

    pstMsSyscfg = CNAS_CCB_GetCurMsSysCfgAddr();

    for (i = 0; i < ucMinRatNum; i++)
    {
        pstMsSyscfg->aulRatPrio[i] = penRatMode[i];
    }

    pstMsSyscfg->ucRatNum = ucMinRatNum;
...
)
{
    ...
    pstMsSysCfg->ucRatNum = 0;

    NAS_MEM_SET_S(pstMsSysCfg->aulRatPrio,
                  sizeof(pstMsSysCfg->aulRatPrio) ,
                  VOS_RATMODE_BUTT,
                  sizeof(pstMsSysCfg->aulRatPrio));
...
{
    VOS_UINT16                                              usSid;
    CNAS_CCB_NATIONAL_COUNTRY_CODE_INFO                    *pstCnasCcbCountryList = VOS_NULL_PTR;
    VOS_UINT32                                              i;
    CNAS_CCB_1X_SYS_INFO_STRU                               stCcb1XSysInfo;
    VOS_UINT32                                              ulTblSize;

    NAS_MEM_SET_S(&stCcb1XSysInfo, sizeof(CNAS_CCB_1X_SYS_INFO_STRU), 0x7F, sizeof(CNAS_CCB_1X_SYS_INFO_STRU));
    usSid                   = CNAS_CCB_Get1xSysInfo()->usSid;
    pstCnasCcbCountryList   = g_astCnasCcbCountryList;
    ulTblSize               = (VOS_UINT32)(sizeof(g_astCnasCcbCountryList)/sizeof(CNAS_CCB_NATIONAL_COUNTRY_CODE_INFO));

    /* 当前未获取到1X系统信息 */
    if (0 == PS_MEM_CMP(CNAS_CCB_Get1xSysInfo(), &stCcb1XSysInfo, sizeof(CNAS_CCB_1X_SYS_INFO_STRU)))
    {
        return CNAS_CCB_NETWORK_PROPERTY_UNKNOWN;
    }

    /* 根据当前驻留的SID判断是否处于国内网络 */
    for (i = 0; i < ulTblSize; i++)
    {
        if ((usSid >= pstCnasCcbCountryList[i].usSidRangeStart)
         && (usSid <= pstCnasCcbCountryList[i].usSidRangeEnd))
        {
            return CNAS_CCB_NETWORK_PROPERTY_NATIONAL;
        }
    }

    return CNAS_CCB_NETWORK_PROPERTY_INTERNATIONAL;
}
...
{
    CNAS_CCB_GetCcbCtxAddr()->enEmcCbState = enEmcState;
...
{
    return CNAS_CCB_GetCcbCtxAddr()->enEmcCbState;
}


VOS_VOID CNAS_CCB_InitHrpdNetWorkSrchInfo(VOS_VOID)
{
    CNAS_CCB_HRPD_NETWORK_SRCH_INFO_STRU                   *pstHrpdNetWorkSrchInfo = VOS_NULL_PTR;

    pstHrpdNetWorkSrchInfo = CNAS_CCB_GetHrpdNetWorkSrchInfo();

    NAS_MEM_SET_S(pstHrpdNetWorkSrchInfo,
                  sizeof(CNAS_CCB_HRPD_NETWORK_SRCH_INFO_STRU),
                  0x00,
                  sizeof(CNAS_CCB_HRPD_NETWORK_SRCH_INFO_STRU));

    pstHrpdNetWorkSrchInfo->ucHrpdNetWorkSrchingFlg = VOS_FALSE;
...
{
    return &(CNAS_CCB_GetCcbCtxAddr()->stHrpdNetWorkSrchInfo);
}
...
)
{
    ...
    pstHrpdNetWorkSrchInfo = CNAS_CCB_GetHrpdNetWorkSrchInfo();
    pstHrpdNetWorkSrchInfo->ucHrpdNetWorkSrchingFlg = ucHrpdNetWorkSrchingFlg;
...
VOS_UINT8 CNAS_CCB_GetHrpdNetWorkSrchingFlg(VOS_VOID)
{
    ...
    return pstHrpdNetWorkSrchInfo->ucHrpdNetWorkSrchingFlg;
}
...
VOS_UINT32 CNAS_CCB_IsSyscfgSupportLte( VOS_VOID )
{
    ...
    for (i = 0 ; i < CNAS_CCB_GetCurMsSysCfgAddr()->ucRatNum; i++)
    {
        if (VOS_RATMODE_LTE == CNAS_CCB_GetCurMsSysCfgAddr()->aulRatPrio[i])
        {
            return VOS_TRUE;
        }
    }
...
CNAS_CCB_VERSION_ENUM_UINT8 CNAS_CCB_IsVersionSrlte( VOS_VOID )
{
    ...
    enVersionType = (CNAS_CCB_VERSION_ENUM_UINT8)(RRM_GetVersionType());

    return enVersionType;
}


VOS_VOID CNAS_CCB_Init1xCallState( VOS_VOID )
{
    CNAS_CCB_1X_SO_TYPE_ENUM_UINT8      enSoType;

    for (enSoType = CNAS_CCB_1X_SO_TYPE_VOICE; enSoType < CNAS_CCB_1X_SO_TYPE_BUTT; enSoType++)
    {
        CNAS_CCB_Set1xCallState(CNAS_CCB_1X_CALL_STATE_IDLE, enSoType);
    }
}
...
{
    CNAS_CCB_GetCcbCtxAddr()->enPrlSrcType = enPrlSrcType;
}

CNAS_CCB_PRL_SOURCE_TYPE_ENUM_UINT8 CNAS_CCB_GetPrlSrcType(VOS_VOID)
{
    return CNAS_CCB_GetCcbCtxAddr()->enPrlSrcType;
}
