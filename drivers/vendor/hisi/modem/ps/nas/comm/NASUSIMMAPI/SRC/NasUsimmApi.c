...
NAS_USIMM_SRVICES_TYPE_STRU g_NasUsimSrviceTypeTab[NAS_USIM_SVR_MAX_SERVICE] =
{
    ...
    {NAS_USIM_SVR_OP_CSG ,                  UICC_SVR_NONE,                  USIM_SVR_OP_CGS},               /* 90 */
...
{
    VOS_UINT32                          i;
    PLATAFORM_RAT_CAPABILITY_STRU      *pstModem0Plat = VOS_NULL_PTR;

    pstModem0Plat                       = NAS_SNDUSIMM_GetModem0PlatRat();

    PS_MEM_SET(pstModem0Plat, 0x00, sizeof(PLATAFORM_RAT_CAPABILITY_STRU));

    for (i = 0; i < pstModem0PlatRat->usRatNum; i++)
    {
        if (PLATFORM_RAT_BUTT != pstModem0PlatRat->aenRatList[i])
        {
            pstModem0Plat->aenRatList[pstModem0Plat->usRatNum] = pstModem0PlatRat->aenRatList[i];
            pstModem0Plat->usRatNum++;
        }
    }
