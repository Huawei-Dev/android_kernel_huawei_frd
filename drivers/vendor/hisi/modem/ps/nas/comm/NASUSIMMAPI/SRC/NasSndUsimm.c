...
PLATAFORM_RAT_CAPABILITY_STRU           g_stNasUsimmApiModem0PlatRat;      /* 开机时读取Modem0的平台能力并进行存储，开机后不再读取平台能力 */
...
PLATAFORM_RAT_CAPABILITY_STRU *NAS_SNDUSIMM_GetModem0PlatRat(
    VOS_VOID
)
{
    return &g_stNasUsimmApiModem0PlatRat;
}
...
{
    VOS_UINT16                          i;
    PLATAFORM_RAT_CAPABILITY_STRU      *pstModem0PlatRat = VOS_NULL_PTR;

    pstModem0PlatRat                    = &g_stNasUsimmApiModem0PlatRat;

    for (i = 0; i < pstModem0PlatRat->usRatNum; i++)
    {
        if (PLATFORM_RAT_1X == pstModem0PlatRat->aenRatList[i])
        {
            return VOS_TRUE;
        }

        if (PLATFORM_RAT_HRPD == pstModem0PlatRat->aenRatList[i])
        {
            return VOS_TRUE;
        }
    }
...
)
{
    ...
    PS_MEM_SET(pstMsg, 0x00, sizeof(USIMM_READFILE_REQ_STRU));
...
)
{
    ...
    PS_MEM_SET(pstMsg, 0x00, sizeof(USIMM_QUERYFILE_REQ_STRU));
