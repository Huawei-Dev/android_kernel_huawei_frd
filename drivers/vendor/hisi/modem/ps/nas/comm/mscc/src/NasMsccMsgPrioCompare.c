...
NAS_MSCC_MSG_COMPARE_STRU g_astMsccMsgPrioCompareWithSystemAcquireTbl[] =
{
    ...
    NAS_MSCC_COMPARE_TBL_ITEM(NAS_BuildEventType(VOS_PID_TIMER, TI_NAS_MSCC_1X_BSR_LTE_TIMER),
                             NAS_MSCC_CompareTi1xBsrLteTimerExpiredWithSystemAcquire),
...
#if (FEATURE_ON == FEATURE_UE_MODE_CDMA)
    NAS_MSCC_COMPARE_TBL_ITEM(NAS_BuildEventType(UEPS_PID_HSD, ID_HSD_MSCC_RF_AVAILABLE_IND),
                              NAS_MSCC_CompareHsdRfAvailableIndPrioWithBSR),

    NAS_MSCC_COMPARE_TBL_ITEM(NAS_BuildEventType(WUEPS_PID_MMC, ID_MMC_MSCC_RF_AVAILABLE_IND),
                              NAS_MSCC_CompareMmcRfAvaialbleIndPrioWithBSR),
#endif
...
)
{
    ...
    /* RF可用才需要进状态机触发流程-不可用一般都通过搜网回复NO RF携带上来,此条消息不需要处理 */
    ...
    /* CAS在1x电话结束后不立即释放RF资源,直到1x驻留后才释放RF资源,MSCC的保护无必要 */

    if ((NAS_MSCC_SYSACQ_STA_WAIT_HSD_POWER_SAVE_CNF  == ulState)
     || (NAS_MSCC_SYSACQ_STA_WAIT_MMC_POWER_SAVE_CNF  == ulState))
    {
        return NAS_MSCC_MSG_COMPARE_PRIO_RSLT_STORE;
    }

    /* LTE搜网过程中如做全频段搜或pref band搜,1X有服务的时候先打断LTE的搜素;
       否则缓存 */
    if (NAS_MSCC_SYSACQ_STA_WAIT_MMC_PLMN_SEARCH_CNF == ulState)
    {
        if (VOS_TRUE == NAS_MSCC_Is1xNormalService())
        {
            ...
            if ((NAS_MSCC_PIF_PLMN_SEARCH_HISTORY                != NAS_MSCC_GetSysAcqLteSrchType_SysAcq())
             && ((NAS_MSCC_SYS_ACQ_SCENE_AVAILABLE_TIMER_EXPIRED == enSysAcqScene)
              || (NAS_MSCC_SYS_ACQ_SCENE_HRPD_LOST_NO_RF         == enSysAcqScene)))
            {
                return NAS_MSCC_MSG_COMPARE_PRIO_RSLT_ABORT;
            }
        }
...
)
{
    ...
    if ((NAS_MSCC_SYSACQ_STA_WAIT_MMC_POWER_SAVE_CNF == ulState)
...
NAS_MSCC_MSG_COMPARE_PRIO_RSLT_ENUM_UINT8 NAS_MSCC_CompareTi1xBsrLteTimerExpiredWithSystemAcquire(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    ...
    /* 搜DO的时候非快搜直接打断；否则缓存 */
    if ((NAS_MSCC_SYSACQ_STA_WAIT_HSD_ACQUIRE_CNF    == ulState)
     || (NAS_MSCC_SYSACQ_STA_WAIT_HSD_POWER_SAVE_CNF == ulState))
    {
        if (NAS_MSCC_SYS_ACQ_SCENE_QUICK_SEARCH_HRPD_MRUO == NAS_MSCC_GetCLSysAcqScene_SysAcq())
        {
            return NAS_MSCC_MSG_COMPARE_PRIO_RSLT_STORE;
        }
...
)
{
    ...
    /* 等待MMC的power save cnf或初搜状态,时间太久,因此直接打断,在DO下快搜MRU0 */
    if ((NAS_MSCC_SYSACQ_STA_WAIT_MMC_POWER_SAVE_CNF == ulState)
     || (NAS_MSCC_SYSACQ_STA_WAIT_INIT_LOC_INFO_IND  == ulState))
    {
        return NAS_MSCC_MSG_COMPARE_PRIO_RSLT_ABORT;
    }

    /* 已经在LTE的搜索流程中或即将开始LTE的搜索流程,等待LTE搜索完成后,处理缓存 */
    if ((NAS_MSCC_SYSACQ_STA_WAIT_HSD_POWER_SAVE_CNF   == ulState)
     || (NAS_MSCC_SYSACQ_STA_WAIT_MMC_PLMN_SEARCH_CNF  == ulState)
     || (NAS_MSCC_SYSACQ_STA_WAIT_MMC_RF_AVAILABLE_IND == ulState))
    {
        return NAS_MSCC_MSG_COMPARE_PRIO_RSLT_STORE;
    }
...
NAS_MSCC_MSG_COMPARE_PRIO_RSLT_ENUM_UINT8 NAS_MSCC_CompareMmcRfAvaialbleIndPrioWithBSR(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    ...
    if ((NAS_MSCC_BSR_STA_WAIT_MMC_POWER_SAVE_CNF                == ulState)
     || (NAS_MSCC_BSR_STA_WAIT_MMC_POWER_SAVE_CNF_REACQ_HRPD     == ulState)
     || (NAS_MSCC_BSR_STA_WAIT_HSD_SYSTEM_ACQUIRE_CNF_REACQ_HRPD == ulState)
     || (NAS_MSCC_BSR_STA_WAIT_HSD_SYSTEM_ACQUIRE_CNF            == ulState)
     || (NAS_MSCC_BSR_STA_WAIT_HSD_POWER_SAVE_CNF                == ulState)
     || (NAS_MSCC_BSR_STA_WAIT_HSD_POWER_SAVE_CNF_REACQ_LTE      == ulState))
    {
        return NAS_MSCC_MSG_COMPARE_PRIO_RSLT_STORE;
    }
...
NAS_MSCC_MSG_COMPARE_PRIO_RSLT_ENUM_UINT8 NAS_MSCC_CompareHsdRfAvailableIndPrioWithBSR(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    ...
    if ((NAS_MSCC_BSR_STA_WAIT_MMC_POWER_SAVE_CNF               == ulState)
     || (NAS_MSCC_BSR_STA_WAIT_MMC_POWER_SAVE_CNF_REACQ_HRPD    == ulState)
     || (NAS_MSCC_BSR_STA_WAIT_MMC_PLMN_SEARCH_CNF_REACQ_LTE    == ulState)
     || (NAS_MSCC_BSR_STA_WAIT_MMC_PLMN_SEARCH_CNF              == ulState)
     || (NAS_MSCC_BSR_STA_WAIT_HSD_POWER_SAVE_CNF               == ulState)
     || (NAS_MSCC_BSR_STA_WAIT_HSD_POWER_SAVE_CNF_REACQ_LTE     == ulState))
    {
        return NAS_MSCC_MSG_COMPARE_PRIO_RSLT_STORE;
