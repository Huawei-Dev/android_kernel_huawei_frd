...
)
{
    ...
    *pulOutBsrTimerLenMilliSec = 0;
    ...
    ucBsrPhaseNum              = NAS_MSCC_GetCurBsrTimerPhase();
    ...
    if (VOS_FALSE == NAS_MSCC_IsLteSysTypeExistedInMsplTable(pstMatchedMsplTable))
    {
        ...
        if (VOS_TRUE == pstSysAcqBsrCtrl->ucBsrTimerActivateFlag)
        {
            *pulOutBsrTimerLenMilliSec = pstSysAcqBsrCtrl->astBsrTimerInfo[ucBsrPhaseNum].ulBsrTimerLenWithNoMatchedMsplRec;
        }
        else
        {
            /* NV关闭使用默认值 */
            *pulOutBsrTimerLenMilliSec = TI_NAS_MSCC_DEFAULT_BSR_TIMER_LEN_WITH_UNMATCHED_MSPL_REC;
        }
    ...
    if (VOS_TRUE == pstSysAcqBsrCtrl->ucBsrTimerActivateFlag)
    {
        *pulOutBsrTimerLenMilliSec = pstSysAcqBsrCtrl->astBsrTimerInfo[ucBsrPhaseNum].ulBsrTimerLen;
        ucGetBsrTimerLenRslt       = VOS_TRUE;
    }
...
)
{
    ...
    /* 不再需要对sPriMoreRecPos进行>=NAS_MSCC_MAX_NUM_MSPL_REC的检查保护, 函数入口的检查可以保证 */

    /* 从下标0开始到MORE下标结束的所有记录比当前记录高， 依次填入列表 */
    /*lint -save -e661 -specific(-e661)*/
    /*lint -save -e662 -specific(-e662)*/
    sMsplRecordNum = NAS_COMM_MIN(sPriMoreRecPos, (NAS_MSCC_MAX_NUM_MSPL_REC - 1));

    for (sLoop = 0; sLoop <= sMsplRecordNum; sLoop++)
    {
        if (VOS_FALSE == pstInMsplTable->astMsplRec[sLoop].ucIsValid)
        {
            continue;
...
)
{
    ...
    NAS_MSCC_PIF_SIM_PLMN_WITH_RAT_STRU                    *pstTempOutPlmnList  = VOS_NULL_PTR;

    pstTempOutPlmnList                                      = pstOutPlmnList;
    ...
    *pusOutPlmnCnt = pstSimEhPlmnInfo->ucEhPlmnNum + pstUserPlmnInfo->ucUserPlmnNum
    ...
    {
        PS_MEM_CPY(pstTempOutPlmnList, pstSimEhPlmnInfo->astEhPlmnInfo,
                   pstSimEhPlmnInfo->ucEhPlmnNum * sizeof(NAS_MSCC_PIF_SIM_PLMN_WITH_RAT_STRU));
    }

    if (pstUserPlmnInfo->ucUserPlmnNum > 0)
    {
        pstPlmnListStartPos = pstTempOutPlmnList + pstSimEhPlmnInfo->ucEhPlmnNum;
