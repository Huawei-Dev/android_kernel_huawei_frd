...
)
{
    ...
    pstAcqCnf->ucIsViaModem                  = pstMsg->ucIsViaModem;
...
VOS_VOID NAS_MSCC_SndMmaCsgSpecSearchCnf(
    MMC_MSCC_CSG_SPEC_SEARCH_CNF_STRU  *pstMsg
)
{
    MSCC_MMA_CSG_SPEC_SEARCH_CNF_STRU  *pstCsgSpecSearchCnf = VOS_NULL_PTR;

    pstCsgSpecSearchCnf = (MSCC_MMA_CSG_SPEC_SEARCH_CNF_STRU *)PS_ALLOC_MSG(UEPS_PID_MSCC,
                              sizeof(MSCC_MMA_CSG_SPEC_SEARCH_CNF_STRU)- VOS_MSG_HEAD_LENGTH);

    if( VOS_NULL_PTR == pstCsgSpecSearchCnf )
    {
        NAS_WARNING_LOG(UEPS_PID_MSCC, "NAS_MSCC_SndMmaCsgSpecSearchCnf():ERROR:Memory Alloc Error");
        return;
    }

    PS_MEM_SET(((VOS_UINT8*)pstCsgSpecSearchCnf) + VOS_MSG_HEAD_LENGTH, 0, sizeof(MSCC_MMA_CSG_SPEC_SEARCH_CNF_STRU) - VOS_MSG_HEAD_LENGTH);

    /* 填充消息头 */
    pstCsgSpecSearchCnf->stMsgHeader.ulMsgName           = ID_MSCC_MMA_CSG_SPEC_SEARCH_CNF;
    pstCsgSpecSearchCnf->stMsgHeader.ulSenderCpuId       = VOS_LOCAL_CPUID;
    pstCsgSpecSearchCnf->stMsgHeader.ulSenderPid         = UEPS_PID_MSCC;
    pstCsgSpecSearchCnf->stMsgHeader.ulReceiverCpuId     = VOS_LOCAL_CPUID;
    pstCsgSpecSearchCnf->stMsgHeader.ulReceiverPid       = WUEPS_PID_MMA;
    pstCsgSpecSearchCnf->stMsgHeader.ulLength            = sizeof(MSCC_MMA_CSG_SPEC_SEARCH_CNF_STRU) - VOS_MSG_HEAD_LENGTH;
    pstCsgSpecSearchCnf->enResult                        = pstMsg->enResult;

    NAS_MSCC_LogMsgInfo((MSG_HEADER_STRU*)pstCsgSpecSearchCnf);

    PS_SEND_MSG(UEPS_PID_MSCC, pstCsgSpecSearchCnf);
...
VOS_VOID NAS_MSCC_SndMmaCsgSpecSearchAbortCnf(
    MMC_MSCC_CSG_SPEC_SEARCH_ABORT_CNF_STRU                *pstMsg
)
{
    MSCC_MMA_CSG_SPEC_SEARCH_ABORT_CNF_STRU                *pstCsgSpecSearchAbortCnf = VOS_NULL_PTR;

    pstCsgSpecSearchAbortCnf = (MSCC_MMA_CSG_SPEC_SEARCH_ABORT_CNF_STRU *)PS_ALLOC_MSG(UEPS_PID_MSCC,
                              sizeof(MSCC_MMA_CSG_SPEC_SEARCH_ABORT_CNF_STRU)- VOS_MSG_HEAD_LENGTH);

    if( VOS_NULL_PTR == pstCsgSpecSearchAbortCnf )
    {
        NAS_WARNING_LOG(UEPS_PID_MSCC, "NAS_MSCC_SndMmaCsgSpecSearchAbortCnf():ERROR:Memory Alloc Error");
        return;
    }

    PS_MEM_SET(((VOS_UINT8*)pstCsgSpecSearchAbortCnf) + VOS_MSG_HEAD_LENGTH, 0, sizeof(MSCC_MMA_CSG_SPEC_SEARCH_ABORT_CNF_STRU) - VOS_MSG_HEAD_LENGTH);


    /* 填充消息头 */
    pstCsgSpecSearchAbortCnf->stMsgHeader.ulMsgName           = ID_MSCC_MMA_CSG_SPEC_SEARCH_ABORT_CNF;
    pstCsgSpecSearchAbortCnf->stMsgHeader.ulSenderCpuId       = VOS_LOCAL_CPUID;
    pstCsgSpecSearchAbortCnf->stMsgHeader.ulSenderPid         = UEPS_PID_MSCC;
    pstCsgSpecSearchAbortCnf->stMsgHeader.ulReceiverCpuId     = VOS_LOCAL_CPUID;
    pstCsgSpecSearchAbortCnf->stMsgHeader.ulReceiverPid       = WUEPS_PID_MMA;
    pstCsgSpecSearchAbortCnf->stMsgHeader.ulLength            = sizeof(MSCC_MMA_CSG_SPEC_SEARCH_ABORT_CNF_STRU) - VOS_MSG_HEAD_LENGTH;

    NAS_MSCC_LogMsgInfo((MSG_HEADER_STRU*)pstCsgSpecSearchAbortCnf);

    PS_SEND_MSG(UEPS_PID_MSCC, pstCsgSpecSearchAbortCnf);

    return;
}

VOS_VOID NAS_MSCC_SndMmaCsgIdHomeNodeBNameInd(
    MMC_MSCC_CSG_ID_HOME_NODEB_NAME_IND_STRU               *pstMsg
)
{
    MSCC_MMA_CSG_ID_HOME_NODEB_NAME_IND_STRU               *pstCsgIdHomeNodeBNameInd = VOS_NULL_PTR;

    pstCsgIdHomeNodeBNameInd = (MSCC_MMA_CSG_ID_HOME_NODEB_NAME_IND_STRU *)PS_ALLOC_MSG(UEPS_PID_MSCC,
                              sizeof(MSCC_MMA_CSG_ID_HOME_NODEB_NAME_IND_STRU)- VOS_MSG_HEAD_LENGTH);

    if( VOS_NULL_PTR == pstCsgIdHomeNodeBNameInd )
    {
        NAS_WARNING_LOG(UEPS_PID_MSCC, "NAS_MSCC_SndMmaCsgIdHomeNodeBNameInd():ERROR:Memory Alloc Error");
        return;
    }

    PS_MEM_SET(((VOS_UINT8*)pstCsgIdHomeNodeBNameInd) + VOS_MSG_HEAD_LENGTH, 0, sizeof(MSCC_MMA_CSG_ID_HOME_NODEB_NAME_IND_STRU) - VOS_MSG_HEAD_LENGTH);

    /* 填充消息头 */
    pstCsgIdHomeNodeBNameInd->stMsgHeader.ulMsgName           = ID_MSCC_MMA_CSG_ID_HOME_NODEB_NAME_IND;
    pstCsgIdHomeNodeBNameInd->stMsgHeader.ulSenderCpuId       = VOS_LOCAL_CPUID;
    pstCsgIdHomeNodeBNameInd->stMsgHeader.ulSenderPid         = UEPS_PID_MSCC;
    pstCsgIdHomeNodeBNameInd->stMsgHeader.ulReceiverCpuId     = VOS_LOCAL_CPUID;
    pstCsgIdHomeNodeBNameInd->stMsgHeader.ulReceiverPid       = WUEPS_PID_MMA;
    pstCsgIdHomeNodeBNameInd->stMsgHeader.ulLength            = sizeof(MSCC_MMA_CSG_ID_HOME_NODEB_NAME_IND_STRU) - VOS_MSG_HEAD_LENGTH;

    if (VOS_NULL_PTR != pstMsg)
    {
        pstCsgIdHomeNodeBNameInd->stPlmnId                    = pstMsg->stPlmnId;
        pstCsgIdHomeNodeBNameInd->ulCsgId                     = pstMsg->ulCsgId;
        pstCsgIdHomeNodeBNameInd->ucHomeNodeBNameLen          = pstMsg->ucHomeNodeBNameLen;
        PS_MEM_CPY(pstCsgIdHomeNodeBNameInd->aucHomeNodeBName,
            pstMsg->aucHomeNodeBName,
            pstMsg->ucHomeNodeBNameLen);
    }

    NAS_MSCC_LogMsgInfo((MSG_HEADER_STRU*)pstCsgIdHomeNodeBNameInd);

    PS_SEND_MSG(UEPS_PID_MSCC, pstCsgIdHomeNodeBNameInd);
...
VOS_VOID NAS_MSCC_SndMmaSrvAcqCnf(
    NAS_MSCC_PIF_SRV_ACQ_RESULT_ENUM_UINT8                  enAcqResult
)
{
    ...
    /* 填充消息内容 */
    pstSrvAcqCnf->enResult                      = enAcqResult;
...
VOS_VOID NAS_MSCC_SndMmaSystemAcquireEndInd(
    ...
    MSCC_MMA_CL_SYSTEM_ACQUIRE_FAIL_REASON_ENUM_UINT32      enCLAcqFailReason
)
{
    ...
    pstMsg->enCLSysAcqFailReason         = enCLAcqFailReason;
...
)
{
    ...
    NAS_MEM_CPY_S(pstMsg->aucSectorId, HSD_MSCC_SUBNET_ID_MAX_LENGTH, pstMsccOhmMsg->aucSectorId, HSD_MSCC_SUBNET_ID_MAX_LENGTH);
...
VOS_VOID NAS_MSCC_SndMma1xUeStatusInd(
...
{
    MSCC_MMA_1X_UE_STATUS_IND_STRU     *pstUeStatus = VOS_NULL_PTR;

    /* 申请消息包 */
    pstUeStatus = (MSCC_MMA_1X_UE_STATUS_IND_STRU *)PS_ALLOC_MSG(UEPS_PID_MSCC,
                                                                 sizeof(MSCC_MMA_1X_UE_STATUS_IND_STRU) - VOS_MSG_HEAD_LENGTH);

    if (VOS_NULL_PTR == pstUeStatus)
    {
        NAS_ERROR_LOG(UEPS_PID_MSCC, "NAS_MSCC_SndMmaUeStatusInd():ERROR:Memory Alloc Error for NAS_MSCC_SndMmaUeStatusInd ");
        return;
    }

    PS_MEM_SET(((VOS_UINT8*)pstUeStatus) + VOS_MSG_HEAD_LENGTH,
               0,
               sizeof(MSCC_MMA_1X_UE_STATUS_IND_STRU) - VOS_MSG_HEAD_LENGTH);

    pstUeStatus->stMsgHeader.ulSenderCpuId     = VOS_LOCAL_CPUID;
    pstUeStatus->stMsgHeader.ulSenderPid       = UEPS_PID_MSCC;
    pstUeStatus->stMsgHeader.ulReceiverCpuId   = VOS_LOCAL_CPUID;
    pstUeStatus->stMsgHeader.ulReceiverPid     = WUEPS_PID_MMA;
    pstUeStatus->stMsgHeader.ulLength          = sizeof(MSCC_MMA_1X_UE_STATUS_IND_STRU) - VOS_MSG_HEAD_LENGTH;
    pstUeStatus->stMsgHeader.ulMsgName         = ID_MSCC_MMA_1X_UE_STATUS_IND;

    pstUeStatus->enUeMainState                 = enUeMainState;
    pstUeStatus->enUeSubState                  = enUeSubState;
    pstUeStatus->enHighPrioServiceType         = enUe1xServiceType;
...
VOS_VOID NAS_MSCC_SndMmaImsRegCnf(
    NAS_MSCC_PIF_IMS_REG_RESULT_ENUM_UINT32                 enResult
)
{
    MSCC_MMA_IMS_REG_CNF_STRU          *pstRegCnf = VOS_NULL_PTR;

    /* 申请消息包 */
    pstRegCnf = (MSCC_MMA_IMS_REG_CNF_STRU *)NAS_MSCC_ALLOC_MSG_WITH_HDR(
                    sizeof(MSCC_MMA_IMS_REG_CNF_STRU));
    if (VOS_NULL_PTR == pstRegCnf)
    {
        NAS_ERROR_LOG(UEPS_PID_MSCC, "NAS_MSCC_SndMmaImsRegCnf():ERROR:Memory Alloc Error for pstRegCnf ");
        return;
    }

    /* 清空消息内容 */
    NAS_MSCC_CLR_MSG_ENTITY(pstRegCnf);

    /* 填写消息头 */
    NAS_MSCC_CFG_MSG_HDR(pstRegCnf, WUEPS_PID_MMA, ID_MSCC_MMA_IMS_REG_CNF);

    /* 填写消息内容 */
    pstRegCnf->enResult                      = enResult;

    NAS_MSCC_LogMsgInfo((MSG_HEADER_STRU*)pstRegCnf);

    /* 发送消息 */
    NAS_MSCC_SEND_MSG(pstRegCnf);

    return;
}


VOS_VOID NAS_MSCC_SndMmaImsDeregCnf(
    NAS_MSCC_PIF_IMS_DEREG_RESULT_ENUM_UINT32               enResult
)
{
    MSCC_MMA_IMS_DEREG_CNF_STRU        *pstDeregCnf = VOS_NULL_PTR;

    /* 申请消息包 */
    pstDeregCnf = (MSCC_MMA_IMS_DEREG_CNF_STRU *)NAS_MSCC_ALLOC_MSG_WITH_HDR(
                        sizeof(MSCC_MMA_IMS_DEREG_CNF_STRU));
    if (VOS_NULL_PTR == pstDeregCnf)
    {
        NAS_ERROR_LOG(UEPS_PID_MSCC, "NAS_MSCC_SndMmaImsDeregCnf():ERROR:Memory Alloc Error for pstDeregCnf ");
        return;
    }

    /* 清空消息内容 */
    NAS_MSCC_CLR_MSG_ENTITY(pstDeregCnf);

    /* 填写消息头 */
    NAS_MSCC_CFG_MSG_HDR(pstDeregCnf, WUEPS_PID_MMA, ID_MSCC_MMA_IMS_DEREG_CNF);

    /* 填写消息内容 */
    pstDeregCnf->enResult                      = enResult;

    NAS_MSCC_LogMsgInfo((MSG_HEADER_STRU*)pstDeregCnf);

    /* 发送消息 */
    NAS_MSCC_SEND_MSG(pstDeregCnf);
...
VOS_VOID NAS_MSCC_SndMmaImsDomainCfgSetCnf(
     NAS_MSCC_PIF_IMS_DOMAIN_CFG_RESULT_ENUM_UINT32     enImsDomainResult
)
{
    MSCC_MMA_IMS_DOMAIN_CFG_SET_CNF_STRU       *pstMsg = VOS_NULL_PTR;

    /* 申请内存  */
    pstMsg = (MSCC_MMA_IMS_DOMAIN_CFG_SET_CNF_STRU *)NAS_MSCC_ALLOC_MSG_WITH_HDR(sizeof(MSCC_MMA_IMS_DOMAIN_CFG_SET_CNF_STRU));

    if ( VOS_NULL_PTR == pstMsg )
    {
        NAS_ERROR_LOG(UEPS_PID_MSCC, "NAS_MSCC_SndMmaImsDomainCfgSetCnf:ERROR: Memory Alloc Error for pMsg");
        return;
    }
    ...
    /* 填写消息头 */
    NAS_MSCC_CFG_MSG_HDR(pstMsg, WUEPS_PID_MMA, ID_MSCC_MMA_IMS_DOMAIN_CFG_SET_CNF);

    /* 填写消息内容 */
    pstMsg->enImsDomainCfgResult = enImsDomainResult;
...
VOS_VOID NAS_MSCC_SndMmaRoamImsSupportSetCnf(
     NAS_MSCC_PIF_ROAM_IMS_SUPPORT_RESULT_ENUM_UINT32     enRoamImsSupportResult
)
{
    MSCC_MMA_ROAM_IMS_SUPPORT_SET_CNF_STRU       *pstMsg = VOS_NULL_PTR;

    /* 申请内存  */
    pstMsg = (MSCC_MMA_ROAM_IMS_SUPPORT_SET_CNF_STRU *)NAS_MSCC_ALLOC_MSG_WITH_HDR(sizeof(MSCC_MMA_ROAM_IMS_SUPPORT_SET_CNF_STRU));

    if ( VOS_NULL_PTR == pstMsg )
    {
        NAS_ERROR_LOG(UEPS_PID_MSCC, "NAS_MSCC_SndMmaRoamImsSupportSetCnf:ERROR: Memory Alloc Error for pMsg");
        return;
    }
    ...
    /* 填写消息头 */
    NAS_MSCC_CFG_MSG_HDR(pstMsg, WUEPS_PID_MMA, ID_MSCC_MMA_ROAM_IMS_SUPPORT_SET_CNF);

    /* 填写消息内容 */
    pstMsg->enRoamImsSupportResult = enRoamImsSupportResult;
...
VOS_VOID NAS_MSCC_SndMmaPrlHeaderInfoInd(
    NAS_MSCC_PRL_HEADER_INFO_IND_STRU                      *pstMsg
)
{
    MSCC_MMA_PRL_HEADER_INFO_IND_STRU  *pstPrlHeaderInfoInd = VOS_NULL_PTR;

    /* 申请消息包 */
    pstPrlHeaderInfoInd = (MSCC_MMA_PRL_HEADER_INFO_IND_STRU *)PS_ALLOC_MSG(UEPS_PID_MSCC,
                    sizeof(MSCC_MMA_PRL_HEADER_INFO_IND_STRU) - VOS_MSG_HEAD_LENGTH);

    if (VOS_NULL_PTR == pstPrlHeaderInfoInd)
    {
        NAS_ERROR_LOG(UEPS_PID_MSCC, "NAS_MSCC_SndMma1xSystemServiceInfoInd():ERROR:Memory Alloc Error for pstPrefPlmnCnf ");
        return;
    }

    PS_MEM_SET(((VOS_UINT8*)pstPrlHeaderInfoInd) + VOS_MSG_HEAD_LENGTH, 0, sizeof(MSCC_MMA_PRL_HEADER_INFO_IND_STRU) - VOS_MSG_HEAD_LENGTH);

    /* 填充消息头 */
    pstPrlHeaderInfoInd->stMsgHeader.ulSenderCpuId     = VOS_LOCAL_CPUID;
    pstPrlHeaderInfoInd->stMsgHeader.ulSenderPid       = UEPS_PID_MSCC;
    pstPrlHeaderInfoInd->stMsgHeader.ulReceiverCpuId   = VOS_LOCAL_CPUID;
    pstPrlHeaderInfoInd->stMsgHeader.ulReceiverPid     = WUEPS_PID_MMA;
    pstPrlHeaderInfoInd->stMsgHeader.ulLength          = sizeof(MSCC_MMA_PRL_HEADER_INFO_IND_STRU) - VOS_MSG_HEAD_LENGTH;
    pstPrlHeaderInfoInd->stMsgHeader.ulMsgName         = ID_MSCC_MMA_PRL_HEADER_INFO_IND;

    /* 填充消息内容 */
    PS_MEM_CPY(&(pstPrlHeaderInfoInd->stPrlHeader), &(pstMsg->stPrlHeader), sizeof(NAS_MSCC_PRL_HEADER_INFO_STRU));

    NAS_MSCC_LogMsgInfo((MSG_HEADER_STRU*)pstPrlHeaderInfoInd);

    /* 发送消息 */
    PS_SEND_MSG(UEPS_PID_MSCC, pstPrlHeaderInfoInd);

}


VOS_VOID NAS_MSCC_SndMmaMsplMlplInfoInd(
    MSCC_MMA_MSPL_MLPL_INFO_SOURCE_ENUM_UINT8               enInfoSource
)
{
    MSCC_MMA_MSPL_MLPL_HEADER_INFO_STRU                    *pstSndMsg   = VOS_NULL_PTR;
    NAS_MSCC_MSPL_STRU                                     *pstMsccMspl = VOS_NULL_PTR;
    NAS_MSCC_MLPL_STRU                                     *pstMsccMlpl = VOS_NULL_PTR;

    /* 申请消息包 */
    pstSndMsg = (MSCC_MMA_MSPL_MLPL_HEADER_INFO_STRU *)PS_ALLOC_MSG(UEPS_PID_MSCC,
                        sizeof(MSCC_MMA_MSPL_MLPL_HEADER_INFO_STRU) - VOS_MSG_HEAD_LENGTH);

    if (VOS_NULL_PTR == pstSndMsg)
    {
        NAS_ERROR_LOG(UEPS_PID_MSCC, "NAS_MSCC_SndMmaMsplMlplInfoInd():ERROR:Memory Alloc Error!");
        return;
    }
    ...
    pstSndMsg->stMsgHeader.ulReceiverCpuId   = VOS_LOCAL_CPUID;
    pstSndMsg->stMsgHeader.ulReceiverPid     = WUEPS_PID_MMA;
    pstSndMsg->stMsgHeader.ulLength          = sizeof(MSCC_MMA_MSPL_MLPL_HEADER_INFO_STRU) - VOS_MSG_HEAD_LENGTH;
    pstSndMsg->stMsgHeader.ulMsgName         = ID_MSCC_MMA_MSPL_MLPL_HEADER_INFO_IND;

    pstMsccMspl = NAS_MSCC_GetMsplInfoAddr();
    pstMsccMlpl = NAS_MSCC_GetMlplInfoAddr();

    pstSndMsg->enInfoSource             = enInfoSource;

    if (MSCC_MMA_MSPL_MLPL_INFO_SOURCE_BUTT != enInfoSource)
    {
        /* mspl文件头信息 */
        pstSndMsg->stMsplInfo.usMsplSize    = pstMsccMspl->stMsplHdr.usMsplSize;
        pstSndMsg->stMsplInfo.ucCurrProtRev = pstMsccMspl->stMsplHdr.ucCurrProtRev;
        pstSndMsg->stMsplInfo.usMsplVerId   = pstMsccMspl->stMsplHdr.usMsplVerId;
        pstSndMsg->stMsplInfo.ucNumMsplId   = pstMsccMspl->stMsplHdr.ucNumMsplId;

        /* mlpl文件头信息 */
        pstSndMsg->stMlplInfo.usMlplSize      = pstMsccMlpl->stMlplHdr.usMlplSize;
        pstSndMsg->stMlplInfo.ucCurrProtRev   = pstMsccMlpl->stMlplHdr.ucCurrProtRev;
        pstSndMsg->stMlplInfo.usMlplVerId     = pstMsccMlpl->stMlplHdr.usMlplVerId;
        pstSndMsg->stMlplInfo.ucMlplId        = pstMsccMlpl->stMlplHdr.ucMlplId;
        pstSndMsg->stMlplInfo.ucNumMlplRec    = pstMsccMlpl->stMlplHdr.ucNumMlplRec;
    }
