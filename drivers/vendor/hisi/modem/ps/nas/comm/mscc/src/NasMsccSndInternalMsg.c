...
)
{
    ...
    pstMsg->ucLteNoRfFlag               = ucLteNoRfFlag;
    pstMsg->ucHrpdNoRfFlag              = ucHrpdNoRfFlag;
...
)
{
    ...
    pstMsg->ucHrpdNoRfFlg               = NAS_MSCC_GetHrpdNoRfFlg_BSR();
...
{
    NAS_MSCC_MSCC_POWER_SAVE_REQ_STRU      *pstMsg = VOS_NULL_PTR;

    pstMsg  = (NAS_MSCC_MSCC_POWER_SAVE_REQ_STRU *)PS_MEM_ALLOC(UEPS_PID_MSCC,
                                                                sizeof(NAS_MSCC_MSCC_POWER_SAVE_REQ_STRU));
    ...
    pstMsg->ulReceiverCpuId             = VOS_LOCAL_CPUID;
    pstMsg->ulReceiverPid               = UEPS_PID_MSCC;
    pstMsg->enMsgId                     = ID_NAS_MSCC_MSCC_POWER_SAVE_REQ;
    pstMsg->ulLength                    = sizeof(NAS_MSCC_MSCC_POWER_SAVE_REQ_STRU) - VOS_MSG_HEAD_LENGTH;

    pstMsg->usRatNum                    = usRatNum;

    if (pstMsg->usRatNum > NAS_MSCC_PIF_MAX_RAT_NUM)
    {
        pstMsg->usRatNum                = NAS_MSCC_PIF_MAX_RAT_NUM;
    }

    NAS_MEM_CPY_S(&pstMsg->aenRat[0],
