...
)
{
    ...
    NAS_MSCC_ResetMsplInfo(stMsplParseCtx.pstMsplInfo, NAS_MSCC_MAX_NUM_MSPL_ID);
...
)
{
    ...
    NAS_MSCC_ResetMlplInfo(stMlplParseCtx.pstMlplInfo, NAS_MSCC_MAX_NUM_MLPL_REC);
...
)
{
    ...
    if (pstMlplRec->stLocParamVal.ucNumSysLocTag > NAS_MSCC_MAX_NUM_SYS_LOC_TAG)
    {
        pstMlplRec->stLocParamVal.ucNumSysLocTag = NAS_MSCC_MAX_NUM_SYS_LOC_TAG;
    }

    ulBuffLen = sizeof(NAS_MSCC_MLPL_RECORD_STRU) - sizeof(NAS_MSCC_MLPL_SYS_LOC_TAG_STRU) * (NAS_MSCC_MAX_NUM_SYS_LOC_TAG - pstMlplRec->stLocParamVal.ucNumSysLocTag);

    pstTempMlplRec = (NAS_MSCC_MLPL_RECORD_STRU *)PS_MEM_ALLOC(UEPS_PID_MSCC, ulBuffLen);

    if (VOS_NULL_PTR == pstTempMlplRec )
    {
        NAS_ERROR_LOG(UEPS_PID_MSCC, "NAS_MSCC_UpdateMlplRecInfo:Malloc failure!");
        return;
    }

    NAS_MEM_CPY_S(pstTempMlplRec, ulBuffLen, pstMlplRec, ulBuffLen);

    pstMlplParseCtx->pstMlplInfo->apstMlplRec[ucIndex] = pstTempMlplRec;

    if (VOS_TRUE == pstTempMlplRec->ucIsRecValid)
    {
     ...
     && (VOS_FALSE == pstMlplParseCtx->pstMlplInfo->ucIsContainDefRec))
    {
        ...
        pstMlplParseCtx->pstMlplInfo->ucMsplIdxInFirstDefRec = pstTempMlplRec->ucMsplIndex;
...
)
{
    ...
    NAS_NORMAL_LOG(UEPS_PID_MSCC,"NAS_MSCC_ResetMsplInfo:Entered");

    ucIndex = 0;
    while (ucIndex < ucMaxIndex)
    {
        if (VOS_NULL_PTR != pstMsplInfo->apstMsplId[ucIndex])
        {
...
)
{
    ...
    NAS_NORMAL_LOG(UEPS_PID_MSCC,"NAS_MSCC_ResetMlplInfo:Entered");

    while (ucIndex < ucMaxIndex)
    {
        if (VOS_NULL_PTR != pstMlplInfo->apstMlplRec[ucIndex])
        {
...
VOS_VOID NAS_MSCC_AddHardCodeDefaultMlpl(VOS_VOID)
{
    NAS_MSCC_MLPL_STRU                                      *pstMlpl;
    NAS_MSCC_MSPL_STRU                                      *pstMspl;
    NAS_MSCC_MSPL_ID_STRU                                   *pstTempMsplId;
    NAS_MSCC_MLPL_RECORD_STRU                               *pstTempMlplRec;
    ...
    /* 如果MSPL表的记录已经达到最大值255，则不添加default记录 */
    if ((NAS_MSCC_MAX_NUM_MSPL_ID  == pstMspl->stMsplHdr.ucNumMsplId)
     || (NAS_MSCC_MAX_NUM_MLPL_REC == pstMlpl->stMlplHdr.ucNumMlplRec))
    {
        NAS_ERROR_LOG(UEPS_PID_MSCC, "NAS_MSCC_BuildHardCodeDefaultMlpl:Already shoot the MAX NUM.");
    ...
    if (VOS_NULL_PTR == pstTempMlplRec)
    {
        NAS_ERROR_LOG(UEPS_PID_MSCC, "NAS_MSCC_BuildHardCodeDefaultMlpl:pstTempMlplRec Malloc failure.");
    ...
    if (VOS_NULL_PTR == pstTempMsplId)
    {
        NAS_ERROR_LOG(UEPS_PID_MSCC, "NAS_MSCC_BuildHardCodeDefaultMlpl:pstTempMsplId Malloc failure.");
    ...
    pstMlpl->ucIsContainDefRec      = VOS_TRUE;

    PS_MEM_SET(pstTempMsplId, 0, sizeof(NAS_MSCC_MSPL_ID_STRU));
    PS_MEM_SET(pstTempMlplRec, 0, sizeof(NAS_MSCC_MLPL_RECORD_STRU));

    pstTempMsplId->ucIsMsplIdValid = VOS_TRUE;
    pstTempMsplId->ucNumMsplRec    = NAS_MSCC_MAX_NUM_DEFAULT_HARD_CODE_MSPL_REC;

    /* 第一条记录 */
    pstTempMsplId->astMsplRec[0].ucIsValid              = VOS_TRUE;
    pstTempMsplId->astMsplRec[0].enSysType              = NAS_MSCC_MLPL_MSPL_SYS_TYPE_LTE;
    pstTempMsplId->astMsplRec[0].enPriClass             = NAS_MSCC_MSPL_SYS_PRI_CLASS_HOME_AND_PREF;
    pstTempMsplId->astMsplRec[0].enSysPri               = NAS_MSCC_MSPL_SYS_PRI_MORE;
    pstTempMsplId->astMsplRec[0].enHigherPriSrchTime    = NAS_MSCC_MSPL_HIGHER_PRI_SRCH_TIME_1_MIN;

    /* 第二条记录 */
    pstTempMsplId->astMsplRec[1].ucIsValid              = VOS_TRUE;
    pstTempMsplId->astMsplRec[1].enSysType              = NAS_MSCC_MLPL_MSPL_SYS_TYPE_CDMA2000_HRPD;
    pstTempMsplId->astMsplRec[1].enPriClass             = NAS_MSCC_MSPL_SYS_PRI_CLASS_HOME_AND_PREF;
    pstTempMsplId->astMsplRec[1].enSysPri               = NAS_MSCC_MSPL_SYS_PRI_MORE;
    pstTempMsplId->astMsplRec[1].enHigherPriSrchTime    = NAS_MSCC_MSPL_HIGHER_PRI_SRCH_TIME_1_MIN;

    /* 第三条记录 */
    pstTempMsplId->astMsplRec[2].ucIsValid              = VOS_TRUE;
    pstTempMsplId->astMsplRec[2].enSysType              = NAS_MSCC_MLPL_MSPL_SYS_TYPE_3GPP_AI;
    pstTempMsplId->astMsplRec[2].enPriClass             = NAS_MSCC_MSPL_SYS_PRI_CLASS_ANY;
    pstTempMsplId->astMsplRec[2].enSysPri               = NAS_MSCC_MSPL_SYS_PRI_SAME;
    pstTempMsplId->astMsplRec[2].enHigherPriSrchTime    = NAS_MSCC_MSPL_HIGHER_PRI_SRCH_TIME_1_MIN;

    /* 第四条记录 */
    pstTempMsplId->astMsplRec[3].ucIsValid              = VOS_TRUE;
    pstTempMsplId->astMsplRec[3].enSysType              = NAS_MSCC_MLPL_MSPL_SYS_TYPE_CDMA2000_AI;
    pstTempMsplId->astMsplRec[3].enPriClass             = NAS_MSCC_MSPL_SYS_PRI_CLASS_ANY;
    pstTempMsplId->astMsplRec[3].enSysPri               = NAS_MSCC_MSPL_SYS_PRI_SAME;
    pstTempMsplId->astMsplRec[3].enHigherPriSrchTime    = NAS_MSCC_MSPL_HIGHER_PRI_SRCH_TIME_1_MIN;

    pstMlpl->ucMsplIdxInFirstDefRec = NAS_MSCC_GetFisrtUnusedMsplId();
    pstMspl->stMsplHdr.ucNumMsplId++;

    pstTempMsplId->ucMsplId        = pstMlpl->ucMsplIdxInFirstDefRec;
    pstMspl->apstMsplId[pstMspl->stMsplHdr.ucNumMsplId - 1] = pstTempMsplId;

    /* 增加一条default MLPL，该MLPL指向新添加的MSPL*/
    pstTempMlplRec->ucIsRecValid = VOS_TRUE;
    pstTempMlplRec->ucMsplIndex  = pstTempMsplId->ucMsplId;
    pstMlpl->apstMlplRec[pstMlpl->stMlplHdr.ucNumMlplRec] = pstTempMlplRec;

    pstMlpl->stMlplHdr.ucNumMlplRec++;

    return;
}


VOS_UINT8 NAS_MSCC_GetFisrtUnusedMsplId(VOS_VOID)
{

    NAS_MSCC_MSPL_STRU                                      *pstMspl;
    VOS_UINT8                                                ucLoop_i;
    VOS_UINT8                                                ucLoop_j;
    VOS_UINT8                                                ucIsFoundUnusedMsplId;
    VOS_UINT8                                                ucUnusedMsplId;

    pstMspl = NAS_MSCC_GetMsplInfoAddr();
    ucUnusedMsplId        = 0;

    for (ucLoop_i = 0 ; ucLoop_i < NAS_MSCC_MAX_NUM_MSPL_ID; ucLoop_i++)
    {
        ucIsFoundUnusedMsplId = VOS_FALSE;

        for (ucLoop_j = 0 ; ucLoop_j < pstMspl->stMsplHdr.ucNumMsplId; ucLoop_j++)
        {
            if (ucLoop_i == pstMspl->apstMsplId[ucLoop_j]->ucMsplId)
            {
                ucIsFoundUnusedMsplId = VOS_TRUE;
                break;
            }
        }

        if (VOS_FALSE == ucIsFoundUnusedMsplId)
        {
            ucUnusedMsplId = ucLoop_i;
            break;
        }
    }

    return ucUnusedMsplId;
}
