...
/* MSCC临终遗言的长度，底软分配的dump空间大小，随意改动会导致复位，改动需要NAS评审 */
#define NAS_MSCC_MNTN_SAVE_EXC_LOG_LENGTH_MODEM0            (4096)
#define NAS_MSCC_MNTN_SAVE_EXC_LOG_LENGTH_MODEM1            (4096)
#define NAS_MSCC_MNTN_SAVE_EXC_LOG_LENGTH_MODEM2            (4096)

#define NAS_MSCC_MNTN_DUMP_BEGIN_TAG                        (0xAA55AA55)
#define NAS_MSCC_MNTN_DUMP_END_TAG                          (0xAA55AA55)
...
typedef VOS_VOID  (*pNasMsccErrLogReportFunc)(
    OM_ERR_LOG_REPORT_REQ_STRU         *pstOmErrLOgRptReq
);
typedef VOS_VOID (*pNasMsccInitErrLogCcbCtxFunc)(VOS_VOID);

typedef struct
{
    VOS_UINT32                          ulAlarmID;
    pNasMsccErrLogReportFunc            pfuncNasMsccErrLogReport;
    pNasMsccInitErrLogCcbCtxFunc        pfuncNasMsccInitErrLogCcbCtx;
}NAS_MSCC_ERRLOG_REPORT_PROC_STRU;
#endif
#endif
...
typedef struct
{
    VOS_UINT32                          ulMsccBeginTag;
    NAS_MSCC_LOG_MSG_STATUS_STRU        stMsccLogMsgState;
    NAS_MSCC_SIM_STATUS_STRU            stSimStatus;
    VOS_UINT8                           ucIsCardChanged;
    VOS_UINT8                           aucReserved[3];
    NAS_MSCC_MS_CFG_INFO_STRU           stMsCfgInfo;
    NAS_MSCC_NW_INFO_STRU               stNwInfo;
    NAS_MSCC_POWER_SAVE_CTRL_CTX_STRU   stPowerSaveCtrl;
    NAS_MSCC_SYS_ACQ_CTRL_CTX_STRU      stSysAcqCtrl;
    VOS_UINT32                          ulMsccEndTag;
}NAS_MSCC_MNTN_SAVE_EXC_LOG_STRU;
