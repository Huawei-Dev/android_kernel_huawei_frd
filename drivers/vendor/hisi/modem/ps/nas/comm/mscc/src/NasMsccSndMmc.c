...
VOS_VOID NAS_MSCC_SndMmcImsVoiceCapNotify(
    ...
    VOS_UINT8                           ucRcvServiceChangeIndFlag
)
{
    ...
    pstMsg->ucRcvServiceChangeIndFlag  = ucRcvServiceChangeIndFlag;
...
VOS_VOID NAS_MSCC_SndMmcCsgSpecSearchReq(
    MMA_MSCC_CSG_SPEC_SEARCH_REQ_STRU  *pstCsgSpecSrchMsg
)
{
    MSCC_MMC_CSG_SPEC_SEARCH_REQ_STRU   *pstMsg = VOS_NULL_PTR;

    /* 申请内存  */
    pstMsg = (MSCC_MMC_CSG_SPEC_SEARCH_REQ_STRU *)PS_ALLOC_MSG(UEPS_PID_MSCC,
                           sizeof(MSCC_MMC_CSG_SPEC_SEARCH_REQ_STRU) - VOS_MSG_HEAD_LENGTH);

    if ( VOS_NULL_PTR == pstMsg )
    {
        NAS_ERROR_LOG(UEPS_PID_MSCC, "NAS_MSCC_SndMmcCsgSpecSearchReq:ERROR: Memory Alloc Error for pMsg");
        return;
    }
    ...
    pstMsg->stMsgHeader.ulLength         = sizeof(MSCC_MMC_CSG_SPEC_SEARCH_REQ_STRU) - VOS_MSG_HEAD_LENGTH;
    pstMsg->stMsgHeader.ulMsgName        = ID_MSCC_MMC_CSG_SPEC_SEARCH_REQ;
    pstMsg->ulCsgId                      = pstCsgSpecSrchMsg->ulCsgId;
    PS_MEM_CPY(&pstMsg->stPlmnWithRat, &pstCsgSpecSrchMsg->stPlmnWithRat, sizeof(pstMsg->stPlmnWithRat));
...
VOS_VOID NAS_MSCC_SndMmcCsgSpecSearchAbortReq(
    MMA_MSCC_CSG_SPEC_SEARCH_ABORT_REQ_STRU                *pstCsgSpecSearchAbortReq
)
{
    MSCC_MMC_CSG_SPEC_SEARCH_ABORT_REQ_STRU                *pstMsg = VOS_NULL_PTR;

    /* 申请内存  */
    pstMsg = (MSCC_MMC_CSG_SPEC_SEARCH_ABORT_REQ_STRU *)PS_ALLOC_MSG(UEPS_PID_MSCC,
                           sizeof(MSCC_MMC_CSG_SPEC_SEARCH_ABORT_REQ_STRU) - VOS_MSG_HEAD_LENGTH);

    if ( VOS_NULL_PTR == pstMsg )
    {
        NAS_ERROR_LOG(UEPS_PID_MSCC, "NAS_MSCC_SndMmcCsgSpecSearchAbortReq:ERROR: Memory Alloc Error for pMsg");
        return;
    }
    ...
    pstMsg->stMsgHeader.ulSenderCpuId    = VOS_LOCAL_CPUID;
    pstMsg->stMsgHeader.ulSenderPid      = UEPS_PID_MSCC;
    pstMsg->stMsgHeader.ulLength         = sizeof(MSCC_MMC_CSG_SPEC_SEARCH_ABORT_REQ_STRU) - VOS_MSG_HEAD_LENGTH;
    pstMsg->stMsgHeader.ulMsgName        = ID_MSCC_MMC_CSG_SPEC_SEARCH_ABORT_REQ;
...
)
{
    ...
    /* 给mmc发的搜网请求中初搜plmm信息为赋初值为无效的全f */
    pstMsg->stInitAcqPlmnInfo.enRat                 = NAS_MSCC_PIF_NET_RAT_TYPE_BUTT;
    pstMsg->stInitAcqPlmnInfo.stInitAcqPlmnId.ulMcc = NAS_MSCC_INVALID_MCC;
    pstMsg->stInitAcqPlmnInfo.stInitAcqPlmnId.ulMnc = NAS_MSCC_INVALID_MNC;
...
)
{
    ...
    NAS_MEM_CPY_S(&pstMsg->stImsCallInfo, sizeof(pstMsg->stImsCallInfo), &pstImsSrvInfoNtf->stImsCallInfo, sizeof(pstImsSrvInfoNtf->stImsCallInfo));
...
)
{
    ...
    pstMsg->ul1xCampedMcc                = ul1xCampedMcc;
...
VOS_VOID NAS_MSCC_SndMmcCurGeoInfoNtf(
    VOS_UINT32                          ulIsPowerOn,
    VOS_UINT32                          ulPlmnNum,
    NAS_MSCC_PIF_PLMN_ID_STRU          *pstGeoPlmn
)
{
    MSCC_MMC_CURR_GEO_INFO_NTF_STRU    *pstMsg = VOS_NULL_PTR;

    /* 申请内存  */
    pstMsg = (MSCC_MMC_CURR_GEO_INFO_NTF_STRU *)PS_ALLOC_MSG(UEPS_PID_MSCC,
                           sizeof(MSCC_MMC_CURR_GEO_INFO_NTF_STRU) - VOS_MSG_HEAD_LENGTH);

    if (VOS_NULL_PTR == pstMsg)
    {
        NAS_ERROR_LOG(UEPS_PID_MSCC, "NAS_MSCC_SndMmcCurGeoInfoNtf:ERROR: Memory Alloc Error for pMsg");
        return;
    }
    ...
    pstMsg->stMsgHeader.ulSenderCpuId    = VOS_LOCAL_CPUID;
    pstMsg->stMsgHeader.ulSenderPid      = UEPS_PID_MSCC;
    pstMsg->stMsgHeader.ulLength         = sizeof(MSCC_MMC_CURR_GEO_INFO_NTF_STRU) - VOS_MSG_HEAD_LENGTH;
    pstMsg->stMsgHeader.ulMsgName        = ID_MSCC_MMC_CURR_GEO_INFO_NTF;
    pstMsg->ulIsPowerOn                  = ulIsPowerOn;

    if (ulPlmnNum > MSCC_MMC_MAX_PLMN_ID_NUM)
    {
        ulPlmnNum = MSCC_MMC_MAX_PLMN_ID_NUM;
    }

    pstMsg->ulPlmnNum   = ulPlmnNum;

    PS_MEM_CPY(pstMsg->astPlmnId, pstGeoPlmn, ulPlmnNum * sizeof(NAS_MSCC_PIF_PLMN_ID_STRU));
...
VOS_VOID NAS_MSCC_SndMmcEplmnInfoNtf(
    NAS_MSCC_EQUPLMN_INFO_STRU         *pstEplmnList
)
{
    MSCC_MMC_EPLMN_INFO_NTF_STRU       *pstMsg = VOS_NULL_PTR;

    /* 申请内存  */
    pstMsg = (MSCC_MMC_EPLMN_INFO_NTF_STRU *)PS_ALLOC_MSG(UEPS_PID_MSCC,
                           sizeof(MSCC_MMC_EPLMN_INFO_NTF_STRU) - VOS_MSG_HEAD_LENGTH);

    if (VOS_NULL_PTR == pstMsg)
    {
        NAS_ERROR_LOG(UEPS_PID_MSCC, "NAS_MSCC_SndMmcEplmnInfoNtf:ERROR: Memory Alloc Error for pMsg");
        return;
    }
    ...
    pstMsg->stMsgHeader.ulSenderCpuId    = VOS_LOCAL_CPUID;
    pstMsg->stMsgHeader.ulSenderPid      = UEPS_PID_MSCC;
    pstMsg->stMsgHeader.ulLength         = sizeof(MSCC_MMC_EPLMN_INFO_NTF_STRU) - VOS_MSG_HEAD_LENGTH;
    pstMsg->stMsgHeader.ulMsgName        = ID_MSCC_MMC_EPLMN_INFO_NTF;

    pstMsg->ulEplmnNum = pstEplmnList->ucEquPlmnNum;

    if (pstEplmnList->ucEquPlmnNum > MSCC_MMC_MAX_EPLMN_ID_NUM)
    {
        pstMsg->ulEplmnNum = MSCC_MMC_MAX_EPLMN_ID_NUM;
    }

    PS_MEM_CPY(&(pstMsg->astEplmnList[0]), &(pstEplmnList->astEquPlmnAddr[0]), (pstMsg->ulEplmnNum)* sizeof(NAS_MSCC_PIF_PLMN_ID_STRU));
