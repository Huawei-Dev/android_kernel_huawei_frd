...
)
{
    ...
    if ((VOS_TRUE == NAS_MSCC_GetHrpdActiveFlg())
     && (NAS_MSCC_PIF_NET_RAT_TYPE_LTE == pstMmcSysInfo->enCurNetWork)
     && (NAS_MSCC_FSM_CL_INTERSYS != enCurrFsmId))
    {

        NAS_WARNING_LOG(UEPS_PID_MSCC, "NAS_MSCC_RcvMmcSysInfoInd_PreProc:Dual Master Mode!");

        NAS_MSCC_SoftReBoot(NAS_MSCC_REBOOT_SCENE_MSCC_DUAL_MASTER_MODE_CAMP_ON_LTE, UEPS_PID_MSCC);
...
)
{
     ...
     || (NAS_MSCC_PIF_SRVDOMAIN_CS_PS == pstRegResultInd->enCnDomainId))
    {
         ...
         || (VOS_FALSE == ulUeSupportIms))
        {
            ...
            /* 通知MMC当前IMS voice不可用 */
            NAS_MSCC_SndMmcImsVoiceCapNotify(MSCC_MMC_IMS_VOICE_CAPABILITY_NOT_AVAIL, MSCC_MMC_PERSISTENT_BEARER_STATE_NOT_EXIST, VOS_TRUE);
...
)
{
    ...
    if (VOS_TRUE == NAS_MSCC_IsPlatformSupportCdma1X())
    {
        if ((VOS_TRUE == pstSrvStatusInd->bitOpSrvSta)
         && (NAS_MSCC_SERVICE_STATUS_NORMAL_SERVICE == NAS_MSCC_ConvertMmcServiceStatusToMsccFormat(pstSrvStatusInd->enServiceStatus)))
        {
            ulMcc     = NAS_MSCC_Get3gppSysInfoAddr()->stPlmnId.ulMcc;
            ulRatType = NAS_MSCC_TransferMsccToVosRat(NAS_MSCC_Get3gppSysInfoAddr()->enSysMode);

            NAS_MSCC_SndXsdNormalServiceInd(ulMcc, ulRatType);
        }
    }
#endif
...
#if (FEATURE_ON == FEATURE_UE_MODE_CDMA)
    /* attahc PS域-不论LTE是否支持-并通知enable LTE */
    if ((NAS_MSCC_PIF_ATTACH_TYPE_GPRS      == pstAttachReq->enAttachType)
     || (NAS_MSCC_PIF_ATTACH_TYPE_GPRS_IMSI == pstAttachReq->enAttachType))
    {
        NAS_MSCC_SetEpsAttachAllowFlg(VOS_TRUE);

        if (VOS_TRUE == NAS_MSCC_IsNeedEnableLteUpOnAttachPs())
        {
            NAS_MSCC_SndHsdEnableLteNtf();

            NAS_MSCC_SetLteCapbilityStatus(NAS_MSCC_LTE_CAPABILITY_STATUS_REENABLE_NOTIFIED_AS);

            /* LTE处于从模且不能进行多模搜网的时候需要在attach ps后触发搜一次lte */
            if (VOS_TRUE == NAS_MSCC_IsNeedAcqLteUponAttachPs())
            {
...
VOS_UINT32 NAS_MSCC_RcvMmcCsgSpecSearchCnf_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    MMC_MSCC_CSG_SPEC_SEARCH_CNF_STRU  *pstCsgSpecSearchCnf = VOS_NULL_PTR;

    pstCsgSpecSearchCnf = (MMC_MSCC_CSG_SPEC_SEARCH_CNF_STRU *)pstMsg;

    /* 转发消息到MMA */
    NAS_MSCC_SndMmaCsgSpecSearchCnf(pstCsgSpecSearchCnf);

    return VOS_TRUE;
}

VOS_UINT32 NAS_MSCC_RcvMmcCsgSpecSearchAbortCnf_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    /* 转发消息到MMA */
    NAS_MSCC_SndMmaCsgSpecSearchAbortCnf((MMC_MSCC_CSG_SPEC_SEARCH_ABORT_CNF_STRU *)pstMsg);

    return VOS_TRUE;
}

VOS_UINT32 NAS_MSCC_RcvMmaCsgSpecSearchReq_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    MMC_MSCC_CSG_SPEC_SEARCH_CNF_STRU  stCsgSpecSearchCnfMsg;

    stCsgSpecSearchCnfMsg.enResult = NAS_MSCC_PIF_CSG_SPEC_SEARCH_RESULT_REJ;
    ...
    if (VOS_TRUE == NAS_MSCC_IsSpecRatSupported(NAS_MSCC_PIF_NET_RAT_TYPE_HRPD))
    {
        ...
        /* 回复指定搜网rej */
        NAS_MSCC_SndMmaCsgSpecSearchCnf(&stCsgSpecSearchCnfMsg);
        return VOS_TRUE;
    }

    /* 转发该消息给MMC模块 */
    NAS_MSCC_SndMmcCsgSpecSearchReq((MMA_MSCC_CSG_SPEC_SEARCH_REQ_STRU *)pstMsg);
    return VOS_TRUE;
}


VOS_UINT32 NAS_MSCC_RcvMmaCsgSpecSearchAbortReq_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    MMC_MSCC_CSG_SPEC_SEARCH_ABORT_CNF_STRU                 stCsgSpecSearchAbortCnf;

    PS_MEM_SET(&stCsgSpecSearchAbortCnf, 0, sizeof(stCsgSpecSearchAbortCnf));

    /* GUL mode or current active mode is LTE, transfer user resel
       request to MMC module */
    if (VOS_TRUE == NAS_MSCC_IsSpecRatSupported(NAS_MSCC_PIF_NET_RAT_TYPE_HRPD))
    {
        NAS_MSCC_SndMmaCsgSpecSearchAbortCnf(&stCsgSpecSearchAbortCnf);

        return VOS_TRUE;
    }

    /* 转发该消息给MMC模块 */
    NAS_MSCC_SndMmcCsgSpecSearchAbortReq((MMA_MSCC_CSG_SPEC_SEARCH_ABORT_REQ_STRU *)pstMsg);

    return VOS_TRUE;
}


VOS_UINT32 NAS_MSCC_RcvMmcCsgIdHomeNodeBNameInd_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    /* 转发消息到MMA */
    NAS_MSCC_SndMmaCsgIdHomeNodeBNameInd((MMC_MSCC_CSG_ID_HOME_NODEB_NAME_IND_STRU *)pstMsg);
...
#if ( (FEATURE_ON == FEATURE_CL_INTERWORK) && (FEATURE_MULTI_MODEM == FEATURE_ON) )
    /* 通知MTC退出power save */
    MTC_SetPowerSaveInfo(UEPS_PID_MSCC, TAF_MTC_POWER_SAVE_EXIT);
#endif
...
)
{
    ...
    enLteSrchType = NAS_MSCC_PIF_PLMN_SEARCH_BUTT;
#endif
#endif
    ...
    PS_MEM_SET(NAS_MSCC_GetClEplmnInfoAddr(), 0x0, sizeof(NAS_MSCC_EQUPLMN_INFO_STRU));

    if (NAS_MSCC_PIF_PLMN_SELECTION_START == pstPlmnStartInd->enPlmnSeleStartFlag)
    {
        if (VOS_TRUE == NAS_MSCC_GetHrpdActiveFlg())
        {
            NAS_WARNING_LOG(UEPS_PID_MSCC, "NAS_MSCC_RcvMmcPlmnSelectStartInd_PreProc:Dual Master Mode!");

            NAS_MSCC_SoftReBoot(NAS_MSCC_REBOOT_SCENE_MSCC_DUAL_MASTER_MODE_ACQUIRE_LTE, UEPS_PID_MSCC);
        ...
        {
            enLteSrchType = NAS_MSCC_GetSysAcqLteSrchType_SysAcq();
        }

        /* 只在CL模式下统计,在LTE一轮搜网开始时，保存CL搜网开始信息 */

        /* 当前HRPD或LTE处于服务态，表示当前处于服务，已经上报过Recovery */
        if ((NAS_MSCC_SERVICE_STATUS_NORMAL_SERVICE == NAS_MSCC_GetCurr3gppPsServiceStatus())
         || (NAS_MSCC_SERVICE_STATUS_NORMAL_SERVICE == NAS_MSCC_GetCurrHrpdServiceStatus()))
        {
            NAS_ERRLOG_CCB_InitClOosSearchInfo();
        }

        NAS_MSCC_SaveErrLogCcbSysAcqStartInfo(VOS_RATMODE_LTE, enLteSrchType);
#endif
#endif
    }
...
)
{
    ...
    if (VOS_FALSE == pstRfAvail->ucRfAvailFlg)
    {
        return VOS_TRUE;
    }
...
)
{
    ...
    if (NAS_MSCC_FSM_SYS_ACQ == enCurrFsmId)
    {
        ...
        {
            pstMmcMsccAcqCnf->ucIsViaModem = VOS_FALSE;
    ...
    pstMmcMsccAcqCnf->ucIsViaModem = VOS_TRUE;
...
#if ( (FEATURE_ON == FEATURE_CL_INTERWORK) && (FEATURE_MULTI_MODEM == FEATURE_ON) )
        /* 通知MTC进入powerSave */
        MTC_SetPowerSaveInfo(UEPS_PID_MSCC, TAF_MTC_POWER_SAVE);
#endif
...
)
{
    ...
    {
        NAS_MSCC_Set1xMoCallConnectingFlag(VOS_TRUE);
...
)
{
    ...
    if (NAS_MSCC_PIF_CDMA_CALL_TYPE_DO_NORMAL_DATA_CALL == pstCdmaMoCallRedialSysAcqNtf->enCallType)
    {
        ...
        */
        /* HRPD不支持的时候，直接返回 */
        if (VOS_FALSE == NAS_MSCC_IsNeedAcqHrpd())
        {
        ...
        {
            NAS_MSCC_SndMmaDataCallRedialSysAcqInd(NAS_MSCC_PIF_ACQUIRED_RESULT_1X_NO_SERVICE, VOS_FALSE);
...
)
{
    ...
    {
        NAS_MSCC_Set1xMoCallConnectingFlag(VOS_FALSE);
...
)
{
    ...
    VOS_UINT32                                              ul1xCampedMcc;

    /* 有服务时,TCH态AS不携带SERVICE INFO,只有频点和SID NID信息，因此此处直接返回 */
    if ((PS_FALSE == pst1xSysSrvInfoInd->stSysSrvInfo.enServiceInfoIncl)
     && (NAS_MSCC_PIF_SERVICE_STATUS_NO_SERVICE != pst1xSysSrvInfoInd->stSysSrvInfo.enServiceStatus))
    {
        NAS_WARNING_LOG(UEPS_PID_MSCC, "NAS_MSCC_ProcCLAssociatedInfoNtfWith1xLocInfo_PreProc:service info is invalid!");
    ...
    NAS_MSCC_SetAllowSrchLteSysPriClass(enLteSysPriClass);

    ul1xCampedMcc = NAS_MSCC_INVALID_MCC;

    /* 有服务且网络携带了MCC才携带MCC给MMC模块 */
    if ((PS_TRUE == pst1xSysSrvInfoInd->stSysSrvInfo.enServiceInfoIncl)
     && (NAS_MSCC_PIF_SERVICE_STATUS_NO_SERVICE != pst1xSysSrvInfoInd->stSysSrvInfo.enServiceStatus))
    {
        ul1xCampedMcc = pst1xSysSrvInfoInd->stSysSrvInfo.stServiceInfo.ulMcc;
    }

    NAS_MSCC_SndMmcCLAssociatedInfoNtf(ul1xCampedMcc, enLteSysPriClass, ucCurAllowSrchLteFlg);
    ...
    */
    ...
    {
        NAS_WARNING_LOG(UEPS_PID_MSCC, "NAS_MSCC_ProcCLAssociatedInfoNtfWith1xLocInfo_PreProc:lte is not allow srched so power save lte!");
     ...
     && (VOS_FALSE == ucPreAllowSrchLteFlg))
    {
        ...
        {
            NAS_WARNING_LOG(UEPS_PID_MSCC, "NAS_MSCC_ProcCLAssociatedInfoNtfWith1xLocInfo_PreProc:lte changed to allow srch from not allow srch so acq lte!");
...
)
{
    ...
    NAS_MSCC_NO_CARD_INIT_SEARCH_WAIT_TYPE_ENUM_UINT32      enNoCardInitSearchWaitType;
    NAS_MSCC_PIF_PLMN_ID_STRU                               astGeoPlmnList[MSCC_MMC_MAX_PLMN_ID_NUM];
    NAS_MSCC_EQUPLMN_INFO_STRU                              stEplmnList;
    NAS_MSCC_EQUPLMN_INFO_STRU                             *pstEplmnList;

    NAS_MEM_SET_S(&astGeoPlmnList[0],
                  sizeof(NAS_MSCC_PIF_PLMN_ID_STRU) * MSCC_MMC_MAX_PLMN_ID_NUM,
                  0x00,
                  sizeof(NAS_MSCC_PIF_PLMN_ID_STRU) * MSCC_MMC_MAX_PLMN_ID_NUM);
    ...
    /* 判定1x位置有效才进行MCC的判定 */
    if (((PS_TRUE == pst1xSysSrvInfoInd->stSysSrvInfo.enServiceInfoIncl)
      && ((pstOld1xSysInfo->ulMcc != pst1xSysSrvInfoInd->stSysSrvInfo.stServiceInfo.ulMcc)
    ...
    if ((VOS_TRUE == ucIs1xAvail)
     && ((PS_TRUE == pst1xSysSrvInfoInd->stSysSrvInfo.enServiceInfoIncl)
      && (pstOld1xSysInfo->ulMcc != pst1xSysSrvInfoInd->stSysSrvInfo.stServiceInfo.ulMcc)))
    {
        astGeoPlmnList[0].ulMcc = pst1xSysSrvInfoInd->stSysSrvInfo.stServiceInfo.ulMcc;
        astGeoPlmnList[0].ulMnc = NAS_MSCC_INVALID_MNC;

        NAS_MSCC_SndMmcCurGeoInfoNtf(VOS_TRUE, 1, astGeoPlmnList);
    }
    ...
    if (VOS_TRUE == NAS_MSCC_IsPlatformSupportLte())
    {
        ...
        {
            pstEplmnList = NAS_MSCC_GetClEplmnInfoAddr();

            PS_MEM_SET(&stEplmnList, 0x00, sizeof(NAS_MSCC_EQUPLMN_INFO_STRU));

            NAS_MSCC_GetAllowSrchedClEplmnlist(&stEplmnList);

            if (VOS_TRUE == NAS_MSCC_IsEplmnListChanged(pstEplmnList, &stEplmnList))
            {
                PS_MEM_CPY(pstEplmnList, &stEplmnList, sizeof(NAS_MSCC_EQUPLMN_INFO_STRU));

                NAS_MSCC_SndMmcEplmnInfoNtf(&stEplmnList);
            }
        }
    }
...
)
{
    ...
    if (HSD_MSCC_SESSION_NEG_RESULT_SUCC == pstSessionInfo->enRslt)
    {
        ...
        if (VOS_TRUE == NAS_MSCC_IsPlatformSupportCdma1X())
        {
            ulMcc = NAS_MSCC_GetHrpdSysInfoAddr()->ulMcc;

            NAS_MSCC_SndXsdNormalServiceInd(ulMcc, VOS_RATMODE_HRPD);
        }
        ...
        if (VOS_TRUE == pstSessionInfo->ucIsEhrpdSupport)
        {
            NAS_MSCC_SetCurHrpdSessionType(NAS_MSCC_HRPD_SESSION_TYPE_EHRPD);
        }
        else
        {
            NAS_MSCC_SetCurHrpdSessionType(NAS_MSCC_HRPD_SESSION_TYPE_HRPD);
        }
...
#if (FEATURE_PTM == FEATURE_ON)
        NAS_ERRLOG_CCB_SetHrpdOosCause(NAS_ERR_LOG_MSCC_PIF_ACQUIRED_RESULT_ENUM_BUTT + pstSessionInfo->enRslt);
#endif
...
)
{
    ...
    if (VOS_TRUE == NAS_MSCC_Get3gppActiveFlg())
    {
        NAS_WARNING_LOG(UEPS_PID_MSCC, "NAS_MSCC_RcvHsdOverheadMsgInd_PreProc:Dual Master Mode!");

        NAS_MSCC_SoftReBoot(NAS_MSCC_REBOOT_SCENE_MSCC_DUAL_MASTER_MODE_CAMP_ON_DO, UEPS_PID_MSCC);
    ...
    {
        if (NAS_MSCC_Get1xSysInfoAddr()->ulMcc != NAS_MSCC_GetHrpdSysInfoAddr()->ulMcc)
        {
            astGeoPlmnList[0].ulMcc = NAS_MSCC_GetHrpdSysInfoAddr()->ulMcc;
            astGeoPlmnList[0].ulMnc = NAS_MSCC_INVALID_MNC;
...
)
{
    ...
    if (HSD_MSCC_HRPD_CAS_STATUS_ENUM_CONN == pstHsdMsccCasStatusInd->enCasStatus)
    {
        ...
        {
            NAS_MSCC_SndXsdLteHrpdConnInfoInd(VOS_TRUE,
                                              NAS_MSCC_GetLteEpsConnExistFlg());
        }
    ...
    else
    {
        ...
        if (VOS_TRUE ==  NAS_MSCC_GetHrpdConnExistFlg())
        {
            NAS_MSCC_SndXsdLteHrpdConnInfoInd(VOS_FALSE,
                                              NAS_MSCC_GetLteEpsConnExistFlg());
        }
...
VOS_UINT32 NAS_MSCC_IsNeedStartBsrTryTimer_PreProc(
    NAS_MSCC_FSM_ID_ENUM_UINT32         enCurrFsmId,
    VOS_UINT8                           ucHrpdConnExistFlg,
    VOS_UINT8                           ucLteEpsConnExistFlg,
    PS_RRM_IS_PREEMPTED_ENUM_UINT8      enRrmPreempted
)
{
    ...
    NAS_NORMAL_LOG3(UEPS_PID_MSCC, "NAS_MSCC_IsNeedStartBsrTryTimer_PreProc:enCurrFsmId, ucHrpdConnExistFlg and enRrmPreempted",
                                    enCurrFsmId, ucHrpdConnExistFlg, enRrmPreempted);

    NAS_NORMAL_LOG3(UEPS_PID_MSCC, "NAS_MSCC_IsNeedStartBsrTryTimer_PreProc:enCurHrpdSessionType, ucEhrpdConnBsrActiveFlg and ucHrpdConnBsrActiveFlg",
                                    NAS_MSCC_GetCurHrpdSessionType(), pstSysAcqBsrCtrl->ucEhrpdConnBsrActiveFlg, pstSysAcqBsrCtrl->ucHrpdConnBsrActiveFlg);
    ...
    if (VOS_TRUE == ucLteEpsConnExistFlg)
    {
        return VOS_TRUE;
    }

    if (VOS_TRUE == ucHrpdConnExistFlg)
    {
        /* SRLTE形态,DO下存在连接,直接返回,不做BSR */
        if (VOS_TRUE == NAS_MSCC_IsSrlte())
        {
            return VOS_TRUE;
        }

        if ((NAS_MSCC_HRPD_SESSION_TYPE_EHRPD == NAS_MSCC_GetCurHrpdSessionType())
         && (VOS_FALSE == pstSysAcqBsrCtrl->ucEhrpdConnBsrActiveFlg))
        {
            return VOS_TRUE;
        }

        if ((NAS_MSCC_HRPD_SESSION_TYPE_HRPD == NAS_MSCC_GetCurHrpdSessionType())
         && (VOS_FALSE == pstSysAcqBsrCtrl->ucHrpdConnBsrActiveFlg))
        {
            return VOS_TRUE;
        }
    }

    if (PS_RRM_COULD_PREEMPTED != enRrmPreempted)
    {
        return VOS_TRUE;
    }
...
VOS_UINT32 NAS_MSCC_ProcBsrTimerExpired_PreProc(VOS_VOID)
{
    ...
    enCurrFsmId             = NAS_MSCC_GetCurrFsmId();
    /* 根据当前的驻留状态，判断出是LTE下背景搜HRPD，还是HRPD下背景搜LTE */
    if (VOS_TRUE == NAS_MSCC_GetHrpdCampOnFlag())
    {
        enRatType  = VOS_RATMODE_LTE;
        enTaskType = RRM_PS_TASK_TYPE_LTE_PS_ATTACH;
    }
    else
    {
        enRatType  = VOS_RATMODE_HRPD;
        enTaskType = RRM_PS_TASK_TYPE_HRPD_ATTACH;
    }
    enRrmPreempted = RRM_RfResourceIsPreempted(VOS_GetModemIDFromPid(UEPS_PID_MSCC),
                                               enRatType,
                                               enTaskType);
    ...
    stBsrCtrlInfo.enRrmPreempted            = enRrmPreempted;
    ...
    /* 不需要背景搜高优先级系统，不需要进入BSR FSM */
    if (VOS_FALSE == NAS_MSCC_IsBgSearchHighPriSysAllowed())
    {
        NAS_MSCC_ResetBsrRelatedCtrlInfo();

        NAS_NORMAL_LOG(UEPS_PID_MSCC, "NAS_MSCC_ProcBsrTimerExpired_PreProc: Bg Search High Pri Sys is not Allowed");
    ...
    {
        NAS_WARNING_LOG(UEPS_PID_MSCC, "NAS_MSCC_ProcBsrTimerExpired_PreProc(): curr mscc fsm id is butt!");
    }

    /* 如果MSCC在子状态机执行过程中，启动BSR Retry定时器，等待 */
    /* 如果在数据业务过程中，启动BSR Retry定时器，等待 */
    /* 资源不可用时，启动BSR Retry定时器，等待 */
    if (VOS_TRUE == NAS_MSCC_IsNeedStartBsrTryTimer_PreProc(enCurrFsmId,
                                                            ucHrpdConnExistFlg,
                                                            ucLteEpsConnExistFlg,
                                                            enRrmPreempted))
    {
    ...
    {
        /* 判断lte是否允许搜索 */
        if (VOS_FALSE == NAS_MSCC_IsNeedAcqLte())
        {
            NAS_MSCC_StartBsrTimer();
...
)
{
    if (VOS_TRUE == NAS_MSCC_Get3gppActiveFlg())
    {
        NAS_WARNING_LOG(UEPS_PID_MSCC, "NAS_MSCC_RcvHsdSysAcqStartInd_PreProc:Dual Master Mode!");

        NAS_MSCC_SoftReBoot(NAS_MSCC_REBOOT_SCENE_MSCC_DUAL_MASTER_MODE_ACQUIRE_DO, UEPS_PID_MSCC);
    ...
    /* 只在CL模式下统计,在HRPD一轮搜网开始时，保存CL搜网开始信息 */
    NAS_MSCC_SaveErrLogCcbSysAcqStartInfo(VOS_RATMODE_HRPD, NAS_MSCC_PIF_PLMN_SEARCH_BUTT);
#endif
...
)
{
    ...
    if (HSD_MSCC_SESSION_NEG_RESULT_NO_RF == enRslt)
    {
        return VOS_TRUE;
    }
...
{
    MMA_MSCC_PS_RAT_TYPE_NTF_STRU      *pstPsRatNtf;

    pstPsRatNtf = (MMA_MSCC_PS_RAT_TYPE_NTF_STRU *)pstMsg;

    if (NAS_MSCC_PIF_PS_RATTYPE_1X == pstPsRatNtf->enPsRatType)
    {
        NAS_MSCC_SetDataCallSwitchTo1xFlg(VOS_TRUE);

        /* 如果当前在L1 MAIN流程中,available定时器在运行,同时启动1x BSR lte的定时器 */
        if (VOS_TRUE == NAS_MSCC_IsNeedStart1xBsrLteTimer())
        {
            NAS_NORMAL_LOG(UEPS_PID_MSCC, "NAS_MSCC_RcvMmaPsRatTypeNtf_Main_PreProc: start 1x bsr lte timer.");
    ...
    {
        NAS_MSCC_SetDataCallSwitchTo1xFlg(VOS_FALSE);
...
{
    XSD_MSCC_1X_UE_STATUS_IND_STRU     *pstUeStatusInd  = VOS_NULL_PTR;
    NAS_MSCC_UE_STATUS_INFO_STRU       *pstUeStatusInfo = VOS_NULL_PTR;

    pstUeStatusInd  = (XSD_MSCC_1X_UE_STATUS_IND_STRU*)pstMsg;

    pstUeStatusInfo = NAS_MSCC_GetUeStatusInfo();
    pstUeStatusInfo->enUe1xMainState   = pstUeStatusInd->enUeMainState;
    pstUeStatusInfo->enUe1xSubState    = pstUeStatusInd->enUeSubState;
    pstUeStatusInfo->enUe1xServiceType = pstUeStatusInd->enHighPrioServiceType;

    NAS_MSCC_SndMma1xUeStatusInd(pstUeStatusInd->enUeMainState,
                                 pstUeStatusInd->enUeSubState,
                                 pstUeStatusInd->enHighPrioServiceType);
...
VOS_UINT32 NAS_MSCC_RcvMmcMmssLteUnavailableInd_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    ...
    /* PS域被用户detach后,设置标记;对于SYSCFGEX触发的detach ps域,不设置该标记,因为后续有attach流程 */
    ...
    {
        NAS_MSCC_SetEpsAttachAllowFlg(VOS_FALSE);
    }
...
)
{
    ...
    if (VOS_TRUE == pstPsServiceConnStatusInd->ucPsServiceConnStatusFlag)
    {
        /* 判断连接状态是否发生变更 */
#if (FEATURE_ON == FEATURE_UE_MODE_CDMA)
        if (VOS_TRUE !=  NAS_MSCC_GetLteEpsConnExistFlg())
        {
            NAS_MSCC_SndXsdLteHrpdConnInfoInd(NAS_MSCC_GetHrpdConnExistFlg(),
                                              VOS_TRUE);
        }
#endif
...
)
{
    ...
    {
        /* DSDS宏打开时,GUL需转发消息给MMC,DSDS宏关闭时仅服务类型为NAS_MSCC_PIF_SRV_TYPE_PS_MO_EMERGENCY_CALL时将请求转发给MMC */
        ...
        if (NAS_MSCC_PIF_SRV_ACQ_RAT_TYPE_GUL == pstRatList->aenRatType[i])
        {
            ...
            if (NAS_MSCC_PIF_SRV_TYPE_PS_MO_EMERGENCY_CALL == pstSrvAcqReq->enSrvType)
            {
                ...
                if (VOS_FALSE == NAS_MSCC_GetHrpdActiveFlg())
                {
                    ...
                    /* 如果是IMS紧急呼触发的搜网,需要在搜到LTE上后给IMSA报限制服务状态,所以此处需要在全局变量中记录 */
                    NAS_MSCC_SetVolteEmcSrvAcqExistFlagBySrvType(pstSrvAcqReq->enSrvType);
                ...
                {
                    NAS_MSCC_SndMmaSrvAcqCnf(NAS_MSCC_PIF_SRV_ACQ_RESULT_FAIL);
                }
...
)
{
    ...
    NAS_MSCC_SERVICE_STATUS_ENUM_UINT8   ulServiceStatus;
    ...
    if (NAS_MSCC_SERVICE_STATUS_NORMAL_SERVICE == NAS_MSCC_GetCurr3gppPsServiceStatus())
    {
        ulServiceStatus = NAS_MSCC_SERVICE_STATUS_NORMAL_SERVICE;
    }
    else
    {
        ulServiceStatus = NAS_MSCC_SERVICE_STATUS_LIMITED_SERVICE;
    }

    if (NAS_MSCC_PIF_SRV_ACQ_RESULT_SUCCESS == pstSrvAcqCnf->enResult)
    {
        NAS_MSCC_Set3gppCampOnFlag(VOS_TRUE);
        /* 如果紧急呼IMS重拨触发的搜网成功,且当前RAT为LTE,通知IMSA限制服务 */
        if (VOS_TRUE == NAS_MSCC_GetVolteEmcSrvAcqExistFlag())
        {
            if (VOS_TRUE == NAS_MSCC_GetLteCampOnFlag())
            {
                NAS_MSCC_SndImsaSrvInfoNotify(ulServiceStatus, VOS_FALSE, VOS_FALSE);
                NAS_MSCC_SndImsaCampInfoChangeInd();
            }
    ...
    /* 转发该消息给MMC模块 */
    NAS_MSCC_SndMmaSrvAcqCnf(pstSrvAcqCnf->enResult);
...
{
    HSD_MSCC_SRV_ACQ_CNF_STRU          *pstSrvAcqCnf = VOS_NULL_PTR;

    pstSrvAcqCnf = (HSD_MSCC_SRV_ACQ_CNF_STRU *)pstMsg;
...
)
{
    ...
    if (NAS_MSCC_FSM_POWER_OFF != pstCurFsm->enFsmId)
    {
        NAS_MSCC_SndMmcImsVoiceCapNotify(ucImsVoiceAvail, pstImsVoiceCap->ucIsExistPersistentBearer,
                                         pstImsVoiceCap->ucRcvServiceChangeIndFlag);
    }
...
)
{
    ...
    NAS_MSCC_SndMmcImsVoiceCapNotify(ucImsVoiceAvailFlg, ucIsExistPersistentBearer, VOS_TRUE);
...
VOS_UINT32 NAS_MSCC_RcvMmaImsRegReq_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    /* 通知IMSA开启IMS协议栈 */
    NAS_MSCC_SndImsaRegReq();

    /* Start timer TI_NAS_MSCC_WAIT_IMSA_START_CNF */
    (VOS_VOID)NAS_MSCC_StartTimer(TI_NAS_MSCC_WAIT_IMSA_REG_CNF, TI_NAS_MSCC_WAIT_IMSA_REG_CNF_LEN);

    return VOS_TRUE;
}


VOS_UINT32 NAS_MSCC_RcvMmaImsDeregReq_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    /* 通知IMSA关闭IMS协议栈 */
    NAS_MSCC_SndImsaDeregReq();

    /* Start timer TI_NAS_IMSA_WAIT_IMSA_POWER_OFF_CNF */
    (VOS_VOID)NAS_MSCC_StartTimer(TI_NAS_MSCC_WAIT_IMSA_DEREG_CNF, TI_NAS_MSCC_WAIT_IMSA_DEREG_CNF_LEN);
...
VOS_UINT32 NAS_MSCC_RcvMmaImsDomainCfgSetReq_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    NAS_MSCC_PIF_IMS_DOMAIN_CFG_SET_REQ_STRU    *pstReqMsg = VOS_NULL_PTR;

    pstReqMsg = (NAS_MSCC_PIF_IMS_DOMAIN_CFG_SET_REQ_STRU *)pstMsg;

    /* 向IMSA发出配置IMS优选域请求 */
    NAS_MSCC_SndImsaImsDomainCfgSetReq((NAS_MSCC_PIF_IMS_DOMAIN_ENUM_UINT32)pstReqMsg->enImsDomain);

    return VOS_TRUE;
}


VOS_UINT32 NAS_MSCC_RcvImsaImsDomainCfgSetCnf_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    IMSA_MSCC_IMS_DOMAIN_CFG_SET_CNF_STRU   *pstImsDomainCnf = VOS_NULL_PTR;

    pstImsDomainCnf = (IMSA_MSCC_IMS_DOMAIN_CFG_SET_CNF_STRU *)pstMsg;

    /* 向MMA发送IMS优选域设置返回信息 */
    NAS_MSCC_SndMmaImsDomainCfgSetCnf((NAS_MSCC_PIF_IMS_DOMAIN_CFG_RESULT_ENUM_UINT32)pstImsDomainCnf->enResult);

    return VOS_TRUE;
}


VOS_UINT32 NAS_MSCC_RcvMmaRoamImsSupportSetReq_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    NAS_MSCC_PIF_ROAM_IMS_SUPPORT_SET_REQ_STRU    *pstReqMsg = VOS_NULL_PTR;

    pstReqMsg = (NAS_MSCC_PIF_ROAM_IMS_SUPPORT_SET_REQ_STRU *)pstMsg;

    /* 向IMSA发出配置漫游下IMS是否支持请求 */
    NAS_MSCC_SndImsaRoamImsSupportSetReq((MSCC_IMSA_SET_ROAM_IMS_SUPPORT_ENUM_UINT32)pstReqMsg->enRoamImsSupport);

    return VOS_TRUE;
}


VOS_UINT32 NAS_MSCC_RcvImsaRoamImsSupportSetCnf_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    IMSA_MSCC_ROAM_IMS_SUPPORT_SET_CNF_STRU   *pstRoamImsSupportCnf = VOS_NULL_PTR;

    pstRoamImsSupportCnf = (IMSA_MSCC_ROAM_IMS_SUPPORT_SET_CNF_STRU *)pstMsg;

    /* 向MMA发送漫游下IMS是否支持设置返回信息 */
    NAS_MSCC_SndMmaRoamImsSupportSetCnf((NAS_MSCC_PIF_ROAM_IMS_SUPPORT_RESULT_ENUM_UINT32)pstRoamImsSupportCnf->enResult);
...
VOS_UINT32 NAS_MSCC_RcvMmaApAreaLostInd_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    NAS_MSCC_SndImsaApAreaLostInd();
...
VOS_UINT32 NAS_MSCC_RcvImsaRegCnf_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    ...
    /* 更新IMS业务能力 */
    NAS_MSCC_SetImsSupportFlag(VOS_TRUE);

    /* Stop the timer */
    NAS_MSCC_StopTimer(TI_NAS_MSCC_WAIT_IMSA_REG_CNF);
    ...
    /* 给IMSA回复START CNF消息 */
    NAS_MSCC_SndMmaImsRegCnf(NAS_MSCC_PIF_IMS_REG_RESULT_SUCC);

    return VOS_TRUE;
}


VOS_UINT32 NAS_MSCC_RcvImsaDeregCnf_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    /* 更新IMS业务能力 */
    NAS_MSCC_SetImsSupportFlag(VOS_FALSE);

    /* Stop the timer */
    NAS_MSCC_StopTimer(TI_NAS_MSCC_WAIT_IMSA_DEREG_CNF);
    ...
    /* 给MMA回复DEREG CNF消息 */
    NAS_MSCC_SndMmaImsDeregCnf(NAS_MSCC_PIF_IMS_DEREG_RESULT_SUCC);

    return VOS_TRUE;
}


VOS_UINT32 NAS_MSCC_RcvTiWaitImsaRegCnfExpired_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    ...
    /* 给MMA回复IMS_REG_CNF消息 */
    NAS_MSCC_SndMmaImsRegCnf(NAS_MSCC_PIF_IMS_REG_RESULT_TIMEOUT);

    return VOS_TRUE;
}


VOS_UINT32 NAS_MSCC_RcvTiWaitImsaDeregCnfExpired_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    ...
    /* 给IMSA回复IMS_DEREG_CNF消息 */
    NAS_MSCC_SndMmaImsDeregCnf(NAS_MSCC_PIF_IMS_DEREG_RESULT_TIMEOUT);
...
)
{
    ...
    if (NAS_MSCC_PIF_POWER_SAVE_SUCCESS == pstPowerSaveCnf->enResult)
    {
        ...
        if (VOS_FALSE == NAS_MSCC_GetAllowSrchLteFlg())
        {
            ...
            NAS_MSCC_SetAllowSrchLteSysPriClass(NAS_MSCC_PIF_PLMN_PRIORITY_ANY);

            NAS_MSCC_SndMmcCLAssociatedInfoNtf(NAS_MSCC_INVALID_MCC, NAS_MSCC_PIF_PLMN_PRIORITY_ANY, VOS_TRUE);
        }
    }
...
)
{
    ...
    if (NAS_MSCC_PIF_POWER_SAVE_SUCCESS == pstPowerSaveCnf->enResult)
    {
        ...
        NAS_MSCC_SndMmaHrpdServiceStatusInd(MSCC_MMA_HRPD_SERVICE_STATUS_POWER_SAVE,
                                            VOS_FALSE,
                                            VOS_FALSE,
                                            MSCC_MMA_SESSION_RELEASE_TYPE_BUTT);
    }
...
{
    XSD_MSCC_1X_SYSTEM_SERVICE_INFO_IND_STRU                st1xSysSrvInfoInd;
    XSD_MSCC_1X_NO_SERVICE_IND_STRU                        *pstXsdMscc1xNoServiceInd = VOS_NULL_PTR;

    pstXsdMscc1xNoServiceInd = (XSD_MSCC_1X_NO_SERVICE_IND_STRU *)pstMsg;

    PS_MEM_SET(&st1xSysSrvInfoInd, 0x0, sizeof(XSD_MSCC_1X_SYSTEM_SERVICE_INFO_IND_STRU));
    st1xSysSrvInfoInd.stSysSrvInfo.enServiceStatus = NAS_MSCC_PIF_SERVICE_STATUS_NO_SERVICE;

    st1xSysSrvInfoInd.stSysSrvInfo.en1xOosCause = pstXsdMscc1xNoServiceInd->en1xOosCause;
...
VOS_UINT32 NAS_MSCC_RcvXsdPrlHeaderInfo_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    NAS_MSCC_PRL_HEADER_INFO_IND_STRU                      *pstPrlHeader;

    pstPrlHeader = (NAS_MSCC_PRL_HEADER_INFO_IND_STRU*)pstMsg;

    NAS_MSCC_SndMmaPrlHeaderInfoInd(pstPrlHeader);
...
#if (FEATURE_ON == FEATURE_UE_MODE_CDMA)
#if (FEATURE_PTM == FEATURE_ON)
NAS_MSCC_ERRLOG_REPORT_PROC_STRU   g_stNasMsccErrLogRptProc[] =
{
    {NAS_ERR_LOG_ALM_CL_OOS_SEARCH, NAS_MSCC_ErrLogReportClOosSearchInfo, NAS_ERRLOG_CCB_InitClOosSearchInfo}
};


VOS_UINT32 NAS_MSCC_RcvOmErrLogReportReq_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
    OM_ERR_LOG_REPORT_REQ_STRU         *pstErrLogRptReq              = VOS_NULL_PTR;
    pNasMsccErrLogReportFunc            pfuncNasMsccErrLogReport     = VOS_NULL_PTR;
    pNasMsccInitErrLogCcbCtxFunc        pfuncNasMsccInitErrLogCcbCtx = VOS_NULL_PTR;
    VOS_UINT32                          ulIndex;

    pstErrLogRptReq = (OM_ERR_LOG_REPORT_REQ_STRU*)pstMsg;

    for (ulIndex = 0; ulIndex < (sizeof(g_stNasMsccErrLogRptProc)/sizeof(NAS_MSCC_ERRLOG_REPORT_PROC_STRU)); ulIndex++)
    {
        if (g_stNasMsccErrLogRptProc[ulIndex].ulAlarmID == pstErrLogRptReq->ucAlarmID)
        {
            pfuncNasMsccErrLogReport     = g_stNasMsccErrLogRptProc[ulIndex].pfuncNasMsccErrLogReport;
            pfuncNasMsccInitErrLogCcbCtx = g_stNasMsccErrLogRptProc[ulIndex].pfuncNasMsccInitErrLogCcbCtx;
            break;
        }
    }
    /*  处理CHR查询，上报查询结果 */
    if (VOS_NULL_PTR != pfuncNasMsccErrLogReport)
    {
        pfuncNasMsccErrLogReport(pstErrLogRptReq);
    }
    /*  CHR查询上报后，清空全局变量 */
    if (VOS_NULL_PTR != pfuncNasMsccInitErrLogCcbCtx)
    {
        pfuncNasMsccInitErrLogCcbCtx();
    }
...
VOS_UINT32 NAS_MSCC_RcvHsdSysAcqEndInd_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
...
#if (FEATURE_PTM == FEATURE_ON)
    enErrLogSrchRslt = NAS_MSCC_ConversionHrpdAcqRsltToClOosSrchRslt(pstHsdSysAcqInd->enRslt);
    NAS_MSCC_SaveErrLogCcbSysAcqEndInfo(VOS_RATMODE_HRPD, enErrLogSrchRslt);
#endif
...
VOS_UINT32 NAS_MSCC_RcvMmcPlmnSelectionRsltInd_PreProc(
    VOS_UINT32                          ulEventType,
    struct MsgCB                       *pstMsg
)
{
#if (FEATURE_ON == FEATURE_UE_MODE_CDMA)
#if (FEATURE_PTM == FEATURE_ON)
    MMC_MSCC_PLMN_SELECTION_RLST_IND_STRU                  *pstMmcPlmnSelecRslt = VOS_NULL_PTR;
    NAS_ERR_LOG_CL_SEARCH_RSLT_ENUM_UINT32                  enErrLogSrchRslt;

    pstMmcPlmnSelecRslt = (MMC_MSCC_PLMN_SELECTION_RLST_IND_STRU *)pstMsg;

    /* LTE搜网结果转换为CHR定义的结果  */
    if (VOS_FALSE == NAS_MSCC_IsGULModeConfigured())
    {
        enErrLogSrchRslt = NAS_MSCC_ConvertLteAcqRsltToClOosSrchRslt(pstMmcPlmnSelecRslt->enResult);
        NAS_MSCC_SaveErrLogCcbSysAcqEndInfo(VOS_RATMODE_LTE, enErrLogSrchRslt);
    }
