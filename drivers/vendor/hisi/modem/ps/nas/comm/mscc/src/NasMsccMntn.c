...
extern VOS_UINT8   *gpucMsccExcAddr;
#elif defined (INSTANCE_2)
extern VOS_UINT8   *gpucMsccExcAddr;
#else
VOS_UINT8          *gpucMsccExcAddr = VOS_NULL_PTR;
#endif
...
)
{
    ...
    if (ucNumSysLocTag > NAS_MSCC_MNTN_MLPL_MAX_NUM_SYS_LOC_TAG)
    {
        ucNumSysLocTag = NAS_MSCC_MNTN_MLPL_MAX_NUM_SYS_LOC_TAG;
    }

    for (ucIndex = 0; ucIndex < ucNumSysLocTag; ucIndex++)
    {
...
)
{
    ...
    for (i = 0; i < pstMlplInfo->stMlplHdr.ucNumMlplRec; i++)
    {
        ...
        NAS_MSCC_MNTN_GetMlplSysLocTag(pstMlplRecList->astMlplRecList[j].stLocParamVal.astSysLocTag,
                                       ...
                                       pstMlplInfo->apstMlplRec[i]->stLocParamVal.ucNumSysLocTag);
...
NAS_MSCC_MNTN_DISCARD_MSG_STRU g_stNasMsccMntnDiscardMsgFromXsdTbl[]=
{
    ...
    {UEPS_PID_XSD,                     UEPS_PID_MSCC,                          ID_XSD_MSCC_1X_UE_STATUS_IND}
};
...
NAS_MSCC_MNTN_DISCARD_MSG_STRU g_stNasMsccMntnDiscardMsgToXsdTbl[]=
{
    ...
    {UEPS_PID_MSCC,                     UEPS_PID_XSD,                           ID_MSCC_XSD_LTE_HRPD_CONN_INFO_IND},
...
VOS_VOID NAS_MSCC_ErrLogReportClOosSearchInfo(
    OM_ERR_LOG_REPORT_REQ_STRU         *pstErrLogRptReq
)
{
    VOS_UINT32                                              ulLength;
    VOS_UINT32                                              ulSearchRecordCnt;
    NAS_ERR_LOG_CL_OOS_SEARCH_STRU                         *pstErrLogClOosSrch        = VOS_NULL_PTR;
    NAS_ERRLOG_CCB_CL_OOS_SEARCH_INFO_STRU                 *pstErrLogCcbClOosSrchInfo = VOS_NULL_PTR;

    pstErrLogClOosSrch = (NAS_ERR_LOG_CL_OOS_SEARCH_STRU *)PS_MEM_ALLOC(UEPS_PID_MSCC,
                                                            sizeof(NAS_ERR_LOG_CL_OOS_SEARCH_STRU));
    if (VOS_NULL_PTR == pstErrLogClOosSrch)
    {
        NAS_ERROR_LOG(UEPS_PID_MSCC, "NAS_MSCC_ErrLogReportClOosSearchInfo:ERROR:Alloc Mem Fail.");
        return;
    }
    NAS_MEM_SET_S(pstErrLogClOosSrch,
                  sizeof(NAS_ERR_LOG_CL_OOS_SEARCH_STRU),
                  0,
                  sizeof(NAS_ERR_LOG_CL_OOS_SEARCH_STRU));

    ulSearchRecordCnt = NAS_ERRLOG_CCB_GetClOosSrchInfoRecordCnt();
    /* 上报的消息中，最多携带20条搜网记录，如果搜网超过20轮，则数组中保存的是最新的20轮搜网记录
        ulSearchRecordCnt等于20
    */
    if (NAS_ERR_LOG_CL_OOS_MAX_SEARCH_RECORD_NUM < ulSearchRecordCnt)
    {
        ulSearchRecordCnt = NAS_ERR_LOG_CL_OOS_MAX_SEARCH_RECORD_NUM;
    }
    pstErrLogClOosSrch->ulSearchRecordCnt = ulSearchRecordCnt;

    pstErrLogCcbClOosSrchInfo = NAS_ERRLOG_CCB_GetClOosSrchInfoCtxAddr();

    pstErrLogClOosSrch->ulIsHrpdAndLteBothSupport = pstErrLogCcbClOosSrchInfo->ulIsHrpdAndLteBothSupport;

    NAS_MEM_CPY_S(pstErrLogClOosSrch->astSearchRecord,
                  (ulSearchRecordCnt * sizeof(NAS_ERR_LOG_CL_OOS_SEARCH_RECORD_STRU)),
                  pstErrLogCcbClOosSrchInfo->astSearchRecord,
                  (ulSearchRecordCnt * sizeof(NAS_ERR_LOG_CL_OOS_SEARCH_RECORD_STRU)));

    ulLength = sizeof(NAS_ERR_LOG_CL_OOS_SEARCH_STRU);
    /*  填充CHR信息的头  */
    CNAS_COMM_BULID_ERRLOG_HEADER_INFO(&(pstErrLogClOosSrch->stHeader),
                                       VOS_GetModemIDFromPid(UEPS_PID_MSCC),
                                       NAS_ERR_LOG_ALM_CL_OOS_SEARCH,
                                       NAS_GetErrLogAlmLevel(NAS_ERR_LOG_ALM_CL_OOS_SEARCH),
                                       VOS_GetSlice(),
                                       (ulLength - sizeof(OM_ERR_LOG_HEADER_STRU)));

    NAS_ERRLOG_CCB_SndAcpuOmFaultErrLogCnf(pstErrLogClOosSrch,
                                           ulLength,
                                           pstErrLogRptReq->ucFaultID,
                                           (VOS_UINT8)NAS_ERR_LOG_ALM_CL_OOS_SEARCH,
                                           UEPS_PID_MSCC);

    PS_MEM_FREE(UEPS_PID_MSCC, pstErrLogClOosSrch);
...
VOS_VOID NAS_MSCC_MNTN_SaveMsccLogMsgState(
    NAS_MSCC_MNTN_SAVE_EXC_LOG_STRU    *pstNasMsccMntnSaveExcLog
)
{
    NAS_MSCC_LOG_MSG_STATUS_STRU       *pstMsccLogMsgState = VOS_NULL_PTR;

    pstMsccLogMsgState = &(NAS_MSCC_GetMsccCtxAddr()->stMntnInfo.stLogMsgState);

    NAS_MEM_CPY_S(&(pstNasMsccMntnSaveExcLog->stMsccLogMsgState),
                  sizeof(NAS_MSCC_LOG_MSG_STATUS_STRU),
                  pstMsccLogMsgState,
                  sizeof(NAS_MSCC_LOG_MSG_STATUS_STRU));
...
VOS_VOID NAS_MSCC_MNTN_SaveMsccGlobalInfo(
    NAS_MSCC_MNTN_SAVE_EXC_LOG_STRU    *pstNasMsccMntnSaveExcLog
)
{
    NAS_MEM_CPY_S(&(pstNasMsccMntnSaveExcLog->stSimStatus),
                  sizeof(NAS_MSCC_SIM_STATUS_STRU),
                  &(NAS_MSCC_GetMsccCtxAddr()->stSimInfo.stSimStatus),
                  sizeof(NAS_MSCC_SIM_STATUS_STRU));

    pstNasMsccMntnSaveExcLog->ucIsCardChanged = NAS_MSCC_GetMsccCtxAddr()->stSimInfo.ucIsCardChanged;

    NAS_MEM_CPY_S(&(pstNasMsccMntnSaveExcLog->stMsCfgInfo),
                  sizeof(NAS_MSCC_MS_CFG_INFO_STRU),
                  &(NAS_MSCC_GetMsccCtxAddr()->stMsCfgInfo),
                  sizeof(NAS_MSCC_MS_CFG_INFO_STRU));

    NAS_MEM_CPY_S(&(pstNasMsccMntnSaveExcLog->stNwInfo),
                  sizeof(NAS_MSCC_NW_INFO_STRU),
                  &(NAS_MSCC_GetMsccCtxAddr()->stNwInfo),
                  sizeof(NAS_MSCC_NW_INFO_STRU));

    NAS_MEM_CPY_S(&(pstNasMsccMntnSaveExcLog->stPowerSaveCtrl),
                  sizeof(NAS_MSCC_POWER_SAVE_CTRL_CTX_STRU),
                  &(NAS_MSCC_GetMsccCtxAddr()->stPowerSaveCtrl),
                  sizeof(NAS_MSCC_POWER_SAVE_CTRL_CTX_STRU));

    NAS_MEM_CPY_S(&(pstNasMsccMntnSaveExcLog->stSysAcqCtrl),
                  sizeof(NAS_MSCC_SYS_ACQ_CTRL_CTX_STRU),
                  &(NAS_MSCC_GetMsccCtxAddr()->stSysAcqCtrl),
                  sizeof(NAS_MSCC_SYS_ACQ_CTRL_CTX_STRU));
...
VOS_VOID  NAS_MSCC_MNTN_SaveExcLog(
    VOS_UINT8                          *pucAddr,
    VOS_UINT32                          ulLength
)
{
    NAS_MSCC_MNTN_SAVE_EXC_LOG_STRU    *pstNasMsccMntnSaveExcLog = VOS_NULL_PTR;
    VOS_UINT32                          ulNasMsccSaveExcLength;

    /* 入参检查 */
    if ( ( VOS_NULL_PTR == pucAddr )
      || ( 0 == ulLength ))
    {
        NAS_ERROR_LOG(UEPS_PID_MSCC, "NAS_MSCC_MNTN_SaveExcLog(): No memory allocated");

        return;
    }

    ulNasMsccSaveExcLength = sizeof(NAS_MSCC_MNTN_SAVE_EXC_LOG_STRU);

    /* 确保NAS_MSCC_MNTN_SAVE_EXC_LOG_STRU结构的长度不会溢出底软分配给Nas的空间，如果超出需要调整 */
    if (ulLength < ulNasMsccSaveExcLength)
    {
        NAS_ERROR_LOG(UEPS_PID_MSCC, "NAS_MSCC_MNTN_SaveExcLog(): Memory allocated not enough");

        return;
    }

    pstNasMsccMntnSaveExcLog = (NAS_MSCC_MNTN_SAVE_EXC_LOG_STRU*)pucAddr;

    /* 标示存储的有效信息的开始和结束字段 */
    pstNasMsccMntnSaveExcLog->ulMsccBeginTag = NAS_MSCC_MNTN_DUMP_BEGIN_TAG;
    pstNasMsccMntnSaveExcLog->ulMsccEndTag   = NAS_MSCC_MNTN_DUMP_END_TAG;

    NAS_NORMAL_LOG(UEPS_PID_MSCC, "NAS_MSCC_MNTN_SaveExcLog(): Save Mscc related info before reset");

    /* Event state信息保存 */
    NAS_MSCC_MNTN_SaveMsccLogMsgState(pstNasMsccMntnSaveExcLog);

    /* 全局信息保存 */
    NAS_MSCC_MNTN_SaveMsccGlobalInfo(pstNasMsccMntnSaveExcLog);

    return ;
}


VOS_VOID NAS_MSCC_MNTN_SaveMsccExcLog( VOS_VOID )
{
    VOS_UINT8                          *pucSaveAddr   = VOS_NULL_PTR;
    VOS_UINT32                          ulLeftLength   = 0;

    if ( VOS_NULL_PTR == gpucMsccExcAddr )
    {
        NAS_ERROR_LOG(UEPS_PID_MSCC, "NAS_MSCC_MNTN_SaveMsccExcLog(): No memory allocated");
        return;
    }

#if defined (INSTANCE_1)
    pucSaveAddr     = gpucMsccExcAddr + NAS_MSCC_MNTN_SAVE_EXC_LOG_LENGTH_MODEM0;
    ulLeftLength    = NAS_MSCC_MNTN_SAVE_EXC_LOG_LENGTH_MODEM1;

    NAS_MSCC_MNTN_SaveExcLog( ( VOS_UINT8 *)pucSaveAddr, ulLeftLength);
#elif defined (INSTANCE_2)
    pucSaveAddr     = gpucMsccExcAddr + NAS_MSCC_MNTN_SAVE_EXC_LOG_LENGTH_MODEM0 + NAS_MSCC_MNTN_SAVE_EXC_LOG_LENGTH_MODEM1;
    ulLeftLength    = NAS_MSCC_MNTN_SAVE_EXC_LOG_LENGTH_MODEM2;

    NAS_MSCC_MNTN_SaveExcLog( ( VOS_UINT8 *)pucSaveAddr, ulLeftLength);
#else
    pucSaveAddr     = gpucMsccExcAddr;
    ulLeftLength    = NAS_MSCC_MNTN_SAVE_EXC_LOG_LENGTH_MODEM0;
...
VOS_VOID NAS_MSCC_RegisterDumpCallBack( VOS_VOID )
{
#if defined (INSTANCE_1)
#elif defined (INSTANCE_2)
#else
    /* 调用底软接口进行申请内存 */
    gpucMsccExcAddr = (VOS_UINT8 *)mdrv_om_register_field(OM_CP_MSCC,
                                                          "MSCC dump",
                                                          VOS_NULL_PTR,
                                                          VOS_NULL_PTR,
                                                          (NAS_MSCC_MNTN_SAVE_EXC_LOG_LENGTH_MODEM0 + \
                                                           NAS_MSCC_MNTN_SAVE_EXC_LOG_LENGTH_MODEM1 + \
                                                           NAS_MSCC_MNTN_SAVE_EXC_LOG_LENGTH_MODEM2),
                                                          0);

    /* 申请失败打印异常信息 */
    if (VOS_NULL_PTR == gpucMsccExcAddr)
    {
        NAS_ERROR_LOG(UEPS_PID_MSCC, "NAS_MSCC_RegisterDumpCallBack(): No memory allocated");
        return;
    }

#endif

    /* 保存MSCC复位前的临终遗言信息,注册回调函数 */
    (VOS_VOID)mdrv_om_register_callback("MSCC_EXCLOG", (dump_hook)NAS_MSCC_MNTN_SaveMsccExcLog);

    NAS_NORMAL_LOG(UEPS_PID_MSCC, "Mscc dump register call back function finished");
