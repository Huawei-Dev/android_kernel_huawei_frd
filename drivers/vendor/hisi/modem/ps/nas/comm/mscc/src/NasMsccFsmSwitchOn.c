...
#if (FEATURE_ON == FEATURE_UE_MODE_CDMA)
    /* 修改卡状态判断，只要同时存在3GPP和3GPP2的应用，就读取卡文件  */
    if ((NAS_MSCC_PIF_CARD_STATUS_ABSENT != pstMsgStartReq->enCsimStatus)
     || (NAS_MSCC_PIF_CARD_STATUS_ABSENT != pstMsgStartReq->enUsimStatus))
    {
...
VOS_VOID NAS_MSCC_ProcMlplMsplReadRslt_SwitchOn(VOS_VOID)
{
    ...
    {
        /**************************************************************
         如果卡文件中的MLPL文件没有default记录，则手动添加一条MSPL
         作为default的MLPL记录
         ***************************************************************/
        if (VOS_FALSE == pstMlpl->ucIsContainDefRec)
        {
            NAS_MSCC_AddHardCodeDefaultMlpl();

            NAS_MSCC_MNTN_LogMlplInfo(NAS_MSCC_GetMlplInfoAddr());
            NAS_MSCC_MNTN_LogMsplInfo(NAS_MSCC_GetMsplInfoAddr());
        }

        /*  从卡中读取mspl和mlpl成功，并且解析成功，向mma上报mspl和mlpl的文件的头信息 */
        NAS_MSCC_SndMmaMsplMlplInfoInd(MSCC_MMA_MSPL_MLPL_INFO_SOURCE_CARD);
    ...
    if (VOS_TRUE == NAS_MSCC_GetMlplMsplValidFlag())
    {
        /*  从NV中读取mspl和mlpl成功，并且解析成功，向mma上报mspl和mlpl的文件的头信息 */
        NAS_MSCC_SndMmaMsplMlplInfoInd(MSCC_MMA_MSPL_MLPL_INFO_SOURCE_NV);
    ...
    if (VOS_TRUE == NAS_MSCC_GetMlplMsplValidFlag())
    {
        /*  从代码中读取预置的mspl和mlpl成功，并且解析成功，向mma上报mspl和mlpl的文件的头信息 */
        NAS_MSCC_SndMmaMsplMlplInfoInd(MSCC_MMA_MSPL_MLPL_INFO_SOURCE_DEFAULT);
    }
}
