...
VOS_VOID NAS_MSCC_SndHsdBgSearchReq(
    ...
    NAS_MSCC_BSR_SRCH_TYPE_ENUM_UINT8   enBsrType,
    VOS_UINT8                           ucPlmnNum,
...
)
{
    ...
    pstMsg->enBsrType = enBsrType;

    pstMsg->ucPlmnNum = ((ucPlmnNum > MSCC_HSD_BSR_REQ_MAX_PLMN_NUM) ? MSCC_HSD_BSR_REQ_MAX_PLMN_NUM : ucPlmnNum);

    for (i = 0; i < pstMsg->ucPlmnNum; i++)
    {
        pstMsg->astPlmnList[i].ulMcc = pstPlmnListEntry[i].ulMcc;
        pstMsg->astPlmnList[i].ulMnc = pstPlmnListEntry[i].ulMnc;
    }
...
VOS_VOID NAS_MSCC_SndHsdEnableLteNtf(VOS_VOID)
{
    MSCC_HSD_ENABLE_LTE_NTF_STRU       *pstMsg = VOS_NULL_PTR;

    /* Allocate memory to send message  */
    pstMsg = (MSCC_HSD_ENABLE_LTE_NTF_STRU *)PS_ALLOC_MSG_WITH_HEADER_LEN(UEPS_PID_MSCC,
              sizeof(MSCC_HSD_ENABLE_LTE_NTF_STRU));

    if (VOS_NULL_PTR == pstMsg)
    {
        NAS_ERROR_LOG(UEPS_PID_MSCC, "NAS_MSCC_SndHsdEnableLteNtf:ERROR: Memory Alloc Error for pMsg");
    ...
    PS_MEM_SET((VOS_INT8*)pstMsg + VOS_MSG_HEAD_LENGTH,
               0,
               sizeof(MSCC_HSD_ENABLE_LTE_NTF_STRU) - VOS_MSG_HEAD_LENGTH);

    pstMsg->stMsgHeader.ulReceiverCpuId  = VOS_LOCAL_CPUID;
    pstMsg->stMsgHeader.ulReceiverPid    = UEPS_PID_HSD;
    pstMsg->stMsgHeader.ulSenderCpuId    = VOS_LOCAL_CPUID;
    pstMsg->stMsgHeader.ulSenderPid      = UEPS_PID_MSCC;
    pstMsg->stMsgHeader.ulLength         = sizeof(MSCC_HSD_ENABLE_LTE_NTF_STRU) - VOS_MSG_HEAD_LENGTH;
    pstMsg->stMsgHeader.ulMsgName        = ID_MSCC_HSD_ENABLE_LTE_NTF;
