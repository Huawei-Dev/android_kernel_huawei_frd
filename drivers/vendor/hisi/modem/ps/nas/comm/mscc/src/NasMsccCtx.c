...
)
{
    ...
    NAS_MSCC_InitBsrCtrlCtx(&pstMsccCtx->stBsrCtrl);
    ...
    NAS_ERRLOG_CCB_InitCcbCtx();
#endif
#endif

#if (FEATURE_ON == FEATURE_UE_MODE_CDMA)
    NAS_MSCC_InitUeStatusInfo(&(pstMsccCtx->stUeStatusInfo));
#endif
}
...
)
{
    ...
    pstNwInfo->stLastSysInfo.enLastCampOnLTEOrDoRat  = NAS_MSCC_PIF_NET_RAT_TYPE_BUTT;
}
...
)
{
    ...
    if (0 < ulState)
    {
        NAS_NORMAL_LOG1(UEPS_PID_MSCC, "NAS_MSCC_InitFsmL2: NAS_MSCC_GetFsmTopState: before load subfsm:", ulState);
    }

    /* The FSM Info is logged in OM */
    NAS_MSCC_MNTN_LogFsmInfo(enCurrFsmId, ulState);
    ...
    /* NULL state of any fsm is 0 */
    if (0 != ulState)
    {
        NAS_WARNING_LOG(UEPS_PID_MSCC, "NAS_MSCC_InitFsmL2: NAS_MSCC_GetFsmTopState(): load subfsm state is not null!");
    }
...
VOS_VOID NAS_MSCC_QuitFsmL2(VOS_VOID)
{
    ...
    if (0 < ulState)
    {
        NAS_NORMAL_LOG1(UEPS_PID_MSCC, "NAS_MSCC_QuitFsmL2: NAS_MSCC_GetFsmTopState(): state before pop subfsm is:", ulState);
    }
    ...
    if (0 < ulState)
    {
        NAS_NORMAL_LOG1(UEPS_PID_MSCC, "NAS_MSCC_QuitFsmL2: NAS_MSCC_GetFsmTopState(): state after pop subfsm is:", ulState);
    }
...
VOS_UINT32 NAS_MSCC_GetImsSupportFlag(VOS_VOID)
{
    ...
    /* 平台不支持LTE能力，认为不支持IMS能力 */
    if (VOS_TRUE != NAS_MSCC_IsPlatformSupportLte())
    {
        return VOS_FALSE;
    }

    /* 如果支持UTRAN/LTE IMS/WIFI IMS, 任何一接入技术支持IMS，即认为支持IMS */
...
VOS_VOID NAS_MSCC_SetImsSupportFlag(VOS_UINT8 ucSupportFlag)
{
#if (FEATURE_ON == FEATURE_LTE)
    NAS_MSCC_IMS_CONFIG_PARA_STRU      *pstImsCfgInfo = VOS_NULL_PTR;
    ...
    /* 平台能力不支持LTE，不允许修改IMS能力项 */
    if (VOS_TRUE != NAS_MSCC_IsPlatformSupportLte())
    {
        return;
    }

    /* UTRAN/LTE IMS/WIFI IMS 对IMS的支持功能同时修改 */
    pstImsCfgInfo->stImsRatSupport.ucWifiImsSupportFlag     = ucSupportFlag;
    pstImsCfgInfo->stImsRatSupport.ucUtranImsSupportFlag    = ucSupportFlag;
    pstImsCfgInfo->stImsRatSupport.ucLteImsSupportFlag      = ucSupportFlag;
#endif
    return;
}
...
VOS_VOID NAS_MSCC_SetModuleOrder_PowerOff(VOS_VOID)
{
    ...
    /* If IMS Feature is ON, Add IMS module ID to the Module order array */
#if (FEATURE_ON == FEATURE_IMS)
    /* VoLTE EMC Need Stop imsa when Power Off */
    if (VOS_TRUE == NAS_MSCC_IsPlatformSupportLte())
    {
        pstModuleOrder->aulModuleID[pstModuleOrder->usNum] = PS_PID_IMSA;
        pstModuleOrder->usNum ++;
...
)
{
    ...
    /* 清除L1 MAIN上下文信息-开关机不会清除L1 MAIN的上下文信息 */
...
VOS_VOID NAS_MSCC_InitFsmCtx_SysAcq(VOS_VOID)
{
    ...
    pstSysAcqFsmCtx->ucReAcqLteUpOnNoRfFinishFlg    = VOS_FALSE;
    pstSysAcqFsmCtx->ucPwrSaveLteUpOnReAcqLteNoRf   = VOS_FALSE;
    ...
    pstSysAcqFsmCtx->ucLteNoRfFlg                   = VOS_FALSE;
    pstSysAcqFsmCtx->ucHrpdNoRfFlg                  = VOS_FALSE;

    pstSysAcqFsmCtx->ucWaitQuickSrchDoCnf            = VOS_FALSE;
}
...
VOS_VOID NAS_MSCC_InitFsmCtx_BSR(VOS_VOID)
{
    ...
    pstMsccCtx->stCurFsm.unFsmCtx.stBsrCtx.enBsrHrpdNoRfType    = NAS_MSCC_BSR_HRPD_NORF_TYPE_BUTT;
    pstMsccCtx->stCurFsm.unFsmCtx.stBsrCtx.enBsrLteNoRfType     = NAS_MSCC_BSR_LTE_NORF_TYPE_BUTT;
    if (VOS_TRUE == NAS_MSCC_GetHrpdCampOnFlag())
    {
        ...
        NAS_MSCC_GetDestBsrSysPriClassAndSysTypeInSysList(VOS_RATMODE_HRPD, &stOutSysList, &enPriClass, &enSysType);
    }
    else
    {
        ...
        NAS_MSCC_GetDestBsrSysPriClassAndSysTypeInSysList(VOS_RATMODE_LTE, &stOutSysList, &enPriClass, &enSysType);
    }

    NAS_MSCC_SetDestBsrSysType_BSR(enSysType);
    NAS_MSCC_SetDestBsrSysPriClass_BSR(enPriClass);

    NAS_MSCC_SetHrpdNoRfFlg_BSR(VOS_FALSE);
...
VOS_UINT8 NAS_MSCC_GetInterSysSearchLteNoRfFlag_SysAcq(VOS_VOID)
{
    return (NAS_MSCC_GetSysAcqFsmCtxAddr()->ucLteNoRfFlg);
}


VOS_VOID NAS_MSCC_SetInterSysSearchLteNoRfFlag_SysAcq(
    VOS_UINT8                           ucLteNoRfFLag
)
{
    (NAS_MSCC_GetSysAcqFsmCtxAddr()->ucLteNoRfFlg) = ucLteNoRfFLag;

    return;
}


VOS_UINT8 NAS_MSCC_GetInterSysSearchHrpdNoRfFlag_SysAcq(VOS_VOID)
{
    return (NAS_MSCC_GetSysAcqFsmCtxAddr()->ucHrpdNoRfFlg);
}


VOS_VOID NAS_MSCC_SetInterSysSearchHrpdNoRfFlag_SysAcq(
    VOS_UINT8                           ucHrpdNoRfFLag
)
{
    (NAS_MSCC_GetSysAcqFsmCtxAddr()->ucHrpdNoRfFlg) = ucHrpdNoRfFLag;

    return;
}


VOS_UINT8 NAS_MSCC_GetReAcqLteUpOnNoRfFinishFlg_SysAcq(VOS_VOID)
{
    return (NAS_MSCC_GetSysAcqFsmCtxAddr()->ucReAcqLteUpOnNoRfFinishFlg);
}


VOS_VOID NAS_MSCC_SetReAcqLteUpOnNoRfFinishFlg_SysAcq(
    VOS_UINT8                           ucReAcqLteUpOnNoRfFinishFlg
)
{
    (NAS_MSCC_GetSysAcqFsmCtxAddr()->ucReAcqLteUpOnNoRfFinishFlg) = ucReAcqLteUpOnNoRfFinishFlg;

    return;
}


VOS_UINT8 NAS_MSCC_GetPwrSaveLteUponReAcqLteNoRfFlg_SysAcq(VOS_VOID)
{
    return (NAS_MSCC_GetSysAcqFsmCtxAddr()->ucPwrSaveLteUpOnReAcqLteNoRf);
}


VOS_VOID NAS_MSCC_SetPwrSaveLteUponReAcqLteNoRfFlg_SysAcq(
    VOS_UINT8                           ucPwrSaveLteUpOnReAcqLteNoRf
)
{
    (NAS_MSCC_GetSysAcqFsmCtxAddr()->ucPwrSaveLteUpOnReAcqLteNoRf) = ucPwrSaveLteUpOnReAcqLteNoRf;
...
VOS_UINT32 NAS_MSCC_GetWaitQuickSrchDoCnfFlg_SysAcq(VOS_VOID)
{
    /* 如果当前状态机不是CL system acquire */
    if (NAS_MSCC_FSM_SYS_ACQ!= NAS_MSCC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MSCC_GetWaitQuickSrchDoCnfFlg_SysAcq,ERROR:FsmId Error");
    }

    return (VOS_UINT32)(g_stNasMsccCtx.stCurFsm.unFsmCtx.stSysAcqCtx.ucWaitQuickSrchDoCnf);
}


VOS_VOID NAS_MSCC_SetWaitQuickSrchDoCnfFlg_SysAcq(
    VOS_UINT8                           ucWaitQuickSrchDoCnfFlg
)
{
    /* 如果当前状态机不是CL system acquire */
    if (NAS_MSCC_FSM_SYS_ACQ!= NAS_MSCC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MSCC_SetWaitQuickSrchDoCnfFlg_SysAcq,ERROR:FsmId Error");
    }

    g_stNasMsccCtx.stCurFsm.unFsmCtx.stSysAcqCtx.ucWaitQuickSrchDoCnf = ucWaitQuickSrchDoCnfFlg;
}
...
VOS_VOID NAS_MSCC_SetHrpdNoRfFlg_BSR(
    VOS_UINT8                           ucHrpdNoRfFlg
)
{
    /* 如果当前状态机不是BSR */
    if (NAS_MSCC_FSM_BETTER_SYSTEM_RESELECTION != NAS_MSCC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(UEPS_PID_MSCC, "NAS_MSCC_SetHrpdNoRfFlg_BSR ERROR:FsmId Error");
    }

    (g_stNasMsccCtx.stCurFsm.unFsmCtx.stBsrCtx.ucHrpdNoRfFlag) = ucHrpdNoRfFlg;
}



VOS_UINT8 NAS_MSCC_GetHrpdNoRfFlg_BSR(VOS_VOID)
{
    /* 如果当前状态机不是BSR */
    if (NAS_MSCC_FSM_BETTER_SYSTEM_RESELECTION != NAS_MSCC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(UEPS_PID_MSCC, "NAS_MSCC_GetHrpdNoRfFlg_BSR ERROR:FsmId Error");
    }

    return (g_stNasMsccCtx.stCurFsm.unFsmCtx.stBsrCtx.ucHrpdNoRfFlag);
}
...
NAS_MSCC_BSR_CTRL_CTX_STRU* NAS_MSCC_GetBsrCtrlCtxAddr(VOS_VOID)
{
    return &(NAS_MSCC_GetMsccCtxAddr()->stBsrCtrl);
}


NAS_MSCC_EQUPLMN_INFO_STRU *NAS_MSCC_GetClEplmnInfoAddr(VOS_VOID)
{
    return &(NAS_MSCC_GetMsccCtxAddr()->stSysAcqCtrl.stEplmnListInClMode);
}
...
{
    NAS_MSCC_GetBsrCtrlCtxAddr()->ulCurrBsrTimerLen = ulBsrTimerLen;
...
{
    return NAS_MSCC_GetBsrCtrlCtxAddr()->ulCurrBsrTimerLen;
}
...
VOS_UINT8 NAS_MSCC_GetVolteEmcSrvAcqExistFlag(VOS_VOID)
{
    return NAS_MSCC_GetSysAcqCtrlInfoAddr()->ucVolteEmcSrvAcqExistFlag;
}


VOS_VOID NAS_MSCC_SetVolteEmcSrvAcqExistFlag(
    VOS_UINT8                           ucVolteEmcSrvAcqExistFlag
)
{
    NAS_MSCC_GetSysAcqCtrlInfoAddr()->ucVolteEmcSrvAcqExistFlag = ucVolteEmcSrvAcqExistFlag;
    return;
}


VOS_VOID NAS_MSCC_SetVolteEmcSrvAcqExistFlagBySrvType(
    NAS_MSCC_PIF_SRV_TYPE_ENUM_UINT8    ucSrvType
)
{
    if (NAS_MSCC_PIF_SRV_TYPE_PS_MO_EMERGENCY_CALL == ucSrvType)
    {
        NAS_MSCC_SetVolteEmcSrvAcqExistFlag(VOS_TRUE);
    }
...
VOS_VOID NAS_MSCC_SetOrigBsrTimerStartRat(VOS_VOID)
{
    if (VOS_TRUE == NAS_MSCC_GetHrpdCampOnFlag())
    {
        NAS_MSCC_GetBsrCtrlCtxAddr()->enOrigBsrTimerStartRat = VOS_RATMODE_HRPD;
    }
    else if (VOS_TRUE == NAS_MSCC_Get3gppCampOnFlag())
    {
        NAS_MSCC_GetBsrCtrlCtxAddr()->enOrigBsrTimerStartRat = VOS_RATMODE_LTE;
    }
    else
    {
    }
...
{
    return NAS_MSCC_GetBsrCtrlCtxAddr()->enOrigBsrTimerStartRat;
}
...
)
{
    ...
    NAS_MSCC_InitClSysAcqDsdsStrategyInfo(&pstCustomCfgInfo->stClSysAcqDsdsStrategy);
...
)
{
    ...
    pstMsSysCfgInfo->enLteCapSta        = NAS_MSCC_LTE_CAPABILITY_STATUS_BUTT;
    pstMsSysCfgInfo->enDisableLReason   = NAS_MSCC_DISABLE_LTE_REASON_BUTT;
...
)
{
    ...
    NAS_MEM_SET_S(&(pstSysAcqTimerCfg->stBsrTimerCtrl), sizeof(pstSysAcqTimerCfg->stBsrTimerCtrl), 0x00, sizeof(pstSysAcqTimerCfg->stBsrTimerCtrl));
    pstSysAcqTimerCfg->stBsrTimerCtrl.ucBsrTimerActivateFlag = VOS_TRUE;

    pstSysAcqTimerCfg->stBsrTimerCtrl.ucBsrPhaseNum          = NAS_MSCC_MAX_BSR_PHASE_NUM;

    pstSysAcqTimerCfg->stBsrTimerCtrl.astBsrTimerInfo[0].ucMaxHistorySrchTimes = NAS_MSCC_DEFAULT_BSR_PHASE_ONE_HISTORY_SRCH_MAX_TIMES;
    pstSysAcqTimerCfg->stBsrTimerCtrl.astBsrTimerInfo[0].ucMaxPrefBandSrchTimes = NAS_MSCC_DEFAULT_BSR_PHASE_ONE_PREF_BAND_SRCH_MAX_TIMES;

    pstSysAcqTimerCfg->stBsrTimerCtrl.astBsrTimerInfo[0].ulBsrTimerLen  = TI_NAS_MSCC_BSR_TIMER_LEN;
    pstSysAcqTimerCfg->stBsrTimerCtrl.astBsrTimerInfo[0].ulBsrTimerLenWithNoMatchedMsplRec = TI_NAS_MSCC_DEFAULT_BSR_TIMER_LEN_WITH_UNMATCHED_MSPL_REC;

    pstSysAcqTimerCfg->stBsrTimerCtrl.astBsrTimerInfo[1].ucMaxHistorySrchTimes = NAS_MSCC_DEFAULT_BSR_PHASE_TWO_HISTORY_SRCH_MAX_TIMES;
    pstSysAcqTimerCfg->stBsrTimerCtrl.astBsrTimerInfo[1].ucMaxPrefBandSrchTimes = NAS_MSCC_DEFAULT_BSR_PHASE_TWO_PREF_BAND_SRCH_MAX_TIMES;

    pstSysAcqTimerCfg->stBsrTimerCtrl.astBsrTimerInfo[1].ulBsrTimerLen  = TI_NAS_MSCC_DEFAULT_PHASE_TWO_BSR_TIMER_LEN;
    pstSysAcqTimerCfg->stBsrTimerCtrl.astBsrTimerInfo[1].ulBsrTimerLenWithNoMatchedMsplRec = TI_NAS_MSCC_DEFAULT_PHASE_TWO_BSR_TIMER_LEN_WITH_UNMATCHED_MSPL_REC;

    pstSysAcqTimerCfg->stBsrTimerCtrl.ucEhrpdConnBsrActiveFlg = VOS_TRUE;
    pstSysAcqTimerCfg->stBsrTimerCtrl.ucHrpdConnBsrActiveFlg  = VOS_FALSE;
...
{
    PS_MEM_SET(pstSysAcqCtrlInfo, 0x00, sizeof(NAS_MSCC_SYS_ACQ_CTRL_CTX_STRU));
    NAS_MSCC_SetVolteEmcSrvAcqExistFlag(VOS_FALSE);

    pstSysAcqCtrlInfo->ucMoCallConnecting            = VOS_FALSE;
    ...
    pstSysAcqCtrlInfo->enLteSysPriClass         = NAS_MSCC_PIF_PLMN_PRIORITY_ANY;

    pstSysAcqCtrlInfo->ucIsEmcCallExist         = VOS_FALSE;
    pstSysAcqCtrlInfo->ulCallBackEnableFlg      = VOS_FALSE;

    pstSysAcqCtrlInfo->ulEpsAttachAllow          = VOS_TRUE;
    pstSysAcqCtrlInfo->ulPrioSrchLastRatFlag     = VOS_FALSE;

    PS_MEM_SET(&pstSysAcqCtrlInfo->stEplmnListInClMode, 0x0, sizeof(NAS_MSCC_EQUPLMN_INFO_STRU));

}



VOS_VOID NAS_MSCC_InitBsrCtrlCtx(
    NAS_MSCC_BSR_CTRL_CTX_STRU         *pstBsrCtrlCtx
)
{
    pstBsrCtrlCtx->enOrigBsrTimerStartRat = VOS_RATMODE_BUTT;
    pstBsrCtrlCtx->ulCurrBsrTimerLen = 0;
    pstBsrCtrlCtx->ulDataCallSwitchTo1x  = VOS_FALSE;
    pstBsrCtrlCtx->ucCurBsrPhase            = 0;
    pstBsrCtrlCtx->ucCurBsrHistroySrchTimes  = 0;
    pstBsrCtrlCtx->ucCurBsrPrefBandSrchTimes = 0;
    pstBsrCtrlCtx->enCurBsrSrchType = NAS_MSCC_BSR_SRCH_TYPE_ENUM_BUTT;
    pstBsrCtrlCtx->enCurHrpdSessionType     = NAS_MSCC_HRPD_SESSION_TYPE_BUTT;
}




VOS_VOID  NAS_MSCC_Set1xMoCallConnectingFlag(
    VOS_UINT8                           ucFlg
)
{
    NAS_MSCC_GetMsccCtxAddr()->stSysAcqCtrl.ucMoCallConnecting = ucFlg;
}


VOS_UINT8  NAS_MSCC_Get1xMoCallConnectingFlag(VOS_VOID)
{
    return NAS_MSCC_GetMsccCtxAddr()->stSysAcqCtrl.ucMoCallConnecting;
}
...
NAS_MSCC_PIF_PLMN_PRIORITY_CLASS_ENUM_UINT8  NAS_MSCC_GetAllowSrchLteSysPriClass(VOS_VOID)
{
    return NAS_MSCC_GetMsccCtxAddr()->stSysAcqCtrl.enLteSysPriClass;
}


VOS_VOID NAS_MSCC_SetAllowSrchLteSysPriClass(
    NAS_MSCC_PIF_PLMN_PRIORITY_CLASS_ENUM_UINT8             enLteSysPriClass
)
{
    NAS_MSCC_GetMsccCtxAddr()->stSysAcqCtrl.enLteSysPriClass = enLteSysPriClass;
}




VOS_UINT32  NAS_MSCC_GetEpsAttachAllowFlg(VOS_VOID)
{
    return NAS_MSCC_GetMsccCtxAddr()->stSysAcqCtrl.ulEpsAttachAllow;
}


VOS_VOID NAS_MSCC_SetEpsAttachAllowFlg(
    VOS_UINT32                          ulEpsAttachAllow
)
{
    NAS_MSCC_GetMsccCtxAddr()->stSysAcqCtrl.ulEpsAttachAllow = ulEpsAttachAllow;
}



VOS_UINT32  NAS_MSCC_GetDataCallSwitchTo1xFlg(VOS_VOID)
{
    return NAS_MSCC_GetBsrCtrlCtxAddr()->ulDataCallSwitchTo1x;
}


VOS_VOID NAS_MSCC_SetDataCallSwitchTo1xFlg(
    VOS_UINT32                          ulDataCallSwitchTo1x
)
{

    NAS_MSCC_GetBsrCtrlCtxAddr()->ulDataCallSwitchTo1x = ulDataCallSwitchTo1x;
...
{
    /* 数据也在1x上BSR LTE只控制CCF用例,主线默认关闭-保证快速回LTE */
    pstSysAcqStrategy->stClSysAcqCtrlInfo.uc1xBsrLteActiveFlg = VOS_FALSE;
    pstSysAcqStrategy->stClSysAcqCtrlInfo.uc1xBsrLteTimerLen  = TI_NAS_MSCC_DEFAULT_DATA_CALL_ON_1X_BSR_LTE_TIMER_LEN;
    ...
    pstSysAcqStrategy->stClSysAcqCtrlInfo.ucSrlte1xBsrLteEnableFlag = VOS_TRUE;
}


NAS_MSCC_1X_SERVICE_CL_SYSTEM_ACQUIRE_CTRL_STRU* NAS_MSCC_Get1xServiceClSysAcqCtrlAddr(VOS_VOID)
{
    return &(NAS_MSCC_GetMsccCtxAddr()->stMsCfgInfo.stCustomCfgInfo.st1xSrvClSysAcqStrategy.stClSysAcqCtrlInfo);
}
...
VOS_VOID NAS_MSCC_InitClSysAcqDsdsStrategyInfo(
    NAS_MSCC_CL_SYSTEM_ACQUIRE_DSDS_STRATEGY_STRU          *pstSysAcqDsdsStrategy
)
{
    pstSysAcqDsdsStrategy->ucReAcqLteWithNoRfEnable    = VOS_TRUE;

    pstSysAcqDsdsStrategy->ucReAcqLteWithNoRfDelayTime = TI_NAS_MSCC_WAIT_MMC_RF_AVAILABLE_IND_LEN;

    pstSysAcqDsdsStrategy->enReAcqLteWithNoRfScene    = NAS_MSCC_CL_SYS_ACQ_DSDS_STRATEGY_SCENE_SWITCH_ON;
}



NAS_MSCC_CL_SYSTEM_ACQUIRE_DSDS_STRATEGY_STRU* NAS_MSCC_GetClSysAcqDsdsStrategyCfgAddr(VOS_VOID)
{
    return &(NAS_MSCC_GetMsccCtxAddr()->stMsCfgInfo.stCustomCfgInfo.stClSysAcqDsdsStrategy);
}


VOS_UINT8  NAS_MSCC_GetReAcqLteWithLteNoRfEnableFlg(VOS_VOID)
{
    return (NAS_MSCC_GetMsccCtxAddr()->stMsCfgInfo.stCustomCfgInfo.stClSysAcqDsdsStrategy.ucReAcqLteWithNoRfEnable);
}


VOS_UINT16  NAS_MSCC_GetReAcqLteWithLteNoRfDelayTime(VOS_VOID)
{
    return (NAS_MSCC_GetMsccCtxAddr()->stMsCfgInfo.stCustomCfgInfo.stClSysAcqDsdsStrategy.ucReAcqLteWithNoRfDelayTime);
}


NAS_MSCC_CL_SYS_ACQ_DSDS_STRATEGY_SCENE_ENUM_UINT8  NAS_MSCC_GetReAcqLteWithLteNoRfSysAcqScene(VOS_VOID)
{
    return (NAS_MSCC_GetMsccCtxAddr()->stMsCfgInfo.stCustomCfgInfo.stClSysAcqDsdsStrategy.enReAcqLteWithNoRfScene);
}
...
NAS_MSCC_DISABLE_LTE_REASON_ENUM_UINT32 NAS_MSCC_GetDisableLReason(VOS_VOID)
{
    return (NAS_MSCC_GetMsccCtxAddr()->stMsCfgInfo.stMsSysCfgInfo.enDisableLReason);
}


NAS_MSCC_LTE_CAPABILITY_STATUS_ENUM_UINT32 NAS_MSCC_GetLteCapbilityStatus(VOS_VOID)
{
    return (NAS_MSCC_GetMsccCtxAddr()->stMsCfgInfo.stMsSysCfgInfo.enLteCapSta);
}


VOS_VOID NAS_MSCC_SetLteCapbilityStatus(
    NAS_MSCC_LTE_CAPABILITY_STATUS_ENUM_UINT32              enLteCapSta
)
{
    (NAS_MSCC_GetMsccCtxAddr()->stMsCfgInfo.stMsSysCfgInfo.enLteCapSta) = enLteCapSta;
}


VOS_VOID NAS_MSCC_SetDisableLteReason(
    NAS_MSCC_DISABLE_LTE_REASON_ENUM_UINT32                 enDisableLReason
)
{
    (NAS_MSCC_GetMsccCtxAddr()->stMsCfgInfo.stMsSysCfgInfo.enDisableLReason) = enDisableLReason;
}




VOS_UINT32  NAS_MSCC_Get1xBsrLteTimerLen(VOS_VOID)
{
    VOS_UINT32                          ulTimerLen;

    ulTimerLen = NAS_MSCC_Get1xServiceClSysAcqCtrlAddr()->uc1xBsrLteTimerLen * NAS_MSCC_THOUSAND_MILLISECOND_UNIT;
...
VOS_VOID NAS_MSCC_InitUeStatusInfo(
    NAS_MSCC_UE_STATUS_INFO_STRU       *pstUeStatusInfo
)
{
    NAS_MEM_SET_S(pstUeStatusInfo,
                  sizeof(NAS_MSCC_UE_STATUS_INFO_STRU),
                  0x00,
                  sizeof(NAS_MSCC_UE_STATUS_INFO_STRU));

    pstUeStatusInfo->enUe1xMainState   = NAS_MSCC_PIF_1X_CAS_STATE_ENUM_BUTT;
    pstUeStatusInfo->enUe1xSubState    = NAS_MSCC_PIF_1X_CAS_SUB_STATE_ENUM_BUTT;
    pstUeStatusInfo->enUe1xServiceType = NAS_MSCC_PIF_1X_SERVICE_TYPE_BUTT;
}


NAS_MSCC_UE_STATUS_INFO_STRU *NAS_MSCC_GetUeStatusInfo(VOS_VOID)
{
    return &(NAS_MSCC_GetMsccCtxAddr()->stUeStatusInfo);
}


NAS_MSCC_PIF_1X_CAS_MAIN_STATE_ENUM_UINT8 NAS_MSCC_Get1xUeMainState(VOS_VOID)
{
    return (NAS_MSCC_GetMsccCtxAddr()->stUeStatusInfo.enUe1xMainState);
}


NAS_MSCC_PIF_1X_SERVICE_TYPE_ENUM_UINT8 NAS_MSCC_Get1xUeServiceType(VOS_VOID)
{
    return (NAS_MSCC_GetMsccCtxAddr()->stUeStatusInfo.enUe1xServiceType);
}


NAS_MSCC_PIF_NET_RAT_TYPE_ENUM_UINT8 NAS_MSCC_GetLastCampOnLTEOrDoRat(VOS_VOID)
{
    return (NAS_MSCC_GetMsccCtxAddr()->stNwInfo.stLastSysInfo.enLastCampOnLTEOrDoRat);
}


VOS_VOID NAS_MSCC_SetLastCampOnLTEOrDoRat(
    NAS_MSCC_PIF_NET_RAT_TYPE_ENUM_UINT8                    enLastCampOnLTEOrDoRat
)
{
    NAS_MSCC_GetMsccCtxAddr()->stNwInfo.stLastSysInfo.enLastCampOnLTEOrDoRat = enLastCampOnLTEOrDoRat;
}


VOS_VOID NAS_MSCC_SetPrioSrchLastRatFlag(
    VOS_UINT32                          ulPrioSrchLastRatFlag
)
{
    NAS_MSCC_GetMsccCtxAddr()->stSysAcqCtrl.ulPrioSrchLastRatFlag = ulPrioSrchLastRatFlag;
}


VOS_UINT32 NAS_MSCC_GetPrioSrchLastRatFlag(VOS_VOID)
{
    return (NAS_MSCC_GetMsccCtxAddr()->stSysAcqCtrl.ulPrioSrchLastRatFlag);
}



VOS_VOID NAS_MSCC_SetBsrHrpdNoRfFlag_BSR(
    NAS_MSCC_BSR_HRPD_NORF_TYPE_ENUM_UINT8                  enNoRfType
)
{
    /* 如果当前状态机不是BSR */
    if (NAS_MSCC_FSM_BETTER_SYSTEM_RESELECTION != NAS_MSCC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(UEPS_PID_MSCC, "NAS_MSCC_SetBsrHrpdNoRfFlag_BSR ERROR:FsmId Error");
    }

    g_stNasMsccCtx.stCurFsm.unFsmCtx.stBsrCtx.enBsrHrpdNoRfType = enNoRfType;

    return;
}


NAS_MSCC_BSR_HRPD_NORF_TYPE_ENUM_UINT8 NAS_MSCC_GetBrsHrpdNoRfType_BSR(VOS_VOID)
{
    /* 如果当前状态机不是BSR */
    if (NAS_MSCC_FSM_BETTER_SYSTEM_RESELECTION != NAS_MSCC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(UEPS_PID_MSCC, "NAS_MSCC_GetBrsHrpdNoRfType_BSR ERROR:FsmId Error");
    }

    return g_stNasMsccCtx.stCurFsm.unFsmCtx.stBsrCtx.enBsrHrpdNoRfType;
}



VOS_VOID NAS_MSCC_SetBsrLteNoRfType_BSR(
    NAS_MSCC_BSR_LTE_NORF_TYPE_ENUM_UINT8                   enLteNoRfType
)
{
    /* 如果当前状态机不是BSR */
    if (NAS_MSCC_FSM_BETTER_SYSTEM_RESELECTION != NAS_MSCC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(UEPS_PID_MSCC, "NAS_MSCC_SetBsrLteNoRfType_BSR ERROR:FsmId Error");
    }

    g_stNasMsccCtx.stCurFsm.unFsmCtx.stBsrCtx.enBsrLteNoRfType = enLteNoRfType;

    return;
}


NAS_MSCC_BSR_LTE_NORF_TYPE_ENUM_UINT8 NAS_MSCC_GetBsrLteNoRfType_BSR(VOS_VOID)
{
    /* 如果当前状态机不是BSR */
    if (NAS_MSCC_FSM_BETTER_SYSTEM_RESELECTION != NAS_MSCC_GetCurrFsmId())
    {
        NAS_ERROR_LOG(UEPS_PID_MSCC, "NAS_MSCC_GetBsrLteNoRfType_BSR ERROR:FsmId Error");
    }

    return g_stNasMsccCtx.stCurFsm.unFsmCtx.stBsrCtx.enBsrLteNoRfType;
}
#endif


NAS_MSCC_SYS_ACQ_BSR_CTRL_STRU*  NAS_MSCC_GetBsrCtrl(VOS_VOID)
{
    return &(NAS_MSCC_GetSysAcqTimerCfgAddr()->stBsrTimerCtrl);
}


VOS_UINT8 NAS_MSCC_GetCurBsrTimerPhase(VOS_VOID)
{
    return NAS_MSCC_GetBsrCtrlCtxAddr()->ucCurBsrPhase;
}


VOS_VOID NAS_MSCC_SetCurBsrTimerPhase(VOS_UINT8 ucCurBsrPhase)
{
    NAS_MSCC_GetBsrCtrlCtxAddr()->ucCurBsrPhase = ucCurBsrPhase;
}


VOS_UINT8 NAS_MSCC_GetCurBsrHistorySrchTimes(VOS_VOID)
{
    return NAS_MSCC_GetBsrCtrlCtxAddr()->ucCurBsrHistroySrchTimes;
}


VOS_VOID NAS_MSCC_SetCurBsrHistorySrchTimes(VOS_UINT8 ucCurBsrTimes)
{
    NAS_MSCC_GetBsrCtrlCtxAddr()->ucCurBsrHistroySrchTimes = ucCurBsrTimes;
}


VOS_UINT8 NAS_MSCC_GetCurBsrPrefBandSrchTimes(VOS_VOID)
{
    return NAS_MSCC_GetBsrCtrlCtxAddr()->ucCurBsrPrefBandSrchTimes;
}


VOS_VOID NAS_MSCC_SetCurBsrPrefBandSrchTimes(VOS_UINT8 ucCurBsrTimes)
{
    NAS_MSCC_GetBsrCtrlCtxAddr()->ucCurBsrPrefBandSrchTimes= ucCurBsrTimes;
}


NAS_MSCC_BSR_SRCH_TYPE_ENUM_UINT8 NAS_MSCC_GetCurBsrSrchType(VOS_VOID)
{
    return NAS_MSCC_GetBsrCtrlCtxAddr()->enCurBsrSrchType;
}


VOS_VOID NAS_MSCC_SetCurBsrSrchType(
    NAS_MSCC_BSR_SRCH_TYPE_ENUM_UINT8   enCurBsrSrchType
)
{
    NAS_MSCC_GetBsrCtrlCtxAddr()->enCurBsrSrchType = enCurBsrSrchType;

}


VOS_VOID NAS_MSCC_ResetBsrRelatedCtrlInfo(VOS_VOID)
{
    NAS_NORMAL_LOG(UEPS_PID_MSCC, "NAS_MSCC_ResetBsrRelatedCtrlInfo Entered.");

    NAS_MSCC_SetCurBsrTimerPhase(0);
...
NAS_MSCC_TEST_CONFIG_STRU* NAS_MSCC_GetTestConfig(VOS_VOID)
{
    return &(NAS_MSCC_GetMsccCtxAddr()->stMsCfgInfo.stCustomCfgInfo.stTestConfig);
}



VOS_UINT8 NAS_MSCC_GetNoCardModeCfgFlag(VOS_VOID)
{
    return NAS_MSCC_GetTestConfig()->ucNoCardModeCfgFlg;
}



VOS_VOID NAS_MSCC_SetNoCardModeCfgFlag(
    VOS_UINT8                           ucNoCardModeCfgFlg
)
{
    NAS_MSCC_GetTestConfig()->ucNoCardModeCfgFlg = ucNoCardModeCfgFlg;
}
...
NAS_MSCC_HRPD_SESSION_TYPE_ENUM_UINT8 NAS_MSCC_GetCurHrpdSessionType(VOS_VOID)
{
    return NAS_MSCC_GetBsrCtrlCtxAddr()->enCurHrpdSessionType;
}


VOS_VOID NAS_MSCC_SetCurHrpdSessionType(
    NAS_MSCC_HRPD_SESSION_TYPE_ENUM_UINT8                   enHrpdSessionType
)
{
    NAS_MSCC_GetBsrCtrlCtxAddr()->enCurHrpdSessionType = enHrpdSessionType;
}
