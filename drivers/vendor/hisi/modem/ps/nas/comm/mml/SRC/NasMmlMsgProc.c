...
)
{
    ...
    switch ( ulRcvPid )
    {
        ...
        case WUEPS_PID_BACKOFF:
            fReturnMsgProcHandle    = NAS_BACKOFF_PidMsgEntry;
            break;
...
)
{
    ...
    while (VOS_NULL_PTR != pstNextMsg)
    {
        ...
        else
        {
            ...
            if ( VOS_FALSE == ulRslt )
            {
                NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MML_MsgProc: UTRANCTRL process finished, go on processing");
...
)
{
    ...
    switch( enInitPhase )
    {
        case   VOS_IP_LOAD_CONFIG:
            ...
            ulReturnCode = VOS_RegisterPIDInfo(WUEPS_PID_BACKOFF,
                                  (Init_Fun_Type) NAS_BACKOFF_PidInit,
                                  (Msg_Fun_Type) NAS_MML_FidMsgProc);
            if( VOS_OK != ulReturnCode )
            {
                return VOS_ERR;
