...
)
{
    ...
    pstSimStatusInfo->stUsimRefreshInfo.ulUsimRefreshFlag = VOS_FALSE;
    pstSimStatusInfo->stUsimRefreshInfo.enRefreshType     = USIMM_REFRESH_BUTT;
}
...
VOS_VOID NAS_MML_InitSimNativeContentInfo(
    NAS_MML_SIM_NATIVE_CONTENT_STRU    *pstSimNativeContent
)
{
    NAS_MEM_SET_S(pstSimNativeContent, sizeof(NAS_MML_SIM_NATIVE_CONTENT_STRU), 0x0, sizeof(NAS_MML_SIM_NATIVE_CONTENT_STRU));
}
...
)
{
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_MML_InitSimForbidPlmnInfo: Power off Scene, do not clear FORB PLMN INFO");
        return;
    }
...
VOS_VOID NAS_MML_InitAttCfg(
    NAS_MML_ATT_CFG_STRU               *pstAttCfg
)
{
    pstAttCfg->ucNetSelMenuFlg          = VOS_FALSE;
    pstAttCfg->ucRatBalancingFlg        = VOS_FALSE;
    pstAttCfg->ucReserve1               = 0;
    pstAttCfg->ucReserve2               = 0;
...
VOS_VOID NAS_MML_InitAiModemCfg(
    NAS_MML_AI_MODEM_CFG_STRU               *pstAiModemCfg
)
{
    PS_MEM_SET(pstAiModemCfg, 0x00, sizeof(NAS_MML_AI_MODEM_CFG_STRU));

    pstAiModemCfg->ucActFlg = VOS_FALSE;
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_SetKeepCsForbInfoCfg: ucKeepCsForbInfoFlg", ucKeepCsForbInfoFlg);
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_GetKeepCsForbInfoCfg: ucKeepCsForbInfoFlg", (NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.ucKeepCsForbInfoFlg));
    return (NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.ucKeepCsForbInfoFlg);
}



VOS_VOID NAS_MML_SetExtendT3240LenFlg(
    VOS_UINT8                           ucExtendT3240LenFlg
)
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_SetExtendT3240LenFlg: ucExtendT3240LenFlg", ucExtendT3240LenFlg);
    NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stExtendT3240LenCfg.ucExtendT3240LenFlg = ucExtendT3240LenFlg;
}


VOS_UINT8 NAS_MML_GetExtendT3240LenFlg( VOS_VOID )
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_GetExtendT3240LenFlg: ucExtendT3240LenFlg", (NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stExtendT3240LenCfg.ucExtendT3240LenFlg));
    return (NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stExtendT3240LenCfg.ucExtendT3240LenFlg);
}


VOS_VOID NAS_MML_SetCustomizedT3240Len(
    VOS_UINT16                           usCustomizedT3240Len
)
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_SetCustomizedT3240Len: usCustomizedT3240Len", usCustomizedT3240Len);
    NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stExtendT3240LenCfg.usCustomizedT3240Len = usCustomizedT3240Len;
}


VOS_UINT16 NAS_MML_GetCustomizedT3240Len( VOS_VOID )
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_GetCustomizedT3240Len: usCustomizedT3240Len", (NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stExtendT3240LenCfg.usCustomizedT3240Len));
    return (NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stExtendT3240LenCfg.usCustomizedT3240Len);
}
...
)
{
    ...
    NAS_MML_InitSimNativeContentInfo(&pstSimInfo->stSimNativeContent);
}
...
)
{
    ...
    PS_MEM_SET(&(pstMsCapability->stMsNotifyNwCapInfo), 0X00, sizeof(NAS_MML_MS_NOTIFY_NW_INFO));
...
)
{
    ...
    NAS_MML_InitSysCfgInfo(&(pstMsSysCfgInfo->stSysCfgInfo));
...
VOS_VOID  NAS_MML_InitCustomSupplementOplmnInfo(
    NAS_MML_CUSTOM_SUPPLEMENT_OPLMN_INFO_STRU              *pstCustomSupplemetnOplmnInfo
)
{
    pstCustomSupplemetnOplmnInfo->ucImsiPlmnListNum = 0;

    PS_MEM_SET( pstCustomSupplemetnOplmnInfo->astImsiPlmnList,
                (VOS_UINT8)0X00,
                sizeof(pstCustomSupplemetnOplmnInfo->astImsiPlmnList));

    pstCustomSupplemetnOplmnInfo->ucOplmnListNum= 0;

    PS_MEM_SET( pstCustomSupplemetnOplmnInfo->astOPlmnList,
                (VOS_UINT8)0X00,
                sizeof(pstCustomSupplemetnOplmnInfo->astOPlmnList));
...
)
{
    ...
    /* 惩罚时间默认60分钟 */
    NAS_MML_SetCsgForbiddenPlmnPeriod(NAS_MML_SIXTY_MINUTE *60 * 1000);
    /* 默认不支持SVLTE特性 */
    ...
    NAS_MML_SetDamActiveFlag(VOS_FALSE);
    ...
    NAS_MML_InitOosBgNetworkSearchCustomInfo(&(pstCustomCfg->stOosBgNetworkSearchCustom));
    ...
    NAS_MML_InitAttCfg(&(pstCustomCfg->stAttCfg));

    NAS_MML_SetKeepCsForbInfoCfg(VOS_FALSE);

    NAS_MML_InitUsimGsmAuthCfgInfo(&(pstCustomCfg->stCleanCksnCfg));

    NAS_MML_SetExtendT3240LenFlg(VOS_FALSE);
    NAS_MML_SetCustomizedT3240Len(MM_TIMER_T3240_VALUE);

    NAS_MML_InitCustomSupplementOplmnInfo(&(pstCustomCfg->stSupplementOplmnInfo));

    NAS_MML_InitAiModemCfg(&(pstCustomCfg->stAiModemCfg));

    NAS_MML_SetRefreshAllFileRestartFlag(VOS_FALSE);
...
)
{
    ...
    pstCampPlmnInfo->ulCsgId              = NAS_MML_INVALID_CSG_ID_VALUE;
    PS_MEM_SET(&pstCampPlmnInfo->stCsgIdHomeNodeBName, 0, sizeof(pstCampPlmnInfo->stCsgIdHomeNodeBName));
...
)
{
    ...
    for ( i = 0; i < RRC_NAS_AT_CELL_MAX_NUM; i++ )
    {
        ...
        pstCampCellInfo->astCellInfo[i].enCellType = MMC_LMM_CELL_TYPE_BUTT;
    }
...
)
{
    ...
    pstCsDomainInfo->stCmSrvStatus.enMoLcsStatus    = NAS_MML_LCS_STATUS_BUTT;

    pstCsDomainInfo->stCmSrvStatus.enMtCsfbPagingTimerStatus = NAS_MML_TIMER_STATUS_STOP;

    pstCsDomainInfo->stCmSrvStatus.enDelayNetworkSearchTimerStatus  = NAS_MML_TIMER_STATUS_STOP;

    pstCsDomainInfo->stCmSrvStatus.ucCellNotSupportCsmoFlg   =   VOS_FALSE;

    pstCsDomainInfo->ucCsDamNoSearchFlag = VOS_FALSE;
}
...
)
{
    ...
    pstPsDomainInfo->ucPsDamNoSearchFlag = VOS_FALSE;
...
)
{
   ...
   pstEpsDomainInfo->ucEpsEmcAttachFlg        = VOS_FALSE;
...
)
{
    ...
    pstImsDomainInfo->ucImsNormalCallExistFlg = VOS_FALSE;
    pstImsDomainInfo->ucImsEmcCallExistFlg = VOS_FALSE;
...
)
{
    ...
    NAS_MML_SetPsRelCause(RRC_REL_CAUSE_BUTT);
}
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_GetSimPresentStatus: ucSimPresentStatus", (NAS_MML_GetMmlCtx()->stSimInfo.stSimStatus.ucSimPresentStatus));
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_GetSimType: enSimType", (NAS_MML_GetMmlCtx()->stSimInfo.stSimStatus.enSimType));
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_SetSimType: enSimType", enSimType);
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_GetSimCsRegStatus: ucSimCsRegStatus", (NAS_MML_GetMmlCtx()->stSimInfo.stSimStatus.ucSimCsRegStatus));
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_GetSimPsRegStatus: ucSimPsRegStatus", (NAS_MML_GetMmlCtx()->stSimInfo.stSimStatus.ucSimPsRegStatus));
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_SetSimPresentStatus: ucSimPresentStatus", ucSimPresentStatus);
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_SetSimCsRegStatus: ucSimCsRegStatus", ucSimCsRegStatus);
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_SetSimPsRegStatus: ucSimPsRegStatus", ucSimPsRegStatus);
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_SetPsUpdateStatus: enPsUpdateStatus", enPsUpdateStatus);
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_GetPsUpdateStatus: enPsUpdateStatus", NAS_MML_GetMmlCtx()->stSimInfo.stSimStatus.enPsUpdateStatus);
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_SetCsUpdateStatus: enCsUpdateStatus", enCsUpdateStatus);
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_GetCsUpdateStatus: enCsUpdateStatus", NAS_MML_GetMmlCtx()->stSimInfo.stSimStatus.enCsUpdateStatus);
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_GetImsiRefreshStatus: ucImsiRefreshStatus", NAS_MML_GetMmlCtx()->stSimInfo.stSimStatus.ucImsiRefreshStatus);
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_SetImsiRefreshStatus: ucImsiRefreshStatus", ucImsiRefreshStatus);
...
{
    if (NAS_MML_CKSN_INVALID == ucCksn)
    {
        NAS_NORMAL_LOG(WUEPS_PID_GMM, "NAS_MML_SetSimPsSecurityCksn: Clear CKSN, Set to NAS_MML_CKSN_INVALID");
    }
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_SetSimHplmnTimerLen: ulSimHplmnTimerLen", ulSimHplmnTimerLen);
...
{
    VOS_UINT8                          *paucHplmnPeriFile  = VOS_NULL_PTR;
    VOS_UINT32                          ulSimHplmnTimerLen;                    /* SIM卡HPLMN定时器时长 */

    paucHplmnPeriFile = NAS_MML_GetSimHplmnPeriFileContent();
    if (VOS_TRUE == NAS_MML_GetScanCtrlEnableFlg())
    {
        if (0xFF == *paucHplmnPeriFile)
        {
            /* R99: 默认值为60分钟*60秒*1000毫秒 */
            ulSimHplmnTimerLen = (NAS_MML_ATT_HPPLMN_DEFAULT_TIMER_LEN_R99 * NAS_MML_SIXTY_SECOND * NAS_MML_ONE_THOUSAND_MILLISECOND);

            if (PS_PTL_VER_PRE_R99 == NAS_Common_Get_Supported_3GPP_Version(MM_COM_SRVDOMAIN_CS))
            {
                /* R97: 默认值为30分钟*60秒*1000毫秒 */
                ulSimHplmnTimerLen = (NAS_MML_ATT_HPPLMN_DEFAULT_TIMER_LEN_R97 * NAS_MML_SIXTY_SECOND * NAS_MML_ONE_THOUSAND_MILLISECOND);
            }

            NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_GetSimHplmnTimerLen: ScanCtrlEnable the SimHplmnTimerLen:",ulSimHplmnTimerLen);

            return ulSimHplmnTimerLen;
        }
    }

    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_GetSimHplmnTimerLen: ulSimHplmnTimerLen", NAS_MML_GetMmlCtx()->stSimInfo.stSimPlmnInfo.ulSimHplmnTimerLen);
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_GetMsMode: enMsMode", (NAS_MML_GetMmlCtx()->stMsCfgInfo.stMsSysCfgInfo.enMsMode));
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_GetLteCsServiceCfg: enLteCsServiceCfg", NAS_MML_GetMmlCtx()->stMsCfgInfo.stMsSysCfgInfo.enLteCsServiceCfg);
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_GetLteCapabilityStatus: enLteCapabilityStatus", (NAS_MML_GetMmlCtx()->stMsCfgInfo.stMsSysCfgInfo.enLteCapabilityStatus));
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_GetLteRoamAllowedFlg: ucLteRoamAllowedFlg", (NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stLteCustomCfgInfo.stLteRoamCfg.ucLteRoamAllowedFlg));
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_GetDisableLteRoamFlg: ulDisableLteRoamFlg", (NAS_MML_GetMmlCtx()->stMsCfgInfo.stMsSysCfgInfo.ulDisableLteRoamFlg));
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_GetLteImsSupportFlg: ucLteImsSupportFlag", NAS_MML_GetMmlCtx()->stMsCfgInfo.stMsSysCfgInfo.stImsConfig.stImsRatSupport.ucLteImsSupportFlag);
...
VOS_VOID NAS_MML_DisableLteCommProc(
    NAS_MML_LTE_CAPABILITY_STATUS_ENUM_UINT32               enLCapabilityStatus,
    MMC_LMM_DISABLE_LTE_REASON_ENUM_UINT32                  enDisableLteReason
)
{
    /* 需要Disable L全局变量 */
    NAS_MML_SetLteCapabilityStatus(enLCapabilityStatus);

    NAS_MML_SetDisableLteReason(enDisableLteReason);
...
VOS_VOID NAS_MML_EnableLteCommProc(VOS_VOID)
{
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_SetLteCapabilityStatus: enLteCapabilityStatus", enLCapabilityStatus);
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_SetDisableLteReason: ",enDisableLteReason);
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_SetDisableLteRoamFlg: is ",ulDisableFlg);
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_SetTinType: enTinType", enTinType);
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_GetLastRplmnRat: enLastRplmnRat", (NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stRplmnCfg.enLastRplmnRat));
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_SetLastRplmnRat: enLastRplmnRat", enRat);
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_GetStkSteeringOfRoamingSupportFlg: ucStkSteeringOfRoamingSupportFlg", (NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stMiscellaneousCfg.ucStkSteeringOfRoamingSupportFlg));
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_SetStkSteeringOfRoamingSupportFlg: ucStkSteeringOfRoamingSupportFlg", ucStkSteeringOfRoamingSupportFlg);
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_GetCsRejSearchSupportFlg: ucCsRejSearchSupportFlg", (NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stMiscellaneousCfg.ucCsRejSearchSupportFlg));
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_GetRoamBrokerActiveFlag: ucRoamBrokerActiveFlag", (NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stMiscellaneousCfg.ucRoamBrokerActiveFlag));
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_GetDtSingleDomainPlmnSearchFlag: ucSingleDomainFailPlmnSrchFlag", (NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stMiscellaneousCfg.ucSingleDomainFailPlmnSrchFlag));
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_SetDtSingleDomainPlmnSearchFlag: ucSingleDomainFailPlmnSrchFlag", ucPlmnSrchFlag);
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_SetCsRejSearchSupportFlg: ucCsRejSearchSupportFlg", ucCsRejSearchSupportFlg);
...
)
{
    ...
    PS_MEM_SET(&pstSimCsgInfo->stAllowedCsgListRecordsStream, 0, sizeof(pstSimCsgInfo->stAllowedCsgListRecordsStream));
    PS_MEM_SET(&pstSimCsgInfo->stAllowedCsgListRecordInfo, 0, sizeof(pstSimCsgInfo->stAllowedCsgListRecordInfo));
    PS_MEM_SET(&pstSimCsgInfo->stAllowedCsgListFilesInfo, 0, sizeof(pstSimCsgInfo->stAllowedCsgListFilesInfo));
    PS_MEM_SET(&pstSimCsgInfo->stOperatorCsgListFilesInfo, 0, sizeof(pstSimCsgInfo->stOperatorCsgListFilesInfo));
    ...
    for (ulIndex = 0; ulIndex < NAS_MML_MAX_PLMN_CSG_ID_NUM; ulIndex++)
    {
        ...
        pstSimCsgInfo->stAllowedCsgList.astPlmnWithCsgIdInfo[ulIndex].enCsgDisplayIndicator          = NAS_MML_CSG_ID_DISPLAY_INDICATOR_TYPE_NOT_EXIST;
		...
		pstSimCsgInfo->stOperatorCsgList.astPlmnWithCsgIdInfo[ulIndex].enCsgDisplayIndicator          = NAS_MML_CSG_ID_DISPLAY_INDICATOR_TYPE_NOT_EXIST;
    }
    ...
    pstSimCsgInfo->pstOperatorCsgListCache = VOS_NULL_PTR;

    return;
}

NAS_MML_CELL_TYPE_ENUM_UINT8  NAS_MML_GetCurrCampCellType( VOS_VOID )
{
    return (NAS_MML_GetMmlCtx()->stNetworkInfo.stCampPlmnInfo.stCampCellInfo.astCellInfo[0].enCellType);
}


VOS_VOID  NAS_MML_SetCurrCampCellType(
    NAS_MML_CELL_TYPE_ENUM_UINT8        enCellType
)
{
    NAS_MML_GetMmlCtx()->stNetworkInfo.stCampPlmnInfo.stCampCellInfo.astCellInfo[0].enCellType = enCellType;
    return;
}
...
NAS_MML_PLMN_WITH_CSG_ID_LIST_STRU*  NAS_MML_GetUeBasedOperatorCsgList(VOS_VOID)
{
    return &(NAS_MML_GetMmlCtx()->stSimInfo.stSimCsgInfo.stUeBasedOperatorCsgList);
}

VOS_VOID NAS_MML_InitPlmnWithCsgIdInfo(
    NAS_MML_PLMN_WITH_CSG_ID_INFO_STRU *pstPlmnWithCsdIdInfo
)
{
    PS_MEM_SET(pstPlmnWithCsdIdInfo, 0, sizeof(NAS_MML_PLMN_WITH_CSG_ID_INFO_STRU));

    pstPlmnWithCsdIdInfo->stPlmnWithCsgId.stPlmnId.ulMcc = NAS_MML_INVALID_MCC;
    pstPlmnWithCsdIdInfo->stPlmnWithCsgId.stPlmnId.ulMnc = NAS_MML_INVALID_MNC;
    pstPlmnWithCsdIdInfo->stPlmnWithCsgId.ulCsgId        = NAS_MML_INVALID_CSG_ID_VALUE;
    pstPlmnWithCsdIdInfo->enCsgDisplayIndicator          = NAS_MML_CSG_ID_DISPLAY_INDICATOR_TYPE_NOT_EXIST;
...
NAS_MML_ACSGL_RECORD_STREAM_STRU*  NAS_MML_GetAllowedCsgListRecordStream(VOS_VOID)
{
    return &(NAS_MML_GetMmlCtx()->stSimInfo.stSimCsgInfo.stAllowedCsgListRecordsStream);
}


NAS_MML_EFACSGL_RECORD_INFO_STRU*  NAS_MML_GetAllowedCsgListRecordInfo(VOS_VOID)
{
    return &(NAS_MML_GetMmlCtx()->stSimInfo.stSimCsgInfo.stAllowedCsgListRecordInfo);
}


NAS_MML_CSG_FILES_RECORD_INFO_STRU*  NAS_MML_GetAllowedCsgListFilesInfo(VOS_VOID)
{
    return &(NAS_MML_GetMmlCtx()->stSimInfo.stSimCsgInfo.stAllowedCsgListFilesInfo);
}


NAS_MML_CSG_FILES_RECORD_INFO_STRU*  NAS_MML_GetOperatorCsgListFilesInfo(VOS_VOID)
{
    return &(NAS_MML_GetMmlCtx()->stSimInfo.stSimCsgInfo.stOperatorCsgListFilesInfo);
}


VOS_VOID NAS_MML_ClearAllowedCsgListRecordStream(VOS_VOID)
{
    NAS_MML_ACSGL_RECORD_STREAM_STRU   *pstAllowedCsgListRecordsStream = VOS_NULL_PTR;
    VOS_UINT32                          i;

    pstAllowedCsgListRecordsStream = NAS_MML_GetAllowedCsgListRecordStream();

    for (i=0; i<NAS_MML_MAX_PLMN_CSG_ID_NUM; i++)
    {
        if (VOS_NULL_PTR != pstAllowedCsgListRecordsStream->apstCsgListRecordInfo[i])
        {
            PS_MEM_FREE(WUEPS_PID_MMC, pstAllowedCsgListRecordsStream->apstCsgListRecordInfo[i]);
            pstAllowedCsgListRecordsStream->apstCsgListRecordInfo[i] = VOS_NULL_PTR;
        }
    }
}


VOS_UINT32 NAS_MML_GetCurrCampCsgId( VOS_VOID )
{
    return (NAS_MML_GetMmlCtx()->stNetworkInfo.stCampPlmnInfo.ulCsgId);
}


NAS_MML_CSG_ID_HOME_NODEB_NAME_STRU* NAS_MML_GetCurrCampCsgIdHomeNodeBName( VOS_VOID )
{
    return &(NAS_MML_GetMmlCtx()->stNetworkInfo.stCampPlmnInfo.stCsgIdHomeNodeBName) ;
}
...
VOS_UINT32 NAS_MML_GetCsgForbiddenPlmnPeriod(VOS_VOID)
{
    return NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stCsgCfgInfo.stCsgForbiddenPlmnCfg.ulCsgPlmnForbiddenPeriod;
}


NAS_MML_CSG_FORBIDDEN_PLMN_CFG_STRU *NAS_MML_GetCsgForbiddenPlmnCfg(VOS_VOID)
{
    return &(NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stCsgCfgInfo.stCsgForbiddenPlmnCfg);
}



VOS_VOID NAS_MML_SetCsgForbiddenPlmnPeriod(
    VOS_UINT32                          ulCsgPlmnForbiddenPeriod
)
{
    NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stCsgCfgInfo.stCsgForbiddenPlmnCfg.ulCsgPlmnForbiddenPeriod = ulCsgPlmnForbiddenPeriod;
}



VOS_UINT32 NAS_MML_GetCsgPeriodicSearchLen(VOS_VOID)
{
    return (NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stCsgCfgInfo.ulCsgPeriodicSearchLen);
}


VOS_VOID NAS_MML_SetCsgPeriodicSearchLen(
    VOS_UINT32                          ulCsgPeriodicSearchLen
)
{
    NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stCsgCfgInfo.ulCsgPeriodicSearchLen = ulCsgPeriodicSearchLen;
}


NAS_MML_CSG_AUTONOMOUS_SEARCH_CFG_STRU* NAS_MML_GetCsgAutonomousSrchCfg(VOS_VOID)
{
    return &(NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stCsgCfgInfo.stCsgAutonomousSrchCfg);
}



NAS_MML_PLMN_WITH_CSG_ID_LIST_STRU* NAS_MML_GetOperatorCsgListCache(VOS_VOID)
{
    return NAS_MML_GetMmlCtx()->stSimInfo.stSimCsgInfo.pstOperatorCsgListCache;
}


NAS_MML_PLMN_WITH_CSG_ID_LIST_STRU* NAS_MML_AllocOperaterCsgListCache(VOS_VOID)
{
    NAS_MML_PLMN_WITH_CSG_ID_LIST_STRU                    **ppstOperatorCsgListCache;
    NAS_MML_PLMN_WITH_CSG_ID_LIST_STRU                     *pstOperatorCsgListCache;
    VOS_UINT32                                              ulIndex;

    ppstOperatorCsgListCache = &(NAS_MML_GetMmlCtx()->stSimInfo.stSimCsgInfo.pstOperatorCsgListCache);
    if (VOS_NULL_PTR != *ppstOperatorCsgListCache)
    {
        PS_MEM_FREE(WUEPS_PID_MMC, *ppstOperatorCsgListCache);
    }

    *ppstOperatorCsgListCache = (NAS_MML_PLMN_WITH_CSG_ID_LIST_STRU*)PS_MEM_ALLOC(WUEPS_PID_MMC,
                                                sizeof(NAS_MML_PLMN_WITH_CSG_ID_LIST_STRU));
    if (VOS_NULL_PTR == *ppstOperatorCsgListCache)
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MML_AllocOperaterCsgListCache:ERROR:Alloc Mem Fail.");
        return VOS_NULL_PTR;
    }
    PS_MEM_SET(*ppstOperatorCsgListCache, 0, sizeof(NAS_MML_PLMN_WITH_CSG_ID_LIST_STRU));
    pstOperatorCsgListCache = *ppstOperatorCsgListCache;
    PS_MEM_SET(pstOperatorCsgListCache, 0x0, sizeof(NAS_MML_PLMN_WITH_CSG_ID_LIST_STRU));

    for (ulIndex = 0; ulIndex < NAS_MML_MAX_PLMN_CSG_ID_NUM; ulIndex++)
    {
        pstOperatorCsgListCache->astPlmnWithCsgIdInfo[ulIndex].stPlmnWithCsgId.stPlmnId.ulMcc = NAS_MML_INVALID_MCC;
        pstOperatorCsgListCache->astPlmnWithCsgIdInfo[ulIndex].stPlmnWithCsgId.stPlmnId.ulMnc = NAS_MML_INVALID_MNC;
        pstOperatorCsgListCache->astPlmnWithCsgIdInfo[ulIndex].stPlmnWithCsgId.ulCsgId        = NAS_MML_INVALID_CSG_ID_VALUE;
		pstOperatorCsgListCache->astPlmnWithCsgIdInfo[ulIndex].enCsgDisplayIndicator          = NAS_MML_CSG_ID_DISPLAY_INDICATOR_TYPE_NOT_EXIST;
    }

    return *ppstOperatorCsgListCache;
}


VOS_VOID  NAS_MML_FreeOperaterCsgListCache(VOS_VOID)
{
    NAS_MML_PLMN_WITH_CSG_ID_LIST_STRU                    **ppstOperatorCsgListCache;
    ...
    *ppstOperatorCsgListCache = VOS_NULL_PTR;
}


VOS_VOID NAS_MML_ClearSimCsgInfo(
    NAS_MML_SIM_CSG_INFO_STRU          *pstSimCsgInfo
)
{
    VOS_UINT32                          ulIndex;
    ...
    ucRecordLen = pstSimCsgInfo->stAllowedCsgListFilesInfo.stCsgListRecordInfo.ucRecordLen;
    for (ulIndex = 0; ulIndex < NAS_MML_MAX_PLMN_CSG_ID_NUM; ulIndex++)
    {
        if (NULL != pstSimCsgInfo->stAllowedCsgListRecordsStream.apstCsgListRecordInfo[ulIndex])
        {
            PS_MEM_SET(&(pstSimCsgInfo->stAllowedCsgListRecordsStream.apstCsgListRecordInfo[ulIndex]), 0xFF, ucRecordLen);
        }
    }
...
NAS_MML_MO_LCS_STATUS_ENUM_UINT8 NAS_MML_GetMoLcsStatus(VOS_VOID)
{
    return (NAS_MML_GetMmlCtx()->stNetworkInfo.stCsDomainInfo.stCmSrvStatus.enMoLcsStatus);
}


VOS_VOID NAS_MML_SetMoLcsStatus(
    NAS_MML_MO_LCS_STATUS_ENUM_UINT8     enMoLcsStatus
)
{
    (NAS_MML_GetMmlCtx()->stNetworkInfo.stCsDomainInfo.stCmSrvStatus.enMoLcsStatus) = enMoLcsStatus;
    return;
}
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_GetTinType: enTinType", NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stRplmnCfg.enTinType);
...
)
{
    ...
    /* 1.紧急PDN连接存在时，不删除EPLMN列表中的forbidden plmn列表，待释放时删除：
         ...
         is released.
       2.紧急注册时，不能删除EPLMN列表中的forbidden plmn列表：
...
VOS_VOID  NAS_MML_UpdateSimHplmnPeriFileContent(
    VOS_UINT8                           ucSimHplmnPeriVaule
)
{
    ...
    pstSimInfo->stSimNativeContent.aucHplmnPeriFile[0]      = ucSimHplmnPeriVaule;
}


VOS_UINT8* NAS_MML_GetSimHplmnPeriFileContent( VOS_VOID )
{
    NAS_MML_SIM_INFO_STRU              *pstSimInfo = VOS_NULL_PTR;

    pstSimInfo  = NAS_MML_GetSimInfo();

    return pstSimInfo->stSimNativeContent.aucHplmnPeriFile;
}
...
NAS_MML_CUSTOM_SUPPLEMENT_OPLMN_INFO_STRU* NAS_MML_GetCustomSupplementOplmnInfo( VOS_VOID )
{
    return &(NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stSupplementOplmnInfo);
}
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_GetIsrSupportFlg: ucIsrSupportFlg", (NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stLteCustomCfgInfo.ucIsrSupportFlg));
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_SetIsrSupportFlg: ucIsrSupport", ucIsrSupport);
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_SetImsVoiceAvailFlg: ucImsVoiceAvail", ucAvail);
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_GetImsVoiceAvailFlg: ucImsVoiceAvail", NAS_MML_GetMmlCtx()->stNetworkInfo.stImsDomainInfo.ucImsVoiceAvail);
...
VOS_VOID    NAS_MML_SetImsNormalCallExistFlg(
    VOS_UINT8                           ucImsNormalCallExistFlg
)
{
    NAS_MML_GetMmlCtx()->stNetworkInfo.stImsDomainInfo.ucImsNormalCallExistFlg = ucImsNormalCallExistFlg;

    return;
}


VOS_UINT8   NAS_MML_GetImsNormalCallExistFlg(VOS_VOID)
{
    return NAS_MML_GetMmlCtx()->stNetworkInfo.stImsDomainInfo.ucImsNormalCallExistFlg;
}


VOS_UINT8 NAS_MML_IsImsCallExist(VOS_VOID)
{
    VOS_UINT8                           ucImsEmcCallExistFlg;
    VOS_UINT8                           ucImsNormalCallExistFlg;

    ucImsEmcCallExistFlg = NAS_MML_GetImsEmcCallExistFlg();
    ucImsNormalCallExistFlg = NAS_MML_GetImsNormalCallExistFlg();

    if ((VOS_TRUE == ucImsEmcCallExistFlg) || (VOS_TRUE == ucImsNormalCallExistFlg))
    {
        return VOS_TRUE;
    }
...
VOS_VOID    NAS_MML_SetImsEmcCallExistFlg(
    VOS_UINT8                           ucImsEmcCallExistFlg
)
{
    NAS_MML_GetMmlCtx()->stNetworkInfo.stImsDomainInfo.ucImsEmcCallExistFlg = ucImsEmcCallExistFlg;

    return;
}


VOS_UINT8   NAS_MML_GetImsEmcCallExistFlg(VOS_VOID)
{
    return NAS_MML_GetMmlCtx()->stNetworkInfo.stImsDomainInfo.ucImsEmcCallExistFlg;
}
...
NAS_MML_PLMN_RAT_PRIO_STRU* NAS_MML_GetMsNotifyNwCsPrioRatList( VOS_VOID )
{
    return &(NAS_MML_GetMmlCtx()->stMsCfgInfo.stMsCapability.stMsNotifyNwCapInfo.stNotifyNwCsPrioRatList);
}


NAS_MML_PLMN_RAT_PRIO_STRU* NAS_MML_GetMsNotifyNwPsPrioRatList( VOS_VOID )
{
    return &(NAS_MML_GetMmlCtx()->stMsCfgInfo.stMsCapability.stMsNotifyNwCapInfo.stNotifyNwPsPrioRatList);
}




VOS_VOID NAS_MML_SetMsNotifyNwCsPrioRatList(
    NAS_MML_PLMN_RAT_PRIO_STRU         *pstPrioRatList
)
{
    NAS_MML_PLMN_RAT_PRIO_STRU         *pstMsNotifyNwCsPrioRatList              = VOS_NULL_PTR;

    pstMsNotifyNwCsPrioRatList          = NAS_MML_GetMsNotifyNwCsPrioRatList();

    PS_MEM_CPY(pstMsNotifyNwCsPrioRatList, pstPrioRatList, sizeof(NAS_MML_PLMN_RAT_PRIO_STRU));
...
VOS_VOID NAS_MML_SetMsNotifyNwPsPrioRatList(
    NAS_MML_PLMN_RAT_PRIO_STRU         *pstPrioRatList
)
{
    NAS_MML_PLMN_RAT_PRIO_STRU         *pstMsNotifyNwPsPrioRatList              = VOS_NULL_PTR;

    pstMsNotifyNwPsPrioRatList          = NAS_MML_GetMsNotifyNwPsPrioRatList();

    PS_MEM_CPY(pstMsNotifyNwPsPrioRatList, pstPrioRatList, sizeof(NAS_MML_PLMN_RAT_PRIO_STRU));

    return;
}


VOS_UINT8 NAS_MML_GetRatBalancingEnableFlg( VOS_VOID )
{
    return (NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stAttCfg.ucRatBalancingFlg);
}



VOS_VOID NAS_MML_SetRatBalancingEnableFlg(
    VOS_UINT8                           ucRatBalancingFlg
)
{
    NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stAttCfg.ucRatBalancingFlg = ucRatBalancingFlg;

    return;
}


VOS_UINT8 NAS_MML_GetNetworkSelectionMenuEnableFlg( VOS_VOID )
{
    return (NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stAttCfg.ucNetSelMenuFlg);
}



VOS_VOID NAS_MML_SetNetworkSelectionMenuEnableFlg(
    VOS_UINT8                           ucNetSelMenuFlg
)
{
    NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stAttCfg.ucNetSelMenuFlg = ucNetSelMenuFlg;
...
VOS_UINT32 NAS_MML_GetPsRelCause(VOS_VOID)
{
    return NAS_MML_GetNetworkInfo()->stPsDomainInfo.ulPsRelCause;
}


VOS_VOID NAS_MML_SetPsRelCause(
    VOS_UINT32                          ulCause
)
{
    NAS_MML_GetNetworkInfo()->stPsDomainInfo.ulPsRelCause = ulCause;
}
...
VOS_VOID NAS_MML_SetDamActiveFlag(
    VOS_UINT8                           ucDamActiveFlg
)
{
    NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.ucDamActiveFlg = ucDamActiveFlg;
    return;
}



VOS_UINT8 NAS_MML_GetDamActiveFlag(VOS_VOID)
{
    return (g_stNasMmlCtx.stMsCfgInfo.stCustomCfg.ucDamActiveFlg);
}
...
VOS_UINT8 NAS_MML_GetCause18EnableLteSupportFlg(VOS_VOID)
{
    return (NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stLteCustomCfgInfo.ucCause18EnableLteSupportFlg);
}
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_SetCsmoSupportedFlg: ucCsmoSupportedFlg", ucCsmoSupportedFlg);
...
{
    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_MML_GetCsmoSupportedFlg: ucCsmoSupportedFlg", NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.ucCsmoSupportedFlg);
...
VOS_VOID  NAS_MML_InitOosBgNetworkSearchCustomInfo(
    NAS_MML_OOS_BG_NETWORK_SEARCH_CUSTOM_STRU         *pstOosBgNetworkSearchCustom
)
{
    PS_MEM_SET(pstOosBgNetworkSearchCustom, 0, sizeof(NAS_MML_OOS_BG_NETWORK_SEARCH_CUSTOM_STRU));

    pstOosBgNetworkSearchCustom->ulDelayNetworkSearchTimerLenAfterEmc   = 0;
    pstOosBgNetworkSearchCustom->ucBgHistorySupportFlg                  = VOS_FALSE;
    pstOosBgNetworkSearchCustom->ucPrefBandListSupportFlg               = VOS_FALSE;
    pstOosBgNetworkSearchCustom->ucFullBandListSupportFlg               = VOS_FALSE;
    return;
}
...
VOS_VOID NAS_MML_SetTimerStatusOfDelayNetworkSearchAfterEmc(
    NAS_MML_TIMER_STATUS_ENUM_UINT8     enStatus
)
{
    NAS_MML_GetMmlCtx()->stNetworkInfo.stCsDomainInfo.stCmSrvStatus.enDelayNetworkSearchTimerStatus = enStatus;

    return;
}


NAS_MML_TIMER_STATUS_ENUM_UINT8 NAS_MML_GetTimerStatusOfDelayNetworkSearchAfterEmc(VOS_VOID)
{
    return (NAS_MML_GetMmlCtx()->stNetworkInfo.stCsDomainInfo.stCmSrvStatus.enDelayNetworkSearchTimerStatus);
}


VOS_VOID NAS_MML_SetTimerLenOfDelayNetworkSearch(
    VOS_UINT32                          ulTimerLen
)
{
    NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stOosBgNetworkSearchCustom.ulDelayNetworkSearchTimerLenAfterEmc    = ulTimerLen;
    return;
}


VOS_UINT32 NAS_MML_GetTimerLenOfDelayNetworkSearch(VOS_VOID)
{
    return (NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stOosBgNetworkSearchCustom.ulDelayNetworkSearchTimerLenAfterEmc);
}


VOS_VOID NAS_MML_SetOosNetworkSearchCustomBgHistorySupportFlg(
    VOS_UINT8                           ucBgHistorySupportFlg
)
{
    NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stOosBgNetworkSearchCustom.ucBgHistorySupportFlg = ucBgHistorySupportFlg;
    return;
}


VOS_UINT8 NAS_MML_GetOosNetworkSearchCustomBgHistorySupportFlg(VOS_VOID)
{
    return (NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stOosBgNetworkSearchCustom.ucBgHistorySupportFlg);
}


VOS_UINT8 NAS_MML_GetOosNetworkSearchCustomPrefBandListSupportFlg(VOS_VOID)
{
    return (NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stOosBgNetworkSearchCustom.ucPrefBandListSupportFlg);
}


VOS_UINT8 NAS_MML_GetOosNetworkSearchCustomFullBandListSupportFlg(VOS_VOID)
{
    return (NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stOosBgNetworkSearchCustom.ucFullBandListSupportFlg);
}
...
VOS_VOID  NAS_MML_InitSysCfgInfo(
    NAS_MML_SYS_CFG_INFO_STRU           *pstSysCfgInfo
)
{
    PS_MEM_SET(pstSysCfgInfo, 0x00, sizeof(NAS_MML_SYS_CFG_INFO_STRU));

    pstSysCfgInfo->ucDoingSysCfgFlg     = VOS_FALSE;
    return;
}


VOS_UINT8 NAS_MML_GetSysCfgFlg( VOS_VOID )
{
    return (NAS_MML_GetMmlCtx()->stMsCfgInfo.stMsSysCfgInfo.stSysCfgInfo.ucDoingSysCfgFlg);
}


VOS_VOID NAS_MML_SetSysCfgFlg(
    VOS_UINT8                           ucDoingSysCfgFlg
)
{
    (NAS_MML_GetMmlCtx()->stMsCfgInfo.stMsSysCfgInfo.stSysCfgInfo.ucDoingSysCfgFlg) = ucDoingSysCfgFlg;
}
...
VOS_VOID NAS_MML_SetEpsEmcAttachFlg(
    VOS_UINT8                           ucFlag
)
{
    NAS_MML_GetMmlCtx()->stNetworkInfo.stEpsDomainInfo.ucEpsEmcAttachFlg = ucFlag;
    return;
}


VOS_UINT8 NAS_MML_GetEpsEmcAttachFlg(VOS_VOID)
{
    return (NAS_MML_GetMmlCtx()->stNetworkInfo.stEpsDomainInfo.ucEpsEmcAttachFlg);
}
...
VOS_VOID NAS_MML_SetAiModemActFlg(
    VOS_UINT8                           ucActFlg
)
{
    NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stAiModemCfg.ucActFlg = ucActFlg;
}

VOS_UINT8 NAS_MML_GetAiModemActFlg(VOS_VOID)
{
    return (NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stAiModemCfg.ucActFlg);
}

VOS_VOID NAS_MML_SetOosPlmnSearchSleepCount(
    VOS_UINT8                           ucSleepCount
)
{
    NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stAiModemCfg.stNasSrchCfg.ucOosSrchSleepCnt = ucSleepCount;
}

VOS_UINT8 NAS_MML_GetOosPlmnSearchSleepCount(VOS_VOID)
{
    return (NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stAiModemCfg.stNasSrchCfg.ucOosSrchSleepCnt);
}

VOS_VOID NAS_MML_SetHighPrioRatHplmnSleepCount(
    VOS_UINT8                           ucSleepCount
)
{
    NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stAiModemCfg.stNasSrchCfg.ucHighPrioSrchSleepCnt = ucSleepCount;
}

VOS_UINT8 NAS_MML_GetHighPrioRatHplmnSleepCount(VOS_VOID)
{
    return (NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stAiModemCfg.stNasSrchCfg.ucHighPrioSrchSleepCnt);
}


VOS_VOID NAS_MML_SetAiModemTimeStart(
    VOS_UINT8                           ucTimeStart
)
{
    NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stAiModemCfg.stTimeCfg.ucTimeStart = ucTimeStart;
}

VOS_UINT8 NAS_MML_GetAiModemTimeStart(VOS_VOID)
{
    return (NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stAiModemCfg.stTimeCfg.ucTimeStart);
}


VOS_VOID NAS_MML_SetAiModemTimeEnd(
    VOS_UINT8                           ucTimeEnd
)
{
    NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stAiModemCfg.stTimeCfg.ucTimeEnd = ucTimeEnd;
}

VOS_UINT8 NAS_MML_GetAiModemTimeEnd(VOS_VOID)
{
    return (NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stAiModemCfg.stTimeCfg.ucTimeEnd);
}


VOS_VOID NAS_MML_SetAiModemApplyArea(
    VOS_UINT8                           ucApplyArea
)
{
    NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stAiModemCfg.ucApplyArea = ucApplyArea;
}

VOS_UINT8 NAS_MML_GetAiModemApplyArea(VOS_VOID)
{
    return (NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.stAiModemCfg.ucApplyArea);
}



VOS_UINT32 NAS_MML_GetUsimRefreshFlag(VOS_VOID)
{
    return NAS_MML_GetMmlCtx()->stSimInfo.stSimStatus.stUsimRefreshInfo.ulUsimRefreshFlag;
}


VOS_VOID NAS_MML_SetUsimRefreshFlag(VOS_UINT32 ulUsimRefreshFlag)
{
    NAS_MML_GetMmlCtx()->stSimInfo.stSimStatus.stUsimRefreshInfo.ulUsimRefreshFlag = ulUsimRefreshFlag;

    return;
}


USIMM_REFRESH_TYPE_ENUM_UINT32 NAS_MML_GetUsimRefreshType(VOS_VOID)
{
    return NAS_MML_GetMmlCtx()->stSimInfo.stSimStatus.stUsimRefreshInfo.enRefreshType;
}


VOS_VOID NAS_MML_SetUsimRefreshType(USIMM_REFRESH_TYPE_ENUM_UINT32 enRefreshType)
{
    NAS_MML_GetMmlCtx()->stSimInfo.stSimStatus.stUsimRefreshInfo.enRefreshType = enRefreshType;

    return;
}

VOS_VOID NAS_MML_SetRefreshAllFileRestartFlag(VOS_UINT8 ucRefreshAllFileRestartFlag)
{
    NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.ucRefreshAllFileRestartFlag = ucRefreshAllFileRestartFlag;

    return;
}

VOS_UINT8 NAS_MML_GetRefreshAllFileRestartFlag(VOS_VOID)
{
    return NAS_MML_GetMmlCtx()->stMsCfgInfo.stCustomCfg.ucRefreshAllFileRestartFlag;
}


VOS_UINT32 NAS_MML_IsNeedClearTmsiPtmsi(VOS_VOID)
{
    VOS_UINT32                          ulUsimRefreshFlag;
    USIMM_REFRESH_TYPE_ENUM_UINT32      enRefreshType;
    VOS_UINT8                           ucRefreshAllFileRestartFlag;

    ulUsimRefreshFlag           = NAS_MML_GetUsimRefreshFlag();
    enRefreshType               = NAS_MML_GetUsimRefreshType();
    ucRefreshAllFileRestartFlag = NAS_MML_GetRefreshAllFileRestartFlag();

    if ((VOS_TRUE == ulUsimRefreshFlag)
     && (USIMM_RESET == enRefreshType)
     && (VOS_TRUE == ucRefreshAllFileRestartFlag))
    {
        return VOS_TRUE;
    }
...
VOS_UINT8 NAS_MML_GetPsDamNoSearchFlag(VOS_VOID)
{
    return NAS_MML_GetMmlCtx()->stNetworkInfo.stPsDomainInfo.ucPsDamNoSearchFlag;
}

VOS_VOID NAS_MML_SetPsDamNoSearchFlag(VOS_UINT8 ucPsDamNoSearchFlag)
{
    NAS_MML_GetMmlCtx()->stNetworkInfo.stPsDomainInfo.ucPsDamNoSearchFlag = ucPsDamNoSearchFlag;

    return;
}

VOS_UINT8 NAS_MML_GetCsDamNoSearchFlag(VOS_VOID)
{
    return NAS_MML_GetMmlCtx()->stNetworkInfo.stCsDomainInfo.ucCsDamNoSearchFlag;
}

VOS_VOID NAS_MML_SetCsDamNoSearchFlag(VOS_UINT8 ucCsDamNoSearchFlag)
{
    NAS_MML_GetMmlCtx()->stNetworkInfo.stCsDomainInfo.ucCsDamNoSearchFlag = ucCsDamNoSearchFlag;

    return;
}

VOS_UINT32 NAS_MML_IsInDamNoSearchStatus(VOS_VOID)
{
    VOS_UINT8                           ucCsDamNoSearchFlag;
    VOS_UINT8                           ucPsDamNoSearchFlag;

    ucCsDamNoSearchFlag = NAS_MML_GetCsDamNoSearchFlag();
    ucPsDamNoSearchFlag = NAS_MML_GetPsDamNoSearchFlag();

    if ( (VOS_TRUE == ucCsDamNoSearchFlag)
      || (VOS_TRUE == ucPsDamNoSearchFlag) )
    {
        return VOS_TRUE;
    }
