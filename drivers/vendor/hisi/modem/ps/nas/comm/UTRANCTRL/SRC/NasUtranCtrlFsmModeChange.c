...
)
{
    ...
    /* 异常打印 */
    NAS_WARNING_LOG1(WUEPS_PID_MMC, "NAS_UTRANCTRL_RcvTiWaitSlaveModeUtranModeCnfExpired_ModeChange_WaitSlaveModeCnf: ucWaitSlaveModeUtranModeCnfFlg is", ucWaitSlaveModeUtranModeCnfFlg);
    ...
    if (NAS_UTRANCTRL_MASTER_MODE_LTE == NAS_UTRANCTRL_GetMasterMode_ModeChange())
    {
        /* 主动触发复位，保留问题现场，便于LRRC进行定位 */
        NAS_MML_SoftReBoot(NAS_MML_REBOOT_SCENE_MMC_WAIT_SLAVE_MODE_UTRAN_MODE_CNF_EXPIRED);
    }
...
)
{
    ...
    /* 异常打印 */
    NAS_WARNING_LOG1(WUEPS_PID_MMC, "NAS_UTRANCTRL_RcvTiWaitMasterModeCnfExpired_ModeChange_WaitMasterModeCnf: enMasterMode is", enMasterMode);
    ...
    {
        /* 主动触发复位，保留问题现场，便于LRRC进行定位 */
        NAS_MML_SoftReBoot(NAS_MML_REBOOT_SCENE_MMC_WAIT_MASTER_MODE_UTRAN_MODE_CNF_EXPIRED);
    }
