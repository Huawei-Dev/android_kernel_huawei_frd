...
)
{
    ...
    {
        NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_UTRANCTRL_MsgProc: Platform not support UTRAN, ulMsgName", pstMsgHeader->ulMsgName);
    ...
    switch (pstMsgHeader->ulReceiverPid)
    {
        case WUEPS_PID_MMC:
            ...
            NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_UTRANCTRL_MsgProc: UTRANCTRL FSM processed, ulRet", ulRet);
            break;
    ...
    if (VOS_FALSE == ulRet)
    {
        NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_UTRANCTRL_MsgProc: Need PorstPorcMsg, ulMsgName", pstMsgHeader->ulMsgName);
...
)
{
    ...
    if ((NAS_UTRANCTRL_FSM_MAIN == NAS_UTRANCTRL_GetCurrFsmId())
     && (NAS_UTRANCTRL_FSM_MAIN != enPreFsmId))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_UTRANCTRL_ProcessMmcMsg: Clear Buffered Snd Utran Req Msg");
...
)
{
    ...
    switch (pstMsgHeader->ulReceiverPid)
    {
        case WUEPS_PID_MMC:
            ...
            if (VOS_TRUE == NAS_UTRANCTRL_GetReplaceMmcMsgFlg())
            {
                *ppstDestMsg = (struct MsgCB *)pstEntryMsg->aucEntryMsgBuffer;
                ...
                NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_UTRANCTRL_PostProcMsg: Need replace MMC MSG, ulEventType", pstEntryMsg->ulEventType);
            ...
            {
                NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_UTRANCTRL_PostProcMsg: No Need replace MMC MSG");
                *ppstDestMsg = pstSrcMsg;
            }
        ...
        default:
            ...
            NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_UTRANCTRL_PostProcMsg: RecevicePID not MMC, do not replace MSG");
            *ppstDestMsg = pstSrcMsg;
    ...
    NAS_NORMAL_LOG4( WUEPS_PID_MMC, "NAS_UTRANCTRL_PostProcMsg: ulSenderPid, ulReceiverPid, ulLength, ulMsgName", pstMsgHeader->ulSenderPid, pstMsgHeader->ulReceiverPid, pstMsgHeader->ulLength, pstMsgHeader->ulMsgName);
...
)
{
     ...
     || (NAS_UTRANCTRL_FSM_SYSCFG      == enCurrFsmId))
    {
        ...
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_UTRANCTRL_ProcessMsgInFsm: Save Cache Msg");
