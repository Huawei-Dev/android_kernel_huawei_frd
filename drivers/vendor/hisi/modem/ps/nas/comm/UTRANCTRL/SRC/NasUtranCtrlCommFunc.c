...
)
{
    ...
    switch ( enNasUtranMode )
    {
        ...
        default:
            NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_UTRANCTRL_ConvertNasUtranModeToLmmUtranMode:UTRANMODE IS BUTT! Set MMC_LMM_UTRAN_MODE_TDD");
            /* 平台不支持GU时，默认给LMM发送的UtranMode为TDD */
            enLmmUtranMode = MMC_LMM_UTRAN_MODE_TDD;
            break;
    }

    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_UTRANCTRL_ConvertNasUtranModeToLmmUtranMode(): return enLmmUtranMode", enLmmUtranMode);
...
VOS_VOID NAS_UTRANCTRL_GetRemainTimerLen(
    NAS_MMC_TIMER_ID_ENUM_UINT16        enTimerId,
    VOS_UINT32                         *pulRemainTimeLen
)
{
    /* MMC在启timer时，ulPara填的是0，此处也固定填成0 */
    NAS_MMC_GetTimerRemainLen(enTimerId, pulRemainTimeLen);
...
VOS_VOID NAS_UTRANCTRL_BuildTimerExpireMsg(
    VOS_UINT32                          ulRcvPid,
    NAS_MMC_TIMER_ID_ENUM_UINT16        enTimerId,
...
)
{
    ...
    NAS_UTRANCTRL_GetRemainTimerLen(enTimerId, &ulRemainTimeLen);

    /* 剩余时长不为0，表示定时器正在运行 */
    if (0 == ulRemainTimeLen)
    {
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_UTRANCTRL_BuildTimerExpireMsg: RemainTimerLen is 0");

        return;
    }


    NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_UTRANCTRL_BuildTimerExpireMsg: Timer is running, TimerId is: ", enTimerId);

    /* 停定时器 */
    NAS_UTRANCTRL_StopTimer(enTimerId);

    /* 构造定时器超时消息 */
    PS_MEM_SET(pstMsg+VOS_MSG_HEAD_LENGTH, 0x00, sizeof(REL_TIMER_MSG) - VOS_MSG_HEAD_LENGTH);

    pstMsg->ulSenderCpuId   = VOS_LOCAL_CPUID;
    pstMsg->ulSenderPid     = VOS_PID_TIMER;
    pstMsg->ulReceiverCpuId = VOS_LOCAL_CPUID;
    pstMsg->ulReceiverPid   = ulRcvPid;
    pstMsg->ulLength        = sizeof(REL_TIMER_MSG) - VOS_MSG_HEAD_LENGTH;
    pstMsg->ulName          = enTimerId;
...
)
{
    ...
    switch(enPlmnSearchType)
    {
        ...
        case RRC_PLMN_SEARCH_USER_LIST:

            enPlmnSrchRslt  = RRC_PLMN_SEARCH_RLT_USER_LIST_FAIL;
            break;

        case RRC_PLMN_SEARCH_INTER_LIST_PREF_BAND:

            enPlmnSrchRslt  = RRC_PLMN_SEARCH_RLT_INTER_LIST_PREF_BAND_FAIL;
            break;

        case RRC_PLMN_SEARCH_INTER_LIST:

            enPlmnSrchRslt  = RRC_PLMN_SEARCH_RLT_INTER_LIST_FAIL;
            break;
...
)
{
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_UTRANCTRL_IsNeedUtranCtrlFsmProcMsg: Support FDD&TDD, return TRUE");
        return VOS_TRUE;
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_UTRANCTRL_IsNeedUtranCtrlFsmProcMsg: Process MMCGMM_START_CNF, return TRUE");
        return VOS_TRUE;
    }

    if (ulEventType == NAS_UTRANCTRL_BuildEventType(VOS_PID_TIMER, TI_NAS_MMC_WAIT_MM_START_CNF))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_UTRANCTRL_IsNeedUtranCtrlFsmProcMsg: Process TI_NAS_MMC_WAIT_MM_START_CNF, return TRUE");
        return VOS_TRUE;
    }

    if (ulEventType == NAS_UTRANCTRL_BuildEventType(WUEPS_PID_MMC, MMCMMC_INTER_ABORT_UTRAN_CTRL_PLMN_SEARCH_REQ))
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_UTRANCTRL_IsNeedUtranCtrlFsmProcMsg: Process MMCMMC_INTER_ABORT_UTRAN_CTRL_PLMN_SEARCH_REQ, return TRUE");
        return VOS_TRUE;
    }

    /* 其他消息在FDD/TDD单模时候，不需要进入UTRANCTRL FSM处理 */
    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_UTRANCTRL_IsNeedUtranCtrlFsmProcMsg: Only Support TDD or FDD, return FALSE");
    return VOS_FALSE;
}
...
VOS_VOID NAS_UTRANCTRL_UpdateRoamZoneRelatedFlags(
...
)
{
    ...
    if (VOS_NULL_PTR == pulRrcMccList)
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_UTRANCTRL_UpdateRoamZoneRelatedFlags ERROR: MEM ALLOC FAIL");
...
)
{
    ...
    if (NAS_UTRANCTRL_UTRAN_SWITCH_MODE_AUTO != NAS_UTRANCTRL_GetCurUtranSwitchMode())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_UTRANCTRL_IsNeedProcGuPlmnSearchCnf: Not NAS_UTRANCTRL_UTRAN_SWITCH_MODE_AUTO, enter MMC FSM");
        return VOS_FALSE;
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_UTRANCTRL_IsNeedProcGuPlmnSearchCnf: PLMN LIST SEARCH RSLT, enter MMC FSM");
        return VOS_FALSE;
    }

    /* 自动切换模式下
       1. 搜网失败NORF
       2. 快速指定搜网或指定搜网 成功或NO RF
       3. history搜成功或pref band搜成功或NO RF
    ...
    if ((RRC_PLMN_SEARCH_SPEC_FAST          == NAS_UTRANCTRL_GetBufferedPlmnSearchReqType())
     ...
     || (RRC_PLMN_SEARCH_RLT_PREF_BAND_SUCC == pstPlmnSrchCnf->ulSearchRlt)
     || (RRC_PLMN_SEARCH_RLT_PREF_BAND_NO_RF== pstPlmnSrchCnf->ulSearchRlt))
    {
        NAS_NORMAL_LOG1(WUEPS_PID_MMC, "NAS_UTRANCTRL_IsNeedProcGuPlmnSearchCnf: enter MMC FSM, ulSearchRlt", pstPlmnSrchCnf->ulSearchRlt);
        return VOS_FALSE;
    }
    ...
    {
        NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_UTRANCTRL_IsNeedProcGuPlmnSearchCnf: GET GEO SEARCH RSLT, enter MMC FSM");
        return VOS_FALSE;
    }

    NAS_NORMAL_LOG(WUEPS_PID_MMC, "NAS_UTRANCTRL_IsNeedProcGuPlmnSearchCnf: return TRUE");
...
)
{
    ...
    /* 自动切换模式下如果是搜网失败NORF/快速指定搜网或指定搜网成功/history搜成功/pref band成功直接透传快速指定搜网回复到MMC状态机处理 */
    /* pref band fail时，根据PLMN列表，更新一下SearchedTdMccFlg和utranMode */
    if ((MMC_LMM_PLMN_SRCH_RLT_SPEC_SUCC        == pstPlmnSrchCnf->enRlst)
     ...
     || (MMC_LMM_PLMN_SRCH_RLT_PREF_BAND_SUCC   == pstPlmnSrchCnf->enRlst)
     || (MMC_LMM_PLMN_SRCH_RLT_PREF_BAND_NO_RF  == pstPlmnSrchCnf->enRlst))
    {
        return VOS_FALSE;
    }
