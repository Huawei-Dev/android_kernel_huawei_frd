...
VOS_VOID  NAS_LPP_RcvIntraDataCnf( NAS_LPP_INTRA_DATA_CNF_STRU *pRcvMsg)
{
    ...
    else if (PS_TRUE == pstTransEntity->ucTransEndFlag)
    {
        ...
        if(  NAS_LPP_UPLINK_MSG_TYPE_NO_NEED_MSG_CNF == pstUplinkMsgBuff->stLppMsgIdentity.enUplinkMsgType)
        {
            if(PS_TRUE != pstUplinkMsgBuff->stLppMsgIdentity.bIsProvideLocFalg)
            {
            ...
            {
                NAS_LPP_WARN_LOG("NAS_LPP_RcvIntraDataCnf: end transaction msg no needing ack cnf!");
                TLPS_PRINT2LAYER_WARNING(NAS_LPP_RcvIntraDataCnf_ENUM, LNAS_LPP_NONeedAckMsgReleaseTrans);
                NAS_LPP_ReleaseTransactionOrSession(ucSessionIndex,
                                                    ucSessionTransIndx,
                                                  &(pstUplinkMsgBuff->stLppMsgIdentity.stTransId));
                return;
