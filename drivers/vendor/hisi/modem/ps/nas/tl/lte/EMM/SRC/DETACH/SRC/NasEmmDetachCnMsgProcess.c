                                                        ...
                                                        VOS_VOID   *pMsgStru)
{
    ...
    /* 删除此处通知ESM CLEAR_CL_BEARER_NOTIFY消息，移到转状态处发送 */
...
)
{
    ...
    /* 向MRRC发送DETACH ACCEPT消息 */
    NAS_EMM_MrrcSendDetAcpMt();
    /* 如果是CSFB流程 ，根据不同的类型和原因值决定是否需要触发搜网到GU继续进行呼叫，这样会提高呼叫接通率*/
    if (VOS_TRUE == NAS_EMM_SER_IsCsfbProcedure())
    {
        NAS_EMM_MsESRWtRrcRelMsgCnNotReAttachDetatchReq();
    }
    NAS_EMM_EsmSendStatResult(EMM_ESM_ATTACH_STATUS_DETACHED);
...
VOS_VOID NAS_EMM_MsESRWtRrcRelMsgCnNotReAttachDetatchReq(VOS_VOID)
{
    TLPS_PRINT2LAYER_INFO(NAS_EMM_MsESRWtRrcRelMsgCnNotReAttachDetatchReq_ENUM,LNAS_ENTRY);

    /* 如果是紧急CSFB,向MMC发送LMM_MMC_SERVICE_RESULT_IND消息 ，触发搜网到GU完成呼叫，不考虑原因值*/
    if (VOS_FALSE == NAS_EMM_SER_IsNotEmergencyCsfb())
    {
        TLPS_PRINT2LAYER_INFO(NAS_EMM_MsESRWtRrcRelMsgCnNotReAttachDetatchReq_ENUM,LNAS_FUNCTION_LABEL1);
        NAS_EMM_MmcSendSerResultIndOtherType(MMC_LMM_SERVICE_RSLT_FAILURE);
        return;
    }
    /* 如果非紧急CSFB,根据不同的类型和原因值处理*/
    else
    {
        /* 如果无原因值的话，CSFB向MMC发送LMM_MMC_SERVICE_RESULT_IND消息 ，触发搜网到GU*/
        if(NAS_EMM_AD_BIT_NO_SLCT == NAS_EMM_GLO_AD_OP_GetDetEmmCau())
        {
            TLPS_PRINT2LAYER_INFO(NAS_EMM_MsESRWtRrcRelMsgCnNotReAttachDetatchReq_ENUM,LNAS_FUNCTION_LABEL2);
            NAS_EMM_MmcSendSerResultIndOtherType(MMC_LMM_SERVICE_RSLT_FAILURE);
            return;
        }
        /* 如果有原因值的话，根据不同的类型和原因值处理*/
        else
        {
            switch (NAS_EMM_GLO_AD_GetDetEmmCau())
            {
                /* 如果原因值是2368，不会向MMC上报service_result，这样就不会触发搜网到GU*/
                case NAS_LMM_CAUSE_IMSI_UNKNOWN_IN_HSS:
                case NAS_LMM_CAUSE_ILLEGAL_UE:
                case NAS_LMM_CAUSE_ILLEGAL_ME:
                case NAS_LMM_CAUSE_EPS_SERV_AND_NON_EPS_SERV_NOT_ALLOW:
                    TLPS_PRINT2LAYER_INFO(NAS_EMM_MsESRWtRrcRelMsgCnNotReAttachDetatchReq_ENUM,LNAS_FUNCTION_LABEL3);
                    NAS_EMM_SetCsfbProcedureFlag(PS_FALSE);
                    NAS_EMM_MmSendCsfbSerEndInd(MM_LMM_CSFB_SERVICE_RSLT_CN_DETACH_FAIL, NAS_LMM_CAUSE_NULL);
                    break;
                /* 如果原因值是11,12,13,15，且是MT CALL,不会向MMC上报service_result，这样就不会触发搜网到GU,
                如果是MO call,那么会搜网到GU继续完成呼叫*/
                case NAS_LMM_CAUSE_PLMN_NOT_ALLOW:
                case NAS_LMM_CAUSE_TA_NOT_ALLOW :
                case NAS_LMM_CAUSE_ROAM_NOT_ALLOW:
                case NAS_LMM_CAUSE_NO_SUITABL_CELL:
                    ...
                    {
                        TLPS_PRINT2LAYER_INFO(NAS_EMM_MsESRWtRrcRelMsgCnNotReAttachDetatchReq_ENUM,LNAS_FUNCTION_LABEL4);
                        NAS_EMM_SetCsfbProcedureFlag(PS_FALSE);
                        NAS_EMM_MmSendCsfbSerEndInd(MM_LMM_CSFB_SERVICE_RSLT_CN_DETACH_FAIL, NAS_LMM_CAUSE_NULL);
                        return;
                    }
                    ...
                    {
                        TLPS_PRINT2LAYER_INFO(NAS_EMM_MsESRWtRrcRelMsgCnNotReAttachDetatchReq_ENUM,LNAS_FUNCTION_LABEL5);
                        NAS_EMM_MmcSendSerResultIndOtherType(MMC_LMM_SERVICE_RSLT_FAILURE);
                    }
                    break;

                default:
                    /* 如果其他原因值的话，CSFB向MMC发送LMM_MMC_SERVICE_RESULT_IND消息 ，触发搜网到GU*/
                    TLPS_PRINT2LAYER_INFO(NAS_EMM_MsESRWtRrcRelMsgCnNotReAttachDetatchReq_ENUM,LNAS_FUNCTION_LABEL6);
                    NAS_EMM_MmcSendSerResultIndOtherType(MMC_LMM_SERVICE_RSLT_FAILURE);
                    break;
            }
        }
...
VOS_VOID  NAS_EMM_ProcDetachAuthRej(VOS_UINT32  ulCause)
{
   ...
   TLPS_PRINT2LAYER_INFO(NAS_EMM_ProcDetachAuthRej_ENUM,LNAS_ENTRY);
    ...
    /* DEREG_INIT态，删除此处通知ESM CLEAR_CL_BEARER_NOTIFY消息，移到转状态处发送 */
