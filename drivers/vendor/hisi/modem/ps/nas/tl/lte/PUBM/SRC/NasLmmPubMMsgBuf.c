...
/*lint -e960*/
/*lint -e961*/
VOS_VOID  NAS_LMM_RemoveMsgFromFsmBufferQue
(
    NAS_LMM_FSM_MSG_BUF_STRU*           pstFsmMsgBuffAddr,
    VOS_UINT32                          ulMsgType,
    VOS_UINT32                          ulPriLevel,
...

{
    ...
    {
        NAS_LMM_PUBM_LOG_WARN(           "NAS_LMM_RemoveMsgFromFsmQue, Get pstFsmMsgBuffAddr Fail!");
        TLPS_PRINT2LAYER_WARNING(NAS_LMM_RemoveMsgFromFsmQue_ENUM,LNAS_NULL_PTR);
        return;
    ...
    {
        NAS_LMM_PUBM_LOG1_WARN(           "NAS_LMM_RemoveMsgFromFsmQue, PRIO Fail!",ulPriLevel);
        TLPS_PRINT2LAYER_WARNING(NAS_LMM_RemoveMsgFromFsmQue_ENUM,LNAS_FAIL);
        return;
    ...
    bRsltRemoveMsgFromQue               = NAS_LMM_RemoveMsgFromBufferQue(   pstMsgQue,
                                                                            ucCnt,
                                                                            ulMsgType,
                                                                            ulSenderPid);
    /*消息成功释放，更新MSG COUNT */
...
VOS_BOOL  NAS_LMM_RemoveMsgFromBufferQue
(
    NAS_LMM_BUFF_MSG_STRU*              pstMsgQue ,
    VOS_UINT8                           ucCnt,
    VOS_UINT32                          ulMsgType,
...
)
{
    ...
    {
        NAS_LMM_PUBM_LOG_WARN(           "NAS_LMM_RemoveMsgFromQue, pstMsgQue == NAS_LMM_NULL_PTR");
        TLPS_PRINT2LAYER_WARNING(NAS_LMM_RemoveMsgFromQue_ENUM,LNAS_NULL_PTR);
        return(VOS_FALSE);
        ...
        ucSearchCnt++ )
    {
        pstBufMsg       = (pstMsgQue+ucSearchCnt);
        ulBufMsgType    = pstBufMsg->ulEvtType;
        ulBufSenderPid  = ((VOS_UINT32*)pstBufMsg->pBuffMsg)[1];

        /* 判断消息是否已存在 */
        if(( ulBufMsgType == ulMsgType )
            &&( ulBufSenderPid == ulSenderPid ))
        {
            bFlag = VOS_TRUE;
            break;
        }
    ...
    if(bFlag)
    {
           ...
           (pstBufMsg->pBuffMsg != NAS_LMM_NULL_PTR) )
        {
            ...
            {
                NAS_LMM_PUBM_LOG_WARN("NAS_LMM_RemoveMsgFromQue, Memory Free is not succ");
                TLPS_PRINT2LAYER_WARNING(NAS_LMM_RemoveMsgFromQue_ENUM,LNAS_FAIL);
