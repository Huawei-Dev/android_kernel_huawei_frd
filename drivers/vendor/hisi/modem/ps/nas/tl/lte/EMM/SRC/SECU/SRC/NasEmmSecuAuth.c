...
VOS_UINT32 NAS_LMM_PreProcUsimAuthCnf(MsgBlock  *pMsg)
{
    ...
    /* 当前连接状态不是数据连接态或者信令连接态时, 鉴权RSP发不到网侧, 所以不需要进入状态机处理
       解决:当LMM收到网侧鉴权请求, 并给USIM发送了鉴权请求在等待回复期间, 底层链路异常通知LMM链路
       释放,而再收到USIM的鉴权失败时, 把当前状态转为AUTH INIT态, 导致状态异常, 此时若底层发生重选
       AUTH INIT态不处理挂起消息从而出现复位 */
    if (NAS_EMM_NO == NAS_EMM_IsNotIdle())
    {
