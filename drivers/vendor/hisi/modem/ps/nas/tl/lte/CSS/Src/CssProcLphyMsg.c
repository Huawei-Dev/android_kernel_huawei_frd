...
)
{
    ...
    VOS_UINT8                           ucUeSupportLteFlag  = 0;
    VOS_UINT8                           ucUeSuportWcdmaFlag = 0;
    VOS_UINT8                           ucUeSuportGsmFlag   = 0;
    VOS_UINT8                           ucBand              = 0;
    ...
    ucBand = pstBandNode->stBandInfo.ucBandInd;

    pstUserSetBand = NAS_MML_GetMsSupportBand();

    if (CSS_NULL_PTR != pstUserSetBand)
    {
        CSS_GetLteSuportFlag(ucBand,CSS_OP_TRUE,pstUserSetBand,CSS_LTE_SUPORT_OP,&ucUeSupportLteFlag);
        pstCssBandScanReq->bitOpLteSupport |= (ucUeSupportLteFlag >> 2);
        CSS_GetWcdmaSuportFlag(ucBand,CSS_OP_TRUE,pstUserSetBand,CSS_WCDMA_SUPORT_OP,&ucUeSuportWcdmaFlag);
        pstCssBandScanReq->bitOpWcdmaSupport |= (ucUeSuportWcdmaFlag >> 1);
        CSS_GetGsmSuportFlag(ucBand,CSS_OP_TRUE,pstUserSetBand,CSS_GSM_SUPORT_OP,&ucUeSuportGsmFlag);
        pstCssBandScanReq->bitOpGsmSupport |= ucUeSuportGsmFlag;
    }
