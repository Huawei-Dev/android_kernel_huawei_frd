...
VOS_VOID IMSA_RegConverterApMac2Ims
(
    IMSA_REG_TYPE_ENUM_UINT8 enRegType,
    IMSA_IMS_INPUT_EVENT_STRU *pstImsaImsInputEvt
)
{
    ...
    if (IMSA_REG_TYPE_NORMAL == enRegType)
    {
        pstImsaImsInputEvt->evt.stInputParaEvent.enInputParaReason = IMSA_IMS_INPUT_PARA_REASON_SET_WLANNODEID;

        /* 设置AP MAC地址 */
        IMSA_UtilStrNCpy(pstImsaImsInputEvt->evt.stInputParaEvent.u.stIWlanNodeId.acApMacAddr, pstRegEntity->acApMacAddr, IMSA_IMS_AP_MAC_LEN);

        /* 设置WIFI接入类型地址 */
        pstImsaImsInputEvt->evt.stInputParaEvent.u.stIWlanNodeId.enNwAccessType = pstRegEntity->enNwAccessType;
    }
...
VOS_VOID IMSA_RegConfigApMacAddress2Ims
(
    IMSA_REG_TYPE_ENUM_UINT8            enRegType
)
{
    ...
    /*清空*/
    IMSA_MEM_SET_S(pstImsaImsInputEvent, sizeof(IMSA_IMS_INPUT_EVENT_STRU),0, sizeof(IMSA_IMS_INPUT_EVENT_STRU));

    IMSA_RegConverterApMac2Ims(enRegType, pstImsaImsInputEvent);
                                 ...
                                 const VOS_CHAR *pcPcscfAddr)
{
    ...
    if (IMSA_REG_TYPE_NORMAL == enRegType)
    {
        ...
        {
            /* set AP MAC address to trigger register */
            IMSA_RegConfigApMacAddress2Ims(enRegType);
        }
        else if(IMSA_IMS_DOMAIN_LTE == IMSA_GetCurrentDomain())
        {
            /* set CGI to let IMS know RAT */
            IMSA_ConfigCgi2Ims();
        }
                                      ...
                                      VOS_CHAR *pucPcscfAddr)
{
    ...
    case IMSA_REG_ADDR_PARAM_NO_REG_EMC_CALL:
...
VOS_UINT32 IMSA_RegFsmProcIdleReqReq(IMSA_REG_ENTITY_STRU *pstEntity, VOS_UINT32 ulEvt, VOS_VOID* pData)
{
    ...
    /* 如果当前作用域为L，如果当前接入类型不支持IMS，不再发起注册 */
    /* 二阶段需要添加判断不支持volte即不支持vowifi的条件 */
    if ((VOS_TRUE != IMSA_IsCurrentAccessTypeSupportIms())
    && (IMSA_REG_ADDR_PARAM_NO_REG_EMC_CALL != pstRegReq->enAddrType))
    {
    ...
    /* 无卡紧急呼场景，仅配置地址对 */
    if (IMSA_REG_ADDR_PARAM_NO_REG_EMC_CALL == pstRegReq->enAddrType)
    {
        /* 仅支持无卡紧急呼场景，wifi不支持紧急呼 */
        if ((IMSA_REG_TYPE_EMC != pstEntity->enRegType)
        || IMSA_IMS_DOMAIN_WIFI == IMSA_GetCurrentDomain())
        {
            (VOS_VOID)IMSA_RegSendIntraMsgRegInd(pstEntity->enRegType, IMSA_REG_RESULT_FAIL, IMSA_RESULT_ACTION_NULL);
            return VOS_FALSE;
        }
        /* set UE IP address to trigger register */
        IMSA_RegConfigIpAddress2Ims(pstEntity->enRegType, aucUeAddr, aucPcscfAddr);
        return VOS_TRUE;
    }
...
)
{
    ...
    if (IMSA_REG_TYPE_EMC == pstEntity->enRegType)
    {
        ...
        if (IMSA_SIP_NW_ERROR_CAUSE_FORBIDDEN == pstImsErrorInfo->usSipStatusCode)
        {
            (VOS_VOID)IMSA_RegSendIntraMsgRegInd(   pstEntity->enRegType,
...
(
    IMSA_REG_ENTITY_STRU                        *pstEntity,
    IMSA_IMS_SERVICE_NOTIFY_EVENT_ENUM_UINT8     ucServerNtyEvent,
    VOS_UINT32                                   ulRetryAfter,
    VOS_UINT16                                   usSipStatusCode
)
{
    ...
    IMSA_INFO_LOG1("IMSA_RegProcNwNotifyMsgByEvent:", ucServerNtyEvent);
    TLPS_PRINT2LAYER_INFO1(IMSA_RegProcNwNotifyMsgByEvent_ENUM, 1, ucServerNtyEvent);

    switch (ucServerNtyEvent)
    {
        case IMSA_IMS_SERVICE_NOTIFY_EVENT_DEACTED:
            /* 如果网侧通知deactive后，需要发起*/
            if (IMSA_TRUE == IMSA_REG_GetNwDeactiveReRegCtrl())
            {
                IMSA_RegReRegWaitForRetryCommonProc( pstEntity,
                                                     pstRegCtx->ulNwDeactReRegTimerLen,
                                                     usSipStatusCode,
                                                     IMSA_REG_RESULT_FAIL_MT_DEREG);
            }
            else
            {
                IMSA_RegReRegWaitForRetryCommonProc( pstEntity,
        ...
        case IMSA_IMS_SERVICE_NOTIFY_EVENT_REJED:
            /* 如果网侧发送rejet后，重新发起注册的开关打开，则启动retry定时器 */
            if(IMSA_TRUE == IMSA_REG_GetNwRejReRegCtrl())
            {
                IMSA_RegReRegWaitForRetryCommonProc( pstEntity,
                                                     pstRegCtx->ulNwDeactReRegTimerLen,
                                                     usSipStatusCode,
...
)
{
    ...
    if (0 == pstImsaRegManager->ulSaveRetryTimes)
    {
        ...
        IMSA_RegReRegWaitForRetryCommonProc( pstEntity,
                                        pstImsErrorInfo->ulRetryAfter,
                                        pstImsErrorInfo->usSipStatusCode,
                                        IMSA_REG_RESULT_FAIL_REREG_FAIL);
    }
    else
    {
        if (pstEntity->ulRetryTimes < pstImsaRegManager->ulSaveRetryTimes)
...
)
{
    ...
    switch (pstImsErrorInfo->usSipStatusCode)
    {
        case IMSA_IMS_INT_ERROR_CODE_TIMERF_OUT:
            ...
            {
                /* 如果有其他P-CSCF地址，则使用其他P-CSCF地址，如果没有，
                   则使用当前P-CSCF地址 */
                (VOS_VOID)IMSA_RegSendIntraMsgDeregInd( pstEntity->enRegType,
                                                        IMSA_REG_RESULT_FAIL,
                                                        IMSA_RESULT_ACTION_REG_RESTORATION);
            }
        ...
        case IMSA_IMS_INT_ERROR_CODE_NOTIFY_DEREGISTER:
            IMSA_RegProcNwNotifyMsgByEvent(pstEntity,
                                           pstImsErrorInfo->ucServerNtyEvent,
                                           pstImsErrorInfo->ulRetryAfter,
                                           pstImsErrorInfo->usSipStatusCode);
            break;
        case IMSA_IMS_INT_ERROR_CODE_TCP_ERROR:
...
VOS_UINT32 IMSA_RegProcUserReregReqMsg(const VOS_VOID *pRcvMsg)
{
    IMSA_REG_ENTITY_STRU *pstRegEntity = VOS_NULL_PTR;
    IMSA_REG_REREG_REQ_STRU *pstReregReqData = VOS_NULL_PTR;

    IMSA_INFO_LOG("IMSA_RegProcUserReregReqMsg is entered!");
    TLPS_PRINT2LAYER_INFO(IMSA_RegProcUserRegReqMsg_ENUM, LNAS_ENTRY);


    pstReregReqData = (IMSA_REG_REREG_REQ_STRU *)pRcvMsg;
    pstRegEntity = IMSA_RegEntityGetByType(pstReregReqData->enReregType);
    /* 当前仅仅支持Normal REG的切换 */
    if( IMSA_REG_TYPE_NORMAL != pstReregReqData->enReregType)
    {
        return VOS_FALSE;
    }
    /* 非注册状态不处理重注册申请 */
    ...
    /* 承载类型只支持LTE&WIFI */
    if( (IMSA_REG_BEARER_TYPE_WIFI != pstReregReqData->enRegBearerType)
        && (IMSA_REG_BEARER_TYPE_LTE != pstReregReqData->enRegBearerType))
    {
        return VOS_FALSE;
    }
    ...
    /* 通过更新承载能力,触发SDK发起重注册流程 */
    if( IMSA_REG_BEARER_TYPE_WIFI == pstReregReqData->enRegBearerType)
    {
        /* 配置AP ADDR信息给IMS */
        IMSA_RegConfigApMacAddress2Ims(pstReregReqData->enReregType);
    }
    else
    {
        /* 配置CGI信息给IMS */
    ...
    /* 发送重注册消息,需要考虑增加判断DSDS需求 */
    IMSA_SndD2MsgServiceRegInfo(IMSA_IMS_INPUT_SERVICE_RE_REGISTER);
...
VOS_VOID IMSA_RegMgrInit(VOS_VOID)
{
    ...
    pstRegCtx->ulNwDeactReRegTimerLen = IMSA_REG_TIMER_LEN_DEREG_REREG;
}
...
VOS_UINT32 IMSA_RegAddrApMacAddr
(
    IMSA_REG_TYPE_ENUM_UINT8 ulRegType,
...
)
{
    ...
    IMSA_INFO_LOG("IMSA_RegAddrApMacAddr is entered!");
    TLPS_PRINT2LAYER_INFO(IMSA_RegAddrPairMgrAddPcscfAddr_ENUM, LNAS_ENTRY);

    /* 保存WIFI接入类型 */
    pstRegEntity->enNwAccessType    = enNwAccessType;

    /* 保存MAC地址 */
    IMSA_UtilStrNCpy(&pstRegEntity->acApMacAddr[0], pacApMacAddr, IMSA_IMS_AP_MAC_LEN);
