...
,LNAS_NULL_PTR);
    ...
    /* 不在上述被禁列表中，则调用gu nas接口判断是否允许注册 */
    else if(MMC_LMM_PLMN_NOT_ALLOW_REG == Nas_PlmnIdIsForbid((MMC_LMM_PLMN_ID_STRU *)&pstAreaInfo->stPlmnId))
    {
        ulForbdInfo = EMMC_EMM_FORBIDDEN_PLMN_NOT_ALLOW_REG;
    }
    else
    {
...
NAS_MM_CELL_TYPE_ENUM_UINT8 NAS_EMM_GetCurrentCellType( VOS_VOID )
{
    ...
    ucCellType                         = pstAreaInfo->ucCellType;

    return ucCellType;
}

VOS_UINT32 NAS_EMM_IsCurrentCellInCsgWhiteList(
    MMC_LMM_PLMN_ID_STRU               *pstPlmn,
    VOS_UINT32                          ulCsgId
)
{
    if (VOS_TRUE == NAS_MML_IsCsgIdInCsgWhiteList(VOS_NULL_PTR, pstPlmn, ulCsgId))
    {
        return VOS_TRUE;
    }
    return VOS_FALSE;
...
)
{
    ...
    {
	#if(FEATURE_ON == FEATURE_CSG)
        if ((0xFFFFFFFF != ulCsgId)
        && (VOS_FALSE == NAS_EMM_IsCurrentCellInCsgWhiteList(pstPlmn, ulCsgId)))
        {
            return VOS_FALSE;
        }
	#endif
...
,LNAS_ENTRY);
     ...
     * 时发起的搜索才是背景LIST搜
     */
    if (((MMC_LMM_PLMN_SRCH_USER_LIST == ulSearchType)||
         (MMC_LMM_PLMN_SRCH_INTER_LIST_FULL_BAND == ulSearchType)||
         (MMC_LMM_PLMN_SRCH_INTER_LIST_PREF_BAND == ulSearchType)) &&
...
VOS_UINT32 NAS_EMMC_IsCsfbSuppForDualRxUEs(VOS_VOID)
{
    NAS_LMM_EMMC_LOG_NORM("NAS_EMMC_IsCsfbSuppForDualRxUEs is entered.");
    TLPS_PRINT2LAYER_INFO(NAS_EMMC_IsCsfbSuppForDualRxUEs_ENUM,LNAS_ENTRY);

    if (PS_TRUE == NAS_EMMC_GetCsfbSuppForDualRxUEsFlg())
    {
        return VOS_TRUE;
    }
