...
#define IMSA_MTU_DEFAULT_LENGTH         (1280)  /* MTU默认值 */
#define IMSA_MTU_MAX_LENGTH             (1500)  /* MTU最大值 */
...
#define IMSA_NV_Write( ulId, pItemData, usItemDataLen ) \
...
enum IMSA_TIMER_ID_ENUM
{
    ...
    TI_IMSA_WAIT_DEREG_TIMER            = 0x0003,           /* IMS动态开关，等待iMS去注册结果的定时器。如果超时，则通知IMS发起本地区注册 */
    TI_IMSA_WAIT_READFILE_CNF_TIMER     = 0x0004,           /* 读卡流程，等待读卡结果的定时器 */
    ...
    TI_IMSA_WAIT_WIFI_PDP_ACTIVE_CNF    = 0x0103,           /* 等待wifi回复active cnf定时器 */
    TI_IMSA_WAIT_WIFI_PDP_READY_IND     = 0x0104,           /* 等待wifi上报READY IND定时器 */
    TI_IMSA_WAIT_WIFI_PDP_DEACTIVE_CNF  = 0x0105,           /* 等待wifi回复deactive cnf定时器 */
    ...
    TI_IMSA_CALL_VEDIO_PDP_LOST_MODIFY_CALL = 0x030a,           /* 视频承载释放后主动视频降语音定时器 */
    ...
    TI_IMSA_DOMAIN_TIMER1_WIFI          = 0x080C,           /* WIFI域惩罚定时器 */
    TI_IMSA_DOMAIN_TIMER1_LTE           = 0x080D,           /* LTE域惩罚定时器 */
...
enum IMSA_OM_MSG_ENUM
{
    ...
    ID_IMSA_OM_NON_REG_REJ_INFO          = ID_IMSA_OM_TIMER_MSG_MAX + 0x03,          /* 非注册被拒原因可维可测 */
    ID_IMSA_OM_NO_REG_CAUSE_INFO         = ID_IMSA_OM_TIMER_MSG_MAX + 0x04,          /* 注册失败原因可维可测 */
    ID_IMSA_OM_SERVICE_STATE_CHANGE_INFO = ID_IMSA_OM_TIMER_MSG_MAX + 0x05,          /* 服务状态切换可维可测 */
    ID_IMSA_RAT_SUPPORT_NV_INFO          = ID_IMSA_OM_TIMER_MSG_MAX + 0x06,          /* 接入技术支持NV可维可测 */
    ID_IMSA_IMSA_CONFIG_NV_INFO          = ID_IMSA_OM_TIMER_MSG_MAX + 0x07,          /* IMSA_CONFIG NV可维可测 */
    ID_IMSA_OM_MSG_BUTT
};
...
extern VOS_VOID  IMSA_SendRrcQualityConfigReq(IMSA_LRRC_CFG_QUALITY_ENUM_UINT8  enQualityReportType);
...
extern VOS_UINT32 IMSA_IsImsTransactionExist(VOS_VOID);
...
/* 新增入参 */
extern VOS_VOID IMSA_SndRrcVolteStatusNotify
(
...
extern VOS_VOID IMSA_ProcLmmAttachBeginInd(const VOS_VOID * pMsg);
...
extern VOS_VOID IMSA_ProcLmmEmcEstFailInd(const VOS_VOID *pMsg);

extern VOS_VOID IMSA_SndOmNonRegRejNvInfo(VOS_VOID);
extern VOS_VOID IMSA_SndOmRatSupportNvInfo(IMSA_NV_IMS_RAT_SUPPORT_STRU* pstNvImsRatSupport);
extern VOS_VOID IMSA_SndOmImsaConfigNvInfo(IMSA_NV_IMSA_CONFIG_STRU* pstNvImsaConfig);
...
extern VOS_UINT32 IMSA_GetIgnoreMediaBearerQciFlg(VOS_VOID);
