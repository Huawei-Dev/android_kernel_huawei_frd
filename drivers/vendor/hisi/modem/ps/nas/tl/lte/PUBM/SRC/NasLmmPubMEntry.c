...
VOS_UINT32 NAS_EMM_RcvRrmPsStatsuInd( MsgBlock  *pMsg );
VOS_UINT32 NAS_EMM_PreProcXccCallEndNtf(MsgBlock  *pMsg);
VOS_UINT32  NAS_EMM_MsRegPreProcXccCallEndNtfMsg(VOS_VOID);
VOS_UINT32  NAS_EMM_MsSerInitPreProcXccCallEndNtfMsg( VOS_VOID);
NAS_LMM_COMM_ACT_STRU               g_astMmPreProcMap[] =
{
    ...
    {ID_MMC_LMM_SUSPEND_REL_REQ,                   WUEPS_PID_MMC,            NAS_EMM_PreProcMmcSuspendRelReq},
    /*========== SUSPEND =========*/
    ...
    {ID_XCC_LMM_CALL_START_NTF,                   UEPS_PID_XCC,            NAS_EMM_PreProcXccCallStartNtf},

    {ID_XCC_LMM_CALL_END_NTF,                     UEPS_PID_XCC,            NAS_EMM_PreProcXccCallEndNtf}
};
...
static VOS_UINT32  NAS_LMM_PreProcIntraConn2IdleReq( MsgBlock *          pMsg )
{
    ...
    {
        /* 当T3412定时器未运行且T3423定时器未运行时, 启动3412定时器并通知MMC */
        if ((NAS_LMM_TIMER_STOPED == NAS_LMM_IsPtlTimerRunning(TI_NAS_EMM_PTL_T3412))
            && (NAS_LMM_TIMER_STOPED == NAS_LMM_IsPtlTimerRunning(TI_NAS_EMM_PTL_T3423)))
        {
...
VOS_UINT32  NAS_LMM_PreProcMmcRelReq( MsgBlock *          pMsg )
{
    ...
    switch(ulCurEmmStat)
    {
        ...
        default:
                if((EMM_MS_REG == NAS_LMM_GetEmmCurFsmMS())
                    ||(EMM_MS_DEREG == NAS_LMM_GetEmmCurFsmMS()))
                {
                    NAS_EMM_RelReq(NAS_LMM_NOT_BARRED);
                }
                /*
...
)
{
    ...
    {
        NAS_LMM_PUBM_LOG_INFO("NAS_LMM_PreProcMsgEsmClLocalDetachNotify is not suspend.");
        TLPS_PRINT2LAYER_INFO(NAS_LMM_PreProcMsgEsmClLocalDetachNotify_ENUM,LNAS_EMM_IS_NOT_SUSPEND);
        /* 识别在C2L搜网时收到该消息处理，判断有承载(没有承载在收到系统消息时控了)，转到去注册态，
            local detach并给mmc上报。
            问题场景:
            从模ESM收到EHSM CLEAR ALL消息，然后马上搜L，L转到主模，这时ESM给LMM发送
            LOCAL DETACH NOTIFY，这时LMM主模也应该处理 */
        if((EMM_SS_RESUME_MMCORI_WAIT_SYS_INFO_IND == NAS_LMM_GetEmmCurFsmSS())
            &&(NAS_MML_PS_BEARER_STATE_ACTIVE == NAS_EMM_IsEpsBearStatusAct()))
        {
            NAS_EMM_AdStateConvert( EMM_MS_DEREG,
...
VOS_UINT32 NAS_LMM_SendMccCellSignReportInd(MsgBlock  *pstMsg)
{
    ...
    pstMmcCellSignRptInd->stRxAntInfo.ucRxANTNum  = pstRrcCellSignPptInd->stRxAntInfo.ucRxANTNum;
    pstMmcCellSignRptInd->stRxAntInfo.asRsrpRx[0] = pstRrcCellSignPptInd->stRxAntInfo.asRsrpRx[0];
    pstMmcCellSignRptInd->stRxAntInfo.asRsrpRx[1] = pstRrcCellSignPptInd->stRxAntInfo.asRsrpRx[1];
    pstMmcCellSignRptInd->stRxAntInfo.asRsrpRx[2] = pstRrcCellSignPptInd->stRxAntInfo.asRsrpRx[2];
    pstMmcCellSignRptInd->stRxAntInfo.asRsrpRx[3] = pstRrcCellSignPptInd->stRxAntInfo.asRsrpRx[3];
    pstMmcCellSignRptInd->stRxAntInfo.alSINRRx[0] = pstRrcCellSignPptInd->stRxAntInfo.alSINRRx[0];
    pstMmcCellSignRptInd->stRxAntInfo.alSINRRx[1] = pstRrcCellSignPptInd->stRxAntInfo.alSINRRx[1];
    pstMmcCellSignRptInd->stRxAntInfo.alSINRRx[2] = pstRrcCellSignPptInd->stRxAntInfo.alSINRRx[2];
    pstMmcCellSignRptInd->stRxAntInfo.alSINRRx[3] = pstRrcCellSignPptInd->stRxAntInfo.alSINRRx[3];
    ...
    NAS_LMM_PUBM_LOG1_INFO("NAS_LMM_SendMccCellSignReportInd:ucRxANTNum:", pstRrcCellSignPptInd->stRxAntInfo.ucRxANTNum);
    TLPS_PRINT2LAYER_INFO1(NAS_LMM_SendMccCellSignReportInd_ENUM,LNAS_FUNCTION_LABEL8,
                                               pstRrcCellSignPptInd->stRxAntInfo.ucRxANTNum);

    NAS_LMM_PUBM_LOG2_INFO("NAS_LMM_SendMccCellSignReportInd:asRsrpRx[0],asRsrpRx[1]:",
                                                pstRrcCellSignPptInd->stRxAntInfo.asRsrpRx[0],
                                                pstRrcCellSignPptInd->stRxAntInfo.asRsrpRx[1]);
    TLPS_PRINT2LAYER_INFO2(NAS_LMM_SendMccCellSignReportInd_ENUM,LNAS_FUNCTION_LABEL9,
                                                pstRrcCellSignPptInd->stRxAntInfo.asRsrpRx[0],
                                                pstRrcCellSignPptInd->stRxAntInfo.asRsrpRx[1]);

    NAS_LMM_PUBM_LOG2_INFO("NAS_LMM_SendMccCellSignReportInd:asRsrpRx[2],asRsrpRx[3]:",
                                                pstRrcCellSignPptInd->stRxAntInfo.asRsrpRx[2],
                                                pstRrcCellSignPptInd->stRxAntInfo.asRsrpRx[3]);
    TLPS_PRINT2LAYER_INFO2(NAS_LMM_SendMccCellSignReportInd_ENUM,LNAS_FUNCTION_LABEL10,
                                                pstRrcCellSignPptInd->stRxAntInfo.asRsrpRx[2],
                                                pstRrcCellSignPptInd->stRxAntInfo.asRsrpRx[3]);

    NAS_LMM_PUBM_LOG2_INFO("NAS_LMM_SendMccCellSignReportInd:alSINRRx[0],alSINRRx[1]:",
                                                pstRrcCellSignPptInd->stRxAntInfo.alSINRRx[0],
                                                pstRrcCellSignPptInd->stRxAntInfo.alSINRRx[1]);
    TLPS_PRINT2LAYER_INFO2(NAS_LMM_SendMccCellSignReportInd_ENUM,LNAS_FUNCTION_LABEL11,
                                                pstRrcCellSignPptInd->stRxAntInfo.alSINRRx[0],
                                                pstRrcCellSignPptInd->stRxAntInfo.alSINRRx[1]);

    NAS_LMM_PUBM_LOG2_INFO("NAS_LMM_SendMccCellSignReportInd:alSINRRx[2],alSINRRx[3]:",
                                                pstRrcCellSignPptInd->stRxAntInfo.alSINRRx[2],
                                                pstRrcCellSignPptInd->stRxAntInfo.alSINRRx[3]);
    TLPS_PRINT2LAYER_INFO2(NAS_LMM_SendMccCellSignReportInd_ENUM,LNAS_FUNCTION_LABEL12,
                                                pstRrcCellSignPptInd->stRxAntInfo.alSINRRx[2],
                                                pstRrcCellSignPptInd->stRxAntInfo.alSINRRx[3]);
...
VOS_UINT32 NAS_EMM_PreProcXccCallEndNtf(MsgBlock  *pMsg)
{
    NAS_LMM_PUBM_LOG_INFO("NAS_EMM_PreProcXccCallEndNtf Enter.");
    TLPS_PRINT2LAYER_INFO(NAS_EMM_PreProcXccCallEndNtf_ENUM,LNAS_ENTRY);

    (VOS_VOID)pMsg;

    if(VOS_FALSE == NAS_EMM_IsSupportSrlte())
    {
        NAS_LMM_PUBM_LOG_INFO("NAS_EMM_PreProcXccCallEndNtf not support SRLTE.");
        TLPS_PRINT2LAYER_INFO(NAS_EMM_PreProcXccCallEndNtf_ENUM,LNAS_FUNCTION_LABEL1);

        /* 清除ESR发起过标识 */
        NAS_EMM_SER_SaveXccEsrFlg(NAS_EMM_XCC_ESR_FLAG_INVALID);

        NAS_EMM_SER_SaveXccCallFlg(NAS_EMM_XCC_CALL_FLAG_INVALID);

        /* 通知ERABM电话结束*/
        NAS_EMM_ERABM_SendCallEndInd();
        return NAS_LMM_MSG_HANDLED;
    }
    ...
    switch(NAS_EMM_CUR_MAIN_STAT)
    {
        ...
        case    EMM_MS_REG:
                (VOS_VOID)NAS_EMM_MsRegPreProcXccCallEndNtfMsg();
                break;

        case    EMM_MS_SER_INIT:
                (VOS_VOID)NAS_EMM_MsSerInitPreProcXccCallEndNtfMsg();
                break;
...
VOS_UINT32  NAS_EMM_MsRegPreProcXccCallEndNtfMsg( VOS_VOID)
{
    NAS_EMM_SER_LOG_NORM( "NAS_EMM_MsRegPreProcXccCallEndNtfMsg:enter!");
    TLPS_PRINT2LAYER_INFO(NAS_EMM_MsRegPreProcXccCallEndNtfMsg_ENUM,LNAS_ENTRY);
    ...
    switch (NAS_LMM_GetEmmCurFsmSS())
    {
        ...
        case    EMM_SS_REG_ATTEMPTING_TO_UPDATE_MM  :
                ...
                if(NAS_EMM_XCC_ESR_FLAG_VALID == NAS_EMM_SER_GetXccEsrFlg())
                {
                    ...
                    /* 发起TAU */
                    NAS_EMM_TAU_StartTAUREQ(NAS_LMM_OM_TAU_START_TYPE_RESUME_PS);
                }
...
VOS_UINT32  NAS_EMM_MsSerInitPreProcXccCallEndNtfMsg( VOS_VOID)
{
    NAS_EMM_SER_LOG_NORM( "NAS_EMM_MsSerInitPreProcXccCallEndNtfMsg enter!");
    TLPS_PRINT2LAYER_INFO(NAS_EMM_MsSerInitPreProcXccCallEndNtfMsg_ENUM,LNAS_ENTRY);
...
VOS_UINT32  NAS_EMM_PreProcMmcSuspendRelReq(MsgBlock *    pMsg)
{
    (VOS_VOID)pMsg;

    /* LRRC挂起上层, EMM在等待ESM/ERABM挂起回复期间收到MMC的SUSPEND REl REQ时需要单独处理
       所以去状态处理, 其他状态直接透传给LRRC */
    if ((EMM_MS_SUSPEND == NAS_LMM_GetEmmCurFsmMS())
        && (EMM_SS_SUSPEND_RRCORI_WAIT_OTHER_SUSPEND == NAS_LMM_GetEmmCurFsmSS()))
    {
        return NAS_LMM_MSG_DISCARD;
    }
