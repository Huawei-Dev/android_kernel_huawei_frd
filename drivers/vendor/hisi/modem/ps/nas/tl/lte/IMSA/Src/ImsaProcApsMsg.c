...
VOS_VOID IMSA_CONN_ProcHoEventInActCnf(VOS_VOID)
{
    ...
    if( IMSA_IMS_HO_TYPE_WIFI2LTE == pstImsaHoEvent->enPdnHoType )
    {
        ...
        if( ((VOS_TRUE == IMSA_CONN_IsPdpInfoIncludeRegPara(IMSA_CONN_TYPE_NORMAL,&IMSA_CONN_GetNormalConnAddr()->astSipSignalPdpArray[0]))
               ||(VOS_TRUE == IMSA_CONN_IsPdpInfoIncludeRegPara(IMSA_CONN_TYPE_NORMAL,&IMSA_CONN_GetNormalConnAddr()->astSipSignalPdpArray[1])))
            && ( VOS_TRUE == IMSA_IsImsAudioChannelExist()))
        {
            /* 通知VC启动切换 */
            IMSA_NtfVcHoEvent(IMSA_VC_MODE_EUTRAN_IMS);
        }

        /* 给CDS配置下行过滤承载 */
        IMSA_CONN_SndCdsSetImsBearerReq(IMSA_CDS_IMS_BEARER_RAT_TYPE_3GPP_EUTRAN);
#endif
        if(VOS_TRUE == pstImsaHoEvent->ucOldPdnRelIndEvent)
        {
            IMSA_CONN_HoConnSucc(  IMSA_CONN_TYPE_NORMAL,  IMSA_IMS_HO_TYPE_WIFI2LTE);
        }
        else if((VOS_FALSE == pstImsaHoEvent->ucOldPdnRelIndEvent)
            ...
            && (IMSA_CONN_PDN_HO_STATE_HOING == pstImsaHoEvent->enPdnHoState))
        {
            ...
            && ((IMSA_CONN_STATUS_CONN == pstWifiNormalConn->enImsaConnStatus)
                ||(IMSA_CONN_STATUS_CONNING == pstWifiNormalConn->enImsaConnStatus))
            && (IMSA_CONN_PDN_HO_STATE_HOING == pstImsaHoEvent->enPdnHoState))
        {
            /* 更新切换状态 */
            pstImsaHoEvent->enPdnHoState = IMSA_CONN_PDN_HO_STATE_AFTER_HOSUCC;
        ...
        {
            IMSA_WARN_LOG("IMSA_CONN_ProcIpv4ActCnf: Ho status wrong!");
        }
    }
    else
    {
        IMSA_WARN_LOG("IMSA_CONN_ProcIpv4ActCnf: Pdn Act Cnf in L2W HO!");
    }
...
VOS_VOID IMSA_CONN_ProcHoEventInDeactInd()
{
    IMSA_CONN_HO_EVENT_STRU            *pstImsaHoEvent  = VOS_NULL_PTR;

    pstImsaHoEvent   = IMSA_CONN_GetHoEventAddr();

    if( (pstImsaHoEvent->enPdnHoState < IMSA_CONN_PDN_HO_STATE_HOING)
        ||(pstImsaHoEvent->enPdnHoState > IMSA_CONN_PDN_HO_STATE_AFTER_HOFAIL))
    {
        IMSA_WARN_LOG("IMSA_CONN_ProcHoEventInDeactInd: Ho status wrong!");
    }
        ...
        &&(IMSA_CONN_PDN_HO_STATE_AFTER_HOFAIL == pstImsaHoEvent->enPdnHoState))
    {
        ...
        IMSA_CONN_HoConnFail(IMSA_CONN_TYPE_NORMAL,
                             IMSA_CONN_RESULT_FAIL_CONN_RELEASING,
                             IMSA_DOMAIN_SWITCH_TYPE_W2L_SWITCH,
                             IMSA_IMS_DOMAIN_LTE);
    }
    else if( (IMSA_IMS_HO_TYPE_LTE2WIFI == pstImsaHoEvent->enPdnHoType )
        && (IMSA_CONN_PDN_HO_STATE_HOING == pstImsaHoEvent->enPdnHoState))
    {
        /* 保存老PDN释放标志,继续等待处理 */
        pstImsaHoEvent->ucOldPdnRelIndEvent = VOS_TRUE;

        /* 如果切换承载没有建立,同时存在呼叫通道,则启动通道切换 */
        if( (VOS_TRUE == IMSA_IsImsAudioChannelExist())
            && (VOS_FALSE == pstImsaHoEvent->ucNewSignalPdnSetupEvent))
        {
            IMSA_WARN_LOG("IMSA_CONN_ProcHoEventInDeactInd:VC Control HO event in voice call!");
            /* 通知VC启动切换 */
    ...
    {
        IMSA_CONN_HoConnSucc(IMSA_CONN_TYPE_NORMAL,pstImsaHoEvent->enPdnHoType);
    ...
    {
        IMSA_WARN_LOG("IMSA_CONN_ProcHoEventInDeactInd: wrong Pdn Deact Ind!");
    }
    ...
    if (IMSA_CONN_TYPE_NORMAL == enConnType)
    {
        ...
        for (i = 0; i < pstNormalConn->ulSipSignalPdpNum; i++)
        {
                ...
                && (0 != IMSA_MEM_CMP(aucTemp, pstPdpIpAddr->aucIpV4Addr, IMSA_IPV4_ADDR_LEN)))
            {
                ...
                {
                    /* 将连接状态转到CONN态 */\
                    IMSA_CONN_SetConnStatus(IMSA_CONN_TYPE_NORMAL,IMSA_CONN_STATUS_CONN, IMSA_IMS_DOMAIN_LTE);
...
)
{
    ...
    */
    ...
    {
        pstDialParaInfo->bitOpReqType = IMSA_OP_TRUE;
        pstDialParaInfo->enPdpRequestType = TAF_PDP_REQUEST_TYPE_HANDOVER;

        pstDialParaInfo->bitOpIpAddr  = IMSA_OP_TRUE;

        /* 将WIFI IP地址配置, 配置切换IP ADDR */
        IMSA_MEM_CPY_S( &pstDialParaInfo->stPdpAddr,
                    sizeof(TAF_PDP_ADDR_STRU),
                    &pstImsaHoEvent->u.stSipSignalPdp.stPdpAddr,
                    sizeof(TAF_PDP_ADDR_STRU));
    }
    else
    {

        pstDialParaInfo->enPdpRequestType = TAF_PDP_REQUEST_TYPE_INITIAL;
    }
...
)
{
    ...
    if (TAF_PDP_IPV4 == pstPdpActivateCnf->stPdpAddr.enPdpType)
    {
        ...
        if (VOS_OK != ulRet)
        {
            ...
            if( IMSA_CONN_PDN_HO_STATE_IDLE == pstImsaHoEvent->enPdnHoState)
            {
                IMSA_CONN_SetupConnSucc(IMSA_CONN_TYPE_NORMAL, IMSA_IMS_DOMAIN_LTE);
            }
            else
            {
                /* 将连接状态转到CONN态 */\
...
)
{
    ...
    if (IMSA_CONN_TYPE_EMC == enConnType)
    {
        ...
        if( IMSA_CONN_PDN_HO_STATE_IDLE == pstImsaHoEvent->enPdnHoState)
        {
            IMSA_CONN_SetupConnSucc(IMSA_CONN_TYPE_EMC, IMSA_IMS_DOMAIN_LTE);
        }
        else
        {
            IMSA_WARN_LOG("IMSA_CONN_ProcIpv4ActCnf:conn type is not valid for handover.");
        }
        return ;
    }
...
VOS_VOID  IMSA_CONN_ProcLteSetupOrHoConnFail
(
    IMSA_CONN_TYPE_ENUM_UINT32                  enConnType,
    IMSA_CONN_RESULT_ENUM_UINT32                enResult,
    TAF_PS_CAUSE_ENUM_UINT32                    enCauuse
)
{
    IMSA_CONN_HO_EVENT_STRU *pstImsaHoEvent = VOS_NULL;

    IMSA_INFO_LOG("IMSA_CONN_ProcLteSetuporHoConnFail is entered");

    pstImsaHoEvent = IMSA_CONN_GetHoEventAddr();

    if(IMSA_CONN_PDN_HO_STATE_IDLE == pstImsaHoEvent->enPdnHoState)
    {
        IMSA_CONN_SetupConnFail(enConnType, IMSA_CONN_RESULT_FAIL_CN_REJ,enCauuse, IMSA_IMS_DOMAIN_LTE);
    }
    else if(IMSA_CONN_PDN_HO_STATE_HOING == pstImsaHoEvent->enPdnHoState)/* 调用到该函数，切换类型一定是W2L */
    {
        ...
        {
            IMSA_WARN_LOG("IMSA_CONN_ProcLteSetuporHoConnFail:ConnType is invalid.");
        }
    }
    else
    {
        IMSA_WARN_LOG("IMSA_CONN_ProcLteSetuporHoConnFail:enPdnHoState is invalid.");
    }
...
#if 0
            IMSA_CONN_SetupConnFail(enConnType, IMSA_CONN_RESULT_FAIL_CN_REJ,pstPdpActivateRej->enCause, IMSA_IMS_DOMAIN_LTE);
#endif
            IMSA_CONN_ProcLteSetupOrHoConnFail(enConnType,
                                               IMSA_CONN_RESULT_FAIL_CN_REJ,
                                               pstPdpActivateRej->enCause);
...
)
{
    ...
    /* 不是CONNING状态，则直接退出 */
    if (IMSA_FALSE == IMSA_CONN_IsConnStatusEqual(enConnType, IMSA_CONN_STATUS_CONNING, IMSA_IMS_DOMAIN_LTE))
    {
    ...
    switch ( pstPdpActivateCnf->stPdpAddr.enPdpType )
    {
        case TAF_PDP_IPV4:
            if((IMSA_CONN_PDN_HO_STATE_IDLE != pstImsaHoEvent->enPdnHoState)
                &&( (IMSA_WIFI_IP_IPV4 == pstImsaHoEvent->u.stSipSignalPdp.stPdpAddr.enIpType)
                    || (IMSA_WIFI_IP_IPV4V6 == pstImsaHoEvent->u.stSipSignalPdp.stPdpAddr.enIpType)))
            {
                /* 如果建立承载的IP地址和PCSCF就是当前注册使用的IP/PCSCF,同时存在呼叫通道,则启动通道切换 */
                if( (VOS_TRUE == IMSA_CONN_IsPdpInfoIncludeRegPara(IMSA_CONN_TYPE_NORMAL,pstPdpInfo))
                    && ( VOS_TRUE == IMSA_IsImsAudioChannelExist()))
                {
                    /* 通知VC启动切换 */
                    IMSA_NtfVcHoEvent(IMSA_VC_MODE_EUTRAN_IMS);
        ...
        case TAF_PDP_IPV6:
            if((IMSA_CONN_PDN_HO_STATE_IDLE != pstImsaHoEvent->enPdnHoState)
                &&( (IMSA_WIFI_IP_IPV6 == pstImsaHoEvent->u.stSipSignalPdp.stPdpAddr.enIpType)
                    || (IMSA_WIFI_IP_IPV4V6 == pstImsaHoEvent->u.stSipSignalPdp.stPdpAddr.enIpType)))
            {
...
VOS_UINT32 IMSA_CONN_ProcNoHoTafPsEvtPdpActivateRej
(
...
)
{
    ...
    IMSA_INFO_LOG1("IMSA_CONN_ProcNOHoTafPsEvtPdpActivateRej is entered! opid:", pstPdpActivateRej->stCtrl.ucOpId);
    ...
    if (IMSA_FAIL == ulResult)
    {
        IMSA_WARN_LOG("IMSA_CONN_ProcHoTafPsEvtPdpActivateRej:Get Conn Type Failed!");
...
VOS_UINT32 IMSA_CONN_ProcHoTafPsEvtPdpActivateRej
(
    TAF_PS_CALL_PDP_ACTIVATE_REJ_STRU  *pstPdpActivateRej
)
{
    ...
    {
        IMSA_WARN_LOG("IMSA_CONN_ProcHoTafPsEvtPdpActivateRej:Conn Type is invalid.");
        return IMSA_FAIL;
    }
    ...
    {
       /* L2W: 如果当前不存在激活的信令承载 更新切换事件中的原域等待pdn状态 为已经收到原域pdn释放指示*/
       if (IMSA_FALSE == IMSA_CONN_HasActiveSipSignalPdp(IMSA_CONN_TYPE_NORMAL))
       {
            pstImsaHoEvent->ucOldPdnRelIndEvent = VOS_TRUE;
            ...
            /* 连接模块状态迁移至IDLE */
            IMSA_CONN_SetConnStatus(IMSA_CONN_TYPE_NORMAL,  IMSA_CONN_STATUS_IDLE, IMSA_IMS_DOMAIN_LTE);
       }
       else
       {
            /* L2W的切换下, 给网侧发送承载去激活请求 ，把L下的承载干掉 */
            if( IMSA_CONN_PDN_HO_STATE_AFTER_HOSUCC == pstImsaHoEvent->enPdnHoState)
            {
                /* 停止IMS拨号定时器 */
...
)
{
    ...
    if(IMSA_CONN_PDN_HO_STATE_IDLE == pstImsaHoEvent->enPdnHoState)
    {
        ulResult = IMSA_CONN_ProcNoHoTafPsEvtPdpActivateRej(pstPdpActivateRej);
    }
    else
    {
        ulResult = IMSA_CONN_ProcHoTafPsEvtPdpActivateRej(pstPdpActivateRej);
    }
...
)
{
    ...
    /* 当前为语音电话等待资源预留结果, 若定制NV打开的情况下, 不检查QCI直接认为建
       立的承载为语音承载, 解决AT&T用例1846由于仪器下发QCI为4的承载, 导致资源预留
       超时用例失败 */
    if (IMSA_TRUE == IMSA_CONN_IsVoiceMediaPdpType((IMSA_PDP_TFT_INFO_STRU *)&pstPdpActivateInd->stTft, pstPdpActivateInd->stEpsQos.ucQCI)
        || ((IMSA_TRUE == IMSA_IsVoiceCallWaitResReady())
            && (IMSA_TRUE == IMSA_GetIgnoreMediaBearerQciFlg())))
    {
        ...
        if((IMSA_CONN_PDN_HO_STATE_IDLE != pstImsaHoEvent->enPdnHoState)
            &&(IMSA_IMS_HO_TYPE_WIFI2LTE == pstImsaHoEvent->enPdnHoType))
        {
            pstImsaHoEvent->ucNewVoicePdnSetupEvent = VOS_TRUE;
        }
    }
    else
    {
        ...
        if((IMSA_CONN_PDN_HO_STATE_IDLE != pstImsaHoEvent->enPdnHoState)
            &&(IMSA_IMS_HO_TYPE_WIFI2LTE == pstImsaHoEvent->enPdnHoType))
        {
            pstImsaHoEvent->ucNewVideoPdnSetupEvent = VOS_TRUE;
        }
        /* 音频和视频承载配给CDS */
        IMSA_CONN_SndCdsSetImsBearerReq(IMSA_CDS_IMS_BEARER_RAT_TYPE_3GPP_EUTRAN);
    }
...
)
{
    ...
    /* 如果是IDLE状态，则直接退出 */
    if (IMSA_TRUE       == IMSA_CONN_IsConnStatusEqual(enConnType, IMSA_CONN_STATUS_IDLE, IMSA_IMS_DOMAIN_LTE))
    {
...
)
{
    ...
    /* 不是RELEASING状态，则直接退出 */
    if (IMSA_FALSE == IMSA_CONN_IsConnStatusEqual(enConnType, IMSA_CONN_STATUS_RELEASING, IMSA_IMS_DOMAIN_LTE))
    {
    ...
    /* W2L Ho fail, and need to release LTE IMS signal pdp */
    if((IMSA_IMS_HO_TYPE_WIFI2LTE == pstImsaHoEvent->enPdnHoType)
        &&(IMSA_CONN_PDN_HO_STATE_AFTER_HOFAIL == pstImsaHoEvent->enPdnHoState ))
    {
        if (IMSA_TRUE == IMSA_CONN_HasActiveSipSignalPdp(enConnType))
        ...
        {
            IMSA_CONN_HoConnFail(IMSA_CONN_TYPE_NORMAL,
                             IMSA_CONN_RESULT_FAIL_OHTERS,
                             IMSA_IMS_HO_TYPE_WIFI2LTE,
                             IMSA_IMS_DOMAIN_LTE);
        }

        return IMSA_SUCC;
    }
    else if(IMSA_IMS_HO_TYPE_WIFI2LTE == pstImsaHoEvent->enPdnHoType)
    {
        IMSA_WARN_LOG("IMSA_CONN_ProcTafPsEvtPdpDeactivateCnf: W2L in other except AFTER_HOFAIL!");
        return IMSA_SUCC;
    }
    /* LTE2WIFI HO Case */
    else if(IMSA_CONN_PDN_HO_STATE_HOING == pstImsaHoEvent->enPdnHoState)
    {
        if(enConnType != IMSA_CONN_TYPE_NORMAL)
        {
            return IMSA_FAIL;
        ...
        {
            /* 请求APS释放连接, not need to handover the state*/
            IMSA_CONN_RequestApsRelConn(enConnType);
        }
    }
    else if(IMSA_CONN_PDN_HO_STATE_AFTER_HOSUCC == pstImsaHoEvent->enPdnHoState)
    {
        ...
        if( IMSA_IMS_HO_TYPE_LTE2WIFI == pstImsaHoEvent->enPdnHoType)
        {
            ...
            else
            {
               ...
               /* 回复切换成功消息 */
               IMSA_CONN_HoConnSucc(enConnType,pstImsaHoEvent->enPdnHoType);
...
)
{
    ...
    IMSA_CONN_HO_EVENT_STRU                *pstImsaHoEvent      = VOS_NULL_PTR;
    IMSA_CONN_MEDIA_PDP_TYPE_ENUM_UINT32    enRelMediaPdpType   = IMSA_CONN_MEDIA_PDP_TYPE_BUTT;
    ...
    /* 若当前释放的是媒体承载, 则在清除之前获取当前释放的媒体承载是语音承载还是视频承载 */
    if (IMSA_CONN_SIP_PDP_TYPE_MEDIA == enSipPdpType)
    {
        enRelMediaPdpType = IMSA_CONN_GetCurrMediaPdpType(pstPdpDeactivateInd->ucCid, pstPdpDeactivateInd->ucRabId);
    }
    ...
    if (IMSA_CONN_TYPE_EMC == enConnType)
    {
        if (IMSA_CONN_SIP_PDP_TYPE_MEDIA == enSipPdpType)
        {
            ...
            /* 新增入参 */
            IMSA_CONN_SndConnRelInd(enConnType, IMSA_CONN_SIP_PDP_TYPE_MEDIA, IMSA_CONN_MEDIA_PDP_TYPE_VOICE, IMSA_IMS_DOMAIN_LTE);
    ...
    if (IMSA_CONN_SIP_PDP_TYPE_MEDIA == enSipPdpType)
    {
        ...
        {
            /* 增加入参 */
            IMSA_CONN_SndConnRelInd(enConnType, IMSA_CONN_SIP_PDP_TYPE_MEDIA, enRelMediaPdpType, IMSA_IMS_DOMAIN_LTE);
        }
        else
        {
             /* 切换中的MEDIA释放,不处理 */
    ...
    if (IMSA_CONN_STATUS_CONNING == IMSA_CONN_GetNormalConnStatus())
    {
        ...
        IMSA_CONN_SndConnRelInd(enConnType, IMSA_CONN_SIP_PDP_TYPE_SIGNAL, IMSA_IMS_DOMAIN_LTE);*/
        ...
        else
        {
            if(IMSA_CONN_PDN_HO_STATE_IDLE == pstImsaHoEvent->enPdnHoState)
            {
                ...
                IMSA_CONN_SetupConnFail(  enConnType,
                                          IMSA_CONN_RESULT_FAIL_TIMER_EXP,
                                          (TAF_PS_CAUSE_ENUM_UINT32)TAF_PS_CAUSE_BUTT,IMSA_IMS_DOMAIN_LTE);
            }
            else
            {
...
)
{
    ...
    /* 如果是CONN状态，则更新PDP上下文 */
    if (IMSA_TRUE == IMSA_CONN_IsConnStatusEqual(enConnType, IMSA_CONN_STATUS_CONN, IMSA_IMS_DOMAIN_LTE))
    {
    ...
    if (IMSA_TRUE == IMSA_CONN_IsConnStatusEqual(enConnType, IMSA_CONN_STATUS_CONNING, IMSA_IMS_DOMAIN_LTE))
    {
        ...
        if (IMSA_TRUE == IMSA_CONN_IsIpv6PrefixChanged(pstPdpContext, pstPsIpv6InfoInd))
        {
            ...
            {
                /* 切换时,如果IPV6前缀不变,需要使用切换前分配的IPV6地址*/
                IMSA_CONN_SaveIpv6InfoInHo(pstPdpContext, pstPsIpv6InfoInd);
            }
...
)
{
    ...
    {
        IMSA_CONN_SetupConnFail(enConnType, IMSA_CONN_RESULT_FAIL_PDP_ACTIVATE_LIMIT, (TAF_PS_CAUSE_ENUM_UINT32)TAF_PS_CAUSE_BUTT, IMSA_IMS_DOMAIN_LTE);
        #if (FEATURE_ON == FEATURE_PTM)
        IMSA_PdnConnFailErrLogRecord(   enConnType,
    ...
    {
        IMSA_CONN_SetupConnFail(enConnType, IMSA_CONN_RESULT_FAIL_SAME_APN_OPERATING, (TAF_PS_CAUSE_ENUM_UINT32)TAF_PS_CAUSE_BUTT, IMSA_IMS_DOMAIN_LTE);
        #if (FEATURE_ON == FEATURE_PTM)
        IMSA_PdnConnFailErrLogRecord(   enConnType,
    ...
    {
        IMSA_CONN_SetupConnFail(enConnType, IMSA_CONN_RESULT_FAIL_PARA_ERR, (TAF_PS_CAUSE_ENUM_UINT32)TAF_PS_CAUSE_BUTT, IMSA_IMS_DOMAIN_LTE);
        #if (FEATURE_ON == FEATURE_PTM)
        IMSA_PdnConnFailErrLogRecord(   enConnType,
...
VOS_VOID IMSA_CONN_ProcNoHoTimerMsgSipSignalPdpOrigExp
(
    const VOS_VOID                     *pMsg
)
{
    ...
    if (IMSA_CONN_TYPE_EMC == enConnType)
    {
        ...
        IMSA_CONN_SetupConnFail(enConnType, IMSA_CONN_RESULT_FAIL_TIMER_EXP, (TAF_PS_CAUSE_ENUM_UINT32)TAF_PS_CAUSE_BUTT, IMSA_IMS_DOMAIN_LTE);
    ...
    else
    {
        ...
        {
            IMSA_CONN_SetupConnFail(enConnType, IMSA_CONN_RESULT_FAIL_TEMP_FORBIDDEN, (TAF_PS_CAUSE_ENUM_UINT32)TAF_PS_CAUSE_BUTT,IMSA_IMS_DOMAIN_LTE);
        }
        else
        {
            IMSA_CONN_SetupConnFail(enConnType, IMSA_CONN_RESULT_FAIL_TIMER_EXP, (TAF_PS_CAUSE_ENUM_UINT32)TAF_PS_CAUSE_BUTT,IMSA_IMS_DOMAIN_LTE);
...
VOS_VOID IMSA_CONN_ProcHoTimerMsgSipSignalPdpOrigExp
(
     const VOS_VOID                     *pMsg
)
{
    ...
    if(enConnType != IMSA_CONN_TYPE_NORMAL)
    {
        IMSA_WARN_LOG("IMSA_CONN_ProcHoTimerMsgSipSignalPdpOrigExp:invalid conn type.");
        return;
    }
    ...
    {    /* W2L下收到该消息的处理 */
        ...
        if (IMSA_FALSE == IMSA_CONN_IsConnStatusEqual(enConnType, IMSA_CONN_STATUS_CONNING, IMSA_IMS_DOMAIN_LTE))
        {
            IMSA_WARN_LOG("IMSA_CONN_ProcHoTimerMsgSipSignalPdpOrigExp:Not CONNING state!");
            return ;
        }

        IMSA_INFO_LOG("IMSA_CONN_ProcHoTimerMsgSipSignalPdpOrigExp:normal,conning!");
        if (IMSA_TRUE == IMSA_CONN_HasActiveSipSignalPdp(enConnType))
        {
            /* 如果IPV6全局地址已获得，回复连接建立成功；否则启动等待IPV6参数定时器 */
            IMSA_CONN_WaitForIpv6InfoProc(enConnType);
...
#if (FEATURE_ON == FEATURE_PTM)
            IMSA_PdnConnFailErrLogRecord(   IMSA_CONN_TYPE_NORMAL,
                                            IMSA_ERR_LOG_NORM_PDNREJ_CAUSE_ORIG_TIMER_EXP,
                                            IMSA_ERR_LOG_PDN_CONN_NW_REJ_CAUSE_NONE,
                                            IMSA_ERR_LOG_EMC_REG_FAIL_REASON_SIP_SIGAL_PDP_ORIG_TIMER_EXP);
#endif
...
)
{
    ...
    if(IMSA_CONN_PDN_HO_STATE_IDLE != pstImsaHoEvent->enPdnHoState )
    {
        IMSA_CONN_ProcHoTimerMsgSipSignalPdpOrigExp(pMsg);
    }
    else
    {
        IMSA_CONN_ProcNoHoTimerMsgSipSignalPdpOrigExp(pMsg);
    }
...
VOS_VOID IMSA_CONN_ProcHoTimerMsgSipSignalPdpEndExp
(
    const VOS_VOID                     *pMsg
)
{
    ...
    IMSA_INFO_LOG("IMSA_CONN_ProcNoHoTimerMsgSipSignalPdpEndExp is entered!");
    ...
    if(enConnType != IMSA_CONN_TYPE_NORMAL)
    {
        IMSA_WARN_LOG("IMSA_CONN_ProcHoTimerMsgSipSignalPdpEndExp:conn type is invalid.");
        return;
    }
        ...
        || ( IMSA_CONN_PDN_HO_STATE_AFTER_HOSUCC == pstImsaHoEvent->enPdnHoState))
    {
        ...
        if( IMSA_IMS_HO_TYPE_LTE2WIFI == pstImsaHoEvent->enPdnHoType)
        {
            if(IMSA_FALSE == IMSA_CONN_HasActiveSipSignalPdp(enConnType))
            {
                ...
                {
                    /* all signal pdp released and then set the flag. */
                    pstImsaHoEvent->ucOldPdnRelIndEvent = VOS_TRUE;
                }
                IMSA_CONN_ClearConnResource(enConnType);
            }
        ...
        else
        {
            ...
            if(IMSA_FALSE == IMSA_CONN_HasActiveSipSignalPdp(enConnType))
            {
                IMSA_CONN_HoConnFail(enConnType, IMSA_CONN_RESULT_FAIL_OHTERS, IMSA_DOMAIN_SWITCH_TYPE_W2L_SWITCH, IMSA_IMS_DOMAIN_LTE);
            }
            else
            {
    ...
    {
        IMSA_WARN_LOG("IMSA_CONN_ProcHoTimerMsgSipSignalPdpEndExp:HO State is invalid.");
    }
...
VOS_VOID IMSA_CONN_ProcNoHoTimerMsgSipSignalPdpEndExp
(
    const VOS_VOID                     *pMsg
)
{
...
)
{
    ...
    if(IMSA_CONN_PDN_HO_STATE_IDLE != pstImsaHoEvent->enPdnHoState)
    {
        IMSA_CONN_ProcHoTimerMsgSipSignalPdpEndExp(pMsg);
    }
    else
    {
        IMSA_CONN_ProcNoHoTimerMsgSipSignalPdpEndExp(pMsg);
...
)
{
    ...
    IMSA_CONN_SetupConnFail(enConnType, IMSA_CONN_RESULT_FAIL_TEMP_FORBIDDEN, enCause,IMSA_IMS_DOMAIN_LTE);
...
)
{
    ...
    IMSA_CONN_SetupConnFail(enConnType, IMSA_CONN_RESULT_FAIL_PERM_FORBIDDEN, enCause,IMSA_IMS_DOMAIN_LTE);
...
)
{
    ...
    if(IMSA_CONN_PDN_HO_STATE_IDLE == pstImsaHoEvent->enPdnHoState)
    {
        IMSA_CONN_SetupConnFail(enConnType, enResult, (TAF_PS_CAUSE_ENUM_UINT32)TAF_PS_CAUSE_BUTT,IMSA_IMS_DOMAIN_LTE);

        #if (FEATURE_ON == FEATURE_PTM)
        /* 增加承载建立失败的CHR上报 */
        IMSA_CONN_PdnConnFailEventReport(IMSA_ERR_LOG_NORM_PDNREJ_CAUSE_NO_PCSCF,IMSA_ERR_LOG_PDN_CONN_NW_REJ_CAUSE_BUTT);
    ...
    else if(IMSA_CONN_PDN_HO_STATE_HOING == pstImsaHoEvent->enPdnHoState)/* 调用到该函数，切换类型一定是W2L */
    {
        ...
        {
            IMSA_WARN_LOG("IMSA_CONN_ProcNoPcscf:ConnType is invalid.");
        }
    }
    else
    {
        IMSA_WARN_LOG("IMSA_CONN_ProcNoPcscf:enPdnHoState is invalid.");
    }
    return ;
}
