...
/* LPP层内存拷贝宏定义 */
//#define __LINE__    0
#define NAS_LPP_LOG_ERR(String)   (VOS_VOID)vos_printf(" %s\r\n",String)
...
#define NAS_LPP_MTA_MEM_FREE(pAddr) \
            {\
                if(VOS_OK != PS_MEM_FREE_ALL_CHECK((UEPS_PID_MTA), (pAddr)))\
                {\
                    NAS_LPP_LOG_ERR("NAS_LMM_MEM_FREE  ERR!!");\
                }\
            }
...
/* LPP层内存释放宏定义 */
#define NAS_LPP_FREE_MSG(pData)\
            PS_FREE_MSG_ALL_CHECK((PS_PID_LPP),(pData))
...
#define NAS_LPP_WAIT_GNSS_CAP_TIMER_LEN             (100)
...
#define NAS_LPP_MODEM_ORIG_WAIT_GNSS_LOC_CNF_TIMER_LEN (128*1000) /*128秒*/
...
#define NAS_LPP_OTDOA_CELL_MIN_NUM          (3) /*测OTDOA最少需要3个小区，1个参考小区2个邻区*/
#define NAS_LPP_IsReportInfinity(ucReportAmount)  ( (0 == ucReportAmount) ? PS_TRUE : PS_FALSE  )
#define NAS_LPP_LOW_ONE_BIT                 (0x01)
#define NAS_LPP_BAND_ARRAY_CNT              (2)
#define NAS_LPP_PER_ARRAY_BAND_BITMAP_CNT   (32)
...
enum NAS_LPP_ERROR_CODE_ENUM
{
    ...
    NAS_LPP_PROCESS_RELATIVITY_CHECK_FAIL,
    NAS_LPP_ERROR_CODE_BUTT
};
...
enum NAS_LPP_UPLINK_MSG_TYPE_ENUM
{
    ...
    NAS_LPP_UPLINK_MSG_TYPE_NO_NEED_MSG_CNF     = 0,         /*上行消息是ACK消息*/
    NAS_LPP_UPLINK_MSG_TYPE_NEED_ACK_CNF        = 1,         /*LPP流程消息*/
...
/*------------------------------------------非状态机对应的定时器--------------------------------------*/
    ...
    TI_NAS_LPP_MODEM_ORIG_WAIT_GNSS_LOC_CNF,    /*modem主动发起定位请求GPS位置回复定时器*/
...
enum NAS_LPP_GNSS_ORIG_TYPE_ENUM
{
    NAS_LPP_GNSS_ORIG_TYPE_NW,
    NAS_LPP_GNSS_ORIG_TYPE_MODEM,
    NAS_LPP_GNSS_ORIG_TYPE_BUTT
};
typedef VOS_UINT8 NAS_LPP_GNSS_ORIG_TYPE_ENUM_UINT8;
...
typedef struct
{
    VOS_UINT32                          bitOpGetGNSSCapByNV :1;

    VOS_UINT32                          bitOpGnssModemOrigFlag:1;/*GNSS流程modem发起主动位置请求的标识*/


    VOS_UINT32                          bitOpGnssSupFlag:1;/*gps芯片能力支持情况*/
    ...
    VOS_UINT8                           aucRsv[3];
    VOS_UINT32                          ulResponseTimeAllowance;/*LPP上报response time的余量*/
...
typedef struct
{
    ...
    NAS_LPP_TIMER_STRU                      stLppWaitGnssLocCnfTimer;    /* 等待MTA回复GNSS 位置评估回复的定时器 */
...
extern VOS_VOID NAS_LPP_SndLppMtaStandaloneGnssMeasAbort();
...
extern VOS_VOID NAS_LPP_SndLppMtaStandaloneGnssMeasReq
(
    LRRC_LPP_LOC_REPORT_TYPE_ENUM_UINT8     enGnssReportType
);
extern VOS_UINT8  NAS_LPP_GetGnssSupFlag(VOS_VOID);
extern VOS_UINT8  NAS_LPP_GetGnssModemOrigFlag(VOS_VOID);
extern NAS_LPP_GNSS_ORIG_TYPE_ENUM_UINT8 NAS_LPP_GetGnssOrigType(VOS_VOID);
extern VOS_VOID NAS_LPP_SetGnssOrigType
(
    NAS_LPP_GNSS_ORIG_TYPE_ENUM_UINT8 enGnssOrigType
);
extern LRRC_LPP_LOC_REPORT_TYPE_ENUM_UINT8 NAS_LPP_GetGnssReportType(VOS_VOID );
extern VOS_VOID NAS_LPP_SetGnssReportType
(
    LRRC_LPP_LOC_REPORT_TYPE_ENUM_UINT8     enGnssReportType
);
...
extern VOS_VOID NAS_LPP_SetGnssCapNVData(LNAS_LPP_NV_CONFIG_PARA_STRU *pstNvLppConfig);
extern VOS_VOID NAS_LPP_SetGnssCap(LNAS_GNSS_CAP_CFG_NV_STRU *pstGnssCapNv);
...
extern VOS_UINT32  RRC_COMM_CalculateDlEarfcn( VOS_UINT8 ucFreqBandIndicator,
                                   VOS_UINT16 usDlFreq, VOS_UINT32 *pusDlEarfcn );

extern VOS_UINT32  NAS_LPP_GetResponseTimeAllowance(VOS_VOID);
