...
)
{
    ...
    /* 原来处理是:状态不为挂起态，说明是异常，丢弃消息。应CNAS要求现在无论主从模都处理,
       我们目前只考虑恢复流程时收到该消息的场景，不考虑发起attach的场景*/
...
)
{
    ...
    if (PS_TRUE == pEhsmMsg->ulAttachFlag)
    {
        ...
        NAS_ESM_SndEsmRabmClearClBearerNotifyMsg(PS_TRUE);
    ...
    for (i = 0; i < NAS_ESM_EHRPD_PDN_MAX_NUM; i++)
    {
        ...
        if (PS_TRUE == pstPdnEntity->ucUsedFlag)
        {
                                        ...
                                        NAS_ESM_GET_EHRPD_ENTITY_STATE())
            {
                ...
                NAS_ESM_SndEsmAppEhrpdSdfRelIndMsg(i);
            }
...
)
{
    ...
    NAS_ESM_SndEsmAppEhrpdSdfRelIndMsg(ulIndex);
...
)
{
    ...
    /*获取对应EpsbCntxtTbl表地址*/
    pstEpsbCntxtInfo = NAS_ESM_GetEpsbCntxtInfoAddr(pstPdnEntity->ulHandoverEpsbId);

    pstEpsBearInfoInd->enPdpOption      = SM_ESM_PDP_OPT_MODIFY;
    pstEpsBearInfoInd->ulEpsbId         = pstPdnEntity->ulHandoverEpsbId;

    pstEpsBearInfoInd->bitOpEhrpdEpsbId = NAS_ESM_OP_TRUE;
    pstEpsBearInfoInd->ulEhrpdEpsbId    = pstPdnEntity->stEhPdnContextInfo.ulEpsbId;
...
VOS_VOID NAS_ESM_SndEsmAppEhrpdSdfRelIndMsg
(
    VOS_UINT32                          ulIndex
)
{
    ...
    VOS_UINT32                          ulCid               = NAS_ESM_NULL;
     ...
     NAS_ESM_INFO_LOG("NAS_ESM_SndEsmAppEhrpdSdfRelIndMsg, entry");
     TLPS_PRINT2LAYER_INFO(NAS_ESM_SndEsmAppEhrpdSdfRelIndMsg_ENUM, LNAS_ENTRY);
    ...
    {
        /*打印异常信息*/
        NAS_ESM_ERR_LOG("NAS_ESM_SndEsmAppEhrpdSdfRelIndMsg:ERROR:APP->SM,Memory Alloc FAIL!");
        TLPS_PRINT2LAYER_ERROR(NAS_ESM_SndEsmAppEhrpdSdfRelIndMsg_ENUM, LNAS_NULL_PTR);
        return ;
    }
    ...
    /* 拷贝PDN信息 */
    pSmAppRelInd->bitOpPdnAddr = NAS_ESM_OP_TRUE;
    ...
    pSmAppRelInd->ulEpsbId = pstPdnEntity->stEhPdnContextInfo.ulEpsbId;
