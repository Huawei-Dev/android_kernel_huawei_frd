...
const IMSA_CALL_REL_REASON_TRANS_STRU   g_astImsaCallRelReasonTransTbl[] =
{
   { IMSA_CALL_NO_SRV_CAUSE_HIFI_EXCEPTION,             IMSA_IMS_CALL_REL_CAUSE_HIFI_SET_START_FAIL},
   { IMSA_CALL_NO_SRV_CAUSE_NON_SRVCC_RAT_CHANGE,       IMSA_IMS_CALL_REL_CAUSE_NON_SRVCC_RAT_CHANGE},
   { IMSA_CALL_NO_SRV_CAUSE_NW_NOT_SUPPORT_IMS_VOICE,   IMSA_IMS_CALL_REL_CAUSE_NW_NOT_SUPPORT_IMS_VOICE},
   { IMSA_CALL_NO_SRV_CAUSE_MEDIA_PDP_RELEASE,          IMSA_IMS_CALL_REL_CAUSE_NW_DEACTIVE_MEDIA_BEAR},
   { IMSA_CALL_NO_SRV_CAUSE_SIP_PDP_ERR,                IMSA_IMS_CALL_REL_CAUSE_NW_DEACTIVE_SIP_BEAR},
   { IMSA_CALL_NO_SRV_CAUSE_REG_ERR,                    IMSA_IMS_CALL_REL_CAUSE_IMS_REG_FAIL},
   { IMSA_CALL_NO_SRV_CAUSE_PORT_CFG_FAIL,              IMSA_IMS_CALL_REL_CAUSE_HIFI_PORT_CFG_FAIL},
   { IMSA_CALL_NO_SRV_CAUSE_SET_DEVICE_FAIL,            IMSA_IMS_CALL_REL_CAUSE_HIFI_SET_DEVICE_FAIL},
   { IMSA_CALL_NO_SRV_CAUSE_SET_START_FAIL,             IMSA_IMS_CALL_REL_CAUSE_HIFI_SET_START_FAIL},
   { IMSA_CALL_NO_SRV_CAUSE_SET_VOLUME_FAIL,            IMSA_IMS_CALL_REL_CAUSE_HIFI_SET_VOLUME_FAIL},
   { IMSA_CALL_NO_SRV_CAUSE_SAMPLE_RATE_FAIL,           IMSA_IMS_CALL_REL_CAUSE_HIFI_SAMPLE_RATE_FAIL},
   { IMSA_CALL_NO_SRV_CAUSE_TI_START_EXPIRED,           IMSA_IMS_CALL_REL_CAUSE_HIFI_TI_START_EXPIRED},
   { IMSA_CALL_NO_SRV_CAUSE_AREA_LOST,                  IMSA_IMS_CALL_REL_CAUSE_AREA_LOST},
   { IMSA_CALL_NO_SRV_CAUSE_IMS_EMC_BAR,                IMSA_IMS_CALL_REL_CAUSE_EMC_BAR},
   { IMSA_CALL_NO_SRV_CAUSE_IMS_EMC_NOT_SUPPORT,        IMSA_IMS_CALL_REL_CAUSE_NOT_SUPPORT_EMC_CALL}
};

const VOS_UINT32 g_astImsaCallRelReasonTransTblSize  = sizeof(g_astImsaCallRelReasonTransTbl) / sizeof(g_astImsaCallRelReasonTransTbl[0]);
...
)
{
    ...
    /* 如果定制NV打开，且ACB enabled时，IMSA不发起该呼叫，通知SPM错误原因值 */
    if ((VOS_TRUE == pstCallCtx->ucACBFlag) && (VOS_TRUE == IMSA_LRRC_IsAcbEnabled()))
    {
        return IMSA_CALL_AVAILABILITY_CHECK_RESULT_REJECT;
    }
...
VOS_UINT32 IMSA_CallCtxInit(VOS_VOID)
{
    ...
    pstCallCtx->stVideoPdpLostModifyCallTimer.phTimer     = VOS_NULL_PTR;
    pstCallCtx->stVideoPdpLostModifyCallTimer.ucMode      = VOS_RELTIMER_NOLOOP;
    pstCallCtx->stVideoPdpLostModifyCallTimer.usName      = TI_IMSA_CALL_VEDIO_PDP_LOST_MODIFY_CALL;
    pstCallCtx->stVideoPdpLostModifyCallTimer.ulTimerLen  = IMSA_CALL_TIMER_LEN_VEDIO_BEAR_LOST_MOD_CALL;
...
VOS_UINT32 IMSA_CallCtxDeinit(VOS_VOID)
{
    ...
    if (VOS_TRUE == IMSA_IsTimerRunning(&pstCallCtx->stVideoPdpLostModifyCallTimer))
    {
        IMSA_StopTimer(&pstCallCtx->stVideoPdpLostModifyCallTimer);
    }
...
VOS_VOID IMSA_CallClearInviteUnsendInfo(VOS_VOID)
{
    VOS_UINT32 i = 0;
    IMSA_CALL_MANAGER_STRU *pstCallCtx = IMSA_CallCtxGet();
    ...
    for (i = 0; i < IMSA_CALL_MAX_NUM; i++)
    {
        if (VOS_TRUE == pstCallCtx->astCallEntity[i].bitOpIsUsed)
        {
            pstCallCtx->astCallEntity[i].ucInviteUnSendFlag = VOS_FALSE;
        }
    }
...
)
{
    ...
    {
        IMSA_ClearVideoLostModCallTypeCtrInfo();
    }
}
                                    ...
                                    IMSA_IMS_INPUT_EVENT_STRU *pstInputEvent)
{
    ...
    /* 设置语音加密标识 */
    pstInputEvent->evt.stInputCallEvent.ucCallEncrypt = pSpmParam->ucEncryptFlag;
}
...
)
{
    ...
    if (IMSA_EMC_CALL_TYPE_NO_REG == enEmcType)
    {
        pstInputEvent->evt.stInputCallEvent.ucNoCardEmergencyFlag = VOS_TRUE;
    }
...
)
{
    ...
    for (i = 0; i < IMSA_CALL_MAX_NUM; i++)
    {
            ...
            (IMSA_CALL_TYPE_EMC == pstCallCtx->astCallEntity[i].enType))
        {
            ...
            else if (IMSA_CALL_NO_SRV_CAUSE_SIP_PDP_ERR == enNoSrvCause)
            {
                                          ...
                                          MN_CALL_TYPE_ENUM_UINT8 enCallType)
{
    ...
    /* 与GUNAS倪洪旺确认，如果IMSA直接拒绝SPM的呼叫请求，则需要且仅需要在CALL_INFO中携带呼叫类型 */
    else
    {
        stOrigCnf.stCallInfo.enCallType = enCallType;
    }
...
VOS_VOID IMSA_CallEntityUpdateCallDisplayNameByAscii(const VOS_CHAR *pcAddress,
                                                VOS_CHAR *pacDstDisplayName)
{
    IMSA_INFO_LOG("IMSA_CallEntityUpdateCallDisplayNameByAscii is entered!");
    TLPS_PRINT2LAYER_INFO(IMSA_CallEntityUpdateCallDisplayNameByAscii_ENUM, LNAS_ENTRY);

    if ((VOS_NULL_PTR == pcAddress) ||
        (VOS_NULL_PTR == pacDstDisplayName))
    {
        IMSA_INFO_LOG("IMSA_CallEntityUpdateCallDisplayNameByAscii: null pointer");
        TLPS_PRINT2LAYER_INFO(IMSA_CallEntityUpdateCallDisplayNameByAscii_ENUM, LNAS_NULL_PTR);
        return;
    }

    IMSA_MEM_SET_S( pacDstDisplayName,
                    IMSA_IMS_DISPLAY_NAME_STRING_SZ,
                    0,
                    IMSA_IMS_DISPLAY_NAME_STRING_SZ);

    IMSA_UtilStrNCpy(pacDstDisplayName, pcAddress, IMSA_IMS_DISPLAY_NAME_STRING_SZ);
...
VOS_VOID IMSA_CallCallDisplayNameImsa2CS(const VOS_CHAR *pacSrcDisplayName,
                                    MN_CALL_DISPLAY_NAME_STRU *pstDstDisplayName)
{
    VOS_UINT8                           ucLoop = 0;

    IMSA_INFO_LOG("IMSA_CallCallDisplayNameImsa2CS is entered!");
    TLPS_PRINT2LAYER_INFO(IMSA_CallCallDisplayNameImsa2CS_ENUM, LNAS_ENTRY);

    /* 获取字符串长度，并将IMSA本地维护的Display Name保存到通知SPM消息的结构体中 */
    for (ucLoop = 0; pacSrcDisplayName[ucLoop] != '\0'; ucLoop++)
    {
        /* 长度异常，保护 */
        if (ucLoop >= IMSA_IMS_DISPLAY_NAME_STRING_SZ)
        {
            break;
        }
        /* 保存Display Name */
        pstDstDisplayName->acDisplayName[ucLoop] = pacSrcDisplayName[ucLoop];
    }

    pstDstDisplayName->acDisplayName[ucLoop] = '\0';
    pstDstDisplayName->ucNumLen = ucLoop;
}
                                          ...
                                          MN_CALL_INFO_PARAM_STRU *pstCallInfoParam)
{
    ...
    pstCallInfoParam->ucEncryptFlag = pstCallEntity->ucEncryptFlag;
    ...
    /* 更新Display name */
    IMSA_CallCallDisplayNameImsa2CS(&pstCallEntity->acDisplayName[0], &pstCallInfoParam->stDisplayName);
}
...
)
{
    ...
    pstCallInfo->ucEncryptFlag = pstCallEntity->ucEncryptFlag;
    ...
    /* 更新Display name */
    IMSA_CallCallDisplayNameImsa2CS(&pstCallEntity->acDisplayName[0], &pstCallInfo->stDisplayName);
...
VOS_VOID IMSA_CallProcTimeoutRedialIntervel(const VOS_VOID *pTimerMsg)
{
    ...
    ulResult = IMSA_CallSendImsMsgDial(IMSA_CALL_INVALID_ID, &stCallOrigReq, pstCallEntity->enEmcType);
    if (VOS_TRUE == ulResult)
    {
        /* 启动保护定时器 */
        IMSA_StartTimer(&pstCallCtx->stProctectTimer);
...
VOS_VOID IMSA_CallProcTimeoutNormalTCall(const VOS_VOID *pTimerMsg)
{
    VOS_UINT32 i = 0;
...
VOS_UINT32 IMSA_CallIsCurrentCallInProcessing(VOS_UINT32 ulCallId)
{
    VOS_UINT32 i = 0;
    IMSA_CALL_MANAGER_STRU *pstCallCtx = IMSA_CallCtxGet();

    IMSA_INFO_LOG("IMSA_CallIsCurrentCallInProcessing is entered!");

    for (i = 0; i < IMSA_CALL_MAX_NUM; i++)
    {
        if ((VOS_UINT8)ulCallId == pstCallCtx->astCallEntity[i].ucImscallId)
        {
            if ((IMSA_CALL_STATUS_IDLE != pstCallCtx->astCallEntity[i].enStatus) &&
                (IMSA_CALL_STATUS_HELD != pstCallCtx->astCallEntity[i].enStatus))
            {
...
)
{
    ...
    TAF_CS_CAUSE_ENUM_UINT32            enSpmErr        = IMSA_NULL;
    VOS_UINT8                           i= 0;
    IMSA_EMC_CALL_TYPE_ENUM_UINT32      enEmcType       = IMSA_EMC_CALL_TYPE_BUTT;      /**< 紧急呼类型 */
        ...
        for (i = 0; i < IMSA_CALL_MAX_NUM; i++)
        {
                ...
                (IMSA_CALL_TYPE_EMC == pstCallCtx->astCallEntity[i].enType))
            {
                ...
                IMSA_MEM_CPY_S( &pstCallEntity->stSpmMsg,
                                ...
                                sizeof(IMSA_CALL_SPM_MSG_STRU));
                /*EMC注册失败，更改EmcType*/
                if(IMSA_CALL_NO_SRV_CAUSE_REG_ERR == enNoSrvCause)
                {
                    pstCallEntity->enEmcType = IMSA_EMC_CALL_TYPE_NO_REG;
                }
                enEmcType = pstCallEntity->enEmcType;
                break;
            }
        }

        ulResult = IMSA_CallSendImsMsgDial(IMSA_CALL_INVALID_ID, pstAppReq, enEmcType);
...
VOS_UINT32 IMSA_CallProcIntraMsgEmcCallSrvStatus(const VOS_VOID *pMsg)
{
    ...
    {
        IMSA_CallProcIntraMsgEmcCallSrvStatusWhenNorSrv(pstSrvStatus->enNoSrvCause);
    }
    else
    {
...
VOS_VOID IMSA_ClearVideoLostModCallTypeCtrInfo(VOS_VOID)
{
    IMSA_CALL_MANAGER_STRU             *pstCallCtx = IMSA_CallCtxGet();

    TLPS_PRINT2LAYER_INFO(IMSA_ClearVideoLostModCallTypeCtrInfo_ENUM, LNAS_ENTRY);

    IMSA_StopTimer(&pstCallCtx->stVideoPdpLostModifyCallTimer);
    pstCallCtx->enVideoCallModifySource   = IMSA_VIDEO_CALL_MODIFY_SOURCE_NONE;
    pstCallCtx->enVideoLostModCallTypeSta = IMSA_VIDEO_BEARER_LOST_STATE_NONE;
}


VOS_VOID IMSA_SetVideoCallModifySource(IMSA_VIDEO_CALL_MODIFY_SOURCE_ENUM_UINT8  enVideoCallModifySource)
{
    IMSA_CALL_MANAGER_STRU             *pstCallCtx = IMSA_CallCtxGet();

    TLPS_PRINT2LAYER_INFO1( IMSA_SetVideoCallModifySource_ENUM,
                            LNAS_FUNCTION_LABEL1,
                            pstCallCtx->enVideoCallModifySource);

    pstCallCtx->enVideoCallModifySource = enVideoCallModifySource;
...
VOS_UINT32 IMSA_IsExistVideoCall(VOS_UINT32 *pulIndex)
{
    VOS_UINT16                          i = 0;
    IMSA_CALL_MANAGER_STRU             *pstCallCtx = IMSA_CallCtxGet();

    TLPS_PRINT2LAYER_INFO(IMSA_IsExistVideoCall_ENUM, LNAS_ENTRY);
    ...
    *pulIndex = 0;
    ...
    {
        if ((IMSA_OP_TRUE == pstCallCtx->astCallEntity[i].bitOpIsUsed)
            &&((IMSA_CALL_TYPE_VIDEO == pstCallCtx->astCallEntity[i].enType)
                || (IMSA_CALL_TYPE_VIDEO_TX == pstCallCtx->astCallEntity[i].enType)
                || (IMSA_CALL_TYPE_VIDEO_RX == pstCallCtx->astCallEntity[i].enType)))
        {
            *pulIndex = i;
            TLPS_PRINT2LAYER_INFO1( IMSA_IsExistVideoCall_ENUM,
                                    LNAS_FUNCTION_LABEL1,
                                    i);
            return VOS_TRUE;
        }
...
VOS_UINT32 IMSA_CallSendImsMsgModifyWithCallIndex
(
    VOS_UINT32                          ulIndex,
...
)
{
    ...
    IMSA_INFO_LOG("IMSA_CallSendImsMsgModifyWithCallIndex is entered!");
    TLPS_PRINT2LAYER_INFO(IMSA_CallSendImsMsgModifyWithCallIndex_ENUM, LNAS_ENTRY);

    if ((IMSA_CALL_MAX_NUM <= ulIndex)
        || (VOS_TRUE != pstCallCtx->astCallEntity[ulIndex].bitOpIsUsed))
    {
        return VOS_FALSE;
    }

    pstCallEntity = &pstCallCtx->astCallEntity[ulIndex];

    pstInputEvent = (IMSA_IMS_INPUT_EVENT_STRU *)IMSA_MEM_ALLOC(sizeof(IMSA_IMS_INPUT_EVENT_STRU));
    if (VOS_NULL_PTR == pstInputEvent)
    {
        IMSA_ERR_LOG("IMSA_CallSendImsMsgModifyWithCallIndex: alloc memory fail");
        TLPS_PRINT2LAYER_ERROR(IMSA_CallSendImsMsgModifyWithCallIndex_ENUM, LNAS_NULL_PTR);
        return VOS_FALSE;
    }
    ...
    pstInputEvent->evt.stInputCallEvent.bitOpModify = IMSA_OP_TRUE;
    pstInputEvent->evt.stInputCallEvent.stModify.enSrcCallType = enSrcCallType;
    pstInputEvent->evt.stInputCallEvent.stModify.enDstCallType = enDstCallType;
...
VOS_UINT32 IMSA_CallSndSpmModifyStatusIndWithCallIndex
(
    VOS_UINT32                          ulIndex,
...
)
{
    ...
    IMSA_INFO_LOG("IMSA_CallSndSpmModifyStatusIndWithCallIndex is entered!");
    TLPS_PRINT2LAYER_INFO(IMSA_CallSndSpmModifyStatusIndWithCallIndex_ENUM, LNAS_ENTRY);
    ...
    {
        IMSA_ERR_LOG("IMSA_CallSndSpmModifyStatusIndWithCallIndex: alloc memory fail");
        TLPS_PRINT2LAYER_ERROR(IMSA_CallSndSpmModifyStatusIndWithCallIndex_ENUM, LNAS_NULL_PTR);
        return VOS_FALSE;
    }
    ...
    pstModifyStatusInd->enCurrCallType = enCurrCallType;
    pstModifyStatusInd->enExpectCallType = enExpectCallType;
...
VOS_VOID IMSA_CallProcTimeoutVideoMediaLost(const VOS_VOID *pTimerMsg)
{
    IMSA_CALL_MANAGER_STRU             *pstCallCtx      = IMSA_CallCtxGet();
    VOS_UINT32                          ulIndex         = 0;

    IMSA_INFO_LOG("IMSA_CallProcTimeoutVideoMediaLost is entered!");
    TLPS_PRINT2LAYER_INFO(IMSA_CallProcTimeoutVideoMediaLost_ENUM, LNAS_ENTRY);

    IMSA_StopTimer(&pstCallCtx->stVideoPdpLostModifyCallTimer);

    /* 如果当前存在其他视频降语音流程, 直接返回 */
    if (IMSA_VIDEO_CALL_MODIFY_SOURCE_NONE != pstCallCtx->enVideoCallModifySource)
    {
        IMSA_INFO_LOG1("IMSA_CallProcTimeoutVideoMediaLost modify Source!", pstCallCtx->enVideoCallModifySource);
        TLPS_PRINT2LAYER_INFO(IMSA_CallProcTimeoutVideoMediaLost_ENUM, LNAS_FUNCTION_LABEL1);
        pstCallCtx->enVideoLostModCallTypeSta = IMSA_VIDEO_BEARER_LOST_STATE_NONE;
        return;
    }

    /* 当前不是在等定时器超时 */
    if (IMSA_VIDEO_BEARER_LOST_WAIT_TIMER_OUT != pstCallCtx->enVideoLostModCallTypeSta)
    {
        IMSA_INFO_LOG1("IMSA_CallProcTimeoutVideoMediaLost modify state!", pstCallCtx->enVideoLostModCallTypeSta);
        TLPS_PRINT2LAYER_INFO(IMSA_CallProcTimeoutVideoMediaLost_ENUM, LNAS_FUNCTION_LABEL2);
        pstCallCtx->enVideoLostModCallTypeSta = IMSA_VIDEO_BEARER_LOST_STATE_NONE;
        return;
    }
    ...
    {
        IMSA_INFO_LOG("IMSA_CallProcTimeoutVideoMediaLost Curr Srvcc!");
        TLPS_PRINT2LAYER_INFO(IMSA_CallProcTimeoutVideoMediaLost_ENUM, LNAS_FUNCTION_LABEL3);
        pstCallCtx->enVideoLostModCallTypeSta = IMSA_VIDEO_BEARER_LOST_STATE_NONE;
        return;
    }

    /* 判断是否有视频承载存在，在定时器启动期间，网侧可能会新建承载； */
    if (IMSA_TRUE == IMSA_CONN_HasActiveVideoPdp())
    {
        IMSA_INFO_LOG("IMSA_CallProcTimeoutVideoMediaLost exit Video Bearer!");
        TLPS_PRINT2LAYER_INFO(IMSA_CallProcTimeoutVideoMediaLost_ENUM, LNAS_FUNCTION_LABEL4);
        pstCallCtx->enVideoLostModCallTypeSta = IMSA_VIDEO_BEARER_LOST_STATE_NONE;
        return;
    }

    /* 判断当前是否存在视频电话 */
    if (IMSA_FALSE == IMSA_IsExistVideoCall(&ulIndex))
    {
        IMSA_INFO_LOG("IMSA_CallProcTimeoutVideoMediaLost Don't exit Video Call!");
        TLPS_PRINT2LAYER_INFO(IMSA_CallProcTimeoutVideoMediaLost_ENUM, LNAS_FUNCTION_LABEL5);
        pstCallCtx->enVideoLostModCallTypeSta = IMSA_VIDEO_BEARER_LOST_STATE_NONE;
        return;
    }

    /* 当语音承载被释放挂断电话定时器正在运行时, 返回 */
    if (VOS_TRUE == IMSA_IsTimerRunning(&pstCallCtx->stNormMediaLostRelCallTimer))
    {
        IMSA_INFO_LOG("IMSA_CallProcTimeoutVideoMediaLost MediaLost Timer Running");
        TLPS_PRINT2LAYER_INFO(IMSA_CallProcTimeoutVideoMediaLost_ENUM, LNAS_FUNCTION_LABEL6);
        pstCallCtx->enVideoLostModCallTypeSta = IMSA_VIDEO_BEARER_LOST_STATE_NONE;
        return;
    }

    /* 修改modify源为IMSA, 并设置为等待IMS回复OK */
    pstCallCtx->enVideoCallModifySource   = IMSA_VIDEO_CALL_MODIFY_SOURCE_IMSA;
    pstCallCtx->enVideoLostModCallTypeSta = IMSA_VIDEO_BEARER_LOST_WAIT_IMS_OK;

    /* 通知IMS视频降语音 */
    (VOS_VOID)IMSA_CallSendImsMsgModifyWithCallIndex(ulIndex, IMSA_IMS_CALL_MODE_VIDEO, IMSA_IMS_CALL_MODE_VOICE);
}


VOS_VOID IMSA_CallProcNrmCallNrmSrvStatus(IMSA_NRM_CALL_SRV_STATUS_IND_STRU *pstSrvStatus)
{
    VOS_UINT32                          ulVideoCallIndex    = 0;
    IMSA_CALL_MANAGER_STRU             *pstCallCtx          = IMSA_CallCtxGet();

    TLPS_PRINT2LAYER_INFO1(IMSA_CallProcNrmCallNrmSrvStatus_ENUM, LNAS_ENTRY, pstSrvStatus->enNormSrvExtendInfo);

    /* 携带的正常服务扩展信息为视频承载丢失 */
    if (IMSA_CALL_NORM_SRV_EXTEND_INFO_VIDEO_PDP_LOST == pstSrvStatus->enNormSrvExtendInfo)
    {
        /* 当前存在视频电话 */
        if ((IMSA_TRUE == IMSA_IsExistVideoCall(&ulVideoCallIndex)) && ((IMSA_FALSE == IMSA_CONN_HasActiveVideoPdp())))
        {
            TLPS_PRINT2LAYER_INFO1(IMSA_CallProcNrmCallNrmSrvStatus_ENUM, LNAS_FUNCTION_LABEL1, pstCallCtx->enVideoCallModifySource);

            /* 当前没有进行视频降语音流程 */
            if (IMSA_VIDEO_CALL_MODIFY_SOURCE_NONE == pstCallCtx->enVideoCallModifySource)
            {
                /* 启动定时器, 超时后把视频电话降为语音电话 */
                pstCallCtx->enVideoLostModCallTypeSta = IMSA_VIDEO_BEARER_LOST_WAIT_TIMER_OUT;
                IMSA_StartTimer(&pstCallCtx->stVideoPdpLostModifyCallTimer);
            }
        }
    }
...
VOS_UINT32 IMSA_CallProcIntraMsgNrmCallSrvStatus(const VOS_VOID *pMsg)
{
    ...
    {
        /* 如果普通服务可用，根据扩展信息进行处理 */
        IMSA_CallProcNrmCallNrmSrvStatus(pstSrvStatus);
    }
    else
    {
...
VOS_VOID IMSA_ProcCallSrvccSatusNoify(const VOS_VOID *pRcvMsg)
{
    ...
    else /*CALL_IMSA_SRVCC_STATUS_FAIL*/
    {
        ...
        /*清除invite消息未发送到网侧的标识 */
        IMSA_CallClearInviteUnsendInfo();
...
)
{
    ...
    ulResult = IMSA_CallSendImsMsgDial(IMSA_CALL_INVALID_ID, pstAppReq, pstCallEntity->enEmcType);
...
)
{
    ...
    if (VOS_TRUE == ulResult)
    {
        ...
        {
            IMSA_SndRrcVolteStatusNotify(IMSA_LRRC_VOLTE_CALL_TYPE_MO, IMSA_LRRC_VOLTE_STATUS_START);
        }
    }
    else
    {
...
VOS_UINT32 IMSA_IsImsCallTransactionExist(VOS_VOID)
{
...
VOS_UINT32 IMSA_ImsCallStateNtfInHo(VOS_VOID)
{

    VOS_UINT16                          i = 0;
    IMSA_CALL_MANAGER_STRU             *pstCallCtx = IMSA_CallCtxGet();
    MN_CALL_EVENT_ENUM_UINT32          enMnCallEvent;
    ...
    for (i = 0; i < IMSA_CALL_MAX_NUM; i ++)
    {
        if ((IMSA_OP_TRUE == pstCallCtx->astCallEntity[i].bitOpIsUsed)
            &&(IMSA_CALL_STATUS_IDLE  != pstCallCtx->astCallEntity[i].enStatus))
        {
            switch( pstCallCtx->astCallEntity[i].enStatus)
            {
                case IMSA_CALL_STATUS_DIALING:
                    enMnCallEvent = MN_CALL_EVT_ORIG;
                    break;
                case IMSA_CALL_STATUS_TRYING:
                    enMnCallEvent = MN_CALL_EVT_CALL_PROC;
                    break;
                case IMSA_CALL_STATUS_ALERTING:
                    enMnCallEvent = MN_CALL_EVT_ALERTING;
                    break;
                case IMSA_CALL_STATUS_ACTIVE:
                    enMnCallEvent = MN_CALL_EVT_CONNECT;
                    break;
                case IMSA_CALL_STATUS_INCOMING:
                    enMnCallEvent = MN_CALL_EVT_INCOMING;
                    break;
                case IMSA_CALL_STATUS_HELD:
                    enMnCallEvent = MN_CALL_EVT_HOLD;
                    break;
                case IMSA_CALL_STATUS_WAITING:
                    enMnCallEvent = MN_CALL_EVT_INCOMING;
                    break;
            ...
            (VOS_VOID)IMSA_CallSendSpmStateEvt( pstCallCtx->astCallEntity[i].ucSpmcallId,
                                                pstCallCtx->astCallEntity[i].usClientId,
                                                pstCallCtx->astCallEntity[i].ulOpId,
                                                enMnCallEvent);
        }
    }
...
{
    VOS_UINT32                          i;
    IMSA_IMS_CALL_REL_CAUSE_ENUM_UINT8  enImsErr = IMSA_IMS_CALL_REL_CAUSE_BUTT;

    /* 遍历列表，找到入参对应的原因值 */
    for (i = 0 ; i < g_astImsaCallRelReasonTransTblSize ; i ++)
    {
        /* 如果找到，则返回对应的IMS原因值 */
        if (enImsaErr == g_astImsaCallRelReasonTransTbl[i].enImsaErr)
        {
            enImsErr = g_astImsaCallRelReasonTransTbl[i].enImsErr;
            break;
        }
    }
...
)
{
    switch(enImsaErr)
    {
        ...
        case IMSA_CALL_NO_SRV_CAUSE_AREA_LOST:
            return IMSA_ERR_LOG_NORM_CALL_FAIL_REASON_AREA_LOST;
        default:
...
VOS_UINT32 IMSA_IsVoiceCallWaitResReady(VOS_VOID)
{
    VOS_UINT32                          i;
    IMSA_CALL_MANAGER_STRU             *pstCallCtx = IMSA_CallCtxGet();
    ...
    for (i = 0; i < IMSA_CALL_MAX_NUM; i++)
    {
        if ((VOS_TRUE == pstCallCtx->astCallEntity[i].bitOpIsUsed)
            && (VOS_TRUE == pstCallCtx->astCallEntity[i].bitOpNeedSendResResult)
            && (IMSA_CALL_TYPE_VOICE == pstCallCtx->astCallEntity[i].enType)
            && (VOS_FALSE == pstCallCtx->ucVoiceBearExistFlag))
        {
            return IMSA_TRUE;
        }
