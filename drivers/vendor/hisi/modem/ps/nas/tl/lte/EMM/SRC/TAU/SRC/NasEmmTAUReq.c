                                            ...
                                            VOS_VOID   *pMsgStru  )
{
    ...
    /* 协议24301中第5.3.5章节指出在EMM-REGISTERED.NO-CELL-AVAILABLE/PLMN-SEARCH
       /UPDATE-NEEDED/LIMITED-SERVICE态下, T3412定时器超时, 若当前ISR激活, 需要
       启动T3423定时器,而挂起态是实现特有状态而不是协议状态 */
...
VOS_UINT32 NAS_EMM_MsRegSsSomeMsgRrmPsStatusInd( VOS_UINT32  ulMsgId,
                                                    VOS_VOID   *pMsgStru )
{
    RRM_PS_STATUS_IND_STRU             *pstRrmPsStatusInd = NAS_EMM_NULL_PTR;

    NAS_LMM_PUBM_LOG_INFO("NAS_EMM_MsRegSsSomeMsgRrmPsStatusInd Enter.");
    TLPS_PRINT2LAYER_INFO(NAS_EMM_MsRegSsSomeMsgRrmPsStatusInd_ENUM, LNAS_ENTRY);
    ...
    {
        /*如果nv没有打开，则不处理*/
        NAS_LMM_PUBM_LOG_INFO("NAS_EMM_MsRegSsSomeMsgRrmPsStatusInd: DSDS2.0 feature is close");
        TLPS_PRINT2LAYER_INFO(NAS_EMM_MsRegSsSomeMsgRrmPsStatusInd_ENUM, NAS_LMM_DsdsFeatureIsClose);
        return NAS_LMM_MSG_HANDLED;
    }

    if(RRM_PS_TASK_TYPE_LTE_PS_HIGH_TAU != pstRrmPsStatusInd->enTaskType)
    {
        /*如果不是TAU类型的资源恢复，则直接返回*/
        return NAS_LMM_MSG_HANDLED;
    }
    ...
    switch(NAS_EMM_CUR_SUB_STAT)
    {
        ...
        case EMM_SS_REG_ATTEMPTING_TO_UPDATE:
             {
                NAS_LMM_PUBM_LOG_INFO("NAS_EMM_MsRegSsSomeMsgRrmPsStatusInd: RF recover, start TAU!");
                TLPS_PRINT2LAYER_INFO(NAS_EMM_MsRegSsSomeMsgRrmPsStatusInd_ENUM, NAS_LMM_RfRecoverStartTau);

                NAS_EMM_TAU_StartTAUREQ(NAS_LMM_OM_TAU_START_TYPE_RF_RECOVER);
             }

        break;

        default:
                NAS_LMM_PUBM_LOG_INFO("NAS_EMM_MsRegSsSomeMsgRrmPsStatusInd: Status do not handle!");
                TLPS_PRINT2LAYER_INFO(NAS_EMM_MsRegSsSomeMsgRrmPsStatusInd_ENUM, NAS_LMM_StatusDoNotHandle);
            break;
    }
