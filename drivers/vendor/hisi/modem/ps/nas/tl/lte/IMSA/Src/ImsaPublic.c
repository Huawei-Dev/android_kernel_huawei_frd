...
VOS_UINT32   g_ulImsaIgnoreMediaBearerQci        = IMSA_FALSE;
...
extern VOS_UINT32  IMSA_DecodeIsimSipUri(const IMSA_ISIM_DATA_STRU *pstIsimData);
...
IMSA_ISIM_ACT_STRU g_astIsimDataMap[] =
{
    ...
    {USIMM_TELE_EFPSISMSC_ID, IMSA_DecodeIsimSipUri,    IMSA_EncodeIsimFile}
};
...
NAS_COMM_PRINT_MSG_LIST_STRU g_astImsaMsccMsgIdArray[] =
{
    ...
    {   ID_MSCC_IMSA_IMS_DOMAIN_CFG_SET_REQ                    ,
            "MSG:  ID_MSCC_IMSA_IMS_DOMAIN_CFG_SET_REQ                  "},
...
NAS_COMM_PRINT_LIST_STRU g_astImsaAtMsgIdArray[] =
{
...

    {
        ID_AT_IMSA_IMS_REG_DOMAIN_QRY_REQ           ,
            "MSG:  ID_AT_IMSA_IMS_REG_DOMAIN_QRY_REQ                              ",
        VOS_NULL_PTR},
    {
        ID_AT_IMSA_IMS_CTRL_MSG         ,
            "MSG:  ID_AT_IMSA_IMS_CTRL_MSG                              ",
        VOS_NULL_PTR},
...
VOS_VOID IMSA_CheckIsimFileAvailable
(
    IMSA_ISIM_DATA_STRU                *pstIsimData,
    VOS_UINT8                          *pucCardFileErrDeregFlag
)
{
    VOS_UINT16                          i;
    VOS_UINT16                          usDataLen;

    /* 文件总长度小于等于2或长度为0，认为文件为空 */
    if ((2 >= pstIsimData->usDataLen)
        || (0 == pstIsimData->pData[1]))
    {
        TLPS_PRINT2LAYER_WARNING(IMSA_CheckIsimFileAvailable_ENUM, 1);
        *pucCardFileErrDeregFlag      = VOS_TRUE;
        return;
    }

    /* 如果文件长度大于总长度-2，则认为文件长度非法，将其置为总长度-2 */
    usDataLen = pstIsimData->pData[1];
    if ((pstIsimData->usDataLen - 2) < usDataLen)
    {
        usDataLen = (pstIsimData->usDataLen - 2);
    }

    /* 文件内容为全0或全F，认为文件为空 */
    for (i = 2 ; i < (usDataLen + 2) ; i ++)
    {
        if ((0xFF != pstIsimData->pData[i]) && (0 != pstIsimData->pData[i]))
        {
            break;
        }
    }
    if (i == usDataLen + 2)
    {
        TLPS_PRINT2LAYER_WARNING(IMSA_CheckIsimFileAvailable_ENUM, 2);
        *pucCardFileErrDeregFlag      = VOS_TRUE;
        return;
    }
    return;
...
VOS_UINT32 IMSA_Domain_ProcReadIsimFileResult(VOS_VOID *pRcvMsg)
{
    ...
    VOS_UINT32                      ulresult;
    /* 记录是否需要发起去注册流程。默认不发起 */
    VOS_UINT8                       ucCardFileErrDeregFlag = VOS_FALSE;
    ...
    {
        ucCardFileErrDeregFlag = VOS_TRUE;
        /* 如果返回的结果为失败，则输出错误信息 */
    ...
    /* 判断文件是否为空，如果为空，直接发起去注册流程 */
    IMSA_CheckIsimFileAvailable(&stIsimData, &ucCardFileErrDeregFlag);

    if (VOS_TRUE == ucCardFileErrDeregFlag)
    {
        /*此处DOMAIN模块不处理，后面给SERVICE模块处理*/
        return VOS_OK;
    }
    ...
    if (VOS_NULL_PTR != pActFun)
    {
        ...
        if ((IMSA_STATUS_STARTED == pstControlManager->enImsaStatus )
            ...
            /* 如果读到的文件时IMSA模块自己使用的，则不需要给IMS协议栈配置参数 */
            && (USIMM_USIM_EFAD_ID != enDefFileID)
            && (USIMM_TELE_EFPSISMSC_ID != enDefFileID))
        {
...
VOS_UINT32 IMSA_ProcReadIsimFileResult(VOS_VOID *pRcvMsg)
{
    ...
    if (VOS_NULL_PTR != pActFun)
    {
            ...
            && (USIMM_TELE_EFPSISMSC_ID != enDefFileID))
        {
            ...
            /* 如果所有的文件都已经读取完成，则根据当前状态决定是否要重新发起注册 */
            IMSA_SRV_RegProcReceivedUsimReadfileCnf();
        }
...
VOS_UINT32  IMSA_DecodeIsimSipUri(const IMSA_ISIM_DATA_STRU *pstIsimData)
{
    IMSA_SMS_SIP_URI_STRU               *pstImsaSipUri;
    VOS_UINT8                            ucFileLen;

    pstImsaSipUri  = IMSA_SMS_GetSipUriEntityAddress();

    /*如果NV配置关闭，则无需解码。*/
    if (VOS_TRUE != pstImsaSipUri->ucNvReadSipUriFlag)
    {
        pstImsaSipUri->ucReadSipUriFlag = VOS_FALSE;
        IMSA_INFO_LOG1("IMSA_DecodeIsimSipUri:tag = ", pstIsimData->pData[0]);
        TLPS_PRINT2LAYER_INFO1(IMSA_DecodeIsimSipUri_ENUM, IMSA_NV_IMS, 0);
        return VOS_TRUE;
    }

    /*31.102 4.5.9描述卡中PSISMSC的格式为TLV，tag = '80'*/
    if( 0x80 != pstIsimData->pData[0])
    {
        IMSA_INFO_LOG1("IMSA_DecodeIsimSipUri:tag = ", pstIsimData->pData[0]);
        TLPS_PRINT2LAYER_INFO1(IMSA_DecodeIsimSipUri_ENUM, IMSA_ISIM_TAG, pstIsimData->pData[0]);
    }

    ucFileLen = pstIsimData->pData[1];

    /*长度不合法，则设置为最大值*/
    if(ucFileLen > IMSA_MAX_SIPURI_LEN)
    {
        IMSA_INFO_LOG1("IMSA_DecodeIsimSipUri:lenth = ", ucFileLen);
        TLPS_PRINT2LAYER_INFO1(IMSA_DecodeIsimSipUri_ENUM, IMSA_ISIM_LENGTH, ucFileLen);

        ucFileLen = IMSA_MAX_SIPURI_LEN;
    }


    if (VOS_FALSE == pstImsaSipUri->ucReadSipUriFlag)
    {
        pstImsaSipUri->ucReadSipUriFlag = VOS_TRUE;

        IMSA_MEM_SET_S( pstImsaSipUri->acSipUri,
                        IMSA_MAX_SIPURI_LEN +1,
                        0,
                        IMSA_MAX_SIPURI_LEN +1);

        IMSA_MEM_CPY_S( pstImsaSipUri->acSipUri,
                        IMSA_MAX_SIPURI_LEN +1,
                        &pstIsimData->pData[2],
                        ucFileLen);
    }
...
VOS_VOID  IMSA_ReadImsaNvImsRatSupportConfig( VOS_VOID)
{
    ...
    {
        /*根据NV内容设置*/
        pstControlManager->stImsaConfigPara.ucWifiImsSupportFlag    = stNvImsRatSupport.ucWifiImsSupportFlag;
        pstControlManager->stImsaConfigPara.ucWifiEmsSupportFlag    = stNvImsRatSupport.ucWifiEmsSupportFlag;
        ...
        pstControlManager->stImsaConfigPara.ucVolteCtrlVowifiFlag   = stNvImsRatSupport.ucVolteCtrlVowifiFlag;
        IMSA_SndOmRatSupportNvInfo(&stNvImsRatSupport);
    }
    else
    {
        ...
        pstControlManager->stImsaConfigPara.ucWifiEmsSupportFlag    = VOS_FALSE;
        ...
        pstControlManager->stImsaConfigPara.ucVolteCtrlVowifiFlag   = VOS_FALSE;
        pstControlManager->stVowifiPara.ucVolteCtrlVowifiFlag       = VOS_FALSE;
        IMSA_SndOmRatSupportNvInfo(VOS_NULL_PTR);
    }

}


VOS_VOID  IMSA_ReadImsaNvWifiParaConfig( VOS_VOID)
{
    IMSA_CONTROL_MANAGER_STRU      *pstControlManager;
    IMSA_DOMAIN_MANAGER_STRU            *pstDomainManager = VOS_NULL_PTR;
    IMSA_VOWIFI_PARA_CONFIG_STRU    stNvImsWifiParaConfig;
    VOS_UINT32                      ulRslt;
    ...
    ulRslt = IMSA_NV_Read(EN_NV_ID_VOWIFI_PARA_CONFIG,&stNvImsWifiParaConfig,\
                 sizeof(IMSA_VOWIFI_PARA_CONFIG_STRU));

    if(ulRslt == VOS_OK)
    {
        /*根据NV内容设置*/
        pstControlManager->stVowifiPara.ucHoSupportInNonActSrvFlag  = stNvImsWifiParaConfig.ucHoSupportInNonActSrvFlag;
        pstControlManager->stVowifiPara.ucW2LHoInActSrvFlag         = stNvImsWifiParaConfig.ucW2LHoInActSrvFlag;
        pstControlManager->stVowifiPara.ucL2WHoInActSrvFlag         = stNvImsWifiParaConfig.ucL2WHoInActSrvFlag;
        pstControlManager->stVowifiPara.ucHoSupportInUnstableSrvFlag  = stNvImsWifiParaConfig.ucHoSupportInUnstableSrvFlag;
        pstControlManager->stVowifiPara.ucL2WHoDelayInrSRVCCFlag    = stNvImsWifiParaConfig.ucL2WHoDelayInrSRVCCFlag;
        pstDomainManager->ucInitTimer1Len                           = stNvImsWifiParaConfig.ucInitTimer1Len;
        pstDomainManager->ucMaxTimer1Len                            = stNvImsWifiParaConfig.ucMaxTimer1Len;
    }
    else
    {
        IMSA_WARN_LOG("IMSA_ReadImsaNvImsRatSupportConfig: read NV err!");
        ...
        /*设置初始值*/
        pstControlManager->stVowifiPara.ucHoSupportInNonActSrvFlag  = VOS_FALSE;
        pstControlManager->stVowifiPara.ucW2LHoInActSrvFlag         = VOS_FALSE;
        pstControlManager->stVowifiPara.ucL2WHoInActSrvFlag         = VOS_FALSE;
        pstControlManager->stVowifiPara.ucHoSupportInUnstableSrvFlag  = VOS_FALSE;
        pstControlManager->stVowifiPara.ucL2WHoDelayInrSRVCCFlag    = VOS_FALSE;
        pstDomainManager->ucInitTimer1Len = 30;   /* 默认初始定时器时长为30 */
        pstDomainManager->ucMaxTimer1Len = 60;    /* 默认最长60分钟 */
    }
...
VOS_VOID IMSA_InitNonRegRejCauseListWithNvConfig(IMSA_COMM_PARA_CONFIG_STRU  *pstNvImsaConfig)
{
    IMSA_NV_NON_REG_REJ_CAUSE_LIST_STRU     *pstNonRegRejCauseList  = VOS_NULL_PTR;
    IMSA_NON_REG_REJ_CTRL_STRU              *pstNonRegRejCtrl       = VOS_NULL_PTR;
    VOS_UINT32                               i                      = 0;

    pstNonRegRejCtrl  = IMSA_CallGetNonRegRejCtrlAddr();

    /* 初始化 */
    IMSA_MEM_SET_S( pstNonRegRejCtrl,
                    sizeof(IMSA_NON_REG_REJ_CTRL_STRU),
                    0,
                    sizeof(IMSA_NON_REG_REJ_CTRL_STRU));

    /* 初始化非注册被拒特性控制开关 */
    if (0 != pstNvImsaConfig->stImsaCtrlBit.bitOpNonRegRejFeatrueCtrl)
    {
        pstNonRegRejCtrl->ucNonRegRejFeatureFlag = IMSA_TRUE;
    }
    pstNonRegRejCauseList = IMSA_CallGetNonRegRejCauseListAddr();

    /* 保存非注册被拒原因值个数 */
    pstNonRegRejCauseList->ucSingleCauseNum = pstNvImsaConfig->stNonRegRejCauseList.ucSingleCauseNum;

    /* 循环保存非注册被拒原因值 */
    if (IMSA_NV_MAX_NON_REG_SINGLE_CAUSE_NUM < pstNonRegRejCauseList->ucSingleCauseNum)
    {
        pstNonRegRejCauseList->ucSingleCauseNum = IMSA_NV_MAX_NON_REG_SINGLE_CAUSE_NUM;
    }

    for (i = 0; i < pstNonRegRejCauseList->ucSingleCauseNum; i++)
    {
        pstNonRegRejCauseList->ausSingleCauseList[i] = pstNvImsaConfig->stNonRegRejCauseList.ausSingleCauseList[i];
    }

    return;
}

VOS_VOID IMSA_InitFeatureCtrlFlagWithNvConfig(IMSA_COMM_PARA_CONFIG_STRU  *pstNvImsaConfig)
{
    IMSA_CONTROL_MANAGER_STRU          *pstControlManager;
    IMSA_CALL_MANAGER_STRU             *pstCallCtx;
    IMSA_SMS_SIP_URI_STRU              *pstImsaSipUri;

    pstControlManager = IMSA_GetControlManagerAddress();
    pstCallCtx        = IMSA_CallCtxGet();
    pstImsaSipUri     = IMSA_SMS_GetSipUriEntityAddress();

    /* 读取ACB Enabled时CSFB开关 */
    if (IMSA_OP_FALSE != pstNvImsaConfig->stImsaCtrlBit.bitOpACBFlag)
    {
        pstCallCtx->ucACBFlag              = VOS_TRUE;
    }
    if (IMSA_OP_FALSE != pstNvImsaConfig->stImsaCtrlBit.bitOpReadSipUriFromIsimFlag)
    {
        pstImsaSipUri->ucNvReadSipUriFlag = VOS_TRUE;
    }

    /* 使用NV配置初始化丢网后是否挂断电话的开关 */
    if (IMSA_OP_TRUE == pstNvImsaConfig->stImsaCtrlBit.bitOpAreaLostNotRelCall)
    {
        pstControlManager->ucAreaLostNotRelCallFlag = IMSA_TRUE;
    }
    else
    {
        pstControlManager->ucAreaLostNotRelCallFlag = IMSA_FALSE;
    }

    if (IMSA_OP_TRUE == pstNvImsaConfig->stImsaCtrlBit.bitOpNonSrvccNotRelCall)
    {
    ...
    /* 使用NV配置的网侧不支持VOICE时，是否发起IMS注册的开关 */
    if (IMSA_OP_TRUE == pstNvImsaConfig->stImsaCtrlBit.bitOpUeSmsCapInitRegFlag)
    {
        pstControlManager->ucUeImsSmsCapInitRegFlag = IMSA_TRUE;
    }
    else
    {
        pstControlManager->ucUeImsSmsCapInitRegFlag = IMSA_FALSE;
    }
}


VOS_VOID IMSA_InitFeatureFlagWithNvConfig(IMSA_COMM_PARA_CONFIG_STRU  *pstNvImsaConfig)
{
    IMSA_REG_MANAGER_STRU               *pstRegCtx              = IMSA_RegCtxGet();


    /* 初始化网侧notify中带deactive去激活UE后，是否需要重新发起注册的开关 */
    if (0 != pstNvImsaConfig->stImsaCtrlBit.bitOpNwDeactiveReReg)
    {
        pstRegCtx->ucNwDeactiveReRegCtrl = IMSA_TRUE;
    }

    /* 初始化网侧notify中带REJECT去激活UE后，是否需要重新发起注册的开关 */
    if (0 != pstNvImsaConfig->stImsaCtrlBit.bitOpNwRejReReg)
    {
        pstRegCtx->ucNwRejReRegCtrl = IMSA_TRUE;
    }
...
VOS_VOID IMSA_InitNonRegRejCauseListWithDefaultConfig(VOS_VOID)
{
    IMSA_NV_NON_REG_REJ_CAUSE_LIST_STRU     *pstNonRegRejCauseList  = VOS_NULL_PTR;
    IMSA_NON_REG_REJ_CTRL_STRU              *pstNonRegRejCtrl       = VOS_NULL_PTR;
    ...
    /* 非注册被拒原因值列表默认值初始化 */
    pstNonRegRejCtrl->ucNonRegRejFeatureFlag     = IMSA_FALSE;

    pstNonRegRejCauseList = IMSA_CallGetNonRegRejCauseListAddr();
    pstNonRegRejCauseList->ucSingleCauseNum = 3;
    pstNonRegRejCauseList->ausSingleCauseList[0]  = 3;
    pstNonRegRejCauseList->ausSingleCauseList[1]  = 403;
    pstNonRegRejCauseList->ausSingleCauseList[2]  = 504;
}


VOS_VOID IMSA_ModifyTimerLenWithCommNvConfig(IMSA_COMM_PARA_CONFIG_STRU *pstNvImsaConfig)
{
    IMSA_SMS_SMR_MO_STRU                *pstSmrMoEntity = IMSA_SMS_GetSmrMoEntityAddress();
    IMSA_SMS_SMR_MT_STRU                *pstSmrMtEntity = IMSA_SMS_GetSmrMtEntityAddress();
    IMSA_CALL_MANAGER_STRU              *pstCallCtx     = IMSA_CallCtxGet();
    IMSA_REG_MANAGER_STRU               *pstRegCtx              = IMSA_RegCtxGet();

    TLPS_PRINT2LAYER_INFO(IMSA_ModifyTimerLenWithCommNvConfig_ENUM, LNAS_ENTRY);

    /* NV设置时长大于0时修改TR1M定时器时长 */
    if (0 < pstNvImsaConfig->usSmsTr1mTimerLen)
    {
        pstSmrMoEntity->stTR1MTimer.ulTimerLen = pstNvImsaConfig->usSmsTr1mTimerLen * 1000;

        TLPS_PRINT2LAYER_INFO1( IMSA_ModifyTimerLenWithCommNvConfig_ENUM,
                                LNAS_FUNCTION_LABEL1,
                                pstSmrMoEntity->stTR1MTimer.ulTimerLen);
    }

    /* NV设置时长大于0时修改TRAM定时器时长 */
    if (0 < pstNvImsaConfig->usSmsTramTimerLen)
    {
        pstSmrMoEntity->stTRAMTimer.ulTimerLen = pstNvImsaConfig->usSmsTramTimerLen * 1000;

        TLPS_PRINT2LAYER_INFO1( IMSA_ModifyTimerLenWithCommNvConfig_ENUM,
                                LNAS_FUNCTION_LABEL2,
                                pstSmrMoEntity->stTRAMTimer.ulTimerLen);
    }

    /* NV设置时长大于0时修改TR2M定时器时长 */
    if (0 < pstNvImsaConfig->usSmsTr2mTimerLen)
    {
        pstSmrMtEntity->stTimerInfo.ulTimerLen = pstNvImsaConfig->usSmsTr2mTimerLen * 1000;

        TLPS_PRINT2LAYER_INFO1( IMSA_ModifyTimerLenWithCommNvConfig_ENUM,
                                LNAS_FUNCTION_LABEL3,
                                pstSmrMtEntity->stTimerInfo.ulTimerLen);
    }

    /* NV设置时长大于0时修改普通媒体承载被释放挂断电话定时器时长 */
    if (0 < pstNvImsaConfig->usNormMediaLostRelCallTimerLen)
    {
        pstCallCtx->stNormMediaLostRelCallTimer.ulTimerLen = pstNvImsaConfig->usNormMediaLostRelCallTimerLen * 1000;

        TLPS_PRINT2LAYER_INFO1( IMSA_ModifyTimerLenWithCommNvConfig_ENUM,
                                LNAS_FUNCTION_LABEL4,
                                pstCallCtx->stNormMediaLostRelCallTimer.ulTimerLen);
    }

    /* NV设置时长大于0时修改紧急媒体承载被释放挂断电话定时器时长 */
    if (0 < pstNvImsaConfig->usEmcMediaLostRelCallTimerLen)
    {
        pstCallCtx->stEmcMediaLostRelCallTimer.ulTimerLen  = pstNvImsaConfig->usEmcMediaLostRelCallTimerLen * 1000;

        TLPS_PRINT2LAYER_INFO1( IMSA_ModifyTimerLenWithCommNvConfig_ENUM,
                                LNAS_FUNCTION_LABEL5,
                                pstCallCtx->stEmcMediaLostRelCallTimer.ulTimerLen);
    }

    /* NV设置时长大于0时修改视频媒体承载被释放视频降语音定时器时长 */
    if (0 < pstNvImsaConfig->usVideoMediaLostMdyCallTimerLen)
    {
        pstCallCtx->stVideoPdpLostModifyCallTimer.ulTimerLen = pstNvImsaConfig->usVideoMediaLostMdyCallTimerLen * 1000;

        TLPS_PRINT2LAYER_INFO1( IMSA_ModifyTimerLenWithCommNvConfig_ENUM,
                                LNAS_FUNCTION_LABEL6,
                                pstCallCtx->stVideoPdpLostModifyCallTimer.ulTimerLen);
    }
    if (0 < pstNvImsaConfig->usNwDeactReRegTimerLen)
    {
        pstRegCtx->ulNwDeactReRegTimerLen = pstNvImsaConfig->usNwDeactReRegTimerLen * 1000;

        TLPS_PRINT2LAYER_INFO1( IMSA_ModifyTimerLenWithCommNvConfig_ENUM,
                                LNAS_FUNCTION_LABEL7,
                                pstRegCtx->ulNwDeactReRegTimerLen);
    }
...
VOS_VOID IMSA_SetNicMtuLength(VOS_UINT32 ulMtuLen)
{
    TLPS_PRINT2LAYER_WARNING1(IMSA_SetNicMtuLength_ENUM, LNAS_ENTRY, ulMtuLen);

    /* NV配置为0或者大于最大值时, 默认设置为初始值 */
    if ((0 == ulMtuLen) || (IMSA_MTU_MAX_LENGTH < ulMtuLen))
    {
        TLPS_PRINT2LAYER_WARNING(IMSA_SetNicMtuLength_ENUM, LNAS_FUNCTION_LABEL1);
        IMS_NIC_MtuSet(IMSA_MTU_DEFAULT_LENGTH);
        return;
    }

    IMS_NIC_MtuSet(ulMtuLen);
    return;
}

VOS_VOID IMSA_SetIgnoreMediaBearerQciFlg(VOS_UINT32 ulOpbit)
{
    if (IMSA_OP_TRUE == ulOpbit)
    {
        g_ulImsaIgnoreMediaBearerQci = IMSA_TRUE;
    }
    else
    {
        g_ulImsaIgnoreMediaBearerQci = IMSA_FALSE;
    }

    TLPS_PRINT2LAYER_INFO1(IMSA_SetIgnoreMediaBearerQciFlg_ENUM, LNAS_ENTRY, g_ulImsaIgnoreMediaBearerQci);
}


VOS_UINT32 IMSA_GetIgnoreMediaBearerQciFlg(VOS_VOID)
{
    TLPS_PRINT2LAYER_INFO1(IMSA_GetIgnoreMediaBearerQciFlg_ENUM, LNAS_ENTRY, g_ulImsaIgnoreMediaBearerQci);
    return g_ulImsaIgnoreMediaBearerQci;
}
...
VOS_VOID IMSA_ReadImsaNvCommParaConfig(VOS_VOID)
{
    ...
    if (VOS_OK == ulRslt)
    {
        ...
        IMSA_InitNonRegRejCauseListWithNvConfig(&stNvImsaConfig);

        IMSA_InitFeatureCtrlFlagWithNvConfig(&stNvImsaConfig);

        /* 配置定时器时长 */
        IMSA_ModifyTimerLenWithCommNvConfig(&stNvImsaConfig);
        #if (FEATURE_ON == FEATURE_PTM)
        IMSA_InitConnAndRegTimeThresholdWithConfig(&stNvImsaConfig);
        #endif

        IMSA_SetNicMtuLength(stNvImsaConfig.ulMtuLength);
        pstControlManager->ucTempForbExpDelayTimeLen = stNvImsaConfig.ucTempForbExpDelayTimeLen;

        IMSA_SetIgnoreMediaBearerQciFlg(stNvImsaConfig.stImsaCtrlBit.bitOpIgnoreMediaBearerQci);

        /* 配置定制特性的开关状态 */
        IMSA_InitFeatureFlagWithNvConfig(&stNvImsaConfig);
    ...
    else
    {
        ...
        IMSA_InitNonRegRejCauseListWithDefaultConfig();
        pstCallCtx->ucACBFlag              = VOS_FALSE;
        pstImsaSipUri->ucNvReadSipUriFlag = VOS_FALSE;
        pstControlManager->ucAreaLostNotRelCallFlag = IMSA_FALSE;
        pstControlManager->ucNonSrvccNotRelCallFlag = IMSA_FALSE;
        ...
        IMSA_SetNicMtuLength(IMSA_MTU_DEFAULT_LENGTH);
        pstControlManager->ucTempForbExpDelayTimeLen     = 0;

        IMSA_SetIgnoreMediaBearerQciFlg(IMSA_OP_TRUE);
    }

    /* 抛出可维可测 */
    IMSA_SndOmPdnRejForbNvInfo();
    IMSA_SndOmNonRegRejNvInfo();
}
...
VOS_VOID  IMSA_ReadImsaNvImsaConfig( VOS_VOID)
{
    ...
    if(ulRslt == VOS_OK)
    {
        ...
        IMSA_SndOmImsaConfigNvInfo(&stNvImsaConfig);
    }
    else
    {
        ...
        IMSA_SndOmImsaConfigNvInfo(VOS_NULL_PTR);
    }
}
...
VOS_VOID IMSA_ReadIsimFile(VOS_VOID)
{
    ...
    /*将FileID转换为文件路径*/
    if (VOS_OK != USIMM_ChangeDefFileToPath(USIMM_TELE_EFPSISMSC_ID, &cPathStr))
    {
        IMSA_WARN_LOG("IMSA_ReadIsimFile: ChangeDefFileToPath err.");
        TLPS_PRINT2LAYER_WARNING(IMSA_ReadIsimFile_ENUM, IMSA_CALL_API_ERROR);
        return;
...
VOS_VOID IMSA_ProcIsimStatusInd(const VOS_VOID *pRcvMsg)
{
    ...
    if((pstIsimStatusInd->stIsimInfo).enCardService == USIMM_CARD_SERVIC_AVAILABLE)
    {
        ...
        /* 卡有效时获取插入的是ISIM卡还是USIM卡 */
        if ((pstIsimStatusInd->stAddInfo).enISIMState == USIMM_REALISIM_STATE_OFF)
        {
            pstControlManager->stImsaCommonInfo.enISIMState = IMSA_REALISIM_STATE_OFF;
            TLPS_PRINT2LAYER_INFO(IMSA_ProcIsimStatusInd_ENUM, 2);
        }

        if ((pstIsimStatusInd->stAddInfo).enISIMState == USIMM_REALISIM_STATE_ON)
        {
            pstControlManager->stImsaCommonInfo.enISIMState = IMSA_REALISIM_STATE_ON;
            TLPS_PRINT2LAYER_INFO(IMSA_ProcIsimStatusInd_ENUM, 3);
        }
    }
    else/*卡不在位*/
    {
        ...
        pstControlManager->stImsaCommonInfo.enISIMState = IMSA_REALISIM_STATE_BUTT;
...
{
    USIMM_STKREFRESH_IND_STRU          *pstIsimRefreshInd;
    IMSA_CONTROL_MANAGER_STRU          *pstControlManager;
    VOS_CHAR                           *cPathStr;
    VOS_UINT8                           ucRecordNum;
    USIMM_FILEPATH_INFO_STRU            stGetFilePath = {0};
    ...
    pstIsimRefreshInd = (USIMM_STKREFRESH_IND_STRU*)pRcvMsg;

    /* 其他场景会进行软关机再开机操作 */
    if ((USIMM_REFRESH_FILE_LIST != pstIsimRefreshInd->enRefreshType)
        && (USIMM_REFRESH_ALL_FILE != pstIsimRefreshInd->enRefreshType))
    {
    ...
    /* 一次遍历USIM上报的文件ID */
    for (i = 0; i < pstIsimRefreshInd->usEfNum; i++ )
    {
        if (USIMM_ISIM_EFIMPI_ID == pstIsimRefreshInd->astEfId[i].usFileId)
        {
            /* 将文件标志位置为FALSE */
            pstControlManager->stImsaCommonInfo.stImsaUeId.bitOpImpi = IMSA_OP_FALSE;
        ...
        if (USIMM_ISIM_EFDOMAIN_ID == pstIsimRefreshInd->astEfId[i].usFileId)
        {
            /* 将文件标志位置为FALSE */
            pstControlManager->stImsaCommonInfo.bitOpHomeNetDomainName = IMSA_OP_FALSE;
        ...
        if (USIMM_ISIM_EFIMPU_ID == pstIsimRefreshInd->astEfId[i].usFileId)
        {
            /* 将文件标志位置为FALSE */
            pstControlManager->stImsaCommonInfo.stImsaUeId.bitOpTImpu = IMSA_OP_FALSE;
    ...
    /* 启动读卡文件等待定时器 */
    IMSA_StartTimer(&pstControlManager->stWaitReadfileCnfTimer);
}
...
)
{
    ...
    if (IMSA_REG_TYPE_NORMAL == enRegType)
    {
        ...
        if (IMSA_REALISIM_STATE_OFF == pstImsaEntity->stImsaControlManager.stImsaCommonInfo.enISIMState)
        {
            pstImsaImsInputEvt->evt.stInputParaEvent.u.stNormalAccount.enISIMState
                                    = IMSA_IMS_REALISIM_STATE_OFF;
            TLPS_PRINT2LAYER_INFO(IMSA_ConverterImsiParam2Ims_ENUM, 2);
        }
        if (IMSA_REALISIM_STATE_ON == pstImsaEntity->stImsaControlManager.stImsaCommonInfo.enISIMState)
        {
            pstImsaImsInputEvt->evt.stInputParaEvent.u.stNormalAccount.enISIMState
                                    = IMSA_IMS_REALISIM_STATE_ON;
            TLPS_PRINT2LAYER_INFO(IMSA_ConverterImsiParam2Ims_ENUM, 3);
        }
    }
    else
    {
        ...
        if (IMSA_REALISIM_STATE_OFF == pstImsaEntity->stImsaControlManager.stImsaCommonInfo.enISIMState)
        {
            pstImsaImsInputEvt->evt.stInputParaEvent.u.stEmcAccount.enISIMState
                                    = IMSA_IMS_REALISIM_STATE_OFF;
            TLPS_PRINT2LAYER_INFO(IMSA_ConverterImsiParam2Ims_ENUM, 4);
        }
        if (IMSA_REALISIM_STATE_ON == pstImsaEntity->stImsaControlManager.stImsaCommonInfo.enISIMState)
        {
            pstImsaImsInputEvt->evt.stInputParaEvent.u.stEmcAccount.enISIMState
                                    = IMSA_IMS_REALISIM_STATE_ON;
            TLPS_PRINT2LAYER_INFO(IMSA_ConverterImsiParam2Ims_ENUM, 5);
        }
    }
}
...
VOS_UINT32 IMSA_IsImsVoiceContidionSatisfied( VOS_VOID )
{
    ...
    {
        /* 如果当前注册域为WIFI */
        if ((VOS_TRUE == pstControlMagnaer->stImsaConfigPara.ucWifiImsSupportFlag)
            && (VOS_TRUE == pstControlMagnaer->stImsaConfigPara.ucVoiceCallOnImsSupportFlag)
            && (IMSA_VOICE_DOMAIN_CS_ONLY != pstControlMagnaer->stImsaConfigPara.enVoiceDomain)
            && (IMSA_SRV_STATUS_CONN_REG == IMSA_SRV_GetNormalSrvStatus()))
        {
...
VOS_VOID  IMSA_SendRrcQualityConfigReq(IMSA_LRRC_CFG_QUALITY_ENUM_UINT8  enQualityReportType)
{
    IMSA_LRRC_QUALITY_CFG_REQ_STRU   *pstLrrcQualityCfgReq;

    /* 申请消息 */
    pstLrrcQualityCfgReq = (VOS_VOID *) IMSA_ALLOC_MSG(sizeof(IMSA_LRRC_QUALITY_CFG_REQ_STRU));
    if(VOS_NULL_PTR == pstLrrcQualityCfgReq)
    {
        return;
    }

    /* 设置为0 */
    IMSA_MEM_SET_S(pstLrrcQualityCfgReq,sizeof(IMSA_LRRC_QUALITY_CFG_REQ_STRU),0,sizeof(IMSA_LRRC_QUALITY_CFG_REQ_STRU));


    /*填写消息头*/
    IMSA_WRITE_LRRC_MSG_HEAD(pstLrrcQualityCfgReq,ID_IMSA_LRRC_QUALITY_CFG_REQ,
                             sizeof(IMSA_LRRC_QUALITY_CFG_REQ_STRU));

    /* 填充消息体 */
    pstLrrcQualityCfgReq->enStartOrStop            = enQualityReportType;

    /* 发送DOPRA消息 */
    IMSA_SND_MSG(pstLrrcQualityCfgReq);
}
...
VOS_UINT32 IMSA_IsImsTransactionExist(VOS_VOID)
{
    if (VOS_TRUE == IMSA_IsImsCallTransactionExist())
    {
        return VOS_TRUE;
    }
...
VOS_VOID IMSA_SndRrcVolteStatusNotify
(
...
)
{
    ...
    /* 新增主被叫类型 */
    pstVolteStatusNotify->enVolteCallTye = enVolteCallTye;
...
VOS_VOID IMSA_ProcLmmAttachBeginInd(const VOS_VOID * pMsg)
{
    /* 清除永久禁止注册标志 */
    IMSA_SetIsImsRegForbidden(IMSA_FALSE);

    /* 清除链接建立永久被禁标志 */
    IMSA_SetIsImsConnForbiddenFlag(IMSA_FALSE);

    return;
}


VOS_VOID IMSA_ProcLmmEmcEstFailInd(const VOS_VOID *pMsg)
{
    LMM_IMSA_EMC_EST_FAIL_IND_STRU     *pstEmcEstFailInd = VOS_NULL_PTR;
    pstEmcEstFailInd = (LMM_IMSA_EMC_EST_FAIL_IND_STRU*)pMsg;

    /* 目前LMM只会通知两个原因值 */
    switch(pstEmcEstFailInd->enEstResult)
    {
        case IMSA_LMM_EST_ACCESS_BARRED_IMS_EMC_CALL:
            IMSA_SRV_SndEmcCallSrvStatusInd(IMSA_CALL_SERVICE_STATUS_NO_SERVICE,
                                            IMSA_CALL_NO_SRV_CAUSE_IMS_EMC_BAR);
            break;

        case IMSA_LMM_EST_NOT_SUPPORT_IMS_EMC_CALL:
            IMSA_SRV_SndEmcCallSrvStatusInd(IMSA_CALL_SERVICE_STATUS_NO_SERVICE,
                                            IMSA_CALL_NO_SRV_CAUSE_IMS_EMC_NOT_SUPPORT);
            break;
...
VOS_VOID IMSA_SndOmNonRegRejNvInfo(VOS_VOID)
{
    IMSA_NON_REG_REJ_NV_INFO_STRU     *pstMsg          = VOS_NULL_PTR;
    IMSA_NON_REG_REJ_CTRL_STRU        *pstNonRegRejCtrl = VOS_NULL_PTR;

    pstMsg = (IMSA_NON_REG_REJ_NV_INFO_STRU*)IMSA_MEM_ALLOC(sizeof(IMSA_NON_REG_REJ_NV_INFO_STRU));
    if (VOS_NULL_PTR == pstMsg)
    {
        return;
    }
    ...
    IMSA_WRITE_INTRA_MSG_HEAD(  pstMsg,
                                ID_IMSA_OM_NON_REG_REJ_INFO,
                                sizeof(IMSA_NON_REG_REJ_NV_INFO_STRU));

    pstNonRegRejCtrl = IMSA_CallGetNonRegRejCtrlAddr();

    IMSA_MEM_CPY_S(&pstMsg->stNonRegRejCtrl, sizeof(IMSA_NON_REG_REJ_CTRL_STRU), pstNonRegRejCtrl, sizeof(IMSA_NON_REG_REJ_CTRL_STRU));
...
VOS_VOID IMSA_SndOmRatSupportNvInfo(IMSA_NV_IMS_RAT_SUPPORT_STRU* pstNvImsRatSupport)
{
    IMSA_RAT_SUPPORT_NV_INFO_STRU     *pstMsg          = VOS_NULL_PTR;

    pstMsg = (IMSA_RAT_SUPPORT_NV_INFO_STRU*)IMSA_MEM_ALLOC(sizeof(IMSA_RAT_SUPPORT_NV_INFO_STRU));
    if (VOS_NULL_PTR == pstMsg)
    {
        return;
    }
    ...
    IMSA_WRITE_INTRA_MSG_HEAD(  pstMsg,
                                ID_IMSA_RAT_SUPPORT_NV_INFO,
                                sizeof(IMSA_RAT_SUPPORT_NV_INFO_STRU));

    if(VOS_NULL_PTR != pstNvImsRatSupport)
    {
        pstMsg->ulNvReadResult = VOS_TRUE;
        IMSA_MEM_CPY_S(&pstMsg->stNvRatSupport,
                        sizeof(IMSA_NV_IMS_RAT_SUPPORT_STRU),
                        pstNvImsRatSupport,
                        sizeof(IMSA_NV_IMS_RAT_SUPPORT_STRU));
    }
    ...
    {
        pstMsg->ulNvReadResult = VOS_FALSE;
        IMSA_MEM_SET_S(&pstMsg->stNvRatSupport,
                        sizeof(IMSA_NV_IMS_RAT_SUPPORT_STRU),
                        0,
                        sizeof(IMSA_NV_IMS_RAT_SUPPORT_STRU));
    }
...
VOS_VOID IMSA_SndOmImsaConfigNvInfo(IMSA_NV_IMSA_CONFIG_STRU* pstNvImsaConfig)
{
    IMSA_IMSA_CONFIG_NV_INFO_STRU     *pstMsg          = VOS_NULL_PTR;

    pstMsg = (IMSA_IMSA_CONFIG_NV_INFO_STRU*)IMSA_MEM_ALLOC(sizeof(IMSA_IMSA_CONFIG_NV_INFO_STRU));
    if (VOS_NULL_PTR == pstMsg)
    {
        return;
    }
    ...
    IMSA_WRITE_INTRA_MSG_HEAD(  pstMsg,
                                ID_IMSA_IMSA_CONFIG_NV_INFO,
                                sizeof(IMSA_IMSA_CONFIG_NV_INFO_STRU));

    if(VOS_NULL_PTR != pstNvImsaConfig)
    {
        pstMsg->ulNvReadResult = VOS_TRUE;
        IMSA_MEM_CPY_S(&pstMsg->stNvImsaConfig,
                        sizeof(IMSA_NV_IMSA_CONFIG_STRU),
                        pstNvImsaConfig,
                        sizeof(IMSA_NV_IMSA_CONFIG_STRU));
    }
    ...
    {
        pstMsg->ulNvReadResult = VOS_FALSE;
        IMSA_MEM_SET_S(&pstMsg->stNvImsaConfig,
                        sizeof(IMSA_NV_IMSA_CONFIG_STRU),
                        0,
                        sizeof(IMSA_NV_IMSA_CONFIG_STRU));
    }
...
VOS_VOID IMSA_SndOmNoRegCauseInfo
(
...
)
{
    ...
    IMSA_OM_NO_REG_CAUSE_INFO         *pstMsg      = VOS_NULL_PTR;

    pstMsg = (IMSA_OM_NO_REG_CAUSE_INFO*)IMSA_MEM_ALLOC(sizeof(IMSA_OM_NO_REG_CAUSE_INFO));
    if (VOS_NULL_PTR == pstMsg)
    {
        return;
    }
    ...
    IMSA_WRITE_INTRA_MSG_HEAD(  pstMsg,
                                ID_IMSA_OM_NO_REG_CAUSE_INFO,
                                sizeof(IMSA_OM_NO_REG_CAUSE_INFO));

    pstMsg->enImsaSrvStatue       = enImsaSrvStatue;
    pstMsg->enImsaSrvConSatStatue = enImsaSrvConSatStatue;
...
VOS_VOID IMSA_SndOmServiceStateChangeInfo
(
...
)
{
    ...
    IMSA_OM_SERVICE_STATE_CHANGE_INFO  *pstMsg      = VOS_NULL_PTR;

    pstMsg = (IMSA_OM_SERVICE_STATE_CHANGE_INFO*)IMSA_MEM_ALLOC(sizeof(IMSA_OM_SERVICE_STATE_CHANGE_INFO));
    if (VOS_NULL_PTR == pstMsg)
    {
        return;
    }
    ...
    IMSA_WRITE_INTRA_MSG_HEAD(  pstMsg,
                                ID_IMSA_OM_SERVICE_STATE_CHANGE_INFO,
                                sizeof(IMSA_OM_SERVICE_STATE_CHANGE_INFO));

    pstMsg->enSrvType                = enSrvType;
    pstMsg->enNewImsaSrvStatue       = enNewImsaSrvStatue;
    pstMsg->enOldImsaSrvStatue       = enOldImsaSrvStatue;
