...
)
{
    ...
    if ((MMC_LMM_ATT_RSLT_ACCESS_BAR == ulAttRslt) ||
        ...
        (MMC_LMM_ATT_RSLT_NO_RF == ulAttRslt))
    {
...
)
{
    ...
    switch(ulForbInfo)
    {
        case EMMC_EMM_FORBIDDEN_PLMN:
            pEmmAppAttMsg->ulAttachRslt  = MMC_LMM_ATT_RSLT_FORBID_PLMN;
            break;

        case EMMC_EMM_FORBIDDEN_PLMN_FOR_GPRS:
            pEmmAppAttMsg->ulAttachRslt  = MMC_LMM_ATT_RSLT_FORBID_PLMN_FOR_GPRS;
            break;

        case EMMC_EMM_FORBIDDEN_TA_FOR_ROAMING:
            pEmmAppAttMsg->ulAttachRslt  = MMC_LMM_ATT_RSLT_FORBID_TA_FOR_ROAMING;
            break;

        case EMMC_EMM_FORBIDDEN_TA_FOR_RPOS:
            pEmmAppAttMsg->ulAttachRslt  = MMC_LMM_ATT_RSLT_FORBID_TA_FOR_RPOS;
            break;

        case EMMC_EMM_NO_FORBIDDEN:
            /* 如果系统消息未被禁且调用此函数上报ATTACH结果，则一定是ANY CELL的情况 */
            pEmmAppAttMsg->ulAttachRslt  = MMC_LMM_ATT_RSLT_FAILURE;
            break;

        case EMMC_EMM_FORBIDDEN_PLMN_NOT_ALLOW_REG:
            /* 调用gu nas接口不允许注册 */
            NAS_EMM_ATTACH_LOG_WARN("NAS_EMM_AppSendAttRsltForb:plmn not allowe register!");
            TLPS_PRINT2LAYER_WARNING(NAS_EMM_AppSendAttRsltForb_ENUM,LNAS_FUNCTION_LABEL1);
            pEmmAppAttMsg->ulAttachRslt  = MMC_LMM_ATT_RSLT_FAILURE;
            break;
...
VOS_VOID    NAS_EMM_CompCnAttachReq(NAS_EMM_MRRC_DATA_REQ_STRU *pAttReqMsg)
{
        ...
        || (EMM_ATTACH_CAUSE_ESM_ATTACH_FOR_INIT_EMC_BERER == NAS_EMM_GLO_AD_GetAttCau()))
    {
        ...
        pTempAttReqMsg->enCallType          = LRRC_LNAS_CALL_TYPE_IMS_EMERGENCY_CALL;
    }
    else
    {
...
)
{
    ...
    *pulIeLength                        = NAS_EMM_NULL;
    ...
    /* 编码IMSI时增加PS SIM卡是否有效判断, 解决当被#3等原因值拒绝之后, 打紧急呼发起紧急注册时携带IMSI */
...
)
{
    ...
    *pulIeLength                        = NAS_EMM_NULL;
    ...
    {
        /* 编码IMSI时增加PS SIM卡是否有效判断判断, 解决当被#3等原因值拒绝之后, 打紧急呼发起紧急注册时携带IMSI */
...
)
{
    ...
    *pulIeLength                        = NAS_EMM_NULL;
    ...
    pucReqMsg[ulIndex++]        = (((pucTmsi[2] & NAS_EMM_LOW7_BYTE)<<1)|((pucTmsi[1] & NAS_EMM_HITH_BYTE)>>7));
    pucReqMsg[ulIndex++]        = (((pucTmsi[1] & NAS_EMM_HITH67_BYTE)<<1) & NAS_EMM_HITH2_BYTE);
...
VOS_VOID    NAS_EMM_MrrcSendAttReq( VOS_VOID )
{
    ...
    if (LRRC_LNAS_CALL_TYPE_IMS_EMERGENCY_CALL != pIntraMsg->enCallType)
    {
        NAS_EMM_SndImsaAttachBeginInd();
    }
...
)
{
    ...
    /* 如果缺省PDN只有1个，则更新状态为紧急注册 */
    if (1 == pstEsmBearerStatusReq->ulPdnConNum)
    {
...
VOS_VOID NAS_EMM_SndAttachReqFailNoRfProc(VOS_VOID)
{
    NAS_EMM_ESM_MSG_BUFF_STRU          *pstEsmMsg = NAS_EMM_NULL_PTR;

    NAS_EMM_ATTACH_LOG_NORM("NAS_EMM_SndAttachReqFailNoRfProc is enter.");
    TLPS_PRINT2LAYER_INFO(NAS_EMM_SndAttachReqFailNoRfProc_ENUM,LNAS_ENTRY);

    /*为了保证快速发起流程，在ATTACH_REQ失败原因为NO_RF时，尝试计数不加*/
    ...
    {
        NAS_EMM_ATTACH_LOG_INFO(" NAS_EMM_SndAttachReqFailNoRfProc: CAUSE_ESM_ATTACH_FOR_INIT_EMC_BERER");
        TLPS_PRINT2LAYER_INFO(NAS_EMM_SndAttachReqFailNoRfProc_ENUM,LNAS_EMM_ATTACH_EMC_BEAER);
    ...
    *pulIsDelBuff = VOS_TRUE;
    ...
    switch (pstRrcMmDataCnf->enSendRslt)
    {
        ...
        case LRRC_LMM_SEND_RSLT_NO_RF:
            ...
            {
                NAS_EMM_SndAttachReqFailNoRfProc();
            }
