...
VOS_VOID    NAS_EMM_SER_SendMrrcDataReq_ServiceReq()
{
    ...
    switch(NAS_EMM_SER_GetEmmSERStartCause())
    {
        ...
        case NAS_EMM_SER_START_CAUSE_ESM_DATA_REQ_EMC:
            ...
            pMrrcDataReqMsg->enCallType = LRRC_LNAS_CALL_TYPE_IMS_EMERGENCY_CALL;
...
)
{
    ...
    /* 24301 8.2.15.2 GUL只有MT CSFB才填充CSFB RESPONSE,CL只有MO CSFB才填充 */
...
/*****************************************************************************
 Function Name   : NAS_EMM_EmcPndReqSerTAURejProc
 Description     : 紧急类型的PDN连接建立引起的SERVICE TAU REJ#9、#10、#40的公共处理
 Input           : ucSs---------------------子状态
 Output          : None
 Return          : VOS_VOID

 History         :
    1.杨磊 00307272      2016-06-23  Draft Enact

*****************************************************************************/
VOS_VOID  NAS_EMM_EmcPndReqSerTAURejProc(VOS_VOID)
{
