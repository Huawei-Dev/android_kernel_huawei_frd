...
VOS_VOID NAS_ERABM_EsmMsgDistr( VOS_VOID *pRcvMsg )
{
    ...
    switch (pSmMsg->ulMsgName)
    {
        ...
        case ID_ESM_ERABM_CLEAR_CL_BEARER_NOTIFY:
            NAS_ERABM_RcvRabmEsmClearClBearerNotify((ESM_ERABM_CLEAR_CL_BEARER_NOTIFY_STRU *)pRcvMsg);
            break;
        #endif
...
VOS_VOID NAS_ERABM_RcvRabmEsmClearClBearerNotify
(
    const ESM_ERABM_CLEAR_CL_BEARER_NOTIFY_STRU *pRcvMsg
)
{
    VOS_UINT32                          ulLoop;
    ...
    /* 如果存在激活承载，则通知CDS释放缓存的数据包，加上此判断是为了避免重复
       通知CDS释放缓存数据包 */
    if ((0 != NAS_ERABM_GetActiveEpsBearerNum())
        ||(0 != NAS_ERABM_GetActiveEhrpdBearerNum()))
    {
        /* 只有不存在EHRPD承载时，才通知CDS释放缓存的数据包，如果存在EHRPD承载时，不能清除 */
        if(PS_FALSE == pRcvMsg->ucAttachFlag)
        {
            /* 通知CDS释放缓存的数据包 */
            NAS_ERABM_SndErabmCdsFreeBuffDataInd();
        }
    ...
    NAS_ERABM_ClearRabmLteResource();
