...
/* RRM-EMM 消息打印数组 */
NAS_COMM_PRINT_MSG_LIST_STRU g_astLmmRrmMsgIdArray[] =
{
    {   ID_RRM_PS_STATUS_IND               ,
            "MSG:  ID_RRM_PS_STATUS_IND                              "},
    {   ID_PS_RRM_REGISTER_IND               ,
            "MSG:  ID_PS_RRM_REGISTER_IND                              "}
};
...
VOS_INT32   NAS_LMM_PUBM_PrintRrmMsg
(
    VOS_CHAR                           *pcBuff,
    VOS_UINT16                          usOffset,
    APP_MM_MSG_TYPE_ENUM_UINT32         enMsgId
)
{
    ...
    ulMmLppMsgNum = sizeof(g_astLmmRrmMsgIdArray)/sizeof(NAS_COMM_PRINT_MSG_LIST_STRU);

    for (ulLoop = 0; ulLoop < ulMmLppMsgNum; ulLoop++)
    {
        if (g_astLmmRrmMsgIdArray[ulLoop].ulId == enMsgId)
        {
            break;
        }
    }
    ...
    if ( ulLoop < ulMmLppMsgNum )
    {
        NAS_COMM_nsprintf(
                    pcBuff,
                    usTotalLen,
                    (VOS_CHAR *)(g_astLmmRrmMsgIdArray[ulLoop].aucPrintString),
                    &ilOutPutLen);
        usTotalLen += (VOS_UINT16)ilOutPutLen;
    ...
    {
        NAS_LMM_PUBM_LOG1_WARN("NAS_LMM_PUBM_PrintRrmMsg, Invalid enMsgId: ",
                              enMsgId);
       TLPS_PRINT2LAYER_WARNING1(NAS_LMM_PUBM_PrintRrmMsg_ENUM
,LNAS_FUNCTION_LABEL1,
                              enMsgId);
    }
...
,LNAS_FUNCTION_LABEL1);
    ...
    switch ( pstMsg->ulReceiverPid )
    {
        ...
        case UEPS_PID_RRM:
            ...
            ilOutPutLen = NAS_LMM_PUBM_PrintRrmMsg(pcBuff,
                                            usTotalLen,
                                            pstMsg->ulMsgName);
            break;
