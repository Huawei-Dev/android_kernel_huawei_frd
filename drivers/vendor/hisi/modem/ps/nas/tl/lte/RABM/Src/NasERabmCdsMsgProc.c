...
)
{
    ...
    /* 增加判断ucEpsbId是专有承载，与该专有承载关联的Eps承载如果是紧急承载，
       则发起紧急SERVICE */
    if (ESM_ERABM_BEARER_TYPE_DEDICATED == NAS_ERABM_GetEpsbBearerType(ucEpsbId))
    {
        ulLinkedEpsbId = NAS_ERABM_GetEpsbLinkedEpsbId(ucEpsbId);
        if (ESM_ERABM_BEARER_TYPE_EMERGENCY == NAS_ERABM_GetEpsbBearerType(ulLinkedEpsbId))
        {
            return VOS_TRUE;
        }
    }
...
)
{
    ...
    /* 已经在CDS_ERABM_GetDrbId中保护 */
    if(NAS_ERABM_XCC_CALL_FLAG_VALID == NAS_ERABM_GetXccCallFlg())
    {
        return ;
    }
...
)
{
    ...
    if(NAS_ERABM_XCC_CALL_FLAG_VALID == NAS_ERABM_GetXccCallFlg())
    {
        ...
        NAS_ERABM_LOG1("CDS_ERABM_GetDrbId:XCC CALL, set data pending = ",NAS_ERABM_GetUpDataPending());
