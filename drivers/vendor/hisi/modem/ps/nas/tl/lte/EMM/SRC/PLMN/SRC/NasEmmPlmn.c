                    ...
                    VOS_VOID                                *pMsg )
{
    ...
    if(LRRC_LNAS_START_SUCCESS == enResult)
    {
        ...
        if((PS_SUCC == LPS_OM_IsTestMode())
          && (PS_FALSE == NAS_LMM_GetT3402DefaultFlag()))
        {
...
VOS_VOID NAS_EMM_ProcLocalStop( VOS_VOID )
{
    ...
    NAS_LMM_EMM_PLMN_LOG2_INFO( "NAS_EMM_ProcLocalStop usim refresh flag .",
                                NAS_MML_GetUsimRefreshFlag(),
                                NAS_MML_GetUsimRefreshType());

    TLPS_PRINT2LAYER_INFO2(     NAS_EMM_ProcLocalStop_ENUM,
    ...
    if (EMM_MS_DEREG != NAS_EMM_CUR_MAIN_STAT)
    {
        ...
        {
            /* 关机时，读取mml两个api，满足条件删除GUTI，LVRTai,设置EU2*/
            if((PS_TRUE == NAS_LMM_GetUiccResetClearFlag())
                &&(VOS_TRUE == NAS_MML_GetUsimRefreshFlag())
                &&(USIMM_RESET == NAS_MML_GetUsimRefreshType()))
            {
                /*删除GUTI*/
                NAS_EMM_ClearGuti();
                ...
                /*保存PS LOC信息*/
                NAS_LMM_WritePsLocForUiccReset(NAS_NV_ITEM_UPDATE);
            }
            else
            {
    ...
    else
    {
            ...
            &&(USIMM_RESET == NAS_MML_GetUsimRefreshType()))
        {
            ...
            NAS_LMM_EMM_PLMN_LOG_NORM(  "NAS_EMM_ProcLocalStop clear guti.");
            TLPS_PRINT2LAYER_INFO(NAS_EMM_ProcLocalStop_ENUM,LNAS_FUNCTION_LABEL2);
