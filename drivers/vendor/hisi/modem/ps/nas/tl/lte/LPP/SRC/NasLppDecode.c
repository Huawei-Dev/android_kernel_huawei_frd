...
)
{
    ...
    if(0 != ulPadLen)
    {
        (VOS_VOID)NAS_LPP_GetDataFromBitStream(pucDecodeBitStream, pulCurrentBitPos, (VOS_UINT8)ulPadLen);
    }
    ...
    LPP_NAV_MODEL_NAV_KEPLERIAN_SET_STRU                   *pstNavKeplerSet)
{
    ...
    /*解码M0*/
    pstNavKeplerSet->lNavM0 = (VOS_INT32)((VOS_INT64)NAS_LPP_GetDataFromBitStream(pucDecodeBitStream,
                                                                                  pulCurrentBitPos, 32) - llTempValue);
    ...
    LPP_NAV_MODEL_SBAS_STRU            *pstSbassEcef)
{
    ...
    {
        pstSbassEcef->usSbasTo = (VOS_UINT16)NAS_LPP_GetDataFromBitStream(pucDecodeBitStream,
                                                                         pulCurrentBitPos, 13);
        if(pstSbassEcef->usSbasTo > 5399)
        {
    ...
    /*解码Accuracy*/
    pstSbassEcef->usSbasAccuracy = (VOS_UINT16)NAS_LPP_GetDataFromBitStream(pucDecodeBitStream,
                                                                           pulCurrentBitPos, 4);
...
)
{
    ...
    for(ulLoop = 0; ulLoop < pstGnssBadSignalList->ulGNSSBadSignalListCnt; ulLoop++)
    {
        ulRslt = NAS_LPP_DecodeGnssBadSignallListElement(pucDecodeBitStream, pulCurrentBitPos,
                                                       &(pstGnssBadSignalList->astGNSSBadSignalListArray[ulLoop]));
        if(NAS_LPP_DECODE_SUCC != ulRslt)
        {
            return ulRslt;
        }
...
)
{
    ...
    /*解码azimuth*/
    pstGnssAcqusitAssitElement->usAzimuth =
        (VOS_UINT16)NAS_LPP_GetDataFromBitStream(pucDecodeBitStream, pulCurrentBitPos, 9);
    /*解码elevation*/
    ...
    LPP_ELLIP_POINT_WITH_ALTITUDE_UNCERTAIN_ELLIPSOID_STRU     *pstEllipAltitude)
{
    ...
    /*解码degreeLatitude*/
    pstEllipAltitude->ulDegreesLatitude = NAS_LPP_GetDataFromBitStream(  pucDecodeBitStream,
                                                                        pulCurrentBitPos, 23);


    /*解码degreeLongitude*/
    lDegreesLongitude = (VOS_INT32)NAS_LPP_GetDataFromBitStream(pucDecodeBitStream,
                                                                pulCurrentBitPos, 24);
    pstEllipAltitude->lDegreesLongitude = lDegreesLongitude - 8388608;
