...
VOS_UINT8 NAS_LMM_IsNeedCause18EnableLte(VOS_VOID)
{
    //测试卡不打开优化
...
VOS_UINT32  NAS_LMM_PreProcMmcEnableReq( MsgBlock  *pMsg )
{
    ...
    if(VOS_TRUE == NAS_LMM_IsNeedCause18EnableLte())
    {
        /*清除拒绝#18标识*/
...
VOS_UINT32  NAS_EMM_PreProcMsgT3412Exp(MsgBlock *          pMsg )
{
    ...
    */
    ...
    if ((EMMC_EMM_NO_FORBIDDEN != ulForbdInfo)
        || (VOS_TRUE != NAS_LMM_IsCellStatusNormal((MMC_LMM_PLMN_ID_STRU*)(&(stTa.stPlmnId)), &stTa, ulCsgId)))
    {
        return NAS_LMM_MSG_HANDLED;
    }
