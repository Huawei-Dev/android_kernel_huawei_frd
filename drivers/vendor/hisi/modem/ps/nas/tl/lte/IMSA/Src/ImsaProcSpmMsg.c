...
)
{
    ...
    IMSA_SPM_CALL_GET_CALL_INFO_CNF_STRU* pstQryCnf = VOS_NULL_PTR;
    ...
    pstQryCnf = IMSA_MEM_ALLOC(sizeof(IMSA_SPM_CALL_GET_CALL_INFO_CNF_STRU));
    if (VOS_NULL_PTR == pstQryCnf)
    {
        /*打印不合法信息*/
        IMSA_ERR_LOG("IMSA_CallProcSpmMsgGetInfo: Mem alloc fail!");
        TLPS_PRINT2LAYER_ERROR(IMSA_CallProcSpmMsgGetInfo_ENUM, LNAS_NULL_PTR);
    ...
    for (i = 0; i < IMSA_CALL_MAX_NUM; i++)
    {
        if (VOS_TRUE == pstCallCtx->astCallEntity[i].bitOpIsUsed)
        {
            ...
            IMSA_CallEntity2SpmCallInfoParam(&pstCallCtx->astCallEntity[i], &pstQryCnf->astCallInfo[ulCount]);
            ulCount++;
        }
    }

    pstQryCnf->ucNumOfCalls = (VOS_UINT8)ulCount;
    pstQryCnf->usClientId = pstAppMsg->usClientId;
    pstQryCnf->enReqType = pstAppMsg->enReqType;


    ulResult = IMSA_CallSendSpmClccInfoCnf(pstQryCnf);

    IMSA_MEM_FREE(pstQryCnf);
    return ulResult;
}
...
VOS_UINT32 IMSA_CallSendSpmOrigCnfMsgRej(VOS_UINT16 usClientId,
                                      VOS_UINT32 ulOpId,
...
)
{
    ...
    if (VOS_NULL_PTR != pstCallEntity)
    {
        ...
        {
            IMSA_SndRrcVolteStatusNotify(IMSA_LRRC_VOLTE_CALL_TYPE_MO, IMSA_LRRC_VOLTE_STATUS_INIT);
        }
        ...
        /* 保存加密标志 */
        pstCallEntity->ucEncryptFlag = pstOrigParam->ucEncryptFlag;
        /* 更新被叫号码 */
    ...
    {
        /* 分配资源失败，拒绝该呼叫请求 */
        (VOS_VOID)IMSA_CallSendSpmOrigCnfMsgRej(pstAppMsg->usClientId,
                                                pstAppMsg->ucOpId,
                                                pstAppMsg->stOrig.enCallType,
                                                TAF_CS_CAUSE_IMSA_ALLOC_ENTITY_FAIL);
    ...
    ulResult = IMSA_CallSendImsMsgDial(IMSA_CALL_INVALID_ID, pstAppMsg, pstCallEntity->enEmcType);
...
VOS_UINT32 IMSA_CallProcSpmMsgNormalOrig(VOS_VOID* pMsg)
{
    ...
    /* 当前为第一路电话(此时CALL实体还没有创建, 所以判断Call实体个数是否为0), 小区信号质量不好则返回呼叫不可用,
       使上层去GU下继续CS电话 */
    if ((0 == IMSA_CallEntityGetUsedCount())
        && (VOS_OK != IMSA_LRRC_JudgeCurrCellSingalQuality())
...
VOS_UINT32 IMSA_CallProcSpmMsgOrig(VOS_VOID* pMsg)
{
    ...
    /* WIFI下面暂不支持视频呼叫，因此收到视频呼叫请求后，自动转为语音呼叫 */
    if ((IMSA_IMS_DOMAIN_WIFI == IMSA_GetCurrentDomain()) &&
         MN_CALL_TYPE_VIDEO == pstOrigParam->enCallType)
    {
        (VOS_VOID)IMSA_CallSendSpmCallTypeChangeInfoIndMsg(pstAppMsg->usClientId,
                                                           pstAppMsg->ucOpId,
                                                           IMSA_CALL_INVALID_ID,
        ...
        pstOrigParam->enCallType =  MN_CALL_TYPE_VOICE;
    }
...
)
{
                                            ...
                                            sizeof(SPM_IMSA_CALL_ORIG_REQ_STRU)))
    {
        ...
        (VOS_VOID)IMSA_CallSendSpmOrigCnfMsgRej(pstCallOrigReq->usClientId,
                                                pstCallOrigReq->ucOpId,
                                                pstCallOrigReq->stOrig.enCallType,
...
VOS_UINT32 IMSA_CallOrigReqUnSendInviteSrvccProc
(
    const SPM_IMSA_CALL_ORIG_REQ_STRU  *pstCallOrigReq
)
{
    ...
    /* 多路电话，最后一路invite消息还未发送，发生SRVCC，同时在SRVCC过程中，用户挂断最后一路电话，
    则在SRVCC BUFFER中存在一个CHLD的消息，一个DIAL消息，同步到CS域时，需要保证先同步DIAL消息，再
    同步CHLD消息，所以需要调整buffer中的顺序 */

    IMSA_MEM_CPY_S( &pstSrvccBuff->aucBuffMsgArray[1],
                    sizeof(IMSA_SRVCC_BUFF_MSG_UNION)*(IMSA_CALL_MSG_SYNC_MAX_NUM - 1),
                    &pstSrvccBuff->aucBuffMsgArray[0],
                    sizeof(IMSA_SRVCC_BUFF_MSG_UNION)*(pstSrvccBuff->ucBuffMsgNum));

    /* 不会同时存在两个没有发送给网侧的ORIG_REQ消息 */
    IMSA_MEM_CPY_S( &pstSrvccBuff->aucBuffMsgArray[0],
                    sizeof(IMSA_SRVCC_BUFF_MSG_UNION),
                    pstCallOrigReq,
                    sizeof(SPM_IMSA_CALL_ORIG_REQ_STRU));
...
)
{
    ...
    for (i = 0; i < pstSrvccBuff->ucBuffMsgNum; i++)
    {
         ...
         {
            case ID_SPM_IMSA_CALL_ORIG_REQ:
                (VOS_VOID)IMSA_CallSendSpmOrigCnfMsgRej(((SPM_IMSA_CALL_ORIG_REQ_STRU*)pstHeader)->usClientId,
                                                        ((SPM_IMSA_CALL_ORIG_REQ_STRU*)pstHeader)->ucOpId,
                                                        ((SPM_IMSA_CALL_ORIG_REQ_STRU*)pstHeader)->stOrig.enCallType,
                                                        ulTafCsCause);
                break;
...
)
{
    ...
    /* SPM下发Modify Req(Vedio->Voice) */
    if ((MN_CALL_TYPE_VIDEO == pstAppMsg->enCurrCallType)
        &&(MN_CALL_TYPE_VOICE == pstAppMsg->enExpectCallType))
    {
        /* 当前若IMSA由于视频承载丢弃已经给IMS发送了Modify Req, 但正在等待IMS回复OK时,
           不需要再给IMS下发Modify, 直接保存SPM消息; 否则按原有处理 */
            ...
            && (IMSA_VIDEO_BEARER_LOST_WAIT_IMS_OK == pstCallCtx->enVideoLostModCallTypeSta))
        {
            ...
            /* 设置Moify发起源为SPM */
            IMSA_SetVideoCallModifySource(IMSA_VIDEO_CALL_MODIFY_SOURCE_SPM);
