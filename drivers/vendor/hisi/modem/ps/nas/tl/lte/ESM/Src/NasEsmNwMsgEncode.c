                                                     ...
                                                     VOS_UINT32 *pulLength)
{
    ...
    if (NAS_ESM_PS_REGISTER_STATUS_DETACHED == NAS_ESM_GetEmmStatus())
    {
        ...
        /* 若PCO中不需要携带Pap或者Chap时, 则PCO信元在Attach Req中携带, 否则在Esm Rsp中携带
           查看对比机LOG, 当只设置了Apn没有设置PAP/CHAP时, PCO信元在Attach Req中携带, ESM RSP
           中只携带了APN, 与对比机对齐, 解决: 当前CL平台开关4G时注册概率被ESM #29拒, 对比机没
           有出现过, 而从消息信元对比来看, PCO携带顺序不一样 */
        if (VOS_FALSE == NAS_ESM_IsNeedEncodePapOrChap(pstSdfPara))
        {
                                                     ...
                                                     VOS_UINT32 *pulLength )
{
    ...
    /* 如果PCO中需要携带Pap或者Chap时, 则表示在ESM RSP中携带PCO信元, 否则PCO信元已经在Attach Req中携带了
       查看对比机LOG, 当只设置了Apn没有设置PAP/CHAP时, PCO信元在Attach Req中携带, ESM RSP
       中只携带了APN, 与对比机对齐, 解决: 当前CL平台开关4G时注册概率被ESM #29拒, 对比机没
       有出现过, 而从消息信元对比来看, PCO携带顺序不一样*/
