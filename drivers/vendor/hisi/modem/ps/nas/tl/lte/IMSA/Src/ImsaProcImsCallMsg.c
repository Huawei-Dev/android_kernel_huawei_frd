...
extern VOS_UINT32 IMSA_CallEntityProcCustomBuildCause
(
    IMSA_CALL_ENTITY_STRU *pstCallEntity,
    const IMSA_IMS_CALL_ERROR_STRU *pstImsError
);
...
VOS_VOID IMSA_CallEntityUpdateByCallSummary(IMSA_CALL_ENTITY_STRU *pstCallEntity, const IMSA_IMS_CALL_SUMMARY_STRU *pstCallSummary)
{
    ...
    else
    {
        ...
        /* 被叫需要保存Display name */
        IMSA_CallEntityUpdateCallDisplayNameByAscii((VOS_CHAR *)pstCallSummary->acDisplayName, pstCallEntity->acDisplayName);
    ...
    pstCallEntity->ucEncryptFlag = pstCallSummary->ucCallEncrypt;
}
...
VOS_UINT32 IMSA_CallProcImsMsgOk(const IMSA_IMS_OUTPUT_CALL_EVENT_STRU *pstOutputCallEvent)
{
    ...
    /* 当前为IMSA视频承载异常去激活降语音等待IMS回复Reason Ok时 */
    if ((IMSA_VIDEO_CALL_MODIFY_SOURCE_IMSA == pstCallCtx->enVideoCallModifySource)
        &&(IMSA_VIDEO_BEARER_LOST_WAIT_IMS_OK == pstCallCtx->enVideoLostModCallTypeSta))
    {
        /* 当前存在Video Call, 通知SPM Video->Voice */
        if (IMSA_TRUE == IMSA_IsExistVideoCall(&ulIndex))
        {
            (VOS_VOID)IMSA_CallSndSpmModifyStatusIndWithCallIndex(ulIndex, MN_CALL_TYPE_VIDEO, MN_CALL_TYPE_VOICE);
        }
...
)
{
    ...
    case ID_SPM_IMSA_CALL_SUPS_CMD_REQ:
        ...
        if (IMSA_IMS_INPUT_CALL_REASON_CONFERENCE == pstCallCtx->stImsMsg.ulCsmReason)
        {
            (VOS_VOID)IMSA_CallEntityProcCustomBuildCause(pstCallEntity, &pstOutputCallEvent->stCallError);
        }
        ...
        (IMSA_IMS_3GPP_TYPE_ACTION_EMERGENCY == pstImsError->stErrorCode.usIms3gppAction))
    {
        ...
        case IMSA_CALL_AVAILABILITY_CHECK_RESULT_CONTINUE:
            ...
            if (VOS_TRUE == ulResult)
            {
                ...
                /* 普通呼叫被380拒绝，再发起紧急呼，需要启动紧急Tcall定时器 */
...
VOS_UINT32 IMSA_CallEntityProcCustomBuildCause(IMSA_CALL_ENTITY_STRU *pstCallEntity, const IMSA_IMS_CALL_ERROR_STRU *pstImsError)
{
    ...
    IMSA_INFO_LOG1("IMSA_CallEntityProcCustomBuildCause: process error code", pstImsError->stErrorCode.usSipStatusCode);
    TLPS_PRINT2LAYER_INFO1(IMSA_CallEntityProcCustomBuildCause_ENUM, LNAS_ERROR, pstImsError->stErrorCode.usSipStatusCode);


    /* 定制NV没有打开，则不处理 */
    if (VOS_TRUE != pstNonRegRejCtrl->ucNonRegRejFeatureFlag)
    {
        TLPS_PRINT2LAYER_INFO(IMSA_CallEntityProcCustomBuildCause_ENUM, IMSA_NOT_SUPPORT);
        return IMSA_NOT_HANDLED;
    }

    /* 循环遍历配置的临时原因值列表，如果列表中有传入的参数的原因值，则发送内部消息用当前地址对发起初始注册 */
    for (i = 0 ; i < pstNonRegRejCauseList->ucSingleCauseNum ; i++)
    {
        if (pstImsError->stErrorCode.usSipStatusCode == pstNonRegRejCauseList->ausSingleCauseList[i])
        {
            TLPS_PRINT2LAYER_INFO1(IMSA_CallEntityProcCustomBuildCause_ENUM, IMSA_REG_ERROR_CODE, pstImsError->stErrorCode.usSipStatusCode);

            /* 用当前地址对发起初始注册 */
            if (VOS_NULL_PTR == pstCallEntity)
            {
                (VOS_VOID)IMSA_CallSendIntraResultAction(IMSA_CALL_TYPE_VOICE, IMSA_RESULT_ACTION_REG_WITH_CURRENT_ADDR_PAIR);
            }
            else
            {
                (VOS_VOID)IMSA_CallSendIntraResultAction(pstCallEntity->enType, IMSA_RESULT_ACTION_REG_WITH_CURRENT_ADDR_PAIR);
            }
            return IMSA_HANDLED;
        }
    }

    /* 走到这里说明列表已经遍历，没有找到对应的原因值，故不作处理 */
    return IMSA_NOT_HANDLED;
}
...
VOS_UINT32 IMSA_CallEntityProcErrorInfo(IMSA_CALL_ENTITY_STRU *pstCallEntity, const IMSA_IMS_CALL_ERROR_STRU *pstImsError)
{
    ...
    /* 处理定制原因值，如果处理完成，则返回 */
    if (IMSA_HANDLED == IMSA_CallEntityProcCustomBuildCause(pstCallEntity, pstImsError))
    {
        return VOS_TRUE;
    }
...
)
{
    ...
    if (IMSA_CallEntityGetUsedCount() == 0)
    {
        ...
        IMSA_SndRrcVolteStatusNotify(IMSA_LRRC_VOLTE_CALL_TYPE_BUTT, IMSA_LRRC_VOLTE_STATUS_END);
    }
...
)
{
    ...
    /* 如果是invite消息未发送成功，就发生了SRVCC，该路call需要去CS域重拨，
    所以不给SPM上报release事件，但是要释放call信息 */
    if (VOS_TRUE == pstCallEntity->ucInviteUnSendFlag)
    {
        IMSA_CallEntityFree(pstCallEntity);
        if (IMSA_CallEntityGetUsedCount() == 0)
        {
...
)
{
    ...
    /* 如果当前状态为dialing或trying，发生SRVCC时，如果invite消息已经发送，则不需要去CS域重拨，
    CALL模块只需要等网侧响应即可；如果invite消息未发送，则通过sync ind将消息带给SPM，由SPM
    去重拨 */
    #if 0
    ...
    #endif
    if(VOS_TRUE == pstCallCtx->ucIsTcallTimeOutProc)
    {
    ...
    else if (IMSA_TRUE == pstCallEntity->ucInviteUnSendFlag)
    {
        /* 如果invite消息未发送，就发生了SRVCC，则需要将该呼叫消息存到SRVCC BUFFER中，
        在SRVCC结束后，将该消息同步给SPM，由SPM去CS域重拨 */
        stCallOrigReq.ulSenderCpuId   = VOS_LOCAL_CPUID;
        stCallOrigReq.ulSenderPid     = WUEPS_PID_TAF;
        stCallOrigReq.ulReceiverCpuId = VOS_LOCAL_CPUID;
        stCallOrigReq.ulReceiverPid   = PS_PID_IMSA;
        stCallOrigReq.ulLength        = sizeof(SPM_IMSA_CALL_ORIG_REQ_STRU) - VOS_MSG_HEAD_LENGTH;
        stCallOrigReq.ulMsgId       = ID_SPM_IMSA_CALL_ORIG_REQ;
        stCallOrigReq.usClientId    = pstCallEntity->stSpmMsg.usClientId;
        stCallOrigReq.ucOpId        = (MN_OPERATION_ID_T)pstCallEntity->stSpmMsg.ulOpId;
        stCallOrigReq.stOrig        = pstCallEntity->stSpmMsg.stParam.stOrigParam;

        /* 如果插入buffer失败，则需要将invite消息未发送的标识清掉，给SPM发送release消息，避免
       该路电话挂死 */
        if (IMSA_FALSE == IMSA_CallOrigReqUnSendInviteSrvccProc(&stCallOrigReq))
        {
            pstCallEntity->ucInviteUnSendFlag    = IMSA_FALSE;
        }
    }
    else
    {
...
)
{
    ...
    /* 通过DISCONNECT消息中携带的callId判断是不是当前正在处理的呼叫，
        如果是当前正在处理的呼叫，则停止正在运行的呼叫相关的定时器*/
    if (VOS_TRUE == IMSA_CallIsCurrentCallInProcessing(pstOutputCallEvent->stCallReport.astCalls[0].ulCallIndex))
    {
        if (VOS_TRUE == IMSA_IsTimerRunning(&pstCallCtx->stNormalTcallTimer))
        {
            IMSA_StopTimer(&pstCallCtx->stNormalTcallTimer);
...
)
{
    ...
    if (pstCallEntity)
    {
        /* 当前为第一路CALL时, 通知LRRC Mo Call Init, 当lRRC收到INIT时,
            在收到START前LRRC会控制不上报B1/B2事件,从而减少BSRVCC发生 */
        if (1 == IMSA_CallEntityGetUsedCount())
        {
            IMSA_SndRrcVolteStatusNotify(IMSA_LRRC_VOLTE_CALL_TYPE_MT, IMSA_LRRC_VOLTE_STATUS_INIT);
        }
...
VOS_UINT32 IMSA_CallProcImsMsgSrvccCallList(const IMSA_IMS_OUTPUT_CALL_EVENT_STRU *pstOutputCallEvent)
{
    ...
    for(ucCount = 0; ucCount < ulCallNum; ucCount++)
    {
        ...
        /* 如果invite消息未发送，则该路电话不转到CS域 */
        if (VOS_TRUE == pstSrvccCallInfo->ucInviteUnSendFlag)
        {
            pstCallEntity->ucInviteUnSendFlag                    = VOS_TRUE;
            continue;
        }

        pstCallInfoNotify->ucCallNum ++;
        ...
        pstCallInfoNotify->astCallInfo[ucCount].ucNeedActiveFlag = pstSrvccCallInfo->ucNeedActiveFlag;
...
VOS_UINT32 IMSA_CallProcImsMsgModifyStatusInd(const IMSA_IMS_OUTPUT_CALL_EVENT_STRU *pstOutputCallEvent)
{
    ...
    /* 当收到IMS的Video->Voice指示时, 设置modify源为IMS */
    if ((IMSA_IMS_CALL_MODE_VIDEO == pstOutputCallEvent->stCallModify.enSrcCallMode)
        &&(IMSA_IMS_CALL_MODE_VOICE == pstOutputCallEvent->stCallModify.enDstCallMode))
    {
        /* 先清除后设置 */
        IMSA_ClearVideoLostModCallTypeCtrInfo();
        IMSA_SetVideoCallModifySource(IMSA_VIDEO_CALL_MODIFY_SOURCE_IMS);
    }
...
VOS_UINT32 IMSA_CallProcImsStateIncoming(IMSA_CALL_ENTITY_STRU *pstCallEntity, const IMSA_IMS_CALL_SUMMARY_STRU *pstCallSummary)
{
    ...
    if (VOS_FALSE == g_ulImsaNotifyRrcVoLteCallStartFlag)
    {
        IMSA_SndRrcVolteStatusNotify(IMSA_LRRC_VOLTE_CALL_TYPE_MT, IMSA_LRRC_VOLTE_STATUS_START);
    }
