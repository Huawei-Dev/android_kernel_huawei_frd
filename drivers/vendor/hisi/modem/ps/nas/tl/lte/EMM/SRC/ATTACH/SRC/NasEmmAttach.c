...
VOS_VOID  NAS_EMM_ProcRrcDataCnfEpsSuccOnly( VOS_VOID )
{
    ...
    /* 修改原因: NAS在EPS only注册成功时没有设置注册态，导致anycell驻留时错误发起了TAU */
...
VOS_VOID  NAS_EMM_MsRegInitSsWtCnAttCnfIntraConnFailNoRfProc( VOS_VOID )
{
    NAS_EMM_ESM_MSG_BUFF_STRU          *pstEsmMsg = NAS_EMM_NULL_PTR;

    NAS_EMM_TAU_LOG_INFO( "NAS_EMM_MsRegInitSsWtCnAttCnfIntraConnFailNoRfProc is entered.");
    TLPS_PRINT2LAYER_INFO(NAS_EMM_MsRegInitSsWtCnAttCnfIntraConnFailNoRfProc_ENUM, LNAS_ENTRY);

    /*为了保证快速发起流程，在INTRA_CONNECT_FAIL_IND失败原因为NO_RF时，尝试计数不加*/

    pstEsmMsg = (NAS_EMM_ESM_MSG_BUFF_STRU*)NAS_LMM_GetEmmInfoEsmBuffAddr();
    if ((EMM_ATTACH_CAUSE_ESM_ATTACH_FOR_INIT_EMC_BERER == NAS_EMM_GLO_AD_GetAttCau())
        && (NAS_EMM_NULL_PTR != pstEsmMsg))
    {
        NAS_EMM_ATTACH_LOG_INFO(" NAS_EMM_MsRegInitSsWtCnAttCnfIntraConnFailNoRfProc: CAUSE_ESM_ATTACH_FOR_INIT_EMC_BERER");
        TLPS_PRINT2LAYER_INFO(NAS_EMM_MsRegInitSsWtCnAttCnfIntraConnFailNoRfProc_ENUM,LNAS_EMM_ATTACH_EMC_BEAER);

        NAS_EMM_AppSendAttOtherType(MMC_LMM_ATT_RSLT_NO_RF);
    ...
    /*挂ATTACH的RF资源*/
    NAS_EMM_SndRrmRegisterInd(RRM_PS_TASK_TYPE_LTE_PS_ATTACH);
                                                    ...
                                                    VOS_VOID   *pMsgStru )
{
    ...
    switch(pMrrcConnectFailInd->enEstResult)
    {
        ...
        case LRRC_EST_CELL_SEARCHING:
            ...
            break;
        case LRRC_EST_NO_RF:
            /* 大数据: 更新attach NO RF次数 */
            NAS_LMM_AddAttachCounter(NAS_LMM_OM_PCE_TIMES_TYPE_NO_RF);
            if(PS_TRUE == NAS_LMM_GetDsdsOptimizeFlag())
            {
                NAS_EMM_MsRegInitSsWtCnAttCnfIntraConnFailNoRfProc();
            }
            else
            {
...
VOS_UINT32 NAS_EMM_MsDrgSsAptToAttachMsgRrmPsStatusInd(  VOS_UINT32  ulMsgId,
                                                    VOS_VOID   *pMsgStru)
{
    RRM_PS_STATUS_IND_STRU             *pstRrmPsStatusInd = NAS_EMM_NULL_PTR;
    VOS_UINT32                          ulRslt;
    NAS_LMM_FSM_STATE_STRU              stEmmState;

    NAS_LMM_PUBM_LOG_INFO("NAS_EMM_MsDrgSsAptToAttachMsgRrmPsStatusInd Enter.");
    TLPS_PRINT2LAYER_INFO(NAS_EMM_MsDrgSsAptToAttachMsgRrmPsStatusInd_ENUM, LNAS_ENTRY);

    pstRrmPsStatusInd = (RRM_PS_STATUS_IND_STRU*)pMsgStru;

    if(PS_TRUE != NAS_LMM_GetDsdsOptimizeFlag())
    {
        /*如果nv没有打开，则不处理*/
        NAS_LMM_PUBM_LOG_INFO("NAS_EMM_MsDrgSsAptToAttachMsgRrmPsStatusInd: DSDS2.0 feature is close");
        TLPS_PRINT2LAYER_INFO(NAS_EMM_MsDrgSsAptToAttachMsgRrmPsStatusInd_ENUM, NAS_LMM_DsdsFeatureIsClose);
        return NAS_LMM_MSG_HANDLED;
    }

    if(RRM_PS_TASK_TYPE_LTE_PS_ATTACH != pstRrmPsStatusInd->enTaskType)
    {
        /*如果不是ATTACH类型的资源恢复，则直接返回*/
        return NAS_LMM_MSG_HANDLED;
    }
    ...
    ulRslt = NAS_EMM_SendIntraAttachReq();
    if (NAS_EMM_SUCC != ulRslt)
    {
        NAS_LMM_EMM_PLMN_LOG_ERR("NAS_EMM_MsDrgSsAptToAttachMsgRrmPsStatusInd : send INTRA ATTACH REQ ERR !");
        TLPS_PRINT2LAYER_ERROR(NAS_EMM_MsDrgSsAptToAttachMsgRrmPsStatusInd_ENUM, LNAS_ERROR);
    }
...
VOS_UINT32 NAS_EMM_MsDrgSsNormalSrvMsgRrmPsStatusInd(VOS_UINT32  ulMsgId,
                                                        VOS_VOID   *pMsgStru )
{
    RRM_PS_STATUS_IND_STRU             *pstRrmPsStatusInd = NAS_EMM_NULL_PTR;
    VOS_UINT32                          ulRslt;

    NAS_LMM_PUBM_LOG_INFO("NAS_EMM_MsDrgSsNormalSrvMsgRrmPsStatusInd Enter.");
    TLPS_PRINT2LAYER_INFO(NAS_EMM_MsDrgSsNormalSrvMsgRrmPsStatusInd_ENUM, LNAS_ENTRY);
    ...
    {
        /*如果nv没有打开，则不处理*/
        NAS_LMM_PUBM_LOG_INFO("NAS_EMM_MsDrgSsNormalSrvMsgRrmPsStatusInd: DSDS2.0 feature is close");
        TLPS_PRINT2LAYER_INFO(NAS_EMM_MsDrgSsNormalSrvMsgRrmPsStatusInd_ENUM, NAS_LMM_DsdsFeatureIsClose);
        return NAS_LMM_MSG_HANDLED;
    }
    ...
    if (NAS_EMM_SUCC != ulRslt)
    {
        NAS_EMM_PUBU_LOG_ERR("NAS_EMM_MsDrgSsNormalSrvMsgRrmPsStatusInd:  send INTRA ATTACH REQ ERR !");
        TLPS_PRINT2LAYER_ERROR(NAS_EMM_MsDrgSsNormalSrvMsgRrmPsStatusInd_ENUM, LNAS_ERROR);
    }
...
VOS_UINT32 NAS_EMM_SndAttachCmpFailNoRfProc(VOS_VOID)
{

    NAS_EMM_ATTACH_LOG_INFO("NAS_EMM_SndAttachCmpFailNoRfProc is entered");
    TLPS_PRINT2LAYER_INFO(NAS_EMM_SndAttachCmpFailNoRfProc_ENUM,LNAS_ENTRY);

    /*为了保证快速发起流程，在ATTACH_CMP失败原因为NO_RF时，尝试计数不加*/
    ...
    if (EMM_ATTACH_CAUSE_ESM_ATTACH_FOR_INIT_EMC_BERER == NAS_EMM_GLO_AD_GetAttCau())
    {
        NAS_EMM_ATTACH_LOG_INFO(" NAS_EMM_SndAttachCmpFailNoRfProc: CAUSE_ESM_ATTACH_FOR_INIT_EMC_BERER");
        TLPS_PRINT2LAYER_INFO(NAS_EMM_SndAttachCmpFailNoRfProc_ENUM,LNAS_EMM_ATTACH_EMC_BEAER);
...
VOS_UINT32 NAS_EMM_SndAttachCmpFailOtherProc(VOS_VOID)
{
    NAS_EMM_ATTACH_LOG_INFO("NAS_EMM_SndAttachCmpFailOtherProc is entered");
    TLPS_PRINT2LAYER_INFO(NAS_EMM_SndAttachCmpFailOtherProc_ENUM,LNAS_ENTRY);

    /* 大数据: 更新attach cmp DATA REQ发送失败次数 */
    NAS_LMM_AddAttachCounter(NAS_LMM_OM_PCE_TIMES_TYPE_DATA_CNF_FAIL);
    if (EMM_ATTACH_CAUSE_ESM_ATTACH_FOR_INIT_EMC_BERER == NAS_EMM_GLO_AD_GetAttCau())
    {
        NAS_EMM_ATTACH_LOG_INFO(" NAS_EMM_SndAttachCmpFailOtherProc: CAUSE_ESM_ATTACH_FOR_INIT_EMC_BERER");
        TLPS_PRINT2LAYER_INFO(NAS_EMM_SndAttachCmpFailOtherProc_ENUM,LNAS_EMM_ATTACH_EMC_BEAER);
...
VOS_UINT32 NAS_EMM_SndAttachCmpFailProc(VOS_VOID* pMsg,VOS_UINT32 *pulIsDelBuff)
{
    ...
    VOS_UINT32 ulRslt = NAS_EMM_SUCC;
    ...
    *pulIsDelBuff = VOS_TRUE;
    ...
    if((LRRC_LMM_SEND_RSLT_NO_RF == pstRrcMmDataCnf->enSendRslt)&&
    (PS_TRUE == NAS_LMM_GetDsdsOptimizeFlag()))
    {
        ulRslt = NAS_EMM_SndAttachCmpFailNoRfProc();
    }
    else
    {
        ulRslt = NAS_EMM_SndAttachCmpFailOtherProc();
    }
...
VOS_VOID NAS_EMM_SndImsaAttachBeginInd(VOS_VOID)
{
