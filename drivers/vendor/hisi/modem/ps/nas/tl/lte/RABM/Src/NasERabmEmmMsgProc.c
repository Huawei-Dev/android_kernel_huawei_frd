...
VOS_VOID NAS_ERABM_EmmMsgDistr( VOS_VOID *pRcvMsg )
{
    ...
    switch (pEmmMsg->ulMsgName)
    {
        ...
        case ID_EMM_ERABM_RESUME_IND:
            ...
            break;
        case ID_EMM_ERABM_CALL_START_NTF:
            NAS_ERABM_RcvRabmEmmCallStartNtf((EMM_ERABM_CALL_START_NTF_STRU *) pRcvMsg);
            break;
        case ID_EMM_ERABM_CALL_END_NTF:
            NAS_ERABM_RcvRabmEmmCallEndNtf((EMM_ERABM_CALL_END_NTF_STRU *) pRcvMsg);
            break;
        default:
...
/*****************************************************************************
 Function Name   : NAS_ERABM_RcvRabmEmmCallStartNtf
 Description     : 收到EMM_ERABM_CALL_START_NTF
 Input           :
 Output          : None
 Return          : None

 History         :
    1.yanglei            2016-03-12      Draft Enact
*****************************************************************************/
VOS_VOID NAS_ERABM_RcvRabmEmmCallStartNtf
(
    const EMM_ERABM_CALL_START_NTF_STRU *pRcvMsg
)
{
    (VOS_VOID)pRcvMsg;
    /*打印进入该函数*/
    NAS_ERABM_INFO_LOG("NAS_ERABM_RcvRabmEmmCallStartNtf is entered.");
    TLPS_PRINT2LAYER_INFO(NAS_ERABM_RcvRabmEmmCallStartNtf_ENUM, LNAS_ENTRY);

    NAS_ERABM_SetXccCallFlg(NAS_ERABM_XCC_CALL_FLAG_VALID);
    return;
}
/*****************************************************************************
 Function Name   : NAS_ERABM_RcvRabmEmmCallEndNtf
 Description     : 收到EMM_ERABM_CALL_END_NTF
 Input           :
 Output          : None
 Return          : None
 ...
 History         :
    1.yanglei            2016-03-12      Draft Enact
*****************************************************************************/
VOS_VOID NAS_ERABM_RcvRabmEmmCallEndNtf
(
    const EMM_ERABM_CALL_END_NTF_STRU *pRcvMsg
)
{
    /*打印进入该函数*/
    NAS_ERABM_INFO_LOG("NAS_ERABM_RcvRabmEmmCallEndNtf is entered.");
    TLPS_PRINT2LAYER_INFO(NAS_ERABM_RcvRabmEmmCallEndNtf_ENUM, LNAS_ENTRY);

    (VOS_VOID)pRcvMsg;

    NAS_ERABM_SetXccCallFlg(NAS_ERABM_XCC_CALL_FLAG_INVALID);
