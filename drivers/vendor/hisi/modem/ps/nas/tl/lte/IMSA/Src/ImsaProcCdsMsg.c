...
VOS_VOID IMSA_CONN_SndCdsSetImsBearerReq
(
...
)
{
    ...
    /*填写响应结果*/
    if (IMSA_CDS_IMS_BEARER_RAT_TYPE_3GPP_EUTRAN == enImsBearerRatType)
    {
        ...
        pstSetImsBearerReq->enImsBearerRatType      = IMSA_CDS_IMS_BEARER_RAT_TYPE_3GPP_EUTRAN;
    }
    else
    {
        pstWifiNormalConn                   = IMSA_CONN_GetWifiNormalConnAddr();

        if (IMSA_PDP_STATE_ACTIVE == pstWifiNormalConn->stSipSignalPdp.enPdpState)
        {
            pstCdsImsBearer                 = &pstSetImsBearerReq->astImsBearerArray[pstSetImsBearerReq->ulImsBearerNum];
            pstCdsImsBearer->enBearerType   = IMSA_CDS_IMS_BEARER_TYPE_SIGNAL;
            ...
            pstSetImsBearerReq->enImsBearerRatType      = IMSA_CDS_IMS_BEARER_RAT_TYPE_IEEE_802_11;
        }
    }
