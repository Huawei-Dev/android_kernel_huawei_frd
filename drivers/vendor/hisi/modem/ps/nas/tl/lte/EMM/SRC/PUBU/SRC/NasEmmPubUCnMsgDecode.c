                                          ...
                                          )
{
    ...
    *pulSum             = NAS_EMM_NULL;
    ...
    while(ulIndex < pstIEOptParaList->ulLength)
    {
        ...
        /* 无法识别收到的NAS消息中的信元, 按照宽进严出原则,所以跳过当前字节对剩余码流进行遍历解析
           解决:TMO测试用例1.1.1.32用例TAU ACP消息中在GUTI信元之前仪器脚本多了一个字节E0(非IEI),
           按照之前实现截断码流返回成功不继续解析, 从而导致代码处理时认为TAU ACP中没有携带GUTI不给
           仪器回复TAU CMP, 用例失败, 而对比机码流相同, 确跳过了非法码流字节解析到了GUTI */
