...
VOS_UINT32 NAS_LMM_GetT3417extTimerLen(NAS_EMM_ESR_CAUSE_TYEP_ENUM_UINT8 ucEsrType)
{
    VOS_UINT32                          ulTimerLen      = NAS_LMM_TIMER_SER_T3417_EXT_LEN;

    /* 支持SRLTE定时器设置为500ms */
    if(( NAS_EMM_ESR_CAUSE_TYPE_CL_CALL == ucEsrType)
        &&(VOS_TRUE == NAS_EMM_IsSupportSrlte()))
    {
        ulTimerLen = NAS_LMM_TIMER_SRLTE_T3417_EXT_LEN;
    }
    return ulTimerLen;
}
