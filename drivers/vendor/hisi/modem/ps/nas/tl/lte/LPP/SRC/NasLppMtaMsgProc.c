...
)
{
    ...
    /* 发给RRC的位置信息组织数据，如果位置信息数据有效，则整体数据有效发送给RRC有效 */
    if ((NAS_LPP_SLCT != pstMtaLppGnssProvideLocInfoInd->bitCommonInfo)
      ||(NAS_LPP_SLCT != pstMtaLppGnssProvideLocInfoInd->stCommonProvideLocInfo.bitOpLocationEstimate))
    {
        TLPS_PRINT2LAYER_ERROR2(NAS_LPP_SndRrcMtaGnssLocInfo_ENUM, LNAS_LPP_NOLocInfoData,
                                pstMtaLppGnssProvideLocInfoInd->bitCommonInfo,
                                pstMtaLppGnssProvideLocInfoInd->stCommonProvideLocInfo.bitOpLocationEstimate);
        return;
    }
    ...
    if ((NAS_LPP_GNSS_ORIG_TYPE_MODEM == NAS_LPP_GetGnssOrigType())
      &&(LRRC_LPP_LOC_REPORT_TYPE_ONCE == NAS_LPP_GetGnssReportType()))
    {
...
)
{
    ...
    /*保存GNSS 测量IND消息到全局结构，临时修改，后续完善处理流程 */
    if ((NAS_LPP_SLCT != pstMtaLppGnssProvideLocInfoInd->bitCommonInfo)
      &&(NAS_LPP_SLCT != pstMtaLppGnssProvideLocInfoInd->bitAgnssInfo))
    {
        return;
    }
    ...
    NAS_LPP_OmMtaLocInfoCnf(&pstMtaLppGnssProvideLocInfoInd->stGnssReqLocationInfo.stGNSSsignalMeasInfo.stGNSSMeasList);
    /* 发送给RRC MTA的位置信息 */
    ...
    pstTransEntity = NAS_LPP_GetTransElementByIndex(pstLocTechMap->ucTransIndx);
...
)
{
    ...
    /* 如果当前没有在跑GNSS的Session */
    pstLppLocMap = NAS_LPP_SearchRunningPositionTech(NAS_LPP_POSITION_TECH_GNSS);
    if((NAS_LPP_NULL_PTR == pstLppLocMap)&&(NAS_LPP_GNSS_ORIG_TYPE_MODEM == NAS_LPP_GetGnssOrigType()))
    {

        NAS_LPP_ERR_LOG("NAS_LPP_RcvMtaLppGnssAssistDataReq: No Running GNSS Entity!");
        TLPS_PRINT2LAYER_ERROR(NAS_LPP_RcvMtaLppGnssAssistDataReq_ENUM, LNAS_LPP_NoRunningGnssEntity);
        return;
    }

    pstLocationReqBuff = NAS_LPP_GetLocationReqBuff();
    /* 如果GNSS在跑，但是网侧不允许请求辅助数据，则直接返回即可 */
    if ((NAS_LPP_SLCT == pstLocationReqBuff->bitOpAgnssLocReq )
     &&(PS_FALSE == pstLocationReqBuff->stAGNSSReqLocationInfo.stGNSSPositonINsturction.bAssistanceAvailability))
    {
        NAS_LPP_ERR_LOG("NAS_LPP_RcvMtaLppGnssAssistDataReq: not allowed to req assist data!");
        TLPS_PRINT2LAYER_ERROR(NAS_LPP_RcvMtaLppGnssAssistDataReq_ENUM, LNAS_LPP_NotAllowedToReqAssistData);
        return;
    }
