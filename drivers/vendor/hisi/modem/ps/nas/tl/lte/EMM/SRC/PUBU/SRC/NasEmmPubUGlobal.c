...
VOS_UINT32                              g_DsdsOptimize = PS_FALSE;

VOS_UINT32                              g_ulNasSrlteFlag = PS_FALSE;

VOS_UINT32                              g_ulNasT3402DefaultFlag = PS_FALSE;

VOS_UINT32                              g_ulUiccResetClearFlag = PS_FALSE;
...
VOS_VOID    NAS_EMM_ReadNvUeNetCap(VOS_VOID)
{
    ...
    VOS_UINT8                           ucMsNetworkCapSrvcc = 0;
    NAS_MML_MS_NETWORK_CAPACILITY_STRU  stMsNetworkCapbility = {0};
    ...
    NAS_MML_GetMsNetworkCapability(&stMsNetworkCapbility);
    ucMsNetworkCapSrvcc = (stMsNetworkCapbility.aucNetworkCapability[2] & 0x0f) >> 3;
    stUeNetworkCapbility.aucUeNetCap[4]     = stUeNetworkCapbility.aucUeNetCap[4] | ucMsNetworkCapSrvcc;
    ...
    pstUeNetCap->aucUeNetCap[4]     =  pstUeNetCap->aucUeNetCap[4] | ucMsNetworkCapSrvcc;
...
VOS_VOID  NAS_LMM_ReadNvDamParaCServiceAndPlmnList(VOS_VOID )
{
    ...
    NAS_MM_DAM_PLMN_LIST_STRU            *pstDamPlmnList = VOS_NULL_PTR;
    ...
    NAS_EMM_GetEmmInfoDamSwitchFlag()      = VOS_FALSE;
    ...
    if(NAS_LMM_NVIM_OK == ulRslt)
    {
        ...
        NAS_EMM_GetEmmInfoDamSwitchFlag()      = stNvDamPara.ucDamFlag[0];
...
{
    if ((VOS_TRUE == NAS_EMM_GetEmmInfoDamSwitchFlag())
...
VOS_UINT32 NAS_EMM_IsNeedIgnoreAuthRej(VOS_VOID)
{
    ...
    /* 功能打开且忽略auth rej消息次数未达到最大值，则需要忽略 */
...
VOS_VOID  NAS_LMM_ReadNvLmmSwitchPara(VOS_VOID )
{
    ...
    NAS_LMM_GetDsdsOptimizeFlag()       = LNAS_NV_GET_NAS_FUN_FLAG(0, LNAS_NV_GET_DSDS_OPTIMIZE_BIT);
    NAS_LMM_GetDsdsOptimizeFlag()       = (0 != NAS_LMM_GetDsdsOptimizeFlag()) ? PS_TRUE : PS_FALSE;
    g_ulNasSrlteFlag             = LNAS_NV_GET_NAS_FUN_FLAG(0, LNAS_NV_GET_SUPP_SRLTE_BIT);
    g_ulNasSrlteFlag             = (0 != g_ulNasSrlteFlag) ? PS_TRUE : PS_FALSE;

    g_ulNasT3402DefaultFlag             = LNAS_NV_GET_NAS_FUN_FLAG(0, LNAS_NV_GET_T3402_Default_BIT);
    g_ulNasT3402DefaultFlag             = (0 != g_ulNasT3402DefaultFlag) ? PS_TRUE : PS_FALSE;

    g_ulUiccResetClearFlag             = LNAS_NV_GET_NAS_FUN_FLAG(0, LNAS_NV_GET_UICC_RESET_CLEAR_BIT);
    g_ulUiccResetClearFlag             = (0 != g_ulUiccResetClearFlag) ? PS_TRUE : PS_FALSE;
...
VOS_UINT32 NAS_EMM_IsSupportSrlte(VOS_VOID)
{
#if (FEATURE_ON == FEATURE_UE_MODE_CDMA)
    if((VOS_TRUE == NAS_LMM_GetSrlteFlag())
        &&(VOS_TRUE == NAS_EMMC_IsCsfbSuppForDualRxUEs())
        &&(PS_RRM_VERSION_SRLTE == RRM_GetVersionType()))
    {
        return VOS_TRUE;
    }
#endif
...
VOS_VOID  NAS_LMM_WritePsLocForUiccReset
(
    NAS_NV_ITEM_OP_TYPE_ENUM_UINT32     enOpType
)
{
    ...
    NAS_LMM_NVIM_LOG_INFO("NAS_LMM_WritePsLocForUiccReset enter.");
    TLPS_PRINT2LAYER_INFO(NAS_LMM_WritePsLocForUiccReset_ENUM,LNAS_ENTRY);
    ...
    if (PS_USIM_SERVICE_AVAILIABLE == ulRslt)
    {
        NAS_LMM_NVIM_LOG_INFO("NAS_LMM_WritePsLocForUiccReset SERVICE AVAILABLE.");
        TLPS_PRINT2LAYER_INFO(NAS_LMM_WritePsLocForUiccReset_ENUM,LNAS_FUNCTION_LABEL1);
        /*向USIM写入PS LOC*/
        NAS_LMM_WriteSimPsLoc();
    }
    else
