...
#include  "NasEmmLppMsgProc.h" /*该头文件不控，因为LPP,LCS都会需要*/
...
VOS_VOID  NAS_EMM_PreProcMsgRrcMmEstCnfWithPresent(VOS_VOID)
{

    NAS_EMM_GIM_NORMAL_LOG("NAS_EMM_PreProcMsgRrcMmEstCnfWithPresent is entered.");
    TLPS_PRINT2LAYER_INFO(NAS_EMM_PreProcMsgRrcMmEstCnfWithPresent_ENUM,LNAS_ENTRY);
...
VOS_UINT32  NAS_EMM_PreProcMsgRrcMmEstCnf(MsgBlock * pMsg)
{
    ...
    switch (ulResult)
    {
        ...
        case LRRC_EST_PRESENT:
            NAS_EMM_PreProcMsgRrcMmEstCnfWithPresent();
            break;
