...
VOS_VOID    NAS_EMM_MsTauInitSsWaitCNCnfMsgIntraConnectFailNoRf(VOS_VOID)
{
    MMC_LMM_TAU_RSLT_ENUM_UINT32        ulTauRslt = MMC_LMM_TAU_RSLT_BUTT;

    NAS_EMM_TAU_LOG_INFO( "NAS_EMM_MsTauInitSsWaitCNCnfMsgIntraConnectFailNoRf is entered.");
    TLPS_PRINT2LAYER_INFO(NAS_EMM_MsTauInitSsWaitCNCnfMsgIntraConnectFailNoRf_ENUM, LNAS_ENTRY);

    /*为了保证快速发起流程，在建链失败原因为NO_RF时，尝试计数不加*/

    NAS_EMM_TAU_ProcAbnormal();

    ulTauRslt = MMC_LMM_TAU_RSLT_NO_RF;
    /*冲突检测，上报TAU结果在冲突检测过程中上报*/
    NAS_EMM_TAU_RelIndCollisionProc(NAS_EMM_MmcSendTauActionResultIndOthertype,
                    (VOS_VOID*)&ulTauRslt);

    /*挂TAU的RF资源*/
    NAS_EMM_SndRrmRegisterInd(RRM_PS_TASK_TYPE_LTE_PS_HIGH_TAU);

    return;
}


VOS_VOID  NAS_EMM_MsTauInitSsWaitCNCnfMsgIntraFailWithEmcPdn
(
...
)
{
    ...
    NAS_EMM_TAU_LOG_INFO("NAS_EMM_MsTauInitSsWaitCNCnfMsgIntraFailWithEmcPdn: enter");
    TLPS_PRINT2LAYER_INFO(NAS_EMM_MsTauInitSsWaitCNCnfMsgIntraFailWithEmcPdn_ENUM, LNAS_ENTRY);
...
VOS_VOID  NAS_EMM_MsTauInitSsWaitCNCnfMsgIntraFailOtherCause(VOS_VOID)
{
    /* 大数据:在给MMC发送TAU结果前，更新TAU EST FAIL次数 */
    NAS_LMM_AddTauCounter(NAS_LMM_OM_PCE_TIMES_TYPE_EST_FAIL);
    NAS_EMM_TAU_GetEmmTAUAttemptCnt()++;
                                                   ...
                                                   VOS_VOID   *pMsgStru)
{
    ...
    {
        NAS_EMM_MsTauInitSsWaitCNCnfMsgIntraFailWithEmcPdn(pMrrcConnectFailRelInd);
    ...
    switch (pMrrcConnectFailRelInd->enEstResult)
    {
        ...
        */
        case LRRC_EST_NO_RF:
             ...
             {
                /*如果特性NV打开，按照新的逻辑处理，影响尝试计数，上报结果，挂资源等*/
                NAS_EMM_MsTauInitSsWaitCNCnfMsgIntraConnectFailNoRf();
             }
             else
             {
                /*如果特性NV没有打开，按照旧的逻辑处理*/
                NAS_EMM_MsTauInitSsWaitCNCnfMsgIntraFailOtherCause();
             }
    ...
    *pulIsDelBuff = VOS_TRUE;
    ...
    switch (pstRrcMmDataCnf->enSendRslt)
    {
        ...
        case LRRC_LMM_SEND_RSLT_NO_RF:
            NAS_LMM_AddTauCounter(NAS_LMM_OM_PCE_TIMES_TYPE_NO_RF);
            if(PS_TRUE == NAS_LMM_GetDsdsOptimizeFlag())
            {
                NAS_EMM_TAU_StartTAUREQ(NAS_LMM_OM_TAU_START_TYPE_NORF);
            }
            break;
        default:
            break;
...
VOS_VOID NAS_EMM_SndRrmRegisterInd
(
...
)
{
    ...
    pstMsg = (PS_RRM_REGISTER_IND_STRU *)NAS_LMM_ALLOC_MSG(sizeof(PS_RRM_REGISTER_IND_STRU));
    /* 内存申请失败 */
    ...
    NAS_LMM_MEM_SET_S(pstMsg,sizeof(PS_RRM_REGISTER_IND_STRU), 0,sizeof(PS_RRM_REGISTER_IND_STRU));
    ...
    /*发送消息*/
    NAS_LMM_SEND_MSG(pstMsg);
    return;
}

VOS_VOID NAS_EMM_TAU_RcvDataCnfNoRfHandle(VOS_VOID)
{
    VOS_UINT32                          ulCurEmmStat;
    MMC_LMM_TAU_RSLT_ENUM_UINT32        ulTauRslt = MMC_LMM_TAU_RSLT_BUTT;

    NAS_EMM_TAU_LOG_INFO( "NAS_EMM_TAU_RcvDataCnfNoRfHandle is entered.");
    TLPS_PRINT2LAYER_INFO(NAS_EMM_TAU_RcvDataCnfNoRfHandle_ENUM, LNAS_ENTRY);

    /*为了保证快速发起流程，在TAU_REQ失败原因为NO_RF时，尝试计数不加*/
    ...
    if (NAS_EMM_TAU_START_CAUSE_ESM_EMC_PDN_REQ == NAS_EMM_TAU_GetEmmTAUStartCause())
    {
        NAS_EMM_TAU_LOG_INFO("NAS_EMM_TAU_RcvDataCnfNoRfHandle:CAUSE_ESM_EMC_PDN_REQ");
        TLPS_PRINT2LAYER_INFO(NAS_EMM_TAU_RcvDataCnfNoRfHandle_ENUM,LNAS_FUNCTION_LABEL1);
        NAS_EMM_EmcPndReqTauAbnormalCommProc(   NAS_EMM_MmcSendTauActionResultIndOthertype,
                                                (VOS_VOID*)&ulTauRslt,
                                                EMM_SS_DEREG_NORMAL_SERVICE);
    ...
    /*如果当前状态转到了REG+REG_ATTEMPT_TO_UPDATE,则给LRRC发送释放*/
    ulCurEmmStat = NAS_LMM_PUB_COMP_EMMSTATE(NAS_EMM_CUR_MAIN_STAT,
                                                NAS_EMM_CUR_SUB_STAT);

    if(ulCurEmmStat == NAS_LMM_PUB_COMP_EMMSTATE(EMM_MS_REG, EMM_SS_REG_ATTEMPTING_TO_UPDATE))
    {
        NAS_EMM_RelReq(NAS_LMM_NOT_BARRED);
    }
    ...
    *pulIsDelBuff = VOS_TRUE;
    ...
    switch (pstRrcMmDataCnf->enSendRslt)
    {
        ...
        case LRRC_LMM_SEND_RSLT_NO_RF:
            if(PS_TRUE == NAS_LMM_GetDsdsOptimizeFlag())
            {
                NAS_EMM_TAU_RcvDataCnfNoRfHandle();
            }
            break;
