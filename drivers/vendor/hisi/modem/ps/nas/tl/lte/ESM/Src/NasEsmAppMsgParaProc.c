...
VOS_VOID NAS_ESM_AppIpv6InfoNotifyMsgProc(VOS_VOID* pRcvMsg)
{
    ...
    {
        /* 支持1X或EHRPD接入技术才通知EHSM */
        if (NAS_ESM_SUCCESS == NAS_EMM_IsEhrpdOr1XRatExist())
        {
            NAS_ESM_SndEhsmSyncPdnInfoModIndMsg(ucEpsbId);
        }
    }
#endif
    ...
    /* 删除给APS上报的bearer_info_ind,此前GUL模式APS通知ESM IPV6前缀，ESM收到前缀后将
        IPV6地址(前缀加接口标识符)报给APS，但是CL模式，收到该消息后会认为是进行了C2L
        流程。现在改为不通知APS该消息，APS自己保存IPV6地址(此前APS不保存，等ESM通知
        IPV6地址)。*/
