...
)
{
    ...
    stOmMtaAssistData.ulLength = sizeof(NAS_LPP_OM_MTA_ASSIST_DATA_STRU) - VOS_MSG_HEAD_LENGTH;
...
)
{
    ...
    stOmMtaLocInfoReq.ulLength = sizeof(NAS_LPP_OM_MTA_LOC_INFO_REQ_STRU) - VOS_MSG_HEAD_LENGTH;
...
/*****************************************************************************
 Function Name   : NAS_LPP_OmMtaLocInfoCnf
 Description     :
 Input           : None
 Output          : None
 Return          : VOS_UINT32
 ...
 History         :
    1.wangensheng      2016-04-23  Draft Enact

*****************************************************************************/
VOS_VOID  NAS_LPP_OmMtaLocInfoCnf
(
    MTA_LPP_GNSS_MEAS_LIST_STRU        *pstGnssMeasList
)
{
    NAS_LPP_OM_MTA_LOC_INFO_CNF_STRU    stMtaLocInfoCnf         = {0};
    VOS_UINT32                          ulGnssCnt               = 0;
    VOS_UINT32                          ulSgnCnt                = 0;
    VOS_UINT32                          ulSatCnt                = 0;
    MTA_LPP_GNSS_SGN_MEAS_ELEMENT_STRU *pstGNSSSgnMeasList      = NAS_LPP_NULL_PTR;
    MTA_LPP_GNSS_MEAS_FOR_ONE_GNSS_STRU    *pstGNSSMeasList    = NAS_LPP_NULL_PTR;
    if (NAS_LPP_NO_SLCT == NAS_LPP_GetLppLayerSwitchFlag())
    {
        return;
    }

    NAS_LPP_WRITE_OM_MSG_HEAD(&stMtaLocInfoCnf,ID_NAS_LPP_OM_MTA_LOC_INFO_IND);

    stMtaLocInfoCnf.ulLength = sizeof(NAS_LPP_OM_MTA_LOC_INFO_CNF_STRU) - VOS_MSG_HEAD_LENGTH;

    for (ulGnssCnt = 0;ulGnssCnt < pstGnssMeasList->ulGNSSMeasListCnt; ulGnssCnt++ )
    {
        stMtaLocInfoCnf.ulGnssIndex = ulGnssCnt;
        pstGNSSMeasList = pstGnssMeasList->pastGNSSMeasListArray[ulGnssCnt];
        stMtaLocInfoCnf.ulGnssId = pstGNSSMeasList->stGNSSId.enGnssId;
        for (ulSgnCnt = 0; ulSgnCnt < pstGNSSMeasList->stGNSSSgnMeasList.ulGNSSSgnMeasListCnt;ulSgnCnt++)
        {
            pstGNSSSgnMeasList = &(pstGNSSMeasList->stGNSSSgnMeasList.astGNSSSgnMeasListArray[ulSgnCnt]);
            stMtaLocInfoCnf.ulSignalIndex = ulSgnCnt;
            stMtaLocInfoCnf.ulSignalID = pstGNSSSgnMeasList->stGNSSSignalId.ucGNSSSignalId;
            stMtaLocInfoCnf.ulGNSSCodePhaseAmbiguity = pstGNSSSgnMeasList->ulGNSSCodePhaseAmbiguity;
            for (ulSatCnt = 0;ulSatCnt < pstGNSSSgnMeasList->stGNSSSatMeasList.ulGNSSSatMeasListCnt;ulSatCnt++)
            {
                stMtaLocInfoCnf.ulGNSSMeasListIndex = ulSatCnt;
                NAS_LPP_MEM_CPY_S(&(stMtaLocInfoCnf.stGNSSSatMeasList),
                                  sizeof(MTA_LPP_GNSS_SAT_MEAS_ELEMENT_STRU),
                                  &(pstGNSSSgnMeasList->stGNSSSatMeasList.astGNSSSatMeasListArray[ulSatCnt]),
                                  sizeof(MTA_LPP_GNSS_SAT_MEAS_ELEMENT_STRU));
                (VOS_VOID)LTE_MsgHook( (VOS_VOID*)(&stMtaLocInfoCnf) );
            }
        }
    }
