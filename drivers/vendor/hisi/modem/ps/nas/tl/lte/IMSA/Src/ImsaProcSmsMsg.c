    ...
    if(IMSA_SMS_SMR_STATE_IDLE == pstSmrMoEntity->enState)
    {
        ...
        switch (IMSA_DsdsGetResourceStatus(RRM_PS_TASK_TYPE_LTE_IMS_MO_SMS))
        {
            case IMSA_RESOURCE_STATUS_ACTIVE:
                ...
                /* 如果读取到了卡中存储的SIP URI，则将它作为短信中心配置给IMS */
                if ((VOS_TRUE == pstImsaSipUri->ucReadSipUriFlag)
                 && (VOS_TRUE == pstImsaSipUri->ucNvReadSipUriFlag))
                {
                    IMSA_MEM_CPY_S( acSmsc,
                                    sizeof(acSmsc),
                                    pstImsaSipUri->acSipUri,
                                    sizeof(acSmsc));
                }
                else
                {
