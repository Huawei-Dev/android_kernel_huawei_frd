...
,LNAS_FUNCTION_LABEL1);
    ...
    */
    ...
    {
        /* 若是list搜索(包括搜网优化5.0中改后的3种list搜) , 则回复list拒绝 */
        if( (LRRC_LNAS_PLMN_SEARCH_USER_LIST == ulPlmnType)
            ||(LRRC_LNAS_PLMN_SRCH_INTER_LIST_FULL_BAND == ulPlmnType)
            ||(LRRC_LNAS_PLMN_SRCH_INTER_LIST_PREF_BAND == ulPlmnType))
        {
            NAS_LMM_EMMC_LOG_INFO(" NAS_EMMC_RcvMmcPlmnSrchReq: PLMN LIST TYPE IS REJECT!");
            TLPS_PRINT2LAYER_INFO(NAS_EMMC_RcvMmcPlmnSrchReq_ENUM
...
,LNAS_FUNCTION_LABEL4);
    ...
    {
        /* 三种list搜网类型都响应 */
        if( (MMC_LMM_PLMN_SRCH_USER_LIST != pstMmcPlmnReq->enSrchType)
            &&(MMC_LMM_PLMN_SRCH_INTER_LIST_FULL_BAND != pstMmcPlmnReq->enSrchType)
            &&(MMC_LMM_PLMN_SRCH_INTER_LIST_PREF_BAND != pstMmcPlmnReq->enSrchType))
        {
...
,LNAS_FUNCTION_LABEL5);
    ...
    if ( (pstMmcPlmnReq->enSrchType != MMC_LMM_PLMN_SRCH_USER_LIST)
        &&(pstMmcPlmnReq->enSrchType != MMC_LMM_PLMN_SRCH_INTER_LIST_FULL_BAND)
        &&(pstMmcPlmnReq->enSrchType != MMC_LMM_PLMN_SRCH_INTER_LIST_PREF_BAND))
    {
...
,LNAS_FUNCTION_LABEL1,NAS_EMM_GetConnState());
    ...
    NAS_EMM_SendLppCellChangeInd(pstSysInfoInd->ulCellId,
                                 ...
                                 pstSysInfoInd->aulLteBand);
    #endif
    #if (FEATURE_ON == FEATURE_UE_MODE_CDMA)
    if(PS_TRUE == pstSysInfoInd->enCsfbSuppForDualRxUEsFlg)
    {
        NAS_EMMC_SetCsfbSuppForDualRxUEsFlg(PS_TRUE);
    }
    else
    {
        NAS_EMMC_SetCsfbSuppForDualRxUEsFlg(PS_FALSE);
    }
    #endif
...
VOS_UINT32  NAS_EMMC_RcvMmcIntersysStartNTF(MsgBlock *pMsg)
{
    NAS_EMM_PUBU_LOG_NORM("NAS_EMMC_RcvMmcIntersysStartNTF is entered");
    TLPS_PRINT2LAYER_INFO(NAS_EMMC_RcvMmcIntersysStartNTF_ENUM,LNAS_ENTRY);
