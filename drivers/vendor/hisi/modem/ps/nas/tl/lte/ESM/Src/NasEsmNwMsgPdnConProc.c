...
)
{
    ...
    if(NAS_ESM_SUCCESS == NAS_ESM_ProcHandoverAttachNwMsgEsmCause(ulStateTblIndex))
    {
        return;
    }
    #endif
...
)
{
    ...
    /* 自研CL:如果ATTACH过程中发起的是handover的PDN连接请求，清除所有EHRPD承载
       若再次发起attach则发起initial类型的attach */
    #if (FEATURE_ON == FEATURE_UE_MODE_CDMA)
    if(NAS_ESM_SUCCESS == NAS_ESM_ProcHandoverAttachNwMsgEsmCause(ulStateTblIndex))
    {
        return;
...
)
{
    ...
    if(NAS_ESM_SUCCESS == ulIsEhPdnHandover)
    {
        ...
        /*在承载上下文中设置IPV6的前缀*/
        NAS_ESM_SetIpv6PrefixIntoBearCtxt(pstEhrpdEntity,stEncodeInfo.ulEpsbId);
...
VOS_UINT32 NAS_ESM_ProcHandoverAttachNwMsgEsmCause
(
    VOS_UINT32                          ulStateTblIndex
)
{
    ...
    NAS_ESM_INFO_LOG("NAS_ESM_ProcHandoverAttachNwMsgEsmCause is entered.");
    TLPS_PRINT2LAYER_INFO(NAS_ESM_ProcHandoverAttachNwMsgEsmCause_ENUM, LNAS_ENTRY);

    pstStateAddr = NAS_ESM_GetStateTblAddr(ulStateTblIndex);

    /* 恢复承载才处理*/
    if((NAS_ESM_PDN_REQ_TYPE_HANDOVER == pstStateAddr->enPdnReqType)
        &&(NAS_ESM_SUCCESS == NAS_ESM_GetEhrpdPdnEntityIndexBasedCid(pstStateAddr->ulCid, &ulIndex)))
    {
        NAS_ESM_INFO_LOG("NAS_ESM_ProcHandoverAttachNwMsgEsmCause is handover bearer.");
        TLPS_PRINT2LAYER_INFO(NAS_ESM_ProcHandoverAttachNwMsgEsmCause_ENUM, LNAS_FUNCTION_LABEL1);

        /* 如果ATTACH过程中发起的是handover的PDN连接请求，清除所有EHRPD承载
            若再次发起attach则发起initial类型的attach */
        if(NAS_ESM_PS_REGISTER_STATUS_ATTACHED != NAS_ESM_GetEmmStatus())
        {

            NAS_ESM_INFO_LOG("NAS_ESM_ProcHandoverAttachNwMsgEsmCause is handover attach.");
            TLPS_PRINT2LAYER_INFO(NAS_ESM_ProcHandoverAttachNwMsgEsmCause_ENUM, LNAS_FUNCTION_LABEL2);
            ...
            /* 通知RABM清除所有承载资源 */
            NAS_ESM_SndEsmRabmClearClBearerNotifyMsg(PS_FALSE);
...
VOS_VOID  NAS_ESM_SetIpv6PrefixIntoBearCtxt
(
    NAS_ESM_EHRPD_PDN_ENTITY_STRU      *pstEhrpdEntity,
    VOS_UINT32                          ulEpsbId
)
{
    NAS_ESM_EPSB_CNTXT_INFO_STRU       *pstEpsbCntxtInfo    = VOS_NULL_PTR;
    ...
    /*打印进入该函数*/
    NAS_ESM_INFO_LOG("NAS_ESM_SetIpv6PrefixIntoBearCtxt is entered.");
    TLPS_PRINT2LAYER_INFO(NAS_ESM_SetIpv6PrefixIntoBearCtxt_ENUM, LNAS_ENTRY);

    if((ulEpsbId < NAS_ESM_MIN_EPSB_ID)
        ||(ulEpsbId > NAS_ESM_MAX_EPSB_ID))
    {

        NAS_ESM_LOG1("NAS_ESM_SetIpv6PrefixIntoBearCtxt:ulEpsbId is invalid! ulEpsbId =", ulEpsbId);
        TLPS_PRINT2LAYER_INFO1(NAS_ESM_SetIpv6PrefixIntoBearCtxt_ENUM, LNAS_ERROR, ulEpsbId);
        return;
    ...
    /*拷贝EHRPD中的IPV6前缀到承载上下文中*/
    if((EHSM_ESM_PDN_TYPE_IPV6 == pstEhrpdEntity->stEhPdnContextInfo.stPdnAddr.enPdnType)
        ||(EHSM_ESM_PDN_TYPE_IPV4V6 == pstEhrpdEntity->stEhPdnContextInfo.stPdnAddr.enPdnType))
    {
        NAS_ESM_MEM_CPY_S(  pstEpsbCntxtInfo->stPdnAddrInfo.aucIpV6Addr,
                            APP_MAX_IPV6_PREFIX_LEN,
                            pstEhrpdEntity->stEhPdnContextInfo.stPdnAddr.aucIpv6Addr,
                            APP_MAX_IPV6_PREFIX_LEN);
    }
