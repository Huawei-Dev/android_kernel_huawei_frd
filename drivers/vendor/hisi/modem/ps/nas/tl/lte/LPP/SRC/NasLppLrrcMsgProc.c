...
)
{
    ...
    switch(enLcsMeasRslt)
    {
        ...
        case LRRC_LPP_LCS_MEAS_PARAM_INVALID_ERROR:
            /*对于测量cnf失败的场景，根据不通的类型的错误，分别处理*/
            /*本阶段对应LRRC上报的测量cnf fail的原因值统一释放对应的实体即可*/
...
(
    ...
    LPP_ECID_PROVIDE_LOCATION_INFO_STRU     *pstEcidProvideLocInfo
)
{
    ...
    static VOS_UINT32               ulHaveEncodeCellNum = 0;

    pstMeasRslt = NAS_LPP_GetMeasRsltAddr();
    ulCellNum   = pstMeasRslt->stEcidMeasRslt.stLrrcLppEcidMeasResultList.ulCellNum;
    ...
    /*RxTxTimeDiff&SFN*/
    /*E-SMLC请求了RXTXDIFF的测量*/
    if (0 != (ucLocReqLocFlag & 0x01))
    {
        /*ECID上报的小区计数加1*/
        ulHaveEncodeCellNum++;

        /*LRRC上报了的RXTXDIFF测量而且有效, LRRC能力上报的时候也是支持RXTXDIFF的测量*/
        if((NAS_LPP_SLCT == pstLrrcLppMeasRslt->ucUeRxTxTimeDiffMeasFlag)
           && (VOS_TRUE == pstLrrcSupportCap->bRrcCapbilityFlag)
           && (LPP_LRRC_INFO_FLAG_VALID == pstLrrcSupportCap->stRrcCapbility.ucUeRxTxTimeDiffMeasFlag))
        {
        ...
        {
            /*
             This field provides a ECID specific error cause. If the cause value is 'notAllRequestedMeasurementsPossible',
             the target device was not able to provide all requested ECID measurements (but may be able to provide some measurements).
             In this case, the  target device should include any of the 'rsrpMeasurementNotPossible', 'rsrqMeasurementNotPossible',
             or 'ueRxTxMeasurementNotPossible' fields, as applicable
             对于上述逻辑，仅对RXTXDIFF有效，因为rsrp，rsrq是始终有效的，不存在没有的情况
            */
            if(ulHaveEncodeCellNum == ulCellNum)
            {
                /*只有满足条件，请求了rxtx，而且是最后一个小区正常测量编码完成之后，编码ecid error项*/
                pstEcidProvideLocInfo->bitOpEcidError = NAS_LPP_SLCT;
                pstEcidProvideLocInfo->stEcidError.ulChoice = LPP_ECID_TARGET_DEVICE_ERROR_CAUSE_CHOSEN;
                pstEcidProvideLocInfo->stEcidError.u.stTargetDeviceErrorCauses.enEcidTargetDeviceErrorCause =
                                LPP_ECID_TARGET_DEVICE_ERROR_CAUSE_NOT_ALL_REQ_POSSIBLE;
                pstEcidProvideLocInfo->stEcidError.u.stTargetDeviceErrorCauses.bitOpRxTxMeasNotPossible = NAS_LPP_SLCT;
            }
        }
    }
...
)
{
    ...
    pstProvideLocR9Ies->stAGNSSPRovideLocationInfo.bitOpGNSSSignalMeasInfo =
        pstGnssMeasRslt->stGnssReqLocationInfo.bitOpGNSSSignalMeasInfo;

    pstProvideLocR9Ies->stAGNSSPRovideLocationInfo.bitOpGNSSError =
        pstGnssMeasRslt->stGnssReqLocationInfo.bitOpGNSSError;
     ...
     &&(NAS_LPP_SLCT == pstGnssMeasRslt->bitAgnssInfo))
    {
        ...
        NAS_LPP_MEM_CPY_S(&(pstProvideLocR9Ies->stAGNSSPRovideLocationInfo.stGNSSError),
                          sizeof(LPP_GNSS_ERROR_STRU),
                         &(pstGnssMeasRslt->stGnssReqLocationInfo.stGNSSError),
                         sizeof(LPP_MTA_COMMON_GNSS_ERROR_STRU));
    ...
    if(1 == ulCellNum)
    {
        ...
        */
        ...
        NAS_LPP_PrePareEcidParameterElement(pstNeighberMeasRsltEle,
                                            pstLrrcLppMeasRslt,
                                            pstLocationReqBuff,
                                            &(pstProvideLocR9Ies->stEcidPRovideLocationInfo));
...
{
    LPP_OTDOA_SIGNAL_MEAS_INFO_STRU        *pstOtdoaSignalMeasInfo  = NAS_LPP_NULL_PTR;
    NAS_LPP_MEAS_RESULT_STRU               *pstMeasRslt             = NAS_LPP_NULL_PTR;
    VOS_UINT16                              ausErrorCause[LRRC_LPP_OTDOA_TARGET_DEVICE_ERROR_CAUSES_BUFF] =
                                           {LPP_OTDOA_LOCATION_TARGET_DEVICE_ERROR_UNDEFINED,
                                            LPP_OTDOA_LOCATION_TARGET_DEVICE_ERROR_UNDEFINED,
    ...
    if (LRRC_LPP_OTDOA_MEAS_NO_ERROR != pstMeasRslt->stOtdoaMeasRslt.usError)
    {
        ...
        pstProvideLocR9Ies->stOtdoaPRovideLocationInfo.stOtdoaError.u.stTargetDeviceErrorCauses.enOtoaTargetDeviceErrorCause
            = ausErrorCause[pstMeasRslt->stOtdoaMeasRslt.usError];
        pstProvideLocR9Ies->bitOpOtdoaPRovideLocationInfo = NAS_LPP_SLCT;
        if (LPP_OTDOA_LOCATION_TARGET_DEVICE_ERROR_ATTEMP_BUT_UNABLE_TO_MEAS_SOME_NEIGHBOUR_CELL !=
            pstProvideLocR9Ies->stOtdoaPRovideLocationInfo.stOtdoaError.u.stTargetDeviceErrorCauses.enOtoaTargetDeviceErrorCause)
        {
            TLPS_PRINT2LAYER_INFO(NAS_LPP_SendUplinkMsg_ENUM, LNAS_LPP_ConnStatusConnect);
            return;
        }
    }
...
)
{
    ...
    */
    ...
    if((PS_TRUE == pstMeasRslt->stEcidMeasRslt.bHaveEcidMeasFlag)
       && (0 != pstMeasRslt->stEcidMeasRslt.stLrrcLppEcidMeasResultList.ulCellNum))
    {
...
)
{
    ...
    {
        if (LRRC_LPP_OTDOA_TARGET_DEVICE_ERROR_CAUSES_BUFF != pstMeasRslt->stOtdoaMeasRslt.usError)
        {
...
)
{
    ...
    NAS_LPP_TRANSACTION_ELEMENT_ENTITY_STRU *pstTransElementEntity  = NAS_LPP_NULL_PTR;
    NAS_LPP_LOC_TECH_MAP_STRU               *pstRuningLocTech       = NAS_LPP_NULL_PTR;
    ...
    pstRuningLocTech = NAS_LPP_SearchRunningPositionTech(NAS_LPP_POSITION_TECH_GNSS);
    ...
    if ((NAS_LPP_SLCT == pstLppNvParaConfig->bitOpPeriodProCtrlFlag)
     &&((NAS_LPP_REPORT_LOCATION_TYPE_PERIOD == pstLppCommCfgInfo->stReportCriteria.enType)
     &&(2000 >= pstLppCommCfgInfo->stReportCriteria.u.stReportPeriod.ulReportIntraval))
     &&((NAS_LPP_NULL_PTR != pstRuningLocTech)
     &&(ucTransIndx == pstRuningLocTech->ucTransIndx)))
    {
        pstUplinkProvideLoc->bitOpAcknowledgement = NAS_LPP_NO_SLCT;
        pstLppMsgIdentity->enUplinkMsgType        = NAS_LPP_UPLINK_MSG_TYPE_NO_NEED_MSG_CNF;
    }
    else
    {
        pstUplinkProvideLoc->bitOpAcknowledgement               = NAS_LPP_SLCT;
        pstUplinkProvideLoc->stAcknowledgement.bAckRequested    = PS_TRUE;
        pstLppMsgIdentity->enUplinkMsgType                      = NAS_LPP_UPLINK_MSG_TYPE_NEED_ACK_CNF;
    }
    ...
    {
        /* 预编译定位技术数据代码，此时可能会出错，如果发生编码出错，则将对应定位技术
           数据设置为error， */
        ...
        {
            usBitPos = 0;
            pstMeasRslt->stEcidMeasRslt.bHaveEcidMeasFlag = PS_FALSE;
            pstProvideLocR9Ies->bitOpEcidPRovideLocationInfo = NAS_LPP_SLCT;
            pstProvideLocR9Ies->stEcidPRovideLocationInfo.bitOpEcidError = NAS_LPP_SLCT;
            pstProvideLocR9Ies->stEcidPRovideLocationInfo.stEcidError.ulChoice = LPP_ECID_TARGET_DEVICE_ERROR_CAUSE_CHOSEN;
            pstProvideLocR9Ies->stEcidPRovideLocationInfo.stEcidError.u.stTargetDeviceErrorCauses.enEcidTargetDeviceErrorCause
                                = LPP_ECID_TARGET_DEVICE_ERROR_CAUSE_NOT_ALL_REQ_POSSIBLE;
            pstProvideLocR9Ies->stEcidPRovideLocationInfo.bitOpEcidSignalMeasInfo = NAS_LPP_NO_SLCT;
        }
    }
    ...
    {
        /* 预编译定位技术数据代码，此时可能会出错，如果发生编码出错，则将对应定位技术
           数据设置为error， 这样不会影响其他定位技术的上报，同时保证编码通过 */
        ...
        {
            usBitPos = 0;
            pstMeasRslt->stGnssMeasRslt.bHaveGnssMeasFlag = PS_FALSE;
            pstProvideLocR9Ies->bitOpAGNSSPRovideLocationInfo = NAS_LPP_SLCT;
            pstProvideLocR9Ies->stAGNSSPRovideLocationInfo.bitOpGNSSError = NAS_LPP_SLCT;
            pstProvideLocR9Ies->stAGNSSPRovideLocationInfo.stGNSSError.ulChoice = LPP_TARGET_DEVICE_ERROR_CAUSE_CHOSEN;

            pstProvideLocR9Ies->stAGNSSPRovideLocationInfo.stGNSSError.u.stTargetDeviceErrorCauses.enGNSSTargetDeviceErrorCause =
                LPP_GNSS_LOCATION_TARGET_DEVICE_ERROR_UNDEFINED;

            pstProvideLocR9Ies->stAGNSSPRovideLocationInfo.bitOpGNSSLocationInfo = NAS_LPP_NO_SLCT;
            pstProvideLocR9Ies->stAGNSSPRovideLocationInfo.bitOpGNSSSignalMeasInfo = NAS_LPP_NO_SLCT;
        }
    }
       ...
       &&(VOS_FALSE == NAS_LPP_IsOtdoaAssistDataChng() ) )
    {
        ...
        if (NAS_LPP_ENCODE_SUCC != usRslt)
        {
            ...
            pstProvideLocR9Ies->stOtdoaPRovideLocationInfo.bitOpOtdoaSignalMeasInfo = NAS_LPP_NO_SLCT;
            pstProvideLocR9Ies->bitOpOtdoaPRovideLocationInfo = NAS_LPP_SLCT;
            pstProvideLocR9Ies->stOtdoaPRovideLocationInfo.bitOpOtdoaError = NAS_LPP_SLCT;
            pstProvideLocR9Ies->stOtdoaPRovideLocationInfo.stOtdoaError.ulChoice = LPP_OTDOA_TARGET_DEVICE_ERROR_CAUSE_CHOSEN;
            pstProvideLocR9Ies->stOtdoaPRovideLocationInfo.stOtdoaError.u.stTargetDeviceErrorCauses.enOtoaTargetDeviceErrorCause
    ...
    *pulLppMsgLen = usBitPos/8;
    ...
    pstLppMsgIdentity->bIsProvideLocFalg                = PS_TRUE;
...
)
{
    ...
    {
        NAS_LPP_ERR_LOG("NAS_LPP_EncodeProvideLocationMsgWithIndentityInfo: trans index fail!");
        TLPS_PRINT2LAYER_ERROR(NAS_LPP_CmplOtdoaLocationInfo_ENUM,LNAS_LPP_GetTransIdIndexFail);
        return ;
    }
    ...
    {
        TLPS_PRINT2LAYER_ERROR(NAS_LPP_CmplOtdoaLocationInfo_ENUM,LNAS_LPP_Otdoa_Not_Running);
        return;
    }

    /* 如果当前transaction不是启动OTDOA的transaction，则直接返回，不再发送位置信息 */
    if (ucTransIndx != pstLocTechMap->ucTransIndx)
    {
        TLPS_PRINT2LAYER_ERROR(NAS_LPP_CmplOtdoaLocationInfo_ENUM,LNAS_LPP_CurrentTransNotRunningOtdoa);
        return;
    }
...
)
{
    ...
    if (stPosSearchRslt.ucNum == usMeasReadyNum)
    {
        ...
        /* 停止上报定时器 */
        NAS_LPP_TimerStop( ucSessionIndx, ucSessionTransIndx, TI_NAS_LPP_RESPONSE_TIMER);
    }
}
...
)
{
    ...
    switch(enReprotLocationType)
    {
        ...
        case NAS_LPP_REPORT_LOCATION_TYPE_ONCE:
            ...
            pstTransEntity = NAS_LPP_GetTransElementByIndex(ucTransIndx);
            if ((NAS_LPP_NULL_PTR == pstTransEntity)
              ||(LPP_SS_WAIT_LOCATION_CN_ACK == pstTransEntity->enLppSubState))
            {
                return;
            }
...
)
{
    ...
    switch ( pstOtdoaMeasInd->usError )
    {
        case LRRC_LPP_OTDOA_TARGET_DEVICE_UNDEFINED:
        case LRRC_LPP_OTDOA_ASSISTANCE_DATA_MISSING:
            /* 跟物理层确认这三种错误原因值比较严重，或目前不可能出现，给网侧发送abort消息 */
        case LRRC_LPP_OTDOA_CONFIG_PHY_FAIL:
            /* 这个原因值为RRC发生小区切换后恢复OTDOA测量时配置物理层失败，回复此枚举值,当前暂不做处理，后续完善此部分 */
    ...
    pstOtdoaMeasRslt->usError = pstOtdoaMeasInd->usError;
...
VOS_VOID NAS_LPP_SaveLrrcReqLocReportType
(
    LRRC_LPP_LOC_REPORT_TYPE_ENUM_UINT8             enLocReportType
)
{

}

VOS_VOID NAS_LPP_RcvLrrcLppLocRequirementInd
(
    LRRC_LPP_LOC_REQUIREMENT_IND_STRU        *pstLocRequirementInd
)
{
    NAS_LPP_LOC_TECH_MAP_STRU           *pstLppLocMap       = NAS_LPP_NULL_PTR;

    NAS_LPP_INFO_LOG("NAS_LPP_RcvLrrcLppLocRequirementInd enter!");
    TLPS_PRINT2LAYER_INFO(NAS_LPP_RcvLrrcLppLocRequirementInd_ENUM,LNAS_LPP_Func_Enter);

    /*如果modem主动发起位置请求的NV没有打开，则直接返回*/
    if((NAS_LPP_SLCT != NAS_LPP_GetGnssModemOrigFlag())&&(PS_SUCC != LPS_OM_IsTestMode()))
    {
        NAS_LPP_ERR_LOG("NAS_LPP_RcvLrrcLppLocRequirementInd: Not allowed to get loc info!");
        TLPS_PRINT2LAYER_ERROR1(NAS_LPP_RcvLrrcLppLocRequirementInd_ENUM, LNAS_LPP_ModemOrigNvClose,NAS_LPP_GetGnssModemOrigFlag());
        return;
    }

    if (LRRC_LPP_LOC_REQUIREMENT_CMD_SETUP == pstLocRequirementInd->enLocRequirementCmd)
    {
        /*设置流程标识，表明当前是modem主动发起的定位流程 */
        NAS_LPP_SetGnssOrigType(NAS_LPP_GNSS_ORIG_TYPE_MODEM);

        /* 存储上报LRRC位置信息方式 */
        NAS_LPP_SetGnssReportType(pstLocRequirementInd->enLocReportType);
        ...
        if(NAS_LPP_NULL_PTR == pstLppLocMap)
        {
            ...
            NAS_LPP_ERR_LOG("NAS_LPP_RcvLrrcLppLocRequirementInd: No Running GNSS Entity!");
            TLPS_PRINT2LAYER_ERROR(NAS_LPP_RcvLrrcLppLocRequirementInd_ENUM, LNAS_LPP_NoRunningGnssEntity);

            /*发送请求给MTA，对应的AT+CPORR<pos_meas_req>到GPS */
            NAS_LPP_SndLppMtaStandaloneGnssMeasReq(pstLocRequirementInd->enLocReportType);
...
VOS_VOID NAS_LPP_LrrcMsgDistr( VOS_VOID *pRcvMsg )
{
    ...
    switch(pEmmMsg->ulMsgName)
    {
        ...
        case ID_LRRC_LPP_LOC_REQUIREMENT_IND:
            NAS_LPP_RcvLrrcLppLocRequirementInd( (LRRC_LPP_LOC_REQUIREMENT_IND_STRU *) pRcvMsg );
            break;
