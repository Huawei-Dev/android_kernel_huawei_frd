...
{
    if(NAS_LPP_UPLINK_MSG_TYPE_NO_NEED_MSG_CNF == pstLppMsgIdentity->enUplinkMsgType)
    {
        return PS_TRUE;
    }
...
VOS_UINT32  NAS_LPP_CheckCommomDownLinkMsg(LPP_MESSAGE_STRU *pstLppMsg)
{
       ...
       &&(LPP_SUPPORT_MESSAGE_BODY_REQUEST_LOCATION_INFO_CHOSEN == pstLppMsg->stLppMessageBody.u.stC1.ulChoice) )
    {
        ...
        {
            TLPS_PRINT2LAYER_ERROR(NAS_LPP_CheckCommomDownLinkMsg_ENUM, LNAS_LPP_CommonIEsReqLocationInfoNotSlt);
            /*
            Verify when "CommonIESRequestLocationInformantion" is NOT populated, the UE will generate Protocol error
            如果CommonIESRequestLocationInformantion IE不存在，则需要给网侧上报ERROR
            */
...
VOS_UINT32  NAS_LPP_CheckAssisData( LPP_MESSAGE_PROVIDE_ASSIST_DATA_STRU *pstProvideAssistanceData )
{
       ...
       &&(NAS_LPP_SLCT == pstProvideAssitDataR9->stOTDOAProvideAssistanceData.bitOpOtdoaNeighbourCellInfo) )
    {
        ...
        {
            /*OTDOA辅助数据可能分两次下发，先下发的参考小区，后下发邻区信息的情况，此时参考小区频点已经存储了*/
        ...
        {
            /*OTDOA辅助数据在一条消息里面下发，参考小区频点直接从参考小区去取即可*/
            ulRslt = NAS_LPP_CheckOtdoaNeighbourCell(&pstProvideAssitDataR9->stOTDOAProvideAssistanceData.stOtdoaNeighbourCellInfo,
...
VOS_UINT32  NAS_LPP_CheckOtdoaAssisDataAfterSave( VOS_VOID )
{
    ...
    /*
    统计辅助数据小区个数， 并且验证邻区中的PRS-INFO信元必须存在, Verify if either PRS for neghbour cell
    info  is missing in the OTDOA Assistance Data, the UE would generate an Error
    所以要满足以下两个条件:
    1. 至少有三个邻小区
    2. 每个邻区有PRS-INFO信元
    */
    for ( ulLoop = 0 ; ulLoop < pstOtdoaAssisCfg->stOtdoaNeighbourCellInfo.ulFreqInfoNum; ulLoop++ )
    {
        ...
        /* 如果临区频点与参考小区频点一致，则不对PRS信号数据做检查，直接将临区计数统计到总小区个数中 */
        if (pstOtdoaAssisCfg->stOtdoaReferenceCellInfo.ulEarfcn == pstNeighborFreqInfo->ulEarfcn)
        {
        ...
        else
        {
            ...
            {
                if(LPP_LRRC_INFO_FLAG_VALID == pstNeighborFreqInfo->stOtdoaNeighbourCellInfoList[ulLoop1].ucPrsInfoFlag)
                {
                    ulAssistDataCellNum += 1;
                }
            }
    ...
    {
        NAS_LPP_ERR_LOG("NAS_LPP_CheckOtdoaAssisDataAfterSave: Prs-info check And Cell Count Check fail!");
        TLPS_PRINT2LAYER_ERROR(NAS_LPP_CheckOtdoaAssisDataAfterSave_ENUM, LNAS_LPP_PrsInfoCheckAndCellCountCheckFail);
        return NAS_LPP_MSG_VERIFY_FAIL;
    }
...
VOS_UINT32  NAS_LPP_CheckLppMsgProcessRelativity
(
...
)
{
    ...
    VOS_UINT32                                  ulAirMsgType        = LPP_SUPPORT_MESSAGE_BODY_SPARE0_CHOSEN;
    ...
    NAS_LPP_INFO_LOG("NAS_LPP_CheckLppMsgProcessRelativity ENTER!");
    TLPS_PRINT2LAYER_INFO(NAS_LPP_CheckLppMsgProcessRelativity_ENUM, LNAS_LPP_Func_Enter);

    ucSessionIndx = NAS_LPP_SearchSessionIndx(ulSessionId);
    if(PS_NULL_UINT8 == ucSessionIndx)
    {
        /*假设该条消息对应的session不存在，则直接返回成功*/
        NAS_LPP_INFO_LOG("NAS_LPP_CheckLppMsgProcessRelativity: Session don't find!");
        TLPS_PRINT2LAYER_INFO(NAS_LPP_CheckLppMsgProcessRelativity_ENUM, LNAS_LPP_CanntFindSession);
        return NAS_LPP_MSG_VERIFY_SUCC;
    }

    /*session已经创建了，则说明该条消息不是该session中的第一条消息，则需要检查*/
    ...
    {
        /*如果session在，但是transaction不存在，则直接返回成功，无需检查*/
        NAS_LPP_INFO_LOG("NAS_LPP_CheckLppMsgProcessRelativity:  transaction don't find!");
        TLPS_PRINT2LAYER_INFO(NAS_LPP_CheckLppMsgProcessRelativity_ENUM, LNAS_LPP_CanntFindTransaction);
        return NAS_LPP_MSG_VERIFY_SUCC;
    }

    ulAirMsgType = pstLppMsg->stLppMessageBody.u.stC1.ulChoice;
    /*如果是error或者abort消息，直接返回成功，无条件接受该消息*/
    if((LPP_SUPPORT_MESSAGE_BODY_ABORT_CHOSEN == ulAirMsgType)
       ||(LPP_SUPPORT_MESSAGE_BODY_ERROR_CHOSEN == ulAirMsgType))
    {
        return NAS_LPP_MSG_VERIFY_SUCC;
    }

    /* 此处修改，增加重复消息规避问题，如果是重复消息，则相关性检查直接返回成功即可 */
    if ((NAS_LPP_SLCT == pstLppMsg->bitOpSequenceNumber)
      &&(pstSessionEntityPool->astLppSessionEntity[ucSessionIndx].ucLastDownSequenceNum
      == pstLppMsg->stSequenceNumber.ucSequenceNumber))
    {
        NAS_LPP_INFO_LOG("NAS_LPP_CheckLppMsgProcessRelativity:  Capabilitity msg recieved !");
        TLPS_PRINT2LAYER_INFO(NAS_LPP_CheckLppMsgProcessRelativity_ENUM, LNAS_ProcessMultipleReq);
        return NAS_LPP_MSG_VERIFY_SUCC;
    }

    switch(pstTransEntity->enLppMainState)
    {
        case LPP_MS_CAPABILITY:
            /*在主状态时能力请求的过程中，用相同的transactionID发送的其他消息
            除开ACK之外都是异常消息，走到这里，一定不是ack消息，所以直接返回error即可*/
            NAS_LPP_INFO_LOG("NAS_LPP_CheckLppMsgProcessRelativity:  Capabilitity Process conflict!");
            TLPS_PRINT2LAYER_INFO(NAS_LPP_CheckLppMsgProcessRelativity_ENUM, LNAS_LPP_CapabilitityProcessConflict);
            return NAS_LPP_PROCESS_RELATIVITY_CHECK_FAIL;

        case LPP_MS_ASSISTDATA:
            /*当前transaction是辅助数据流程*/
            if(LPP_SS_ASSISTDATA_REQ_WAIT_CN_ACK == pstTransEntity->enLppSubState)
            {
                /*当前transaction是LPP创建的，主动请求辅助数据等网络侧ACK，
                走到这里，说明一定不是ACK消息，直接返回error即可
                */
                if ((NAS_LPP_SLCT == pstLppMsg->bitOpAcknowledgement)
                  &&(LPP_SUPPORT_MESSAGE_BODY_PROVIDE_ASSISTANCE_DATA_CHOSEN == ulAirMsgType))
                {
                    NAS_LPP_INFO_LOG("NAS_LPP_CheckLppMsgProcessRelativity:  Assistdata Process ack conflict!");
                    TLPS_PRINT2LAYER_INFO(NAS_LPP_CheckLppMsgProcessRelativity_ENUM, LNAS_LPP_MSGWithACK);
                    return NAS_LPP_MSG_VERIFY_SUCC;
                }
                else
                {
                    NAS_LPP_INFO_LOG("NAS_LPP_CheckLppMsgProcessRelativity:  Assistdata Process  no ack conflict!");
                    TLPS_PRINT2LAYER_INFO(NAS_LPP_CheckLppMsgProcessRelativity_ENUM, LNAS_LPP_AssistDataProcessACKConflict);
                    return NAS_LPP_PROCESS_RELATIVITY_CHECK_FAIL;
                }
            }

            if(LPP_SS_ASSISTDATA_WAIT_CN_CNF == pstTransEntity->enLppSubState)
            {
                /*当前子状态时等待辅助数据回复，有两种情况，一种是网侧主动下发辅助
                多条的场景，一种是UE主动请求网侧下发多条的场景，如果下发消息不是辅助
                数据，则认为是错误*/
                if(LPP_SUPPORT_MESSAGE_BODY_PROVIDE_ASSISTANCE_DATA_CHOSEN != ulAirMsgType)
                {
                    NAS_LPP_INFO_LOG("NAS_LPP_CheckLppMsgProcessRelativity:  isn't AssistData");
                    TLPS_PRINT2LAYER_INFO(NAS_LPP_CheckLppMsgProcessRelativity_ENUM, LNAS_LPP_ConflictIsNotAssistData);
                    return NAS_LPP_PROCESS_RELATIVITY_CHECK_FAIL;
                }
            }
            break;

        case  LPP_MS_LOCATION:
            /*当前transaction是位置信息请求流程，对于同一transactionID的位置信息
            请求流程，唯一可能冲突也是ack消息，逻辑跟能力交互一致，直接返回error*/
            NAS_LPP_INFO_LOG("NAS_LPP_CheckLppMsgProcessRelativity:  Location Process conflict!");
            TLPS_PRINT2LAYER_INFO(NAS_LPP_CheckLppMsgProcessRelativity_ENUM, LNAS_LocationProcessConflict);
            return NAS_LPP_PROCESS_RELATIVITY_CHECK_FAIL;
...
VOS_UINT32  NAS_LPP_CheckDownLinkMsg
(
    LPP_MESSAGE_STRU           *pstLppMsg,
    VOS_UINT32                  ulSessionId
)
{
    ...
    /*消息的流程无关性检测，是LPP消息，需要检测该条消息是否是对应transaction对应期望的消息*/
    ulRslt = NAS_LPP_CheckLppMsgProcessRelativity(pstLppMsg, ulSessionId);
    if(NAS_LPP_MSG_VERIFY_SUCC != ulRslt)
    {
        NAS_LPP_ERR_LOG("NAS_LPP_CheckDownLinkMsg: Process Relativity check fail!");
        TLPS_PRINT2LAYER_ERROR(NAS_LPP_CheckDownLinkMsg_ENUM, LNAS_LPP_ProcessRelativityCheckFail);
        return ulRslt;
    }
...
)
{
    ...
    pstUplinkAckMsg->stTransactionId.enInitiator = pstLppHead->stTransactionId.enInitiator;
    pstUplinkAckMsg->stTransactionId.ucTransactionNumber = pstLppHead->stTransactionId.ucTransactionNumber;
#endif
    /*开始编码*/
    ...
    *pulLppMsgLen = usBitPos/8;
    ...
    pstLppMsgIdentity->enUplinkMsgType                  =
        NAS_LPP_UPLINK_MSG_TYPE_NO_NEED_MSG_CNF;
        ...
        for(ucLoop = 0; ucLoop < NAS_LPP_MAX_BUFF_MSG_COUNT; ucLoop++)
        {
            ...
            */
            if((pstUpMsgBufPool->astLppUpMsg[ucLoop].stLppMsgIdentity.ulSessionId == pstLppMsgIdentity->ulSessionId)
               ...
               && (NAS_LPP_UPLINK_MSG_TYPE_NEED_ACK_CNF == pstUpMsgBufPool->astLppUpMsg[ucLoop].stLppMsgIdentity.enUplinkMsgType)
...
)
{
    ...
    /* 启动等GNSS CAP CNF 定时器 */
    NAS_LPP_TimerStart(ucSessionIndx, ucSessionTransIndx, TI_NAS_LPP_WAIT_GNSS_CAP_CNF);
...
)
{
    ...
    if((NAS_LPP_SLCT == pstTransElement->stCapReq.bitOpAgnssCapReq)&&(NAS_LPP_SLCT == NAS_LPP_GetGnssSupFlag()))
    {
    ...
    *pulLppMsgLen = usBitPos/8;
    ...
    pstLppMsgIdentity->enUplinkMsgType                  =
        NAS_LPP_UPLINK_MSG_TYPE_NEED_ACK_CNF;
...
)
{
    ...
    else
    {
        ...
        /* ACK消息可能不带TRANSACTIONID，此时需要通过MsgBuff寻找对应的transaction，如果携带了则正常寻找即可 */
        if ((NAS_LPP_SLCT == pstLppMsgHead->bitOpAcknowledgement)
          &&(NAS_LPP_NO_SLCT == pstLppMsgHead->bitOpLppTransactionId))
        {
            /*查找上行缓存buffer*/
            pstUplinkMsgBuf = NAS_LPP_FindUpLinkBuffer(
                                            pstLppMsgHead->stAcknowledgement.stAckIndicator.ucSequenceNumber,
                                            ulSessionId);
            if(NAS_LPP_NULL_PTR == pstUplinkMsgBuf)
            {
                /*加保护如果等于空，则上行缓存中没有找到对应消息缓存*/
...
)
{
    ...
    /* 当请求了GPS能力，并且GPS芯片存在且没有能力数据时则获取GPS能力数据 */
    if((NAS_LPP_SLCT == pstTransEntity->stCapReq.bitOpAgnssCapReq)
     &&(PS_FALSE == pstGnssData->bGnssCapbilityFlag))
    {
        /* 如果能力数据异常丢失，保证能够根据能力数据获取方式再次获取 */

        if (NAS_LPP_SLCT == NAS_LPP_GetGnssSupFlag())
        {
            /* 根据GNSS能力获取方式获取GNSS能力数据 */
            ...
            {
                NAS_LPP_GNSSCapReq(ucSessionIndx, ucSessionTransIndx);
...
)
{
    ...
    VOS_UINT8                               ucResponseTimeAllowance = 0;
    ...
    ucResponseTimeAllowance = (VOS_UINT8)(NAS_LPP_GetResponseTimeAllowance() / 1000);
    pstMsgBuff->stLppMessageBody.u.stC1.u.stRequestLocationInformation.u.stC1.u.stReqLocationInfoR9.stCommonIEsReqLocationInfo.stQos.stResponseTime.ucTime -=
                ucResponseTimeAllowance;
...
)
{
    ...
    VOS_UINT8                                   ucSessionIndx       = 0;
    NAS_LPP_CONFIG_PARA_STRU                   *pstCfgPara          = NAS_LPP_NULL_PTR;
    VOS_BOOL                                    bEcidOrOtdoaSlt     = PS_FALSE;
    VOS_BOOL                                    bAGNSSSelect        = PS_FALSE;
    ...
    */
    ...
    /* 配置GNSS测量请求，如果GPS芯片存在 */
    if((NAS_LPP_SLCT == pstCfgPara->bitGnssReq)&&(NAS_LPP_SLCT == NAS_LPP_GetGnssSupFlag()))
    {
        /*迭代四再做，实现步骤如下:
        1. 发送GNSS测量请求给MTA
        2. 将当前的定位计数加1
    ...
    switch(NAS_LPP_GetSessionEntityReportType(ucSessionIndx,ucSessionTransIndx))
    {
        ...
        case NAS_LPP_REPORT_LOCATION_TYPE_PERIOD:
            ...
            周期性定时器
            此时如果ECID 或者OTDOA 没有被选中，那么GNSS需要启动周期上报定时器
            如果有ECID或者OTDOA被选择，在收到MEAS_CNF时会启动定时器，此处不需要启动
            */
            if ((PS_FALSE == bEcidOrOtdoaSlt)&&(PS_TRUE == bAGNSSSelect))
            {
                NAS_LPP_TimerStart(ucSessionIndx,
                                   ucSessionTransIndx,
...
)
{
    ...
    if (NAS_LPP_SLCT == pstOtdoaRefCellInfo->bitOpPrsInfo)
    {
        ...
        pstOtdoaRefCellCfg->stPrsInfo.ucPrsBandWidth = aucBandWidth[pstOtdoaRefCellInfo->stPrsInfo.enPrsBandWidth];
...
)
{
    ...
    if (NAS_LPP_SLCT == pstNgnbrCellElement->bitOpPrsInfo)
    {
        ...
        pstOtdoaNghbrCellCfg->stPrsInfo.ucPrsBandWidth      = aucBandWidth[pstNgnbrCellElement->stPrsInfo.enPrsBandWidth];
        pstOtdoaNghbrCellCfg->stPrsInfo.ucNumdlFrames       = aucNumFrams[pstNgnbrCellElement->stPrsInfo.enNumDlFrames];
...
)
{
    NAS_LPP_SERVING_CELL_STRU      *pstServingCell;
    NAS_LPP_OTDOA_ASSIST_CFG_STRU  *pstOtdoaAssisCfg;
    VOS_BOOL                        bSaveSuccflg            = NAS_LPP_FAILURE;
    ...
    /* 参考小区,临区辅助数据存在，则存储相应数据 */
    if (NAS_LPP_SLCT == pstOtdoaAssist->bitOpOtdoaReferenceCellInfo)
    {
        NAS_LPP_SaveOtdoaRefCell(pstOtdoaAssist,pstServingCell,pstOtdoaAssisCfg);
        bSaveSuccflg = NAS_LPP_SUCCESS;
    }
    if (NAS_LPP_SLCT == pstOtdoaAssist->bitOpOtdoaNeighbourCellInfo)
    {
...
)
{
    ...
    if (LPP_INITIATOR_TYPE_TARGET_DEVICE == pstTransEntity->stTransId.enInitiator)
    {
        ...
        pstGnssAssistDataCnf->enLppMtaAgnssError = LPP_MTA_AGNSS_ASSIST_PROCESS_SUCC;
...
)
{
    ...
    if ((NAS_LPP_SLCT == pstProvideAssitDataR9->bitOpAGNSSProvideAssistData)
      &&(NAS_LPP_SLCT == NAS_LPP_GetGnssSupFlag()))
    {

        /* 收到GNSS辅助数据时，确认是否有RRC请求位置信息流程正在running，如果有且没有网侧流程则释放RRC测量 */
        if (NAS_LPP_GNSS_ORIG_TYPE_MODEM == NAS_LPP_GetGnssOrigType())
        {
            /* 在释放测量时，如果网侧不存在则发送MEAS_ABORT,如果存在，则不做处理 */
...
)
{
    ...
    VOS_UINT32                               ulResponseTimeAllowance = 0;
    LPP_COMMON_IES_REQ_LOCATION_INFO_STRU   *pstCommonIEsReqLocationInfo = NAS_LPP_NULL_PTR;
    ...
    pstReqLocationInfo  = NAS_LPP_GetDownLinkLocationReqIEAddr(pstLppMsgBuff);
    pstCommonIEsReqLocationInfo = &(pstReqLocationInfo->stCommonIEsReqLocationInfo);
            ...
            &&(NAS_LPP_SLCT == pstCommonIEsReqLocationInfo->stQos.bitOpResponseTime))
    {
        ...
        ulResponseTimeAllowance = NAS_LPP_GetResponseTimeAllowance();
        if((NAS_LPP_SLCT == pstLppMsgBuff->stLppMessageBody.u.stC1.u.stRequestLocationInformation.u.stC1.u.stReqLocationInfoR9.bitOpAGNSSReqLocationInfo)
           && (0 != ulResponseTimeAllowance))
        {
            /*LPP上报的定时器的时间，参考对比机得上报逻辑，LPP上报预留的时间比给GPS的时间多300ms*/
            pstLppCommCfgInfo->stReportCriteria.u.stReportOnce.ulResponseTime
                = (VOS_UINT32)(((pstCommonIEsReqLocationInfo->stQos.stResponseTime.ucTime) *1000) - ulResponseTimeAllowance + 300);/*s转成ms*/
        }
        else
        {
            pstLppCommCfgInfo->stReportCriteria.u.stReportOnce.ulResponseTime =
                  (VOS_UINT32)(pstCommonIEsReqLocationInfo->stQos.stResponseTime.ucTime) *1000;
        }
    }
...
)
{
    ...
    if(NAS_LPP_SLCT == pstReqLocationInfo->bitOpCommonIEsReqLocationInfo)
    {
        ...
        NAS_LPP_SaveCommCfgInfo(pstLppMsg, pstLppCommCfgInfo);
    }
...
)
{
    ...
    switch ( ulDecodeFailRslt )
    {
        ...
        case NAS_LPP_PROCESS_RELATIVITY_CHECK_FAIL:
            ucErrorType = LPP_COMMON_IES_ERROR_LPP_MESSAGE_BODY_ERROR;
            break;
...
VOS_VOID  NAS_LPP_RcvLppEmmDataInd(LMM_LPP_DATA_IND_STRU *pRcvMsg )
{
    ...
    /*解码成功，下行消息asn的检测*/
    ulRslt = NAS_LPP_CheckDownLinkMsg(pstLppMsgBuff, pRcvMsg->ulSessionId);
    if(NAS_LPP_MSG_VERIFY_SUCC != ulRslt)
    {
...
VOS_UINT32  NAS_LPP_GetBandNumber(VOS_UINT32  *pulLteBand)
{
    VOS_UINT32                      i = 0;
    VOS_UINT32                      j = 0;
    VOS_UINT32                      ulLteBandNum = 0;

    /*
    pulLteBand数组中存储的每一个bit表示对应一个band值，
    e.g: pulLteBand[0]的值是4， 则对应的band值是3，
         pulLteBand[1]的值是4， 则对应的band值是32+3=35
    */
    for (i = 0; i < NAS_LPP_BAND_ARRAY_CNT; i++)
    {
        for (j = 0; j < NAS_LPP_PER_ARRAY_BAND_BITMAP_CNT; j++)
        {
            if (1 == ((pulLteBand[i] >> j) & NAS_LPP_LOW_ONE_BIT))
            {
                if(0 == i)
                {
                    ulLteBandNum = j + 1;
                ...
                {
                    ulLteBandNum = NAS_LPP_PER_ARRAY_BAND_BITMAP_CNT + j + 1;
                }
                TLPS_PRINT2LAYER_INFO1(NAS_LPP_GetBandNumber_ENUM, LNAS_LPP_Func_Enter, ulLteBandNum);
                return ulLteBandNum;
            }
        }
    }
...
VOS_UINT32  NAS_LPP_CalculateDlEarfcn
(
    VOS_UINT8                       ucFreqBandIndicator,
    VOS_UINT16                      usDlFreq,
    VOS_UINT32                     *pulDlEarfcn
)
{
    VOS_UINT32                      ulRslt = 0;

    ulRslt = RRC_COMM_CalculateDlEarfcn(ucFreqBandIndicator, usDlFreq, pulDlEarfcn);
...
VOS_VOID  NAS_LPP_RcvLppEmmCellChngInd( LMM_LPP_CELL_CHANGE_IND *pRcvMsg)
{
    ...
    ulRslt = NAS_LPP_CalculateDlEarfcn((VOS_UINT8)NAS_LPP_GetBandNumber(pRcvMsg->aulLteBand),
                                       (VOS_UINT16)pRcvMsg->ulDlFreq,
                                       &(pstServingCell->ulEarfcn));
    if(NAS_LPP_SUCCESS != ulRslt)
    {
        NAS_LPP_ERR_LOG("NAS_LPP_RcvLppEmmCellChngInd: Trans earfcn error!");
        TLPS_PRINT2LAYER_ERROR(NAS_LPP_RcvLppEmmCellChngInd_ENUM, LNAS_LPP_TransEarfcnError);
    }
     ...
     &&(PS_FALSE == pstGnssData->bGnssCapbilityFlag))
    {
        ...
        {
            NAS_LPP_GNSSCapReq(NAS_LPP_TIMER_INVAILD_PARA, NAS_LPP_TIMER_INVAILD_PARA);
        }
    }
    /*3是数组aucPlmnId的长度*/
...
VOS_BOOL NAS_LPP_IsExsitPostionTechInSession
(
    VOS_UINT8                           ucSessionIndx,
    NAS_LPP_POSITION_TECH_ENUM_UINT8    enPostionTech,
    VOS_UINT8                          *pucTransIndx,
    VOS_UINT8                          *pucSessionTransIndx
)
{
    ...
    pstLppLocMap = NAS_LPP_SearchRunningPositionTech(enPostionTech);
    if((NAS_LPP_NULL_PTR != pstLppLocMap)
       &&(ucSessionIndx == pstLppLocMap->ucSessionIndx))
    {
        *pucTransIndx = pstLppLocMap->ucTransIndx;
        *pucSessionTransIndx = pstLppLocMap->ucSessionTranIndx;
        return PS_TRUE;
    }
    else
    {
...
VOS_VOID NAS_LPP_RcvRelSessionHandle(VOS_VOID)
{
    NAS_LPP_SESSION_ENTITY_POOL_STRU   *pstSessionEntityPool = NAS_LPP_NULL_PTR;
    VOS_UINT32                          ulLoop               = 0;
    VOS_UINT32                          ulSessionId          = PS_NULL_UINT32;
    VOS_UINT8                           ucTransIndxGnss      = PS_NULL_UINT8;
    VOS_UINT8                           ucTransIndxOtdoa     = PS_NULL_UINT8;
    VOS_UINT8                           ucTransIndxEcid      = PS_NULL_UINT8;
    VOS_UINT8                           ucSessionTransIndxGnss  = PS_NULL_UINT8;
    VOS_UINT8                           ucSessionTransIndxOtdoa = PS_NULL_UINT8;
    VOS_UINT8                           ucSessionTransIndxEcid  = PS_NULL_UINT8;
    ...
    for ( ulLoop = 0 ; ulLoop < NAS_LPP_MAX_SESSION_COUNT; ulLoop++ )
    {
        ...
        {
            /*当前session有GNSS的定位技术*/
            if(PS_TRUE == NAS_LPP_IsExsitPostionTechInSession(ulLoop, NAS_LPP_POSITION_TECH_GNSS, &ucTransIndxGnss, &ucSessionTransIndxGnss))
            {

                if(PS_TRUE == NAS_LPP_IsExsitPostionTechInSession(ulLoop, NAS_LPP_POSITION_TECH_OTDOA, &ucTransIndxOtdoa, &ucSessionTransIndxOtdoa))
                {
                    /*相同transaction，而且otoda测量不存在*/
                    if((ucTransIndxGnss == ucTransIndxOtdoa)
                       &&(PS_FALSE == pstOtdoaMeasRslt->bMeasRsltFlag))
                    {
                        pstOtdoaMeasRslt->bMeasRsltFlag = PS_TRUE;
                        pstOtdoaMeasRslt->usError = LRRC_LPP_OTDOA_UNABLE_TO_MEASURE_REF_CELL;

                    }
                }

                if(PS_TRUE == NAS_LPP_IsExsitPostionTechInSession(ulLoop, NAS_LPP_POSITION_TECH_ECID, &ucTransIndxEcid, &ucSessionTransIndxEcid))
                {
                    /*相同transaction，而且ecid测量不存在*/
                    if((ucTransIndxGnss == ucTransIndxEcid)
                       &&(PS_FALSE == pstEcidMeasRslt->bHaveEcidMeasFlag))
                    {
                        pstEcidMeasRslt->bHaveEcidMeasFlag = PS_TRUE;

                        /*设置小区计数为零的目的，表示上报的ECID的测量信息为空，发送上行消息的时候自然会设置原因值，当前请求的测量不可用*/
                        pstEcidMeasRslt->stLrrcLppEcidMeasResultList.ulCellNum = 0;
                    }
                }
...
VOS_VOID NAS_LPP_RcvLppEmmRelInd( LMM_LPP_START_IND_STRU *pRcvMsg )
{
    ...
    /*TMO网络跑LPP流程的时候，当收到网侧下发的locationReq之后，随即网侧又下发了链路释放，
      导致流程终止，针对这种场景做如下优化，当收到释放消息的时候，判断当前的running表是否
      有GNSS，如果有，则不释放session，如果有OTDOA，终止OTDOA的测量，同时将OTDOA测量结果的
      原因值设置为不能侧参考小区，如果有ECID，终止ECID的测量，同时将ECID的原因值设置为请求的
      测量不可用，如果当前session没有对应的GNSS的定位计数，则直接释放session即可。
    */
    /*设置当前的链路状态为IDLE*/
    NAS_LPP_SetLppConnStatus(NAS_LPP_CONN_STATUS_IDLE);

    NAS_LPP_RcvRelSessionHandle();

    /* 链路释放时，需要清除LRRC的请求标志数据，同时判定是否需要停止定时器 */
    if (NAS_LPP_GNSS_ORIG_TYPE_MODEM == NAS_LPP_GetGnssOrigType())
    {
        if (LRRC_LPP_LOC_REPORT_TYPE_ONCE == NAS_LPP_GetGnssReportType())
        {
            NAS_LPP_TimerStop( NAS_LPP_TIMER_INVAILD_PARA, NAS_LPP_TIMER_INVAILD_PARA,
                               TI_NAS_LPP_MODEM_ORIG_WAIT_GNSS_LOC_CNF );
        }
        NAS_LPP_SetGnssOrigType(NAS_LPP_GNSS_ORIG_TYPE_NW);
        NAS_LPP_SetGnssReportType(LRRC_LPP_LOC_REPORT_TYPE_BUTT);
    }
