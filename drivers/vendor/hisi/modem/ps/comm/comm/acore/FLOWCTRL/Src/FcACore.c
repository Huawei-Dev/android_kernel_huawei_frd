...
VOS_VOID FC_DrvAssemInit(VOS_VOID)
{
    ...
    PSACORE_MEM_CPY(&g_stCpuDriverAssePara, sizeof(FC_CPU_DRV_ASSEM_PARA_NV_STRU), &stCpuDrvAssemPara, sizeof(FC_CPU_DRV_ASSEM_PARA_NV_STRU));
}
...
VOS_VOID FC_DoJudgeDrvAssem(FC_CPU_DRV_ASSEM_PARA_STRU *pstDrvAssemPara)
{
    ...
    /* 保存当前档位信息 */
    PSACORE_MEM_CPY(&g_stDrvAssemParaEntity.stCurAssemPara, sizeof(FC_CPU_DRV_ASSEM_PARA_STRU), pstDrvAssemPara, sizeof(FC_CPU_DRV_ASSEM_PARA_STRU));
...
VOS_UINT32  FC_CPUA_Init( VOS_VOID )
{
    ...
    PSACORE_MEM_SET(&g_stFcBridgeRate, sizeof(FC_BRIDGE_RATE_STRU), 0, sizeof(FC_BRIDGE_RATE_STRU));
    PSACORE_MEM_SET(&g_stFcCpuACtrl, sizeof(g_stFcCpuACtrl), 0, sizeof(g_stFcCpuACtrl));
...
VOS_UINT32  FC_CDS_DelFcId( FC_ID_ENUM_UINT8 enFcId, MODEM_ID_ENUM_UINT16 enModemId )
{
    ...
    for (enShiftFcId = (FC_PRI_ENUM_UINT8)(enFcIdNum + 1); enShiftFcId < pstFcRabMappingInfoSet->enFcIdCnt; enShiftFcId++)
    {
        ...
        PSACORE_MEM_CPY((pstFcRabMappingInfo - 1), sizeof(FC_RAB_MAPPING_INFO_STRU), pstFcRabMappingInfo, sizeof(FC_RAB_MAPPING_INFO_STRU));
    }

    /* 最后一个映射关系无法被拷贝覆盖，所以需要手动清除 */
    PSACORE_MEM_SET(&(pstFcRabMappingInfoSet->astFcRabMappingInfo[enShiftFcId - 1]), sizeof(FC_RAB_MAPPING_INFO_STRU), 0, sizeof(FC_RAB_MAPPING_INFO_STRU));
...
{
    PSACORE_MEM_SET(g_astFcRabMappingInfoSet, sizeof(g_astFcRabMappingInfoSet), 0, sizeof(g_astFcRabMappingInfoSet));
