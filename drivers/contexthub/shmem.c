...
#define SHMEM_AP_RECV_PHY_ADDR (HISI_RESERVED_SENSORHUB_SHMEM_PHYMEM_BASE)
#define SHMEM_AP_RECV_PHY_SIZE (HISI_RESERVED_SENSORHUB_SHMEM_PHYMEM_SIZE/2)
#define SHMEM_AP_SEND_PHY_ADDR (SHMEM_AP_RECV_PHY_ADDR + SHMEM_AP_RECV_PHY_SIZE)
#define SHMEM_AP_SEND_PHY_SIZE (HISI_RESERVED_SENSORHUB_SHMEM_PHYMEM_SIZE/2)
...
static int shmem_recv(const pkt_header_t *head)
{
	...
	list_for_each_entry_safe(pos, next, &shmem_client_list, node)
		if (pos->module_id == msg->data.module_id) {
			...
			pos->notifier_call(shmem_gov.recv_addr, msg->data.buf_size);
			break;
		}

	if (pos->module_id != msg->data.module_id)
		pr_err("[%s]not find module_id[0x%0x]\n", __func__, msg->data.module_id);

	result = shmem_ipc_send(CMD_SHMEM_AP_RECV_RESP, pos->module_id, msg->data.buf_size);
	...
	void (*notifier_call)(void __iomem *buf_addr, unsigned int buf_size))
{
	...
	if (TAG_BEGIN > module_id || module_id >= TAG_END || NULL == notifier_call)
		return -EINVAL;

	mutex_lock(&shmem_recv_lock);

	list_for_each_entry_safe(pos, next, &shmem_client_list, node) {
		if (pos->module_id == module_id && notifier_call == pos->notifier_call) {
	...
	new_client = (struct shmem_client *)kzalloc(sizeof(struct shmem_client), GFP_KERNEL);
	if (!new_client) {
...
static int  shmem_recv_init(void)
{
	...
	shmem_gov.recv_addr = ioremap_wc(SHMEM_AP_RECV_PHY_ADDR, SHMEM_AP_RECV_PHY_SIZE);
	if (!shmem_gov.recv_addr) {
		unregister_mcu_event_notifier(TAG_SHAREMEM, CMD_SHMEM_AP_RECV_REQ, shmem_recv);
...
{
	int ret = register_mcu_event_notifier(TAG_SHAREMEM,
		CMD_SHMEM_AP_SEND_RESP, shmem_send_resp);
	if (ret) {
		pr_err("[%s] register_mcu_event_notifier err\n", __func__);
		return ret;
	}

	shmem_gov.send_addr = ioremap_wc(SHMEM_AP_SEND_PHY_ADDR, SHMEM_AP_SEND_PHY_SIZE);
